...
def embed_goal_graph_replay(main_goal, graph, encoder, score_net, chosen_goal, updown):
    ...
    representations_no_grad = representations.detach()

    initial_vecs = score_net(representations_no_grad).squeeze(-1).squeeze(-1)
...
def embed_goal_graph(main_goal, graph, encoder, score_net, updown):
    ...
    representations_no_grad = representations.detach()
    # representations_no_grad.required_grad = True
...
class GNNVanilla(Agent):
    ...
    def save(self):
        ...
        torch.save(self.score_net, "model_checkpoints/gnn_updown_vec_score_frozen")
        torch.save(self.updown, "model_checkpoints/gnn_updown_vec_updown_frozen")

        torch.save(self.tac_net, "model_checkpoints/gnn_up_down_vec_tac_frozen")
        torch.save(self.arg_net, "model_checkpoints/gnn_up_down_vec_arg_frozen")
        torch.save(self.term_net, "model_checkpoints/gnn_up_down_vec_term_frozen")
        torch.save(self.induct_gnn, "model_checkpoints/gnn_up_down_vec_induct_frozen")
        torch.save(self.encoder_premise, "model_checkpoints/gnn_up_down_vec_encoder_premise_e2e_frozen")
        torch.save(self.encoder_goal, "model_checkpoints/gnn_up_down_vec_encoder_goal_e2e_frozen")

        
    
    def load(self):
        self.score_net = torch.load("model_checkpoints/gnn_updown_vec_score_frozen")
        self.updown = torch.load("model_checkpoints/gnn_updown_vec_updown_frozen")

        self.tac_net = torch.load("model_checkpoints/gnn_up_down_vec_tac_frozen")
        self.arg_net = torch.load("model_checkpoints/gnn_up_down_vec_arg_frozen")
        self.term_net = torch.load("model_checkpoints/gnn_up_down_vec_term_frozen")
        self.induct_gnn = torch.load("model_checkpoints/gnn_up_down_vec_induct_frozen")

        self.encoder_premise = torch.load("model_checkpoints/gnn_up_down_vec_encoder_premise_e2e_frozen")
        self.encoder_goal = torch.load("model_checkpoints/gnn_up_down_vec_encoder_goal_e2e_frozen")
    ...
    def update_params(self, reward_pool, fringe_pool, arg_pool, tac_pool, step_count):
        ...
        # print ("params: \n\n")
        # print ([param.grad for param in self.score_net.parameters()])
        # print ([param.grad for param in self.tac_net.parameters()])
        # print ([param.grad for param in self.arg_net.parameters()])
        # print ([param.grad for param in self.encoder_goal.parameters()])
        # print ([param.grad for param in self.encoder_premise.parameters()])
        # print ([(name, param.grad) for name, param in self.updown.named_parameters()])

        # print (self.updown)
        # print ([param.grad for param in self.induct_gnn.parameters()])
        # print ([param.grad for param in self.term_net.parameters()])
    ...
    def save_replays(self):
        ...
        with open("old_replays/gnn_agent_up_down_vec_replays_frozen.pk", "wb") as f:
    ...
    def train(self):
        ...
        for iteration in range(self.num_iterations):
            ...
            if self.train_mode:
                ...
                with open(f"traces/graph_goal/gnn_updown_vec_proved_{date}_frozen.pk", "wb") as f:
...
def run_experiment():
    ...
    try:
        agent = GNNVanilla(tactic_pool, train_mode=True, replay_dir="old_replays/gnn_agent_up_down_vec_replays_frozen.pk")
