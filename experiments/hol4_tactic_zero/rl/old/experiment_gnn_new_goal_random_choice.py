...
def embed_goal_graph_replay(main_goal, graph, encoder, score_net, chosen_goal, updown):
    ...
    goal_probs = F.softmax(initial_vecs, dim=0)
...
# return best score without any other info
def embed_goal_graph_naive(main_goal, graph, encoder, score_net, updown):
    reverted = []
...
class GNNVanilla(Agent):
    def __init__(self, tactic_pool, replay_dir = None, train_mode = True):
        ...
        self.score_net = utp_model.ScorePolicy(1).to(self.device)
    ...
    def save(self):
        ...
        torch.save(self.score_net, "model_checkpoints/gnn_updown_vec_score_test")

        torch.save(self.updown, "model_checkpoints/gnn_updown_vec_updown_test")

        torch.save(self.tac_net, "model_checkpoints/gnn_up_down_vec_tac_test")
        torch.save(self.arg_net, "model_checkpoints/gnn_up_down_vec_arg_test")
        torch.save(self.term_net, "model_checkpoints/gnn_up_down_vec_term_test")
        torch.save(self.induct_gnn, "model_checkpoints/gnn_up_down_vec_induct_test")
        torch.save(self.encoder_premise, "model_checkpoints/gnn_up_down_vec_encoder_premise_e2e_test")
        torch.save(self.encoder_goal, "model_checkpoints/gnn_up_down_vec_encoder_goal_e2e_test")

        
    
    def load(self):
        self.score_net = torch.load("model_checkpoints/gnn_updown_vec_score_test")
        self.updown = torch.load("model_checkpoints/gnn_updown_vec_updown_test")

        self.tac_net = torch.load("model_checkpoints/gnn_up_down_vec_tac_test")
        self.arg_net = torch.load("model_checkpoints/gnn_up_down_vec_arg_test")
        self.term_net = torch.load("model_checkpoints/gnn_up_down_vec_term_test")
        self.induct_gnn = torch.load("model_checkpoints/gnn_up_down_vec_induct_test")

        self.encoder_premise = torch.load("model_checkpoints/gnn_up_down_vec_encoder_premise_e2e_test")
        self.encoder_goal = torch.load("model_checkpoints/gnn_up_down_vec_encoder_goal_e2e_test")
    ...
    def run(self, env, allowed_fact_batch, allowed_arguments_ids, candidate_args, max_steps=50):
        ...
        for t in range(max_steps):
            ...
            try:
                ...
                target_goal_node, target_representation, goal_prob = embed_goal_graph_naive(env.graph, env.current_goals, self.encoder_goal, self.score_net, self.updown)
    ...
    def save_replays(self):
        ...
        with open("old_replays/gnn_agent_up_down_vec_replays_test.pk", "wb") as f:
    ...
    def train(self):
        ...
        for iteration in range(self.num_iterations):
            ...
            if self.train_mode:
                ...
                with open(f"traces/graph_goal/gnn_updown_vec_proved_{date}_test.pk", "wb") as f:
...
def run_experiment():
    ...
    try:
        agent = GNNVanilla(tactic_pool, train_mode=True, replay_dir="old_replays/gnn_agent_up_down_vec_replays_test.pk")
