...
def gather_encoded_content_gnn(history, encoder):
    ...
    # graphs = [
    #     graph_db[t] if t in graph_db.keys() else ast_def.graph_to_torch_labelled(ast_def.goal_to_graph_labelled(t),
    #                                                                              token_enc=token_enc) for t
    #     in reverted]
...
class GNNVanilla:
    def __init__(self, tactic_pool, replay_dir=None, train_mode=True):
        ...
        self.induct_gnn = FormulaNetEdges(2020, self.embedding_dim, 1, global_pool=False,
    ...
    def train(self):
        ...
        for iteration in range(self.num_iterations):
            ...
            for goal in tqdm(self.goals):
                ...
                result = self.agent.run(env, allowed_fact_batch, allowed_arguments_ids, candidate_args, max_steps=5)
                ...
                if len(result) == 2:
                    ...
                    if replay_flag:
                        try:
                            ...
                            continue
                        try:
                            self.agent.replay_known_proof(env, allowed_fact_batch, allowed_arguments_ids,
                                                          candidate_args)
                        except Exception as e:
