...
def embed_goal_graph_replay(main_goal, graph, encoder, score_net, chosen_goal):
    reverted = []
...
def embed_goal_graph(main_goal, graph, encoder, score_net):
    ...
    try:
        ...
        # print (f"goal_probs {goal_probs.shape}")
        goal_m = Categorical(goal_probs)
...
class GNNVanilla(Agent):
    ...
    def run(self, env, allowed_fact_batch, allowed_arguments_ids, candidate_args, max_steps=50):
        ...
        for t in range(max_steps):
            ...
            # todo need target representation as well, so maybe add representation attribute to GoalNode class
            ...
            if done == True:
                ...
                if env.goal in self.replays.keys():
                    ...
                    # self.replays[env.goal].append((env.history, env.action_history, reward_pool, (fringe_pool.cpu().detach().numpy(), tac_pool.cpu().detach().numpy(), arg_pool.cpu().detach().numpy())))
                    ...
                    if env.history is not None:
                        # self.replays[env.goal] = [(env.history, env.action_history, reward_pool, (fringe_pool.cpu().detach().numpy(), tac_pool.cpu().detach().numpy(), arg_pool.cpu().detach().numpy()))]
    ...
    def save_replays(self):
        ...
        with open("old_replays/gnn_agent_new_goal_replays.pk", "wb") as f:
...
def run_experiment():
    ...
    try:
        agent = GNNVanilla(tactic_pool, train_mode=True, replay_dir="old_replays/gnn_agent_new_goal_replays.pk")
