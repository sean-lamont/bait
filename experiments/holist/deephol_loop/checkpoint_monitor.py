...
def get_latest_checkpoint(dirname: str):
    ...
    if not os.path.exists(chkpt_export_folder):
        logging.info('Eval export folder %s does not exist', chkpt_export_folder)
        return None
    num_lines = 0
    with open(chkpt_file) as f:
        for l in f:
            num_lines += 1
            if l.startswith('model_checkpoint_path:'):
                ckpt_prefix = l.strip().split()[1][1:-1]
                return ckpt_prefix
                # return os.path.basename(ckpt_prefix)
    return None
    ...
    if os.path.exists(chkpt_file):
        ...
        with open(chkpt_file) as f:
            lines = [
                ...
                if l.startswith('all_model_checkpoint_paths:')
            ]
    else:
...
class CheckpointMonitor(object):
    ...
    def copy_latest_checkpoint(self):
        ...
        files = glob.glob(os.path.join(self.model_directory, chkpt) + '.*')
