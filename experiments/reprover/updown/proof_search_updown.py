...
from updown.search_tree_updown import *
...
class BestFirstSearchProver:
    ...
    def _get_best_goal(self):
        ...
        for goal, node in self.nodes.items():
            ...
            if node.context and len(node.context[0]) > 1:
...
class CpuProver(BestFirstSearchProver):
    ...
    def __init__(
        ...
        goal_model = SimpleGoalModel.load(goal_path, device=torch.device("cpu"), freeze=True)
