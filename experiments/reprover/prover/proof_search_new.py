...
class BestFirstSearchProver:
    ...
    def _run_tactic(self, node: InternalNode, tactic: str, logprob: float) -> Edge:
        ...
        if type(response) in (
            ...
            # ensure that the selected goal was actually worked on (i.e. no additional rotates etc. in sampled tactic, no self cycles)
            ...
            # no new goals, and previous goal not present, so selected goal is proven
                ...
                if response.num_goals >= node.state.num_goals:
                    logger.info(f'edge case: {new_goals}, prev_goals: {prev_goals}, response: {response}, state: {node.state}')
                    response = TreeError(f'edge case 1: {new_goals}, prev_goals: {prev_goals}, response: {response}, state: {node.state}')
                    result_node = ErrorNode(response)
                elif not all([g in prev_goals for g in response_goals]):
                    logger.info(f'edge case 2: {new_goals}, prev_goals: {prev_goals}, response: {response}, state: {node.state}')
                    response = TreeError(f'edge case 2: {new_goals}, prev_goals: {prev_goals}, response: {response}, state: {node.state}')
                    result_node = ErrorNode(response)
                elif response.num_goals != node.state.num_goals - 1:
                    logger.info(f'edge case 3: {new_goals}, prev_goals: {prev_goals}, response: {response}, state: {node.state}')
                    response = TreeError(f'edge case 3: {new_goals}, prev_goals: {prev_goals}, response: {response}, state: {node.state}')
                    result_node = ErrorNode(response)
                else:
                    result_node = ProofFinishedNode(GoalFinished())
        ...
        edge = Edge(tactic=tactic, src=node, dst=result)
