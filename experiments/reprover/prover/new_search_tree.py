...
class InternalNode(Node):
    ...
    # only field compared for hashing is the goal
    ...
    def _recompute_visit_count(self):
        """
        Recursively update the visit_count of the current node and its ancestors.
        """

        if self.out_edges:
            self.visit_count = sum([edge.visit_count() for edge in self.out_edges])
        else:
            self.visit_count = 0

        for edge in self.in_edges:
            edge.src._recompute_visit_count()
...
class Edge:
    ...
    def visit_count(self) -> float:
        return 1 + sum([d.visit_count for d in self.dst])
