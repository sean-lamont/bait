...
class SoftGoalModel(pl.LightningModule):
    ...
    # todo model which uses embeddings
    ...
    def forward(
        ...
        provable_prob = torch.nn.functional.softmax(torch.stack([provable_logit, unprovable_logit], dim=1), dim=1)[:, 0]

        # get the weighted sum over buckets as the final score
        loss = self.mseloss(provable_prob, soft_targets)
