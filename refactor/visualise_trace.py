...
from refactor.proof_node import InternalNode, ProofFinishedNode, Status
...
def render_bestfs(trace, node_map, rev_map, step=0):
    siblings = []
            ...
            for s in dst:
                if s not in processed_nodes:
                    ...
                    data = {'id': str(s), 'goal': goal,
                            ...
                            'label': str(s),
                            'cumulative_logprob': str(trace.nodes[rev_map[s]].cumulative_logprob)
                            }
