...
def rank_edges(goal, edges):
    ...
    w_l = [{'goal': goal, 'winner': w.tactic, 'winner_prob': w.logprob, 'loser': l.tactic, 'loser_prob': l.logprob,
    ...
    w_l.extend([{'goal': goal, 'winner': w.tactic, 'winner_prob': w.logprob, 'loser': l.tactic, 'loser_prob': l.logprob,
    ...
    w_l.extend(
        [{'goal': goal, 'winner': ranked_proofs[i].tactic,
          'winner_prob': ranked_proofs[i].logprob, 'loser': ranked_proofs[j].tactic,
          'loser_prob': ranked_proofs[j].logprob,
            ...
            if set(edge_1_outcome) == set(edge_2_outcome):
                if edge.time < edge_2.time:
                    same_outcome_ranks.append(
                        {'goal': goal, 'winner': edge.tactic, 'winner_prob': edge.logprob, 'loser': edge_2.tactic,
                         'loser_prob': edge_2.logprob, 'type': 'same_outcome'})
                else:
                    same_outcome_ranks.append(
                        {'goal': goal, 'winner': edge_2.tactic, 'winner_prob': edge_2.logprob, 'loser': edge.tactic,
                         'loser_prob': edge.logprob, 'type': 'same_outcome'})
...
if __name__ == '__main__':
    ...
    traces = get_traces('../experiments/runs/eval_loop/leandojo_eval_2023_11_10/14_09_52/traces/*')
