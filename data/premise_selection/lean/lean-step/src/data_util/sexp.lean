...
notation `PRINT_TACTIC_STATE` := λ ts, (format.to_string ∘ format.flatten ∘ sexp.to_format) <$> tactic_state.to_sexp ts

notation `TO_EXPR` := λ ts, sexp.to_format <$> sexp_of_expr SEXP_DEPTH_LIMIT ts
