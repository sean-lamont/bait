...
let UNIONS_IMAGE = prove
 (`!f s. UNIONS (IMAGE f s) = {y | ?x. x IN s /\ y IN f x}`,
  REPEAT GEN_TAC THEN  GEN_REWRITE_TAC "sets.ml:I" I [EXTENSION] THEN
...
let UNIONS_GSPEC = prove
 (`(!P f. UNIONS {f x | P x} = {a | ?x. P x /\ a IN (f x)}) /\
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "sets.ml:I" I [EXTENSION] THEN
...
let UNIONS_INTERS = prove
 (`!s. UNIONS s = UNIV DIFF (INTERS {UNIV DIFF t | t IN s})`,
  GEN_TAC THEN GEN_REWRITE_TAC "sets.ml:I" I [EXTENSION] THEN
 ...
 (`!(s:A->bool) x. FINITE (s DELETE x) <=> FINITE s`,
  ...
  ASM_CASES_TAC `x:A IN s` THENL
   [SUBGOAL_THEN `s = x INSERT (s DELETE x:A)`
    (fun th -> GEN_REWRITE_TAC "sets.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [th]) THEN
...
let FINITE_FINITE_PREIMAGE_GENERAL = prove
 (`!f:A->B s t.
  ...
  SUBST1_TAC THENL
   [GEN_REWRITE_TAC "sets.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_ELIM_THM; IN_UNIONS] THEN
...
let UNIONS_MAXIMAL_SETS = prove
 (`!f. FINITE f
  ...
  GEN_REWRITE_TAC "sets.ml:I" I [TAUT `p <=> ~ ~ p`] THEN DISCH_TAC THEN
...
let FINITE_SUBSET_UNIONS = prove
 (`!f s:A->bool.
  ...
  GEN_REWRITE_TAC "sets.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
 ...
 (`!(f:A->B->B) b.
  ...
  REPEAT GEN_TAC THEN DISCH_TAC THEN INDUCT_TAC THENL
   [REWRITE_TAC[FINREC_1_LEMMA] THEN REWRITE_TAC[FINREC] THEN
    ...
    REPEAT GEN_TAC THEN
    GEN_REWRITE_TAC "sets.ml:LAND_CONV" LAND_CONV [FINREC] THEN
 ...
 (`!P (R:A->B->C->bool).
  ...
  REPEAT STRIP_TAC THEN BETA_TAC THEN EQ_TAC THENL
   [STRIP_TAC THEN MATCH_MP_TAC SELECT_UNIQUE THEN ASM_MESON_TAC[];
    DISCH_THEN(SUBST1_TAC o SYM) THEN CONV_TAC "sets.ml:SELECT_CONV" SELECT_CONV THEN
    ASM_MESON_TAC[]]);;
 ...
 (`!(f:A->B->B) b.
  ...
  EXISTS_TAC `g:(A->bool)->B` THEN CONJ_TAC THENL
   [SUBGOAL_THEN `FINITE(EMPTY:A->bool)`
    (ANTE_RES_THEN (fun th -> GEN_REWRITE_TAC "sets.ml:I" I [GSYM th])) THENL
     [REWRITE_TAC[FINITE_RULES];
...
let FINITE_RECURSION = prove
 (`!(f:A->B->B) b.
  ...
  CONV_TAC "sets.ml:SELECT_CONV" SELECT_CONV THEN MATCH_MP_TAC SET_RECURSION_LEMMA THEN
  ASM_REWRITE_TAC[]);;
 ...
 (`!x:A s. FINITE(s)
  ...
  REPEAT STRIP_TAC THEN COND_CASES_TAC THENL
   [SUBGOAL_THEN `s = x:A INSERT (s DELETE x)`
     (fun th -> GEN_REWRITE_TAC "sets.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [th])
...
let HAS_SIZE_UNIONS = prove
 (`!s t:A->B->bool m n.
  ...
  GEN_REWRITE_TAC "sets.ml:(funpow 4 BINDER_CONV o funpow 2 LAND_CONV)" (funpow 4 BINDER_CONV o funpow 2 LAND_CONV) [HAS_SIZE] THEN
...
let CARD_LE_UNIONS_CHAIN = prove
 (`!(f:(A->bool)->bool) n.
  ...
  GEN_REWRITE_TAC "sets.ml:I" I [GSYM CONTRAPOS_THM] THEN
...
let CROSS_INTER = prove
 (`(!s t u. s CROSS (t INTER u) = (s CROSS t) INTER (s CROSS u)) /\
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "sets.ml:TAUT" TAUT);;
 ...
 (`!d n t:B->bool m s:A->bool.
  ...
  REWRITE_TAC[HAS_SIZE_CLAUSES] THENL
   [REPEAT STRIP_TAC THEN ASM_REWRITE_TAC[NOT_IN_EMPTY; EXP] THEN
    CONV_TAC "sets.ml:HAS_SIZE_CONV" HAS_SIZE_CONV THEN EXISTS_TAC `(\x. d):A->B` THEN
  ...
  CONV_TAC "sets.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let HAS_SIZE_BOOL = prove
 (`(:bool) HAS_SIZE 2`,
  SUBGOAL_THEN `(:bool) = {F,T}` SUBST1_TAC THENL
   [REWRITE_TAC[EXTENSION; IN_UNIV; IN_INSERT] THEN CONV_TAC "sets.ml:TAUT" TAUT;
 ...
 (`!(s:A->bool) n. s HAS_SIZE n ==> {t | t SUBSET s} HAS_SIZE (2 EXP n)`,
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[EXTENSION; IN_ELIM_THM; IN_UNIV; SUBSET; IN; CONTRAPOS_THM];
    ...
    CONV_TAC "sets.ml:HAS_SIZE_CONV" HAS_SIZE_CONV THEN MAP_EVERY EXISTS_TAC [`T`; `F`] THEN
 ...
 (`(!a. ~FINITE {x:real | a < x}) /\
  ...
  ASSUME_TAC THENL
   [REPEAT GEN_TAC THEN REWRITE_TAC[GSYM REAL_NOT_LT] THEN
    ...
    MATCH_MP_TAC EQ_IMP THEN AP_TERM_TAC THEN CONV_TAC "sets.ml:SYM_CONV" SYM_CONV THEN
...
let HAS_SIZE_INDEX = prove
 (`!s n. s HAS_SIZE n
  ...
  ASM_REWRITE_TAC[IN_DELETE] THEN
  CONV_TAC "sets.ml:(ONCE_DEPTH_CONV COND_ELIM_CONV)" (ONCE_DEPTH_CONV COND_ELIM_CONV) THEN
 ...
 (`!s:num->bool.
  ...
  SUBGOAL_THEN `!n:num. ?x. n <= x /\ x IN s` MP_TAC THENL
   [ASM_MESON_TAC[INFINITE; num_FINITE; LT_IMP_LE; NOT_LE];
    GEN_REWRITE_TAC "sets.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [num_WOP]] THEN
...
let LIST_OF_SET_PROPERTIES = prove
 (`!s:A->bool. FINITE(s)
  ...
  CONV_TAC "sets.ml:(BINDER_CONV(RAND_CONV SELECT_CONV))" (BINDER_CONV(RAND_CONV SELECT_CONV)) THEN
...
let MEM_LIST_OF_SET = prove
 (`!s:A->bool. FINITE(s) ==> !x. MEM x (list_of_set s) <=> x IN s`,
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "sets.ml:(BINDER_CONV o funpow 2 RAND_CONV)" (BINDER_CONV o funpow 2 RAND_CONV)
    [GSYM th]) THEN
...
let INTER_UNIONS_PAIRWISE_DISJOINT = prove
 (`!s t:(A->bool)->bool.
  ...
  GEN_REWRITE_TAC "sets.ml:RAND_CONV" RAND_CONV [EXTENSION] THEN
...
let SUP = prove
 (`!s. ~(s = {}) /\ (?b. !x. x IN s ==> x <= b)
  ...
  REWRITE_TAC[sup] THEN CONV_TAC "sets.ml:(ONCE_DEPTH_CONV SELECT_CONV)" (ONCE_DEPTH_CONV SELECT_CONV) THEN
...
let SUP_UNIQUE = prove
 (`!s b. (!c. (!x. x IN s ==> x <= c) <=> b <= c) ==> sup s = b`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "sets.ml:RAND_CONV" RAND_CONV [GSYM SUP_SING] THEN
...
let INF = prove
 (`!s. ~(s = {}) /\ (?b. !x. x IN s ==> b <= x)
  ...
  CONV_TAC "sets.ml:(ONCE_DEPTH_CONV SELECT_CONV)" (ONCE_DEPTH_CONV SELECT_CONV) THEN
...
let INF_UNIQUE = prove
 (`!s b. (!c. (!x. x IN s ==> c <= x) <=> c <= b) ==> inf s = b`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "sets.ml:RAND_CONV" RAND_CONV [GSYM INF_SING] THEN
