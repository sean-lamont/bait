...
let ISQRT_WORKS = prove
 (`!n. ISQRT(n) EXP 2 <= n /\ n < (ISQRT(n) + 1) EXP 2`,
  GEN_TAC THEN REWRITE_TAC[ISQRT] THEN CONV_TAC "100/bertrand.ml:SELECT_CONV" SELECT_CONV THEN
...
let LN_2_COMPOSITION = prove
 (`ln(&2) =
  ...
  CONV_TAC "100/bertrand.ml:(GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 4 [GSYM LN_POW; GSYM LN_MUL; GSYM LN_DIV; REAL_POW_LT; real_div; REAL_LT_ADD; REAL_LT_MUL; REAL_LT_INV_EQ; REAL_OF_NUM_LT; ARITH])" (GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 4 [GSYM LN_POW; GSYM LN_MUL; GSYM LN_DIV; REAL_POW_LT; real_div; REAL_LT_ADD; REAL_LT_MUL; REAL_LT_INV_EQ; REAL_OF_NUM_LT; ARITH]) THEN
  AP_TERM_TAC THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let LN_N_CONV =
  let pth = prove
   (`x = (&1 + inv(&8)) pow n * (x / (&1 + inv(&8)) pow n)`,
    CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    CONV_TAC "100/bertrand.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_DIV_LMUL THEN
    MATCH_MP_TAC REAL_POW_NZ THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV)
  and qth = prove
   (`&0 < x
...
let FLOOR_DOUBLE_NUM = prove
 (`!n r d.
  ...
  CONV_TAC "100/bertrand.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[GSYM FLOOR_UNIQUE] THEN
...
let LN_FACT_BOUNDS = prove
 (`!n. ~(n = 0) ==> abs(ln(&(FACT n)) - (&n * ln(&n) - &n)) <= &1 + ln(&n)`,
  ...
  CONV_TAC "100/bertrand.ml:(LAND_CONV(BINDER_CONV(RAND_CONV(RAND_CONV(LAND_CONV (LAND_CONV num_CONV))))))" (LAND_CONV(BINDER_CONV(RAND_CONV(RAND_CONV(LAND_CONV (LAND_CONV num_CONV)))))) THEN
  ...
  ASSUME_TAC THENL
   [GEN_TAC THEN
    GEN_REWRITE_TAC "100/bertrand.ml:(LAND_CONV o funpow 3 RAND_CONV)" (LAND_CONV o funpow 3 RAND_CONV)
     [GSYM REAL_OF_NUM_ADD] THEN
    MATCH_MP_TAC(REAL_ARITH
     `abs(x - (y - z)) <= a ==> abs(x - (y - (z + &1))) <= &1 + a`) THEN
    FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "100/bertrand.ml:(LAND_CONV o RAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV o RAND_CONV) [GSYM th]) THEN
    SUBGOAL_THEN
     `sum(1,n + 1) (\i. ln (&i)) = sum(1,n) (\i. ln(&(i + 1)))`
    SUBST1_TAC THENL
     [GEN_REWRITE_TAC "100/bertrand.ml:RAND_CONV" RAND_CONV [SUM_DIFF] THEN
      REWRITE_TAC[SUM_1; ADD_CLAUSES; LN_1; REAL_SUB_RZERO] THEN
      GEN_REWRITE_TAC "100/bertrand.ml:(funpow 3 LAND_CONV)" (funpow 3 LAND_CONV) [SYM(NUM_REDUCE_CONV `0 + 1`)] THEN
...
let PRIMEPOW_GE_2 = prove
 (`!q. primepow q ==> 2 <= q`,
  ...
  ASM_SIMP_TAC[PRIME_GE_2] THEN GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [GSYM EXP_1] THEN
...
let APRIMEDIVISOR = prove
 (`!n. ~(n = 1) ==> prime(aprimedivisor n) /\ (aprimedivisor n) divides n`,
  ...
  CONV_TAC "100/bertrand.ml:SELECT_CONV" SELECT_CONV THEN ASM_SIMP_TAC[PRIME_FACTOR]);;
...
let LN_PRIMEFACT = prove
 (`!n. ~(n = 0)
  ...
  DISCH_THEN(X_CHOOSE_THEN `p:num` MP_TAC) THEN
  DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
  GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [divides] THEN
  ...
  MP_TAC THENL
   [MP_TAC(SPECL [`p:num`; `p * m:num`] BIG_POWER_LEMMA) THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "100/bertrand.ml:I" I [TAUT `a ==> b ==> c <=> b /\ a ==> c`] THEN
  GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [num_MAX] THEN
   ...
   [ONCE_REWRITE_TAC[SUM_DIFF] THEN
    ...
    SUBGOAL_THEN `1 + p * m = (1 + m) + (p * m - m)` SUBST1_TAC THENL
     [MATCH_MP_TAC(ARITH_RULE
      ...
      ALL_TAC] THEN
    GEN_REWRITE_TAC "100/bertrand.ml:RAND_CONV" RAND_CONV [GSYM SUM_TWO] THEN
   ...
   [MATCH_MP_TAC SUM_EQ THEN
    ...
    DISCH_THEN SUBST_ALL_TAC THEN ASM_CASES_TAC `q = p:num` THENL
     [FIRST_X_ASSUM SUBST_ALL_TAC THEN
      ...
      FIRST_ASSUM((fun th -> GEN_REWRITE_TAC "100/bertrand.ml:(funpow 2 LAND_CONV o RAND_CONV)" (funpow 2 LAND_CONV o RAND_CONV)
                  [th]) o MATCH_MP
...
let MANGOLDT = prove
 (`!n. ln(&(FACT n)) = sum(1,n) (\d. mangoldt(d) * floor(&n / &d))`,
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SUM_EQ THEN X_GEN_TAC `d:num` THEN STRIP_TAC THEN
    ...
    FIRST_ASSUM(MP_TAC o MATCH_MP (ARITH_RULE
     `d < n + 1 ==> (n = d + (n - d))`)) THEN
    DISCH_THEN(fun th ->
     GEN_REWRITE_TAC "100/bertrand.ml:(RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV) [th]) THEN
  ...
  FIRST_ASSUM(MP_TAC o MATCH_MP (ARITH_RULE
    `1 <= d ==> (d = 1 + (d - 1))`)) THEN
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/bertrand.ml:(funpow 2 LAND_CONV o RAND_CONV)" (funpow 2 LAND_CONV o RAND_CONV) [th]) THEN
  ...
  GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [LE_EXISTS] THEN
 ...
 (`!n. ln(&(FACT(n))) - &2 * ln(&(FACT(n DIV 2))) <= psi(n) /\
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[GSYM SUM_CMUL; GSYM SUM_SUB] THEN
    ...
    GEN_REWRITE_TAC "100/bertrand.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
  ...
  COND_CASES_TAC THENL
   [MATCH_MP_TAC REAL_LE_MUL THEN REWRITE_TAC[MANGOLDT_POS] THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [REAL_ARITH `a = a * &1`] THEN
  MATCH_MP_TAC REAL_LE_LMUL THEN REWRITE_TAC[MANGOLDT_POS] THEN
 ...
 (`!n. abs((ln(&(FACT(n))) - &2 * ln(&(FACT(n DIV 2)))) - &n * ln(&2))
  ...
  REPEAT GEN_TAC THEN ASM_CASES_TAC `n = 0` THENL
   [ASM_SIMP_TAC[FACT; MULT_CLAUSES; LN_1; DIV_0; ARITH_EQ] THEN
    REWRITE_TAC[REAL_MUL_LZERO] THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    ALL_TAC] THEN
  ...
  ASM_CASES_TAC `q = 0` THENL
   [UNDISCH_TAC `~(q * 2 + r = 0)` THEN
    ...
    CONV_TAC "100/bertrand.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[LN_1] THEN
...
let PSI_UBOUND_128 = prove
 (`!n. n <= 128 ==> psi(n) <= &133 / &128 * &n`,
  ...
  and tac = time (CONV_TAC "100/bertrand.ml:(LAND_CONV LN_N2_CONV THENC REALCALC_REL_CONV)" (LAND_CONV LN_N2_CONV THENC REALCALC_REL_CONV)) in
  REWRITE_TAC[ARITH_RULE `n <= 128 <=> n < 129`] THEN
  CONV_TAC "100/bertrand.ml:EXPAND_CASES_CONV" EXPAND_CASES_CONV THEN REWRITE_TAC(PSI_LIST_300) THEN
  REWRITE_TAC[LN_1] THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  REPEAT
   ((MATCH_MP_TAC lemma THEN
     CONV_TAC "100/bertrand.ml:(LAND_CONV REAL_RAT_REDUCE_CONV)" (LAND_CONV REAL_RAT_REDUCE_CONV) THEN
     GEN_REWRITE_TAC "100/bertrand.ml:I" I [TAUT `T /\ a <=> a`])
    ORELSE
...
let PSI_UBOUND_128_LOG = prove
 (`!n. n <= 128 ==> psi(n) <= (&3 / &2 * ln(&2)) * &n`,
  ...
  CONV_TAC "100/bertrand.ml:(ONCE_DEPTH_CONV LN_N2_CONV THENC REALCALC_REL_CONV)" (ONCE_DEPTH_CONV LN_N2_CONV THENC REALCALC_REL_CONV));;
...
let OVERPOWER_LEMMA = prove
 (`!f g d a.
  ...
  MP_TAC(SPECL [`\x:real. g(x) - f(x)`; `d:real->real`; `a:real`; `x:real`]
               MVT_ALT) THEN
  UNDISCH_TAC `a <= x` THEN GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [REAL_LE_LT] THEN
  ...
  prove(tm,
    ...
    ASM_CASES_TAC (mk_comb(`(<=) (n:num)`,mk_numeral m)) THENL
     [FIRST_ASSUM(MP_TAC o MATCH_MP th) THEN
      MATCH_MP_TAC(REAL_ARITH `a <= b ==> x <= a ==> x <= b`) THEN
      MATCH_MP_TAC REAL_LE_RMUL THEN REWRITE_TAC[REAL_POS] THEN
      MATCH_MP_TAC REAL_LE_RMUL THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
      MATCH_MP_TAC LN_POS THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
      ALL_TAC] THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LE_LMUL THEN CONJ_TAC THENL
       [MATCH_MP_TAC REAL_LE_MUL THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
        MATCH_MP_TAC LN_POS THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
        ALL_TAC] THEN
      ...
      ALL_TAC] THEN
    GEN_REWRITE_TAC "100/bertrand.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [real_div] THEN
    MATCH_MP_TAC(REAL_ARITH
     `logtm <= ((c - a * b) * l2) * n
    ...
    FIRST_X_ASSUM(MP_TAC o MATCH_MP (ARITH_RULE
     `~(n <= b) ==> b + 1 <= n`)) THEN
    GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [GSYM REAL_OF_NUM_LE] THEN
    DISCH_THEN(MP_TAC o MATCH_MP (REAL_ARITH
     `a <= n ==> a - &1 <= n - &1`)) THEN
    ABBREV_TAC `x = &n - &1` THEN
    CONV_TAC "100/bertrand.ml:(LAND_CONV NUM_REDUCE_CONV THENC REAL_RAT_REDUCE_CONV)" (LAND_CONV NUM_REDUCE_CONV THENC REAL_RAT_REDUCE_CONV) THEN
    ...
    W(fun (asl,w) ->
        ...
        MP_TAC th) THEN
    GEN_REWRITE_TAC "100/bertrand.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
    ...
    CONJ_TAC THENL
     [CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[real_sub] THEN
      CONV_TAC "100/bertrand.ml:(ONCE_DEPTH_CONV LN_N2_CONV)" (ONCE_DEPTH_CONV LN_N2_CONV) THEN
      CONV_TAC "100/bertrand.ml:REALCALC_REL_CONV" REALCALC_REL_CONV;
      ALL_TAC] THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LE_INV2 THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
      POP_ASSUM MP_TAC THEN REAL_ARITH_TAC;
      ALL_TAC] THEN
    SIMP_TAC[REAL_LE_RDIV_EQ; REAL_LT_DIV; REAL_OF_NUM_LT; ARITH] THEN
    GEN_REWRITE_TAC "100/bertrand.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
    SIMP_TAC[GSYM REAL_LE_LDIV_EQ; REAL_LT_DIV; REAL_OF_NUM_LT; ARITH] THEN
    CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    CONV_TAC "100/bertrand.ml:(ONCE_DEPTH_CONV LN_N2_CONV)" (ONCE_DEPTH_CONV LN_N2_CONV) THEN CONV_TAC "100/bertrand.ml:REALCALC_REL_CONV" REALCALC_REL_CONV);;
 ...
 (`&4 * ln(&1 + &2 pow j) + &3 <= (d * ln(&2)) * (&1 + &2 pow j) /\
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC REAL_LE_INV2 THEN ASM_REWRITE_TAC[REAL_LE_LADD] THEN
    ...
    ALL_TAC] THEN
  SIMP_TAC[REAL_LE_RDIV_EQ; REAL_OF_NUM_LT; ARITH] THEN
  GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
...
let PSI_UBOUND_LOG = prove
 (`!n. psi(n) <= (&4407 / &2048 * ln (&2)) * &n`,
  ...
  CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  CONV_TAC "100/bertrand.ml:(ONCE_DEPTH_CONV LN_N2_CONV)" (ONCE_DEPTH_CONV LN_N2_CONV) THEN
  CONJ_TAC THEN CONV_TAC "100/bertrand.ml:REALCALC_REL_CONV" REALCALC_REL_CONV);;
 ...
 (`!n. 4 <= n ==> &3 / &5 * &n <= psi(n)`,
  let lemma = prove
   (`a <= b /\ b <= l /\ rest ==> a <= l /\ b <= l /\ rest`,
    MESON_TAC[REAL_LE_TRANS])
  and tac = time (CONV_TAC "100/bertrand.ml:(RAND_CONV LN_N2_CONV THENC REALCALC_REL_CONV)" (RAND_CONV LN_N2_CONV THENC REALCALC_REL_CONV)) in
  GEN_TAC THEN ASM_CASES_TAC `n < 300` THENL
   [POP_ASSUM MP_TAC THEN SPEC_TAC(`n:num`,`n:num`) THEN
    CONV_TAC "100/bertrand.ml:EXPAND_CASES_CONV" EXPAND_CASES_CONV THEN
    REWRITE_TAC(PSI_LIST_300) THEN
    REWRITE_TAC[LN_1; ARITH] THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    REPEAT
     ((MATCH_MP_TAC lemma THEN
       CONV_TAC "100/bertrand.ml:(LAND_CONV REAL_RAT_REDUCE_CONV)" (LAND_CONV REAL_RAT_REDUCE_CONV) THEN
       GEN_REWRITE_TAC "100/bertrand.ml:I" I [TAUT `T /\ a <=> a`])
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    MATCH_MP_TAC REAL_LE_RMUL THEN REWRITE_TAC[REAL_POS] THEN
    REWRITE_TAC[real_sub] THEN CONV_TAC "100/bertrand.ml:(ONCE_DEPTH_CONV LN_N2_CONV)" (ONCE_DEPTH_CONV LN_N2_CONV) THEN
    CONV_TAC "100/bertrand.ml:REALCALC_REL_CONV" REALCALC_REL_CONV] THEN
  ABBREV_TAC `x = &n - &1` THEN
  ...
  EXISTS_TAC `inv(&1 + &299)` THEN CONJ_TAC THENL
   [ALL_TAC; CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV] THEN
...
let PSI_THETA = prove
 (`!n. theta(n) + psi(ISQRT n) <= psi(n) /\
  ...
  GEN_TAC THEN
  GEN_REWRITE_TAC "100/bertrand.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [PSI_SPLIT] THEN
  GEN_REWRITE_TAC "100/bertrand.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [PSI_SPLIT] THEN
 ...
 (`!n. 5 <= n ==> &1 / &2 * &n <= theta(n)`,
   ...
   [MP_TAC(CONJUNCT2(SPEC `n:num` PSI_THETA)) THEN
    ...
    SUBGOAL_THEN `&(ISQRT n) pow 2 <= (&n * &1 / &30) pow 2` MP_TAC THENL
     [ALL_TAC;
      ...
      ASM_REWRITE_TAC[] THEN CONV_TAC "100/bertrand.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    ...
    GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_RID] THEN
  ...
  ASM_CASES_TAC `n < 413` THENL
   [UNDISCH_TAC `5 <= n` THEN UNDISCH_TAC `n < 413` THEN
    ...
    CONV_TAC "100/bertrand.ml:EXPAND_CASES_CONV" EXPAND_CASES_CONV THEN CONV_TAC "100/bertrand.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_OF_NUM_LE] THEN
 ...
 (`!n m k. (m + 2 * k = n)
  ...
  ASM_CASES_TAC `m = 1` THENL
   [DISCH_THEN(K ALL_TAC) THEN ASM_REWRITE_TAC[] THEN
    ...
    CONV_TAC "100/bertrand.ml:(ONCE_DEPTH_CONV PRIMEPOW_CONV)" (ONCE_DEPTH_CONV PRIMEPOW_CONV) THEN
    REWRITE_TAC[COND_ID] THEN CONV_TAC "100/bertrand.ml:SYM_CONV" SYM_CONV THEN
  ...
  FIRST_X_ASSUM(MP_TAC o SPEC `SUC k`) THEN ANTS_TAC THENL
   [POP_ASSUM_LIST(MP_TAC o end_itlist CONJ) THEN ARITH_TAC;
    ALL_TAC] THEN
  GEN_REWRITE_TAC "100/bertrand.ml:(LAND_CONV o funpow 2 RAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o funpow 2 RAND_CONV o TOP_DEPTH_CONV)
  ...
  GEN_REWRITE_TAC "100/bertrand.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [ADD_SYM] THEN
 ...
 (`!n. ln(&(FACT(n))) - &2 * ln(&(FACT(n DIV 2))) =
  ...
  SUBST1_TAC THENL
   [SUBGOAL_THEN `n = n DIV 2 + (n - n DIV 2)`
     (fun th -> GEN_REWRITE_TAC "100/bertrand.ml:(RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV) [th])
...
let PSI_EXPANSION_CUTOFF = prove
 (`!n m p. m <= p
  ...
  GEN_REWRITE_TAC "100/bertrand.ml:BINDER_CONV" BINDER_CONV [SWAP_FORALL_THM] THEN
...
let FACT_PSI_BOUND_ODD = prove
 (`!n k. ODD(k)
  ...
  DISCH_THEN(CONJUNCTS_THEN2
   (fun th -> GEN_REWRITE_TAC "100/bertrand.ml:(LAND_CONV o LAND_CONV o RAND_CONV)" (LAND_CONV o LAND_CONV o RAND_CONV) [th])
   MP_TAC) THEN
...
let FACT_PSI_BOUND_2_3 = prove
 (`!n. psi(n) - psi(n DIV 2)
  ...
  CONV_TAC "100/bertrand.ml:(ONCE_DEPTH_CONV REAL_SUM_CONV)" (ONCE_DEPTH_CONV REAL_SUM_CONV) THEN
 ...
 (`!n. n >= 1200 ==> &n / &6 <= psi(n) - psi(n DIV 2)`,
  ...
  EXISTS_TAC `&n / &6 + &n / &2` THEN CONJ_TAC THENL
   [REWRITE_TAC[REAL_LE_LADD] THEN MP_TAC(SPEC `n DIV 3` PSI_UBOUND_3_2) THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LE_LMUL THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
      SIMP_TAC[REAL_LE_RDIV_EQ; REAL_OF_NUM_LT; ARITH] THEN
      REWRITE_TAC[REAL_OF_NUM_MUL; REAL_OF_NUM_LE] THEN
      MP_TAC(SPECL [`n:num`; `3`] DIVISION) THEN ARITH_TAC;
      ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
      REWRITE_TAC[real_div; GSYM REAL_MUL_ASSOC] THEN
      CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_LE_REFL]];
    ALL_TAC] THEN
  ...
  EXISTS_TAC `&n * &1 / &38` THEN CONJ_TAC THENL
   [ALL_TAC;
    MATCH_MP_TAC REAL_LE_LMUL THEN REWRITE_TAC[REAL_POS] THEN
    SIMP_TAC[REAL_LE_SUB_LADD] THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    CONV_TAC "100/bertrand.ml:(RAND_CONV LN_N2_CONV)" (RAND_CONV LN_N2_CONV) THEN CONV_TAC "100/bertrand.ml:REALCALC_REL_CONV" REALCALC_REL_CONV] THEN
  SUBST1_TAC(REAL_ARITH `&n = &1 + (&n - &1)`) THEN
  FIRST_X_ASSUM(MP_TAC o MATCH_MP (ARITH_RULE
   `n >= b ==> b <= n:num`)) THEN
  GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [GSYM REAL_OF_NUM_LE] THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC REAL_LE_INV2 THEN CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    POP_ASSUM MP_TAC THEN REAL_ARITH_TAC;
    ALL_TAC] THEN
  CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let PII_UBOUND_CASES_50 = prove
 (`!n. n < 50 ==> 3 <= n ==> ln(&n) * pii(n) <= &5 * &n`,
  let tac = CONV_TAC "100/bertrand.ml:(ONCE_DEPTH_CONV LN_N2_CONV THENC REALCALC_REL_CONV)" (ONCE_DEPTH_CONV LN_N2_CONV THENC REALCALC_REL_CONV) in
  CONV_TAC "100/bertrand.ml:EXPAND_CASES_CONV" EXPAND_CASES_CONV THEN CONV_TAC "100/bertrand.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let PII_ISQRT_INDUCT = prove
 (`!n. 50 <= n
  ...
  GEN_REWRITE_TAC "100/bertrand.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
  SIMP_TAC[GSYM REAL_LE_LDIV_EQ; REAL_LT_DIV; REAL_OF_NUM_LT; ARITH] THEN
  GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [real_div] THEN
  GEN_REWRITE_TAC "100/bertrand.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
  ...
  CONV_TAC "100/bertrand.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
  SIMP_TAC[GSYM REAL_LE_RDIV_EQ; REAL_LT_DIV; REAL_OF_NUM_LT; ARITH] THEN
   ...
   [REWRITE_TAC[REAL_OF_NUM_LE] THEN
    SUBGOAL_THEN `7 EXP 2 < (ISQRT n + 1) EXP 2` MP_TAC THENL
     [MATCH_MP_TAC LET_TRANS THEN EXISTS_TAC `n:num` THEN
      REWRITE_TAC[ISQRT_WORKS] THEN CONV_TAC "100/bertrand.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
      UNDISCH_TAC `50 <= n` THEN ARITH_TAC;
      ALL_TAC] THEN
...
let PII_UBOUND_5 = prove
 (`!n. 3 <= n ==> pii(n) <= &5 * (&n / ln(&n))`,
  ...
  SIMP_TAC[GSYM real_div; REAL_LE_RDIV_EQ; LN_POS_LT; REAL_OF_NUM_LT;
           ARITH_RULE `3 <= n ==> 1 < n`] THEN
  GEN_REWRITE_TAC "100/bertrand.ml:(BINDER_CONV o RAND_CONV o LAND_CONV)" (BINDER_CONV o RAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
