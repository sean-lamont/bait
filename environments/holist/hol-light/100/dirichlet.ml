 ...
 (`!n. 4 <= n ==> log(&n + &1) / (&n + &1) <= log(&n) / &n`,
  ...
  REWRITE_TAC[REAL_ARITH `~(n + &1 <= x /\ x <= n)`] THEN ANTS_TAC THENL
   [X_GEN_TAC `w:complex` THEN STRIP_TAC THEN COMPLEX_DIFF_TAC THEN
    ...
    DISCH_TAC THEN UNDISCH_TAC `~(w = Cx(&0))` THEN CONV_TAC "100/dirichlet.ml:COMPLEX_FIELD" COMPLEX_FIELD;
    ...
    GEN_REWRITE_TAC "100/dirichlet.ml:LAND_CONV" LAND_CONV [GSYM LOG_EXP] THEN
 ...
 (`!a n. 2 <= n ==> ?z. z pow n = a /\ ~(z = Cx(&1))`,
  ...
  ASM_CASES_TAC  `a = Cx(&0)` THENL
   [EXISTS_TAC `Cx(&0)` THEN ASM_REWRITE_TAC[COMPLEX_POW_ZERO] THEN
    CONV_TAC "100/dirichlet.ml:COMPLEX_RING" COMPLEX_RING;
    ALL_TAC] THEN
...
let DIRICHLET_CHARACTER_EQ_1 = prove
 (`!d c. dirichlet_character d c ==> c(1) = Cx(&1)`,
  ...
  DISCH_THEN(MP_TAC o repeat (SPEC `1`)) THEN CONV_TAC "100/dirichlet.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
...
let DIRICHLET_CHARACTER_PERIODIC_GEN = prove
 (`!d c m n. dirichlet_character d c ==> c(m * d + n) = c(n)`,
  ...
  GEN_TAC THEN
  FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/dirichlet.ml:RAND_CONV" RAND_CONV [GSYM th]) THEN
 ...
 (`!c. dirichlet_character 1 c <=> !n. c n = Cx(&1)`,
  GEN_TAC THEN REWRITE_TAC[dirichlet_character; COPRIME_1] THEN EQ_TAC THENL
   [STRIP_TAC THEN
    ...
    CONV_TAC "100/dirichlet.ml:COMPLEX_RING" COMPLEX_RING;
    DISCH_TAC THEN ASM_REWRITE_TAC[] THEN CONV_TAC "100/dirichlet.ml:COMPLEX_RING" COMPLEX_RING]);;
...
let DIRICHLET_CHARACTER_NONPRINCIPAL_NONTRIVIAL = prove
 (`!d c. dirichlet_character d c /\ ~(c = chi_0 d)
  ...
  CONV_TAC "100/dirichlet.ml:TAUT" TAUT);;
...
let DIRICHLET_CHARACTER_ZEROSUM = prove
 (`!d c. dirichlet_character d c /\ ~(c = chi_0 d)
  ...
  REPEAT(CONJ_TAC THENL
   [CONV_TAC "100/dirichlet.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC VSUM_SUPERSET THEN
...
let DIRICHLET_CHARACTER_SUM_MOD = prove
 (`!d c. dirichlet_character d c /\ ~(c = chi_0 d)
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/dirichlet.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [th]) THEN
  ...
  MATCH_MP_TAC VSUM_EQ THEN REPEAT STRIP_TAC THEN REWRITE_TAC[] THEN
  MATCH_MP_TAC DIRICHLET_CHARACTER_CONG THEN EXISTS_TAC `d:num` THEN
  ASM_REWRITE_TAC[] THEN CONV_TAC "100/dirichlet.ml:NUMBER_RULE" NUMBER_RULE);;
...
let DIRICHLET_CHARACTER_SUM_OVER_NUMBERS = prove
 (`!d c. dirichlet_character d c
  ...
  GEN_REWRITE_TAC "100/dirichlet.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM ETA_AX] THEN
 ...
 (`!f h a d.
  ...
  SUBGOAL_THEN `!x s. x IN h ==> ((x EXP s) MOD d) IN h` ASSUME_TAC THENL
   [REWRITE_TAC[RIGHT_FORALL_IMP_THM] THEN GEN_TAC THEN DISCH_TAC THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "100/dirichlet.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
  ...
  ASSUME_TAC THENL
   [REPEAT STRIP_TAC THEN ASM_SIMP_TAC[DIVIDES_MOD; LE_1] THEN
    ...
    DISCH_THEN(fun th ->
      GEN_REWRITE_TAC "100/dirichlet.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [th]) THEN
   ...
   [REWRITE_TAC[MESON[] `(?g. !x a. p x ==> g(f a x) = h a x) <=>
    ...
    SUBGOAL_THEN `m divides i` MP_TAC THENL
     [FIRST_X_ASSUM MATCH_MP_TAC THEN
      ...
      CONV_TAC "100/dirichlet.ml:NUMBER_RULE" NUMBER_RULE;
      ALL_TAC] THEN
    ...
    EXISTS_TAC `f((y * (am EXP r) MOD d) MOD d):complex` THEN CONJ_TAC THENL
     [AP_TERM_TAC THEN CONV_TAC "100/dirichlet.ml:SYM_CONV" SYM_CONV THEN ASM_SIMP_TAC[CONG_MOD_LT] THEN
      MATCH_MP_TAC CONG_TRANS THEN
      ...
      REWRITE_TAC[EXP_ADD; EXP_EXP] THEN CONV_TAC "100/dirichlet.ml:NUMBER_RULE" NUMBER_RULE;
      ALL_TAC] THEN
  ...
  REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[SUBSET; IN_ELIM_THM; IN_INSERT; IN_UNIV] THEN
    ...
    DISCH_THEN(MP_TAC o SPECL [`1`; `1`]) THEN
    ASM_SIMP_TAC[MULT_CLAUSES; MOD_LT] THEN
    UNDISCH_TAC `~(z = Cx(&1))` THEN CONV_TAC "100/dirichlet.ml:COMPLEX_RING" COMPLEX_RING;
    ...
    GEN_REWRITE_TAC "100/dirichlet.ml:(LAND_CONV)" (LAND_CONV) [ARITH_RULE `1 = 1 * 1`] THEN
 ...
 (`!d n. 1 < d /\ ~((n == 1) (mod d))
  ...
  ASM_CASES_TAC `coprime(n,d)` THENL
   [ALL_TAC;
    ...
    CONV_TAC "100/dirichlet.ml:COMPLEX_RING" COMPLEX_RING] THEN
  ...
  REPEAT CONJ_TAC THEN X_GEN_TAC `x:num` THENL
   [REWRITE_TAC[NUMBER_RULE `coprime(x + d:num,d) <=> coprime(x,d)`] THEN
    ...
    ASM_SIMP_TAC[GSYM CONG; LE_1] THEN CONV_TAC "100/dirichlet.ml:NUMBER_RULE" NUMBER_RULE;
...
let BOUNDED_LFUNCTION_DIRICHLET_MANGOLDT_LEMMA = prove
 (`!d c. dirichlet_character d c /\ ~(c = chi_0 d)
  ...
  GEN_REWRITE_TAC "100/dirichlet.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_INV_INV] THEN
 ...
 (`!d. 1 <= d
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    ...
    EXISTS_TAC `sum(1..x) (\k. log(&p) / &p pow k)` THEN CONJ_TAC THENL
     [MATCH_MP_TAC SUM_SUBSET_SIMPLE THEN REWRITE_TAC[FINITE_NUMSEG] THEN
      ...
      GEN_REWRITE_TAC "100/dirichlet.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
      ...
      COND_CASES_TAC THEN CONV_TAC "100/dirichlet.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
   ...
   W(MP_TAC o PART_MATCH (lhs o rand) SUM_SUM_PRODUCT o rand o snd) THEN
    ...
    MATCH_MP_TAC REAL_EQ_IMP_LE THEN CONV_TAC "100/dirichlet.ml:SYM_CONV" SYM_CONV THEN
 ...
 (`!n t. &0 <= t
  ...
  SUBGOAL_THEN `2 * (n * (n + 1)) DIV 2 = n * (n + 1)` SUBST1_TAC THENL
   [SUBGOAL_THEN `EVEN(n * (n + 1))` MP_TAC THENL
     [REWRITE_TAC[EVEN_ADD; EVEN_MULT; ARITH_EVEN] THEN CONV_TAC "100/dirichlet.ml:TAUT" TAUT;
...
let DIVISORSUM_PRIMEPOW = prove
 (`!f p k. prime p
  ...
  GEN_REWRITE_TAC "100/dirichlet.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM o_DEF] THEN
 ...
 (`!d c. dirichlet_character d c /\ ~(c = chi_0 d) /\ (!n. real(c n))
  ...
  MP_TAC THENL
   [GEN_TAC THEN DISCH_TAC THEN ASM_CASES_TAC `z = Cx(&0)` THENL
    ...
    CONV_TAC "100/dirichlet.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM_REWRITE_TAC[] THEN
    ...
    MATCH_MP_TAC REAL_LE_INV2 THEN CONV_TAC "100/dirichlet.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    MATCH_MP_TAC(NORM_ARITH
     `norm(z) <= norm(w) - h ==> h <= norm(w - z)`) THEN
    REWRITE_TAC[COMPLEX_NORM_CX] THEN CONV_TAC "100/dirichlet.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
   ...
   [REPEAT STRIP_TAC THEN REWRITE_TAC[sums; FROM_INTER_NUMSEG] THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC ALWAYS_EVENTUALLY THEN X_GEN_TAC `x:num` THEN
      ...
      GEN_REWRITE_TAC "100/dirichlet.ml:RAND_CONV" RAND_CONV [REAL_ARITH `a * &x * b = &x * a * b`] THEN
      GEN_REWRITE_TAC "100/dirichlet.ml:(RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV)
      ...
      GEN_REWRITE_TAC "100/dirichlet.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
      ...
      CONJ_TAC THENL
       [REWRITE_TAC[COMPLEX_NORM_POW] THEN
        GEN_REWRITE_TAC "100/dirichlet.ml:RAND_CONV" RAND_CONV [GSYM REAL_POW_1] THEN
        MATCH_MP_TAC REAL_POW_MONO_INV THEN
  ...
  MP_TAC THENL
   [REWRITE_TAC[BOUNDED_POS] THEN ONCE_REWRITE_TAC[SIMPLE_IMAGE_GEN] THEN
    ...
    CONV_TAC "100/dirichlet.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN CONJ_TAC THENL
  ...
  THENL
   [ASM_SIMP_TAC[REAL_SUB_POW_L1; REAL_SUB_LE] THEN
    ...
    GEN_REWRITE_TAC "100/dirichlet.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
  ...
  GEN_REWRITE_TAC "100/dirichlet.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [REAL_MUL_SYM] THEN
 ...
 (`!d c. dirichlet_character d c /\ ~(c = chi_0 d)
  ...
  let lemma = prove
   (`{a,b,c} SUBSET s
    ...
    REPEAT STRIP_TAC THEN CONV_TAC "100/dirichlet.ml:SYM_CONV" SYM_CONV THEN
...
let DIRICHLET_MANGOLDT = prove
 (`!d k. 1 <= d /\ coprime(k,d)
  ...
  SUBGOAL_THEN `(l * n == 1) (mod d) <=> (n == k) (mod d)` SUBST1_TAC THENL
   [UNDISCH_TAC `(k * l == 1) (mod d)` THEN CONV_TAC "100/dirichlet.ml:NUMBER_RULE" NUMBER_RULE;
...
let DIRICHLET_MANGOLDT_EXPLICIT = prove
 (`!d k. 1 <= d /\ coprime (k,d)
  ...
  GEN_REWRITE_TAC "100/dirichlet.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM REAL_ABS_NUM] THEN
