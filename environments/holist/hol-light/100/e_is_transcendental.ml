...
let SOD_POLYDIFF_THEOREM = prove(
    ...
    (ONCE_REWRITE_TAC [SODN]) THEN
    (STRIP_TAC) THEN
    (CONV_TAC "100/e_is_transcendental.ml:(RAND_CONV (ONCE_REWRITE_CONV [NUMSEG_LENGTH_POLYDIFF_LEMMA]))" (RAND_CONV (ONCE_REWRITE_CONV [NUMSEG_LENGTH_POLYDIFF_LEMMA]))) THEN
    ...
    [ (SIMP_TAC [poly_diff]) THEN
      ...
      ;
       ...
       (SIMP_TAC [LENGTH;PRE]) THEN
       (CONV_TAC "100/e_is_transcendental.ml:(RAND_CONV (ONCE_REWRITE_CONV [SUC_ITERATE_POLYADD_LEMMA]))" (RAND_CONV (ONCE_REWRITE_CONV [SUC_ITERATE_POLYADD_LEMMA]))) THEN
       (SIMP_TAC [LENGTH;PRE]) THEN
...
let PLANETMATH_EQN_4 =  prove(
    `(~ (transcendental (exp (&1)))) ==> ? c .
     ...
     (CONV_TAC "100/e_is_transcendental.ml:((RAND_CONV o ABS_CONV o RATOR_CONV o RAND_CONV o RATOR_CONV) (PURE_ONCE_REWRITE_CONV [lem6]))" ((RAND_CONV o ABS_CONV o RATOR_CONV o RAND_CONV o RATOR_CONV) (PURE_ONCE_REWRITE_CONV [lem6]))) THEN
...
let JESSE_POW_LEMMA = prove(
     ...
     STRIP_TAC THEN STRIP_TAC THEN
     (CONV_TAC "100/e_is_transcendental.ml:(RATOR_CONV (ONCE_REWRITE_CONV [c0]))" (RATOR_CONV (ONCE_REWRITE_CONV [c0]))) THEN
     (SIMP_TAC [real_pow])
)
...
let POLY_MUL_ITER_HD_FACTORIAL = prove(
    ...
    [ (ONCE_REWRITE_TAC [Pm_eqn5.POLY_MUL_ITER]) THEN (SIMP_TAC [HD;FACT]) THEN REAL_ARITH_TAC;
      ...
      (CONV_TAC "100/e_is_transcendental.ml:(RATOR_CONV (ONCE_REWRITE_CONV [lem05]))" (RATOR_CONV (ONCE_REWRITE_CONV [lem05]))) THEN REAL_ARITH_TAC
    ]
)
...
let SATURDAY_LEMMA = prove(
    ...
    (REPEAT STRIP_TAC) THEN
    (CONV_TAC "100/e_is_transcendental.ml:(RATOR_CONV (ONCE_REWRITE_CONV [GSYM REAL_MUL_LID]))" (RATOR_CONV (ONCE_REWRITE_CONV [GSYM REAL_MUL_LID]))) THEN
...
let HAMMS_LEMMA = prove(
     ...
     let erica30 = UNDISCH (ONCE_REWRITE_RULE [erica28] (DISCH `n > 0` (erica21))) in
      (CONV_TAC "100/e_is_transcendental.ml:NNF_CONV" NNF_CONV) THEN
      (REPEAT STRIP_TAC) THEN
