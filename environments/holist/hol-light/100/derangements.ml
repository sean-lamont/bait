 ...
 (`!n s:A->bool t:B->bool.
  ...
  let lemma = prove
   (`{p | ?y. y IN t /\ A y p} = UNIONS {{p | A y p} | y IN t} /\
    ...
    CONJ_TAC THEN GEN_REWRITE_TAC "100/derangements.ml:I" I [EXTENSION] THEN
  ...
  INDUCT_TAC THEN REPEAT GEN_TAC THENL
   [REWRITE_TAC[HAS_SIZE_CLAUSES] THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "100/derangements.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [HAS_SIZE_CLAUSES] THEN
  ...
  MATCH_MP_TAC HAS_SIZE_UNIONS THEN REPEAT CONJ_TAC THENL
   [ASM_REWRITE_TAC[HAS_SIZE_SUC];
    ...
    GEN_REWRITE_TAC "100/derangements.ml:I" I [EXTENSION] THEN
...
let NUMBER_OF_DERANGEMENTS = prove
 (`!n s:A->bool. s HAS_SIZE n ==> {p | p deranges s} HAS_SIZE (derangements n)`,
  ...
  REPEAT CONJ_TAC THENL
   [CONV_TAC "100/derangements.ml:(ONCE_DEPTH_CONV HAS_SIZE_CONV)" (ONCE_DEPTH_CONV HAS_SIZE_CONV) THEN REPEAT STRIP_TAC THEN
     ...
     [REWRITE_TAC[IN_UNION; TAUT `a \/ b ==> c <=> (a ==> c) /\ (b ==> c)`;
      ...
      SUBGOAL_THEN `(p:A#A->bool)(x:A,z:A)` STRIP_ASSUME_TAC THENL
       [REPEAT(FIRST_X_ASSUM(SUBST1_TAC o SYM)) THEN
        CONV_TAC "100/derangements.ml:SELECT_CONV" SELECT_CONV THEN
        REPEAT(POP_ASSUM MP_TAC) THEN REL_TAC;
        ALL_TAC] THEN
    ...
    MATCH_MP_TAC HAS_SIZE_IMAGE_INJ THEN CONJ_TAC THENL
     [REWRITE_TAC[FORALL_PAIR_THM; IN_ELIM_THM; PAIR_BETA_THM; PAIR_EQ] THEN
      ...
      SUBGOAL_THEN `p(x:A,z:A) /\ p'(x:A,z':A)` STRIP_ASSUME_TAC THENL
       [REPEAT(FIRST_X_ASSUM(SUBST1_TAC o SYM)) THEN
        CONJ_TAC THEN CONV_TAC "100/derangements.ml:SELECT_CONV" SELECT_CONV THEN
        REPEAT(POP_ASSUM MP_TAC) THEN REL_TAC;
        ALL_TAC] THEN
...
let DERANGEMENTS = prove
 (`!n. ~(n = 0)
  ...
  CONV_TAC "100/derangements.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "100/derangements.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  X_GEN_TAC `n:num` THEN ASM_CASES_TAC `n = 0` THEN ASM_REWRITE_TAC[] THENL
   [ASM_REWRITE_TAC[derangements; ARITH; SUM_2; SUM_1] THEN
    CONV_TAC "100/derangements.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "100/derangements.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    ALL_TAC] THEN
  ...
  CONV_TAC "100/derangements.ml:REAL_FIELD" REAL_FIELD);;
 ...
 (`!n. ~(n = 0)
  ...
  REWRITE_TAC[real_div; GSYM REAL_EXP_NEG] THEN ASM_CASES_TAC `n < 5` THENL
   [FIRST_X_ASSUM(REPEAT_TCL DISJ_CASES_THEN SUBST_ALL_TAC o MATCH_MP
    ...
    REWRITE_TAC[SUM_CLAUSES_NUMSEG] THEN CONV_TAC "100/derangements.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    CONV_TAC "100/derangements.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    REWRITE_TAC[REAL_ARITH `abs x < a <=> --a < x /\ x < a`] THEN
    REWRITE_TAC[real_sub] THEN CONJ_TAC THEN CONV_TAC "100/derangements.ml:REALCALC_REL_CONV" REALCALC_REL_CONV;
    ALL_TAC] THEN
  ...
  MATCH_MP_TAC REAL_LTE_TRANS THEN EXISTS_TAC `&3` THEN CONJ_TAC THENL
   [CONV_TAC "100/derangements.ml:REALCALC_REL_CONV" REALCALC_REL_CONV; ALL_TAC] THEN
...
let ROUND_WORKS = prove
 (`!x. integer(round x) /\ round x - &1 / &2 <= x /\ x < round x + &1 / &2`,
  GEN_TAC THEN REWRITE_TAC[round] THEN CONV_TAC "100/derangements.ml:SELECT_CONV" SELECT_CONV THEN
