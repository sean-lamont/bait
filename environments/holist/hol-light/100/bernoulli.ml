...
let DIFF_BERNPOLY = prove
 (`!n x. ((bernpoly (SUC n)) diffl (&(SUC n) * bernpoly n x)) x`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "100/bernoulli.ml:(RATOR_CONV o LAND_CONV)" (RATOR_CONV o LAND_CONV) [GSYM ETA_AX] THEN
  REWRITE_TAC[bernpoly; SUM_CLAUSES_NUMSEG; LE_0] THEN
  GEN_REWRITE_TAC "100/bernoulli.ml:LAND_CONV" LAND_CONV [GSYM REAL_ADD_RID] THEN
  ...
  ABBREV_TAC `z = x pow (n - k)` THEN CONV_TAC "100/bernoulli.ml:REAL_FIELD" REAL_FIELD);;
 ...
 (`!n x. bernpoly n (x + &1) - bernpoly n x = &n * x pow (n - 1)`,
  ...
  MATCH_MP_TAC INTEGRALS_EQ THEN CONJ_TAC THENL
   [X_GEN_TAC `x:real` THEN FIRST_X_ASSUM(MP_TAC o SPEC `x:real`) THEN
    ...
    GEN_REWRITE_TAC "100/bernoulli.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_RID] THEN
  ...
  CONV_TAC "100/bernoulli.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[REAL_ENTIRE; REAL_POW_EQ_0] THEN
...
let BINOM_CONV =
  let pth = prove
   (`a * b * x = FACT c ==> x = (FACT c) DIV (a * b)`,
    REPEAT STRIP_TAC THEN CONV_TAC "100/bernoulli.ml:SYM_CONV" SYM_CONV THEN
