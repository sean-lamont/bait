...
let ODDPART_LE = prove
 (`!n. oddpart n <= n`,
  GEN_TAC THEN GEN_REWRITE_TAC "100/euler.ml:RAND_CONV" RAND_CONV [INDEX_ODDPART_DECOMPOSITION] THEN
...
let FINITE_PARTITIONS_LEMMA = prove
 (`!m n. FINITE {p | (!i. p(i) <= n) /\ !i. m <= i ==> p(i) = 0}`,
  ...
  CONV_TAC "100/euler.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN X_GEN_TAC `p:num->num` THEN
  EQ_TAC THENL
...
let SUPPORT_DISTINCT_OF_ODD = prove
 (`!p. (!i. p(i) * i <= n) /\
  ...
  GEN_REWRITE_TAC "100/euler.ml:LAND_CONV" LAND_CONV [INDEX_ODDPART_DECOMPOSITION] THEN
...
let ODD_OF_DISTINCT_OF_ODD = prove
 (`!p. (!i. ~(p(i) = 0) ==> ODD i)
  ...
  GEN_REWRITE_TAC "100/euler.ml:RAND_CONV" RAND_CONV [GSYM BINARYSUM_BITSET] THEN
...
let NSUM_DISTINCT_OF_ODD = prove
 (`!p. (!i. ~(p i = 0) ==> 1 <= i /\ i <= n) /\
  ...
  GEN_REWRITE_TAC "100/euler.ml:(RAND_CONV o BINDER_CONV o LAND_CONV)" (RAND_CONV o BINDER_CONV o LAND_CONV)
  ...
  CONV_TAC "100/euler.ml:SYM_CONV" SYM_CONV THEN ONCE_REWRITE_TAC[GSYM NSUM_SUPPORT] THEN
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[EXTENSION; FORALL_PAIR_THM; IN_ELIM_PAIR_THM; IN_ELIM_THM] THEN
    CONV_TAC "100/euler.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
  ...
  CONV_TAC "100/euler.ml:(TOP_DEPTH_CONV GEN_BETA_CONV)" (TOP_DEPTH_CONV GEN_BETA_CONV) THEN
  REWRITE_TAC[ARITH_RULE
   `(if p then 1 else 0) * x * y = (if p then x * y else 0)`] THEN
  GEN_REWRITE_TAC "100/euler.ml:(RAND_CONV o TOP_DEPTH_CONV)" (RAND_CONV o TOP_DEPTH_CONV) [IN_ELIM_THM] THEN
 ...
 (`!p. p IN {p | p partitions n /\ !i. ~(p(i) = 0) ==> ODD i}
  ...
  REWRITE_TAC[DISTINCT_DISTINCT_OF_ODD] THEN CONJ_TAC THENL
   [MATCH_MP_TAC SUPPORT_DISTINCT_OF_ODD;
    FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "100/euler.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
...
let ODD_OF_DISTINCT = prove
 (`!p. p IN {p | p partitions n /\ !i. p(i) <= 1}
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/euler.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
    AP_TERM_TAC THEN ABS_TAC THEN AP_THM_TAC THEN
    ASM_MESON_TAC[DISTINCT_OF_ODD_OF_DISTINCT]] THEN
  CONV_TAC "100/euler.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC NSUM_DISTINCT_OF_ODD THEN
  REWRITE_TAC[ODD_ODD_OF_DISTINCT] THEN
  CONJ_TAC THENL [ASM_MESON_TAC[SUPPORT_ODD_OF_DISTINCT]; ALL_TAC] THEN
