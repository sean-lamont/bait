...
let LEMMA_1 = prove
 (`!x n. x pow (n + 1) - (&n + &1) * x + &n =
  ...
  CONV_TAC "100/arithmetic_geometric_mean.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN GEN_TAC THEN INDUCT_TAC THEN
...
let LEMMA_3 = prove
 (`!n x. 1 <= n /\ (!i. 1 <= i /\ i <= n + 1 ==> &0 <= x i)
  ...
  CONV_TAC "100/arithmetic_geometric_mean.ml:REAL_FIELD" REAL_FIELD);;
 ...
 (`!n a. 1 <= n /\ (!i. 1 <= i /\ i <= n ==> &0 <= a(i))
  ...
  ASM_CASES_TAC `n = 0` THENL
   [ASM_REWRITE_TAC[PRODUCT_CLAUSES_NUMSEG; ARITH; SUM_SING_NUMSEG] THEN
    ...
    GEN_REWRITE_TAC "100/arithmetic_geometric_mean.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN MATCH_MP_TAC REAL_LE_RMUL THEN
