 ...
 (`!s t n. ~(s n) /\ t n /\
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SEQ_LE THEN MAP_EVERY EXISTS_TAC
    ...
    REWRITE_TAC[REAL_POW_ADD; REAL_POW_1] THEN CONV_TAC "100/realsuncountable.ml:REAL_FIELD" REAL_FIELD;
    ALL_TAC] THEN
...
let UNCOUNTABLE_REALS = prove
 (`~countable(UNIV:real->bool)`,
  ...
  GEN_REWRITE_TAC "100/realsuncountable.ml:I" I [MESON[] `(!x. P x) <=> ~(?x. ~P x)`] THEN
  ONCE_REWRITE_TAC[MINIMAL] THEN
