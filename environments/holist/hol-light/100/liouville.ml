...
let LIOUVILLE_INTERVAL = prove
 (`!p x. poly p x = &0 /\ ~(poly p = poly [])
  ...
  ASM_SIMP_TAC[] THEN GEN_REWRITE_TAC "100/liouville.ml:LAND_CONV" LAND_CONV [GSYM REAL_INV_INV] THEN
 ...
 (`!x. algebraic x
  ...
  REPEAT CONJ_TAC THENL
   [ASM_REWRITE_TAC[GSYM REAL_ABS_NZ; REAL_SUB_0];
    ...
    GEN_REWRITE_TAC "100/liouville.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_RID] THEN
...
let LIOUVILLE_SUM_BOUND = prove
 (`!d n. ~(n = 0)
  ...
  CONV_TAC "100/liouville.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[GSYM ADD1; FACT] THEN
...
let TRANSCENDENTAL_LIOUVILLE = prove
 (`transcendental(liouville)`,
  ...
  CONV_TAC "100/liouville.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN DISCH_THEN(X_CHOOSE_TAC `k:num`) THEN
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[real_div] THEN GEN_REWRITE_TAC "100/liouville.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
