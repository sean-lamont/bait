...
let ROTATE_EQ_REFLECT_LEMMA = prove
 (`!a b z t.
  ...
  GEN_REWRITE_TAC "100/morley.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV) [ARG] THEN
  MATCH_MP_TAC(COMPLEX_RING
...
let ROTATE_EQ_REFLECT_PI_LEMMA = prove
 (`!a b z t.
  ...
  CONV_TAC "100/morley.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[COMPLEX_MUL_LID]);;
...
let EQUILATERAL_TRIANGLE_ALGEBRAIC = prove
 (`!A B C j.
  ...
  (CONJUNCTS_THEN SUBST1_TAC) THENL
   [REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "100/morley.ml:COMPLEX_RING" COMPLEX_RING;
    ALL_TAC] THEN
...
let AFFINE_GROUP_ITER_3 = prove
 (`ITER 3 (\z. a * z + b) = (\z. a pow 3 * z + b * (Cx(&1) + a + a pow 2))`,
  ...
  REWRITE_TAC[ITER; FUN_EQ_THM] THEN CONV_TAC "100/morley.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  CONV_TAC "100/morley.ml:COMPLEX_RING" COMPLEX_RING);;

let AFFINE_GROUP_COMPOSE = prove
 (`(\z. a1 * z + b1) o (\z. a2 * z + b2) =
   (\z. (a1 * a2) * z + (b1 + a1 * b2))`,
  REWRITE_TAC[o_THM; FUN_EQ_THM] THEN CONV_TAC "100/morley.ml:COMPLEX_RING" COMPLEX_RING);;

let AFFINE_GROUP_I = prove
 (`I = (\z. Cx(&1) * z + Cx(&0))`,
  REWRITE_TAC[I_THM; FUN_EQ_THM] THEN CONV_TAC "100/morley.ml:COMPLEX_RING" COMPLEX_RING);;
...
let ALGEBRAIC_LEMMA = prove
 (`!a1 a2 a3 b1 b2 b3 A B C.
  ...
  REPEAT GEN_TAC THEN STRIP_TAC THEN ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
   [REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "100/morley.ml:COMPLEX_RING" COMPLEX_RING; ALL_TAC] THEN
  SUBGOAL_THEN
  ...
  REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "100/morley.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
...
let MORLEY = prove
 (`!A B C:real^2 P Q R.
  ...
  REPEAT CONJ_TAC THENL
   [REPEAT GEN_TAC THEN
    GEN_REWRITE_TAC "100/morley.ml:RAND_CONV" RAND_CONV [GSYM IM_COMPLEX_INV_LE_0] THEN
   ...
   [MAP_EVERY EXPAND_TAC ["g1"; "g3"] THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC ROTATE_EQ_REFLECT_LEMMA THEN
      ...
      GEN_REWRITE_TAC "100/morley.ml:RAND_CONV" RAND_CONV [SYM(ASSUME `angle(C:real^2,A,Q) = a`)] THEN
      REWRITE_TAC[angle] THEN ONCE_REWRITE_TAC[VECTOR_ANGLE_SYM] THEN
      ASM_SIMP_TAC[VECTOR_ANGLE_ARG; COMPLEX_SUB_0];
      ALL_TAC] THEN
    CONV_TAC "100/morley.ml:SYM_CONV" SYM_CONV THEN ONCE_REWRITE_TAC[REFLECT_ACROSS_SYM] THEN
    MATCH_MP_TAC ROTATE_EQ_REFLECT_PI_LEMMA THEN
    ASM_REWRITE_TAC[GSYM REAL_MUL_RNEG] THEN
    REWRITE_TAC[REAL_ARITH `&2 * a = &4 * pi + &2 * --c <=>
                            a = &2 * pi - c`] THEN
    GEN_REWRITE_TAC "100/morley.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
     [SYM(ASSUME `angle(B:real^2,C,A) / &3 = c`)] THEN
    ONCE_REWRITE_TAC[ANGLE_SYM] THEN FIRST_ASSUM(fun th ->
     GEN_REWRITE_TAC "100/morley.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [SYM th]) THEN
    REWRITE_TAC[angle] THEN
    ASM_SIMP_TAC[VECTOR_ANGLE_ARG; COMPLEX_SUB_0] THEN
    GEN_REWRITE_TAC "100/morley.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM COMPLEX_INV_DIV] THEN
