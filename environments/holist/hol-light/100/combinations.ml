...
let lemma = prove
 (`~(a IN t)
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "100/combinations.ml:I" I [EXTENSION] THEN
...
let NUMBER_OF_COMBINATIONS = prove
 (`!n m s:A->bool.
  ...
  MATCH_MP_TAC BINOM_INDUCT THEN REWRITE_TAC[binom] THEN REPEAT CONJ_TAC THENL
   [REPEAT STRIP_TAC THEN CONV_TAC "100/combinations.ml:HAS_SIZE_CONV" HAS_SIZE_CONV THEN
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "100/combinations.ml:LAND_CONV" LAND_CONV [HAS_SIZE_CLAUSES] THEN
