...
let HARMONIC_DIVERGES = prove
 (`~(?s. !e. &0 < e
  ...
  CONV_TAC "100/divharmonic.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN STRIP_TAC THEN
  ...
  CONJ_TAC THENL
   [SIMP_TAC[SUM_CONST_NUMSEG; ARITH_RULE `(2 * x + 1) - (x + 1) = x`] THEN
    ...
    MP_TAC(SPEC `n:num` REAL_POS) THEN CONV_TAC "100/divharmonic.ml:REAL_FIELD" REAL_FIELD;
...
let HARMONIC_LEMMA = prove
 (`!m. sum(1..2 EXP m) (\n. &1 / &n) >= &m / &2`,
  ...
  CONV_TAC "100/divharmonic.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  CONJ_TAC THENL
   [SIMP_TAC[SUM_CONST_NUMSEG; EXP; ARITH_RULE `(2 * x + 1) - (x + 1) = x`] THEN
    ...
    CONV_TAC "100/divharmonic.ml:REAL_FIELD" REAL_FIELD;
    MATCH_MP_TAC SUM_LE_NUMSEG THEN REPEAT STRIP_TAC THEN
    REWRITE_TAC[real_div; REAL_MUL_LID] THEN MATCH_MP_TAC REAL_LE_INV2 THEN
    POP_ASSUM_LIST(MP_TAC o end_itlist CONJ) THEN
    REWRITE_TAC[REAL_OF_NUM_LE; REAL_OF_NUM_LT] THEN ARITH_TAC]);;
...
let LOG_BOUND = prove
 (`&0 < x /\ x < &1 ==> ln(&1 + x) >= x / &2`,
  ...
  GEN_REWRITE_TAC "100/divharmonic.ml:LAND_CONV" LAND_CONV [GSYM LN_EXP] THEN
