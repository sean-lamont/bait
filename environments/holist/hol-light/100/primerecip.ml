 ...
 (`!n. (&3 / (&16 * ln(&32))) * sum(1..n) (\i. &1 / &i) <=
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[ARITH; SUM_SING_NUMSEG] THEN
    CONV_TAC "100/primerecip.ml:(RAND_CONV SUM_CONV)" (RAND_CONV SUM_CONV) THEN REWRITE_TAC[] THEN
    CONV_TAC "100/primerecip.ml:(ONCE_DEPTH_CONV PRIME_CONV)" (ONCE_DEPTH_CONV PRIME_CONV) THEN
    CONV_TAC "100/primerecip.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    ...
    ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN CONV_TAC "100/primerecip.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    SIMP_TAC[GSYM REAL_LE_RDIV_EQ; REAL_LT_DIV; REAL_OF_NUM_LT; ARITH] THEN
    CONV_TAC "100/primerecip.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    GEN_REWRITE_TAC "100/primerecip.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_DIV] THEN
    MATCH_MP_TAC REAL_LE_INV2 THEN CONV_TAC "100/primerecip.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    REWRITE_TAC[LN_2_COMPOSITION; real_div; real_sub] THEN
    CONV_TAC "100/primerecip.ml:REALCALC_REL_CONV" REALCALC_REL_CONV;
    ALL_TAC] THEN
  ...
  GEN_REWRITE_TAC "100/primerecip.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
...
let PRIMERECIP_DIVERGES_NUMSEG = prove
 (`~(convergent (\n. sum (1..n) (\i. if prime i then &1 / &i else &0)))`,
  ...
  GEN_REWRITE_TAC "100/primerecip.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
