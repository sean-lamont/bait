 ...
 (`!s:A->bool t:B->bool m n.
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[EXTENSION; IN_IMAGE; funspace; IN_ELIM_THM] THEN
    ...
    CONV_TAC "100/ballot.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let COUNTING_LEMMA = prove
 (`CARD {f | f IN (1..(n+1) -->_ballot {A,B}) /\ P f} =
  ...
  TRY(FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (MESON[]
   `P x ==> x = y ==> P y`))) THEN
  TRY(GEN_REWRITE_TAC "100/ballot.ml:I" I [FUN_EQ_THM]) THEN ASM_MESON_TAC[ADD1_NOT_IN_NUMSEG]);;
...
let ALL_COUNTINGS_SUC = prove
 (`!a b. all_countings (a + 1) (b + 1) =
  ...
  CONV_TAC "100/ballot.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN
  GEN_REWRITE_TAC "100/ballot.ml:LAND_CONV" LAND_CONV [COUNTING_LEMMA] THEN
...
let VALID_COUNTINGS_SUC = prove
 (`!a b. valid_countings (a + 1) (b + 1) =
  ...
  ABBREV_TAC `n = a + b + 1` THEN
  CONV_TAC "100/ballot.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN
  GEN_REWRITE_TAC "100/ballot.ml:LAND_CONV" LAND_CONV [COUNTING_LEMMA] THEN REWRITE_TAC[] THEN
  ONCE_REWRITE_TAC[COND_RAND] THEN ONCE_REWRITE_TAC[COND_RATOR] THEN
  REWRITE_TAC[vote_DISTINCT] THEN
...
let VALID_COUNTINGS = prove
 (`!a b. (a + b) * valid_countings a b = (a - b) * binom(a + b,a)`,
  ...
  CONV_TAC "100/ballot.ml:REAL_RING" REAL_RING);;
...
let BALLOT = prove
 (`!a b. &(valid_countings a b) =
  ...
  CONV_TAC "100/ballot.ml:REAL_FIELD" REAL_FIELD);;
