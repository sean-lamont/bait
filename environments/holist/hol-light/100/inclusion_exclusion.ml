 ...
 (`!P (f:(A->bool)->real) (A:I->bool) (x:I->(A->bool)).
  ...
  let lemma = prove
   (`{t | t SUBSET (a INSERT s) /\ P t} =
    ...
    GEN_REWRITE_TAC "100/inclusion_exclusion.ml:I" I [EXTENSION] THEN
    REWRITE_TAC[IN_ELIM_THM; IN_UNION] THEN MESON_TAC[]) in
  REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM] THEN
  REPEAT GEN_TAC THEN REWRITE_TAC[IMP_IMP; GSYM CONJ_ASSOC] THEN
  GEN_REWRITE_TAC "100/inclusion_exclusion.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_FORALL_THM] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[REAL_ARITH `x - a:real = x + b <=> b = --a`] THEN
    ...
    GEN_REWRITE_TAC "100/inclusion_exclusion.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [SIMPLE_IMAGE_GEN] THEN
...
let INCLUSION_EXCLUSION_USUAL = prove
 (`!s:(A->bool)->bool.
  ...
  GEN_REWRITE_TAC "100/inclusion_exclusion.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_ELIM_THM] THEN
...
let CARD_SUBSETS_STEP = prove
 (`!x:A s. FINITE s /\ ~(x IN s) /\ u SUBSET s
  ...
  (CONJ_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
   CONV_TAC "100/inclusion_exclusion.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_UNION_EQ THEN
   ASM_SIMP_TAC[FINITE_SUBSETS_RESTRICT; FINITE_INSERT] THEN CONJ_TAC THENL
    [REWRITE_TAC[EXTENSION; NOT_IN_EMPTY; IN_INTER] THEN
     ...
     ASM SET_TAC[];
     ALL_TAC] THEN
   GEN_REWRITE_TAC "100/inclusion_exclusion.ml:I" I [EXTENSION] THEN X_GEN_TAC `t:A->bool` THEN
   ...
   AP_TERM_TAC THEN GEN_REWRITE_TAC "100/inclusion_exclusion.ml:I" I [FUN_EQ_THM] THEN
...
let SUM_ALTERNATING_CANCELS = prove
 (`!s:A->bool f.
  ...
  CONJ_TAC THENL
   [CONV_TAC "100/inclusion_exclusion.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_UNION_EQ THEN
...
let INCLUSION_EXCLUSION_SYMMETRIC = prove
 (`!f g:(A->bool)->real.
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "100/inclusion_exclusion.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC EQ_TRANS THEN
...
let INCLUSION_EXCLUSION_MOBIUS = prove
 (`!f g:(A->bool)->real.
  ...
  REWRITE_TAC[GSYM REAL_POW_POW] THEN CONV_TAC "100/inclusion_exclusion.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
 ...
 (`!f s:A->bool.
  ...
  let lemma = prove
   (`{t | ?u. u SUBSET s /\ t = x INSERT u} =
    ...
    GEN_REWRITE_TAC "100/inclusion_exclusion.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_ELIM_THM; IN_IMAGE] THEN
    MESON_TAC[]) in
