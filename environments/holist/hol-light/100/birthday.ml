 ...
 (`!s:A->bool t:B->bool m n.
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[EXTENSION; IN_IMAGE; funspace; IN_ELIM_THM] THEN
    ...
    CONV_TAC "100/birthday.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let FACT_DIV_MULT = prove
 (`!m n. m <= n ==> FACT n = (FACT(n) DIV FACT(m)) * FACT(m)`,
  ...
  GEN_REWRITE_TAC "100/birthday.ml:(RAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o LAND_CONV o ONCE_DEPTH_CONV) [MULT_SYM] THEN
...
let HAS_SIZE_DIFF = prove
 (`!s t:A->bool m n.
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/birthday.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [th]) THEN
...
let BIRTHDAY_THM_EXPLICIT = prove
 (`!s t. s HAS_SIZE 23 /\ t HAS_SIZE 365
  ...
  CONV_TAC "100/birthday.ml:(ONCE_DEPTH_CONV NUM_SUB_CONV)" (ONCE_DEPTH_CONV NUM_SUB_CONV) THEN
  REPEAT(CHANGED_TAC
