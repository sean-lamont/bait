...
let HYPERPLANE_CELL_SING_CASES = prove
 (`!a b c:real^N->bool.
  ...
  GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV) [EQ_SYM_EQ] THEN
...
let HYPERPLANE_CELL_CONVEX = prove
 (`!A c:real^N->bool. hyperplane_cell A c ==> convex c`,
  ...
  CONV_TAC "100/polyhedron.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN REWRITE_TAC[hyperplane_side] THEN
  REPEAT_TCL DISJ_CASES_THEN SUBST1_TAC
...
let HYPERPLANE_CELL_INTERS = prove
 (`!A C. (!c:real^N->bool. c IN C ==> hyperplane_cell A c) /\
  ...
  GEN_REWRITE_TAC "100/polyhedron.ml:I" I [EXTENSION] THEN
...
let HYPERPLANE_CELLCOMPLEX_UNIONS = prove
 (`!A C. (!s:real^N->bool. s IN C ==> hyperplane_cellcomplex A s)
  ...
  GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
  ...
  GEN_REWRITE_TAC "100/polyhedron.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_UNIONS; IN_ELIM_THM] THEN
  ASM SET_TAC[]);;
 ...
 (`!A C. (!s:real^N->bool. s IN C ==> hyperplane_cellcomplex A s)
  ...
  let lemma = prove
   (`UNIONS s = UNIONS {t | t IN s /\ ~(t = {})}`,
    REWRITE_TAC[UNIONS_GSPEC] THEN GEN_REWRITE_TAC "100/polyhedron.ml:I" I [EXTENSION] THEN
  ...
  GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [hyperplane_cellcomplex] THEN
  ...
  SUBST1_TAC THENL
   [GEN_REWRITE_TAC "100/polyhedron.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_ELIM_THM] THEN
    ASM_MESON_TAC[];
    ALL_TAC] THEN
 ...
 (`!A s. hyperplane_cellcomplex A s
  ...
  SUBST1_TAC THENL
   [SUBST1_TAC(SYM(ISPEC `A:real^N#real->bool` UNIONS_HYPERPLANE_CELLS)) THEN
    ...
    AP_TERM_TAC THEN GEN_REWRITE_TAC "100/polyhedron.ml:I" I [FUN_EQ_THM] THEN
 ...
 (`!A B s:real^N->bool.
  ...
  REWRITE_TAC[FORALL_IN_GSPEC] THEN CONJ_TAC THENL
   [X_GEN_TAC `c':real^N->bool` THEN STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    GEN_REWRITE_TAC "100/polyhedron.ml:LAND_CONV" LAND_CONV [EXTENSION] THEN
...
let EULER_CHARACTERISTIC_CELL = prove
 (`!A c. hyperplane_cell A c
  ...
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM UNIONS_1] THEN
...
let EULER_CHARACTERISTIC_CELLCOMPLEX_UNION = prove
 (`!A s t:real^N->bool.
  ...
  REPEAT STRIP_TAC THEN REWRITE_TAC[euler_characteristic] THEN
  CONV_TAC "100/polyhedron.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_UNION_EQ THEN
...
let EULER_CHARACTERISTIC = prove
 (`!A s:real^N->bool.
  ...
  ANTS_TAC THENL
   [ASM_SIMP_TAC[FINITE_RESTRICT_HYPERPLANE_CELLS] THEN
    GEN_REWRITE_TAC "100/polyhedron.ml:I" I [SUBSET] THEN
 ...
 (`!A h s:real^N->bool.
  ...
  MAP_EVERY (fun t ->
   ASM_CASES_TAC t THENL
    [MATCH_MP_TAC EQ_TRANS THEN EXISTS_TAC
     ...
     MATCH_MP_TAC(MESON[] `s = t ==> sum s f = sum t f`) THEN
     GEN_REWRITE_TAC "100/polyhedron.ml:I" I [EXTENSION] THEN X_GEN_TAC `c':real^N->bool` THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC(MESON[] `s = t ==> sum s f = sum t f`) THEN
    ...
    REWRITE_TAC[IN_INSERT; NOT_IN_EMPTY] THEN CONV_TAC "100/polyhedron.ml:TAUT" TAUT;
    ALL_TAC] THEN
   ...
   [MP_TAC(ISPECL [`A:real^N#real->bool`; `c:real^N->bool`]
    ...
    THENL
     [REWRITE_TAC[INTER_ASSOC] THEN CONJ_TAC THEN
      GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [INTER_COMM] THEN
...
let EULER_POLYHEDRAL_CONE = prove
 (`!s. polyhedron s /\ conic s /\ ~(interior s = {}) /\ ~(s = (:real^N))
       ...
       [EXISTS_TAC `min (&1 / &2) (b / ((a:real^N) dot x))` THEN
        ...
        SUBGOAL_THEN `&0 < (a:real^N) dot x` ASSUME_TAC THENL
         [MATCH_MP_TAC REAL_LT_TRANS THEN EXISTS_TAC `b:real` THEN
          ...
          CONV_TAC "100/polyhedron.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o BINDER_CONV o funpow 2 RAND_CONV)" (LAND_CONV o BINDER_CONV o funpow 2 RAND_CONV)
   [EQ_SYM_EQ] THEN
     ...
     [X_GEN_TAC `f:real^N->bool` THEN STRIP_TAC THEN
       ...
       [ASM_CASES_TAC `f:real^N->bool = s` THENL
        ...
        THENL
         [ONCE_REWRITE_TAC[SIMPLE_IMAGE_GEN] THEN
          ...
          ASM SET_TAC[];
          ALL_TAC] THEN
        DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:LAND_CONV" LAND_CONV [th]) THEN
        ASM_REWRITE_TAC[] THEN GEN_REWRITE_TAC "100/polyhedron.ml:I" I [EXTENSION] THEN
       ...
       [FIRST_X_ASSUM(X_CHOOSE_THEN `J:(real^N->bool)->bool`
        ...
        REPEAT CONJ_TAC THENL
         [ASM_REWRITE_TAC[FINITE_UNION] THEN MATCH_MP_TAC FINITE_IMAGE THEN
          ...
          FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:LAND_CONV" LAND_CONV [th]) THEN
          ...
          ASM SET_TAC[]];
        ALL_TAC] THEN
      GEN_REWRITE_TAC "100/polyhedron.ml:LAND_CONV" LAND_CONV
         [MESON[HAS_SIZE]
           `(?f. FINITE f /\ P f) <=> (?n f. f HAS_SIZE n /\ P f)`] THEN
      GEN_REWRITE_TAC "100/polyhedron.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
      ...
      ASSUME_TAC THENL
       [SUBGOAL_THEN `~(relative_interior f:real^N->bool = {})` MP_TAC THENL
        ...
        ASM_REWRITE_TAC[] THEN DISCH_THEN(fun th ->
          GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [SYM(CONJUNCT2 th)]) THEN
        REWRITE_TAC[IN_ELIM_THM] THEN REAL_ARITH_TAC;
        ALL_TAC] THEN
      FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:RAND_CONV" RAND_CONV [SYM th] THEN
        MP_TAC(SYM th)) THEN
      FIRST_X_ASSUM(X_CHOOSE_THEN `K:(real^N->bool)->bool` MP_TAC) THEN
      DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
      DISCH_THEN(fun th -> ASSUME_TAC(SYM th) THEN
        GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o LAND_CONV o ONCE_DEPTH_CONV) [th]) THEN
      ...
      SUBGOAL_THEN `DISJOINT (J:(real^N->bool)->bool) K` ASSUME_TAC THENL
       [UNDISCH_TAC `~(relative_interior f:real^N->bool = {})` THEN
        FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV) [SYM th]) THEN
      ...
      ASSUME_TAC THENL
       [MATCH_MP_TAC SUBSET_ANTISYM THEN CONJ_TAC THENL
         [ALL_TAC;
          FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
          GEN_REWRITE_TAC "100/polyhedron.ml:I" I [SUBSET] THEN
          ...
          REAL_ARITH_TAC] THEN
        GEN_REWRITE_TAC "100/polyhedron.ml:I" I [SUBSET] THEN X_GEN_TAC `x:real^N` THEN
        ...
        DISCH_THEN(MP_TAC o SPEC `h:real^N->bool`) THEN
        ASM_REWRITE_TAC[] THEN
        FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
      ...
      MATCH_MP_TAC MONO_EXISTS THEN X_GEN_TAC `z:real^N` THEN
      GEN_REWRITE_TAC "100/polyhedron.ml:RAND_CONV" RAND_CONV [EXTENSION] THEN
      ...
      GEN_REWRITE_TAC "100/polyhedron.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [IN] THEN
      ...
      MP_TAC THENL
       [FIRST_ASSUM(MP_TAC o GEN_REWRITE_RULE I [HYPERPLANE_CELL]) THEN
        ...
        REWRITE_TAC[hyperplane_side; REAL_SUB_RZERO] THEN
        GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)
         [EQ_SYM_EQ] THEN
        ...
        GEN_REWRITE_TAC "100/polyhedron.ml:I" I [EXTENSION] THEN X_GEN_TAC `y:real^N` THEN
        ...
        ASM_REWRITE_TAC[] THEN CONV_TAC "100/polyhedron.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
       ...
       [ONCE_REWRITE_TAC[SIMPLE_IMAGE] THEN REWRITE_TAC[INTERS_IMAGE] THEN
        ...
        ASM_CASES_TAC `(y:real^N) IN s` THEN ASM_REWRITE_TAC[] THENL
         [MATCH_MP_TAC(TAUT `a ==> (a /\ b <=> b)`) THEN
          ...
          FIRST_X_ASSUM(MP_TAC o SPEC `h:real^N->bool`) THEN
          ANTS_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
          DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
      ...
      FIRST_X_ASSUM(MP_TAC o SPEC `h:real^N->bool`) THEN
      ANTS_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
      DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV)
        [GSYM(CONJUNCT2 th)]) THEN
   ...
   [GEN_TAC THEN DISCH_TAC THEN MATCH_MP_TAC HYPERPLANE_CELLCOMPLEX_MONO THEN
    EXISTS_TAC `{((fa:(real^N->bool)->real^N) h,&0)}` THEN CONJ_TAC THENL
     [MATCH_MP_TAC HYPERPLANE_CELL_CELLCOMPLEX THEN
      ...
      FIRST_X_ASSUM(MP_TAC o SPEC `h:real^N->bool`) THEN
      ASM_REWRITE_TAC[] THEN DISCH_THEN(fun th ->
        GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [SYM(CONJUNCT2 th)]) THEN
   ...
   [MATCH_MP_TAC SUM_EQ THEN REWRITE_TAC[FORALL_IN_GSPEC] THEN
    ...
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`s:real^N->bool`; `H:(real^N->bool)->bool`;
      ...
      CONV_TAC "100/polyhedron.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
      ...
      CONJ_TAC THENL
       [EXPAND_TAC "s" THEN REWRITE_TAC[IN_INTERS] THEN
        ...
        X_GEN_TAC `h:real^N->bool` THEN DISCH_TAC THEN
        FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
         [SYM(CONJUNCT2(MATCH_MP th (ASSUME `(h:real^N->bool) IN H`)))]) THEN
    ...
    ASSUME_TAC THENL
     [SUBGOAL_THEN
      ...
      DISCH_THEN(fun th ->
        GEN_REWRITE_TAC "100/polyhedron.ml:(RAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o LAND_CONV o ONCE_DEPTH_CONV) [SYM th] THEN
        GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [SYM th]) THEN
  ...
  ANTS_TAC THENL
   [CONJ_TAC THENL
     [MATCH_MP_TAC FINITE_SUBSET THEN
      ...
      ASM_SIMP_TAC[FINITE_IMAGE];
      GEN_REWRITE_TAC "100/polyhedron.ml:I" I [SUBSET] THEN REWRITE_TAC[FORALL_IN_IMAGE] THEN
   ...
   [MATCH_MP_TAC SUM_EQ THEN X_GEN_TAC `n:num` THEN
    ...
    ANTS_TAC THENL
     [MATCH_MP_TAC FINITE_SUBSET THEN
      ...
      ASM_SIMP_TAC[FINITE_IMAGE];
      DISCH_THEN SUBST1_TAC] THEN
    GEN_REWRITE_TAC "100/polyhedron.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN AP_TERM_TAC THEN
    AP_TERM_TAC THEN MATCH_MP_TAC EQ_TRANS THEN
    EXISTS_TAC `CARD {t | t SUBSET {(:real^N) DIFF t | t IN H} /\
                          t HAS_SIZE n}` THEN
    CONJ_TAC THENL
     [AP_TERM_TAC THEN GEN_REWRITE_TAC "100/polyhedron.ml:I" I [EXTENSION] THEN
...
let EULER_POINCARE_LEMMA = prove
 (`!p:real^N->bool.
   ...
   [DISCH_TAC THEN MP_TAC(ISPEC `s:real^N->bool`
    ...
    SUBGOAL_THEN `s SUBSET {x:real^N | x$1 = &1}` MP_TAC THENL
     [FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP (SET_RULE
      ...
      CONJ_TAC THENL
       [FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:LAND_CONV" LAND_CONV [SYM th]) THEN
  ...
  THENL
   [GEN_REWRITE_TAC "100/polyhedron.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_ELIM_THM; IN_SING] THEN
  ...
  SUBGOAL_THEN `dimindex(:N) = (dimindex(:N)-1)+1`
   (fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [th])
   ...
   [REPEAT STRIP_TAC THEN ASM_CASES_TAC `f:real^N->bool = {}` THEN
    ...
    ASM_CASES_TAC `x:real^N = a` THENL
     [REWRITE_TAC[IN_SEGMENT] THEN DISCH_THEN
      ...
      GEN_REWRITE_TAC "100/polyhedron.ml:LAND_CONV" LAND_CONV [CART_EQ] THEN
     ...
     [FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [IN_OPEN_SEGMENT]) THEN
      ...
      REPEAT CONJ_TAC THENL
       [REWRITE_TAC[REAL_MUL_LZERO] THEN MATCH_MP_TAC REAL_LE_MUL THEN
        ...
        CONV_TAC "100/polyhedron.ml:REAL_FIELD" REAL_FIELD];
 ...
 (`!p:real^N->bool.
  ...
  ANTS_TAC THENL
   [MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
    ...
    DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/polyhedron.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
    MATCH_MP_TAC SUM_EQ_NUMSEG THEN X_GEN_TAC `d:num` THEN STRIP_TAC THEN
    REWRITE_TAC[] THEN AP_TERM_TAC THEN AP_TERM_TAC THEN
    GEN_REWRITE_TAC "100/polyhedron.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [SIMPLE_IMAGE_GEN] THEN
    CONV_TAC "100/polyhedron.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_IMAGE_INJ THEN CONJ_TAC THENL
...
let EULER_RELATION = prove
 (`!p:real^3->bool.
  ...
  CONV_TAC "100/polyhedron.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "100/polyhedron.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
