...
let REAL_LT_1_POW2 = prove
 (`!n. &1 < &2 pow n <=> ~(n = 0)`,
  ...
  CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let REAL_POW2_CLAUSES = prove
 (`(!n. &0 <= &2 pow n) /\
  ...
  CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let SUM_PERMUTE_0 = prove
 (`!n p. (!y. y < n ==> ?!x. x < n /\ (p(x) = y))
  ...
  CONV_TAC "100/piseries.ml:(ONCE_DEPTH_CONV EXISTS_UNIQUE_CONV)" (ONCE_DEPTH_CONV EXISTS_UNIQUE_CONV) THEN
  DISCH_THEN(CONJUNCTS_THEN2 MP_TAC ASSUME_TAC) THEN
  DISCH_THEN(X_CHOOSE_THEN `k:num` STRIP_ASSUME_TAC) THEN
  GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [sum] THEN REWRITE_TAC[ADD_CLAUSES] THEN
  ...
  SUBGOAL_THEN `!y:num. y < n ==> ?!x. x < n /\ (q x = y)` MP_TAC THENL
   [X_GEN_TAC `y:num` THEN DISCH_TAC THEN (MP_TAC o ASSUME)
       ...
       [EXISTS_TAC `x:num` THEN EXPAND_TAC "q" THEN BETA_TAC THEN
        ...
        DISCH_THEN(DISJ_CASES_THEN2 ASSUME_TAC SUBST_ALL_TAC) THENL
         [EXISTS_TAC `x - 1` THEN EXPAND_TAC "q" THEN BETA_TAC THEN
          ...
          REWRITE_TAC[GSYM LE_SUC_LT] THEN CONV_TAC "100/piseries.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
       ...
       [CONJ_TAC THEN COND_CASES_TAC THEN ASM_REWRITE_TAC[LT_SUC] THEN
        ...
        REPEAT COND_CASES_TAC THEN REWRITE_TAC[SUC_INJ] THENL
         [DISCH_THEN SUBST_ALL_TAC THEN UNDISCH_TAC `~(x2 < k:num)` THEN
          CONV_TAC "100/piseries.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THEN
    ...
    DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV)
    ...
    GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o RATOR_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o RATOR_CONV o ONCE_DEPTH_CONV)
      [ASSUME `n = k + d:num`] THEN REWRITE_TAC[GSYM SUM_TWO] THEN
    GEN_REWRITE_TAC "100/piseries.ml:(RATOR_CONV o ONCE_DEPTH_CONV)" (RATOR_CONV o ONCE_DEPTH_CONV)
    ...
    REWRITE_TAC[GSYM REAL_ADD_ASSOC] THEN BINOP_TAC THENL
     [MATCH_MP_TAC SUM_EQ THEN X_GEN_TAC `r:num` THEN
      ...
      GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [REAL_ADD_SYM] THEN
...
let SUM_REVERSE = prove
 (`!n m f. sum(m,n) f = sum(m,n) (\k. f(((n + 2 * m) - 1) - k))`,
  ...
  GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [SUM_REVERSE_0] THEN
 ...
 (`!x n. abs(sin x -
  ...
  W(C SUBGOAL_THEN (fun th -> REWRITE_TAC[th]) o funpow 2 lhand o snd) THENL
   [CONJ_TAC THENL
    ...
    SPEC_TAC(`d:num`,`d:num`) THEN CONV_TAC "100/piseries.ml:EXPAND_CASES_CONV" EXPAND_CASES_CONV THEN
    CONV_TAC "100/piseries.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SUM_EQ THEN X_GEN_TAC `r:num` THEN STRIP_TAC THEN
    ...
    DISCH_THEN(CONJUNCTS_THEN2 MP_TAC ASSUME_TAC) THEN
    DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [th] THEN
      MP_TAC(SYM th)) THEN
    REWRITE_TAC[EVEN_ADD; EVEN_MULT; ARITH_EVEN] THEN
    UNDISCH_TAC `r MOD 4 < 4` THEN
    SPEC_TAC(`r MOD 4`,`d:num`) THEN CONV_TAC "100/piseries.ml:EXPAND_CASES_CONV" EXPAND_CASES_CONV THEN
    CONV_TAC "100/piseries.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[] THEN
  ...
  MATCH_MP_TAC REAL_LE_MUL2 THEN REWRITE_TAC[REAL_ABS_POS] THEN CONJ_TAC THENL
   [REWRITE_TAC[real_div; REAL_ABS_MUL] THEN
    GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
...
let COT_HALF_TAN = prove
 (`~(integer x)
  ...
  SUBGOAL_THEN `pi * x = &2 * pi * x / &2`
   (fun th -> GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [th])
  THENL
 ...
 (`~(integer x)
  ...
  DISCH_TAC THEN INDUCT_TAC THENL
   [REWRITE_TAC[EXP; real_pow; REAL_DIV_1; ADD_CLAUSES; REAL_POW_1] THEN
    CONV_TAC "100/piseries.ml:(ONCE_DEPTH_CONV REAL_SUM_CONV)" (ONCE_DEPTH_CONV REAL_SUM_CONV) THEN
    ...
    ALL_TAC] THEN
  FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [th]) THEN
    ...
    REWRITE_TAC[] THEN BINOP_TAC THENL
     [MATCH_MP_TAC COT_HALF_POS THEN
      ...
      SUBGOAL_THEN `x = &2 pow n * (x + &k) / &2 pow n - &k`
        (fun th -> GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [th])
      THENL
  ...
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
  ...
  CONV_TAC "100/piseries.ml:(ONCE_DEPTH_CONV (ALPHA_CONV `j:num`))" (ONCE_DEPTH_CONV (ALPHA_CONV `j:num`)) THEN
...
let COT_HALF_KNOPP = prove
 (`~(integer x)
  ...
  GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [real_div] THEN
  GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [REAL_ADD_RDISTRIB] THEN
  ...
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [SUM_ADD] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(funpow 2 (LAND_CONV o RAND_CONV) o RAND_CONV)" (funpow 2 (LAND_CONV o RAND_CONV) o RAND_CONV)
    [SUM_REVERSE] THEN
  ...
  BINOP_TAC THENL
   [GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
    ...
    CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_MUL_RID]; ALL_TAC] THEN
  ...
  GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [real_div] THEN
  REWRITE_TAC[REAL_MUL_LID] THEN REWRITE_TAC[GSYM real_div] THEN
...
let NOT_INTEGER_LEMMA = prove
 (`~(x = &0) /\ abs(x) < &1 ==> ~(integer x)`,
  ...
  CONV_TAC "100/piseries.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN SIMP_TAC[integer; LEFT_IMP_EXISTS_THM] THEN
 ...
 (`!x y. &0 <= x /\ x < pi / &2 /\ &0 <= y /\ y < pi / &2
  ...
  ASSUME_TAC THENL
   [ALL_TAC;
    ...
    CONV_TAC "100/piseries.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let KNOPP_TERM_EQUIVALENT = prove
 (`~(integer x) /\ k < 2 EXP n
  ...
  GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [REAL_POW_ADD] THEN
  REWRITE_TAC[REAL_POW_1; real_div] THEN
  GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [AC REAL_MUL_AC
  ...
  SUBGOAL_THEN `sin(a - b) * cos(a + b) + sin(a + b) * cos(a - b) =
                sin(&2 * a)`
  SUBST1_TAC THENL
   [GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [REAL_MUL_SYM] THEN
    REWRITE_TAC[GSYM SIN_ADD] THEN AP_TERM_TAC THEN REAL_ARITH_TAC;
    ALL_TAC] THEN
  REWRITE_TAC[SIN_DOUBLE] THEN
  GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [REAL_ARITH
...
let SIN_LINEAR_ABOVE = prove
 (`!x. abs(x) < &1 ==> abs(sin x) <= &2 * abs(x)`,
  ...
  CONV_TAC "100/piseries.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  CONJ_TAC THENL [ALL_TAC; CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV] THEN
...
let SIN_LINEAR_BELOW = prove
 (`!x. abs(x) < &2 ==> abs(sin x) >= abs(x) / &3`,
  ...
  CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  CONV_TAC "100/piseries.ml:(LAND_CONV(RAND_CONV(RAND_CONV num_CONV)))" (LAND_CONV(RAND_CONV(RAND_CONV num_CONV))) THEN
  ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
 ...
 (`~(integer x) /\ k < 2 EXP n /\ &6 * abs(x) < &k
  ...
  MATCH_MP_TAC REAL_LE_INV2 THEN CONJ_TAC THENL
   [REWRITE_TAC[REAL_SUB_LT] THEN
    ...
    GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_LID] THEN
    ...
    GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [GSYM REAL_MUL_LID] THEN
  ...
  GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [REAL_ABS_DIV] THEN
  GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [real_div] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[GSYM real_ge] THEN MATCH_MP_TAC SIN_LINEAR_BELOW THEN
    ...
    CONV_TAC "100/piseries.ml:(LAND_CONV REAL_RAT_REDUCE_CONV)" (LAND_CONV REAL_RAT_REDUCE_CONV) THEN
    ...
    GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
...
let KNOPP_TERM_BOUND = prove
 (`~(integer x) /\ k < 2 EXP n /\ &6 * abs(x) < &k
  ...
  GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_DIV] THEN AP_TERM_TAC THEN
 ...
 (`(\n. inv(&(n + 1) * &(n + 2))) sums &1`,
  ...
  THENL
   [INDUCT_TAC THEN REWRITE_TAC[sum] THEN
    ...
    CONV_TAC "100/piseries.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_MUL_LINV_UNIQ THEN
    ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [GSYM REAL_SUB_RZERO] THEN
 ...
 (`((\x. x * cot(x)) tends_real_real &1)(&0)`,
  SUBGOAL_THEN `(cos tends_real_real &1)(&0)` MP_TAC THENL
   [MP_TAC(SPEC `&0` DIFF_COS) THEN
    ...
    CONV_TAC "100/piseries.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN REWRITE_TAC[]; ALL_TAC] THEN
...
let COT_LIMIT_LEMMA1 = prove
 (`~(x = &0)
  ...
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [GSYM REAL_MUL_LID] THEN
  FIRST_ASSUM(SUBST1_TAC o GSYM o MATCH_MP REAL_MUL_LINV) THEN
  REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN
  GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
 ...
 (`?M. !x. &0 < x /\ abs(x) <= &1 ==> abs(x * cot(x)) <= M`,
  ...
  W(C SUBGOAL_THEN (fun th -> REWRITE_TAC[th]) o funpow 2 lhand o snd) THENL
   [X_GEN_TAC `x:real` THEN STRIP_TAC THEN
    ...
    GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
    REWRITE_TAC[cot] THEN MATCH_MP_TAC CONT_DIV THEN REPEAT CONJ_TAC THENL
     [MATCH_MP_TAC DIFF_CONT THEN
      ...
      CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV]; ALL_TAC] THEN
 ...
 (`~(integer x)
  ...
  ASSUME_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    MATCH_MP_TAC REAL_LE_LMUL THEN REWRITE_TAC[REAL_ABS_POS] THEN
    GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV
   ...
   [X_CHOOSE_TAC `Bd:real` COT_X_BOUND_LEMMA THEN
    ...
    ASSUME_TAC THENL
     [REPEAT STRIP_TAC THEN REWRITE_TAC[real_div; SUM_CMUL] THEN
      ...
      GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV o LAND_CONV) [ADD_SYM] THEN
      REWRITE_TAC[SUM_REINDEX] THEN
  ...
  MP_TAC THENL
   [REPEAT GEN_TAC THEN DISCH_TAC THEN
    ...
    GEN_REWRITE_TAC "100/piseries.ml:(funpow 3 RAND_CONV o funpow 3 LAND_CONV)" (funpow 3 RAND_CONV o funpow 3 LAND_CONV)
      [ARITH_RULE `x = 0 + x`] THEN
    REWRITE_TAC[SUM_REINDEX] THEN
    ONCE_REWRITE_TAC
     [REWRITE_RULE[REAL_ARITH `(a = b - c) <=> (c + a = b)`] SUM_DIFF] THEN
    ASM_SIMP_TAC[ARITH_RULE `n < p ==> (n + p - 1 - n = p - 1)`] THEN
    GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o funpow 2 RAND_CONV o funpow 3 LAND_CONV)" (LAND_CONV o funpow 2 RAND_CONV o funpow 3 LAND_CONV)
     [ARITH_RULE `x = 0 + x`] THEN
    REWRITE_TAC[SUM_REINDEX] THEN
...
let SUM_SWAP = prove
 (`!m1 m2 n1 n2.
  ...
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV o LAND_CONV)
    [ARITH_RULE `m = 0 + m`] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o LAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV o LAND_CONV)
    [ARITH_RULE `m = 0 + m`] THEN
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o RAND_CONV o BINDER_CONV o LAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o BINDER_CONV o LAND_CONV o LAND_CONV)
    [ARITH_RULE `m = 0 + m`] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o RAND_CONV o BINDER_CONV o LAND_CONV o LAND_CONV)" (RAND_CONV o RAND_CONV o BINDER_CONV o LAND_CONV o LAND_CONV)
    [ARITH_RULE `m = 0 + m`] THEN
   ...
   [X_GEN_TAC `e:real` THEN DISCH_TAC THEN UNDISCH_TAC `z sums l` THEN
    ...
    MP_TAC THENL
     [SUBGOAL_THEN `&0 < e / (&2 * &(M + 1))` MP_TAC THENL
      ...
      SPEC_TAC(`M:num`,`n:num`) THEN
      GEN_REWRITE_TAC "100/piseries.ml:I" I [SWAP_FORALL_THM] THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LE_TRANS THEN
      ...
      CONV_TAC "100/piseries.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN ASM_REWRITE_TAC[]; ALL_TAC] THEN
    MATCH_MP_TAC REAL_LTE_TRANS THEN EXISTS_TAC `e / &2 + e / &2` THEN
    CONJ_TAC THENL
     [ALL_TAC;
      SIMP_TAC[REAL_LE_REFL; GSYM REAL_MUL_2; REAL_DIV_LMUL;
    ...
    EXISTS_TAC `sum(0,M) (\n. e / (&2 * &(M + 1)))` THEN CONJ_TAC THENL
     [MATCH_MP_TAC SUM_LE THEN CONV_TAC "100/piseries.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
 ...
 (`!x. &0 < abs(x) /\ abs(x) < &1
  ...
  REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [SIMP_TAC[REAL_POS; REAL_POW_LE; REAL_LE_MUL;
    ...
    GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o BINDER_CONV o LAND_CONV)" (LAND_CONV o BINDER_CONV o LAND_CONV)
...
let COT_POWSER_SQUARED_FORM = prove
 (`!x. &0 < abs(x) /\ abs(x) < pi
  ...
  REWRITE_TAC[] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o LAND_CONV o ABS_CONV o RAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o LAND_CONV o ABS_CONV o RAND_CONV o ONCE_DEPTH_CONV)
      [REAL_POW_DIV] THEN
  ...
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV)
...
let COT_X_POWSER = prove
 (`!x. &0 < abs(x) /\ abs(x) < pi
  ...
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
 ...
 (`!x. &0 < abs(x) /\ abs(x) < pi / &2
  ...
  SUBGOAL_THEN `~(sin x = &0)` ASSUME_TAC THENL
   [REWRITE_TAC[SIN_ZERO] THEN
    ...
    CONV_TAC "100/piseries.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[DE_MORGAN_THM] THEN
    REWRITE_TAC[OR_EXISTS_THM] THEN
    REWRITE_TAC[TAUT `a /\ b \/ a /\ c <=> a /\ (b \/ c)`] THEN
    DISCH_THEN(X_CHOOSE_THEN `n:num` MP_TAC) THEN
    DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
    ...
    DISJ1_TAC THEN
    GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [REAL_ARITH `x = &1 * x`] THEN
  ...
  GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV
...
let TAN_POWSER_WEAK = prove
 (`!x. &0 < abs(x) /\ abs(x) < pi / &2
  ...
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [REAL_MUL_ASSOC] THEN
  ...
  DISCH_THEN(MP_TAC o SPEC `inv(x)` o MATCH_MP SER_CMUL) THEN
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV)
...
let DIFF_CHAIN_TAN = prove
 (`~(cos x = &0)
  ...
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM REAL_POW2_ABS] THEN
...
let TAN_DERIV_POWSER = prove
 (`!n x. abs(x) < pi / &2
  ...
  REWRITE_TAC[ITER] THEN CONV_TAC "100/piseries.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
...
let HARMONICSUMS_TANNUMBER = prove
 (`!n. EVEN n /\ ~(n = 0)
  ...
  REWRITE_TAC[GSYM real_div] THEN CONV_TAC "100/piseries.ml:SYM_CONV" SYM_CONV THEN
...
let EVEN_POLY_DIFF = prove
 (`(!x. poly p (--x) = --(poly p x))
  ...
  REWRITE_TAC[] THEN
  GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [GSYM REAL_NEG_NEG] THEN
 ...
 (`!n x. (poly (tanpoly n) (--x) =
  ...
  INDUCT_TAC THENL
   [REWRITE_TAC[EVEN; tanpoly] THEN
    CONV_TAC "100/piseries.ml:(ONCE_DEPTH_CONV POLY_DIFF_CONV)" (ONCE_DEPTH_CONV POLY_DIFF_CONV) THEN
...
let TANNUMBER_EVEN = prove
 (`!n. EVEN n ==> (tannumber n = &0)`,
  ...
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM REAL_NEG_0] THEN
...
let TAYLOR_TAN_CONVERGES = prove
 (`!x. abs(x) < pi / &2
  ...
  GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN AP_TERM_TAC THEN
  CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_MUL_LID] THEN
 ...
 (`!x. &0 < abs(x) /\ abs(x) < pi
  ...
  ASM_CASES_TAC `ODD n` THEN ASM_REWRITE_TAC[] THENL
   [SUBGOAL_THEN `tannumber(n - 1) = &0`
    ...
    DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [th]) THEN
...
let TANNUMBER_BOUND = prove
 (`!n. abs(tannumber n) <= &4 * &(FACT n) * (&2 / pi) pow (n + 1)`,
  ...
  (CONV_TAC "100/piseries.ml:o" o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 5)
   ...
   [REWRITE_TAC[REAL_ABS_MUL; REAL_ABS_NUM] THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC(REAL_ARITH `(y = x) ==> x <= y`) THEN
      ...
      MATCH_MP_TAC SUMMABLE_INVERSE_POWERS THEN REWRITE_TAC[LE_REFL];
      ALL_TAC] THEN
    REWRITE_TAC[SUM_1; ADD_CLAUSES] THEN CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
 ...
 (`!x n. abs(x) <= &1
  ...
  SUBGOAL_THEN `~(&2 / pi * abs(x) = &1)` ASSUME_TAC THENL
   [UNDISCH_TAC `&2 / pi * abs x < &2 / &3` THEN
    ...
    SIMP_TAC[] THEN CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV; ALL_TAC] THEN
  GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [AC REAL_MUL_AC `(a * b) * c = (a * c) * b`] THEN
  MATCH_MP_TAC(REAL_ARITH `abs(x) <= a ==> x <= a`) THEN
  ONCE_REWRITE_TAC[REAL_ABS_MUL] THEN
  MATCH_MP_TAC REAL_LE_MUL2 THEN REWRITE_TAC[REAL_ABS_POS] THEN CONJ_TAC THENL
   [ALL_TAC;
  ...
  REWRITE_TAC[GSYM real_div] THEN CONJ_TAC THENL
   [MATCH_MP_TAC(REAL_ARITH
    ...
    (CONV_TAC "100/piseries.ml:o" o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 4)
  ...
  GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_INV] THEN
  MATCH_MP_TAC REAL_LE_INV2 THEN
  CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  MATCH_MP_TAC(REAL_ARITH
...
let TAYLOR_TANX_BOUND = prove
 (`!x n k. abs(x) <= inv(&2 pow k) /\ ~(x = &0)
  ...
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [REAL_MUL_SYM] THEN
  REWRITE_TAC[GSYM SUM_CMUL] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
  ...
  REWRITE_TAC[SUM_1] THEN
  CONV_TAC "100/piseries.ml:(ONCE_DEPTH_CONV TANNUMBER_CONV)" (ONCE_DEPTH_CONV TANNUMBER_CONV) THEN
  CONV_TAC "100/piseries.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[real_pow] THEN
  CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_SUB_RZERO] THEN
  MATCH_MP_TAC REAL_LE_TRANS THEN
 ...
 (`!x n. abs(x) <= &1 /\ ~(x = &0)
  ...
  REWRITE_TAC[] THEN CONJ_TAC THENL
   [MATCH_MP_TAC SUM_LE THEN
    ...
    REWRITE_TAC[real_div; REAL_INV_MUL; GSYM REAL_MUL_ASSOC] THEN
    GEN_REWRITE_TAC "100/piseries.ml:RAND_CONV" RAND_CONV [AC REAL_MUL_AC `a * b * c = (c * a) * b`] THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SUM_LE THEN
    ...
    GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [GSYM REAL_INV_INV] THEN
    REWRITE_TAC[GSYM REAL_INV_MUL] THEN
    MATCH_MP_TAC REAL_LE_INV2 THEN
    SIMP_TAC[REAL_LT_MUL; REAL_LT_INV_EQ; REAL_OF_NUM_LT;
             ARITH; REAL_POW_LT] THEN
    REWRITE_TAC[GSYM ADD1; ADD_CLAUSES; real_pow] THEN
    REWRITE_TAC[REAL_MUL_ASSOC] THEN CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  REWRITE_TAC[GSYM real_div] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [REAL_POW_ADD] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
  ...
  REWRITE_TAC[REAL_ABS_POS] THEN CONJ_TAC THENL
   [REWRITE_TAC[REAL_ABS_MUL; REAL_ABS_NUM] THEN
    ...
    MATCH_MP_TAC REAL_LE_LMUL THEN REWRITE_TAC[REAL_ABS_POS] THEN
    REWRITE_TAC[REAL_ABS_INV] THEN
    MATCH_MP_TAC REAL_LE_INV2 THEN CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    MP_TAC PI_APPROX_25_BITS THEN
    MATCH_MP_TAC(REAL_ARITH
     `b + e <= a ==> abs(p - a) <= e ==> b <= abs p`) THEN
    CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV; ALL_TAC] THEN
...
let TAYLOR_COT_BOUND = prove
 (`!x n k. abs(x) <= inv(&2 pow k) /\ ~(x = &0)
  ...
  GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
  MATCH_MP_TAC REAL_LE_LMUL THEN CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let TAYLOR_COTXX_BOUND = prove
 (`!x n k. abs(x) <= inv(&2 pow k) /\ ~(x = &0)
  ...
  REWRITE_TAC[GSYM REAL_ABS_MUL; REAL_SUB_RDISTRIB] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o RAND_CONV o LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV o RAND_CONV)
   [AC REAL_MUL_AC `(a * b) * c = b * a * c`] THEN
  ASM_SIMP_TAC[REAL_MUL_RINV; REAL_MUL_RID] THEN
  REWRITE_TAC[GSYM real_div] THEN
  REWRITE_TAC[GSYM REAL_SUB_RDISTRIB] THEN
  ...
  ASM_CASES_TAC `n = 0` THENL
   [ASM_REWRITE_TAC[] THEN CONV_TAC "100/piseries.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[sum] THEN
    ...
    GEN_REWRITE_TAC "100/piseries.ml:LAND_CONV" LAND_CONV [GSYM REAL_INV_DIV] THEN
    REWRITE_TAC[REAL_ABS_INV] THEN
    MATCH_MP_TAC REAL_LE_INV2 THEN
  ...
  REWRITE_TAC[real_div; REAL_MUL_LZERO; REAL_ADD_RID] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o LAND_CONV o RAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV o RAND_CONV)
        [REAL_MUL_SYM] THEN
  REWRITE_TAC[GSYM SUM_CMUL] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(RAND_CONV o LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)
        [REAL_MUL_SYM] THEN
  REWRITE_TAC[GSYM real_div] THEN
  MATCH_MP_TAC(REAL_ARITH
  ...
  CONV_TAC "100/piseries.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN
  MATCH_MP_TAC REAL_LE_LMUL THEN REWRITE_TAC[REAL_POS] THEN
  REWRITE_TAC[real_div; REAL_MUL_LID] THEN
  GEN_REWRITE_TAC "100/piseries.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
  REWRITE_TAC[REAL_POW_MUL; REAL_POW_INV] THEN
