...
let NEARZETA_BOUND_LEMMA = prove
 (`!s n. ~(n = 0) /\ &0 <= Re s + &1
   ...
   [REWRITE_TAC[CONVEX_SEGMENT] THEN CONJ_TAC THENL
    ...
    THENL
     [FIRST_X_ASSUM(DISJ_CASES_THEN SUBST_ALL_TAC o MATCH_MP
      ...
      CONV_TAC "100/pnt.ml:COMPLEX_FIELD" COMPLEX_FIELD;
      ALL_TAC] THEN
  ...
  CONV_TAC "100/pnt.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  CONV_TAC "100/pnt.ml:COMPLEX_RING" COMPLEX_RING);;
...
let ZETADIFF_CONVERGES = prove
 (`!n s. &0 < Re(s)
  ...
  MATCH_MP_TAC REAL_EQ_IMP_LE THEN CONV_TAC "100/pnt.ml:SYM_CONV" SYM_CONV THEN
...
let COMPLEX_DERIVATIVE_ZETA_CONVERGES = prove
 (`!s. &1 < Re s
  ...
  REWRITE_TAC[OPEN_HALFSPACE_RE_GT; IN_ELIM_THM] THEN ANTS_TAC THENL
   [CONJ_TAC THENL
     [REWRITE_TAC[IN_FROM] THEN REPEAT STRIP_TAC THEN COMPLEX_DIFF_TAC THEN
      ...
      CONJ_TAC THENL [CONV_TAC "100/pnt.ml:COMPLEX_FIELD" COMPLEX_FIELD; ALL_TAC] THEN
  ...
  CONV_TAC "100/pnt.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC HAS_COMPLEX_DERIVATIVE_DERIVATIVE THEN
  FIRST_ASSUM(MP_TAC o SPEC `s:complex`) THEN SIMP_TAC[ASSUME `&1 < Re s`] THEN
 ...
 (`!n. 1 <= n
  ...
  REWRITE_TAC[OPEN_HALFSPACE_RE_GT] THEN CONJ_TAC THENL
   [MAP_EVERY X_GEN_TAC [`m:num`; `s:complex`] THEN
    ...
    CONJ_TAC THENL [ALL_TAC; CONV_TAC "100/pnt.ml:COMPLEX_FIELD" COMPLEX_FIELD] THEN
...
let HOLOMORPHIC_NEARZETA_STRONG = prove
 (`!n s. 1 <= n /\ &0 < Re s
  ...
  GEN_REWRITE_TAC "100/pnt.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
  ...
  ASSUME_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    X_GEN_TAC `w:complex` THEN DISCH_TAC THEN CONV_TAC "100/pnt.ml:SYM_CONV" SYM_CONV THEN
...
let HOLOMORPHIC_ZETA = prove
 (`zeta holomorphic_on {s | Re(s) > &0 /\ ~(s = Cx(&1))}`,
  GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN REWRITE_TAC[zeta; genzeta] THEN
...
let SERIES_DIVISORS_LEMMA = prove
 (`!x p l k.
  ...
  GEN_REWRITE_TAC "100/pnt.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM o_DEF] THEN
 ...
 (`!s ps. &1 < Re s /\ FINITE ps /\ (!p. p IN ps ==> prime p)
  ...
  let lemma = prove
   (`(x sums (k + l)) (s UNION t) /\ s INTER t = {}
    ...
    CONV_TAC "100/pnt.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC VSUM_DIFF THEN
...
let ZETA_NONZERO_LEMMA = prove
 (`!s. &1 < Re s ==> ~(zeta s = Cx(&0))`,
  ...
  CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let EULER_PRODUCT = prove
 (`!s. &1 < Re s
       ==> ((\n. cproduct {p | prime p /\ p <= n}
            ...
            --> zeta(s)) sequentially`,
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "100/pnt.ml:(PAT_CONV `\x. ((\n. x) --> x) sq`)" (PAT_CONV `\x. ((\n. x) --> x) sq`)
 ...
 (`~(zeta(Cx(&1)) = Cx(&0))`,
  ...
  MP_TAC THENL
   [REWRITE_TAC[RE_DEF] THEN
    ...
    GEN_REWRITE_TAC "100/pnt.ml:I" I [GSYM REAL_EXP_MONO_LT] THEN
    SIMP_TAC[EXP_LOG; REAL_OF_NUM_LT; ARITH] THEN
    SUBGOAL_THEN `(&1 + &1 / &2) pow 2 <= exp(&1 / &2) pow 2` MP_TAC THENL
     [MATCH_MP_TAC REAL_POW_LE2 THEN
      CONJ_TAC THENL [CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV; ALL_TAC] THEN
...
let ZETA_MULTIPLE_BOUND = prove
 (`!x y. real x /\ real y /\ &1 < Re x
  ...
  GEN_REWRITE_TAC "100/pnt.ml:BINOP_CONV" BINOP_CONV [GSYM REAL_INV_INV] THEN
  MATCH_MP_TAC REAL_LE_INV2 THEN
  ...
  ASM_SIMP_TAC[REAL_POW_LE; REAL_SUB_LE; lemma2] THEN CONJ_TAC THENL
   [REWRITE_TAC[REAL_ARITH
    ...
    REWRITE_TAC[REAL_ARITH `-- &1 <= &2 * x pow 2 - &1 <=> &0 <= x * x`;
                ...
                REAL_LE_SQUARE] THEN
    GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [GSYM REAL_POW2_ABS] THEN
 ...
 (`!s. &1 <= Re s ==> ~(zeta s = Cx(&0))`,
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    ...
    GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [GSYM o_DEF] THEN
...
let NEARZETA_NONZERO = prove
 (`!s. &1 <= Re s ==> ~(nearzeta 1 s + Cx (&1) = Cx(&0))`,
  ...
  REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "100/pnt.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
...
let LOGZETA_EXISTS = prove
 (`?logzeta logzeta'.
  ...
  CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN
  MATCH_MP_TAC MONO_EXISTS THEN X_GEN_TAC `N:num` THEN
 ...
 (`!s. &1 < Re s ==> cexp(--(logzeta s)) = zeta s`,
  ...
  REWRITE_TAC[TRIVIAL_LIMIT_SEQUENTIALLY] THEN CONJ_TAC THENL
   [MP_TAC(ISPECL [`cexp`; `--logzeta s`] CONTINUOUS_AT_SEQUENTIALLY) THEN
    ...
    AP_TERM_TAC THEN CONV_TAC "100/pnt.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CEXP_CLOG THEN
    ...
    CONV_TAC "100/pnt.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
 ...
 (`!s. &1 < Re s ==> (zeta has_complex_derivative
  ...
  ASM_REWRITE_TAC[REAL_SUB_LT] THEN CONJ_TAC THENL
   [ONCE_REWRITE_TAC[DIST_SYM] THEN REWRITE_TAC[GSYM IN_BALL] THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "100/pnt.ml:(RATOR_CONV o LAND_CONV)" (RATOR_CONV o LAND_CONV) [GSYM o_DEF] THEN
...
let BOUND_LEMMA_2 = prove
 (`!R x z. Re(z) = --x /\ abs(Im(z)) = R /\ &0 <= x /\ &0 < R
  ...
  CONV_TAC "100/pnt.ml:(RAND_CONV REAL_POLY_CONV)" (RAND_CONV REAL_POLY_CONV) THEN
 ...
 (`!a n. (!m. 1 <= m ==> norm(a(m)) <= &1) /\
  ...
  and lemma1' = prove
   (`!n x.
    ...
    ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
     [CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN CONJ_TAC THENL
       [X_GEN_TAC `u:complex` THEN STRIP_TAC THEN COMPLEX_DIFF_TAC THEN
        CONJ_TAC THENL
         [SUBGOAL_THEN `?y. u = Cx y` (CHOOSE_THEN SUBST_ALL_TAC) THENL
           [ASM_MESON_TAC[REAL_SEGMENT; REAL_CX; REAL]; ALL_TAC] THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC VSUM_NORM_LE THEN REWRITE_TAC[FINITE_NUMSEG; IN_NUMSEG] THEN
    ...
    GEN_REWRITE_TAC "100/pnt.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
 ...
 (`!f a. (!n. 1 <= n ==> norm(a(n)) <= &1) /\
   ...
   [SUBGOAL_THEN
    ...
    MP_TAC THENL
     [X_GEN_TAC `z:complex` THEN DISCH_TAC THEN
      ...
      ALL_TAC] THEN
    GEN_REWRITE_TAC "100/pnt.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
  ...
  SUBGOAL_THEN `winding_number(A++B,Cx(&0)) = Cx(&1)` ASSUME_TAC THENL
   [MATCH_MP_TAC WINDING_NUMBER_EQ_1 THEN
    ...
    ASM_SIMP_TAC[] THEN (CONJ_TAC THENL [CONV_TAC "100/pnt.ml:COMPLEX_FIELD" COMPLEX_FIELD; ALL_TAC]) THEN
   ...
   [MP_TAC(ISPECL
    ...
    REWRITE_TAC[COMPLEX_MUL_RID] THEN ANTS_TAC THENL
     [ALL_TAC;
      ...
      ABBREV_TAC `wever = inv(Cx R pow 2)` THEN CONV_TAC "100/pnt.ml:COMPLEX_FIELD" COMPLEX_FIELD] THEN
    CONJ_TAC THENL
  ...
  (LABEL_TAC "s'A") THENL
   [SUBGOAL_THEN `(A:real^1->complex) = (--) o (--) o A` SUBST1_TAC THENL
    ...
    GEN_REWRITE_TAC "100/pnt.ml:(RATOR_CONV o LAND_CONV o ABS_CONV)" (RATOR_CONV o LAND_CONV o ABS_CONV)
   ...
   [MP_TAC(ISPECL
    ...
    ANTS_TAC THENL
     [MAP_EVERY UNDISCH_TAC [`~(N = 0)`; `N + 1 <= n`] THEN ARITH_TAC;
      ALL_TAC] THEN
    DISCH_THEN(SUBST1_TAC o SYM) THEN CONV_TAC "100/pnt.ml:SYM_CONV" SYM_CONV THEN
   ...
   [MP_TAC(SPECL
    ...
    ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
     [ALL_TAC;
      ...
      CONV_TAC "100/pnt.ml:REAL_FIELD" REAL_FIELD] THEN
    CONJ_TAC THENL
     [REPEAT(MATCH_MP_TAC REAL_LE_MUL THEN CONJ_TAC) THEN
  ...
  REWRITE_TAC[VALID_PATH_LINEPATH; SUBSET_UNIV; IN_UNIV] THEN ANTS_TAC THENL
   [X_GEN_TAC `z:complex` THEN COMPLEX_DIFF_TAC THEN
    ...
    SUBGOAL_THEN `~(clog(Cx(&N)) = Cx(&0))` MP_TAC THENL
     [ALL_TAC; CONV_TAC "100/pnt.ml:COMPLEX_FIELD" COMPLEX_FIELD] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [EXPAND_TAC "integral_bound" THEN
    ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
    GEN_REWRITE_TAC "100/pnt.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_MUL_LID] THEN
  ...
  (SUBGOAL_THEN
   ...
   GEN_REWRITE_TAC "100/pnt.ml:(RAND_CONV o TOP_DEPTH_CONV)" (RAND_CONV o TOP_DEPTH_CONV)
   ...
   ASM_SIMP_TAC[REAL_ARITH `&0 < d ==> abs d = d`; REAL_LE_RMUL_EQ;
                REAL_MUL_ASSOC] THEN
   GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV
  ...
  ANTS_TAC THENL
   [CONJ_TAC THENL
     [X_GEN_TAC `z:complex` THEN DISCH_TAC THEN COMPLEX_DIFF_TAC THEN
      ...
      MATCH_MP_TAC(TAUT `(a ==> b) /\ a ==> a /\ b`) THEN
      CONJ_TAC THENL
       [UNDISCH_TAC `~(z = Cx(&0))` THEN CONV_TAC "100/pnt.ml:COMPLEX_FIELD" COMPLEX_FIELD;
...
let NEARNEWMAN_EXISTS = prove
 (`?f. !s. s IN {s | Re(s) > &1 / &2}
  ...
  GEN_REWRITE_TAC "100/pnt.ml:RAND_CONV" RAND_CONV [REAL_ADD_RDISTRIB] THEN
  MATCH_MP_TAC(NORM_ARITH
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[CPOW_ADD; CX_ADD; CPOW_N; CX_INJ; REAL_OF_NUM_EQ] THEN
    ...
    CONJ_TAC THENL
     [GEN_REWRITE_TAC "100/pnt.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_INV] THEN
      MATCH_MP_TAC REAL_LE_INV2 THEN
      MP_TAC(SPEC `s - z:complex` COMPLEX_NORM_GE_RE_IM) THEN
      REWRITE_TAC[RE_SUB] THEN ASM_REAL_ARITH_TAC;
      ALL_TAC] THEN
  ...
  EXISTS_TAC `clog(Cx(&p)) / Cx(&p) cpow (z + z)` THEN CONJ_TAC THENL
   [REWRITE_TAC[CPOW_ADD; complex_div; COMPLEX_MUL_ASSOC; COMPLEX_INV_MUL] THEN
    ...
    REWRITE_TAC[real_div; GSYM REAL_MUL_ASSOC] THEN
    MATCH_MP_TAC REAL_LE_LMUL THEN REWRITE_TAC[NORM_POS_LE] THEN
    GEN_REWRITE_TAC "100/pnt.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_INV_INV] THEN
    ...
    CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_LE_REFL] THEN
    MATCH_MP_TAC REAL_LE_MUL2 THEN
 ...
 (`!s. &0 < Re s /\ ~(s = Cx(&1))
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    ...
    CONV_TAC "100/pnt.ml:COMPLEX_FIELD" COMPLEX_FIELD] THEN
  GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [GSYM COMPLEX_ADD_RID] THEN
 ...
 (`?a. (\z. if z = Cx(&1) then a
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC ANALYTIC_ON_MUL THEN CONJ_TAC THENL
    ...
    UNDISCH_TAC `~(w = Cx(&1))` THEN CONV_TAC "100/pnt.ml:COMPLEX_FIELD" COMPLEX_FIELD]);;
 ...
 (`?c a. (\z. if z = Cx(&1) then a
  ...
  SIMP_TAC[NEARZETA_1; LE_REFL] THEN REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC ANALYTIC_ON_ADD THEN CONJ_TAC THENL
    ...
    GEN_REWRITE_TAC "100/pnt.ml:(funpow 4 RAND_CONV o ONCE_DEPTH_CONV)" (funpow 4 RAND_CONV o ONCE_DEPTH_CONV) [zeta] THEN
...
let GENZETA_OFFSET = prove
 (`!m n s. &1 < Re s /\ m <= n
  ...
  GEN_REWRITE_TAC "100/pnt.ml:(PAT_CONV `\n. (f sums (a - vsum(m..n) s)) k`)" (PAT_CONV `\n. (f sums (a - vsum(m..n) s)) k`)
...
let NEWMAN_CONVERGES = prove
 (`!s. &1 < Re s
  ...
  GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [sums] THEN
...
let PNT_PARTIAL_SUMMATION = prove
 (`&(CARD {p | prime p /\ p <= n}) =
  ...
  GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_RID] THEN
  ...
  FIRST_ASSUM(fun th ->
   GEN_REWRITE_TAC "100/pnt.ml:(PAT_CONV `\x. l = a * (sum(1..x) f - s)`)" (PAT_CONV `\x. l = a * (sum(1..x) f - s)`)
   ...
   [REPEAT STRIP_TAC THEN
    ...
    REWRITE_TAC[IN_NUMSEG; FINITE_NUMSEG] THEN ANTS_TAC THENL
     [REPEAT STRIP_TAC THEN
      ...
      ALL_TAC] THEN
    GEN_REWRITE_TAC "100/pnt.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [REAL_SUB_RDISTRIB] THEN
    REWRITE_TAC[SUM_SUB_NUMSEG] THEN
    GEN_REWRITE_TAC "100/pnt.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [SUM_PARTIAL_SUC] THEN
    GEN_REWRITE_TAC "100/pnt.ml:(LAND_CONV o LAND_CONV o RAND_CONV o RAND_CONV)" (LAND_CONV o LAND_CONV o RAND_CONV o RAND_CONV)
...
let REALLIM_NA_OVER_N = prove
 (`!a. ((\n. (&n + a) / &n) ---> &1) sequentially`,
  ...
  GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [GSYM REAL_ADD_RID] THEN
  MATCH_MP_TAC REALLIM_ADD THEN CONJ_TAC THENL
   [MATCH_MP_TAC REALLIM_TRANSFORM_EVENTUALLY THEN
    EXISTS_TAC `\n:num. &1` THEN REWRITE_TAC[REALLIM_CONST] THEN
    REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN EXISTS_TAC `1` THEN
    REWRITE_TAC[GSYM REAL_OF_NUM_LE] THEN CONV_TAC "100/pnt.ml:REAL_FIELD" REAL_FIELD;
...
let REALLIM_N_OVER_NA = prove
 (`!a. ((\n. &n / (&n + &1)) ---> &1) sequentially`,
  ...
  GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [GSYM REAL_INV_1] THEN
  MATCH_MP_TAC REALLIM_INV THEN
  REWRITE_TAC[REALLIM_NA_OVER_N] THEN CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
 ...
 (`((\n. log(&n + &1) / log(&n)) ---> &1) sequentially`,
  ...
  EXISTS_TAC `\n. &1 + log(&1 + &1 / &n) / log(&n)` THEN CONJ_TAC THENL
   [REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN EXISTS_TAC `2` THEN
    ...
    AP_TERM_TAC THEN POP_ASSUM MP_TAC THEN CONV_TAC "100/pnt.ml:REAL_FIELD" REAL_FIELD;
    ALL_TAC] THEN
  GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [GSYM REAL_ADD_RID] THEN
  ...
  GEN_REWRITE_TAC "100/pnt.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
  MATCH_MP_TAC REAL_LE_MUL2 THEN REWRITE_TAC[REAL_ABS_POS] THEN
  CONJ_TAC THENL
  ...
  MATCH_MP_TAC LOG_MONO_LE_IMP THEN CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let REALLIM_LOG_OVER_LOG1 = prove
 (`((\n. log(&n) / log(&n + &1)) ---> &1) sequentially`,
  ...
  GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [GSYM REAL_INV_1] THEN
  MATCH_MP_TAC REALLIM_INV THEN
  REWRITE_TAC[REALLIM_LOG1_OVER_LOG] THEN CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let ADHOC_BOUND_LEMMA = prove
 (`!k. 1 <= k ==> abs((&k + &1) * (log(&k + &1) - log(&k)) - &1)
  ...
  CONV_TAC "100/pnt.ml:(ONCE_DEPTH_CONV NUMSEG_CONV)" (ONCE_DEPTH_CONV NUMSEG_CONV) THEN
  SIMP_TAC[VSUM_CLAUSES; FINITE_INSERT; FINITE_RULES] THEN
  REWRITE_TAC[IN_INSERT; NOT_IN_EMPTY; ARITH] THEN
  CONV_TAC "100/pnt.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
...
let REALLIM_MUL_SERIES = prove
 (`!x y z B.
  ...
  GEN_REWRITE_TAC "100/pnt.ml:LAND_CONV" LAND_CONV [EVENTUALLY_SEQUENTIALLY] THEN
 ...
 (`((\n. &(CARD {p | prime p /\ p <= n}) / (&n / log(&n)))
   ...
   [REWRITE_TAC[REAL_ARITH
    ...
    REWRITE_TAC[REALLIM_LOG_OVER_N] THEN CONJ_TAC THENL
     [ALL_TAC;
      ...
      CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN MATCH_MP_TAC EQ_IMP THEN
      AP_THM_TAC THEN AP_THM_TAC THEN AP_TERM_TAC THEN
    ...
    GEN_REWRITE_TAC "100/pnt.ml:RAND_CONV" RAND_CONV [GSYM REAL_SUB_LE] THEN
  ...
  MATCH_MP_TAC(TAUT `b /\ (b ==> a) ==> a /\ b`) THEN CONJ_TAC THENL
   [MATCH_MP_TAC REALLIM_TRANSFORM_STRADDLE THEN
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN CONJ_TAC THENL
     [X_GEN_TAC `n:num` THEN DISCH_TAC THEN CONJ_TAC THENL
      ...
      CONV_TAC "100/pnt.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THENL
       [MATCH_MP_TAC REAL_LE_TRANS THEN EXISTS_TAC `&4 * log(&2)` THEN
        CONJ_TAC THENL [MP_TAC LOG_2_BOUNDS THEN REAL_ARITH_TAC; ALL_TAC] THEN
        SIMP_TAC[GSYM LOG_POW; REAL_OF_NUM_LT; ARITH] THEN
      ...
      MATCH_MP_TAC REALLIM_ADD THEN CONJ_TAC THENL
       [MATCH_MP_TAC REALLIM_TRANSFORM_EVENTUALLY THEN
        ...
        SUBGOAL_THEN `&0 < log(&n + &1)` ASSUME_TAC THENL
         [ALL_TAC; POP_ASSUM MP_TAC THEN CONV_TAC "100/pnt.ml:REAL_FIELD" REAL_FIELD] THEN
