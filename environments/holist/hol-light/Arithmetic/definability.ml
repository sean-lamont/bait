     ...
     [REPEAT STRIP_TAC THEN
      ...
      REPEAT CONJ_TAC THENL
       [MATCH_MP_TAC LE_TRANS THEN EXISTS_TAC `y:num` THEN
        ASM_REWRITE_TAC[] THEN
        GEN_REWRITE_TAC "Arithmetic/definability.ml:LAND_CONV" LAND_CONV [ARITH_RULE `y = 1 * y`] THEN
...
let RELPOW_LEMMA = prove
 (`RELPOW n R x y <=>
  ...
  GEN_REWRITE_TAC "Arithmetic/definability.ml:RAND_CONV" RAND_CONV [SWAP_EXISTS_THM] THEN
 ...
 (`RTC R x y <=>
  ...
  REWRITE_TAC[RTC_RELPOW] THEN EQ_TAC THENL
   [DISCH_THEN(X_CHOOSE_THEN `n:num` MP_TAC) THEN
    ...
    GEN_REWRITE_TAC "Arithmetic/definability.ml:(LAND_CONV o funpow 3 BINDER_CONV)" (LAND_CONV o funpow 3 BINDER_CONV)
     [LEFT_AND_EXISTS_THM] THEN
    GEN_REWRITE_TAC "Arithmetic/definability.ml:(LAND_CONV o BINDER_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV o BINDER_CONV)
     [SWAP_EXISTS_THM] THEN
    REWRITE_TAC[UNWIND_THM2] THEN
    GEN_REWRITE_TAC "Arithmetic/definability.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [SWAP_EXISTS_THM] THEN
    GEN_REWRITE_TAC "Arithmetic/definability.ml:LAND_CONV" LAND_CONV [SWAP_EXISTS_THM] THEN
...
let FV_TAC ths =
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Arithmetic/definability.ml:I" I [EXTENSION] THEN
...
let ARITH_PRIMREC = prove
 (`!fn e f R c.
  ...
  CONV_TAC "Arithmetic/definability.ml:(TOP_DEPTH_CONV ETA_CONV)" (TOP_DEPTH_CONV ETA_CONV) THEN
