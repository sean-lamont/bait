  ...
  let create_equality_axioms =
    let eq_thms = (CONJUNCTS o prove)
     (`(x:A = x) /\
      ...
      ASM_REWRITE_TAC[] THEN CONV_TAC "meson.ml:TAUT" TAUT) in
...
let GEN_MESON_TAC min max step ths =
  ...
  CONV_TAC "meson.ml:(PRESIMP_CONV THENC TOP_DEPTH_CONV BETA_CONV THENC LAMBDA_ELIM_CONV THENC CONDS_CELIM_CONV THENC Meson.QUANT_BOOL_CONV)" (PRESIMP_CONV THENC TOP_DEPTH_CONV BETA_CONV THENC LAMBDA_ELIM_CONV THENC CONDS_CELIM_CONV THENC Meson.QUANT_BOOL_CONV) THEN
