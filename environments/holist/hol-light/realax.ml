...
let DIST_ELIM_THM = prove
 (`P(dist(x,y)) <=> !d. ((x = y + d) ==> P(d)) /\ ((y = x + d) ==> P(d))`,
  ...
  GEN_REWRITE_TAC "realax.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [EQ_SYM_EQ] THEN
...
let DIST_LE_CASES,DIST_ADDBOUND,DIST_TRIANGLE,DIST_ADD2,DIST_ADD2_REV =
  let DIST_ELIM_TAC =
    ...
    CONV_TAC "realax.ml:conv" conv THEN TRY GEN_TAC THEN CONJ_TAC THEN
  ...
  let DIST_ELIM_TAC' =
    ...
    CONV_TAC "realax.ml:(LAND_CONV NUM_CANCEL_CONV)" (LAND_CONV NUM_CANCEL_CONV) THEN
...
let DIST_TRIANGLES_LE = prove
 (`!m n p q r s.
  ...
  EXISTS_TAC `n:num` THEN GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [ADD_SYM] THEN
  REWRITE_TAC[GSYM ADD_ASSOC] THEN MATCH_MP_TAC LE_ADD2 THEN
  ASM_REWRITE_TAC[] THEN MATCH_MP_TAC DIST_TRIANGLE_LE THEN
  EXISTS_TAC `q:num` THEN GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [ADD_SYM] THEN
...
let BOUNDS_LINEAR = prove
 (`!A B C. (!n. A * n <= B * n + C) <=> A <= B`,
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [CONV_TAC "realax.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_LE] THEN
 ...
 (`!P. (?B. !n. P(n) <= B) <=>
  ...
  GEN_TAC THEN EQ_TAC THEN STRIP_TAC THENL
   [MAP_EVERY EXISTS_TAC [`B:num`; `0`] THEN
    ...
    GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [MULT_SYM] THEN
    ...
    GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [ADD_SYM] THEN
    GEN_REWRITE_TAC "realax.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [MULT_SYM] THEN
 ...
 (`!x. ?B. !m n. dist(fn x (m * n),m * fn x n) <= B * m + B`,
  ...
  ASM_CASES_TAC `n = 0` THENL
   [MATCH_MP_TAC (LE_IMP DIST_ADDBOUND) THEN
    ...
    REWRITE_TAC[LE_EXISTS] THEN CONV_TAC "realax.ml:(ONCE_DEPTH_CONV NUM_CANCEL_CONV)" (ONCE_DEPTH_CONV NUM_CANCEL_CONV) THEN
    REWRITE_TAC[GSYM EXISTS_REFL]; UNDISCH_TAC `~(n = 0)`] THEN
  REWRITE_TAC[TAUT `(~a ==> b) <=> a \/ b`; GSYM LE_MULT_LCANCEL;
              DIST_LMUL] THEN
  REWRITE_TAC[MULT_ASSOC] THEN GEN_REWRITE_TAC "realax.ml:(LAND_CONV o RAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o RAND_CONV o LAND_CONV) [MULT_SYM] THEN
  POP_ASSUM(MATCH_MP_TAC o LE_IMP) THEN
  REWRITE_TAC[LE_EXISTS; RIGHT_ADD_DISTRIB; LEFT_ADD_DISTRIB; MULT_AC] THEN
  CONV_TAC "realax.ml:(ONCE_DEPTH_CONV NUM_CANCEL_CONV)" (ONCE_DEPTH_CONV NUM_CANCEL_CONV) THEN
...
let NADD_ADDITIVE = prove
 (`!x. ?B. !m n. dist(fn x (m + n),fn x m + fn x n) <= B`,
  ...
  GEN_REWRITE_TAC "realax.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV) [RIGHT_ADD_DISTRIB] THEN
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[SYM(REWRITE_CONV [ARITH] `1 + 1 + 1`)] THEN
    ...
    REWRITE_TAC[MULT_AC] THEN CONV_TAC "realax.ml:NUM_CANCEL_CONV" NUM_CANCEL_CONV THEN REFL_TAC;
...
let NADD_ALTMUL = prove
 (`!x y. ?A B. !n. dist(n * fn x (fn y n),fn x n * fn y n) <= A * n + B`,
  ...
  GEN_REWRITE_TAC "realax.ml:(LAND_CONV o RAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV o RAND_CONV) [MULT_SYM] THEN
...
let NADD_EQ_SYM = prove
 (`!x y. x === y <=> y === x`,
  ...
  GEN_REWRITE_TAC "realax.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [DIST_SYM] THEN REFL_TAC);;
...
let NADD_LE_TOTAL_LEMMA = prove
 (`!x y. ~(x <<= y) ==> !B. ?n. ~(n = 0) /\ fn y n + B < fn x n`,
  ...
  CONV_TAC "realax.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_LT] THEN
  DISCH_THEN(MATCH_MP_TAC o LE_IMP) THEN REWRITE_TAC[ADD_ASSOC; LE_ADD]);;

let NADD_LE_TOTAL = prove
 (`!x y. x <<= y \/ y <<= x`,
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC "realax.ml:I" I [TAUT `a <=> ~ ~ a`] THEN
  ...
  CONJ_TAC THEN GEN_REWRITE_TAC "realax.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [MULT_SYM] THEN
...
let NADD_ADD = prove
 (`!x y. fn(x ++ y) = \n. fn x n + fn y n`,
  ...
  GEN_REWRITE_TAC "realax.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [LEFT_ADD_DISTRIB] THEN
...
let NADD_ADD_SYM = prove
 (`!x y. (x ++ y) === (y ++ x)`,
  ...
  GEN_REWRITE_TAC "realax.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [ADD_SYM] THEN
 ...
 (`!x y. x <<= y ==> ?d. y === x ++ d`,
  ...
  SUBGOAL_THEN `fn(afn d) = d` SUBST1_TAC THENL
   [REWRITE_TAC[GSYM nadd_rep; is_nadd] THEN
    ...
    GEN_REWRITE_TAC "realax.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [LEFT_ADD_DISTRIB] THEN
    MATCH_MP_TAC(LE_IMP DIST_ADD2) THEN REWRITE_TAC[RIGHT_ADD_DISTRIB] THEN
    GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [ADD_SYM] THEN MATCH_MP_TAC LE_ADD2 THEN
    ONCE_REWRITE_TAC[ADD_SYM] THEN ASM_REWRITE_TAC[] THEN
    GEN_REWRITE_TAC "realax.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [MULT_SYM] THEN
...
let NADD_MUL_SYM = prove
 (`!x y. (x ** y) === (y ** x)`,
  ...
  GEN_REWRITE_TAC "realax.ml:(LAND_CONV o funpow 2 RAND_CONV)" (LAND_CONV o funpow 2 RAND_CONV) [MULT_SYM] THEN
  ASM_REWRITE_TAC[]);;
...
let NADD_LE_RADD = prove
 (`!x y z. x ++ z <<= y ++ z <=> x <<= y`,
  REPEAT GEN_TAC THEN REWRITE_TAC[nadd_le; NADD_ADD] THEN
  GEN_REWRITE_TAC "realax.ml:(LAND_CONV o funpow 2 BINDER_CONV o RAND_CONV)" (LAND_CONV o funpow 2 BINDER_CONV o RAND_CONV)
    [ADD_SYM] THEN
  REWRITE_TAC[ADD_ASSOC; LE_ADD_RCANCEL] THEN
  GEN_REWRITE_TAC "realax.ml:(LAND_CONV o funpow 2 BINDER_CONV o RAND_CONV)" (LAND_CONV o funpow 2 BINDER_CONV o RAND_CONV)
...
let NADD_ARCH_ZERO = prove
 (`!x k. (!n. &n ** x <<= k) ==> (x === &0)`,
  REPEAT GEN_TAC THEN CONV_TAC "realax.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_TAC THEN
 ...
 (`!P. (?x. P x) /\ (?M. !x. P x ==> x <<= M) ==>
  ...
  EXISTS_TAC `afn r` THEN SUBGOAL_THEN `fn(afn r) = r` ASSUME_TAC THENL
   [REWRITE_TAC[GSYM nadd_rep] THEN REWRITE_TAC[is_nadd; DIST_LE_CASES] THEN
    ...
    GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [SWAP_FORALL_THM] THEN
    GEN_REWRITE_TAC "realax.ml:(LAND_CONV o funpow 2 BINDER_CONV o funpow 2 RAND_CONV)" (LAND_CONV o funpow 2 BINDER_CONV o funpow 2 RAND_CONV) [ADD_SYM] THEN
   ...
   [X_GEN_TAC `x:nadd` THEN DISCH_TAC THEN
    ...
    EXISTS_TAC `&(SUC(r(n:num)))` THEN CONJ_TAC THENL
     [FIRST_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[];
      ...
      GEN_REWRITE_TAC "realax.ml:LAND_CONV" LAND_CONV [MULT_SYM] THEN
      MATCH_MP_TAC LE_TRANS THEN EXISTS_TAC `n * r(i:num) + n` THEN
    ...
    EXISTS_TAC `&(r(n:num)) ++ &1` THEN CONJ_TAC THENL
     [ASM_REWRITE_TAC[nadd_le; NADD_ADD; NADD_MUL; NADD_OF_NUM] THEN
      ...
      GEN_TAC THEN GEN_REWRITE_TAC "realax.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [MULT_SYM] THEN
      ASM_REWRITE_TAC[];
...
let NADD_UBOUND = prove
 (`!x. ?B N. !n. N <= n ==> fn x n <= B * n`,
  ...
  REWRITE_TAC[RIGHT_ADD_DISTRIB; LE_ADD_LCANCEL] THEN
  GEN_REWRITE_TAC "realax.ml:LAND_CONV" LAND_CONV [GSYM(el 3 (CONJUNCTS MULT_CLAUSES))] THEN
  ASM_REWRITE_TAC[LE_MULT_LCANCEL]);;
 ...
 (`!x. ~(x === &0) ==> ?A B. !n. nadd_rinv x n <= A * n + B`,
  ...
  ASM_REWRITE_TAC[LE_MULT_LCANCEL] THEN CONJ_TAC THENL
   [DISJ2_TAC THEN FIRST_ASSUM MATCH_MP_TAC THEN MATCH_MP_TAC LE_TRANS THEN
    ...
    DISCH_THEN(fun t -> GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [CONJUNCT1(SPEC_ALL t)]) THEN
    GEN_REWRITE_TAC "realax.ml:LAND_CONV" LAND_CONV [MULT_SYM] THEN REWRITE_TAC[LE_ADD]]);;
...
let NADD_MUL_LINV_LEMMA1 = prove
 (`!x n. ~(fn x n = 0) ==> dist(fn x n * nadd_rinv(x) n, n * n) <= fn x n`,
  ...
  GEN_REWRITE_TAC "realax.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV) [MULT_SYM] THEN
...
let NADD_MUL_LINV_LEMMA4 = prove
 (`!x. ~(x === &0) ==> ?N. !m n. N <= m /\ N <= n ==>
  ...
  GEN_REWRITE_TAC "realax.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [DIST_SYM] THEN
...
let NADD_MUL_LINV_LEMMA5 = prove
 (`!x. ~(x === &0) ==> ?B N. !m n. N <= m /\ N <= n ==>
  ...
  CONJ_TAC THENL
   [GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [MULT_SYM] THEN
    GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [GSYM MULT_ASSOC] THEN
    GEN_REWRITE_TAC "realax.ml:(funpow 2 RAND_CONV)" (funpow 2 RAND_CONV) [MULT_SYM] THEN
    ASM_REWRITE_TAC[LE_MULT_LCANCEL];
 ...
 (`!x. ~(x === &0) ==> ?B N. !m n. N <= m /\ N <= n ==>
  ...
  SUBGOAL_THEN `N <= m /\ N <= n` MP_TAC THENL
   [CONJ_TAC THEN MATCH_MP_TAC LE_TRANS THEN EXISTS_TAC `N + 1` THEN
    ...
    CONV_TAC "realax.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THEN
 ...
 (`!x. ~(x === &0) ==> !N. ?A B. !m n. m <= N ==>
  ...
  INDUCT_TAC THENL
   [MAP_EVERY EXISTS_TAC [`nadd_rinv x 0`; `0`] THEN
    ...
    GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [MULT_SYM] THEN MATCH_ACCEPT_TAC LE_REFL;
   ...
   [DISJ_CASES_THEN2 ASSUME_TAC MP_TAC (SPECL [`N:num`; `n:num`] LE_CASES)
    THENL
     [MATCH_MP_TAC LE_TRANS THEN EXISTS_TAC `B0 * (m + n)` THEN CONJ_TAC THENL
       [FIRST_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[];
        GEN_REWRITE_TAC "realax.ml:(RAND_CONV o funpow 2 LAND_CONV)" (RAND_CONV o funpow 2 LAND_CONV) [ADD_SYM] THEN
        REWRITE_TAC[RIGHT_ADD_DISTRIB; GSYM ADD_ASSOC; LE_ADD]];
      DISCH_THEN(ANTE_RES_THEN ASSUME_TAC) THEN
    ...
    MATCH_MP_TAC LE_TRANS THEN EXISTS_TAC `A * n + B` THEN
    ASM_REWRITE_TAC[LE_ADD_RCANCEL] THEN
    GEN_REWRITE_TAC "realax.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [ADD_SYM] THEN
    REWRITE_TAC[LEFT_ADD_DISTRIB; RIGHT_ADD_DISTRIB; GSYM ADD_ASSOC;
      LE_ADD]]);;
...
let HREAL_MUL_LZERO = prove
 (`!m. &0 * m = &0`,
  ...
  GEN_REWRITE_TAC "realax.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [GSYM HREAL_ADD_LID] THEN
...
let TREAL_EQ_TRANS = prove
 (`!x y z. x treal_eq y /\ y treal_eq z ==> x treal_eq z`,
  ...
  GEN_REWRITE_TAC "realax.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [HREAL_ADD_SYM] THEN
...
let TREAL_LE_MUL = prove
 (`!x y. (treal_of_num 0) treal_le x /\ (treal_of_num 0) treal_le y
  ...
  GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [HREAL_ADD_SYM] THEN
...
let TREAL_MUL_LINV = prove
 (`!x. ~(x treal_eq treal_of_num 0) ==>
  ...
  DISCH_THEN(fun th -> ASSUME_TAC (SYM th) THEN
                       GEN_REWRITE_TAC "realax.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [th]) THEN
 ...
 (`!x y. x treal_eq y ==> (treal_inv x) treal_eq (treal_inv y)`,
  let lemma = prove
   (`(@d. x = x + d) = &0`,
    ...
    GEN_TAC THEN GEN_REWRITE_TAC "realax.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [GSYM HREAL_ADD_RID] THEN
  ...
  ASM_CASES_TAC `x2 <= x1` THEN ASM_REWRITE_TAC[] THENL
   [FIRST_ASSUM(ASSUME_TAC o SYM o SELECT_RULE o MATCH_MP HREAL_LE_EXISTS) THEN
    ...
    GEN_REWRITE_TAC "realax.ml:(RAND_CONV o LAND_CONV o RAND_CONV o BINDER_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV o BINDER_CONV o LAND_CONV) [HREAL_ADD_SYM] THEN
    ...
    COND_CASES_TAC THENL
     [UNDISCH_TAC `(@d. x2 = x1 + d) + y1 <= y1:hreal` THEN
      GEN_REWRITE_TAC "realax.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM HREAL_ADD_LID] THEN
      ...
      DISCH_TAC THEN SUBGOAL_THEN `(@d. x2 = x1 + d) = &0` MP_TAC THENL
       [ASM_REWRITE_TAC[GSYM HREAL_LE_ANTISYM] THEN
        GEN_REWRITE_TAC "realax.ml:RAND_CONV" RAND_CONV [GSYM HREAL_ADD_LID] THEN
        REWRITE_TAC[HREAL_LE_ADD];
        DISCH_THEN SUBST_ALL_TAC THEN
        UNDISCH_TAC `x1 + &0 = x2` THEN
        ASM_REWRITE_TAC[HREAL_ADD_RID]];
      GEN_REWRITE_TAC "realax.ml:(funpow 3 RAND_CONV o BINDER_CONV o LAND_CONV)" (funpow 3 RAND_CONV o BINDER_CONV o LAND_CONV)
        [HREAL_ADD_SYM] THEN
      REWRITE_TAC[HREAL_EQ_ADD_LCANCEL; TREAL_EQ_REFL]]]);;
 ...
 (`?r:hreal->real.
  ...
  GEN_TAC THEN EQ_TAC THENL
   [MP_TAC(INST [`dest_real x`,`r:hreal#hreal->bool`] (snd real_tybij)) THEN
    ...
    GEN_REWRITE_TAC "realax.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [HREAL_ADD_SYM] THEN
    REWRITE_TAC[GSYM HREAL_ADD_ASSOC; HREAL_EQ_ADD_LCANCEL] THEN
   ...
   [X_GEN_TAC `z:real` THEN DISCH_TAC THEN
    DISJ_CASES_TAC(SPECL [`&0`; `z:real`] REAL_LE_TOTAL) THENL
     [ANTE_RES_THEN(SUBST1_TAC o SYM) (ASSUME `&0 <= z`) THEN
      FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "realax.ml:I" I [GSYM th]) THEN
      FIRST_ASSUM MATCH_MP_TAC THEN
  ...
  DISJ_CASES_TAC (SPECL [`&0`; `x:real`] REAL_LE_TOTAL) THENL
   [MATCH_MP_TAC REAL_COMPLETE_SOMEPOS THEN CONJ_TAC THENL
    ...
    EXISTS_TAC `B + x` THEN CONJ_TAC THENL
     [GEN_TAC THEN GEN_REWRITE_TAC "realax.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [lemma] THEN
