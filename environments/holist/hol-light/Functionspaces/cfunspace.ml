...
let REAL_TAC ?(alternatives=[]) g =
  ...
  let LOOP = TRY_REAL_THM ORELSE (ASM_SIMP_TAC[] THEN NO_TAC)
    ORELSE (CHANGED_TAC (ASM_SIMP_TAC[real]) THEN CONV_TAC "Functionspaces/cfunspace.ml:COMPLEX_FIELD" COMPLEX_FIELD)
...
let CFUN_ARITH_TAC =
  ...
  THEN (CONV_TAC "Functionspaces/cfunspace.ml:COMPLEX_FIELD" COMPLEX_FIELD
     ORELSE SIMPLE_COMPLEX_ARITH_TAC
    ORELSE (REPEAT STRIP_TAC THEN CONV_TAC "Functionspaces/cfunspace.ml:PRENEX_CONV" PRENEX_CONV
      THEN MATCH_MP_TAC lemma THEN CONV_TAC "Functionspaces/cfunspace.ml:COMPLEX_FIELD" COMPLEX_FIELD));;
...
let CFUN_EQ_SMUL_LCANCEL2 = prove
  ...
  THEN GEN_TAC THEN POP_ASSUM MP_TAC THEN CONV_TAC "Functionspaces/cfunspace.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
...
let ORTHOGONAL_SUM_NORM = inner_space_prove
  ...
  THEN (CONV_TAC "Functionspaces/cfunspace.ml:o" o DEPTH_CONV o CHANGED_CONV) COMPLEX_POLY_CONV
...
let SCHWARZ_INEQUALITY = inner_space_prove
  ...
  THEN FIRST_X_ASSUM (wrap (CHANGED_TAC o GEN_REWRITE_TAC "Functionspaces/cfunspace.ml:(PATH_CONV \\"rl\\" o ONCE_DEPTH_CONV)" (PATH_CONV \\"rl\\" o ONCE_DEPTH_CONV)))
  ...
  THENL [
    ...
    ASM_REWRITE_TAC[] THEN GCONV_TAC "Functionspaces/cfunspace.ml:COMPLEX_POLY_CONV" COMPLEX_POLY_CONV
...
let COP_ARITH_TAC =
  let lemma = MESON[] `(!x. P x <=> Q x) ==> (!x. P x) = (!x. Q x)` in
  CONV_TAC "Functionspaces/cfunspace.ml:(TOP_DEPTH_CONV COP_POW_CONV)" (TOP_DEPTH_CONV COP_POW_CONV)
...
let COP_ZERO_NEQ_ID = prove
  ...
  THEN Pa.EXISTS_TAC `\x. Cx(&1):` THEN CONV_TAC "Functionspaces/cfunspace.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
...
let COP_EQ_MUL_LCANCEL2 = prove
  ...
  THEN REPEAT GEN_TAC THEN POP_ASSUM MP_TAC THEN CONV_TAC "Functionspaces/cfunspace.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
