...
let EXP_MULT = prove
 (`!m n p. m EXP (n * p) = (m EXP n) EXP p`,
  ...
  ASM_REWRITE_TAC[EXP_ADD; EXP; MULT_CLAUSES] THENL
   [CONV_TAC "arith.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
    INDUCT_TAC THEN ASM_REWRITE_TAC[EXP; MULT_CLAUSES];
...
let LT_MULT2 = prove
 (`!m n p q. m < n /\ p < q ==> m * p < n * q`,
  ...
  UNDISCH_TAC `m < n` THEN CONV_TAC "arith.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN SIMP_TAC[LT]);;
...
let num_WOP = prove
 (`!P. (?n. P n) <=> (?n. P(n) /\ !m. m < n ==> ~P(m))`,
  ...
  CONV_TAC "arith.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_EXISTS_THM] THEN
   ...
   [DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC `a:num`) MP_TAC) THEN
    ...
    SPEC_TAC(`m:num`,`m:num`) THEN INDUCT_TAC THENL
     [REWRITE_TAC[LE; LT] THEN DISCH_THEN(IMP_RES_THEN SUBST_ALL_TAC) THEN
      ...
      REWRITE_TAC[LT] THEN CONV_TAC "arith.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
 ...
 (`!n. (?k m. ODD m /\ (n = 2 EXP k * m)) <=> ~(n = 0)`,
  ...
  ASM_CASES_TAC `m = 0` THEN ASM_REWRITE_TAC[MULT_EQ_0] THENL
   [REWRITE_TAC[MULT_CLAUSES; LT] THEN
    ...
    ALL_TAC] THEN
  ANTS_TAC THENL
   [GEN_REWRITE_TAC "arith.ml:LAND_CONV" LAND_CONV [GSYM(el 2 (CONJUNCTS MULT_CLAUSES))] THEN
...
let LEFT_SUB_DISTRIB = prove
 (`!m n p. m * (n - p) = m * n - m * p`,
  REPEAT GEN_TAC THEN CONV_TAC "arith.ml:SYM_CONV" SYM_CONV THEN
...
let LT_EXP = prove
 (`!x m n. x EXP m < x EXP n <=> 2 <= x /\ m < n \/
  ...
  EQ_TAC THENL
   [CONV_TAC "arith.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
    ...
    CONJ_TAC THENL
     [ASM_REWRITE_TAC[MULT_CLAUSES];
      ...
      CONV_TAC "arith.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_LE] THEN
      REWRITE_TAC[ONE; LT]];
    STRIP_TAC THEN
    FIRST_ASSUM(X_CHOOSE_THEN `d:num` SUBST1_TAC o
...
let LE_EXP = prove
 (`!x m n. x EXP m <= x EXP n <=>
  ...
  CONV_TAC "arith.ml:(EQT_INTRO o TAUT)" (EQT_INTRO o TAUT));;

let EQ_EXP = prove
 (`!x m n. x EXP m = x EXP n <=>
           if x = 0 then (m = 0 <=> n = 0)
           else (x = 1) \/ m = n`,
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC "arith.ml:LAND_CONV" LAND_CONV [GSYM LE_ANTISYM; LE_EXP] THEN
  COND_CASES_TAC THEN ASM_REWRITE_TAC[LE_EXP] THEN
  REWRITE_TAC[GSYM LE_ANTISYM] THEN CONV_TAC "arith.ml:TAUT" TAUT);;
...
let EXP_MONO_EQ = prove
 (`!x y n. x EXP n = y EXP n <=> x = y \/ n = 0`,
  REWRITE_TAC[GSYM LE_ANTISYM; EXP_MONO_LE] THEN CONV_TAC "arith.ml:TAUT" TAUT);;
...
let DIVMOD_EXIST = prove
 (`!m n. ~(n = 0) ==> ?q r. (m = q * n + r) /\ r < n`,
  ...
  CONV_TAC "arith.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN ASM_REWRITE_TAC[NOT_LT] THEN
 ...
 (`!m n q1 r1 q2 r2. ((m = q1 * n + r1) /\ r1 < n) /\
  ...
  SUBGOAL_THEN `r1:num = r2` MP_TAC THENL
   [UNDISCH_TAC `m = q2 * n + r2` THEN
    ...
    CONV_TAC "arith.ml:SYM_CONV" SYM_CONV THEN
...
let DIV_LE = prove
 (`!m n. ~(n = 0) ==> m DIV n <= m`,
  ...
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "arith.ml:RAND_CONV" RAND_CONV [MATCH_MP DIVISION th]) THEN
...
let DIV_MUL_LE = prove
 (`!m n. n * (m DIV n) <= m`,
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "arith.ml:RAND_CONV" RAND_CONV [CONJUNCT1 th]) THEN
...
let MOD_MOD = prove
 (`!m n p. ~(n * p = 0) ==> ((m MOD (n * p)) MOD n = m MOD n)`,
  ...
  CONV_TAC "arith.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MOD_EQ THEN
...
let MOD_EXISTS = prove
 (`!m n. (?q. m = n * q) <=> if n = 0 then (m = 0) else (m MOD n = 0)`,
  ...
  SUBGOAL_THEN `m = (m DIV n) * n + m MOD n`
   (fun th -> GEN_REWRITE_TAC "arith.ml:LAND_CONV" LAND_CONV [th]) THENL
...
let LE_LDIV_EQ = prove
 (`!a b n. ~(a = 0) ==> (b DIV a <= n <=> b < a * (n + 1))`,
  ...
  GEN_REWRITE_TAC "arith.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM LE_SUC_LT] THEN
...
let MOD_MULT_RMOD = prove
 (`!m n p. ~(n = 0) ==> ((m * (p MOD n)) MOD n = (m * p) MOD n)`,
  REPEAT STRIP_TAC THEN CONV_TAC "arith.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MOD_EQ THEN
...
let DIV_ADD_MOD = prove
 (`!a b n. ~(n = 0)
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "arith.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [th]) THEN
...
let MOD_LE = prove
 (`!m n. ~(n = 0) ==> m MOD n <= m`,
  REPEAT GEN_TAC THEN
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "arith.ml:RAND_CONV" RAND_CONV
...
let MULT_DIV_LE = prove
 (`!m n p. ~(p = 0) ==> m * (n DIV p) <= (m * n) DIV p`,
  ...
  DISCH_THEN(fun th ->
    GEN_REWRITE_TAC "arith.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [CONJUNCT1 th]) THEN
...
let MINIMAL = prove
 (`!P. (?n. P n) <=> P((minimal) P) /\ (!m. m < (minimal) P ==> ~(P m))`,
  GEN_TAC THEN REWRITE_TAC[minimal] THEN CONV_TAC "arith.ml:(RAND_CONV SELECT_CONV)" (RAND_CONV SELECT_CONV) THEN
...
let DEPENDENT_CHOICE_FIXED = prove
 (`!P R a:A.
  ...
  ASM_REWRITE_TAC[] THEN GEN_REWRITE_TAC "arith.ml:LAND_CONV" LAND_CONV
