...
let NUMSEG_CLAUSES = prove
 (`(!m. m..0 = if m = 0 then {0} else {}) /\
  ...
  GEN_REWRITE_TAC "iterate.ml:I" I [EXTENSION] THEN
...
let ITERATE_CLAUSES_GEN = prove
 (`!op. monoidal op
  ...
  GEN_REWRITE_TAC "iterate.ml:(LAND_CONV o RATOR_CONV o LAND_CONV)" (LAND_CONV o RATOR_CONV o LAND_CONV) [COND_RAND] THEN
...
let ITERATE_INCL_EXCL = prove
 (`!op. monoidal op
  ...
  GEN_REWRITE_TAC "iterate.ml:(LAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)
    [SET_RULE `s:A->bool = s DIFF t UNION s INTER t`] THEN
  GEN_REWRITE_TAC "iterate.ml:(LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)
...
let ITERATE_ITERATE_PRODUCT = prove
 (`!op. monoidal op
  ...
  FIRST_ASSUM(fun th ->
   ...
   rand o snd)) THEN
  ANTS_TAC THENL
   [SIMP_TAC[FORALL_PAIR_THM] THEN CONV_TAC "iterate.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
    ASM_SIMP_TAC[PAIR_EQ];
    DISCH_THEN SUBST1_TAC THEN REWRITE_TAC[o_DEF] THEN
    CONV_TAC "iterate.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN REWRITE_TAC[ETA_AX]]);;
 ...
 (`!op. monoidal op
  ...
  EXISTS_TAC `iterate op s ((g:B->C) o (h:A->B))` THEN CONJ_TAC THENL
   [ASM_MESON_TAC[ITERATE_EQ; o_THM];
    CONV_TAC "iterate.ml:SYM_CONV" SYM_CONV THEN
...
let ITERATE_INJECTION = prove
 (`!op. monoidal op
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "iterate.ml:SYM_CONV" SYM_CONV THEN
...
let ITERATE_OP_GEN = prove
 (`!op. monoidal op
  ...
  CONJ_TAC THENL
   [CONV_TAC "iterate.ml:SYM_CONV" SYM_CONV;
...
let ITERATE_PAIR = prove
 (`!op. monoidal op
  ...
  GEN_TAC THEN DISCH_TAC THEN GEN_TAC THEN GEN_TAC THEN
  INDUCT_TAC THEN CONV_TAC "iterate.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THENL
 ...
 (`!f:A->B g s.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC NSUM_EQ THEN ASM_REWRITE_TAC[] THEN X_GEN_TAC `x:A` THEN
    ...
    ASM_MESON_TAC[];
    GEN_REWRITE_TAC "iterate.ml:(funpow 2 RAND_CONV o ABS_CONV o RAND_CONV)" (funpow 2 RAND_CONV o ABS_CONV o RAND_CONV)
     [GSYM ETA_AX] THEN
...
let NSUM_MUL_BOUND = prove
 (`!a b s:A->bool.
  ...
  ASM_SIMP_TAC[] THEN GEN_REWRITE_TAC "iterate.ml:LAND_CONV" LAND_CONV [GSYM NSUM_SING] THEN
...
let NSUM_REFLECT = prove
 (`!x m n. nsum(m..n) x = if n < m then 0 else nsum(0..n-m) (\i. x(n - i))`,
  REPEAT GEN_TAC THEN REWRITE_TAC[nsum] THEN
  GEN_REWRITE_TAC "iterate.ml:LAND_CONV" LAND_CONV [MATCH_MP ITERATE_REFLECT MONOIDAL_ADD] THEN
...
let CARD_UNIONS = prove
 (`!s:(A->bool)->bool.
  ...
  CONV_TAC "iterate.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_UNION_EQ THEN
...
let SUM_MUL_BOUND = prove
 (`!a b s:A->bool.
  ...
  ASM_SIMP_TAC[] THEN GEN_REWRITE_TAC "iterate.ml:LAND_CONV" LAND_CONV [GSYM SUM_SING] THEN
...
let SUM_REFLECT = prove
 (`!x m n. sum(m..n) x = if n < m then &0 else sum(0..n-m) (\i. x(n - i))`,
  REPEAT GEN_TAC THEN REWRITE_TAC[sum] THEN
  GEN_REWRITE_TAC "iterate.ml:LAND_CONV" LAND_CONV [MATCH_MP ITERATE_REFLECT MONOIDAL_REAL_ADD] THEN
...
let REAL_SUB_POLYFUN = prove
 (`!a x y n.
  ...
  GEN_REWRITE_TAC "iterate.ml:LAND_CONV" LAND_CONV [MATCH_MP SUM_CLAUSES_LEFT (SPEC_ALL LE_0)] THEN
...
let REAL_POLYFUN_EQ_0 = prove
 (`!n c. (!x. sum(0..n) (\i. c i * x pow i) = &0) <=>
  ...
  REPEAT GEN_TAC THEN EQ_TAC THEN DISCH_TAC THENL
   [GEN_REWRITE_TAC "iterate.ml:I" I [TAUT `p <=> ~ ~p`] THEN DISCH_THEN(MP_TAC o MATCH_MP
 ...
 (`!n c k. (!x. sum(0..n) (\i. c i * x pow i) = k) <=>
  ...
  CONJ_TAC THENL
   [SIMP_TAC[SUM_CLAUSES_LEFT; LE_0; real_pow; REAL_MUL_RID] THEN
    ...
    GEN_REWRITE_TAC "iterate.ml:LAND_CONV" LAND_CONV [MESON[]
...
let POLYNOMIAL_FUNCTION_MUL = prove
 (`!p q. polynomial_function p /\ polynomial_function q
  ...
  REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM] THEN
  GEN_TAC THEN DISCH_TAC THEN
  GEN_REWRITE_TAC "iterate.ml:(BINDER_CONV o LAND_CONV)" (BINDER_CONV o LAND_CONV) [polynomial_function] THEN
