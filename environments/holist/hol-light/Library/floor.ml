...
let RATIONAL_INTEGER = prove
 (`!x. integer x ==> rational x`,
  ...
  ASM_SIMP_TAC[INTEGER_CLOSED] THEN CONV_TAC "Library/floor.ml:REAL_FIELD" REAL_FIELD);;
...
let RATIONAL_INV = prove
 (`!x. rational(x) ==> rational(inv x)`,
  ...
  REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Library/floor.ml:REAL_FIELD" REAL_FIELD);;
...
let REAL_TRUNCATE_POS = prove
 (`!x. &0 <= x ==> ?n r. &0 <= r /\ r < &1 /\ (x = &n + r)`,
  ...
  GEN_REWRITE_TAC "Library/floor.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
...
let FRAC_DIV_MOD = prove
 (`!m n. ~(n = 0) ==> frac(&m / &n) = &(m MOD n) / &n`,
  ...
  DISCH_THEN(fun th ->
    GEN_REWRITE_TAC "Library/floor.ml:(RAND_CONV o LAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o LAND_CONV o RAND_CONV)
                    [CONJUNCT1 th]) THEN
 ...
 (`!a b. integer a /\ integer b
  ...
  SUBGOAL_THEN
   `{x | integer(x) /\ a <= x /\ x <= b} =
    IMAGE (\n. a + &n) {n | &n <= b - a}`
  SUBST1_TAC THENL
   [CONV_TAC "Library/floor.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
...
let REAL_CARD_INTSEG_INT = prove
 (`!a b. integer a /\ integer b
  ...
  GEN_REWRITE_TAC "Library/floor.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [GSYM int_of_num_th] THEN
