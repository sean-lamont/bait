...
let nat_mod = prove
 (`!x y n:num. (mod n) x y <=> ?q1 q2. x + n * q1 = y + n * q2`,
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC "Library/pratt.ml:LAND_CONV" LAND_CONV [GSYM cong] THEN
 ...
 (`!n. { m | 0 < m /\ m < n } HAS_SIZE (n - 1)`,
  INDUCT_TAC THENL
   [SUBGOAL_THEN `{m | 0 < m /\ m < 0} = EMPTY` SUBST1_TAC THENL
    ...
    CONV_TAC "Library/pratt.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
    ASM_CASES_TAC `n = 0` THENL
     [SUBGOAL_THEN `{m | 0 < m /\ m < SUC n} = EMPTY` SUBST1_TAC THENL
       [ASM_REWRITE_TAC[EXTENSION; IN_ELIM_THM; NOT_IN_EMPTY] THEN
        ARITH_TAC; ALL_TAC] THEN
      ASM_REWRITE_TAC[] THEN CONV_TAC "Library/pratt.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
...
let COPRIME_MOD = prove
 (`!a n. ~(n = 0) ==> (coprime(a MOD n,n) <=> coprime(a,n))`,
  ...
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/pratt.ml:(RAND_CONV o RAND_CONV o LAND_CONV)" (RAND_CONV o RAND_CONV o LAND_CONV)
 ...
 (`!x y n. ~(n = 0) ==> ((x == y) (mod n) <=> (x MOD n = y MOD n))`,
  ...
  REPEAT STRIP_TAC THEN EQ_TAC THEN STRIP_TAC THENL
   [ASM_CASES_TAC `x <= y` THENL
     [CONV_TAC "Library/pratt.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MOD_EQ THEN EXISTS_TAC `q1 - q2`;
...
let CONG_SUB_CASES = prove
 (`!x y n. (x == y) (mod n) <=>
  ...
  COND_CASES_TAC THENL
   [GEN_REWRITE_TAC "Library/pratt.ml:LAND_CONV" LAND_CONV [SWAP_EXISTS_THM]; ALL_TAC] THEN
...
let PHI_LIMIT = prove
 (`!n. phi(n) <= n`,
  ...
  GEN_REWRITE_TAC "Library/pratt.ml:RAND_CONV" RAND_CONV [GSYM CARD_NUMSEG_LT] THEN
 ...
 (`!n. (phi n = n - 1) /\ ~(n = 0) /\ ~(n = 1) <=> prime n`,
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    ...
    REWRITE_TAC[COPRIME_SYM] THEN CONV_TAC "Library/pratt.ml:TAUT" TAUT] THEN
...
let ITSET_MODMULT = prove
 (`!n s. FINITE s /\ ~(n = 0) /\ ~(n = 1) /\ coprime(a,n)
  ...
  ASM_SIMP_TAC[IMAGE_CLAUSES; CARD_CLAUSES; FINITE_IMAGE] THEN CONJ_TAC THENL
   [REWRITE_TAC[EXP; MULT_CLAUSES] THEN STRIP_TAC THEN CONV_TAC "Library/pratt.ml:SYM_CONV" SYM_CONV THEN
...
let FERMAT_LITTLE = prove
 (`!a n. coprime(a,n) ==> (a EXP (phi n) == 1) (mod n)`,
  ...
  ASM_SIMP_TAC[COPRIME_0; PHI_0; CONG_MOD_0] THEN CONV_TAC "Library/pratt.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    ...
    X_GEN_TAC `c:num` THEN EQ_TAC THENL
     [DISCH_THEN(X_CHOOSE_THEN `b:num` MP_TAC) THEN
      ...
      SUBGOAL_THEN `a * b = (a * b) DIV n * n + (a * b) MOD n`
       (fun th -> GEN_REWRITE_TAC "Library/pratt.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [th])
     ...
     [EXISTS_TAC `(c * x) MOD n` THEN
      ...
      ASM_SIMP_TAC[DIVISION] THEN CONJ_TAC THENL
       [SUBGOAL_THEN `coprime(n,c * x)` MP_TAC THENL
        ...
        FIRST_X_ASSUM MATCH_MP_TAC THEN
        ASM_SIMP_TAC[DIVIDES_ADD; DIVIDES_LMUL; DIVIDES_REFL]; ALL_TAC] THEN
      ASM_SIMP_TAC[MOD_MULT_RMOD] THEN CONV_TAC "Library/pratt.ml:SYM_CONV" SYM_CONV THEN
      ...
      CONV_TAC "Library/pratt.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MOD_UNIQ THEN
  ...
  GEN_REWRITE_TAC "Library/pratt.ml:(LAND_CONV o RATOR_CONV o RAND_CONV)" (LAND_CONV o RATOR_CONV o RAND_CONV)
   [ARITH_RULE `x = x * 1`] THEN
  GEN_REWRITE_TAC "Library/pratt.ml:(LAND_CONV o RATOR_CONV o LAND_CONV)" (LAND_CONV o RATOR_CONV o LAND_CONV) [MULT_SYM] THEN
 ...
 (`!p a. prime p ==> (a EXP p == a) (mod p)`,
  ...
  ONCE_REWRITE_TAC[COPRIME_SYM] THEN STRIP_TAC THENL
   [ASM_REWRITE_TAC[EXP_ONE; CONG_REFL];
    ...
    GEN_REWRITE_TAC "Library/pratt.ml:RAND_CONV" RAND_CONV [CONG_SYM] THEN ASM_REWRITE_TAC[CONG_0] THEN
  ...
  GEN_REWRITE_TAC "Library/pratt.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [MULT_SYM] THEN
...
let LUCAS = prove
 (`!a n. 2 <= n /\
  ...
  SUBGOAL_THEN `~(r = 1)` MP_TAC THENL
   [UNDISCH_TAC `m < m * r` THEN CONV_TAC "Library/pratt.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let PRIMEFACT = prove
 (`!n. ~(n = 0) ==> ?ps. primefact ps n`,
  ...
  GEN_REWRITE_TAC "Library/pratt.ml:(funpow 3 LAND_CONV)" (funpow 3 LAND_CONV) [ARITH_RULE `n = 1 * n`] THEN
...
let EXP_EQ_MOD_CONV =
  let pth = prove
   (`~(n = 0)
    ...
    CONV_TAC "Library/pratt.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN
    ASM_SIMP_TAC[MOD_MULT_LMOD; MOD_MULT_RMOD] THEN
