...
let POWDIFF_LEMMA = prove(
  ...
  BETA_TAC THEN GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
...
let POWDIFF = prove(
  ...
  INDUCT_TAC THENL
   [REPEAT GEN_TAC THEN REWRITE_TAC[sum] THEN
    ...
    GEN_REWRITE_TAC "Library/transc.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [REAL_MUL_SYM] THEN
    CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[REAL_ADD_LID_UNIQ] THEN
    GEN_REWRITE_TAC "Library/transc.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [REAL_MUL_SYM] THEN
...
let POWREV = prove(
  ...
  REPEAT GEN_TAC THEN ASM_CASES_TAC `x:real = y` THENL
   [ASM_REWRITE_TAC[GSYM POW_ADD] THEN
    ...
    GEN_REWRITE_TAC "Library/transc.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [REAL_MUL_SYM] THEN
    RULE_ASSUM_TAC(ONCE_REWRITE_RULE[GSYM REAL_SUB_0]) THEN
    FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/transc.ml:I" I [MATCH_MP REAL_EQ_LMUL2' th]) THEN
    GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [GSYM REAL_NEGNEG] THEN
...
let POWSER_INSIDEA = prove(
  ...
  EXISTS_TAC `\n. (K * abs(z pow n)) / abs(x pow n)` THEN CONJ_TAC THENL
   [EXISTS_TAC `0` THEN X_GEN_TAC `n:num` THEN DISCH_THEN(K ALL_TAC) THEN
    ...
    CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN REWRITE_TAC[] THEN
    MATCH_MP_TAC SER_CMUL THEN
    GEN_REWRITE_TAC "Library/transc.ml:(RATOR_CONV o ONCE_DEPTH_CONV)" (RATOR_CONV o ONCE_DEPTH_CONV) [GSYM real_div] THEN
    ...
    MATCH_MP_TAC POW_INV THEN CONV_TAC "Library/transc.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let TERMDIFF_LEMMA1 = prove(
  ...
  REWRITE_TAC[REAL_SUB_LDISTRIB; GSYM POW_ADD] THEN BINOP_TAC THENL
   [MATCH_ACCEPT_TAC REAL_MUL_SYM;
    ...
    CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUB_ADD THEN
...
let TERMDIFF_LEMMA2 = prove(
  ...
  GEN_REWRITE_TAC "Library/transc.ml:(RATOR_CONV o ONCE_DEPTH_CONV)" (RATOR_CONV o ONCE_DEPTH_CONV) [POWREV] THEN
  ...
  GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
  REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN AP_TERM_TAC THEN
  ...
  AP_TERM_TAC THEN AP_TERM_TAC THEN CONV_TAC "Library/transc.ml:(TOP_DEPTH_CONV num_CONV)" (TOP_DEPTH_CONV num_CONV) THEN
...
let TERMDIFF_LEMMA3 = prove(
  ...
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/transc.ml:I" I [MATCH_MP REAL_LE_LMUL_LOCAL th]) THEN
  ...
  REWRITE_TAC[POW_ADD] THEN GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV
   [AC REAL_MUL_AC
        `(a * b) * c = b * (c * a)`] THEN
...
let TERMDIFF_LEMMA4 = prove(
  ...
  SUBGOAL_THEN `&0 <= K` MP_TAC THENL
   [FIRST_ASSUM(MP_TAC o SPEC `k / &2`) THEN
    ...
    DISCH_THEN(fun th -> GEN_REWRITE_TAC "Library/transc.ml:I" I [GSYM th]) THEN
...
let TERMDIFF = prove(
  ...
  DISCH_THEN MATCH_MP_TAC THEN REPEAT CONJ_TAC THENL
   [ASM_REWRITE_TAC[REAL_SUB_LT];
    ...
    CONV_TAC "Library/transc.ml:(X_FUN_EQ_CONV `n:num`)" (X_FUN_EQ_CONV `n:num`) THEN BETA_TAC THEN GEN_TAC THEN
    REWRITE_TAC[real_div; GSYM REAL_MUL_ASSOC] THEN
    GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV
     [AC REAL_MUL_AC
    ...
    MATCH_MP_TAC REAL_MUL_RINV THEN CONV_TAC "Library/transc.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
 ...
 (`!x. abs(x) < &1 ==> (\n. &n * x pow n) tends_num_real &0`,
  ...
  EXISTS_TAC `SUC N` THEN CONJ_TAC THENL
   [REWRITE_TAC[real_div; REAL_MUL_ASSOC] THEN REWRITE_TAC[GSYM real_div] THEN
    ...
    FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/transc.ml:I" I [GSYM th]) THEN
  ...
  GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [AC REAL_MUL_AC
  ...
  UNDISCH_TAC `m:num <= n` THEN GEN_REWRITE_TAC "Library/transc.ml:LAND_CONV" LAND_CONV [LE_EXISTS] THEN
...
let TERMDIFF_CONVERGES = prove
 (`!K. (!x. abs(x) < K ==> summable(\n. c(n) * x pow n))
  ...
  GEN_REWRITE_TAC "Library/transc.ml:LAND_CONV" LAND_CONV [AC REAL_MUL_AC `(a * b) * c = b * a * c`] THEN
...
let POWSER_0 = prove
 (`!a. (\n. a n * (&0) pow n) sums a(0)`,
  ...
  SUBGOAL_THEN `a(0) = sum(0,1) (\n. a n * (&0) pow n)` SUBST1_TAC THENL
   [CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV REAL_SUM_CONV)" (ONCE_DEPTH_CONV REAL_SUM_CONV) THEN
...
let POWSER_LIMIT_0 = prove
 (`!f a s. &0 < s /\
  ...
  SUBGOAL_THEN `suminf (\n. a n * &0 pow n) = a(0)` SUBST1_TAC THENL
   [CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_UNIQ THEN
...
let POWSER_EQUAL = prove
 (`!f a b P.
  ...
  GEN_REWRITE_TAC "Library/transc.ml:I" I [TAUT `p <=> ~ ~ p`] THEN
  GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [NOT_FORALL_THM] THEN
  ...
  SUBGOAL_THEN `!a m. a(m + n) * x pow m =
   ...
   (fun th -> GEN_REWRITE_TAC "Library/transc.ml:(BINOP_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (BINOP_CONV o LAND_CONV o ONCE_DEPTH_CONV) [th])
  THENL
...
let POW_EQ = prove(
  ...
  CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THENL
   [ALL_TAC; CONV_TAC "Library/transc.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV)] THEN
  MATCH_MP_TAC REAL_LT_IMP_NE THEN
...
let REAL_EXP_CONVERGES = prove(
  ...
  GEN_REWRITE_TAC "Library/transc.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
...
let REAL_EXP_FDIFF = prove(
  ...
  CONV_TAC "Library/transc.ml:(X_FUN_EQ_CONV `n:num`)" (X_FUN_EQ_CONV `n:num`) THEN GEN_TAC THEN BETA_TAC THEN
  REWRITE_TAC[FACT; GSYM REAL_MUL] THEN
  SUBGOAL_THEN `~(&(SUC n) = &0) /\ ~(&(FACT n) = &0)` ASSUME_TAC THENL
   [CONJ_TAC THEN CONV_TAC "Library/transc.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
    MATCH_MP_TAC REAL_LT_IMP_NE THEN
    REWRITE_TAC[REAL_LT; LT_0; FACT_LT];
    FIRST_ASSUM(fun th -> REWRITE_TAC[MATCH_MP REAL_INV_MUL_WEAK th]) THEN
    GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
...
let DIFF_EXP = prove(
  ...
  GEN_REWRITE_TAC "Library/transc.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [GSYM REAL_EXP_FDIFF] THEN
  CONV_TAC "Library/transc.ml:(LAND_CONV BETA_CONV)" (LAND_CONV BETA_CONV) THEN
...
let REAL_EXP_0 = prove(
  `exp(&0) = &1`,
  REWRITE_TAC[exp] THEN CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN
   ...
   [MP_TAC(SPECL [`\n. ((\n. inv(&(FACT n)))) n * (x pow n)`; `2`]
    ...
    funpow 2 (fst o dest_imp) o snd) THENL
     [GEN_TAC THEN DISCH_THEN(K ALL_TAC) THEN
      ...
      CONV_TAC "Library/transc.ml:(TOP_DEPTH_CONV num_CONV)" (TOP_DEPTH_CONV num_CONV) THEN REWRITE_TAC[sum] THEN
...
let REAL_EXP_ADD_MUL = prove(
  ...
  REPEAT GEN_TAC THEN
  CONV_TAC "Library/transc.ml:(LAND_CONV(X_BETA_CONV `x:real`))" (LAND_CONV(X_BETA_CONV `x:real`)) THEN
...
let REAL_EXP_POS_LE = prove(
  `!x. &0 <= exp(x)`,
  GEN_TAC THEN
  GEN_REWRITE_TAC "Library/transc.ml:(funpow 2 RAND_CONV)" (funpow 2 RAND_CONV) [GSYM REAL_HALF_DOUBLE] THEN
...
let REAL_EXP_NZ = prove(
  ...
  CONV_TAC "Library/transc.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
  MATCH_ACCEPT_TAC REAL_10);;

let REAL_EXP_POS_LT = prove(
  `!x. &0 < exp(x)`,
  GEN_TAC THEN REWRITE_TAC[REAL_LT_LE] THEN
  CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
...
let REAL_EXP_MONO_IMP = prove(
  ...
  SUBGOAL_THEN `&1 < exp(y) / exp(x) <=>
   ...
   [CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_LT_RMUL_EQ THEN
...
let REAL_EXP_MONO_LT = prove(
  ...
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[REAL_NOT_LT] THEN
...
let REAL_EXP_TOTAL = prove(
  ...
  DISJ_CASES_TAC(SPECL [`&1`; `y:real`] REAL_LET_TOTAL) THENL
   [FIRST_ASSUM(X_CHOOSE_TAC `x:real` o MATCH_MP REAL_EXP_TOTAL_LEMMA) THEN
    ...
    MATCH_MP_TAC REAL_INVINV THEN CONV_TAC "Library/transc.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
   ...
   [REWRITE_TAC[exp; BETA_THM] THEN MATCH_MP_TAC SER_LE THEN
    REWRITE_TAC[summable; BETA_THM] THEN REPEAT CONJ_TAC THENL
     [GEN_TAC THEN
      ...
      ASM_REWRITE_TAC[] THEN CONV_TAC "Library/transc.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV];
     ...
     [CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_UNIQ THEN
      ...
      SUBGOAL_THEN `(&1 - x) * inv (&1 - x) = &1` SUBST1_TAC THENL
       [MATCH_MP_TAC REAL_MUL_RINV THEN
        ...
        POP_ASSUM MP_TAC THEN CONV_TAC "Library/transc.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
        CONJ_TAC THENL
         [MATCH_MP_TAC REAL_LET_TRANS THEN
          EXISTS_TAC `inv(&2) - x` THEN
          ASM_REWRITE_TAC[REAL_ARITH `&0 <= x - y <=> y <= x`] THEN
          ASM_REWRITE_TAC[REAL_ARITH `a - x < b - x <=> a < b`] THEN
          CONV_TAC "Library/transc.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
          ...
          CONV_TAC "Library/transc.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let LN_EXP = prove(
  ...
  CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN CONV_TAC "Library/transc.ml:(RAND_CONV(ONCE_DEPTH_CONV SYM_CONV))" (RAND_CONV(ONCE_DEPTH_CONV SYM_CONV)) THEN
  CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN MATCH_MP_TAC SELECT_AX THEN
...
let LN_INJ = prove(
  ...
  REPEAT GEN_TAC THEN STRIP_TAC THEN
  EVERY_ASSUM(fun th -> GEN_REWRITE_TAC "Library/transc.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV)
    [SYM(REWRITE_RULE[GSYM REAL_EXP_LN] th)]) THEN
  CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_ACCEPT_TAC REAL_EXP_INJ);;
...
let LN_MONO_LT = prove(
  ...
  EVERY_ASSUM(fun th -> GEN_REWRITE_TAC "Library/transc.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV)
    [SYM(REWRITE_RULE[GSYM REAL_EXP_LN] th)]) THEN
  CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_ACCEPT_TAC REAL_EXP_MONO_LT);;
...
let LN_MONO_LE = prove(
  ...
  EVERY_ASSUM(fun th -> GEN_REWRITE_TAC "Library/transc.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV)
    [SYM(REWRITE_RULE[GSYM REAL_EXP_LN] th)]) THEN
  CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_ACCEPT_TAC REAL_EXP_MONO_LE);;
...
let LN_LE = prove(
  `!x. &0 <= x ==> ln(&1 + x) <= x`,
  GEN_TAC THEN DISCH_TAC THEN
  GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV  [GSYM LN_EXP] THEN
...
let DIFF_LN = prove(
  ...
  FIRST_ASSUM (fun th ->  GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV  [GSYM th]) THEN
...
let ROOT_LN = prove(
  ...
  ASM_REWRITE_TAC[] THEN EQ_TAC THENL
   [DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC (SUBST1_TAC o SYM)) THEN
    ...
    DISCH_THEN(fun th -> GEN_REWRITE_TAC "Library/transc.ml:I" I [th]) THEN
     ...
     [REWRITE_TAC[real_div; GSYM REAL_MUL_ASSOC] THEN
      SUBGOAL_THEN `inv(&(SUC n)) * &(SUC n) = &1` SUBST1_TAC THENL
       [MATCH_MP_TAC REAL_MUL_LINV THEN REWRITE_TAC[REAL_INJ; NOT_SUC];
        ...
        CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC LN_POW THEN
        ASM_REWRITE_TAC[]];
...
let POW_ROOT_POS = prove(
  ...
  DISCH_TAC THEN ASM_REWRITE_TAC[] THENL
   [DISJ_CASES_THEN MP_TAC (REWRITE_RULE[REAL_LE_LT] (ASSUME `&0 <= x`)) THENL
    ...
    ASM_REWRITE_TAC[REAL_LT_LE] THEN CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let ROOT_MONO_LT = prove
 (`!x y. &0 <= x /\ x < y ==> root(SUC n) x < root(SUC n) y`,
  ...
  UNDISCH_TAC `x < y` THEN CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let SQRT_EVEN_POW2 = prove
 (`!n. EVEN n ==> (sqrt(&2 pow n) = &2 pow (n DIV 2))`,
  ...
  AP_TERM_TAC THEN
  GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [MATCH_MP DIVISION (ARITH_RULE `~(2 = 0)`)] THEN
...
let REAL_DIV_SQRT = prove
 (`!x. &0 <= x ==> (x / sqrt(x) = sqrt(x))`,
  ...
  DISCH_TAC THEN CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SQRT_POS_UNIQ THEN
 ...
 (`!x. &0 < x ==> (sqrt diffl inv(&2 * sqrt(x))) x`,
  ...
  REWRITE_TAC[REAL_OF_NUM_EQ; ARITH_EQ] THEN REPEAT CONJ_TAC THENL
   [ASM_MESON_TAC[POW_2_SQRT; REAL_ARITH `abs(x - y) < y ==> &0 <= x`];
    REPEAT STRIP_TAC THEN CONV_TAC "Library/transc.ml:CONTINUOUS_CONV" CONTINUOUS_CONV;
...
let SIN_0 = prove(
  `sin(&0) = &0`,
  REWRITE_TAC[sin] THEN CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN
  MATCH_MP_TAC SUM_UNIQ THEN BETA_TAC THEN
  W(MP_TAC o C SPEC SER_0 o rand o rator o snd) THEN
...
let COS_0 = prove(
  `cos(&0) = &1`,
  REWRITE_TAC[cos] THEN CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN
  MATCH_MP_TAC SUM_UNIQ THEN BETA_TAC THEN
  W(MP_TAC o C SPEC SER_0 o rand o rator o snd) THEN
  DISCH_THEN(MP_TAC o SPEC `1`) THEN
...
let SIN_CIRCLE = prove(
  ...
  GEN_TAC THEN CONV_TAC "Library/transc.ml:(LAND_CONV(X_BETA_CONV `x:real`))" (LAND_CONV(X_BETA_CONV `x:real`)) THEN
...
let SIN_BOUND = prove(
  `!x. abs(sin x) <= &1`,
  GEN_TAC THEN GEN_REWRITE_TAC "Library/transc.ml:I" I [TAUT `a <=> ~ ~a`] THEN
...
let SIN_COS_ADD = prove(
  ...
  W(C SUBGOAL_THEN (SUBST1_TAC o SYM) o subst[`&0`,`x:real`] o snd) THENL
   [BETA_TAC THEN REWRITE_TAC[SIN_0; COS_0] THEN
    ...
    GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN BINOP_TAC THENL
...
let SIN_POS = prove(
  ...
  ASM_REWRITE_TAC[] THEN CONV_TAC "Library/transc.ml:(REDEPTH_CONV num_CONV)" (REDEPTH_CONV num_CONV) THEN
...
let COS_2 = prove(
  `cos(&2) < &0`,
  GEN_REWRITE_TAC "Library/transc.ml:LAND_CONV" LAND_CONV [GSYM REAL_NEGNEG] THEN
...
let COS_ISZERO = prove(
  ...
  W(C SUBGOAL_THEN ASSUME_TAC o hd o conjuncts o snd) THENL
   [MATCH_MP_TAC IVT2 THEN REPEAT CONJ_TAC THENL
    ...
    GEN_REWRITE_TAC "Library/transc.ml:I" I [TAUT `a <=> ~ ~a`] THEN
...
let PI_POS = prove(
  `&0 < pi`,
  GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [GSYM REAL_HALF_DOUBLE] THEN
...
let SIN_PI2 = prove(
  ...
  GEN_REWRITE_TAC "Library/transc.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM REAL_MUL_LID] THEN
  ONCE_REWRITE_TAC[GSYM REAL_SUB_0] THEN
  REWRITE_TAC[GSYM REAL_DIFFSQ; REAL_ENTIRE] THEN
  DISCH_THEN DISJ_CASES_TAC THEN ASM_REWRITE_TAC[] THEN
  POP_ASSUM MP_TAC THEN CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THEN
...
let COS_PI = prove(
  ...
  CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_DIV_LMUL THEN
  REWRITE_TAC[REAL_INJ] THEN NUM_REDUCE_TAC);;
...
let SIN_PI = prove(
  ...
  REWRITE_TAC[REAL_DOUBLE] THEN CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN
...
let COS_TOTAL = prove(
  ...
  GEN_TAC THEN STRIP_TAC THEN
  CONV_TAC "Library/transc.ml:EXISTS_UNIQUE_CONV" EXISTS_UNIQUE_CONV THEN CONJ_TAC THENL
  ...
  CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THEN
...
let SIN_TOTAL = prove(
  ...
  (fun th -> REWRITE_TAC[th]) THENL
   [GEN_TAC THEN REWRITE_TAC[COS_ADD; SIN_PI2; COS_PI2] THEN
    ...
    GEN_REWRITE_TAC "Library/transc.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [GSYM REAL_HALF_DOUBLE] THEN
  ...
  CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV EXISTS_UNIQUE_CONV)" (ONCE_DEPTH_CONV EXISTS_UNIQUE_CONV) THEN
...
let COS_ZERO_LEMMA = prove(
  ...
  SUBGOAL_THEN `&0 <= x - &n * pi /\ (x - &n * pi) <= pi /\
   ...
   [ASM_REWRITE_TAC[REAL_SUB_LE] THEN
    ...
    CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_DIV_RMUL THEN
    REWRITE_TAC[REAL_INJ] THEN NUM_REDUCE_TAC]);;
...
let COS_ONE_2PI = prove
 (`!x. (cos(x) = &1) <=> (?n. x = &n * &2 * pi) \/ (?n. x = --(&n * &2 * pi))`,
  ...
  COND_CASES_TAC THEN CONV_TAC "Library/transc.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM_REWRITE_TAC[]);;
...
let DIFF_TAN = prove(
  ...
  CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN ONCE_REWRITE_TAC[REAL_ADD_SYM] THEN
...
let TAN_TOTAL_LEMMA = prove(
  ...
  MP_TAC THENL
   [SUBST1_TAC(SYM(SPEC `&1` REAL_DIV_LZERO)) THEN
    CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN MATCH_MP_TAC LIM_DIV THEN
    REWRITE_TAC[REAL_10] THEN CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
  ...
  REWRITE_TAC[real_div] THEN IMP_SUBST_TAC REAL_INV_MUL_WEAK THENL
   [REWRITE_TAC[GSYM DE_MORGAN_THM; GSYM REAL_ENTIRE; GSYM real_div] THEN
    ...
    GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN AP_TERM_TAC THEN
    MATCH_MP_TAC REAL_INVINV THEN MATCH_MP_TAC REAL_POS_NZ THEN
...
let TAN_TOTAL = prove(
  ...
  GEN_TAC THEN CONV_TAC "Library/transc.ml:EXISTS_UNIQUE_CONV" EXISTS_UNIQUE_CONV THEN CONJ_TAC THENL
   [DISJ_CASES_TAC(SPEC `y:real` REAL_LE_NEGTOTAL) THEN
    ...
    REPEAT_TCL DISJ_CASES_THEN ASSUME_TAC
     ...
     [DISCH_THEN(K ALL_TAC) THEN POP_ASSUM ACCEPT_TAC;
      ...
      CONV_TAC "Library/transc.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN ONCE_REWRITE_TAC[CONJ_SYM]] THEN
    (STRIP_TAC THEN MP_TAC(SPECL [`tan`; `x1:real`; `x2:real`] ROLLE) THEN
     ASM_REWRITE_TAC[] THEN CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let TAN_PI4 = prove
 (`tan(pi / &4) = &1`,
  ...
  CONV_TAC "Library/transc.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN MATCH_MP_TAC REAL_MUL_RINV THEN
 ...
 (`!x. abs(x) < pi / &4 ==> abs(tan x) < &1`,
  ...
  ASSUME_TAC THENL
   [REPEAT STRIP_TAC THENL
    ...
    MAP_EVERY UNDISCH_TAC [`x < z`; `z < pi / &4`; `&0 < x`] THEN
    REAL_ARITH_TAC; ALL_TAC] THEN
  GEN_REWRITE_TAC "Library/transc.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [real_abs] THEN
  REWRITE_TAC[REAL_LE_LT] THEN
 ...
 (`!x. abs(x) < pi / &2 ==> abs(x) <= abs(tan x)`,
  ...
  MATCH_MP_TAC(TAUT `a /\ (b ==> c) ==> (a ==> b) ==> c`) THEN CONJ_TAC THENL
   [REPEAT STRIP_TAC THEN BETA_TAC THEN MATCH_MP_TAC DIFF_TAN THEN
    ...
    GEN_REWRITE_TAC "Library/transc.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_RID] THEN
...
let ATN_0 = prove
 (`atn(&0) = &0`,
  GEN_REWRITE_TAC "Library/transc.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [SYM TAN_0] THEN
...
let COS_ATN_NZ = prove(
  ...
  CONV_TAC "Library/transc.ml:NOT_EXISTS_CONV" NOT_EXISTS_CONV THEN X_GEN_TAC `n:num` THEN
  ...
  GEN_REWRITE_TAC "Library/transc.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)  [GSYM REAL_MUL_LID] THEN
...
let ATN_LT_PI4 = prove
 (`!x. abs(x) < &1 ==> abs(atn x) < pi / &4`,
  ...
  GEN_REWRITE_TAC "Library/transc.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [GSYM ATN_0] THEN
...
let COS_SIN_SQRT = prove(
  ...
  CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC POW_ROOT_POS THEN
  ASM_REWRITE_TAC[]);;
...
let COS_ASN_NZ = prove(
  ...
  CONJ_TAC THEN CONV_TAC "Library/transc.ml:NOT_EXISTS_CONV" NOT_EXISTS_CONV THEN
...
let DIFF_ASN = prove(
  ...
  SUBGOAL_THEN `sin(asn x) = x` MP_TAC THENL
   [MATCH_MP_TAC ASN_SIN THEN CONJ_TAC THEN
    MATCH_MP_TAC REAL_LT_IMP_LE THEN ASM_REWRITE_TAC[];
    DISCH_THEN(fun th ->  GEN_REWRITE_TAC "Library/transc.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV)  [GSYM th]) THEN
  ...
  REWRITE_TAC[NOT_SUC] THEN EQ_TAC THENL
   [DISCH_TAC THEN MATCH_MP_TAC LESS_EQUAL_ANTISYM THEN CONJ_TAC THENL
     [POP_ASSUM(MP_TAC o SPEC `1` o CONJUNCT1) THEN
      ...
      CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
      REWRITE_TAC[num_CONV `1`; NOT_SUC_LESS_EQ; CONJUNCT1 LE] THEN
      DISCH_THEN SUBST1_TAC THEN REWRITE_TAC[NOT_SUC; NOT_IMP] THEN
      REWRITE_TAC[LE_0] THEN CONV_TAC "Library/transc.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
   ...
   [MATCH_MP_TAC REAL_LE_TRANS THEN EXISTS_TAC `(D:num->real) r` THEN
    ...
    MATCH_MP_TAC REAL_LE_TRANS THEN EXISTS_TAC `b:real` THEN CONJ_TAC THENL
     [MATCH_MP_TAC DIVISION_LE THEN
      ...
      MATCH_MP_TAC REAL_EQ_IMP_LE THEN CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN
  ...
  X_GEN_TAC `n:num` THEN DISCH_TAC THEN BETA_TAC THENL
   [ASM_CASES_TAC `(SUC n) < (dsize D1)` THEN ASM_REWRITE_TAC[] THENL
    ...
    UNDISCH_TAC `n >= ((dsize D1) + (dsize D2))` THENL
     [CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_TAC THEN
...
let DIVISION_APPEND_LEMMA2 = prove(
  `!a b c D1 D2. division(a,b) D1 /\ division(b,c) D2 ==>
                   (dsize(\n. if (n < (dsize D1)) then  D1(n) else
       D2(n - (dsize D1))) = dsize(D1) + dsize(D2))`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Library/transc.ml:LAND_CONV" LAND_CONV [dsize] THEN
       ...
       [SUBGOAL_THEN `D1(N:num) < D2(dsize D2)` MP_TAC THENL
         [MATCH_MP_TAC REAL_LTE_TRANS THEN EXISTS_TAC `b:real` THEN
          CONJ_TAC THENL
           [MATCH_MP_TAC DIVISION_UBOUND_LT THEN EXISTS_TAC `a:real` THEN
            ...
            MATCH_MP_TAC DIVISION_LBOUND THEN
            EXISTS_TAC `c:real` THEN ASM_REWRITE_TAC[]];
          CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN ASM_REWRITE_TAC[] THEN
        ...
        SUBGOAL_THEN `D2(N - (dsize D1)) < D2(dsize D2)` MP_TAC THENL
         [MATCH_MP_TAC DIVISION_LT_GEN THEN
          ...
          CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[] THEN
          DISCH_THEN SUBST1_TAC THEN REWRITE_TAC[REAL_LT_REFL]]]];
  DISCH_THEN SUBST1_TAC THEN CONJ_TAC THENL
 ...
 (`!a b c g d1 p1 d2 p2.
  ...
  DISJ_CASES_TAC(GSYM (SPEC `dsize(D1)` LESS_0_CASES)) THENL
   [ASM_REWRITE_TAC[NOT_LESS_0; SUB_0] THEN
    CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
   ...
   [RULE_ASSUM_TAC(REWRITE_RULE[tdiv]) THEN
    ...
    SUBGOAL_THEN `c = (\n. if (n < (dsize D1)) then  D1(n) else D2(n -
     ...
     [BETA_TAC THEN REWRITE_TAC[GSYM NOT_LE; LE_ADD] THEN
      ONCE_REWRITE_TAC[ADD_SYM] THEN REWRITE_TAC[ADD_SUB] THEN
      CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC DIVISION_RHS THEN
...
let DIVISION_EXISTS = prove(
  ...
  CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
  W(C SUBGOAL_THEN (fun t ->REWRITE_TAC[t]) o
  funpow 2 (fst o dest_imp) o snd) THENL
   [CONJ_TAC;
...
let DINT_UNIQ = prove(
  ...
  REPEAT GEN_TAC THEN DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
  GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [GSYM REAL_SUB_0] THEN
  CONV_TAC "Library/transc.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN ONCE_REWRITE_TAC[ABS_NZ] THEN DISCH_TAC THEN
  ...
  MP_TAC THENL
   [MATCH_MP_TAC REAL_LET_TRANS THEN
    ...
    CONJ_TAC THENL
     [GEN_REWRITE_TAC "Library/transc.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [real_sub] THEN
      GEN_REWRITE_TAC "Library/transc.ml:(funpow 2 RAND_CONV)" (funpow 2 RAND_CONV) [GSYM ABS_NEG] THEN
...
let STRADDLE_LEMMA = prove(
  ...
  ASSUME_TAC THENL
   [GEN_TAC THEN ASM_CASES_TAC `&0 < abs(z - x)` THENL
    ...
    DISCH_THEN(fun th -> FIRST_ASSUM(MP_TAC o C MATCH_MP th)) THEN
    FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/transc.ml:LAND_CONV" LAND_CONV
   ...
   [MATCH_MP_TAC REAL_LE_TRANS THEN
    EXISTS_TAC `(e / &2) * abs(v - x)` THEN CONJ_TAC THENL
     [FIRST_ASSUM MATCH_MP_TAC THEN
      ...
      GEN_REWRITE_TAC "Library/transc.ml:LAND_CONV" LAND_CONV
...
let FTC1 = prove(
  ...
  SUBST1_TAC THENL
   [ONCE_REWRITE_TAC[SYM(BETA_CONV `(\n. (D(SUC n) - D(n))) n`)] THEN
    ...
    MAP_EVERY (IMP_RES_THEN SUBST1_TAC) [DIVISION_LHS; DIVISION_RHS] THEN
    CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_DIV_RMUL THEN
    REWRITE_TAC[REAL_SUB_0] THEN CONV_TAC "Library/transc.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let INTEGRABLE_DEFINT = prove
 (`!f a b. integrable(a,b) f ==> defint(a,b) f (integral(a,b) f)`,
  ...
  CONV_TAC "Library/transc.ml:(RAND_CONV SELECT_CONV)" (RAND_CONV SELECT_CONV) THEN REWRITE_TAC[]);;
 ...
 (`!f a b c i j. a <= b /\ b <= c /\ defint(a,b) f i /\ defint(b,c) f j
   ...
   [ASM_REWRITE_TAC[REAL_SUB_REFL; REAL_MUL_RZERO; REAL_ADD_RID] THEN
    ...
    SUBGOAL_THEN `dsize(\i. d (i + SUC m)) = PRE n` ASSUME_TAC THENL
     [MATCH_MP_TAC DIVISION_DSIZE_EQ_ALT THEN
      ...
      GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [CONJ_SYM] THEN
  ...
  THENL
   [MATCH_MP_TAC DIVISION_DSIZE_EQ_ALT THEN
    ...
    FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [DIVISION_THM]) THEN
    DISCH_THEN(MP_TAC o CONJUNCT2) THEN ASM_REWRITE_TAC[ADD_CLAUSES] THEN
    GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [CONJ_SYM] THEN REWRITE_TAC[NOT_SUC] THEN
    MATCH_MP_TAC MONO_AND THEN CONJ_TAC THENL
  ...
  SUBGOAL_THEN `n = 1 + PRE n`
   (fun th -> GEN_REWRITE_TAC "Library/transc.ml:(LAND_CONV o LAND_CONV o RAND_CONV)" (LAND_CONV o LAND_CONV o RAND_CONV) [th])
...
let DEFINT_DELTA = prove
 (`!a b c. defint(a,b) (\x. if x = c then &1 else &0) (&0)`,
  ...
  GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [GSYM REAL_ADD_LID] THEN
...
let INTEGRABLE_CAUCHY = prove
 (`!f a b. integrable(a,b) f <=>
  ...
  GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_DIV] THEN
...
let MCLAURIN = prove(
  ...
  SUBGOAL_THEN `?B. f(h) = sum(0,n)(\m. (diff(m)(&0) / &(FACT m)) * (h pow m))
   ...
   [ONCE_REWRITE_TAC[REAL_ADD_SYM] THEN
    ...
    REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN
    GEN_REWRITE_TAC "Library/transc.ml:(RATOR_CONV o RAND_CONV)" (RATOR_CONV o RAND_CONV) [GSYM REAL_MUL_RID] THEN
    AP_TERM_TAC THEN CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN
    ONCE_REWRITE_TAC[AC REAL_MUL_AC
      `a * b * c * d = (d * a) * (b * c)`] THEN
    GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN BINOP_TAC THEN
  ...
  SUBGOAL_THEN `difg(0):real->real = g` ASSUME_TAC THENL
   [EXPAND_TAC "difg" THEN BETA_TAC THEN EXPAND_TAC "g" THEN
    CONV_TAC "Library/transc.ml:FUN_EQ_CONV" FUN_EQ_CONV THEN GEN_TAC THEN BETA_TAC THEN
  ...
  SUBGOAL_THEN `(!m t. m < n /\ (& 0) <= t /\ t <= h ==>
   ...
   [REPEAT GEN_TAC THEN DISCH_TAC THEN EXPAND_TAC "difg" THEN BETA_TAC THEN
    CONV_TAC "Library/transc.ml:((funpow 2 RATOR_CONV o RAND_CONV) HABS_CONV)" ((funpow 2 RATOR_CONV o RAND_CONV) HABS_CONV) THEN
    ...
    MATCH_MP_TAC DIFF_ADD THEN CONJ_TAC THENL
     [ALL_TAC;
      ...
      MATCH_MP_TAC EQ_IMP THEN
      AP_THM_TAC THEN CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV(ALPHA_CONV `t:real`))" (ONCE_DEPTH_CONV(ALPHA_CONV `t:real`)) THEN
      AP_TERM_TAC THEN GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
    ...
    CONV_TAC "Library/transc.ml:(funpow 2 RATOR_CONV (RAND_CONV HABS_CONV))" (funpow 2 RATOR_CONV (RAND_CONV HABS_CONV)) THEN
    GEN_REWRITE_TAC "Library/transc.ml:(RATOR_CONV o RAND_CONV)" (RATOR_CONV o RAND_CONV) [GSYM REAL_ADD_RID] THEN
    ...
    DISCH_THEN(MP_TAC o SPEC `t:real`) THEN
    MATCH_MP_TAC EQ_IMP THEN
    CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV(ALPHA_CONV `z:real`))" (ONCE_DEPTH_CONV(ALPHA_CONV `z:real`)) THEN
    AP_THM_TAC THEN AP_TERM_TAC THEN
    ...
    SUBGOAL_THEN `&(SUC k) = inv(inv(&(SUC k)))` SUBST1_TAC THENL
     [CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_INVINV THEN
      REWRITE_TAC[REAL_INJ; NOT_SUC]; ALL_TAC] THEN
    IMP_SUBST_TAC(GSYM REAL_INV_MUL_WEAK) THENL
     [CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN REWRITE_TAC[REAL_FACT_NZ] THEN
  ...
  SUBGOAL_THEN `!m. m < n ==>
   ...
   [ALL_TAC;
    ...
    GEN_REWRITE_TAC "Library/transc.ml:(funpow 2 RAND_CONV)" (funpow 2 RAND_CONV)
     [AC REAL_MUL_AC
      `(a * b) * c = a * (c * b)`] THEN
 ...
 (`!f diff h n.
  ...
  funpow 2 (fst o dest_imp) o snd) THENL
   [REPEAT GEN_TAC THEN
    ...
    CONV_TAC "Library/transc.ml:(ONCE_DEPTH_CONV(ALPHA_CONV `z:real`))" (ONCE_DEPTH_CONV(ALPHA_CONV `z:real`)) THEN
    AP_THM_TAC THEN AP_TERM_TAC THEN
    CONV_TAC "Library/transc.ml:(AC REAL_MUL_AC)" (AC REAL_MUL_AC);
    DISCH_THEN(X_CHOOSE_THEN `t:real` STRIP_ASSUME_TAC)] THEN
 ...
 (`!x n.
  ...
  ASSUME_TAC THENL
   [INDUCT_TAC THEN REWRITE_TAC[NOT_SUC; PRE] THEN
    ...
    ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
    GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
    REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN
  ...
  (fun th -> REWRITE_TAC[th]) THENL
   [GEN_TAC THEN REWRITE_TAC[real_div; GSYM REAL_MUL_ASSOC] THEN
    ...
    GEN_REWRITE_TAC "Library/transc.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN AP_TERM_TAC THEN
    REWRITE_TAC[REAL_INV_MUL] THEN
    GEN_REWRITE_TAC "Library/transc.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
    REWRITE_TAC[REAL_MUL_ASSOC] THEN AP_THM_TAC THEN AP_TERM_TAC THEN
    ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN REWRITE_TAC[GSYM real_div] THEN
 ...
 (`!x n. abs(sin x -
  ...
  W(C SUBGOAL_THEN (fun th -> REWRITE_TAC[th]) o funpow 2 lhand o snd) THENL
   [CONJ_TAC THENL
    ...
    SPEC_TAC(`d:num`,`d:num`) THEN CONV_TAC "Library/transc.ml:EXPAND_CASES_CONV" EXPAND_CASES_CONV THEN
    CONV_TAC "Library/transc.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SUM_EQ THEN X_GEN_TAC `r:num` THEN STRIP_TAC THEN
    ...
    DISCH_THEN(fun th -> GEN_REWRITE_TAC "Library/transc.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [th] THEN
      MP_TAC(SYM th)) THEN
    REWRITE_TAC[EVEN_ADD; EVEN_MULT; ARITH_EVEN] THEN
    UNDISCH_TAC `r MOD 4 < 4` THEN
    SPEC_TAC(`r MOD 4`,`d:num`) THEN CONV_TAC "Library/transc.ml:EXPAND_CASES_CONV" EXPAND_CASES_CONV THEN
    CONV_TAC "Library/transc.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[] THEN
...
let REAL_ATN_POWSER_DIFFS_SUM = prove
 (`!x. abs(x) < &1
  ...
  SUBST1_TAC THENL
   [ABS_TAC THEN
    CONV_TAC "Library/transc.ml:(LAND_CONV(LAND_CONV(RAND_CONV(TOP_DEPTH_CONV num_CONV))))" (LAND_CONV(LAND_CONV(RAND_CONV(TOP_DEPTH_CONV num_CONV)))) THEN
 ...
 (`!x. abs(x) < &1
  ...
  STRIP_ASSUME_TAC THENL
   [ALL_TAC;
    ...
    GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN ASM_SIMP_TAC[]] THEN
  ASM_CASES_TAC `x = &0` THENL
   ...
   [MATCH_MP_TAC(REAL_ARITH `(a = &0) /\ (b = &0) ==> (a - b = &0)`) THEN
    CONJ_TAC THENL
     [CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_UNIQ THEN
      ...
      CONV_TAC "Library/transc.ml:SYM_CONV" SYM_CONV THEN
      REWRITE_TAC[REAL_ENTIRE; REAL_POW_EQ_0] THEN ASM_MESON_TAC[EVEN];
      GEN_REWRITE_TAC "Library/transc.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM TAN_0] THEN
      MATCH_MP_TAC TAN_ATN THEN
      SIMP_TAC[PI2_BOUNDS; REAL_ARITH `&0 < x ==> --x < &0`]];
    ALL_TAC] THEN
  ASM_CASES_TAC `x = &0` THEN ASM_REWRITE_TAC[] THEN
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
 ...
 (`!x n. abs(x) < &1
  ...
  ASSUME_TAC THENL
   [GEN_TAC THEN COND_CASES_TAC THEN
    ...
    MATCH_MP_TAC REAL_LE_RMUL THEN SIMP_TAC[REAL_POW_LE; REAL_ABS_POS] THEN
    REWRITE_TAC[real_div; REAL_MUL_LID] THEN
    GEN_REWRITE_TAC "Library/transc.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_1] THEN
    MATCH_MP_TAC REAL_LE_INV2 THEN CONV_TAC "Library/transc.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
