...
let BINARYSUM_BITSET = prove
 (`!n. binarysum (bitset n) = n`,
  CONV_TAC "Library/binary.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN REWRITE_TAC[binarysum] THEN
...
let BINARYSUM_DIV = prove
 (`!k s. FINITE s
  ...
  CONJ_TAC THENL
   [AP_THM_TAC THEN AP_TERM_TAC THEN CONV_TAC "Library/binary.ml:SYM_CONV" SYM_CONV THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC BINARYSUM_DIV_DIVISIBLE THEN SIMP_TAC[IN_ELIM_THM] THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "Library/binary.ml:RAND_CONV" RAND_CONV [MULT_SYM] THEN
