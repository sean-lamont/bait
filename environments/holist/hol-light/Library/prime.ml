...
let MULT_FIX = prove(
  ...
  GEN_REWRITE_TAC "Library/prime.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV)
...
let DIFF_LEMMA = prove(
  ...
  GEN_REWRITE_TAC "Library/prime.ml:LAND_CONV" LAND_CONV [GSYM (CONJUNCT1 ADD_CLAUSES)] THEN
...
let DIFF_SQUARE = prove(
  ...
  DISJ_CASES_TAC(SPECL [`x:num`; `y:num`] LE_CASES) THENL
   [SUBGOAL_THEN `(x * x) <= (y * y)` MP_TAC THENL
    ...
    AP_TERM_TAC THEN GEN_REWRITE_TAC "Library/prime.ml:LAND_CONV" LAND_CONV [ADD_SYM] THEN
...
let DIVIDES_ONE = prove(
  ...
  CONV_TAC "Library/prime.ml:(LAND_CONV(ONCE_DEPTH_CONV SYM_CONV))" (LAND_CONV(ONCE_DEPTH_CONV SYM_CONV)) THEN
 ...
 (`!x y. x divides y /\ y divides x <=> (x = y)`,
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [REWRITE_TAC[divides] THEN
    ...
    CONV_TAC "Library/prime.ml:(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN
...
let DIVIDES_DIVIDES_DIV = prove
 (`!n d. 1 <= n /\ d divides n
  ...
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/prime.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [DIVIDES_DIV_MULT] THEN
...
let IND_EUCLID = prove(
  ...
  REPEAT STRIP_TAC THEN REPEAT_TCL DISJ_CASES_THEN MP_TAC
   ...
   [DISCH_THEN SUBST1_TAC THEN
    GEN_REWRITE_TAC "Library/prime.ml:RAND_CONV" RAND_CONV [GSYM ADD_0] THEN
  ...
  DISCH_THEN(fun th -> SUBST1_TAC(SYM(MATCH_MP SUB_ADD
   ...
   [DISCH_THEN SUBST1_TAC THEN
    FIRST_ASSUM (CONV_TAC "Library/prime.ml:o" o REWR_CONV) THEN
...
let BEZOUT_ADD = prove(
  ...
  REPEAT CONJ_TAC THENL
   [REPEAT GEN_TAC THEN REPEAT
     (AP_TERM_TAC THEN GEN_REWRITE_TAC "Library/prime.ml:I" I [FUN_EQ_THM] THEN
    GEN_TAC THEN BETA_TAC) THEN
    GEN_REWRITE_TAC "Library/prime.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [DISJ_SYM] THEN
    GEN_REWRITE_TAC "Library/prime.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [CONJ_SYM] THEN REFL_TAC;
...
let BEZOUT_ADD_STRONG = prove
 (`!a b. ~(a = 0)
  ...
  GEN_REWRITE_TAC "Library/prime.ml:(LAND_CONV o LAND_CONV o RAND_CONV)" (LAND_CONV o LAND_CONV o RAND_CONV)
  ...
  ANTS_TAC THENL
   [ONCE_REWRITE_TAC[AC MULT_AC `(b - 1) * b * x = b * (b - 1) * x`] THEN
    ...
    GEN_REWRITE_TAC "Library/prime.ml:LAND_CONV" LAND_CONV [ARITH_RULE `d = d * 1`] THEN
  ...
  DISCH_THEN(fun th ->
    ...
    MP_TAC th) THEN
  ASM_REWRITE_TAC[] THEN
  GEN_REWRITE_TAC "Library/prime.ml:(RAND_CONV o RAND_CONV o LAND_CONV)" (RAND_CONV o RAND_CONV o LAND_CONV) [LEFT_SUB_DISTRIB] THEN
  REWRITE_TAC[MULT_AC]);;
...
let GCD_LMUL = prove(
  ...
  REPEAT GEN_TAC THEN CONV_TAC "Library/prime.ml:SYM_CONV" SYM_CONV THEN
...
let GCD_RMUL = prove(
  ...
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/prime.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [MULT_SYM] THEN
...
let GCD_REFL = prove(
  `!a. gcd(a,a) = a`,
  GEN_TAC THEN CONV_TAC "Library/prime.ml:SYM_CONV" SYM_CONV THEN
  ONCE_REWRITE_TAC[GSYM GCD_UNIQUE] THEN
...
let GCD_MULTIPLE = prove(
  `!a b. gcd(b,a * b) = b`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/prime.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV)
...
let COPRIME = prove(
  ...
  REPEAT(EQ_TAC ORELSE STRIP_TAC) THEN ASM_REWRITE_TAC[DIVIDES_1] THENL
   [FIRST_ASSUM MATCH_MP_TAC;
    FIRST_ASSUM(CONV_TAC "Library/prime.ml:o" o REWR_CONV o GSYM) THEN CONJ_TAC] THEN
  ASM_REWRITE_TAC[]);;
...
let COPRIME_REXP = prove
 (`!m n k. coprime(m,n EXP k) <=> coprime(m,n) \/ k = 0`,
  ...
  REWRITE_TAC[EXP] THEN CONV_TAC "Library/prime.ml:NUMBER_RULE" NUMBER_RULE);;
...
let DIVISION_DECOMP = prove(
  ...
  ASM_CASES_TAC `gcd(a,b) = 0` THENL
   [ASM_REWRITE_TAC[] THEN EXISTS_TAC `1` THEN
    ...
    FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/prime.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [GSYM th]) THEN
    FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/prime.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV)
...
let PRIME_GE_2 = prove(
  `!p. prime(p) ==> 2 <= p`,
  GEN_TAC THEN CONV_TAC "Library/prime.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_LE] THEN
  REWRITE_TAC[num_CONV `2`; num_CONV `1`; LESS_THM; NOT_LESS_0] THEN
...
let COPRIME_PRIME_EQ = prove(
  ...
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [DISCH_THEN(fun th -> REWRITE_TAC[MATCH_MP COPRIME_PRIME th]);
    CONV_TAC "Library/prime.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[coprime] THEN
...
let EQ_PRIME_EXP = prove
 (`!p q m n. prime p /\ prime q
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Library/prime.ml:LAND_CONV" LAND_CONV [GSYM DIVIDES_ANTISYM] THEN
...
let COPRIME_2 = prove
 (`(!n. coprime(2,n) <=> ODD n) /\ (!n. coprime(n,2) <=> ODD n)`,
  GEN_REWRITE_TAC "Library/prime.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [COPRIME_SYM] THEN
...
let DIVIDES_EXP_PLUS1 = prove
 (`!n k. ODD k ==> (n + 1) divides (n EXP k + 1)`,
  ...
  INDUCT_TAC THEN CONV_TAC "Library/prime.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
 ...
 (`!k n. (n - 1) divides (n EXP k - 1)`,
  REPEAT GEN_TAC THEN ASM_CASES_TAC `n = 0` THENL
   [STRUCT_CASES_TAC(SPEC `k:num` num_CASES) THEN
    ASM_REWRITE_TAC[EXP; MULT_CLAUSES] THEN CONV_TAC "Library/prime.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    REWRITE_TAC[DIVIDES_REFL];
...
let PRIME_IRREDUCIBLE = prove
 (`!p. prime p <=>
  ...
  GEN_REWRITE_TAC "Library/prime.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [divides] THEN
  REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN
  ...
  REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Library/prime.ml:NUM_RING" NUM_RING);;
...
let COPRIME_POW = prove
 (`!n a b c. coprime(a,b) /\ a * b = c EXP n
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "Library/prime.ml:BINDER_CONV" BINDER_CONV [SWAP_FORALL_THM] THEN
  GEN_REWRITE_TAC "Library/prime.ml:I" I [SWAP_FORALL_THM] THEN ASM_CASES_TAC `n = 0` THEN
   ...
   [ASM_REWRITE_TAC[EXP_ZERO; MULT_EQ_0] THEN
    ...
    (ANTS_TAC THENL
      [MATCH_MP_TAC(NUM_RING `!p. ~(p = 0) /\ a * p = b * p ==> a = b`) THEN
       ...
       FIRST_X_ASSUM(MP_TAC o SYM) THEN CONV_TAC "Library/prime.ml:NUM_RING" NUM_RING;
...
let PRIME_EXP = prove
 (`!p n. prime(p EXP n) <=> prime(p) /\ (n = 1)`,
  ...
  GEN_REWRITE_TAC "Library/prime.ml:LAND_CONV" LAND_CONV [GSYM EXP_1] THEN
 ...
 (`!k x y p. prime p /\ (x * y = p EXP k)
  ...
  DISCH_THEN(DISJ_CASES_THEN (X_CHOOSE_THEN `d:num` SUBST_ALL_TAC)) THENL
   [UNDISCH_TAC `(p * d) * y = p * p EXP k`;
    ...
    GEN_REWRITE_TAC "Library/prime.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [MULT_SYM]] THEN
...
let INDEX_FACT_UNBOUNDED = prove
 (`!p n. prime p
  ...
  CONV_TAC "Library/prime.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC NSUM_SUPERSET THEN
...
let INDEX_EQ_0 = prove
 (`!p n. index p n = 0 <=> n = 0 \/ p = 1 \/ ~(p divides n)`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/prime.ml:LAND_CONV" LAND_CONV [ARITH_RULE `n = 0 <=> ~(1 <= n)`] THEN
   ...
   [REWRITE_TAC[divides] THEN AP_TERM_TAC THEN REWRITE_TAC[FUN_EQ_THM] THEN
    ...
    CONJ_TAC THENL
     [ASM_REWRITE_TAC[EQ_MULT_LCANCEL; GCD_ZERO];
      ...
      GEN_REWRITE_TAC "Library/prime.ml:LAND_CONV" LAND_CONV [MULT_SYM] THEN
...
let LCM = prove
 (`!m n. m divides lcm(m,n) /\
  ...
  ASM_SIMP_TAC[GCD_ZERO; LE_1; ADD_CLAUSES] THEN CONV_TAC "Library/prime.ml:SYM_CONV" SYM_CONV THENL
   [ALL_TAC; GEN_REWRITE_TAC "Library/prime.ml:RAND_CONV" RAND_CONV [MULT_SYM]] THEN
...
let DIVIDES_LCM_GCD = prove
 (`!m n d. d divides lcm(m,n) <=> d * gcd(m,n) divides m * n`,
  ...
  ASM_SIMP_TAC[DIV_MULT] THEN CONV_TAC "Library/prime.ml:NUMBER_RULE" NUMBER_RULE);;
...
let LCM_ZERO = prove
 (`!m n. lcm(m,n) = 0 <=> m = 0 \/ n = 0`,
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC "Library/prime.ml:LAND_CONV" LAND_CONV [MULTIPLES_EQ] THEN
...
let LCM_ASSOC = prove
 (`!m n p. lcm(m,lcm(n,p)) = lcm(lcm(m,n),p)`,
  ...
  REWRITE_TAC[LCM_ZERO] THEN CONV_TAC "Library/prime.ml:TAUT" TAUT);;
...
let LCM_GCD_DISTRIB = prove
 (`!a b c. lcm(a,gcd(b,c)) = gcd(lcm(a,b),lcm(a,c))`,
  ...
  SIMP_TAC[PRIMEPOW_DIVIDES_LCM; DIVIDES_GCD] THEN CONV_TAC "Library/prime.ml:TAUT" TAUT);;
...
let LCM_LMUL = prove
 (`!a b c. lcm(c * a,c * b) = c * lcm(a,b)`,
  ...
  CONV_TAC "Library/prime.ml:NUMBER_RULE" NUMBER_RULE);;
...
let LCM_EXP = prove
 (`!n a b. lcm(a EXP n,b EXP n) = lcm(a,b) EXP n`,
  ...
  CONV_TAC "Library/prime.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[GSYM DIVIDES_DIV_MULT] THEN
  CONV_TAC "Library/prime.ml:NUMBER_RULE" NUMBER_RULE);;
 ...
 (`!P. (!a b. 1 < a /\ 1 < b /\ coprime(a,b) /\ P a /\ P b ==> P(a * b)) /\
  ...
  CONJ_TAC THENL
   [FIRST_X_ASSUM(MP_TAC o C MATCH_MP (ARITH_RULE `k < k + 1`)) THEN
    ...
    GEN_REWRITE_TAC "Library/prime.ml:LAND_CONV" LAND_CONV [ARITH_RULE `m = 1 * m`] THEN
...
let GCD_CONV =
  ...
  let pth1 = prove
   (`!m n x y d m' n'.
    ...
    CONV_TAC "Library/prime.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN ASM_REWRITE_TAC[GSYM GCD_UNIQUE] THEN
