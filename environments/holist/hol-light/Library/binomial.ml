...
let BINOM_FACT = prove
 (`!n k. FACT n * FACT k * binom(n+k,k) = FACT(n + k)`,
  ...
  REWRITE_TAC[ADD_CLAUSES; FACT; binom] THEN CONV_TAC "Library/binomial.ml:NUM_RING" NUM_RING);;
...
let BINOM_GE_TOP = prove
 (`!m n. 1 <= m /\ m < n ==> n <= binom(n,m)`,
  ...
  CONV_TAC "Library/binomial.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN STRIP_TAC THEN ASM_CASES_TAC `m = 0` THEN
...
let BINOM_TOP_STEP = prove
 (`!n k. ((n + 1) - k) * binom(n + 1,k) = (n + 1) * binom(n,k)`,
  ...
  REWRITE_TAC[LT_NZ] THEN CONV_TAC "Library/binomial.ml:NUM_RING" NUM_RING);;
...
let BINOM_TOP_STEP_REAL = prove
 (`!n k. &(binom(n + 1,k)) =
  ...
  CONV_TAC "Library/binomial.ml:REAL_FIELD" REAL_FIELD);;
...
let BINOM = prove
 (`!n k. binom(n,k) =
  ...
  REPEAT GEN_TAC THEN COND_CASES_TAC THEN
  ASM_SIMP_TAC[BINOM_EQ_0; GSYM NOT_LE] THEN CONV_TAC "Library/binomial.ml:SYM_CONV" SYM_CONV THEN
 ...
 (`!m n k. k <= m
  ...
  ASM_CASES_TAC `n:num < m` THENL
   [REPEAT(COND_CASES_TAC THEN
    ...
    REWRITE_TAC[GSYM REAL_OF_NUM_EQ] THEN CONV_TAC "Library/binomial.ml:REAL_FIELD" REAL_FIELD]);;
