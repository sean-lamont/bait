...
let EQ_C_BIJECTIONS = prove
 (`!s:A->bool t:B->bool.
  ...
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Library/card.ml:I" I [FUN_EQ_THM] THEN
 ...
 (`s =_c t <=>
  ...
  REWRITE_TAC[eq_c] THEN EQ_TAC THENL
   [DISCH_THEN(X_CHOOSE_THEN `f:A->B` STRIP_ASSUME_TAC) THEN
    ...
    CONV_TAC "Library/card.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN ASM_MESON_TAC[];
    DISCH_THEN(CHOOSE_THEN (CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
    DISCH_THEN(CONJUNCTS_THEN2 MP_TAC ASSUME_TAC) THEN
    GEN_REWRITE_TAC "Library/card.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
...
let CARD_EQ_SYM = prove
 (`!s t. s =_c t <=> t =_c s`,
  ...
  GEN_REWRITE_TAC "Library/card.ml:RAND_CONV" RAND_CONV [SWAP_EXISTS_THM] THEN
 ...
 (`!s:A->bool t:B->bool. s <=_c t \/ t <=_c s`,
  ...
  THENL
   [FIRST_X_ASSUM(K ALL_TAC o SPEC `\(x:A,y:B). T`) THEN
    ...
    CONV_TAC "Library/card.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let CARD_LT_LE = prove
 (`!s t. s <_c t <=> s <=_c t /\ ~(s =_c t)`,
  REWRITE_TAC[lt_c; GSYM CARD_LE_ANTISYM] THEN CONV_TAC "Library/card.ml:TAUT" TAUT);;

let CARD_LE_LT = prove
 (`!s t. s <=_c t <=> s <_c t \/ s =_c t`,
  REPEAT GEN_TAC THEN REWRITE_TAC[GSYM CARD_NOT_LT] THEN
  GEN_REWRITE_TAC "Library/card.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [CARD_LT_LE] THEN
...
let INFINITE_CARD_LE = prove
 (`!s:A->bool. INFINITE s <=> (UNIV:num->bool) <=_c s`,
  ...
  SUBGOAL_THEN `!n. (f:num->A)(n) IN (s DIFF IMAGE f {m | m < n})` MP_TAC THENL
   [GEN_TAC THEN ONCE_ASM_REWRITE_TAC[] THEN CONV_TAC "Library/card.ml:SELECT_CONV" SELECT_CONV THEN
...
let CARD_LE_CARD = prove
 (`!s:A->bool t:B->bool.
  ...
  DISCH_TAC THEN GEN_REWRITE_TAC "Library/card.ml:LAND_CONV" LAND_CONV [CARD_LE_EQ_SUBSET] THEN
 ...
 (`!s:A->bool. ?l:A#A->bool. ordinal l /\ fl l =_c s`,
  ...
  CONJ_TAC THENL
   [FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ_ALT]
    ...
    GEN_REWRITE_TAC "Library/card.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
...
let CARD_LE_SQUARE = prove
 (`!s:A->bool. s <=_c s *_c s`,
  ...
  CONV_TAC "Library/card.ml:(ONCE_DEPTH_CONV SELECT_CONV)" (ONCE_DEPTH_CONV SELECT_CONV) THEN MESON_TAC[]);;
...
let CARD_SQUARE_NUM = prove
 (`(UNIV:num->bool) *_c (UNIV:num->bool) =_c (UNIV:num->bool)`,
  ...
  CONV_TAC "Library/card.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN MESON_TAC[NUMPAIR_INJ]);;
  ...
  ANTS_TAC THENL
   [CONJ_TAC THENL
     [FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN REWRITE_TAC[] THEN
      ...
      GEN_REWRITE_TAC "Library/card.ml:LAND_CONV" LAND_CONV [CARD_EQ_SYM] THEN
    ...
    GEN_REWRITE_TAC "Library/card.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
     [TAUT `a ==> b /\ c <=> (a ==> b) /\ (a ==> c)`] THEN
    GEN_REWRITE_TAC "Library/card.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [FORALL_AND_THM] THEN
    MATCH_MP_TAC(TAUT
     `(c /\ d ==> f) /\ (a /\ b ==> e)
      ==> (a /\ (b /\ c) /\ d ==> e /\ f)`) THEN
    CONJ_TAC THENL
...
let CARD_LE_UNIONS2 = prove
 (`!u:((A->bool)->bool) k:B->bool l:C->bool.
  ...
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/card.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [LE_C_IMAGE_SUBSET] THEN
...
let COUNTABLE_PRODUCT_DEPENDENT = prove
 (`!f:A->B->C s t.
  ...
  GEN_REWRITE_TAC "Library/card.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [RIGHT_IMP_EXISTS_THM] THEN
 ...
 (`!P s:A->bool.
  ...
  REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [INDUCT_TAC THEN REWRITE_TAC[LE] THEN
    ...
    GEN_REWRITE_TAC "Library/card.ml:I" I [EXTENSION] THEN
...
let COUNTABLE_UNION_OF_IDEMPOT = prove
 (`!P:(A->bool)->bool.
  ...
  GEN_REWRITE_TAC "Library/card.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
...
let COUNTABLE_UNION_OF_INTER = prove
 (`!P:(A->bool)->bool.
  ...
  GEN_REWRITE_TAC "Library/card.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [UNION_OF] THEN
...
let CARD_EQ_LIST_GEN = prove
 (`!s:A->bool. INFINITE(s) ==> {l | !x. MEM x l ==> x IN s} =_c s`,
  ...
  GEN_REWRITE_TAC "Library/card.ml:I" I [FORALL_CURRY] THEN
 ...
 (`(:real) =_c (:num->bool)`,
  ...
  REWRITE_TAC[GSYM CARD_LE_ANTISYM] THEN CONJ_TAC THENL
   [TRANS_TAC CARD_LE_TRANS `(:num) *_c (:num->bool)` THEN CONJ_TAC THENL
    ...
    GEN_REWRITE_TAC "Library/card.ml:LAND_CONV" LAND_CONV [num_WOP] THEN MATCH_MP_TAC MONO_EXISTS THEN
    ...
    ONCE_REWRITE_TAC[GSYM CONTRAPOS_THM] THEN
    REWRITE_TAC[EXTENSION; NOT_FORALL_THM] THEN
    GEN_REWRITE_TAC "Library/card.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
         ...
         [FIRST_X_ASSUM SUBST_ALL_TAC THEN
          ...
          CONJ_TAC THENL
           [MATCH_MP_TAC SUM_EQ_NUMSEG THEN
            ...
            CONV_TAC "Library/card.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let CARD_EXP_ADD = prove
 (`!s:A->bool t:B->bool u:C->bool.
  ...
  CONV_TAC "Library/card.ml:SYM_CONV" SYM_CONV THEN TRY(FIRST_X_ASSUM MATCH_MP_TAC) THEN
 ...
 (`!s:A->bool s':B->bool t:C->bool t':D->bool.
  ...
  ASM_CASES_TAC `s:A->bool = {}` THEN ASM_SIMP_TAC[CARD_EQ_EMPTY] THENL
   [STRIP_TAC THEN MP_TAC(ISPECL [`t:C->bool`; `0`] CARD_EXP_ZERO) THEN
    ASM_REWRITE_TAC[] THEN GEN_REWRITE_TAC "Library/card.ml:LAND_CONV" LAND_CONV [CARD_EQ_SYM] THEN
 ...
 (`(!s:A->bool t:B->bool.
  ...
  MATCH_MP_TAC lemma THEN REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC CARD_LE_SUBSET THEN
    ...
    CONJ_TAC THENL
     [ALL_TAC;
      GEN_REWRITE_TAC "Library/card.ml:I" I [LE_C] THEN
    ...
    TRANS_TAC CARD_LE_TRANS
     `{{}} UNION
      {k:A->bool | k SUBSET s /\ ~(k = {}) /\ k <=_c (t:B->bool)}` THEN
    CONJ_TAC THENL
     [MATCH_MP_TAC CARD_LE_SUBSET THEN GEN_REWRITE_TAC "Library/card.ml:I" I [SUBSET] THEN
