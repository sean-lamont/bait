...
let PERMUTES_INSERT = prove
 (`{p:A->A | p permutes (a INSERT s)} =
  ...
  CONV_TAC "Library/permutations.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN EQ_TAC THENL
...
let HAS_SIZE_PERMUTATIONS = prove
 (`!s:A->bool n. s HAS_SIZE n ==> {p | p permutes s} HAS_SIZE (FACT n)`,
  ...
  REWRITE_TAC[NOT_IN_EMPTY] THEN CONJ_TAC THENL
   [GEN_TAC THEN DISCH_THEN(SUBST1_TAC o SYM) THEN CONV_TAC "Library/permutations.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
    ALL_TAC] THEN
  ...
  CONV_TAC "Library/permutations.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let SWAP_COMMON' = prove
 (`!a b c:A. ~(a = b) /\ ~(a = c)
  ...
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Library/permutations.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [SWAP_SYM] THEN
...
let SWAPSEQ_INVERSE_EXISTS = prove
 (`!n p:A->A. swapseq n p ==> ?q. swapseq n q /\ p o q = I /\ q o p = I`,
  ...
  GEN_REWRITE_TAC "Library/permutations.ml:(BINOP_CONV o LAND_CONV o RAND_CONV)" (BINOP_CONV o LAND_CONV o RAND_CONV) [o_ASSOC] THEN
...
let FIXING_SWAPSEQ_DECREASE = prove
 (`!n p a b:A.
  ...
  GEN_REWRITE_TAC "Library/permutations.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [swapseq_CASES] THEN
...
let SWAPSEQ_IDENTITY_EVEN = prove
 (`!n. swapseq n (I:A->A) ==> EVEN n`,
  ...
  GEN_REWRITE_TAC "Library/permutations.ml:LAND_CONV" LAND_CONV [swapseq_CASES] THEN
...
let PERMUTATION_INVERSE_COMPOSE = prove
 (`!p q. permutation p /\ permutation q
  ...
  GEN_REWRITE_TAC "Library/permutations.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [o_ASSOC] THEN
...
let SIGN_IDEMPOTENT = prove
 (`!p. sign(p) * sign(p) = &1`,
  ...
  COND_CASES_TAC THEN ASM_REWRITE_TAC[] THEN CONV_TAC "Library/permutations.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let REAL_SGN_SIGN = prove
 (`!p:A->A. real_sgn(sign p) = sign p`,
  ...
  CONV_TAC "Library/permutations.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let SUM_PERMUTATIONS_INVERSE = prove
 (`!f m n. sum {p | p permutes m..n} f =
  ...
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/permutations.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [GSYM IMAGE_INVERSE_PERMUTATIONS] THEN
  GEN_REWRITE_TAC "Library/permutations.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV)
   [SET_RULE `{f x | p x} = IMAGE f {x | p x}`] THEN
  GEN_REWRITE_TAC "Library/permutations.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM o_DEF] THEN
...
let SUM_PERMUTATIONS_COMPOSE_L = prove
 (`!f m n q.
  ...
  REPEAT STRIP_TAC THEN
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/permutations.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV)
 ...
 (`!s:A->bool. FINITE s /\ 2 <= CARD s
  ...
  SUBST1_TAC THENL
   [FIRST_ASSUM(SUBST1_TAC o SYM o MATCH_MP CARD_PERMUTATIONS) THEN
    ...
    CONV_TAC "Library/permutations.ml:TAUT" TAUT;
 ...
 (`!p:A->A s. FINITE s /\ (!x. p(p x) = x) /\ (!x. ~(x IN s) ==> p x = x)
  ...
  TRANS_TAC LE_TRANS `CARD {a:A,p a}` THEN CONJ_TAC THENL
   [ASM_SIMP_TAC[CARD_CLAUSES; FINITE_RULES; IN_SING; NOT_IN_EMPTY] THEN
    CONV_TAC "Library/permutations.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
