...
let COUNT_BOUND = prove
 (`!s. count s n <= n`,
  ...
  GEN_REWRITE_TAC "Examples/schnirelmann.ml:RAND_CONV" RAND_CONV [GSYM CARD_NUMSEG_1] THEN
 ...
 (`!s. schnirelmann s = &1 <=> !n. 1 <= n ==> n IN s`,
  GEN_TAC THEN EQ_TAC THENL
   [ONCE_REWRITE_TAC[GSYM CONTRAPOS_THM] THEN
    ...
    REPEAT STRIP_TAC THEN
    GEN_REWRITE_TAC "Examples/schnirelmann.ml:RAND_CONV" RAND_CONV [GSYM SCHNIRELMANN_UNIV] THEN
...
let SCHNIRELMANN_THEOREM = prove
 (`!s t. 0 IN (s INTER t) /\ schnirelmann s + schnirelmann t >= &1
  ...
  GEN_REWRITE_TAC "Examples/schnirelmann.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_LID] THEN
 ...
 (`!s. 0 IN s /\ schnirelmann s > &0
  ...
  THENL
   [FIRST_ASSUM(MP_TAC o SPEC `n:num` o MATCH_MP CHOOSE_SUBSET) THEN
    ...
    GEN_REWRITE_TAC "Examples/schnirelmann.ml:RAND_CONV" RAND_CONV [ARITH_RULE `n = 2 * n - n`] THEN
...
let SCHNIRELMANN_DIRECT = prove
 (`!s. schnirelmann s > &0
  ...
  GEN_REWRITE_TAC "Examples/schnirelmann.ml:RAND_CONV" RAND_CONV [ADD_SYM] THEN AP_TERM_TAC THEN
