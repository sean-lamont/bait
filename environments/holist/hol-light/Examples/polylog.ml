...
let MAGIC_DERIVATIVE = prove
 (`!x. abs(x) < &1
   ...
   [REWRITE_TAC[GSYM CONJ_ASSOC] THEN
    ...
    REPEAT CONJ_TAC THENL
     [MATCH_MP_TAC(REAL_ARITH `&0 <= x ==> &0 < x + &1`) THEN
      ...
      GEN_REWRITE_TAC "Examples/polylog.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [REAL_ADD_SYM] THEN
    ...
    ALL_TAC] THEN
  STRIP_TAC THEN
  CONV_TAC "Examples/polylog.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  GEN_REWRITE_TAC "Examples/polylog.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [GSYM REAL_POW_2] THEN
  ...
  THENL
   [GEN_TAC THEN
    ...
    GEN_REWRITE_TAC "Examples/polylog.ml:LAND_CONV" LAND_CONV [REAL_ARITH
  ...
  CONV_TAC "Examples/polylog.ml:(TOP_DEPTH_CONV num_CONV)" (TOP_DEPTH_CONV num_CONV) THEN
  REWRITE_TAC[real_pow] THEN CONV_TAC "Examples/polylog.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  REAL_ARITH_TAC);;
 ...
 (`!a b x. ~(a = 0) /\ ~(b = 0) /\ abs(x) < &1
  ...
  EXISTS_TAC `\n. abs(x) pow (a * n + b)` THEN CONJ_TAC THENL
   [EXISTS_TAC `0` THEN X_GEN_TAC `n:num` THEN DISCH_TAC THEN
    ...
    GEN_REWRITE_TAC "Examples/polylog.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
...
let POLYLOG_DERIVATIVE = prove
 (`!a b x. ~(a = 0) /\ ~(b = 0) /\ abs(x) < &1
   ...
   [MATCH_MP_TAC TERMDIFF_STRONG THEN
    ...
    EXISTS_TAC `\n. abs(k) pow n` THEN CONJ_TAC THENL
     [EXISTS_TAC `0` THEN X_GEN_TAC `n:num` THEN DISCH_TAC THEN
      ...
      GEN_REWRITE_TAC "Examples/polylog.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_RID] THEN
      MATCH_MP_TAC REAL_LE_LMUL THEN REWRITE_TAC[REAL_ABS_POS] THEN
   ...
   [REWRITE_TAC[SUC_SUB1] THEN MATCH_MP_TAC SER_COMPAR THEN
    EXISTS_TAC `\n. abs(x pow a) pow n` THEN CONJ_TAC THENL
     [EXISTS_TAC `0` THEN X_GEN_TAC `n:num` THEN DISCH_TAC THEN
      ...
      GEN_REWRITE_TAC "Examples/polylog.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
  ...
  CONV_TAC "Examples/polylog.ml:(LAND_CONV(ONCE_DEPTH_CONV(ALPHA_CONV `n:num`)))" (LAND_CONV(ONCE_DEPTH_CONV(ALPHA_CONV `n:num`))) THEN
  ...
  MATCH_MP_TAC SER_CMUL THEN
  GEN_REWRITE_TAC "Examples/polylog.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [REAL_MUL_SYM] THEN
  REWRITE_TAC[GSYM REAL_POW_POW] THEN
 ...
 (`(\n. inv(&16 pow n) * (&4 / &(8 * n + 1) -
  ...
  ASSUME_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    CONV_TAC "Examples/polylog.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ASM_REWRITE_TAC[real_pow] THEN
    DISCH_THEN(MP_TAC o SPEC `&4 * sqrt(&2)` o MATCH_MP DIFF_CMUL) THEN
    MP_TAC(SPECL [`8`; `4`; `x:real`] POLYLOG_DERIVATIVE) THEN
    CONV_TAC "Examples/polylog.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ASM_REWRITE_TAC[] THEN
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "Examples/polylog.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV; ALL_TAC] THEN
  MP_TAC(SPEC
   `\x. suminf (\n. &4 * sqrt (&2) * x pow (8 * n + 1) / &(8 * n + 1) -
                    &8 * x pow (8 * n + 4) / &(8 * n + 4) -
                    &4 * sqrt (&2) * x pow (8 * n + 5) / &(8 * n + 5) -
     ...
     [MATCH_MP_TAC(REAL_ARITH
      ...
      EXISTS_TAC `sqrt((&5 / &4) pow 2)` THEN CONJ_TAC THENL
       [SIMP_TAC[POW_2_SQRT; REAL_LE_DIV; REAL_POS] THEN
        CONV_TAC "Examples/polylog.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV; ALL_TAC] THEN
      MATCH_MP_TAC SQRT_MONO_LE THEN CONV_TAC "Examples/polylog.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
      ALL_TAC] THEN
  ...
  CONV_TAC "Examples/polylog.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  CONV_TAC "Examples/polylog.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[LN_1; REAL_ADD_LID] THEN
  REWRITE_TAC[GSYM REAL_ADD_LDISTRIB] THEN
  ONCE_REWRITE_TAC[ADD_SYM] THEN
  ...
  SUBGOAL_THEN `inv(sqrt(&2)) pow 2 = &1 / &2` SUBST1_TAC THENL
   [REWRITE_TAC[REAL_POW_INV; real_div; REAL_MUL_LID] THEN AP_TERM_TAC THEN
    SIMP_TAC[SQRT_POW_2; REAL_POS]; ALL_TAC] THEN
  REWRITE_TAC[REAL_MUL_ASSOC] THEN CONV_TAC "Examples/polylog.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ONCE_REWRITE_TAC[ADD_SYM] THEN
  ...
  MP_TAC THENL
   [MATCH_MP_TAC SER_COMPAR THEN
    EXISTS_TAC `\n. &8 / &16 pow n` THEN CONJ_TAC THENL
     [EXISTS_TAC `0` THEN X_GEN_TAC `n:num` THEN DISCH_TAC THEN
      REWRITE_TAC[REAL_ABS_MUL; GSYM REAL_ABS_POW; REAL_ABS_NUM] THEN
      GEN_REWRITE_TAC "Examples/polylog.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
    ...
    MATCH_MP_TAC SER_CMUL THEN
    MATCH_MP_TAC GP THEN CONV_TAC "Examples/polylog.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV; ALL_TAC] THEN
  ...
  REWRITE_TAC[ATN_TAN] THEN CONV_TAC "Examples/polylog.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  CONJ_TAC THENL
   [SUBST1_TAC(SYM ATN_0) THEN MATCH_MP_TAC ATN_MONO_LT THEN
    CONV_TAC "Examples/polylog.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    MATCH_MP_TAC ATN_LT_PI4_POS THEN CONV_TAC "Examples/polylog.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV]);;
