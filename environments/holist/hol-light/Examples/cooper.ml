...
let INT_MOD_LEMMA = prove
 (`!d x. &0 < d ==> ?c. &1 <= x + c * d /\ x + c * d <= d`,
  ...
  REWRITE_TAC[NOT_FORALL_THM] THEN GEN_REWRITE_TAC "Examples/cooper.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
...
let MINUSINF_REPEATS = prove
 (`!p c d x. alldivide d p
  ...
  CONV_TAC "Examples/cooper.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN MATCH_MP_TAC cform_INDUCT THEN
...
let MAINTHM_A = prove
 (`!p d. alldivide d p /\ &0 < d
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Examples/cooper.ml:LAND_CONV" LAND_CONV [GSYM EXISTS_NEG] THEN
...
let INT_LE_CONV,INT_LT_CONV,
  ...
  let [pth_lt1; pth_lt2a; pth_lt2b; pth_lt3] = (CONJUNCTS o prove)
   (`(&m < --(&n) <=> F) /\
    ...
    CONV_TAC "Examples/cooper.ml:TAUT" TAUT) in
...
let ADJUSTCOEFF_CONV =
  ...
  and pth_eq = prove
   (`~(d = &0) ==> ((&0 = e) <=> (&0 = d * e))`,
    DISCH_TAC THEN CONV_TAC "Examples/cooper.ml:(BINOP_CONV SYM_CONV)" (BINOP_CONV SYM_CONV) THEN
