...
let BRUNN_MINKOWSKI_INTERVAL = prove
 (`!a b c d:real^N.
  ...
  ASM_SIMP_TAC[MEASURE_INTERVAL; CONTENT_CLOSED_INTERVAL; VECTOR_ADD_COMPONENT;
               REAL_ARITH `a <= b /\ c <= d ==> a + c <= b + d`] THEN
  GEN_REWRITE_TAC "Examples/brunn_minkowski.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
 ...
 (`!s t:real^N->bool.
  ...
  REPEAT CONJ_TAC THENL
   [REPEAT GEN_TAC THEN MATCH_MP_TAC MONO_IMP THEN
    ...
    CONV_TAC "Examples/brunn_minkowski.ml:(LAND_CONV(BINOP_CONV HAS_SIZE_CONV))" (LAND_CONV(BINOP_CONV HAS_SIZE_CONV)) THEN
     ...
     [REMOVE_THEN "+" (K ALL_TAC) THEN REMOVE_THEN "*" MP_TAC THEN
           ...
           [ASM_REWRITE_TAC[UNION_EMPTY] THEN
            MATCH_MP_TAC NEGLIGIBLE_UNIONS THEN CONJ_TAC THENL
             [MATCH_MP_TAC FINITE_RESTRICT THEN
              ...
              FIRST_ASSUM(fun th ->
                GEN_REWRITE_TAC "Examples/brunn_minkowski.ml:I" I [MATCH_MP FORALL_IN_DIVISION th]) THEN
      ...
      MP_TAC THENL
       [FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP CHOOSE_SUBSET o
        ...
        ALL_TAC] THEN
      CONV_TAC "Examples/brunn_minkowski.ml:(LAND_CONV(ONCE_DEPTH_CONV HAS_SIZE_CONV))" (LAND_CONV(ONCE_DEPTH_CONV HAS_SIZE_CONV)) THEN
      ...
      GEN_REWRITE_TAC "Examples/brunn_minkowski.ml:LAND_CONV" LAND_CONV [REAL_LE_BETWEEN] THEN
   ...
   [ALL_TAC;
    ...
    MP_TAC THENL
     [CONJ_TAC THEN MATCH_MP_TAC(SET_RULE
      ...
      REAL_ARITH_TAC;
      DISCH_THEN(fun th ->
        GEN_REWRITE_TAC "Examples/brunn_minkowski.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [th])] THEN
  ...
  ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
   [REPEAT CONJ_TAC THENL
     [EXPAND_TAC "n" THEN MATCH_MP_TAC LTE_ADD2 THEN
      ...
      CONV_TAC "Examples/brunn_minkowski.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN ASM_SIMP_TAC[REAL_LT_IMP_NZ];
      ...
      REPEAT CONJ_TAC THENL
       [FIRST_ASSUM(fun th ->
        ...
        GEN_REWRITE_TAC "Examples/brunn_minkowski.ml:LAND_CONV" LAND_CONV [SET_RULE `s = {} UNION s`] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [CONJ_TAC THEN MATCH_MP_TAC MEASURABLE_COMPACT THEN
    ...
    ASM_MESON_TAC[ELEMENTARY_COMPACT];
    ALL_TAC] THEN
 ASM (CONV_TAC "Examples/brunn_minkowski.ml:o" o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 5)
  ...
  THENL
   [MAP_EVERY EXPAND_TAC ["dl"; "dr"] THEN
    ...
    REWRITE_TAC[SET_RULE `{x | x IN s /\ ~(x = a)} = s DELETE a`] THEN
    CONJ_TAC THEN
    GEN_REWRITE_TAC "Examples/brunn_minkowski.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [SET_RULE `s = {} UNION s`] THEN
    REWRITE_TAC[GSYM UNIONS_INSERT] THEN
    REWRITE_TAC[SET_RULE `x INSERT (s DELETE x) = x INSERT s`] THEN
    REWRITE_TAC[UNIONS_INSERT; UNION_EMPTY] THEN
    REWRITE_TAC[GSYM SIMPLE_IMAGE; GSYM INTER_UNIONS] THEN
   ...
   [MATCH_MP_TAC REALLIM_TRANSFORM_EVENTUALLY THEN
    ...
    REWRITE_TAC[] THEN CONJ_TAC THENL
     [REWRITE_TAC[EVENTUALLY_WITHINREAL] THEN
      ...
      REPEAT STRIP_TAC THEN BINOP_TAC THEN CONV_TAC "Examples/brunn_minkowski.ml:SYM_CONV" SYM_CONV THEN
      ...
      GEN_REWRITE_TAC "Examples/brunn_minkowski.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
   ...
   [ASM_SIMP_TAC[MEASURE_EMPTY; ROOT_0; DIMINDEX_NONZERO; REAL_ADD_RID] THEN
    ...
    EXISTS_TAC `measure(IMAGE (\x:real^N. a + x) s)` THEN CONJ_TAC THENL
     [ASM_SIMP_TAC[MEASURE_TRANSLATION; MEASURE_INTERIOR;
      ...
      ASM_SIMP_TAC[MEASURABLE_TRANSLATION_EQ; MEASURABLE_CONVEX;
                   CONVEX_SUMS; BOUNDED_SUMS] THEN
      GEN_REWRITE_TAC "Examples/brunn_minkowski.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [VECTOR_ADD_SYM] THEN
      ASM SET_TAC[]];
    ALL_TAC] THEN
...
let INTERS_SUMS_CLOSED_BALL_SEQUENTIAL = prove
 (`!s:real^N->bool.
        ...
        ==> INTERS {{x + d | x IN s /\ d IN ball(vec 0,inv(&n + &1))} |
                    n IN (:num)} = s`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Examples/brunn_minkowski.ml:I" I [EXTENSION] THEN
  ...
  X_GEN_TAC `x:real^N` THEN EQ_TAC THENL
   [ASM_CASES_TAC `(x:real^N) IN s` THEN ASM_REWRITE_TAC[] THEN
    ...
    GEN_REWRITE_TAC "Examples/brunn_minkowski.ml:LAND_CONV" LAND_CONV [REAL_ARCH_INV] THEN
 ...
 (`!s t:real^N->bool.
  ...
  REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ASM (CONV_TAC "Examples/brunn_minkowski.ml:o" o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 5)
    ...
    ASM (CONV_TAC "Examples/brunn_minkowski.ml:o" o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 6)
     [REAL_ROOT_RPOW; DIMINDEX_NONZERO; MEASURE_POS_LE; MEASURABLE_COMPACT;
