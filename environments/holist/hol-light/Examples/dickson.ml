...
let MINIMAL_BAD_SEQUENCE = prove
 (`!(bad:(num->A)->bool) (m:A->num).
  ...
  CONV_TAC "Examples/dickson.ml:SELECT_CONV" SELECT_CONV THEN REWRITE_TAC[LEFT_AND_EXISTS_THM] THEN
 ...
 (`!n x:num->num->num. ?i j. i < j /\ (!k. k < n ==> x i k <= x j k)`,
  ...
  STRIP_ASSUME_TAC THENL
   [MATCH_MP_TAC MINIMAL_BAD_SEQUENCE THEN ASM_REWRITE_TAC[] THEN
    ...
    GEN_REWRITE_TAC "Examples/dickson.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [LT_SUC_LE] THEN
