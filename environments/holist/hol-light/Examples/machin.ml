...
let REAL_LT_1_POW2 = prove
 (`!n. &1 < &2 pow n <=> ~(n = 0)`,
  ...
  CONV_TAC "Examples/machin.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let REAL_POW2_CLAUSES = prove
 (`(!n. &0 <= &2 pow n) /\
  ...
  CONV_TAC "Examples/machin.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
 ...
 (`&0 < &2 pow n /\
  ...
  SUBGOAL_THEN `!m n. &2 pow m <= &2 pow n <=> m <= n` ASSUME_TAC THENL
   [REPEAT GEN_TAC THEN EQ_TAC THEN
    ...
    CONV_TAC "Examples/machin.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
 ...
 (`!x. abs(x) < &1
  ...
  EXISTS_TAC `\n. abs(x) pow n` THEN CONJ_TAC THENL
   [EXISTS_TAC `0` THEN REPEAT STRIP_TAC THEN REWRITE_TAC[] THEN
    ...
    GEN_REWRITE_TAC "Examples/machin.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_RID] THEN
...
let REAL_ATN_POWSER_DIFFS_SUM = prove
 (`!x. abs(x) < &1
  ...
  SUBST1_TAC THENL
   [ABS_TAC THEN
    CONV_TAC "Examples/machin.ml:(LAND_CONV(LAND_CONV(RAND_CONV(TOP_DEPTH_CONV num_CONV))))" (LAND_CONV(LAND_CONV(RAND_CONV(TOP_DEPTH_CONV num_CONV)))) THEN
  ...
  SUBGOAL_THEN `(\n. --(x pow 2) pow n) sums inv (&1 + x pow 2)` MP_TAC THENL
   [ONCE_REWRITE_TAC[REAL_ARITH `&1 + x = &1 - (--x)`] THEN
    ...
    GEN_REWRITE_TAC "Examples/machin.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
 ...
 (`!x. abs(x) < &1
  ...
  STRIP_ASSUME_TAC THENL
   [ALL_TAC;
    ...
    GEN_REWRITE_TAC "Examples/machin.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN ASM_SIMP_TAC[]] THEN
 ...
 (`!x. abs(x) < &1
  ...
  MP_TAC THENL
   [MATCH_MP_TAC(REAL_ARITH `(a = &0) /\ (b = &0) ==> (a - b = &0)`) THEN
    CONJ_TAC THENL
     [CONV_TAC "Examples/machin.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_UNIQ THEN
      ...
      CONV_TAC "Examples/machin.ml:SYM_CONV" SYM_CONV THEN
      REWRITE_TAC[REAL_ENTIRE; REAL_POW_EQ_0] THEN ASM_MESON_TAC[EVEN];
      GEN_REWRITE_TAC "Examples/machin.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM TAN_0] THEN
      MATCH_MP_TAC TAN_ATN THEN
      SIMP_TAC[PI2_BOUNDS; REAL_ARITH `&0 < x ==> --x < &0`]];
    ALL_TAC] THEN
  ASM_CASES_TAC `x = &0` THEN ASM_REWRITE_TAC[] THEN
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Examples/machin.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
  ...
  THENL
   [DISCH_THEN(MP_TAC o SPECL [`&0`; `x:real`]);
    CONV_TAC "Examples/machin.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
 ...
 (`!x n k. abs(x) <= inv(&2 pow k) /\ ~(k = 0)
  ...
  EXISTS_TAC `suminf (\m. abs(x) pow (m + n))` THEN CONJ_TAC THENL
   [ALL_TAC;
    ...
    GEN_REWRITE_TAC "Examples/machin.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
    MATCH_MP_TAC REAL_LE_LMUL THEN SIMP_TAC[REAL_ABS_POS; REAL_POW_LE] THEN
    GEN_REWRITE_TAC "Examples/machin.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_INV] THEN
    ...
    MATCH_MP_TAC REAL_LE_INV2 THEN CONV_TAC "Examples/machin.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    GEN_REWRITE_TAC "Examples/machin.ml:LAND_CONV" LAND_CONV [GSYM REAL_POW_1] THEN
  ...
  ASSUME_TAC THENL
   [GEN_TAC THEN COND_CASES_TAC THEN
    ...
    REWRITE_TAC[real_div; REAL_MUL_LID] THEN
    GEN_REWRITE_TAC "Examples/machin.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_1] THEN
    MATCH_MP_TAC REAL_LE_INV2 THEN CONV_TAC "Examples/machin.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
 ...
 (`!x n k. abs(x) <= inv(&2 pow k) /\ ~(k = 0)
  ...
  REPEAT STRIP_TAC THEN ASM_CASES_TAC `n = 0` THENL
   [ASM_REWRITE_TAC[sum; REAL_SUB_RZERO; MULT_CLAUSES; SUB_0] THEN
    ...
    CONV_TAC "Examples/machin.ml:(ONCE_DEPTH_CONV REAL_SUM_CONV)" (ONCE_DEPTH_CONV REAL_SUM_CONV) THEN
    REWRITE_TAC[real_pow; REAL_POW_1] THEN CONV_TAC "Examples/machin.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    CONV_TAC "Examples/machin.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    ...
    CONV_TAC "Examples/machin.ml:(RAND_CONV REAL_RAT_REDUCE_CONV)" (RAND_CONV REAL_RAT_REDUCE_CONV) THEN REWRITE_TAC[REAL_POW_1] THEN
 ...
 (`!x y. ~(x * y = &1) /\
  ...
  SUBGOAL_THEN `tan(atn(x) + atn(y)) = (x + y) / (&1 - x * y)` MP_TAC THENL
   [ASM_SIMP_TAC[ATN_TAN; TAN_ADD; COS_ATN_NZ; TAN_ADD_ATN_SIDECOND];
    DISCH_THEN(SUBST1_TAC o SYM) THEN CONV_TAC "Examples/machin.ml:SYM_CONV" SYM_CONV THEN
...
let ATN_ADD_SMALL_LEMMA_POS = prove
 (`!x y. &0 < y /\ x * y < &1
  ...
  SUBGOAL_THEN `pi / &2 - atn y = atn(tan(pi / &2 - atn y))` SUBST1_TAC THENL
   [CONV_TAC "Examples/machin.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC TAN_ATN THEN
    MATCH_MP_TAC(REAL_ARITH
     `&0 < x /\ x < a ==> --a < a - x /\ a - x < a`) THEN
    REWRITE_TAC[ATN_BOUNDS] THEN
    GEN_REWRITE_TAC "Examples/machin.ml:LAND_CONV" LAND_CONV [GSYM ATN_0] THEN
...
let ATN_ADD_SMALL_LEMMA = prove
 (`!x y. abs(x * y) < &1 ==> abs(atn(x) + atn(y)) < pi / &2`,
  ...
  GEN_REWRITE_TAC "Examples/machin.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM ATN_0] THEN
...
let pi_approx_rule,PI_APPROX_RULE =
  ...
  let pth = prove
   (`(q1 = p + 5) /\
    ...
    CONV_TAC "Examples/machin.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    REWRITE_TAC[real_div; REAL_MUL_LID] THEN
    GEN_REWRITE_TAC "Examples/machin.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
    ...
    GEN_REWRITE_TAC "Examples/machin.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [ADD_SYM] THEN
    ...
    CONV_TAC "Examples/machin.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV) in
...
let pi_approx_binary_rule,PI_APPROX_BINARY_RULE =
  let pth = prove
   (`abs(x - r) <= inv(&2 pow (SUC p))
    ...
    CONV_TAC "Examples/machin.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "Examples/machin.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV)
  and num_2 = Int 2 in
