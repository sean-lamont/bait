...
let TERM_VALUATION_VFREE_IN = prove
 (`!tau sigma1 sigma2 t.
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "Model/semantics.ml:BINDER_CONV" BINDER_CONV [SWAP_FORALL_THM] THEN
  GEN_REWRITE_TAC "Model/semantics.ml:I" I [SWAP_FORALL_THM] THEN MATCH_MP_TAC term_INDUCT THEN
...
let TERM_VALUATION_ITLIST = prove
 (`!ilist sigma tau.
  ...
  CONV_TAC "Model/semantics.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let ITLIST_VALMOD_FILTER = prove
 (`!ilist sigma sem x ty y yty.
  ...
  GEN_REWRITE_TAC "Model/semantics.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [COND_RAND] THEN
  GEN_REWRITE_TAC "Model/semantics.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [COND_RATOR] THEN
 ...
 (`!tm sigma tau ilist.
  ...
  REWRITE_TAC[VALMOD] THENL
   [ALL_TAC;
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "Model/semantics.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
 ...
 (`!n tm env tyin.
  ...
  REPEAT CONJ_TAC THENL
   [POP_ASSUM(K ALL_TAC) THEN
    ...
    GEN_REWRITE_TAC "Model/semantics.ml:I" I [IMP_CONJ] THEN
   ...
   [ALL_TAC;
    ...
    CONJ_TAC THENL
     [ASM_REWRITE_TAC[VFREE_IN; term_INJ] THEN
      ...
      AP_TERM_TAC THEN GEN_REWRITE_TAC "Model/semantics.ml:I" I [FUN_EQ_THM] THEN
  ...
  ASM_REWRITE_TAC[] THEN
  GEN_REWRITE_TAC "Model/semantics.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
  ...
  REWRITE_TAC[DEST_VAR] THEN REWRITE_TAC[CONJ_ASSOC] THEN CONJ_TAC THENL
   [ASM_REWRITE_TAC[] THEN CONJ_TAC THEN
    ...
    MAP_EVERY X_GEN_TAC [`k:string`; `kty:type`] THEN
    REWRITE_TAC[VALMOD] THEN CONV_TAC "Model/semantics.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let HOL_IS_CONSISTENT = prove
 (`?p. p has_type Bool /\ ~([] |- p)`,
  ...
  CONV_TAC "Model/semantics.ml:SELECT_CONV" SELECT_CONV THEN MATCH_MP_TAC TYPESET_INHABITED THEN
