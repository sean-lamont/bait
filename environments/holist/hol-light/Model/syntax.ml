...
let WELLTYPED_CLAUSES = prove
 (`(!n ty. welltyped(Var n ty)) /\
  ...
  (GEN_REWRITE_TAC "Model/syntax.ml:BINDER_CONV" BINDER_CONV [has_type_CASES] ORELSE
   GEN_REWRITE_TAC "Model/syntax.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [has_type_CASES]) THEN
...
let EQUATION_HAS_TYPE_BOOL = prove
 (`!s t. (s === t) has_type Bool
  ...
  GEN_REWRITE_TAC "Model/syntax.ml:(LAND_CONV o BINDER_CONV o LAND_CONV)" (LAND_CONV o BINDER_CONV o LAND_CONV) [has_type_CASES] THEN
  REWRITE_TAC[term_DISTINCT; term_INJ] THEN
  REWRITE_TAC[GSYM CONJ_ASSOC; RIGHT_EXISTS_AND_THM] THEN
  REWRITE_TAC[UNWIND_THM1] THEN
  GEN_REWRITE_TAC "Model/syntax.ml:(LAND_CONV o funpow 2(BINDER_CONV o LAND_CONV))" (LAND_CONV o funpow 2(BINDER_CONV o LAND_CONV))
    [has_type_CASES] THEN
...
let RACONV = prove
 (`(RACONV env (Var x1 ty1,Var x2 ty2) <=>
  ...
  GEN_REWRITE_TAC "Model/syntax.ml:LAND_CONV" LAND_CONV [RACONV_CASES] THEN
...
let ALPHAVARS_REFL = prove
 (`!env t. ALL (\(s,t). s = t) env ==> ALPHAVARS env (t,t)`,
  ...
  CONV_TAC "Model/syntax.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN MESON_TAC[PAIR_EQ]);;
 ...
 (`!t env. ALL (\(s,t). s = t) env ==> RACONV env (t,t)`,
  ...
  REWRITE_TAC[RACONV] THEN REPEAT STRIP_TAC THENL
   [ASM_SIMP_TAC[ALPHAVARS_REFL];
    ...
    CONV_TAC "Model/syntax.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN ASM_REWRITE_TAC[]]);;
...
let ALPHAVARS_TYPE = prove
 (`!env s t. ALPHAVARS env (s,t) /\
  ...
  CONV_TAC "Model/syntax.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let VFREE_IN_FINITE_ALT = prove
 (`!t ty. FINITE {x | VFREE_IN (Var x ty) t}`,
  ...
  EXISTS_TAC `Var x ty` THEN CONV_TAC "Model/syntax.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
  ASM_REWRITE_TAC[]);;
...
let VFREE_IN_VSUBST = prove
 (`!tm u uty ilist.
  ...
  GEN_REWRITE_TAC "Model/syntax.ml:LAND_CONV" LAND_CONV [term_INJ] THEN
  ...
  CONV_TAC "Model/syntax.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN MESON_TAC[]);;
