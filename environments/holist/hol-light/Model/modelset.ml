...
let SET_DECOMP = prove
 (`!s. isaset s
  ...
  REWRITE_TAC[set] THEN CONV_TAC "Model/modelset.ml:SELECT_CONV" SELECT_CONV THEN
 ...
 (`!x. x <: boolset <=> (x = true) \/ (x = false)`,
  ...
  SUBGOAL_THEN `IMAGE I_BOOL UNIV = {I_BOOL F,I_BOOL T}` SUBST1_TAC THENL
   [REWRITE_TAC[EXTENSION; IN_IMAGE; IN_UNIV; IN_INSERT; NOT_IN_EMPTY] THEN
    ...
    ALL_TAC] THEN
  GEN_TAC THEN
  GEN_REWRITE_TAC "Model/modelset.ml:(RAND_CONV o BINOP_CONV o LAND_CONV)" (RAND_CONV o BINOP_CONV o LAND_CONV) [GSYM SET] THEN
