...
let CLOSED_IN_INTERS = prove
 (`!top k:(A->bool)->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/metric.ml:I" I [EXTENSION] THEN ONCE_REWRITE_TAC[SIMPLE_IMAGE] THEN
...
let TOPSPACE_SUBTOPOLOGY = prove
 (`!top u. topspace(subtopology top u) = topspace top INTER u`,
  ...
  GEN_REWRITE_TAC "Multivariate/metric.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_ELIM_THM]);;
...
let DERIVED_SET_OF_SUBTOPOLOGY = prove
 (`!top u s:A->bool.
  ...
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/metric.ml:I" I [EXTENSION] THEN
...
let DERIVED_SET_OF_UNION = prove
 (`!top s t:A->bool.
  ...
  ASM_REWRITE_TAC[] THEN GEN_REWRITE_TAC "Multivariate/metric.ml:I" I [GSYM CONTRAPOS_THM] THEN
...
let CLOSURE_OF_SUBSET_EQ = prove
 (`!top s:A->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/metric.ml:RAND_CONV" RAND_CONV [OPEN_IN_SUBOPEN] THEN
...
let CLOSURE_OF_HULL = prove
 (`!top s:A->bool.
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/metric.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC HULL_UNIQUE THEN
...
let INTERIOR_OF_EQ = prove
 (`!top s:A->bool. (top interior_of s = s) <=> open_in top s`,
  ...
  GEN_REWRITE_TAC "Multivariate/metric.ml:RAND_CONV" RAND_CONV [OPEN_IN_SUBOPEN] THEN MESON_TAC[SUBSET]);;
...
let OPEN_IN_INTERIOR_OF = prove
 (`!top s:A->bool. open_in top (top interior_of s)`,
  ...
  GEN_REWRITE_TAC "Multivariate/metric.ml:I" I [OPEN_IN_SUBOPEN] THEN
...
let INTERIOR_OF_UNIONS_OPEN_IN_SUBSETS = prove
 (`!top s:A->bool.
  ...
  REPEAT GEN_TAC THEN CONV_TAC "Multivariate/metric.ml:SYM_CONV" SYM_CONV THEN
...
let INTERIOR_OF_CLOSURE_OF = prove
 (`!top s:A->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/metric.ml:LAND_CONV" LAND_CONV [INTERIOR_OF_RESTRICT] THEN
...
let FRONTIER_OF_RESTRICT = prove
 (`!top s:A->bool. top frontier_of s = top frontier_of (topspace top INTER s)`,
  ...
  BINOP_TAC THEN GEN_REWRITE_TAC "Multivariate/metric.ml:LAND_CONV" LAND_CONV [CLOSURE_OF_RESTRICT] THEN
  AP_TERM_TAC THEN SET_TAC[]);;
...
let FRONTIER_OF_COMPLEMENT = prove
 (`!top s:A->bool. top frontier_of (topspace top DIFF s) = top frontier_of s`,
  ...
  REWRITE_TAC[FRONTIER_OF_CLOSURES] THEN
  GEN_REWRITE_TAC "Multivariate/metric.ml:RAND_CONV" RAND_CONV [INTER_COMM] THEN
 ...
 (`!e. eventually (\n. inv(&n) < e) sequentially <=> &0 < e`,
  GEN_TAC THEN REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN EQ_TAC THENL
   [DISCH_THEN(X_CHOOSE_THEN `n:num` (MP_TAC o SPEC `n + 1`)) THEN
    ...
    GEN_REWRITE_TAC "Multivariate/metric.ml:LAND_CONV" LAND_CONV [REAL_ARCH_INV] THEN
...
let ARCH_EVENTUALLY_POW_INV = prove
 (`!x e. &0 < e /\ abs(x) < &1
  ...
  GEN_REWRITE_TAC "Multivariate/metric.ml:BINOP_CONV" BINOP_CONV [GSYM REAL_INV_INV] THEN
...
let EVENTUALLY_IN_SEQUENTIALLY = prove
 (`!P. eventually P sequentially <=> FINITE {n | ~P n}`,
  ...
  GEN_REWRITE_TAC "Multivariate/metric.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [GSYM CONTRAPOS_THM] THEN
...
let OPEN_IN_MTOPOLOGY = prove
 (`!m:A metric u.
  ...
  CONJUNCT2)
  topology_tybij THEN
  GEN_REWRITE_TAC "Multivariate/metric.ml:LAND_CONV" LAND_CONV [GSYM IN] THEN REWRITE_TAC[IN_ELIM_THM]);;
...
let METRIC_ARITH_TAC = CONV_TAC "Multivariate/metric.ml:METRIC_ARITH" METRIC_ARITH;;
 ...
 (`!m:A metric g.
  ...
  [LABEL_INDUCT_TAC THENL
   [REMOVE_THEN "b1" (fun b1 -> REWRITE_TAC[b1]) THEN
    ...
    ALL_TAC] THEN
   USE_THEN "b1" (fun b1 -> GEN_REWRITE_TAC "Multivariate/metric.ml:RAND_CONV" RAND_CONV [b1]) THEN
  ...
  [LABEL_INDUCT_TAC THENL
   [CONV_TAC "Multivariate/metric.ml:(RAND_CONV REAL_RAT_REDUCE_CONV)" (RAND_CONV REAL_RAT_REDUCE_CONV) THEN HYP REWRITE_TAC "r0lt1" [];
    TRANS_TAC REAL_LTE_TRANS `r (n:num) / &2` THEN
    HYP REWRITE_TAC "r1lt" [real_pow] THEN REMOVE_THEN "ind_n" MP_TAC THEN
    REMOVE_THEN "rpos" (MP_TAC o SPEC `n:num`) THEN CONV_TAC "Multivariate/metric.ml:REAL_FIELD" REAL_FIELD];
   ALL_TAC] THEN
  CLAIM_TAC "nested"
...
let DIMENSION_LE_MONO = prove
 (`!top:(A)topology m n. top dimension_le m /\ m <= n ==> top dimension_le n`,
  ...
  GEN_REWRITE_TAC "Multivariate/metric.ml:I" I [DIMENSION_LE_CASES] THEN
...
let DIMENSION_LE_EQ_EMPTY = prove
 (`!top:(A)topology. top dimension_le (-- &1) <=> topspace top = {}`,
  ...
  CONV_TAC "Multivariate/metric.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN
...
let DIMENSION_LE_SUBTOPOLOGY = prove
 (`!top n s:A->bool.
  ...
  X_GEN_TAC `s:A->bool` THEN GEN_REWRITE_TAC "Multivariate/metric.ml:I" I [DIMENSION_LE_CASES] THEN
  ASM_REWRITE_TAC[] THEN MAP_EVERY X_GEN_TAC [`u':A->bool`; `a:A`] THEN
  GEN_REWRITE_TAC "Multivariate/metric.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [OPEN_IN_SUBTOPOLOGY] THEN
 ...
 (`!top s:A->bool n.
        ...
        !v a. open_in top v /\ a IN v /\ a IN s
              ==> ?u. a IN u /\ u SUBSET v /\ open_in top u /\
                      ...
                      dimension_le (n - &1)`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/metric.ml:LAND_CONV" LAND_CONV [DIMENSION_LE_CASES] THEN
  ...
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/metric.ml:I" I [FUN_EQ_THM] THEN
  ...
  GEN_REWRITE_TAC "Multivariate/metric.ml:LAND_CONV" LAND_CONV [SWAP_EXISTS_THM] THEN
