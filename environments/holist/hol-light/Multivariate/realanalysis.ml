...
let REAL_OPEN_IN = prove
 (`!s. real_open s <=> open_in euclideanreal s`,
  GEN_TAC THEN REWRITE_TAC[euclideanreal] THEN CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN
...
let REALLIM_NULL_NEG = prove
 (`!net f. ((\x. --(f x)) ---> &0) net <=> (f ---> &0) net`,
  ...
  MATCH_MP_TAC REALLIM_NULL_LMUL_EQ THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
 ...
 (`!f k. (f ---> &0) sequentially
  ...
  EXISTS_TAC `\n. (f:num->real) k - f(n + 1)` THEN CONJ_TAC THENL
   [REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN EXISTS_TAC `k:num` THEN
    SIMP_TAC[];
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [GSYM REAL_SUB_RZERO] THEN
...
let REAL_PARTIAL_SUMS_LE_INFSUM = prove
 (`!f s n.
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o LAND_CONV o BINDER_CONV o RAND_CONV o RAND_CONV)" (LAND_CONV o LAND_CONV o BINDER_CONV o RAND_CONV o RAND_CONV)
   [GSYM LIFT_DROP] THEN
...
let REAL_SERIES_POS = prove
 (`!f s y.
        ...
        ==> &0 <= y`,
  REWRITE_TAC[REAL_SUMS] THEN REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [GSYM LIFT_DROP] THEN
...
let REAL_SERIES_COMPARISON_BOUND = prove
 (`!f g s a.
  ...
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(BINDER_CONV o RAND_CONV o RAND_CONV)" (BINDER_CONV o RAND_CONV o RAND_CONV) [GSYM LIFT_DROP] THEN
...
let TRIVIAL_LIMIT_WITHINREAL_WITHINCOMPLEX = prove
 (`trivial_limit(atreal x within s) <=>
  ...
  REPEAT CONJ_TAC THEN TRY EQ_TAC THEN REWRITE_TAC[LEFT_IMP_EXISTS_THM] THENL
   [DISCH_THEN(MP_TAC o SPECL [`&0`; `&1`]) THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_RING" REAL_RING;
    DISCH_THEN(MP_TAC o SPECL [`Cx(&0)`; `Cx(&1)`]) THEN
    CONV_TAC "Multivariate/realanalysis.ml:COMPLEX_RING" COMPLEX_RING;
...
let LIM_ATREAL_ZERO = prove
 (`!f l a. (f --> l) (atreal a) <=> ((\x. f (a + x)) --> l) (atreal (&0))`,
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [LIM_AT_ZERO] THEN
...
let REALLIM_ATREAL_ZERO = prove
 (`!f l a. (f ---> l) (atreal a) <=> ((\x. f (a + x)) ---> l) (atreal (&0))`,
  REPEAT GEN_TAC THEN REWRITE_TAC[TENDSTO_REAL] THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [LIM_ATREAL_ZERO] THEN
  REWRITE_TAC[o_DEF]);;
 ...
 (`!s a. real_summable s a
  ...
  EXISTS_TAC `&1` THEN REWRITE_TAC[REAL_LT_01] THEN CONJ_TAC THENL
   [REWRITE_TAC[IMP_CONJ; IN_INTER; IN_ELIM_THM; IN_IMAGE] THEN
    ...
    CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[GSYM REAL; o_DEF; CX_MUL; CX_POW] THEN
...
let HAS_REAL_DERIVATIVE_WITHINREAL = prove
 (`(f has_real_derivative f') (atreal a within s) <=>
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [REALLIM_NULL] THEN
 ...
 (`!f f' z.
  ...
  EQ_TAC THEN STRIP_TAC THEN ASM_REWRITE_TAC[] THENL
   [EXISTS_TAC `\w. if w = z then f':real else (f(w) - f(z)) / (w - z)` THEN
    ...
    CONV_TAC "Multivariate/realanalysis.ml:REAL_FIELD" REAL_FIELD;
...
let REAL_MIDPOINT_IN_CONVEX = prove
 (`!s x y. is_realinterval s /\ x IN s /\ y IN s ==> ((x + y) / &2) IN s`,
  ...
  CONV_TAC "Multivariate/realanalysis.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM_REWRITE_TAC[]);;
...
let REAL_INTERVAL_TRANSLATION = prove
 (`(!c a b. real_interval[c + a,c + b] =
   ...
   (!c a b. real_interval(c + a,c + b) =
            IMAGE (\x. c + x) (real_interval(a,b)))`,
  REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN
...
let HAS_REAL_DERIVATIVE_DIV_WITHIN = prove
 (`!f f' g g' x s.
  ...
  AP_THM_TAC THEN AP_TERM_TAC THEN
  POP_ASSUM MP_TAC THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_FIELD" REAL_FIELD);;
...
let REAL_DIFFERENTIABLE_TAC =
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:I" I [real_differentiable] THEN
  W(fun (asl,w) ->
...
let REALLIM_NULL_POW_EQ = prove
 (`!net f n.
  ...
  MATCH_MP_TAC REAL_CONTINUOUS_WITHINREAL_COMPOSE THEN CONJ_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
...
let LIM_NULL_RPOW = prove
 (`!net p x:A->real.
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM LIFT_DROP] THEN
...
let HAS_REAL_DERIVATIVE_FRAC = prove
 (`!x. ~(integer x) ==> (frac has_real_derivative (&1)) (atreal x)`,
  ...
  X_GEN_TAC `y:real` THEN DISCH_TAC THEN CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN
...
let REAL_MVT = prove
 (`!f f' a b.
  ...
  CONJ_TAC THENL [UNDISCH_TAC `a < b` THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_FIELD" REAL_FIELD; ALL_TAC] THEN
  REPEAT STRIP_TAC THEN MATCH_MP_TAC HAS_REAL_DERIVATIVE_SUB THEN
  ASM_SIMP_TAC[] THEN GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_RID] THEN
...
let REAL_MVT_CAUCHY = prove
 (`!f g f' g' a b.
  ...
  UNDISCH_TAC `a < b` THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_FIELD" REAL_FIELD);;
...
let REAL_TAYLOR_MVT_POS = prove
 (`!f a x n.
  ...
  ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
   [CONJ_TAC THENL
     [SIMP_TAC[SUM_CLAUSES_LEFT; LE_0] THEN
      ...
      CONV_TAC "Multivariate/realanalysis.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    ...
    MATCH_MP_TAC HAS_REAL_DERIVATIVE_ADD THEN CONJ_TAC THENL
     [ALL_TAC;
      REAL_DIFF_TAC THEN REWRITE_TAC[ADD_SUB] THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_RING" REAL_RING] THEN
  ...
  FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [GSYM th]) THEN
  ...
  SUBGOAL_THEN `~(&(FACT n) = &0)` MP_TAC THENL
   [REWRITE_TAC[REAL_OF_NUM_EQ; FACT_NZ]; CONV_TAC "Multivariate/realanalysis.ml:REAL_FIELD" REAL_FIELD]);;
...
let REAL_TAYLOR_MVT_NEG = prove
 (`!f a x n.
  ...
  MATCH_MP_TAC REAL_DIFF_CHAIN_WITHIN THEN CONJ_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RATOR_CONV o LAND_CONV)" (RATOR_CONV o LAND_CONV) [GSYM ETA_AX] THEN
 ...
 (`!f l. ((\x. f(&1 / x)) --> l) (atreal (&0)) ==> (f --> l) at_posinfinity`,
  ...
  ASM_REWRITE_TAC[REAL_ABS_INV; REAL_LT_INV_EQ] THEN CONJ_TAC THENL
   [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REAL_ARITH
     `a <= z ==> &0 < a ==> &0 < abs z`));
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_INV] THEN
...
let REAL_CONVEX_ON_JENSEN = prove
 (`!f s. is_realinterval s
  ...
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/realanalysis.ml:I" I [FUN_EQ_THM] THEN
 ...
 (`!f f' s a b.
  ...
  ANTS_TAC THENL
   [ASM_REWRITE_TAC[] THEN X_GEN_TAC `z:real` THEN DISCH_TAC THEN
    ...
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:I" I [GSYM REAL_SUB_LE] THEN
 ...
 (`!f f' f'' s.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC REAL_CONVEX_ON_DERIVATIVE_INCREASING;
    CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC HAS_REAL_DERIVATIVE_INCREASING] THEN
  ASM_REWRITE_TAC[]);;
 ...
 (`!a x k:A->bool.
  ...
  MP_TAC THENL
   [MATCH_MP_TAC version2 THEN
    ...
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [GSYM SUM_SUPPORT] THEN
    REWRITE_TAC[support; NEUTRAL_REAL_ADD];
    MATCH_MP_TAC EQ_IMP THEN CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN BINOP_TAC THENL
 ...
 (`!k:A->bool a x y.
  ...
  REWRITE_TAC[NOT_IMP] THEN STRIP_TAC THENL
   [MATCH_MP_TAC(REAL_ARITH
    ...
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID]] THEN
 ...
 (`!a x y. &0 < a /\ ~(a = &1) /\ &0 < x /\ x < y
  ...
  CONJ_TAC THENL
   [ASM_SIMP_TAC[rpow] THEN REPEAT STRIP_TAC THEN REAL_DIFF_TAC THEN
    REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_FIELD" REAL_FIELD;
    ALL_TAC] THEN
  X_GEN_TAC `z:real` THEN DISCH_TAC THEN
...
let REAL_EXP_LIMIT_RPOW_LE = prove
 (`!x r s. &0 <= r /\ r <= s /\ x <= r
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [GSYM REAL_EXP_0] THEN
...
let LOG_CONVEX_IMP_POS = prove
 (`!f s x:real^N.
  ...
  CONV_TAC "Multivariate/realanalysis.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM_MESON_TAC[]);;
...
let LOG_CONVEX_ON = prove
 (`!f s:real^N->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RAND_CONV o funpow 4 BINDER_CONV o RAND_CONV)" (RAND_CONV o funpow 4 BINDER_CONV o RAND_CONV)
...
let REAL_LOG_CONVEX_LIM = prove
 (`!net:A net f g s.
  ...
  REPEAT GEN_TAC THEN STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [GSYM REAL_SUB_LE] THEN
...
let REAL_INTEGRABLE_INTEGRAL = prove
 (`!f i. f real_integrable_on i
  ...
  CONV_TAC "Multivariate/realanalysis.ml:(RAND_CONV SELECT_CONV)" (RAND_CONV SELECT_CONV) THEN REWRITE_TAC[]);;
                       ...
                       (!a b.
                            ...
                            ==> abs
                                (real_integral (real_interval[a,b])
                                 ...
                                 i) < e)))`,
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [has_real_integral] THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [HAS_INTEGRAL_ALT] THEN
...
let HAS_REAL_INTEGRAL_BOUND = prove
 (`!f:real->real a b i B.
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RAND_CONV o RAND_CONV o BINOP_CONV)" (RAND_CONV o RAND_CONV o BINOP_CONV) [GSYM LIFT_DROP] THEN
...
let HAS_REAL_INTEGRAL_LE = prove
 (`!f g s i j.
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:BINOP_CONV" BINOP_CONV [GSYM LIFT_DROP] THEN
...
let REAL_FUNDAMENTAL_THEOREM_OF_CALCULUS = prove
 (`!f f' a b.
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o LAND_CONV o BINOP_CONV)" (LAND_CONV o LAND_CONV o BINOP_CONV) [GSYM LIFT_DROP] THEN
...
let REAL_INTEGRAL_HAS_REAL_DERIVATIVE_POINTWISE = prove
 (`!f a b x.
  ...
  REWRITE_TAC[LIFT_DROP] THEN CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN
...
let REAL_FUNDAMENTAL_THEOREM_OF_CALCULUS_INTERIOR_STRONG = prove
 (`!f f' s a b.
  ...
  REWRITE_TAC[REAL_CONTINUOUS_ON; GSYM IMAGE_o; IMAGE_LIFT_DROP] THEN
  REWRITE_TAC[LIFT_DROP] THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o RAND_CONV o LAND_CONV o BINOP_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV o BINOP_CONV)
   [GSYM LIFT_DROP] THEN
  DISCH_THEN(MP_TAC o
...
let REAL_INDEFINITE_INTEGRAL_CONTINUOUS_RIGHT = prove
 (`!f:real->real a b.
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:I" I [GSYM DROP_EQ] THEN
  REWRITE_TAC[INTERVAL_REAL_INTERVAL; LIFT_DROP; GSYM o_DEF] THEN
  CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_INTEGRAL THEN
  MATCH_MP_TAC REAL_INTEGRABLE_SUBINTERVAL THEN
  MAP_EVERY EXISTS_TAC [`a:real`; `b:real`] THEN
  ASM_REWRITE_TAC[SUBSET_REAL_INTERVAL] THEN
...
let HAS_REAL_DERIVATIVE_INDEFINITE_INTEGRAL = prove
 (`!f a b.
  ...
  REWRITE_TAC[GSYM IMAGE_LIFT_REAL_INTERVAL] THEN CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN
  MATCH_MP_TAC REAL_INTEGRAL THEN
  MATCH_MP_TAC REAL_INTEGRABLE_SUBINTERVAL THEN
  MAP_EVERY EXISTS_TAC [`a:real`; `b:real`] THEN
  ASM_REWRITE_TAC[SUBSET_REAL_INTERVAL] THEN
...
let REAL_NEGLIGIBLE_ON_INTERVALS = prove
 (`!s. real_negligible s <=>
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [NEGLIGIBLE_ON_INTERVALS] THEN
...
let REAL_MONOTONE_CONVERGENCE_INCREASING = prove
 (`!f:num->real->real g s.
  ...
  CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[GSYM o_DEF] THEN
...
let HAS_REAL_MEASURE_REAL_NEGLIGIBLE_UNIONS_IMAGE = prove
 (`!f:A->(real->bool) s.
  ...
  SUBST1_TAC THENL
   [CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN ONCE_REWRITE_TAC[GSYM o_DEF] THEN
...
let HAS_REAL_MEASURE_REAL_NEGLIGIBLE_UNIONS_IMAGE_STRONG = prove
 (`!f:A->real->bool s.
  ...
  MATCH_MP_TAC EQ_IMP THEN BINOP_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/realanalysis.ml:I" I [EXTENSION] THEN
    REWRITE_TAC[IN_UNIONS; IN_IMAGE; IN_ELIM_THM] THEN
    MESON_TAC[NOT_IN_EMPTY];
    CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_SUPERSET THEN
...
let REAL_MEASURE_UNION = prove
 (`!s t. real_measurable s /\ real_measurable t
  ...
  (CONJ_TAC THENL
    [CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_MEASURE_DISJOINT_UNION THEN
...
let REAL_MEASURE_UNIONS_LE_IMAGE = prove
 (`!f:A->bool s:A->(real->bool).
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM o_DEF] THEN
...
let HAS_REAL_MEASURE_INNER_OUTER = prove
 (`!s m. s has_real_measure m <=>
                ...
                (!e. &0 < e ==> ?u. s SUBSET u /\ real_measurable u /\
                                    real_measure u < m + e)`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV
  ...
  MATCH_MP_TAC(TAUT `a /\ (a ==> b) ==> a /\ b`) THEN CONJ_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/realanalysis.ml:I" I [REAL_MEASURABLE_INNER_OUTER] THEN
    X_GEN_TAC `e:real` THEN DISCH_TAC THEN
...
let HAS_REAL_MEASURE_TRANSLATION = prove
 (`!s m a. s has_real_measure m ==> (IMAGE (\x. a + x) s) has_real_measure m`,
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [REAL_ARITH `m = abs(&1) * m`] THEN
  REWRITE_TAC[HAS_REAL_MEASURE_AFFINITY]);;
 ...
 (`!s:num->real->bool B.
  ...
  SUBGOAL_THEN
   ...
   (fun th -> REWRITE_TAC[th] THEN ONCE_REWRITE_TAC[SIMPLE_IMAGE] THEN
              REWRITE_TAC[]) THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:I" I [EXTENSION] THEN X_GEN_TAC `x:real` THEN
...
let SLICE_CBALL = prove
 (`!k t x r.
  ...
  SUBGOAL_THEN
   ...
   (fun th -> GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [th])
  THENL
   [CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC DROPOUT_PUSHIN THEN ASM_ARITH_TAC;
    ASM_SIMP_TAC[DOT_DROPOUT] THEN
...
let FUBINI_CLOSED_INTERVAL = prove
 (`!k a b:real^N.
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RAND_CONV o RATOR_CONV)" (RAND_CONV o RATOR_CONV) [COND_RAND] THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [COND_RATOR] THEN
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN AP_TERM_TAC THEN
    ...
    REWRITE_TAC[] THEN CONJ_TAC THENL
     [X_GEN_TAC `i:num` THEN COND_CASES_TAC THEN ASM_REWRITE_TAC[] THENL
       [ASM_MESON_TAC[REAL_NOT_LT; IN_NUMSEG; REAL_NOT_LE; INTERVAL_EQ_EMPTY];
        REWRITE_TAC[EMPTY_SUBSET] THEN CONV_TAC "Multivariate/realanalysis.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
 ...
 (`!k s:real^N->bool e.
  ...
  REWRITE_TAC[] THEN ANTS_TAC THENL
   [CONJ_TAC THENL
    ...
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV) [IN_UNIV] THEN
    ...
    ASM_REWRITE_TAC[SLICE_UNIONS] THEN ANTS_TAC THENL
     [ALL_TAC;
      ...
      GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o RATOR_CONV o LAND_CONV)" (LAND_CONV o RATOR_CONV o LAND_CONV) [GSYM o_DEF] THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC REAL_INTEGRAL_LE THEN ASM_REWRITE_TAC[] THEN
    ...
    X_GEN_TAC `j:num` THEN STRIP_TAC THEN REWRITE_TAC[] THEN
    CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_INTEGRAL_UNIQUE THEN
    SUBGOAL_THEN `?u v. u$k <= v$k /\
                          (d:num->real^N->bool) j = interval[u,v]`
    STRIP_ASSUME_TAC THENL [ASM_MESON_TAC[]; ALL_TAC] THEN
    ASM_REWRITE_TAC[] THEN
...
let REAL_CONTINUOUS_FRAC = prove
 (`!x. ~(integer x) ==> frac real_continuous (atreal x)`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
       ...
       [ASM_SIMP_TAC[REAL_DIV_REFL; REAL_LT_IMP_NZ; REAL_SUB_LT] THEN
        ...
        SUBST1_TAC THENL
         [REWRITE_TAC[REAL_FRAC_EQ] THEN
          ...
          ASM_REAL_ARITH_TAC;
          AP_TERM_TAC THEN UNDISCH_TAC `a:real < b` THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_FIELD" REAL_FIELD]];
      ASM_SIMP_TAC[REAL_FIELD
   ...
   [SUBGOAL_THEN
    ...
    THENL
     [ASM_REAL_ARITH_TAC;
      ...
      ASM_REWRITE_TAC[REAL_ADD_LID; REAL_LE_REFL] THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_RING" REAL_RING;
      ALL_TAC] THEN
...
let BERNSTEIN_LEMMA = prove
 (`!n x. sum(0..n) (\k. (&k - &n * x) pow 2 * bernstein n k x) =
   ...
   [REPEAT GEN_TAC THEN MATCH_MP_TAC REAL_DERIVATIVE_UNIQUE_ATREAL THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC HAS_REAL_DERIVATIVE_SUM THEN REWRITE_TAC[FINITE_NUMSEG];
      ASM_REWRITE_TAC[]] THEN
    REPEAT STRIP_TAC THEN REAL_DIFF_TAC THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_RING" REAL_RING;
    ALL_TAC] THEN
  SUBGOAL_THEN
   `!x y. sum(0..n)
   ...
   [REPEAT GEN_TAC THEN MATCH_MP_TAC REAL_DERIVATIVE_UNIQUE_ATREAL THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC HAS_REAL_DERIVATIVE_SUM THEN REWRITE_TAC[FINITE_NUMSEG];
      ASM_REWRITE_TAC[]] THEN
    REPEAT STRIP_TAC THEN REAL_DIFF_TAC THEN
    REWRITE_TAC[ARITH_RULE `n - 1 - 1 = n - 2`] THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_RING" REAL_RING;
    ALL_TAC] THEN
  REWRITE_TAC[REAL_ARITH
   `(a - b) pow 2 * x =
  ...
  SUBST1_TAC THENL [ALL_TAC; CONV_TAC "Multivariate/realanalysis.ml:REAL_RING" REAL_RING] THEN
 ...
 (`!(P:(real^N->real)->bool) (s:real^N->bool).
   ...
   [SUBGOAL_THEN `!f. A(f) /\ (!x. x IN s ==> abs(f x) <= &1 / &4)
     ...
     [ALL_TAC;
      ...
      MP_TAC THENL
       [USE_THEN "mul" MATCH_MP_TAC THEN ASM_REWRITE_TAC[] THEN
        ...
        CONV_TAC "Multivariate/realanalysis.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    ...
    REWRITE_TAC[] THEN CONJ_TAC THENL
     [SUBGOAL_THEN
      ...
      GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(PAT_CONV `\x. abs(abs x - a) < e`)" (PAT_CONV `\x. abs(abs x - a) < e`)
  ...
  MP_TAC THENL
   [REPEAT STRIP_TAC THEN ASM_CASES_TAC `y:real^N = x` THENL
     [EXISTS_TAC `\z:real^N. (f:real^N->real) x` THEN ASM_SIMP_TAC[];
      ...
      ASM_SIMP_TAC[] THEN
      UNDISCH_TAC `~((h:real^N->real) x = h y)` THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_FIELD" REAL_FIELD];
      ALL_TAC] THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
  ...
  MP_TAC THENL
   [REPEAT STRIP_TAC THEN FIRST_ASSUM(MP_TAC o
    ...
    REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [REAL_ARITH `x = min x x`] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
  ...
  ANTS_TAC THENL
   [CONJ_TAC THENL [ALL_TAC; ASM SET_TAC[]] THEN EXPAND_TAC "H" THEN
    ...
    REWRITE_TAC[OPEN_HALFSPACE_COMPONENT_LT; IN_ELIM_THM] THEN
    REWRITE_TAC[GSYM drop; LIFT_DROP; o_DEF] THEN
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV)
   ...
   [ASM_REWRITE_TAC[IMP_IMP; RIGHT_IMP_FORALL_THM; IN_ELIM_THM] THEN
    REPEAT CONJ_TAC THENL
     [X_GEN_TAC `c:real` THEN EXISTS_TAC `\x:real^N. Cx(c)` THEN
      ...
      REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
      GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [COMPLEX_EQ] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV) [COMPLEX_EXPAND] THEN
     ...
     [MESON_TAC[];
      ...
      ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
       [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
          FINITE_SUBSET)) THEN
        SET_TAC[];
        ABS_TAC THEN CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_EQ_SUPERSET THEN
     ...
     [REPEAT GEN_TAC THEN DISCH_THEN(CONJUNCTS_THEN2
      ...
      REWRITE_TAC[] THEN CONJ_TAC THENL
       [MATCH_MP_TAC FINITE_SUBSET THEN
        ...
        CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_ADD_GEN THEN
    ...
    REWRITE_TAC[] THEN CONJ_TAC THENL
     [MATCH_MP_TAC FINITE_SUBSET THEN
      ...
      GEN_TAC THEN DISCH_THEN(fun th ->
       ...
       GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [GSYM th]) THEN
...
let STONE_WEIERSTRASS_REAL_POLYNOMIAL_FUNCTION = prove
 (`!f:real^N->real s e.
  ...
  REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [CART_EQ] THEN
  ...
  REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC VECTOR_POLYNOMIAL_FUNCTION_VSUM THEN
    ...
    SUBGOAL_THEN
     ...
     (fun th -> GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [SYM th])
    THENL
     [MATCH_MP_TAC ORTHONORMAL_BASIS_EXPAND THEN
      ...
      REWRITE_TAC[NORM_POW_2] THEN GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [dot] THEN
...
let RECTIFIABLE_PATH_VECTOR_POLYNOMIAL_FUNCTION = prove
 (`!p:real^1->real^N. vector_polynomial_function p ==> rectifiable_path p`,
  ...
  DISCH_THEN(fun th -> STRIP_TAC THEN MP_TAC th THEN ASM_REWRITE_TAC[]) THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [GSYM o_DEF] THEN
 ...
 (`(!n x. sum(0..n) (\k. &(binom(n,k)) * bernoulli k x) - bernoulli n x =
  ...
  let lemma = prove
   (`(!n x. sum (0..n) (\k. &(binom(n,k)) * B k x) - B n x =
    ...
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [cth] THEN
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [cth] THEN
    ...
    CONV_TAC "Multivariate/realanalysis.ml:REAL_FIELD" REAL_FIELD) in
  ...
  MP_TAC THENL
   [REPEAT GEN_TAC THEN MP_TAC(ISPECL
    ...
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [GSYM APPELL_SEQUENCE] THEN
...
let BERNOULLI_NUMBER = prove
 (`!n. sum (0..n) (\k. &(binom (n,k)) * bernoulli k (&0)) - bernoulli n (&0) =
  ...
  CONV_TAC "Multivariate/realanalysis.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[real_pow; REAL_MUL_LID] THEN
...
let BERNOULLI_1 = prove
 (`!n. bernoulli n (&1) =
  ...
  GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM REAL_ADD_LID] THEN
...
let SUM_OF_POWERS = prove
 (`!m n. sum(0..n) (\k. &k pow m) =
  ...
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o RAND_CONV o BINDER_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV o BINDER_CONV o RAND_CONV)
   [GSYM SUC_SUB1] THEN
...
let HAS_REAL_DERIVATIVE_BERNOULLI = prove
 (`!n x. ((bernoulli n) has_real_derivative (&n * bernoulli (n - 1) x))
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RATOR_CONV o RAND_CONV)" (RATOR_CONV o RAND_CONV) [GSYM REAL_ADD_RID] THEN
...
let HAS_REAL_INTEGRAL_BERNOULLI = prove
 (`!n. ((bernoulli n) has_real_integral (if n = 0 then &1 else &0))
  ...
  REWRITE_TAC[REAL_POS] THEN ANTS_TAC THENL
   [REPEAT STRIP_TAC THEN REAL_DIFF_TAC THEN
    REWRITE_TAC[ADD_SUB; GSYM REAL_OF_NUM_ADD] THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_FIELD" REAL_FIELD;
...
let POLYNOMIAL_FUNCTION_BERNOULLI = prove
 (`!n. polynomial_function(bernoulli n)`,
  GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [GSYM ETA_AX] THEN
  ONCE_REWRITE_TAC[BERNOULLI_EXPANSION] THEN
 ...
 (`!p n. polynomial_function p /\
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[GSYM INFINITE] THEN
    ...
    CONV_TAC "Multivariate/realanalysis.ml:(BINDER_CONV SYM_CONV)" (BINDER_CONV SYM_CONV) THEN INDUCT_TAC THEN
    ...
    EXISTS_TAC `real_interval[&0,&1]` THEN CONJ_TAC THENL
     [GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [REAL_ARITH `x = x * (&1 - &0)`] THEN
...
let BERNOULLI_RAABE_2 = prove
 (`!n x. bernoulli n ((x + &1) / &2) + bernoulli n (x / &2) =
  ...
  ASM_REWRITE_TAC[bernoulli] THEN CONV_TAC "Multivariate/realanalysis.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  SIMP_TAC[REAL_LT_POW2; REAL_FIELD
   `&0 < p ==> (x = &2 / p * y <=> p / &2 * x = y)`] THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:I" I [GSYM FUN_EQ_THM] THEN
  ...
  REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC POLYNOMIAL_FUNCTION_LMUL THEN
    ...
    CONV_TAC "Multivariate/realanalysis.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[GSYM IMP_CONJ_ALT] THEN
    ...
    CONV_TAC "Multivariate/realanalysis.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ONCE_REWRITE_TAC[CONJ_SYM] THEN
...
let BERNOULLI_1_0 = prove
 (`!n. bernoulli n (&1) = --(&1) pow n * bernoulli n (&0)`,
  ...
  CONV_TAC "Multivariate/realanalysis.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_POW_ONE; REAL_MUL_LID]);;
  ...
  let lemma = prove
   (`(!n x. x IN real_interval(&0,&1 / &2)
    ...
    REWRITE_TAC[AND_FORALL_THM; IN_REAL_INTERVAL] THEN INDUCT_TAC THENL
     [CONV_TAC "Multivariate/realanalysis.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
      CONV_TAC "Multivariate/realanalysis.ml:(ONCE_DEPTH_CONV BERNOULLI_CONV)" (ONCE_DEPTH_CONV BERNOULLI_CONV) THEN REAL_ARITH_TAC;
      ...
      MP_TAC(ISPECL
       [`\x. bernoulli (2 * SUC n + 1) x / (&2 * &n + &3)`;
        `bernoulli (2 * SUC n)`; `x:real`; `&1 / &2`]
          REAL_ROLLE_SIMPLE) THEN
      ASM_REWRITE_TAC[NOT_IMP] THEN REPEAT CONJ_TAC THENL
       [REWRITE_TAC[real_div; REAL_MUL_LZERO] THEN CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN
        REWRITE_TAC[REAL_ENTIRE] THEN DISJ1_TAC THEN
        CONV_TAC "Multivariate/realanalysis.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[BERNOULLI_HALF] THEN
...
let BERNOULLI_NUMBER_EQ_0 = prove
 (`!n. bernoulli n (&0) = &0 <=> ODD n /\ ~(n = 1)`,
  ...
  CONV_TAC "Multivariate/realanalysis.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN DISCH_TAC THEN
  ...
  MATCH_MP_TAC(TAUT `q /\ ~p ==> (p <=> q) ==> r`) THEN CONJ_TAC THENL
   [EXISTS_TAC `n:num` THEN SIMP_TAC[IN_NUMSEG; LE_0; LE_REFL; SUB_REFL] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:RAND_CONV" RAND_CONV [BERNOULLI_EXPANSION] THEN
     ...
     [REWRITE_TAC[IN_REAL_INTERVAL] THEN REPEAT STRIP_TAC THEN
      ...
      ASM_CASES_TAC `m = 0` THENL
       [ASM_REWRITE_TAC[MULT_CLAUSES; ADD_CLAUSES] THEN
        CONV_TAC "Multivariate/realanalysis.ml:(ONCE_DEPTH_CONV BERNOULLI_CONV)" (ONCE_DEPTH_CONV BERNOULLI_CONV) THEN ASM_REAL_ARITH_TAC;
...
let REAL_EULER_MACLAURIN = prove
 (`!f m n p.
  ...
  CONV_TAC "Multivariate/realanalysis.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[CONJUNCT1 bernoulli] THEN
  ...
  SPEC_TAC(`p:num`,`p:num`) THEN INDUCT_TAC THENL
   [REWRITE_TAC[SUM_CLAUSES_NUMSEG] THEN CONV_TAC "Multivariate/realanalysis.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    REAL_ARITH_TAC;
    GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
...
let REAL_EULER_MACLAURIN_ANTIDERIVATIVE = prove
 (`!f m n p.
  ...
  CONV_TAC "Multivariate/realanalysis.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN DISCH_THEN(SUBST1_TAC o CONJUNCT2) THEN
  MP_TAC(ISPECL
   [`f 0:real->real`; `f (0 + 1):real->real`; `&m`; `&n`]
        REAL_FUNDAMENTAL_THEOREM_OF_CALCULUS) THEN
  ASM_SIMP_TAC[REAL_OF_NUM_LE; LE_0] THEN CONV_TAC "Multivariate/realanalysis.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  DISCH_THEN(SUBST1_TAC o MATCH_MP REAL_INTEGRAL_UNIQUE) THEN
  AP_TERM_TAC THEN AP_TERM_TAC THEN
 ...
 (`!f:real^N->real^2.
  ...
  MP_TAC THENL
   [X_GEN_TAC `n:num` THEN MATCH_MP_TAC TIETZE_UNBOUNDED THEN CONJ_TAC THENL
    ...
    REWRITE_TAC[VECTOR_SUB_EQ] THEN CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN
    FIRST_X_ASSUM MATCH_MP_TAC THEN
...
let REAL_LEBESGUE_MEASURABLE_COMPL = prove
 (`!s. real_lebesgue_measurable((:real) DIFF s) <=>
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(RAND_CONV)" (RAND_CONV) [GSYM LEBESGUE_MEASURABLE_COMPL] THEN
...
let REAL_LEBESGUE_MEASURABLE_ON_SUBINTERVALS = prove
 (`!s. real_lebesgue_measurable s <=>
  ...
  GEN_TAC THEN REWRITE_TAC[REAL_LEBESGUE_MEASURABLE] THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [LEBESGUE_MEASURABLE_ON_SUBINTERVALS] THEN
...
let REAL_MEASURABLE_ON_PREIMAGE_HALFSPACE_LT = prove
 (`!f. f real_measurable_on (:real) <=>
  ...
  CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
...
let REAL_MEASURABLE_ON_PREIMAGE_OPEN = prove
 (`!f. f real_measurable_on (:real) <=>
  ...
  MATCH_MP_TAC EQ_IMP THEN AP_TERM_TAC THENL
   [CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV; ALL_TAC] THEN
  MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
...
let REAL_MEASURABLE_ON_SIMPLE_FUNCTION_LIMIT = prove
 (`!f. f real_measurable_on (:real) <=>
  ...
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [MEASURABLE_ON_SIMPLE_FUNCTION_LIMIT] THEN
  EQ_TAC THENL
    ...
    THENL
     [GEN_TAC THEN
      ...
      SUBGOAL_THEN
       ...
       (fun th -> REWRITE_TAC[th])
      THENL
       [GEN_TAC THEN CONV_TAC "Multivariate/realanalysis.ml:SYM_CONV" SYM_CONV THEN
...
let REAL_MEASURABLE_ON_DECREASING_UNIV = prove
 (`!f. (!x y. x <= y ==> f y <= f x) ==> f real_measurable_on (:real)`,
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:I" I [GSYM REAL_MEASURABLE_ON_NEG_EQ] THEN
...
let ABSOLUTELY_REAL_INTEGRABLE_INCREASING = prove
 (`!f a b. (!x y. x IN real_interval[a,b] /\ y IN real_interval[a,b] /\ x <= y
           ...
           ==> f absolutely_real_integrable_on real_interval[a,b]`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/realanalysis.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
  GEN_REWRITE_TAC "Multivariate/realanalysis.ml:(LAND_CONV o ABS_CONV)" (LAND_CONV o ABS_CONV) [GSYM REAL_MUL_RID] THEN
...
let REAL_LEBESGUE_DIFFERENTIATION_THEOREM_ALT = prove
 (`!f s. is_realinterval s /\ f has_bounded_real_variation_on s
  ...
  REWRITE_TAC[IN_DIFF; IN_ELIM_THM] THEN CONV_TAC "Multivariate/realanalysis.ml:TAUT" TAUT);;
