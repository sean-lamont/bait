 ...
 (`!s t:real^N->bool.
  ...
  EQ_TAC THEN STRIP_TAC THENL
   [CONJ_TAC THENL
    ...
    CONV_TAC "Multivariate/polytope.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    ...
    STRIP_ASSUME_TAC THENL
     [ASM_REWRITE_TAC[IN_DIFF] THEN ONCE_ASM_REWRITE_TAC[] THEN
      ...
      UNDISCH_TAC `u < &1` THEN CONV_TAC "Multivariate/polytope.ml:REAL_FIELD" REAL_FIELD;
...
let FACE_OF_AFF_DIM_LT = prove
 (`!f s:real^N->bool.
  ...
  ASM_CASES_TAC `f:real^N->bool = {}` THENL
   [CONV_TAC "Multivariate/polytope.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
 ...
 (`!f s:real^N->bool.
  ...
  let lemma = prove
   (`!s x y:real^N.
    ...
    CONV_TAC "Multivariate/polytope.ml:REAL_FIELD" REAL_FIELD) in
  REPEAT STRIP_TAC THEN REWRITE_TAC[face_of] THEN
  ...
  ASM_CASES_TAC `sum (s DIFF f:real^N->bool) c = &0` THENL
   [SUBGOAL_THEN `!x:real^N. x IN (s DIFF f) ==> c x = &0` MP_TAC THENL
    ...
    ASM_SIMP_TAC[] THEN CONJ_TAC THEN FIRST_X_ASSUM(fun th g ->
      (GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [GSYM th] THEN CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV THEN
 ...
 (`!s f:real^N->bool.
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[VECTOR_ARITH `&2 % a - b:real^N = a + &1 % (a - b)`] THEN
    ...
    EXISTS_TAC `&1 / &2` THEN CONV_TAC "Multivariate/polytope.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    VECTOR_ARITH_TAC]);;
  ...
  ASSUME_TAC THENL
   [ALL_TAC;
    ...
    SUBGOAL_THEN `f = (s:real^N->bool) INSERT (f DELETE s)` MP_TAC THENL
     [ASM SET_TAC[];
      DISCH_THEN(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(funpow 2 RAND_CONV)" (funpow 2 RAND_CONV) [th])] THEN
...
let FACES_OF_TRANSLATION = prove
 (`!s a:real^N.
  ...
  REPEAT GEN_TAC THEN CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV THEN
 ...
 (`!s f:real^N->bool. conic s /\ f face_of s ==> conic f`,
  ...
  MP_TAC THENL
   [FIRST_X_ASSUM(DISJ_CASES_TAC o MATCH_MP (REAL_ARITH
    ...
    UNDISCH_TAC `d:real < e` THEN CONV_TAC "Multivariate/polytope.ml:REAL_FIELD" REAL_FIELD]);;
...
let RELATIVE_FRONTIER_FACIAL_PARTITION_ALT = prove
 (`!s:real^N->bool.
  ...
  FIRST_ASSUM(fun th ->
   GEN_REWRITE_TAC "Multivariate/polytope.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV)
...
let FRONTIER_OF_CONVEX_CLOSED = prove
 (`!s:real^N->bool.
  ...
  ASM_SIMP_TAC[frontier; CLOSURE_CLOSED] THEN FIRST_ASSUM(fun th ->
   GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV)
...
let EMPTY_EXPOSED_FACE_OF = prove
 (`!s:real^N->bool. {} exposed_face_of s`,
  ...
  REWRITE_TAC[DOT_LZERO] THEN CONV_TAC "Multivariate/polytope.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN SET_TAC[]);;
...
let EXPOSED_FACE_OF_TRANSLATION_EQ = prove
 (`!a f s:real^N->bool.
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
  ...
  AP_TERM_TAC THEN AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [FUN_EQ_THM] THEN
...
let EXTREME_POINT_NOT_IN_RELATIVE_INTERIOR = prove
 (`!s x:real^N.
  ...
  REPEAT GEN_TAC THEN CONV_TAC "Multivariate/polytope.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
 ...
 (`!s a b:real^N.
  ...
  MAP_EVERY EXISTS_TAC [`b:real^N`; `(b:real^N) dot b`] THEN CONJ_TAC THENL
   [X_GEN_TAC `x:real^N` THEN DISCH_TAC THEN
    ...
    CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV THEN ASM_REWRITE_TAC[VECTOR_EQ] THEN
...
let COLLINEAR_EXTREME_POINTS = prove
 (`!s. collinear s
  ...
  CONV_TAC "Multivariate/polytope.ml:(ONCE_DEPTH_CONV HAS_SIZE_CONV)" (ONCE_DEPTH_CONV HAS_SIZE_CONV) THEN
 ...
 (`!s f:real^N->bool.
  ...
  EXISTS_TAC `f INTER affine hull s:real^N->bool` THEN CONJ_TAC THENL
   [MP_TAC(SPECL [`s:real^N->bool`; `s:real^N->bool`]
    ...
    DISCH_THEN(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [GSYM th]) THEN
 ...
 (`!s:real^N->bool.
  ...
  REPEAT STRIP_TAC THENL
   [ASM_SIMP_TAC[CONIC_HULL_EQ_AFFINE_HULL; CLOSED_AFFINE_HULL;
                 CLOSURE_CLOSED] THEN
    CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV THEN
    GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [GSYM AFFINE_HULL_CLOSURE] THEN
 ...
 (`!u s:real^N->bool.
  ...
  ASM_CASES_TAC `(vec 0:real^N) IN affine hull u` THENL
   [SUBGOAL_THEN
    ...
    SIMP_TAC[SUM_CLAUSES] THEN CONV_TAC "Multivariate/polytope.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV]);;
    ...
    CONJ_TAC THENL
     [MP_TAC(ISPECL [`s:real^N->bool`; `s:real^N->bool`]
      ...
      ASM_REWRITE_TAC[SUBSET_REFL] THEN DISCH_THEN(fun th ->
        GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
...
let INTER_CONIC_HULL_SUBSETS_CONVEX_RELATIVE_FRONTIER = prove
 (`!s t u:real^N->bool.
  ...
  MATCH_MP_TAC REAL_WLOG_LT THEN REWRITE_TAC[] THEN CONJ_TAC THENL
   [REPEAT GEN_TAC THEN
    GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [SWAP_FORALL_THM] THEN
...
let FACET_OF_EMPTY = prove
 (`!s. ~(s facet_of {})`,
  REWRITE_TAC[facet_of; FACE_OF_EMPTY] THEN CONV_TAC "Multivariate/polytope.ml:TAUT" TAUT);;
...
let EXTREME_POINT_EXISTS_CONVEX = prove
 (`!s:real^N->bool.
  ...
  FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [th]) THEN
   ...
   [GEN_TAC THEN WF_INDUCT_TAC `dim(s:real^N->bool)` THEN STRIP_TAC THEN
    ASM_CASES_TAC `(vec 0:real^N) IN relative_interior s` THENL
     [MP_TAC(ISPEC `s:real^N->bool` KREIN_MILMAN) THEN
      ...
      FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o RAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV o RAND_CONV) [th]) THEN
    ...
    CONV_TAC "Multivariate/polytope.ml:REAL_RING" REAL_RING;
    ALL_TAC] THEN
...
let EXTREME_POINTS_OF_CONVEX_HULL_EQ = prove
 (`!s:real^N->bool.
  ...
  CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC KREIN_MILMAN_MINKOWSKI THEN
   ...
   [DISCH_THEN(CONJUNCTS_THEN2 MP_TAC STRIP_ASSUME_TAC) THEN
    ...
    SUBGOAL_THEN `(s DIFF c:real^N->bool) HAS_SIZE 1` MP_TAC THENL
     [ASM_SIMP_TAC[HAS_SIZE; FINITE_DIFF; CARD_DIFF;
      ...
      CONV_TAC "Multivariate/polytope.ml:(LAND_CONV HAS_SIZE_CONV)" (LAND_CONV HAS_SIZE_CONV) THEN
...
let FACET_OF_CONVEX_HULL_AFFINE_INDEPENDENT_ALT = prove
 (`!s t:real^N->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [FUN_EQ_THM] THEN
 ...
 (`!f s a:real^N.
  ...
  let lemma = prove
   (`!a b c p:real^N u v w x.
    ...
    UNDISCH_TAC `~(x = &0)` THEN CONV_TAC "Multivariate/polytope.ml:REAL_FIELD" REAL_FIELD) in
  REPEAT STRIP_TAC THEN EQ_TAC THENL
   ...
   [ASM_REWRITE_TAC[VECTOR_MUL_LZERO; VECTOR_ADD_LID] THEN
     ...
     [UNDISCH_THEN `uc = &0` SUBST_ALL_TAC THEN
      ...
      STRIP_TAC THENL
       [ASM_REAL_ARITH_TAC;
        ...
        ASM_REWRITE_TAC[] THEN CONV_TAC "Multivariate/polytope.ml:VECTOR_ARITH" VECTOR_ARITH];
      ALL_TAC] THEN
    ...
    REWRITE_TAC[REAL_MUL_LZERO; REAL_MUL_LID] THEN REPEAT CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LT_MUL THEN ASM_REAL_ARITH_TAC;
      ...
      UNDISCH_TAC `~(ux = &0)` THEN CONV_TAC "Multivariate/polytope.ml:REAL_FIELD" REAL_FIELD];
    ...
    MATCH_MP_TAC(TAUT `p ==> ~p ==> q`) THEN
    CONJ_TAC THENL [CONV_TAC "Multivariate/polytope.ml:REAL_RING" REAL_RING; REPEAT CONJ_TAC] THEN
 ...
 (`!s a b c:real^N.
  ...
  SUBGOAL_THEN
   `inv(u - w * u + w * v) * (u - w * u) =
    &1 - inv(u - w * u + w * v) * w * v`
  SUBST1_TAC THENL
   [UNDISCH_TAC `&0 < u - w * u + w * v` THEN CONV_TAC "Multivariate/polytope.ml:REAL_FIELD" REAL_FIELD;
   ...
   (`!a x:real^N s.
    ...
    STRIP_ASSUME_TAC THENL
     [EXISTS_TAC `setdist({vec 0:real^N},s) / a - norm(x:real^N)` THEN
      ...
      GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
      ASM_SIMP_TAC[REAL_LE_LMUL_EQ] THEN CONV_TAC "Multivariate/polytope.ml:NORM_ARITH" NORM_ARITH;
      MATCH_MP_TAC(MESON[]
      ...
      REWRITE_TAC[IN_BALL] THEN CONV_TAC "Multivariate/polytope.ml:NORM_ARITH" NORM_ARITH]) in
  let flemma = prove
   (`!a x:real^N s.
       convex s /\ bounded s /\ ~(vec 0 IN closure s) /\
       &0 < a /\ ~(x = vec 0) /\ ~(s = {}) /\
...
let CONVEX_SYMDIFF_CLOSE_TO_FRONTIER = prove
 (`!s t:real^N->bool e.
  ...
  ASM_REWRITE_TAC[] THEN
  GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV [GSYM CONTRAPOS_THM] THEN
  ...
  ASM_REWRITE_TAC[IN_BALL_0; GSYM dist] THEN CONV_TAC "Multivariate/polytope.ml:VECTOR_ARITH" VECTOR_ARITH);;
...
let POLYTOPE_LINEAR_IMAGE_EQ = prove
 (`!f:real^M->real^N s.
  ...
  MP_TAC(ISPEC `f:real^M->real^N` QUANTIFY_SURJECTION_THM) THEN
  ASM_REWRITE_TAC[] THEN
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)[th]) THEN
  MP_TAC(end_itlist CONJ
   (mapfilter (ISPEC `f:real^M->real^N`) (!invariant_under_linear))) THEN
  ASM_REWRITE_TAC[] THEN DISCH_TAC THEN ASM_REWRITE_TAC[]);;
...
let FINITE_POLYTOPE_FACES = prove
 (`!s:real^N->bool. polytope s ==> FINITE {f | f face_of s}`,
  ...
  GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [SUBSET] THEN
...
let POLYTOPE_AFF_DIM_1 = prove
 (`!p:real^N->bool.
  ...
  ASM_REWRITE_TAC[AFF_DIM_EMPTY] THEN CONV_TAC "Multivariate/polytope.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN
  DISCH_TAC THEN
 ...
 (`!s:real^N->bool e.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SUBSET_SUMS_RCANCEL THEN
    ...
    ASM_REWRITE_TAC[GSYM dist] THEN CONV_TAC "Multivariate/polytope.ml:VECTOR_ARITH" VECTOR_ARITH;
    FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP (REAL_ARITH
     `&0 < e ==> x <= e / &2 ==> x < e`)) THEN
    MATCH_MP_TAC REAL_HAUSDIST_LE THEN
    ASM_REWRITE_TAC[CONVEX_HULL_EQ_EMPTY] THEN CONJ_TAC THENL
...
let POLYHEDRON_POSITIVE_ORTHANT = prove
 (`polyhedron {x:real^N | !i. 1 <= i /\ i <= dimindex(:N) ==> &0 <= x$i}`,
  ...
  SIMP_TAC[FINITE_IMAGE; FINITE_NUMSEG; FORALL_IN_IMAGE] THEN CONJ_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [EXTENSION] THEN REWRITE_TAC[INTERS_IMAGE] THEN
...
let POLYHEDRON_INTER_AFFINE_PARALLEL = prove
 (`!s:real^N->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
   [RIGHT_IMP_EXISTS_THM] THEN
  ...
  MP_TAC THENL
   [GEN_TAC THEN STRIP_TAC THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
  REWRITE_TAC[SKOLEM_THM; LEFT_IMP_EXISTS_THM] THEN
  ...
  REWRITE_TAC[FORALL_IN_GSPEC] THEN CONJ_TAC THENL
   [ALL_TAC;
    ...
    ASM_MESON_TAC[]] THEN
  FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV [GSYM th]) THEN
  GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [EXTENSION] THEN X_GEN_TAC `x:real^N` THEN
...
let POLYHEDRON_INTER_AFFINE_PARALLEL_MINIMAL = prove
 (`!s. polyhedron s <=>
  ...
  GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV
   [MESON[HAS_SIZE]
     `(?f. FINITE f /\ P f) <=> (?n f. f HAS_SIZE n /\ P f)`] THEN
  GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
  ...
  CONJ_TAC THENL
   [REPEAT STRIP_TAC THEN FIRST_X_ASSUM MATCH_MP_TAC THEN ASM SET_TAC[];
    ...
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV [th]) THEN
    ASM SET_TAC[]]);;
...
let RELATIVE_INTERIOR_POLYHEDRON_EXPLICIT = prove
 (`!s:real^N->bool f a b.
  ...
  GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_ELIM_THM] THEN
  X_GEN_TAC `x:real^N` THEN EQ_TAC THENL
   [ALL_TAC;
    ...
    CONJ_TAC THENL
     [X_GEN_TAC `h:real^N->bool` THEN DISCH_TAC THEN
      ...
      FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
      MATCH_MP_TAC(SET_RULE
 ...
 (`!s:real^N->bool f a b.
  ...
  (LABEL_TAC "face") THENL
   [REPEAT STRIP_TAC THEN REWRITE_TAC[facet_of] THEN CONJ_TAC THENL
    ...
    GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV [EXTENSION] THEN
    DISCH_THEN(MP_TAC o SPEC `x:real^N`) THEN
    ASM_REWRITE_TAC[IN_ELIM_THM] THEN
    DISCH_THEN(fun th ->
    ...
    SUBGOAL_THEN `(w:real^N) IN s` ASSUME_TAC THENL
     [FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [th]) THEN
      ...
      FIRST_ASSUM(fun t -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [t]) THEN
    ...
    MATCH_MP_TAC(TAUT `b /\ (b ==> a) ==> a /\ b`) THEN CONJ_TAC THENL
     [EXPAND_TAC "c" THEN REWRITE_TAC[DOT_RADD; DOT_RMUL] THEN
      ASM_REWRITE_TAC[] THEN CONV_TAC "Multivariate/polytope.ml:REAL_RING" REAL_RING;
      DISCH_TAC] THEN
    FIRST_ASSUM(fun t -> GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [t]) THEN
    REWRITE_TAC[IN_INTER; IN_INTERS] THEN CONJ_TAC THENL
     [EXPAND_TAC "c" THEN
      MATCH_MP_TAC(REWRITE_RULE[AFFINE_ALT] AFFINE_AFFINE_HULL) THEN
      ASM_SIMP_TAC[HULL_INC];
...
let FACE_OF_POLYHEDRON_EXPLICIT = prove
 (`!s:real^N->bool f a b.
  ...
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [th]) THEN
  MATCH_MP_TAC(SET_RULE
...
let FACE_OF_POLYHEDRON = prove
 (`!s:real^N->bool c.
  ...
  ANTS_TAC THENL [ASM_REWRITE_TAC[] THEN ASM_MESON_TAC[]; ALL_TAC] THEN
  DISCH_THEN(MP_TAC o SPEC `c:real^N->bool`) THEN ASM_REWRITE_TAC[] THEN
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV [th]) THEN
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [EXTENSION] THEN
 ...
 (`!s:real^N->bool. polyhedron s ==> FINITE {f | f face_of s}`,
  ...
  EXISTS_TAC
   `{INTERS {s INTER {x:real^N | a(h:real^N->bool) dot x = b h} | h | h IN f'}
    |f'| f' SUBSET f}` THEN
  GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [SIMPLE_IMAGE_GEN] THEN
  ASM_SIMP_TAC[FINITE_POWERSET; FINITE_IMAGE] THEN
  GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [SUBSET] THEN REWRITE_TAC[IN_DELETE; IN_ELIM_THM] THEN
  MP_TAC(ISPECL [`s:real^N->bool`; `f:(real^N->bool)->bool`;
                 `a:(real^N->bool)->real^N`; `b:(real^N->bool)->real`]
         FACE_OF_POLYHEDRON_EXPLICIT) THEN
  ANTS_TAC THENL [ASM_REWRITE_TAC[] THEN ASM_MESON_TAC[]; ALL_TAC] THEN
 ...
 (`!s:real^N->bool.
  ...
  SUBGOAL_THEN `~(relative_interior(s:real^N->bool) = {})` MP_TAC THENL
   [ASM_SIMP_TAC[RELATIVE_INTERIOR_EQ_EMPTY];
    GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV [GSYM MEMBER_NOT_EMPTY] THEN
...
let POLYHEDRON_TRANSLATION_EQ = prove
 (`!a s. polyhedron (IMAGE (\x:real^N. a + x) s) <=> polyhedron s`,
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [th]) THEN
...
let POLYHEDRON_NEGATIONS = prove
 (`!s:real^N->bool. polyhedron s ==> polyhedron(IMAGE (--) s)`,
  GEN_TAC THEN MATCH_MP_TAC EQ_IMP THEN CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV THEN
...
let POLYHEDRON_LINEAR_PREIMAGE = prove
 (`!f:real^M->real^N s.
  ...
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [polyhedron] THEN
...
let POLYHEDRON_CONVEX_CONE_HULL = prove
 (`!s:real^N->bool. FINITE s ==> polyhedron(convex_cone hull s)`,
    ...
    SUBGOAL_THEN `&0 < (b:(real^N->bool)->real) h` ASSUME_TAC THENL
     [ASM_REWRITE_TAC[REAL_LT_LE] THEN
      ...
      SUBGOAL_THEN `h = {x:real^N | a h dot x <= b h}`
       (fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [th])
  ...
  SUBGOAL_THEN `x:real^N = inv(inf(IMAGE t (f:(real^N->bool)->bool))) %
                           inf(IMAGE t f) % x`
  SUBST1_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV [GSYM VECTOR_MUL_LID] THEN
    REWRITE_TAC[VECTOR_MUL_ASSOC] THEN AP_THM_TAC THEN AP_TERM_TAC THEN
    CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_MUL_LINV THEN
 ...
 (`!s:real^N->bool.
  ...
  STRIP_ASSUME_TAC THENL
   [EXISTS_TAC `interval[--vec 1:real^N,vec 1:real^N]` THEN
    ...
    CONV_TAC "Multivariate/polytope.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    ALL_TAC] THEN
...
let POLYHEDRON_AS_CONE_PLUS_CONV = prove
 (`!s:real^N->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o REDEPTH_CONV)" (LAND_CONV o REDEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
   ...
   [MP_TAC(ISPEC `s':real^(N,1)finite_sum->bool`
    ...
    SUBST1_TAC THENL
     [MAP_EVERY EXPAND_TAC ["s"; "s'"] THEN
      ...
      GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [EXTENSION] THEN
    ...
    AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [FUN_EQ_THM] THEN
    X_GEN_TAC `a:real^(N,1)finite_sum` THEN REWRITE_TAC[] THEN
    MATCH_MP_TAC(TAUT `(p ==> (q <=> r)) ==> (p /\ q <=> p /\ r)`) THEN
    DISCH_TAC THEN AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [FUN_EQ_THM] THEN
...
let POLYHEDRAL_CONVEX_CONE = prove
 (`!s:real^N->bool.
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [GSYM th]) THEN
  FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [GSYM MEMBER_NOT_EMPTY]) THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC CONVEX_CONE_MUL THEN
    ...
    ASM_SIMP_TAC[REAL_LT_IMP_LE] THEN CONV_TAC "Multivariate/polytope.ml:NORM_ARITH" NORM_ARITH;
    REWRITE_TAC[VECTOR_ADD_LDISTRIB; VECTOR_MUL_ASSOC] THEN
  ...
  REPEAT STRIP_TAC THENL
   [SUBGOAL_THEN `&0 <= (b - (A:real^N^M) ** x) dot y` MP_TAC THENL
     [REWRITE_TAC[dot] THEN MATCH_MP_TAC SUM_POS_LE THEN
      ...
      GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [DOT_SYM] THEN
 ...
 (`!s:real^N->bool.
  ...
  THENL
   [ASM_SIMP_TAC[CARD_EQ_0; CONVEX_HULL_EMPTY] THEN SET_TAC[];
    ...
    REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[EMPTY_UNIONS] THEN
   ...
   [REWRITE_TAC[frontier] THEN MATCH_MP_TAC EQ_TRANS THEN
    EXISTS_TAC `(convex hull s:real^N->bool) DIFF {}` THEN CONJ_TAC THENL
     [BINOP_TAC THEN
      ...
      CONJ_TAC THENL
       [GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV [CARATHEODORY_AFF_DIM] THEN
     ...
     [ASM_SIMP_TAC[GSYM RELATIVE_BOUNDARY_OF_CONVEX_HULL; frontier] THEN
      BINOP_TAC THENL
       [MATCH_MP_TAC CLOSURE_CLOSED THEN
        ASM_SIMP_TAC[CLOSURE_CLOSED; COMPACT_IMP_CLOSED; COMPACT_CONVEX_HULL;
                     FINITE_IMP_COMPACT; FINITE_INSERT; FINITE_EMPTY];
        CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC RELATIVE_INTERIOR_INTERIOR THEN
 ...
 (`!a b c:real^2.
  ...
  SUBGOAL_THEN `{a:real^2,b,c} HAS_SIZE (dimindex(:2) + 1)` ASSUME_TAC THENL
   [SIMP_TAC[HAS_SIZE; CARD_CLAUSES; FINITE_INSERT; FINITE_EMPTY] THEN
    ...
    CONV_TAC "Multivariate/polytope.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
...
let POLYHEDRON_RIDGE_TWO_FACETS = prove
 (`!p:real^N->bool r.
   ...
   [REPEAT CONJ_TAC THENL
     [DISJ_CASES_TAC(REAL_ARITH
      ...
      (ANTS_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
       ...
       DISCH_TAC THEN
       FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:LAND_CONV" LAND_CONV [SYM th]) THEN
       AP_TERM_TAC)
      THENL
  ...
  MP_TAC THENL
   [ASM_SIMP_TAC[] THEN REPEAT STRIP_TAC THENL
     [FIRST_X_ASSUM(MP_TAC o SPEC `f DELETE (h3:real^N->bool)`);
      ...
      FIRST_X_ASSUM(MP_TAC o SPEC `f DELETE (h1:real^N->bool)`)] THEN
    (ANTS_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
     FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [SYM th]) THEN
  ...
  CONJ_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/polytope.ml:RAND_CONV" RAND_CONV [GSYM DIM_SPAN] THEN
...
let POLYTOPE_FACET_LOWER_BOUND = prove
 (`!p:real^N->bool.
  ...
  CONV_TAC "Multivariate/polytope.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN STRIP_TAC THEN
  SUBGOAL_THEN
  ...
  SUBGOAL_THEN `!h:real^N->bool. h IN H ==> &0 <= b h` ASSUME_TAC THENL
   [UNDISCH_TAC `(vec 0:real^N) IN p` THEN EXPAND_TAC "p" THEN
    ...
    DISCH_THEN(fun t -> GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM t]) THEN
...
let SIMPLEX_EMPTY = prove
 (`!n. n simplex {} <=> n = -- &1`,
  ...
  GEN_REWRITE_TAC "Multivariate/polytope.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [EQ_SYM_EQ] THEN
...
let FACES_OF_SIMPLEX = prove
 (`!n s. n simplex s
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [EXTENSION] THEN
  REWRITE_TAC[IN_ELIM_THM] THEN
...
let SIMPLEX_ALT1 = prove
 (`!s:real^N->bool.
  ...
  CONV_TAC "Multivariate/polytope.ml:TAUT" TAUT);;
 ...
 (`!m:(real^N->bool)->bool d e.
  ...
  and lemma2 = prove
   (`!m:(real^N->bool)->bool d.
    ...
    GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [IMP_CONJ] THEN
     ...
     [MATCH_MP_TAC(TAUT
      ...
      REPEAT CONJ_TAC THENL
       [MESON_TAC[POLYTOPE_IMP_CONVEX; POLYTOPE_IMP_CLOSED];
        ASM SET_TAC[];
        REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/polytope.ml:SELECT_CONV" SELECT_CONV THEN
    ...
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[FINITE_UNION] THEN
      ...
      ASM_CASES_TAC `(z:real^N) IN j` THENL
       ...
       CONJ_TAC THENL
        [GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [SUBSET] THEN REWRITE_TAC[FORALL_IN_GSPEC] THEN
         X_GEN_TAC `k:real^N->bool` THEN STRIP_TAC THEN
         REWRITE_TAC[IN_UNION] THEN DISJ2_TAC THEN
         REWRITE_TAC[IN_ELIM_THM] THEN ASM_MESON_TAC[];
         ALL_TAC] THEN
       REWRITE_TAC[GSYM(ONCE_REWRITE_RULE[CONJ_SYM] SUBSET_ANTISYM_EQ)] THEN
       CONJ_TAC THEN GEN_REWRITE_TAC "Multivariate/polytope.ml:I" I [SUBSET] THENL
 ...
 (`!s u:real^N->bool.
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[GSYM UNIONS_UNION] THEN EXPAND_TAC "u" THEN AP_TERM_TAC THEN
    SET_TAC[];
    ALL_TAC] THEN
  CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC lemma THEN
...
let TRIANGULAR_SUBDIVISION_OF_CELL_COMPLEX = prove
 (`!m:(real^N->bool)->bool d.
  ...
  CONJ_TAC THENL [ASM SET_TAC[]; CONV_TAC "Multivariate/polytope.ml:SYM_CONV" SYM_CONV] THEN
