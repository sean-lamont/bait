...
let HAS_DERIVATIVE_COMPONENTWISE_WITHIN = prove
 (`!f:real^M->real^N f' a s.
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
...
let FRECHET_DERIVATIVE_WORKS = prove
 (`!f net. f differentiable net <=>
  ...
  CONV_TAC "Multivariate/derivatives.ml:(RAND_CONV SELECT_CONV)" (RAND_CONV SELECT_CONV) THEN REWRITE_TAC[differentiable]);;
...
let LIM_MUL_NORM_WITHIN = prove
 (`!f a s. (f --> vec 0) (at a within s)
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
...
let HAS_DERIVATIVE_WITHIN_ALT = prove
 (`!f:real^M->real^N f' s x.
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [REAL_MUL_SYM] THEN
...
let DIFF_CHAIN_WITHIN = prove
 (`!f:real^M->real^N g:real^N->real^P f' g' x s.
  ...
  CONV_TAC "Multivariate/derivatives.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN
...
let HAS_DERIVATIVE_AT_REFLECT = prove
 (`!f:real^M->real^N f' a.
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:RAND_CONV" RAND_CONV [GSYM HAS_DERIVATIVE_WITHIN_REFLECT] THEN
...
let DIFFERENTIABLE_ON_REFLECT = prove
 (`!f:real^M->real^N s.
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [GSYM HAS_DERIVATIVE_WITHIN_REFLECT] THEN
...
let DIFFERENTIABLE_VSUM = prove
 (`!f net s.
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
...
let FRECHET_DERIVATIVE_UNIQUE_WITHIN = prove
 (`!f:real^M->real^N f' f'' x s.
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:I" I [TAUT `p = ~ ~p`] THEN
...
let HAS_FRECHET_DERIVATIVE_UNIQUE_AT = prove
 (`!f:real^M->real^N f' x.
  ...
  ASM_REWRITE_TAC[frechet_derivative] THEN CONV_TAC "Multivariate/derivatives.ml:SELECT_CONV" SELECT_CONV THEN
  ASM_MESON_TAC[]);;
  ...
  REWRITE_TAC[DROP_VEC; REAL_LT_01; o_THM] THEN ANTS_TAC THENL
   [CONJ_TAC THENL
     [MATCH_MP_TAC CONTINUOUS_ON_COMPOSE THEN
      ...
      CONJ_TAC THENL
       [GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o ABS_CONV)" (LAND_CONV o ABS_CONV) [GSYM VECTOR_ADD_LID] THEN
        REWRITE_TAC[VECTOR_ARITH
 ...
 (`!f:real^M->real^N g:real^M->real^P f' g' x.
  ...
  MP_TAC THENL
   [FIRST_X_ASSUM(CONJUNCTS_THEN(MP_TAC o CONJUNCT2 o
    ...
    GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [LIM_NULL_NORM] THEN
    ...
    REWRITE_TAC[NORM_LIFT] THEN CONV_TAC "Multivariate/derivatives.ml:NORM_ARITH" NORM_ARITH;
    ...
    ASM_REWRITE_TAC[REAL_SUB_LT] THEN FIRST_ASSUM(fun th -> MP_TAC th THEN
     REWRITE_TAC[IMP_IMP] THEN
     GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [GSYM EVENTUALLY_AND]) THEN
    FIRST_ASSUM(CONJUNCTS_THEN(ASSUME_TAC o CONJUNCT1 o
      GEN_REWRITE_RULE I [has_derivative_at])) THEN
    ASM_SIMP_TAC[ONORM_LE_EVENTUALLY] THEN
    GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [EVENTUALLY_AT_ZERO] THEN
 ...
 (`!f:real^M->real^N f' g g' s a.
  ...
  CONJ_TAC THENL
   [RULE_ASSUM_TAC(REWRITE_RULE[FUN_EQ_THM; o_THM; I_THM]) THEN
    ASM_SIMP_TAC[LINEAR_SUB] THEN CONV_TAC "Multivariate/derivatives.ml:NORM_ARITH" NORM_ARITH;
    ALL_TAC] THEN
 ...
 (`!f:real^M->real^N g f' g' t y.
  ...
  ASSUME_TAC THENL
   [X_GEN_TAC `e:real` THEN DISCH_TAC THEN
    ...
    GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [GSYM NORM_NEG] THEN
    REWRITE_TAC[VECTOR_ARITH
  ...
  STRIP_ASSUME_TAC THENL
   [EXISTS_TAC `&2 * C` THEN
    ...
    CONV_TAC "Multivariate/derivatives.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    MATCH_MP_TAC MONO_EXISTS THEN X_GEN_TAC `d:real` THEN
    ...
    CONV_TAC "Multivariate/derivatives.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM_REWRITE_TAC[NORM_TRIANGLE_SUB];
    ALL_TAC] THEN
  ...
  REWRITE_TAC[] THEN ANTS_TAC THENL
   [ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
     [ONCE_REWRITE_TAC[GSYM o_DEF] THEN
      ...
      GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [NORM_SUB] THEN
      DISCH_TAC THEN MATCH_MP_TAC REAL_LE_TRANS THEN
...
let HAS_DERIVATIVE_INVERSE_STRONG_X = prove
 (`!f:real^N->real^N g f' g' s y.
        ...
        ==> (g has_derivative g') (at y)`,
  REPEAT STRIP_TAC THEN
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/derivatives.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [SYM th]) THEN
 ...
 (`!s f:num->real^M->real^N f' g'.
  ...
  MP_TAC THENL
   [REWRITE_TAC[GSYM SKOLEM_THM; RIGHT_EXISTS_IMP_THM] THEN
    X_GEN_TAC `x:real^M` THEN DISCH_TAC THEN
    GEN_REWRITE_TAC "Multivariate/derivatives.ml:I" I [CONVERGENT_EQ_CAUCHY] THEN
  ...
  MATCH_MP_TAC(REAL_ARITH
   `d <= a + b + c
    ==> a <= e / &3 * n ==> b <= e / &3 * n ==> c <= e / &3 * n
        ==> d <= e * n`) THEN
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:(funpow 2 RAND_CONV o LAND_CONV)" (funpow 2 RAND_CONV o LAND_CONV) [NORM_SUB] THEN
  MATCH_MP_TAC(REAL_ARITH
   `(norm(x + y + z) = norm(a)) /\
...
let HAS_DERIVATIVE_BILINEAR_WITHIN = prove
 (`!h:real^M->real^N->real^P f g f' g' x:real^Q s.
  ...
  SUBGOAL_TAC "contf"
   ...
   [GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [GSYM VECTOR_ADD_RID] THEN
   ...
   [X_GEN_TAC `n:num` THEN MATCH_MP_TAC CONTINUOUS_ON_MUL THEN
    REWRITE_TAC[o_DEF] THEN CONJ_TAC THENL
     [MATCH_MP_TAC(REWRITE_RULE[o_DEF] CONTINUOUS_ON_INV) THEN
      ...
      GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [GSYM o_DEF] THEN
      MATCH_MP_TAC CONTINUOUS_ON_COMPOSE THEN CONJ_TAC THENL
       [MATCH_MP_TAC CONTINUOUS_ON_ADD THEN
...
let GATEAUX_DERIVATIVE_LIPSCHITZ = prove
 (`!f:real^M->real^N f' x s.
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o LAND_CONV o RAND_CONV)" (LAND_CONV o LAND_CONV o RAND_CONV)
 ...
 (`!f:real^N->real^N f' a s.
  ...
  MP_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM]] THEN
  ...
  MP_TAC THENL
   [UNDISCH_TAC `(a:real^N) IN s` THEN SPEC_TAC(`a:real^N`,`x:real^N`);
    ...
    GEN_REWRITE_TAC "Multivariate/derivatives.ml:RAND_CONV" RAND_CONV [SWAP_EXISTS_THM] THEN
   ...
   [X_GEN_TAC `e0:real` THEN REPEAT STRIP_TAC THEN
    ...
    TRANS_TAC REAL_LTE_TRANS `(e:real) pow 2` THEN CONJ_TAC THENL
     [REWRITE_TAC[REAL_ARITH `a / &2 / b * x < a <=> a * x / b < a * &2`] THEN
      ...
      UNDISCH_TAC `&0 < l` THEN CONV_TAC "Multivariate/derivatives.ml:NORM_ARITH" NORM_ARITH;
   ...
   [REPEAT STRIP_TAC THEN REMOVE_THEN "L2" (MP_TAC o SPEC `x:real^N`) THEN
    ...
    CONJ_TAC THENL
     [RULE_ASSUM_TAC(REWRITE_RULE[FUN_EQ_THM; o_THM; I_THM]) THEN
      ...
      AP_TERM_TAC THEN CONV_TAC "Multivariate/derivatives.ml:VECTOR_ARITH" VECTOR_ARITH;
  ...
  STRIP_ASSUME_TAC THENL
   [ONCE_REWRITE_TAC[MESON[] `(?k. P k) <=> ~(!k. ~P k)`] THEN
    ...
    REWRITE_TAC[REAL_LT_INV_EQ; REAL_ARITH `&0 < &n + &1`] THEN
    GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [NOT_FORALL_THM] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [SWAP_EXISTS_THM] THEN
   ...
   [X_GEN_TAC `v:real^N->bool` THEN STRIP_TAC THEN
    ...
    REPEAT CONJ_TAC THENL
     [MATCH_MP_TAC OPEN_INTER THEN REWRITE_TAC[OPEN_BALL] THEN
      ...
      GEN_REWRITE_TAC "Multivariate/derivatives.ml:RAND_CONV" RAND_CONV [DISJ_SYM] THEN
...
let JACOBIAN_SIGN_INVARIANCE = prove
 (`!f:real^N->real^N f' s.
  ...
  MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
   [TRANS_TAC SUBSET_TRANS `ball(a:real^N,&2 * r)` THEN
    ...
    DISCH_TAC] THEN
  X_GEN_TAC `b:real^N` THEN DISCH_TAC THEN CONV_TAC "Multivariate/derivatives.ml:SYM_CONV" SYM_CONV THEN
  ...
  MP_TAC THENL
   [REPEAT GEN_TAC THEN STRIP_TAC THEN REWRITE_TAC[IN_BALL] THEN
    ...
    MATCH_MP_TAC(REAL_ARITH `y <= x /\ &0 <= y - x + &1
      ==> abs(x - y) <= &1`) THEN
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LE_MUL2 THEN CONV_TAC "Multivariate/derivatives.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
      ASM_SIMP_TAC[REAL_POW_LE] THEN MATCH_MP_TAC REAL_POW_MONO_INV THEN
      CONV_TAC "Multivariate/derivatives.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ASM_REAL_ARITH_TAC;
  ...
  CONJ_TAC THENL
   [EXISTS_TAC `&1` THEN REWRITE_TAC[REAL_LT_01; FORALL_PASTECART] THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:I" I [LIM_NULL] THEN
 ...
 (POP_ASSUM_LIST(K ALL_TAC) THEN REWRITE_TAC[has_derivative_at] THEN
  ...
  SUBST1_TAC THENL
   [FIRST_ASSUM(fun th -> REWRITE_TAC[MATCH_MP LINEAR_CMUL th]) THEN
    GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [GSYM VECTOR_MUL_LID] THEN
   ...
   [REWRITE_TAC[VECTOR_MUL_ASSOC] THEN MATCH_MP_TAC LIM_VMUL THEN
    ...
    MATCH_MP_TAC LIM_MUL THEN REWRITE_TAC[o_DEF] THEN CONJ_TAC THENL
     [SUBST1_TAC(GSYM REAL_INV_1) THEN
      ...
      CONV_TAC "Multivariate/derivatives.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [VECTOR_ARITH `x:real^N = &1 % x - &0 % x`] THEN
  SUBGOAL_THEN
       ...
       [MATCH_MP_TAC LIM_CONTINUOUS_SELF_WITHIN THEN
        ...
        MATCH_MP_TAC LIM_NULL_ADD THEN CONJ_TAC THENL
         [MATCH_MP_TAC LIM_NULL_SUB THEN CONJ_TAC THEN
          ...
          REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/derivatives.ml:VECTOR_ARITH" VECTOR_ARITH;
         ...
         [MATCH_MP_TAC ALWAYS_EVENTUALLY THEN
          ...
          ASM_REWRITE_TAC[] THENL
           [REWRITE_TAC[VECTOR_SUB_REFL] THEN
            ...
            BINOP_TAC THENL [ALL_TAC; CONV_TAC "Multivariate/derivatives.ml:VECTOR_ARITH" VECTOR_ARITH] THEN
            REWRITE_TAC[GSYM NORM_1] THEN
            MATCH_MP_TAC(REAL_FIELD `~(y = &0) ==> (x * y) * inv y = x`) THEN
            ASM_REWRITE_TAC[NORM_EQ_0; VECTOR_SUB_EQ]];
          GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [LIM_NULL_NORM] THEN
          REWRITE_TAC[NORM_MUL; REAL_ABS_INV; REAL_ABS_NORM] THEN
          REWRITE_TAC[NORM_1; GSYM REAL_ABS_INV] THEN
          REWRITE_TAC[GSYM NORM_MUL; GSYM LIM_NULL_NORM] THEN
          GEN_REWRITE_TAC "Multivariate/derivatives.ml:RAND_CONV" RAND_CONV [LIM_NULL] THEN
          MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ] LIM_TRANSFORM) THEN
          REWRITE_TAC[VECTOR_ARITH
 ...
 (`!f:real^M->real^N f' g' s a.
  ...
  CONJ_TAC THEN X_GEN_TAC `u:real^M` THEN DISCH_TAC THEN REWRITE_TAC[] THENL
   [EXPAND_TAC "ph" THEN
    ...
    GEN_REWRITE_TAC "Multivariate/derivatives.ml:(RATOR_CONV o BINDER_CONV)" (RATOR_CONV o BINDER_CONV) [GSYM VECTOR_SUB_RZERO] THEN
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [GSYM VECTOR_NEG_SUB] THEN
     ...
     [DISCH_THEN(fun th -> MATCH_MP_TAC LIM_MATRIX_INV THEN MP_TAC th) THEN
      ...
      CONJ_TAC THENL
       [ALL_TAC;
        ...
        X_GEN_TAC `y:real^N` THEN STRIP_TAC THEN CONV_TAC "Multivariate/derivatives.ml:SYM_CONV" SYM_CONV] THEN
      ...
      SIMP_TAC[EVENTUALLY_TRUE] THEN CONJ_TAC THENL
       [SUBGOAL_THEN `x = (g:real^N->real^N)(f x)`
         (fun th -> GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [th])
        THENL
         [RULE_ASSUM_TAC(REWRITE_RULE[homeomorphism]) THEN ASM SET_TAC[];
        ...
        FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (MESON[]
         `(x --> l) net
          ==> m = l /\ ((x --> l) net ==> (y --> l) net)
              ==> (y --> m) net`)) THEN
        CONJ_TAC THENL
         [CONV_TAC "Multivariate/derivatives.ml:SYM_CONV" SYM_CONV;
          MATCH_MP_TAC(ONCE_REWRITE_RULE[IMP_CONJ]
           (REWRITE_RULE[CONJ_ASSOC] LIM_TRANSFORM_WITHIN_OPEN)) THEN
          EXISTS_TAC `t:real^N->bool` THEN ASM_REWRITE_TAC[] THEN
     ...
     [MATCH_MP_TAC HAS_DERIVATIVE_ADD THEN CONJ_TAC THENL
      ...
      ASM_SIMP_TAC[GSYM LINEAR_SURJECTIVE_RIGHT_INVERSE_EQ] THEN ANTS_TAC THENL
       [MATCH_MP_TAC LINEAR_COMPOSE_ADD THEN
        ...
        GEN_REWRITE_TAC "Multivariate/derivatives.ml:RAND_CONV" RAND_CONV [GSYM o_DEF] THEN
      ...
      CONV_TAC "Multivariate/derivatives.ml:VECTOR_ARITH" VECTOR_ARITH];
   ...
   [REPEAT STRIP_TAC THEN MP_TAC(ISPECL
    ...
    ASM_REWRITE_TAC[OPEN_IN_TRANSLATION_EQ] THEN ANTS_TAC THENL
     [REPEAT CONJ_TAC THENL
       [REWRITE_TAC[VECTOR_ARITH
        ...
        FIRST_X_ASSUM(MP_TAC o SPEC `x:real^N`) THEN ASM_REWRITE_TAC[] THEN
        STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
        GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [SYM(CONJUNCT1(SPEC_ALL I_O_ID))] THEN
        MATCH_MP_TAC DIFF_CHAIN_WITHIN THEN CONJ_TAC THENL
         [GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [SYM(CONJUNCT2(SPEC_ALL I_O_ID))] THEN
          MATCH_MP_TAC DIFF_CHAIN_WITHIN THEN
    ...
    ASM_SIMP_TAC[] THEN REPEAT CONJ_TAC THENL
     [SUBGOAL_THEN `p = IMAGE (\x:real^N. a + x) (IMAGE (\x. --a + x) p)`
      ...
      GEN_REWRITE_TAC "Multivariate/derivatives.ml:(RAND_CONV o TOP_DEPTH_CONV)" (RAND_CONV o TOP_DEPTH_CONV) [o_DEF] THEN
      ...
      SUBST1_TAC THENL
       [REWRITE_TAC[GSYM IMAGE_o; o_DEF; ETA_AX; VECTOR_ARITH
         `a + --a + x:real^N = x`];
        GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [SYM(CONJUNCT1(SPEC_ALL I_O_ID))]];
      X_GEN_TAC `x:real^N` THEN DISCH_TAC THEN
      GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [SYM(CONJUNCT1(SPEC_ALL I_O_ID))]] THEN
    (MATCH_MP_TAC DIFF_CHAIN_WITHIN THEN CONJ_TAC THENL
      [GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [SYM(CONJUNCT2(SPEC_ALL I_O_ID))] THEN
...
let VECTOR_DERIVATIVE_WORKS = prove
 (`!net f:real^1->real^N.
  ...
  CONV_TAC "Multivariate/derivatives.ml:(RAND_CONV SELECT_CONV)" (RAND_CONV SELECT_CONV) THEN
...
let HAS_VECTOR_DERIVATIVE_UNIQUE_AT = prove
 (`!f:real^1->real^N f' x.
  ...
  ASM_REWRITE_TAC[vector_derivative] THEN CONV_TAC "Multivariate/derivatives.ml:SELECT_CONV" SELECT_CONV THEN
  ASM_MESON_TAC[]);;
...
let VECTOR_DIFFERENTIABLE_BOUND = prove
 (`!f f':real^1->real^N s B.
  ...
  [HYP_TAC "diff" (REWRITE_RULE[has_vector_derivative]) THEN
   ...
   GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
...
let HAS_VECTOR_DERIVATIVE_WITHIN_1D = prove
 (`!f:real^1->real^N s x.
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [LIM_NULL] THEN
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:LAND_CONV" LAND_CONV [LIM_NULL_NORM] THEN
  REWRITE_TAC[NORM_MUL; REAL_ABS_INV; REAL_ABS_NORM] THEN
  REWRITE_TAC[NORM_1; GSYM REAL_ABS_INV] THEN
...
let NORM_VECTOR_DERIVATIVES_LE_WITHIN = prove
 (`!f:real^1->real^M g:real^1->real^N f' g' x s.
  ...
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:BINOP_CONV" BINOP_CONV [GSYM LIFT_DROP] THEN
...
let CONVEX_ON_SECANT_DERIVATIVE_IMP = prove
 (`!f f' s x y:real^N.
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM VECTOR_NEG_SUB] THEN
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:I" I [GSYM LIFT_EQ] THEN REWRITE_TAC[LIFT_NEG] THEN
...
let CONVEX_ON_SECANT_DERIVATIVE = prove
 (`!f f' s:real^N->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/derivatives.ml:RAND_CONV" RAND_CONV [SWAP_FORALL_THM] THEN
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/derivatives.ml:I" I [FUN_EQ_THM] THEN
