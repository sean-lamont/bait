...
let CLOSED_REAL = prove
 (`closed real`,
  GEN_REWRITE_TAC "Multivariate/canal.ml:RAND_CONV" RAND_CONV [SET_RULE `s = {x | s x}`] THEN
...
let TRIVIAL_LIMIT_WITHIN_REAL = prove
 (`!z. trivial_limit (at z within real) <=> ~(real z)`,
  ...
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/canal.ml:RAND_CONV" RAND_CONV [GSYM IN] THEN
    ...
    ASM_REWRITE_TAC[EVENTUALLY_AND] THEN CONJ_TAC THENL
     [FIRST_X_ASSUM(MP_TAC o SPEC `b / &2`) THEN
      ...
      FIRST_X_ASSUM(fun th -> MP_TAC th THEN REWRITE_TAC[IMP_IMP] THEN
        GEN_REWRITE_TAC "Multivariate/canal.ml:LAND_CONV" LAND_CONV [GSYM EVENTUALLY_AND]) THEN
 ...
 (`!f:real^N->complex s a l.
  ...
  STRIP_ASSUME_TAC THENL
   [FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [LIM_AT]) THEN
    ...
    GEN_REWRITE_TAC "Multivariate/canal.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_INV] THEN
  ...
  CONV_TAC "Multivariate/canal.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN CONJ_TAC THENL
   [MATCH_MP_TAC(REAL_ARITH `x >= &1 ==> &1 / &2 <= x`) THEN
    ...
    CONV_TAC "Multivariate/canal.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM_REWRITE_TAC[] THEN
...
let SERIES_COMPLEX_LMUL = prove
 (`!f l c s. (f sums l) s ==> ((\x. c * f x) sums c * l) s`,
  ...
  ASM_REWRITE_TAC[] THEN GEN_REWRITE_TAC "Multivariate/canal.ml:RAND_CONV" RAND_CONV [GSYM ETA_AX] THEN
...
let HAS_COMPLEX_DERIVATIVE_WITHIN = prove
 (`!f s a. (f has_complex_derivative f') (at a within s) <=>
  ...
  GEN_REWRITE_TAC "Multivariate/canal.ml:RAND_CONV" RAND_CONV [LIM_NULL] THEN
...
let HAS_COMPLEX_DERIVATIVE_LINEAR = prove
 (`!net c. ((\x. c * x) has_complex_derivative c) net`,
  ...
  REWRITE_TAC[linear; COMPLEX_CMUL] THEN CONV_TAC "Multivariate/canal.ml:COMPLEX_RING" COMPLEX_RING);;
...
let HAS_COMPLEX_DERIVATIVE_MUL_WITHIN = prove
 (`!f f' g g' x s.
  ...
  CONV_TAC "Multivariate/canal.ml:COMPLEX_RING" COMPLEX_RING);;
...
let HAS_COMPLEX_DERIVATIVE_POW_WITHIN = prove
 (`!f f' x s n. (f has_complex_derivative f') (at x within s)
  ...
  CONV_TAC "Multivariate/canal.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[SUC_SUB1; complex_pow] THEN
  CONV_TAC "Multivariate/canal.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
...
let HAS_COMPLEX_DERIVATIVE_INV_BASIC = prove
 (`!x. ~(x = Cx(&0))
  ...
  ASM_REWRITE_TAC[COMPLEX_CMUL] THEN CONJ_TAC THENL
   [POP_ASSUM MP_TAC THEN CONV_TAC "Multivariate/canal.ml:COMPLEX_FIELD" COMPLEX_FIELD; ALL_TAC] THEN
  SUBGOAL_THEN `((\y. inv(x pow 2 * y) * (y - x)) --> Cx(&0)) (at x)`
  MP_TAC THENL
   [LIM_TAC THEN POP_ASSUM MP_TAC THEN CONV_TAC "Multivariate/canal.ml:COMPLEX_FIELD" COMPLEX_FIELD; ALL_TAC] THEN
  ...
  CONV_TAC "Multivariate/canal.ml:REAL_FIELD" REAL_FIELD);;
...
let HAS_COMPLEX_DERIVATIVE_DIV_WITHIN = prove
 (`!f f' g g' x s.
  ...
  POP_ASSUM MP_TAC THEN CONV_TAC "Multivariate/canal.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
...
let HAS_COMPLEX_DERIVATIVE_INVERSE_BASIC = prove
 (`!f g f' t y.
  ...
  UNDISCH_TAC `~(f' = Cx(&0))` THEN CONV_TAC "Multivariate/canal.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
 ...
 (`!f s. open s /\ connected s /\
  ...
  MP_TAC THENL
   [MATCH_MP_TAC FRECHET_DERIVATIVE_UNIQUE_AT THEN
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "Multivariate/canal.ml:REAL_RING" REAL_RING]);;
...
let COMPLEX_DIFFERENTIABLE_TAC =
  ...
  GEN_REWRITE_TAC "Multivariate/canal.ml:I" I [complex_differentiable] THEN
  W(fun (asl,w) ->
   ...
   [ASM_REWRITE_TAC[CONVEX_SEGMENT] THEN X_GEN_TAC `u:complex` THEN
    ...
    MP_TAC THENL
     [MATCH_MP_TAC HAS_COMPLEX_DERIVATIVE_VSUM THEN
      ...
      MATCH_MP_TAC EQ_IMP THEN AP_THM_TAC THEN AP_TERM_TAC THEN
      REWRITE_TAC[complex_div] THEN CONV_TAC "Multivariate/canal.ml:COMPLEX_RING" COMPLEX_RING;
      ALL_TAC] THEN
    ASM_SIMP_TAC[] THEN MATCH_MP_TAC EQ_IMP THEN AP_THM_TAC THEN
    AP_TERM_TAC THEN REWRITE_TAC[VSUM_ADD_NUMSEG] THEN
    GEN_REWRITE_TAC "Multivariate/canal.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [lemma] THEN
   ...
   [REWRITE_TAC[DROP_VEC; REAL_POS] THEN
    ...
    MATCH_MP_TAC DIFF_CHAIN_AT THEN CONJ_TAC THENL
     [REWRITE_TAC[VECTOR_ARITH `(&1 - t) % w + t % z = w + t % (z - w)`] THEN
      GEN_REWRITE_TAC "Multivariate/canal.ml:(RATOR_CONV o RAND_CONV o ABS_CONV)" (RATOR_CONV o RAND_CONV o ABS_CONV)
    ...
    CONV_TAC "Multivariate/canal.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
 ...
 (`!n z. norm(z) < &1
  ...
  EXISTS_TAC `\m. (z pow n - z pow SUC m) / (Cx (&1) - z)` THEN CONJ_TAC THENL
   [ASM_REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/canal.ml:(RATOR_CONV o RAND_CONV)" (RATOR_CONV o RAND_CONV) [GSYM COMPLEX_SUB_RZERO] THEN
...
let HAS_INTEGRAL_COMPLEX_LMUL = prove
 (`!f y i c. (f has_integral y) i ==> ((\x. c * f(x)) has_integral (c * y)) i`,
  ...
  ASM_REWRITE_TAC[linear; COMPLEX_CMUL] THEN CONV_TAC "Multivariate/canal.ml:COMPLEX_RING" COMPLEX_RING);;
...
let HAS_BOUNDED_VARIATION_ON_COMPLEX_MUL = prove
 (`!f g:real^1->complex s.
  ...
  CONJ_TAC THEN
  GEN_REWRITE_TAC "Multivariate/canal.ml:(LAND_CONV o BINDER_CONV o LAND_CONV)" (LAND_CONV o BINDER_CONV o LAND_CONV)
   [GSYM LIFT_DROP] THEN
...
let LIM_AT_INFINITY_COMPLEX_0 = prove
 (`!f l:real^N.
  ...
  FIRST_X_ASSUM MATCH_MP_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/canal.ml:LAND_CONV" LAND_CONV [GSYM REAL_INV_INV] THEN
  ...
  and lemma1 = prove
   (`!m n. m < n
    ...
    SIMP_TAC[VSUM_CLAUSES_NUMSEG; LT; LT_IMP_LE] THEN STRIP_TAC THENL
     [ASM_REWRITE_TAC[VSUM_SING_NUMSEG; complex_pow] THEN CONV_TAC "Multivariate/canal.ml:COMPLEX_RING" COMPLEX_RING;
    ...
    CONV_TAC "Multivariate/canal.ml:COMPLEX_RING" COMPLEX_RING) in
  SUBGOAL_THEN
   `!M a e.
     ...
     [ASM_SIMP_TAC[lemma1] THEN
      ...
      REWRITE_TAC[COMPLEX_NORM_MUL; COMPLEX_NORM_POW] THEN CONJ_TAC THENL
       [MATCH_MP_TAC REAL_LET_TRANS THEN
        ...
        GEN_REWRITE_TAC "Multivariate/canal.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
 ...
 (`!a k f z r.
  ...
  REWRITE_TAC[INTERIOR_CBALL; IN_BALL] THEN CONJ_TAC THENL
   [ALL_TAC;
    ...
    CONV_TAC "Multivariate/canal.ml:NORM_ARITH" NORM_ARITH] THEN
  MATCH_MP_TAC(ISPEC `sequentially` CONTINUOUS_UNIFORM_LIMIT) THEN
  EXISTS_TAC
  ...
  REWRITE_TAC[EVENTUALLY_SEQUENTIALLY; dist] THEN ANTS_TAC THENL
   [REWRITE_TAC[RE_CX; NORM_POS_LE; REAL_CX] THEN CONJ_TAC THENL
     [MATCH_MP_TAC POWER_SERIES_CONV_IMP_ABSCONV THEN
      ...
      REWRITE_TAC[COMPLEX_NORM_CX] THEN
      EXPAND_TAC "R" THEN UNDISCH_TAC `dist(z:complex,w) < r` THEN
      CONV_TAC "Multivariate/canal.ml:NORM_ARITH" NORM_ARITH;
      ...
      UNDISCH_TAC `norm(z - x:complex) <= R` THEN CONV_TAC "Multivariate/canal.ml:NORM_ARITH" NORM_ARITH];
    ...
    EXISTS_TAC `k:num->bool` THEN REWRITE_TAC[] THEN CONJ_TAC THENL
     [FIRST_X_ASSUM MATCH_MP_TAC THEN
      ...
      REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Multivariate/canal.ml:NORM_ARITH" NORM_ARITH;
...
let POWER_SERIES_LIMIT_POINT_OF_ZEROS = prove
 (`!f c z k r s.
  ...
  GEN_REWRITE_TAC "Multivariate/canal.ml:RAND_CONV" RAND_CONV [num_WOP] THEN
