 ...
 (`!s p k f:real^M->real^N g:real^M->real^N.
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[LIFT_CMUL] THEN MATCH_MP_TAC INTEGRABLE_CMUL THEN
    ...
    GEN_REWRITE_TAC "Multivariate/lpspaces.ml:(RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV) [GSYM CARD_NUMSEG_1] THEN
 ...
 (`!s p q f:real^M->real^N g:real^M->real^N.
  ...
  REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[LIFT_CMUL] THEN
    GEN_REWRITE_TAC "Multivariate/lpspaces.ml:(LAND_CONV o ABS_CONV o LAND_CONV)" (LAND_CONV o ABS_CONV o LAND_CONV)
        [GSYM LIFT_DROP] THEN
...
let LNORM_EQ_0 = prove
 (`!s p f. ~(p = &0) /\ f IN lspace s p
  ...
  SIMP_TAC[HAS_INTEGRAL_NEGLIGIBLE_EQ; lift; LAMBDA_BETA; NORM_POS_LE;
           RPOW_POS_LE] THEN
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/lpspaces.ml:I" I [EXTENSION] THEN
...
let HOELDER_INEQUALITY = prove
 (`!s p q f:real^M->real^N g:real^M->real^N.
  ...
  REPEAT
   (GEN_REWRITE_TAC "Multivariate/lpspaces.ml:LAND_CONV" LAND_CONV [REAL_ARITH `&0 <= x <=> x = &0 \/ &0 < x`] THEN
    DISCH_THEN(DISJ_CASES_THEN2 MP_TAC ASSUME_TAC) THENL
     [ASM_SIMP_TAC[LNORM_EQ_0; REAL_LT_IMP_NZ] THEN REPEAT DISCH_TAC THEN
      ...
      MESON_TAC[];
      ALL_TAC]) THEN
  GEN_REWRITE_TAC "Multivariate/lpspaces.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
...
let LNORM_TRIANGLE = prove
 (`!s p f:real^M->real^N g:real^M->real^N.
  ...
  GEN_REWRITE_TAC "Multivariate/lpspaces.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM REAL_POW_1] THEN
  ...
  CONV_TAC "Multivariate/lpspaces.ml:(LAND_CONV(SUBS_CONV[REAL_ARITH `p = &1 + (p - &1)`]))" (LAND_CONV(SUBS_CONV[REAL_ARITH `p = &1 + (p - &1)`])) THEN
 ...
 (`!f:num->real^M->real^N p s.
  ...
  SUBGOAL_THEN `!n:num. lnorm s p (g n:real^M->real^1) <= M` ASSUME_TAC THENL
   [GEN_TAC THEN EXPAND_TAC "g" THEN
    ...
    GEN_REWRITE_TAC "Multivariate/lpspaces.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [SET_RULE `s = UNIV INTER s`] THEN
  ...
  ASSUME_TAC THENL
   [X_GEN_TAC `x:real^M` THEN DISCH_TAC THEN
    ...
    ASM_SIMP_TAC[] THEN ANTS_TAC THENL
     [GEN_REWRITE_TAC "Multivariate/lpspaces.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM LIFT_DROP] THEN
  ...
  GEN_REWRITE_TAC "Multivariate/lpspaces.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
  ...
  ASSUME_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    GEN_REWRITE_TAC "Multivariate/lpspaces.ml:LAND_CONV" LAND_CONV [GSYM LIFT_DROP] THEN
  ...
  ANTS_TAC THENL
   [REPEAT CONJ_TAC THENL
     [X_GEN_TAC `n:num` THEN
      ...
      ASM_SIMP_TAC[]];
    GEN_REWRITE_TAC "Multivariate/lpspaces.ml:(LAND_CONV o RATOR_CONV o LAND_CONV o ABS_CONV)" (LAND_CONV o RATOR_CONV o LAND_CONV o ABS_CONV)
     [GSYM LIFT_DROP] THEN
  ...
  ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
   [ASM_SIMP_TAC[LSPACE_SUB; ETA_AX; REAL_ARITH `&1 <= p ==> &0 <= p`] THEN
    ...
    EXISTS_TAC `\n. inv(&2) pow n` THEN CONJ_TAC THENL
     [MATCH_MP_TAC REAL_SUMMABLE_GP THEN CONV_TAC "Multivariate/lpspaces.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
  ...
  REWRITE_TAC[] THEN ANTS_TAC THENL
   [REPEAT CONJ_TAC THENL
     [X_GEN_TAC `k:num` THEN
      ...
      GEN_REWRITE_TAC "Multivariate/lpspaces.ml:LAND_CONV" LAND_CONV [LIM_NULL] THEN
      DISCH_THEN(MP_TAC o ISPEC
       `(lift o (\x. x rpow p) o  drop) o (lift o (norm:real^N->real))` o
         MATCH_MP(REWRITE_RULE[IMP_CONJ_ALT] LIM_CONTINUOUS_FUNCTION)) THEN
      ASM_SIMP_TAC[o_THM; DROP_VEC; RPOW_ZERO; REAL_LT_IMP_NZ; LIFT_NUM] THEN
