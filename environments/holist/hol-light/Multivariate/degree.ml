...
let HAS_SIZE_1_EXISTS = prove
 (`!s. s HAS_SIZE 1 <=> ?!x. x IN s`,
  REPEAT GEN_TAC THEN CONV_TAC "Multivariate/degree.ml:(LAND_CONV HAS_SIZE_CONV)" (LAND_CONV HAS_SIZE_CONV) THEN
...
let IN_CONIC_CONVEX_HULL_ROWS_QFREE = prove
 (`!A:real^N^N z:real^N.
  ...
  GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o BINDER_CONV o RAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o BINDER_CONV o RAND_CONV o RAND_CONV o LAND_CONV)
   [GSYM DET_TRANSP] THEN
  SIMP_TAC[transp; LAMBDA_BETA; real_ge] THEN
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/degree.ml:I" I [FUN_EQ_THM] THEN
...
let FINITE_SET_AS_MATRIX_ROWS = prove
 (`!s. FINITE s /\ ~(s = {}) /\ CARD s <= dimindex(:N)
  ...
  GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM CARD_NUMSEG_1] THEN
...
let RELATIVE_ORIENTATION = prove
 (`!A:real^N^N f s:real^N->bool.
  ...
  CONV_TAC "Multivariate/degree.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN
...
let RELATIVE_ORIENTATION_LINEAR = prove
 (`!f:real^N->real^N c.
  ...
  SUBGOAL_THEN `~(det(A:real^N^N) = &0)` MP_TAC THENL
   [ASM_MESON_TAC[DET_ORDERED_SIMPLEX_NZ]; CONV_TAC "Multivariate/degree.ml:REAL_FIELD" REAL_FIELD]);;
 ...
 (`!f:real^N->real^N g:real^N->real^N c.
  ...
  ANTS_TAC THENL
   [REWRITE_TAC[ROWS_MAPROWS; vertex_image] THEN
    FIRST_ASSUM(SUBST1_TAC o SYM) THEN CONV_TAC "Multivariate/degree.ml:SYM_CONV" SYM_CONV THEN
  ...
  CONV_TAC "Multivariate/degree.ml:REAL_FIELD" REAL_FIELD);;
  ...
  MP_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    REWRITE_TAC[GSYM dist; GSYM SETDIST_SINGS] THENL
     [ALL_TAC; GEN_REWRITE_TAC "Multivariate/degree.ml:RAND_CONV" RAND_CONV [SETDIST_SYM]] THEN
    MATCH_MP_TAC SETDIST_SUBSET_RIGHT THEN ASM SET_TAC[];
    GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
 ...
 (`!f:real^N->real^N t u x y.
   ...
   (`!f:A->real s.
    ...
    CONJ_TAC THENL
     [W(MP_TAC o PART_MATCH (lhs o rand) SUM_SUM_PRODUCT o lhand o snd) THEN
      ...
      CONJ_TAC THENL
       [AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/degree.ml:I" I [EXTENSION] THEN
        ...
        GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [SWAP_EXISTS_THM] THEN
   ...
   (`!A:real^N^N z k.
    ...
    MP_TAC THENL
     [REPEAT STRIP_TAC THEN
      ...
      GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [RIGHT_IMP_EXISTS_THM] THEN
      REWRITE_TAC[SKOLEM_THM; LEFT_IMP_EXISTS_THM]] THEN
  ...
  THENL
   [X_GEN_TAC `w:real^N` THEN TRANS_TAC EQ_TRANS
    ...
    CONJ_TAC THENL
     [CONV_TAC "Multivariate/degree.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_UNION_EQ THEN
   ...
   [X_GEN_TAC `w:real^N` THEN REWRITE_TAC[EXTENSION; IN_ELIM_THM] THEN
    ...
    MATCH_MP_TAC FACE_OF_SUBSET_FRONTIER_AFF_DIM THEN CONJ_TAC THENL
     [MATCH_MP_TAC FACE_OF_CONIC_HULL THEN
      ...
      GEN_REWRITE_TAC "Multivariate/degree.ml:RAND_CONV" RAND_CONV [GSYM AFF_DIM_CONVEX_HULL] THEN
      AP_TERM_TAC THEN MATCH_MP_TAC KREIN_MILMAN_MINKOWSKI THEN
      ASM_MESON_TAC[FACE_OF_POLYTOPE_POLYTOPE; SIMPLEX_IMP_POLYTOPE;
                    POLYTOPE_IMP_CONVEX; POLYTOPE_IMP_COMPACT]];
    ONCE_REWRITE_TAC[SET_RULE
     ...
     [REWRITE_TAC[SET_RULE
      ...
      SUBGOAL_THEN `~(ball(a:real^N,e) SUBSET conic hull c)` MP_TAC THENL
       [SUBGOAL_THEN `(a:real^N) IN frontier(conic hull c)` MP_TAC THENL
        ...
        GEN_REWRITE_TAC "Multivariate/degree.ml:RAND_CONV" RAND_CONV [SWAP_EXISTS_THM] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [REPEAT CONJ_TAC THEN CONV_TAC "Multivariate/degree.ml:SYM_CONV" SYM_CONV THEN
  ...
  ASSUME_TAC THENL
   [SIMP_TAC[rows; GSYM IN_NUMSEG] THEN MATCH_MP_TAC(SET_RULE
    ...
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/degree.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
    ...
    CONJ_TAC THENL
     [AP_TERM_TAC THEN MATCH_MP_TAC(SET_RULE
      ...
      REWRITE_TAC[vertex_image] THEN AP_TERM_TAC THEN FIRST_X_ASSUM(fun th ->
        GEN_REWRITE_TAC "Multivariate/degree.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [SYM th]) THEN
      ONCE_REWRITE_TAC[SIMPLE_IMAGE_GEN] THEN
   ...
   [MP_TAC(ISPEC `conic hull vertex_image (f:real^N->real^N) k`
    ...
    ASM_REWRITE_TAC[NOT_IMP] THEN CONJ_TAC THENL
     [MATCH_MP_TAC FACE_OF_TRANS THEN
      ...
      MP_TAC THENL
       [FIRST_X_ASSUM(fun th ->
          GEN_REWRITE_TAC "Multivariate/degree.ml:(funpow 3 RAND_CONV)" (funpow 3 RAND_CONV) [SYM th]) THEN
        REWRITE_TAC[GSYM ROWS_MAPROWS] THEN
        ASM_MESON_TAC[DET_DEPENDENT_ROWS];
  ...
  MP_TAC THENL
   [ALL_TAC;
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV] THEN
   ...
   [REWRITE_TAC[frontier; INTERIOR_INTERIOR] THEN
    ...
    SUBST1_TAC THENL
     [MATCH_MP_TAC CONVEX_CLOSURE_INTERIOR THEN
      ...
      MATCH_MP_TAC FACE_OF_SUBSET_FRONTIER_AFF_DIM THEN
      ASM_SIMP_TAC[AFF_DIM_CONIC_HULL] THEN
      CONJ_TAC THENL [ALL_TAC; CONV_TAC "Multivariate/degree.ml:INT_ARITH" INT_ARITH] THEN
  ...
  ASM_CASES_TAC `real_sgn(det(B:real^N^N)) = real_sgn(det(A:real^N^N))` THENL
   [ALL_TAC;
    ...
    CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM_MESON_TAC[]] THEN
  SUBGOAL_THEN
...
let BROUWER_DEGREE3_LINEAR = prove
 (`!f:real^N->real^N t u y.
  ...
  CONV_TAC "Multivariate/degree.ml:HAS_SIZE_CONV" HAS_SIZE_CONV THEN
...
let BROUWER_DEGREE2_HOMOTOPY_INVARIANCE_LEMMA = prove
 (`!f g:real^N->real^N.
  ...
  ASM_REWRITE_TAC[REAL_LT_MIN; REAL_HALF] THEN ANTS_TAC THENL
   [CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
   [ASM_SIMP_TAC[] THEN EXPAND_TAC "z" THEN CONV_TAC "Multivariate/degree.ml:SELECT_CONV" SELECT_CONV THEN
  ...
  STRIP_ASSUME_TAC THENL
   [CONJ_TAC THENL [EXPAND_TAC "x"; EXPAND_TAC "y"] THEN
    CONV_TAC "Multivariate/degree.ml:SELECT_CONV" SELECT_CONV THEN
    ONCE_REWRITE_TAC[SET_RULE `(?x. P x) <=> ~({x | P x} = {})`] THEN
    MATCH_MP_TAC(MESON[CLOSURE_EMPTY; UNIV_NOT_EMPTY]
     `closure s = (:real^N) ==> ~(s = {})`) THEN
    MATCH_MP_TAC CLOSURE_CONIC_HULL_VERTEX_IMAGE_NONFRONTIERS THEN
 ...
 (`!f g:real^N->real^N.
  ...
  REPEAT STRIP_TAC THEN ASM_CASES_TAC `2 <= dimindex(:N)` THENL
   [MP_TAC(ISPECL
    ...
    MAP_EVERY EXPAND_TAC ["x"; "y"] THEN CONV_TAC "Multivariate/degree.ml:SELECT_CONV" SELECT_CONV THEN
    ONCE_REWRITE_TAC[SET_RULE `(?x. P x) <=> ~({x | P x} = {})`] THEN
    MATCH_MP_TAC(MESON[CLOSURE_EMPTY; UNIV_NOT_EMPTY]
     `closure s = (:real^N) ==> ~(s = {})`) THEN
    MATCH_MP_TAC CLOSURE_CONIC_HULL_VERTEX_IMAGE_NONFRONTIERS THEN
    ...
    REWRITE_TAC[CONNECTED_INTERVAL] THEN ANTS_TAC THENL
     [MATCH_MP_TAC CONTINUOUS_ON_COMPOSE THEN
      ...
      GEN_REWRITE_TAC "Multivariate/degree.ml:I" I [GSYM CONTRAPOS_THM] THEN
...
let HOMOTOPIC_ORTHOGONAL_TRANSFORMATIONS_IMP = prove
 (`!f g:real^N->real^N.
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/degree.ml:I" I [REAL_EQ_SGN_ABS] THEN CONJ_TAC THENL
   [MATCH_MP_TAC HOMOTOPIC_LINEAR_MAPS_IMP THEN
    ...
    REWRITE_TAC[VECTOR_MUL_LID] THEN CONJ_TAC THENL
     [MATCH_MP_TAC CONTINUOUS_ON_MUL THEN
      ...
      GEN_REWRITE_TAC "Multivariate/degree.ml:LAND_CONV" LAND_CONV [GSYM o_DEF] THEN
      ...
      CONV_TAC "Multivariate/degree.ml:NORM_ARITH" NORM_ARITH];
    MATCH_MP_TAC(REAL_ARITH
     `(x = &1 \/ x = -- &1) /\ (y = &1 \/ y = -- &1)
      ==> abs x = abs y`) THEN
 ...
 (`!f:real^N->real^N.
  ...
  ASM_REWRITE_TAC[NOT_IMP] THEN REPEAT CONJ_TAC THENL
   [SIMP_TAC[CONTINUOUS_ON_NEG; CONTINUOUS_ON_ID];
    ...
    CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV]);;
   ...
   [MATCH_MP_TAC MONO_EXISTS THEN SIMP_TAC[];
    ...
    ASM_REWRITE_TAC[NOT_IMP] THEN REPEAT CONJ_TAC THENL
     [MATCH_MP_TAC CONTINUOUS_ON_MUL THEN CONJ_TAC THENL
      ...
      FIRST_X_ASSUM(fun th ->
        GEN_REWRITE_TAC "Multivariate/degree.ml:(RAND_CONV o RAND_CONV o LAND_CONV)" (RAND_CONV o RAND_CONV o LAND_CONV) [SYM th]) THEN
      REWRITE_TAC[ORTHOGONAL_MUL; ORTHOGONAL_LNEG; ORTHOGONAL_REFL;
                  REAL_INV_EQ_0; NORM_EQ_0] THEN
      CONV_TAC "Multivariate/degree.ml:TAUT" TAUT];
 ...
 (`!A:real^N^N.
  ...
  ASM_REWRITE_TAC[VECTOR_NEG_NEG] THEN ANTS_TAC THENL
   [SIMP_TAC[LINEAR_CONTINUOUS_ON; LINEAR_COMPOSE_NEG; LINEAR_ID;
    ...
    REWRITE_TAC[REAL_POW_ONE] THEN CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV]);;
 ...
 (`!a t s:real^N->bool.
  ...
  ASM_SIMP_TAC[IMAGE_SUBSET; FORALL_IN_IMAGE] THEN REPEAT CONJ_TAC THENL
   [REPEAT(MATCH_MP_TAC CONTINUOUS_ON_COMPOSE THEN
    ...
    GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)
     [GSYM IMAGE_o] THEN
...
let RETRACT_OF_PCROSS = prove
 (`!s:real^M->bool s' t:real^N->bool t'.
  ...
  MATCH_MP_TAC CONTINUOUS_ON_PASTECART THEN
  CONJ_TAC THEN GEN_REWRITE_TAC "Multivariate/degree.ml:LAND_CONV" LAND_CONV [GSYM o_DEF] THEN
  MATCH_MP_TAC CONTINUOUS_ON_COMPOSE THEN
   ...
   [MATCH_MP_TAC CONTINUOUS_ON_ADD THEN REWRITE_TAC[CONTINUOUS_ON_ID] THEN
    ...
    CONJ_TAC THENL
     [REWRITE_TAC[o_DEF; LIFT_CMUL; LIFT_SUB] THEN
      ...
      CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[LIFT_CMUL]] THEN
    ASM_SIMP_TAC[CONTINUOUS_ON_CMUL; CONTINUOUS_ON_SUB; CONTINUOUS_ON_ID;
 ...
 (`!s:real^N->bool. convex s ==> dimension s = aff_dim s`,
  ...
  SUBGOAL_THEN `dim(IMAGE basis (1..n):real^N->bool) = n` ASSUME_TAC THENL
   [REWRITE_TAC[DIM_BASIS_IMAGE] THEN
    GEN_REWRITE_TAC "Multivariate/degree.ml:RAND_CONV" RAND_CONV [GSYM CARD_NUMSEG_1] THEN
    AP_TERM_TAC THEN
   ...
   [SIMP_TAC[GSYM NUMSEG_RREC] THEN
    ...
    CONJ_TAC THENL
     [ASM_CASES_TAC `i = 1` THENL
       [ASM_REWRITE_TAC[] THEN CONV_TAC "Multivariate/degree.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
        CONV_TAC "Multivariate/degree.ml:(ONCE_DEPTH_CONV NUMSEG_CONV)" (ONCE_DEPTH_CONV NUMSEG_CONV) THEN
 ...
 (`!u s:real^N->bool.
  ...
  CONJ_TAC THENL
   [ASM_SIMP_TAC[DIMENSION_EQ_AFF_DIM; CONVEX_INTER; AFFINE_IMP_CONVEX;
    ...
    MATCH_MP_TAC INT_EQ_IMP_LE THEN CONV_TAC "Multivariate/degree.ml:SYM_CONV" SYM_CONV THEN
    GEN_REWRITE_TAC "Multivariate/degree.ml:RAND_CONV" RAND_CONV [GSYM AFF_DIM_AFFINE_HULL] THEN
 ...
 (`!s:real^N->bool. AR s <=> ANR s /\ contractible s /\ ~(s = {})`,
  ...
  REWRITE_TAC[] THEN CONJ_TAC THENL
   [SUBGOAL_THEN `w:real^(N,1)finite_sum->bool = (w DIFF v) UNION (w DIFF v')`
    MP_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
    DISCH_THEN(fun th ->
        GEN_REWRITE_TAC "Multivariate/degree.ml:RAND_CONV" RAND_CONV [th] THEN
        MATCH_MP_TAC CONTINUOUS_ON_CASES_LOCAL THEN
   ...
   (`!s t u:real^N->bool.
    ...
    SUBGOAL_THEN `(s INTER t:real^N->bool) retract_of w` MP_TAC THENL
     [MATCH_MP_TAC AR_IMP_ABSOLUTE_RETRACT THEN
      ...
      ASM SET_TAC[];
      GEN_REWRITE_TAC "Multivariate/degree.ml:LAND_CONV" LAND_CONV [retract_of] THEN
      REWRITE_TAC[retraction; LEFT_IMP_EXISTS_THM] THEN
  ...
  MAP_EVERY (fun x ->
    REWRITE_TAC[FINITE_INSERT; FINITE_EMPTY] THEN ANTS_TAC THENL
     [REWRITE_TAC[SIMPLE_IMAGE; IMAGE_CLAUSES; UNIONS_2] THEN
      ...
      GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [SWAP_FORALL_THM] THEN
      GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC ANR_OPEN_IN THEN
    ...
    ASM_CASES_TAC `(x:real^(N,1)finite_sum) IN t1` THENL
     [ASM_SIMP_TAC[DROP_VEC] THEN CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
      ASM SET_TAC[]];
    REWRITE_TAC[SUBSET; FORALL_IN_IMAGE; IN_INTER; IN_ELIM_THM] THEN
    X_GEN_TAC `x:real^(N,1)finite_sum` THEN
    REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
     ...
     [REPEAT STRIP_TAC THEN
      ...
      ASM_SIMP_TAC[] THEN EXPAND_TAC "a" THEN CONJ_TAC THENL
       [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        ...
        ASM SET_TAC[]];
      GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
      REWRITE_TAC[SKOLEM_THM; LEFT_IMP_EXISTS_THM]] THEN
    MAP_EVERY X_GEN_TAC
    ...
    SUBST1_TAC THENL
     [REWRITE_TAC[GSYM SUBSET_ANTISYM_EQ] THEN CONJ_TAC THENL
       [REWRITE_TAC[UNIONS_IMAGE; IN_UNIV; SUBSET; IN_ELIM_THM] THEN
        X_GEN_TAC `x:real^N` THEN GEN_REWRITE_TAC "Multivariate/degree.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
        MATCH_MP_TAC MONO_EXISTS THEN
...
let LOCALLY_ANR = prove
 (`!s:real^N->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/degree.ml:LAND_CONV" LAND_CONV [SWAP_FORALL_THM] THEN
...
let ANR_COVERING_SPACE_EQ = prove
 (`!p:real^M->real^N s c.
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/degree.ml:SYM_CONV" SYM_CONV THEN
...
let ANR_COMPONENTWISE = prove
 (`!s:real^N->bool.
  ...
  REWRITE_TAC[COUNTABLE_ANR_COMPONENTS] THEN CONJ_TAC THENL
   [DISCH_TAC THEN GEN_REWRITE_TAC "Multivariate/degree.ml:RAND_CONV" RAND_CONV [UNIONS_COMPONENTS] THEN
  ...
  REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC HOMOTOPIC_WITH_LINEAR THEN
    ...
    SUBGOAL_THEN `dd(x - a:real^N) = &1`
     (fun th -> REWRITE_TAC[th] THEN CONV_TAC "Multivariate/degree.ml:VECTOR_ARITH" VECTOR_ARITH) THEN
...
let HOMOTOPIC_ON_COMPONENTS = prove
 (`!s t f g:real^M->real^N.
        ...
        ==> homotopic_with (\x. T) (s,t) f g`,
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/degree.ml:(RATOR_CONV o LAND_CONV o LAND_CONV)" (RATOR_CONV o LAND_CONV o LAND_CONV) [UNIONS_COMPONENTS] THEN
   ...
   [X_GEN_TAC `c:real^M->bool` THEN DISCH_TAC THEN
    ...
    THENL
      [MATCH_MP_TAC SUBSET_ANTISYM THEN CONJ_TAC THENL
        [GEN_REWRITE_TAC "Multivariate/degree.ml:LAND_CONV" LAND_CONV [UNIONS_COMPONENTS] THEN
   ...
   [REPEAT STRIP_TAC THEN
    ...
    MP_TAC(ISPECL
     ...
     THENL
      [REWRITE_TAC[GSYM SUBSET_ANTISYM_EQ] THEN CONJ_TAC THENL
        [GEN_REWRITE_TAC "Multivariate/degree.ml:LAND_CONV" LAND_CONV [UNIONS_COMPONENTS] THEN
         GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM IMAGE_ID] THEN
   ...
   [CONJ_TAC THENL
    ...
    CONJ_TAC THENL
     [ALL_TAC;
      ...
      GEN_REWRITE_TAC "Multivariate/degree.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
      ...
      MATCH_MP_TAC CONTINUOUS_ON_CASES_LOCAL THEN REPEAT CONJ_TAC THENL
       [REWRITE_TAC[CLOSED_IN_CLOSED] THEN EXISTS_TAC
        ...
        SIMP_TAC[CONTINUOUS_ON_SNDCART; LINEAR_CONTINUOUS_ON;
                 LINEAR_SNDCART];
        ASM_REWRITE_TAC[] THEN GEN_REWRITE_TAC "Multivariate/degree.ml:LAND_CONV" LAND_CONV [GSYM o_DEF] THEN
        MATCH_MP_TAC CONTINUOUS_ON_COMPOSE THEN CONJ_TAC THENL
         [MATCH_MP_TAC CONTINUOUS_ON_PASTECART THEN
  ...
  CONJ_TAC THENL
   [W(MP_TAC o PART_MATCH (lhand o rand) HOMOTOPIC_WITH o snd) THEN
    ...
    REWRITE_TAC[retraction; o_THM] THEN REPEAT CONJ_TAC THENL
     [FIRST_ASSUM(MP_TAC o GEN_REWRITE_RULE I [retraction]) THEN SET_TAC[];
      MATCH_MP_TAC CONTINUOUS_ON_COMPOSE THEN CONJ_TAC THENL
       [GEN_REWRITE_TAC "Multivariate/degree.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
        SIMP_TAC[CONTINUOUS_ON_PASTECART; CONTINUOUS_ON_CONST;
                 CONTINUOUS_ON_ID];
        FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
          CONTINUOUS_ON_SUBSET)) THEN
 ...
 (`!f s a:real^N.
  ...
  ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
   [CONJ_TAC THENL
    ...
    FIRST_ASSUM(fun th ->
     GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
    ...
    CONV_TAC "Multivariate/degree.ml:VECTOR_ARITH" VECTOR_ARITH]);;
 ...
 (`!A:real^N^N.
  ...
  ASSUME_TAC THENL
   [X_GEN_TAC `x:real^N` THEN ASM_CASES_TAC `x:real^N = vec 0` THEN
    ASM_REWRITE_TAC[DOT_RZERO] THEN CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    REWRITE_TAC[REAL_ARITH `&0 < x <=> &0 <= x /\ ~(x = &0)`] THEN
 ...
 (`!a b u v x:real^N.
  ...
  REPEAT STRIP_TAC THENL
   [MATCH_MP_TAC REAL_LE_MUL THEN CONJ_TAC THEN
    ...
    GEN_REWRITE_TAC "Multivariate/degree.ml:LAND_CONV" LAND_CONV [REAL_LE_LT] THEN STRIP_TAC THENL
...
let INTERVAL_BIJ_BIJ = prove
 (`!a b u v x:real^N.
  ...
  REWRITE_TAC[] THEN CONV_TAC "Multivariate/degree.ml:REAL_FIELD" REAL_FIELD);;
...
let FASHODA_UNIT = prove
 (`!f:real^1->real^2 g:real^1->real^2.
        ...
        ==> ?s t. s IN interval[--vec 1,vec 1] /\
                  ...
                  f(s) = g(t)`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/degree.ml:I" I [TAUT `p <=> ~ ~p`] THEN
   ...
   [REWRITE_TAC[INTERIOR_CLOSED_INTERVAL; INTERVAL_NE_EMPTY] THEN
    ...
    MATCH_MP_TAC(REWRITE_RULE[o_DEF] CONTINUOUS_ON_COMPOSE) THEN CONJ_TAC THENL
     [MATCH_MP_TAC CONTINUOUS_ON_SUB THEN CONJ_TAC THEN
      ...
      GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM o_DEF] THEN
  ...
  SUBGOAL_THEN `infnorm(x:real^2) = &1` MP_TAC THENL
   [FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV)
     [SYM th]) THEN
   ...
   [ASM_SIMP_TAC[CONTINUOUS_ON_COMPOSE; CONTINUOUS_ON_INTERVAL_BIJ] THEN
    REWRITE_TAC[IMAGE_o] THEN REPLICATE_TAC 2 (CONJ_TAC THENL
     [REWRITE_TAC[SUBSET] THEN ONCE_REWRITE_TAC[FORALL_IN_IMAGE] THEN
      ...
      CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM SET_TAC[];
      ALL_TAC]) THEN
...
let PATH_CONNECTED_COMPLEMENT_ABSOLUTE_RETRACT = prove
 (`!s:real^N->bool.
  ...
  MATCH_MP_TAC EQ_IMP THEN CONV_TAC "Multivariate/degree.ml:SYM_CONV" SYM_CONV THEN
 ...
 (`!s p:real^N a b.
  ...
  STRIP_ASSUME_TAC THENL
   [EXISTS_TAC `\x:real^N. if x IN frontier c then r x - x else vec 0` THEN
    ...
    CONJ_TAC THENL [ALL_TAC; ASM SET_TAC[]] THEN
    GEN_REWRITE_TAC "Multivariate/degree.ml:RAND_CONV" RAND_CONV [UNION_COMM] THEN
  ...
  SUBGOAL_THEN `!x:real^N. x IN frontier c ==> f x = x` (LABEL_TAC "1") THENL
   [EXPAND_TAC "f" THEN REWRITE_TAC[] THEN ASM_SIMP_TAC[IN_UNION] THEN
    REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/degree.ml:VECTOR_ARITH" VECTOR_ARITH;
    ALL_TAC] THEN
  SUBGOAL_THEN
   ...
   [MP_TAC(ISPECL [`(:real^N) DIFF h`; `q:real^N`; `l:real^N`]
    ...
    EXPAND_TAC "h" THEN CONJ_TAC THENL
     [MESON_TAC[BOUNDED_SUBSET; NOT_BOUNDED_UNIV; BOUNDED_CBALL];
      ...
      REWRITE_TAC[IN_DIFF; IN_UNIV; IN_CBALL] THEN CONV_TAC "Multivariate/degree.ml:NORM_ARITH" NORM_ARITH];
    ALL_TAC] THEN
  SUBGOAL_THEN
 ...
 (`!s c p:real^N b.
  ...
  MP_TAC THENL
   [MATCH_MP_TAC DEPENDENT_CHOICE THEN CONJ_TAC THENL
     [CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN MP_TAC(ISPECL
  ...
  REWRITE_TAC[IN_UNIV] THEN ANTS_TAC THENL
   [REPEAT CONJ_TAC THENL
     [X_GEN_TAC `x:real^1` THEN REWRITE_TAC[IN_INTERVAL_1; IN_DELETE] THEN
      ...
      REWRITE_TAC[] THEN ANTS_TAC THENL
       [CONJ_TAC THENL
         [EXISTS_TAC `0` THEN CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
          ASM_REWRITE_TAC[];
          ...
          MATCH_MP_TAC REAL_LT_INV2 THEN REWRITE_TAC[REAL_LT_POW2] THEN
          MATCH_MP_TAC REAL_POW_MONO_LT THEN CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
          ASM_REWRITE_TAC[]];
   ...
   [EXPAND_TAC "g" THEN REWRITE_TAC[pathstart; IN_DELETE] THEN
    ...
    ASM_CASES_TAC `x:real^1 = vec 0` THEN ASM_SIMP_TAC[] THENL
     [REWRITE_TAC[LIM_WITHIN_LE] THEN X_GEN_TAC `e:real` THEN DISCH_TAC THEN
      ...
      ASM_REWRITE_TAC[REAL_POW_INV] THEN CONV_TAC "Multivariate/degree.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    ...
    ANTS_TAC THENL
     [FIRST_X_ASSUM(MP_TAC o SPEC `vec 1:real^1`) THEN
      ...
      GEN_REWRITE_TAC "Multivariate/degree.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM pathfinish] THEN
       ...
       [ONCE_REWRITE_TAC[PATH_COMPONENT_SYM_EQ] THEN
        ...
        ASM_REWRITE_TAC[SUBSET_INTER] THEN REPEAT CONJ_TAC THENL
         [ASM_MESON_TAC[IN_COMPONENTS_CONNECTED];
          ...
          CONV_TAC "Multivariate/degree.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV]]]) in
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN `open(c:real^N->bool)` ASSUME_TAC THENL
   [REPEAT(FIRST_X_ASSUM((MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ_ALT]
        OPEN_COMPONENTS)))) THEN
