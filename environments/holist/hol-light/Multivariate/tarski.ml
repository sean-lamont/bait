   ...
   [ASM_CASES_TAC `d = &1 /\ e = &0` THENL [ALL_TAC; ASM_REAL_ARITH_TAC] THEN
    ...
    CONJ_TAC THENL
     [EXISTS_TAC `e / (&1 - d + e)` THEN
      ...
      UNDISCH_TAC `&0 < &1 - d + e` THEN CONV_TAC "Multivariate/tarski.ml:REAL_FIELD" REAL_FIELD;
...
let TARSKI_AXIOM_8_EUCLIDEAN = prove
 (`?a b c:real^2. ~between b (a,c) /\ ~between c (b,a) /\ ~between a (c,b)`,
  ...
  CONV_TAC "Multivariate/tarski.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let TARSKI_AXIOM_9_EUCLIDEAN = prove
 (`!p q a b c:real^2.
  ...
  CONV_TAC "Multivariate/tarski.ml:REAL_FIELD" REAL_FIELD);;
