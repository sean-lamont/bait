 ...
 (`!f:real^M->real^N s t.
   ...
   (`!f:real^N->real^N s t.
    ...
    ASSUME_TAC THENL
     [EXPAND_TAC "g" THEN MATCH_MP_TAC DIFFERENTIABLE_ON_MUL THEN
      ...
      GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [GSYM o_DEF] THEN
      MATCH_MP_TAC DIFFERENTIABLE_ON_COMPOSE THEN CONJ_TAC THENL
       [MATCH_MP_TAC DIFFERENTIABLE_ON_MUL THEN
        ...
        REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
  ...
  let lemma2 = prove
   (`!f:real^N->real^N s t.
    ...
    CONV_TAC "Multivariate/moretop.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    ...
    ASSUME_TAC THENL
     [X_GEN_TAC `x:real^N` THEN STRIP_TAC THEN
      ...
      CONV_TAC "Multivariate/moretop.ml:NORM_ARITH" NORM_ARITH;
      ALL_TAC] THEN
     ...
     [EXPAND_TAC "h" THEN MATCH_MP_TAC DIFFERENTIABLE_ON_MUL THEN
      ...
      MATCH_MP_TAC DIFFERENTIABLE_ON_COMPOSE THEN CONJ_TAC THENL
       [MATCH_MP_TAC DIFFERENTIABLE_ON_COMPOSE THEN
        ...
        GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
        MATCH_MP_TAC REAL_DIFFERENTIABLE_INV_ATREAL THEN
       ...
       [MATCH_MP_TAC HOMOTOPIC_WITH_LINEAR THEN
        ...
        CONJ_TAC THENL
         [REWRITE_TAC[SEGMENT_CONVEX_HULL] THEN MATCH_MP_TAC HULL_MINIMAL THEN
          ...
          REWRITE_TAC[IN_SPHERE_0] THEN CONV_TAC "Multivariate/moretop.ml:NORM_ARITH" NORM_ARITH];
        DISCH_THEN(MP_TAC o
   ...
   (`!s:real^M->bool u:real^N->bool.
    ...
    REPEAT GEN_TAC THEN ASM_CASES_TAC `s:real^M->bool = {}` THENL
     [STRIP_TAC THEN EXISTS_TAC `{vec 0:real^N}` THEN
      ...
      CONV_TAC "Multivariate/moretop.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
 ...
 (`!s t:real^N->bool.
  ...
  REWRITE_TAC[SET_RULE `s INTER u DIFF s INTER t = s INTER u DIFF t`] THENL
   [MATCH_MP_TAC PATH_CONNECTED_CONVEX_DIFF_COUNTABLE THEN
    ...
    ASM_REWRITE_TAC[]] THEN
 (ASM (CONV_TAC "Multivariate/moretop.ml:o" o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 5)
   ...
   (`!f:real^M->real^N m n t.
    ...
    MATCH_MP_TAC num_INDUCTION THEN CONJ_TAC THENL
     [REWRITE_TAC[INT_ARITH `d < &0 <=> (--(&1) <= d ==> d:int = --(&1))`] THEN
      ...
      THENL
       [GEN_REWRITE_TAC "Multivariate/moretop.ml:I" I [EXTENSION] THEN X_GEN_TAC `d:real^M->bool` THEN
      ...
      THENL
       [SUBGOAL_THEN `~(UNIONS n = (:real^M))` MP_TAC THENL
         [MATCH_MP_TAC(MESON[NOT_BOUNDED_UNIV]
          ...
          GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [EXTENSION]] THEN
  ...
  let lemma2 = prove
   (`!f:real^M->real^N s t k p u.
    ...
    SUBGOAL_THEN `~(((u:real^M->bool) DIFF s) INTER k = {})` MP_TAC THENL
     [ASM SET_TAC[]; ALL_TAC] THEN
    GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o RAND_CONV o LAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV o LAND_CONV)
    ...
    MP_TAC THENL
     [X_GEN_TAC `c:real^M->bool` THEN STRIP_TAC THEN
      ...
      ASM SET_TAC[];
      ALL_TAC] THEN
    GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
     ...
     [REPEAT CONJ_TAC THENL
       [MATCH_MP_TAC lemma0 THEN EXISTS_TAC `u:real^M->bool` THEN
        ...
        REPEAT CONJ_TAC THENL
         [MATCH_MP_TAC CLOSED_IN_UNIONS THEN REWRITE_TAC[FORALL_IN_GSPEC] THEN
          ...
          GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o LAND_CONV o TOP_DEPTH_CONV)
           [INTER_UNIONS] THEN
          REWRITE_TAC[SET_RULE
      ...
      REWRITE_TAC[] THEN CONJ_TAC THENL
       [REWRITE_TAC[SET_RULE
        ...
        GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [UNIONS_COMPONENTS] THEN
        REWRITE_TAC[UNIONS_GSPEC] THEN ASM SET_TAC[];
      ...
      SUBGOAL_THEN
        `x IN ((u:real^M->bool) DIFF s)` MP_TAC THENL
          [ASM SET_TAC[]; ALL_TAC] THEN
      GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [UNIONS_COMPONENTS] THEN
      REWRITE_TAC[IN_UNIONS] THEN
      DISCH_THEN(X_CHOOSE_THEN `c:real^M->bool` STRIP_ASSUME_TAC) THEN
  ...
  ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
   [REPEAT CONJ_TAC THENL
     [MATCH_MP_TAC COMPACT_CONTINUOUS_IMAGE THEN CONJ_TAC THENL
      ...
      ASM_REWRITE_TAC[] THEN
      GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [GSYM MEMBER_NOT_EMPTY] THEN
      ...
      SUBGOAL_THEN `t' IN interval[vec 0:real^1,vec 1]` ASSUME_TAC THENL
       [EXPAND_TAC "t'" THEN SIMP_TAC[IN_INTERVAL_1; LIFT_DROP; DROP_VEC] THEN
        ASM_REAL_ARITH_TAC;
        ALL_TAC] THEN
      GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [SUBSET] THEN
       ...
       [MATCH_MP_TAC CONNECTED_CONTINUOUS_IMAGE THEN
        ...
        CONJ_TAC THENL
         [TRANS_TAC SUBSET_TRANS
          ...
          FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/moretop.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
           [SYM th]) THEN
 ...
 (`!s a. compact s /\ ~(a IN s)
  ...
  EQ_TAC THENL
   [ONCE_REWRITE_TAC[GSYM CONTRAPOS_THM] THEN REWRITE_TAC[] THEN
    ...
    ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
     [GEN_REWRITE_TAC "Multivariate/moretop.ml:RAND_CONV" RAND_CONV [IN] THEN
...
let HOMOTOPIC_BORSUK_MAPS_IN_BOUNDED_COMPONENT = prove
 (`!s a b.
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/moretop.ml:I" I [GSYM IN] THEN
  MP_TAC(ISPECL
 ...
 (`!s t:real^N->bool a r.
  ...
  REPEAT STRIP_TAC THEN ASM_CASES_TAC `FINITE(sphere(a:real^N,r))` THENL
   [REWRITE_TAC[CONNECTED_EQ_CARD_COMPONENTS] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [CARD_EQ_SYM] THEN
   ...
   [MATCH_MP_TAC HOMEOMORPHIC_IMP_HOMOTOPY_EQUIVALENT THEN
    ...
    CONJ_TAC THENL
     [SIMP_TAC[RELATIVE_FRONTIER_CBALL; REAL_OF_NUM_EQ; ARITH_EQ] THEN
      ...
      REWRITE_TAC[AFF_DIM_CBALL; REAL_LT_01] THEN CONV_TAC "Multivariate/moretop.ml:SYM_CONV" SYM_CONV THEN
 ...
 (`!g s:real^N->bool.
  ...
  ASM_CASES_TAC `3 <= dimindex(:N)` THENL
   [REPEAT STRIP_TAC THEN MATCH_MP_TAC
    ...
    GEN_REWRITE_TAC "Multivariate/moretop.ml:RAND_CONV" RAND_CONV [GSYM COMPL_COMPL] THEN
      ...
      REPEAT CONJ_TAC THENL
       [REPEAT(MATCH_MP_TAC CONTINUOUS_ON_COMPOSE THEN CONJ_TAC) THEN
        ...
        MATCH_MP_TAC EQ_IMP THENL
         [CONV_TAC "Multivariate/moretop.ml:SYM_CONV" SYM_CONV;
  ...
  ASM_REWRITE_TAC[HOMEOMORPHIC_EMPTY] THENL
   [FIRST_ASSUM(MP_TAC o MATCH_MP HOMEOMORPHIC_COMPACTNESS) THEN
    ...
    FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [homeomorphic]) THENL
     [ALL_TAC; GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [SWAP_EXISTS_THM]] THEN
    MATCH_MP_TAC MONO_EXISTS THEN GEN_TAC THEN
    ...
    SUBST1_TAC THENL
     [MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
      ...
      GEN_REWRITE_TAC "Multivariate/moretop.ml:I" I [TAUT `(p <=> q) <=> (~p <=> ~q)`] THEN
 ...
 (`!f g s:real^N->bool.
  ...
  DISCH_THEN(SUBST1_TAC o SYM) THEN CONJ_TAC THENL
   [REWRITE_TAC[GSYM COMPLETE_EQ_CLOSED; complete] THEN
    ...
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/moretop.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
 ...
 (`!s:real^N->bool t:real^M->bool.
  ...
  EXISTS_TAC `IMAGE (h:real^M->real^N) t` THEN CONJ_TAC THENL
   [MATCH_MP_TAC HOMEOMORPHIC_INTERIORS_SAME_DIMENSION THEN REWRITE_TAC[] THEN
    ...
    MATCH_MP_TAC EQ_IMP THEN CONV_TAC "Multivariate/moretop.ml:SYM_CONV" SYM_CONV THEN
...
let HOMEOMORPHIC_AFFINE_SETS_EQ = prove
 (`!s:real^M->bool t:real^N->bool.
  ...
  POP_ASSUM MP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/moretop.ml:(funpow 3 RAND_CONV)" (funpow 3 RAND_CONV) [EQ_SYM_EQ] THEN
 ...
 (`!a:real^M b:real^N r s.
  ...
  ASM_REWRITE_TAC[] THEN EQ_TAC THENL
   [REWRITE_TAC[homeomorphic; HOMEOMORPHISM; LEFT_IMP_EXISTS_THM] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [GSYM CONTRAPOS_THM] THEN
...
let SIMPLY_CONNECTED_PUNCTURED_UNIVERSE_EQ = prove
 (`!a. simply_connected((:real^N) DELETE a) <=> 3 <= dimindex(:N)`,
  ...
  CONV_TAC "Multivariate/moretop.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ONCE_REWRITE_TAC[EQ_SYM_EQ] THEN
  MATCH_MP_TAC HOMOTOPY_EQUIVALENT_SIMPLE_CONNECTEDNESS THEN
  MP_TAC(ISPECL [`cball(a:real^N,&1)`; `a:real^N`]
        HOMOTOPY_EQUIVALENT_RELATIVE_FRONTIER_PUNCTURED_AFFINE_HULL) THEN
  REWRITE_TAC[CONVEX_CBALL; BOUNDED_CBALL; RELATIVE_INTERIOR_CBALL;
  ...
  ASSUME_TAC THENL
   [REWRITE_TAC[IN_CBALL] THEN REPEAT STRIP_TAC THEN
    ...
    MATCH_MP_TAC REAL_LET_TRANS THEN EXISTS_TAC `&2` THEN CONJ_TAC THENL
     [REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Multivariate/moretop.ml:NORM_ARITH" NORM_ARITH;
...
let CONTINUOUS_SQRT_ON_CONTRACTIBLE = prove
 (`!f:real^N->complex s.
  ...
  CONV_TAC "Multivariate/moretop.ml:COMPLEX_RING" COMPLEX_RING);;
     ...
     [ASM_REWRITE_TAC[COMPLEX_SUB_REFL; complex_div; COMPLEX_MUL_LZERO];
      ...
      SUBGOAL_THEN `~(cexp(g(w:complex)) = cexp(g z))` MP_TAC THENL
       [UNDISCH_TAC `~((g:complex->complex) w = g z)` THEN
        ...
        CONV_TAC "Multivariate/moretop.ml:COMPLEX_FIELD" COMPLEX_FIELD]]]));;
...
let CONTRACTIBLE_IMP_HOLOMORPHIC_ACS_BOUNDED = prove
 (`!f s a.
  ...
  CONV_TAC "Multivariate/moretop.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN REWRITE_TAC[CCOS_EQ] THEN
  ASM_MESON_TAC[]);;
 ...
 (`!f:real^N->complex s t.
  ...
  MP_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [CONTINUOUS_ON_EQ_CONTINUOUS_WITHIN] THEN
    DISCH_THEN(MP_TAC o SPEC `x:real^N`) THEN
    ASM_REWRITE_TAC[continuous_within] THEN
    DISCH_THEN(MP_TAC o SPEC `&1 / &5`) THEN
    CONV_TAC "Multivariate/moretop.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [dist] THEN
    ...
    ASM_REWRITE_TAC[REAL_LT_MIN; REAL_HALF] THEN CONJ_TAC THENL
     [ASM_MESON_TAC[NORM_SUB];
      ...
      ASM_REAL_ARITH_TAC];
    GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
    REWRITE_TAC[SKOLEM_THM; LEFT_IMP_EXISTS_THM] THEN
   ...
   [FIRST_X_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    ANTS_TAC THENL
     [MATCH_MP_TAC CONTINUOUS_ON_COMPLEX_DIV THEN
      ...
      ASM_REWRITE_TAC[] THEN GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [GSYM o_DEF] THEN
      MATCH_MP_TAC CONTINUOUS_ON_COMPOSE THEN
  ...
  ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
   [CONJ_TAC THENL
     [MATCH_MP_TAC CONTINUOUS_ON_SUB THEN
      ...
      FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ_ALT]
       REAL_LET_TRANS)) THEN
      GEN_REWRITE_TAC "Multivariate/moretop.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [COMPLEX_RING
   ...
   (CONJ_TAC THENL
    ...
    ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
     [(MATCH_MP_TAC CONTINUOUS_ON_ADD ORELSE
      ...
      ALL_TAC] THEN
  GEN_REWRITE_TAC "Multivariate/moretop.ml:I" I [TAUT `p ==> q ==> F <=> ~(p /\ q)`] THEN
  DISCH_THEN(MP_TAC o MATCH_MP (SET_RULE
 ...
 (`!s t:real^N->bool.
  ...
  MATCH_MP_TAC SUBSET_ANTISYM THEN CONJ_TAC THENL
   [REWRITE_TAC[SUBSET] THEN X_GEN_TAC `x:real^N` THEN DISCH_TAC THEN
    ...
    GEN_REWRITE_TAC "Multivariate/moretop.ml:RAND_CONV" RAND_CONV [GSYM FRONTIER_COMPLEMENT] THEN
...
let SEPARATION_BY_UNION_CLOSED_POINTWISE = prove
 (`!s t a b. closed s /\ closed t /\ DISJOINT s t /\
  ...
  REWRITE_TAC[IN_DIFF; IN_UNIV] THEN REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/moretop.ml:I" I [TAUT `p <=> ~ ~ p`] THEN DISCH_THEN(MP_TAC o MATCH_MP
     ...
     [MATCH_MP_TAC SUBSET_TRANS THEN
      ...
      ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
       [REWRITE_TAC[GSYM UNIONS_UNION] THEN
        GEN_REWRITE_TAC "Multivariate/moretop.ml:RAND_CONV" RAND_CONV [UNIONS_COMPONENTS] THEN
  ...
  ANTS_TAC THENL
   [REPEAT CONJ_TAC THENL
     [MATCH_MP_TAC OPEN_UNION THEN ASM_REWRITE_TAC[] THEN
      ...
      REWRITE_TAC[FORALL_IN_GSPEC; SUBSET_UNION];
      GEN_REWRITE_TAC "Multivariate/moretop.ml:I" I [EXTENSION] THEN X_GEN_TAC `x:real^N` THEN
 ...
 (`!s:real^N->bool.
   ...
   [SUBGOAL_THEN
    ...
    STRIP_ASSUME_TAC THENL
     [ASM_CASES_TAC `s:real^N->bool = {}` THEN
      ...
      CONV_TAC "Multivariate/moretop.ml:VECTOR_ARITH" VECTOR_ARITH];
  ...
  STRIP_ASSUME_TAC THENL
   [FIRST_ASSUM(MP_TAC o GEN_REWRITE_RULE I [open_def]) THEN
    ...
    GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [GSYM CONTRAPOS_THM] THEN
...
let JORDAN_CURVE_THEOREM = prove
 (`!c:real^1->real^2.
  ...
  STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
  REPLICATE_TAC 5 (GEN_REWRITE_TAC "Multivariate/moretop.ml:I" I [CONJ_ASSOC]) THEN
  MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
...
let SPLIT_INSIDE_SIMPLE_CLOSED_CURVE = prove
 (`!c1 c2 c a b:real^2.
  ...
  STRIP_ASSUME_TAC THENL
   [CONJ_TAC THENL
     [ALL_TAC; GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [UNION_COMM]] THEN
   ...
   [MATCH_MP_TAC(TAUT `(~p ==> F) ==> p`) THEN DISCH_TAC THEN
    ...
    MP_TAC THENL
     [AP_TERM_TAC THEN MATCH_MP_TAC SUBSET_ANTISYM THEN ASM_REWRITE_TAC[] THEN
      GEN_REWRITE_TAC "Multivariate/moretop.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [UNION_COMM] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [CONJ_TAC THEN REWRITE_TAC[INSIDE_OUTSIDE] THEN
    REWRITE_TAC[SET_RULE `UNIV DIFF t SUBSET UNIV DIFF s <=> s SUBSET t`] THENL
     [ALL_TAC; GEN_REWRITE_TAC "Multivariate/moretop.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [UNION_COMM]] THEN
    MATCH_MP_TAC(SET_RULE
   ...
   [REWRITE_TAC[SUBSET] THEN CONJ_TAC THEN
    X_GEN_TAC `x:real^2` THEN DISCH_TAC THENL
     [SUBGOAL_THEN `?z:real^2. z IN path_image c1 /\
      ...
      GEN_REWRITE_TAC "Multivariate/moretop.ml:LAND_CONV" LAND_CONV [EXTENSION] THEN
