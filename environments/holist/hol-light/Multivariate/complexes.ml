...
let COMPLEX_EQ_0 = prove
 (`z = Cx(&0) <=> Re(z) pow 2 + Im(z) pow 2 = &0`,
  ...
  EQ_TAC THEN SIMP_TAC[] THEN CONV_TAC "Multivariate/complexes.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let SIMPLE_COMPLEX_ARITH_TAC =
  ...
  CONV_TAC "Multivariate/complexes.ml:REAL_FIELD" REAL_FIELD;;
...
let COMPLEX_POW_II_2 = prove
 (`ii pow 2 = --Cx(&1)`,
  ...
  CONV_TAC "Multivariate/complexes.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;

let COMPLEX_POW_EQ_0 = prove
 (`!x n. (x pow n = Cx(&0)) <=> (x = Cx(&0)) /\ ~(n = 0)`,
  GEN_TAC THEN INDUCT_TAC THEN
  ASM_REWRITE_TAC[NOT_SUC; complex_pow; COMPLEX_ENTIRE] THENL
   [SIMPLE_COMPLEX_ARITH_TAC; CONV_TAC "Multivariate/complexes.ml:TAUT" TAUT]);;
...
let CX_INV = prove
 (`!x. Cx(inv x) = inv(Cx x)`,
  ...
  CONV_TAC "Multivariate/complexes.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  POP_ASSUM MP_TAC THEN CONV_TAC "Multivariate/complexes.ml:REAL_FIELD" REAL_FIELD);;
...
let COMPLEX_NORM_II = prove
 (`norm ii = &1`,
  ...
  CONV_TAC "Multivariate/complexes.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[SQRT_1]);;
...
let COMPLEX_RING,complex_ideal_cofactors =
  ...
  and COMPLEX_RABINOWITSCH = prove
   (`!x y:complex. ~(x = y) <=> ?z. (x - y) * z = Cx(&1)`,
    REPEAT GEN_TAC THEN
    GEN_REWRITE_TAC "Multivariate/complexes.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM COMPLEX_SUB_0] THEN
...
let COMPLEX_INV_MUL = prove
 (`!w z. inv(w * z) = inv(w) * inv(z)`,
  ...
  REWRITE_TAC[GSYM REAL_SOS_EQ_0] THEN CONV_TAC "Multivariate/complexes.ml:REAL_FIELD" REAL_FIELD);;
...
let COMPLEX_INV_INV = prove
 (`!x:complex. inv(inv x) = x`,
  ...
  CONV_TAC "Multivariate/complexes.ml:COMPLEX_RING" COMPLEX_RING);;
...
let COMPLEX_DIV_1 = prove
 (`!z. z / Cx(&1) = z`,
  CONV_TAC "Multivariate/complexes.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
...
let COMPLEX_INV_NEG = prove
 (`!x:complex. inv(--x) = --(inv x)`,
  ...
  POP_ASSUM MP_TAC THEN CONV_TAC "Multivariate/complexes.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
...
let COMPLEX_NORM_INV = prove
 (`!z. norm(inv z) = inv(norm z)`,
  ...
  CONV_TAC "Multivariate/complexes.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_MUL_RINV_UNIQ THEN
...
let COMPLEX_POW_EQ_1 = prove
 (`!z n. z pow n = Cx(&1) ==> norm(z) = &1 \/ n = 0`,
  ...
  SIMP_TAC[REAL_ABS_NORM] THEN CONV_TAC "Multivariate/complexes.ml:TAUT" TAUT);;
...
let CNJ_EQ_CX = prove
 (`!x z. cnj z = Cx x <=> z = Cx x`,
  ...
  CONV_TAC "Multivariate/complexes.ml:REAL_RING" REAL_RING);;
...
let COMPLEX_DIV_CNJ = prove
 (`!a b. a / b = (a * cnj b) / Cx(norm b) pow 2`,
  ...
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/complexes.ml:LAND_CONV" LAND_CONV [COMPLEX_INV_CNJ] THEN
 ...
 (`!z. csqrt(z) pow 2 = z`,
  ...
  REWRITE_TAC[COMPLEX_EQ; RE; IM] THEN CONJ_TAC THENL
   [ASM_SIMP_TAC[REAL_POW_DIV; REAL_POW2_ABS;
    ...
    REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN CONV_TAC "Multivariate/complexes.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let CSQRT_EQ_0 = prove
 (`!z. csqrt z = Cx(&0) <=> z = Cx(&0)`,
  GEN_TAC THEN MP_TAC (SPEC `z:complex` CSQRT) THEN CONV_TAC "Multivariate/complexes.ml:COMPLEX_RING" COMPLEX_RING);;
...
let LINEAR_COMPLEX_MUL = prove
 (`!c. linear (\x. c * x)`,
   REWRITE_TAC[linear; COMPLEX_CMUL] THEN CONV_TAC "Multivariate/complexes.ml:COMPLEX_RING" COMPLEX_RING);;

let BILINEAR_COMPLEX_MUL = prove
 (`bilinear( * )`,
  REWRITE_TAC[bilinear; linear; COMPLEX_CMUL] THEN  CONV_TAC "Multivariate/complexes.ml:COMPLEX_RING" COMPLEX_RING);;
...
let LINEAR_COMPLEX_LMUL = prove
 (`!f:real^N->complex c. linear f ==> linear (\x. c * f x)`,
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/complexes.ml:COMPLEX_RING" COMPLEX_RING);;
 ...
 (`!f:complex->complex.
  ...
  GEN_TAC THEN EQ_TAC THENL
   [STRIP_TAC THEN ASM_REWRITE_TAC[LINEAR_COMPLEX_MUL] THEN
    ...
    FIRST_ASSUM(fun th ->
      GEN_REWRITE_TAC "Multivariate/complexes.ml:LAND_CONV" LAND_CONV [MATCH_MP MATRIX_VECTOR_MUL th]) THEN
...
let COMPLEX_LINEAR_ALT = prove
 (`!f:complex->complex.
  ...
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/complexes.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
 ...
 (`!f:complex->complex.
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[COMPLEX_LINEAR] THEN
    ...
    REWRITE_TAC[ORTHOGONAL_MATRIX_2; DET_2] THEN CONV_TAC "Multivariate/complexes.ml:REAL_RING" REAL_RING;
    REWRITE_TAC[RIGHT_AND_EXISTS_THM] THEN
    AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/complexes.ml:I" I [FUN_EQ_THM] THEN
...
let IN_SEGMENT_CX = prove
 (`!a b x. Cx(x) IN segment[Cx(a),Cx(b)] <=>
  ...
  (CONJ_TAC THENL
    [ALL_TAC; UNDISCH_TAC `~(a:real = b)` THEN CONV_TAC "Multivariate/complexes.ml:REAL_FIELD" REAL_FIELD]) THEN
...
let REAL_NORM = prove
 (`!z. real z ==> norm(z) = abs(Re z)`,
  SIMP_TAC[real; complex_norm] THEN CONV_TAC "Multivariate/complexes.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let COMPLEX_SUB_POLYFUN = prove
 (`!a x y n.
  ...
  GEN_REWRITE_TAC "Multivariate/complexes.ml:LAND_CONV" LAND_CONV [MATCH_MP VSUM_CLAUSES_LEFT (SPEC_ALL LE_0)] THEN
...
let COMPLEX_POLYFUN_EXTREMAL_LEMMA = prove
 (`!c n e. &0 < e
  ...
  GEN_REWRITE_TAC "Multivariate/complexes.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [REAL_POW_ADD] THEN
...
let COMPLEX_POLYFUN_ROOTBOUND = prove
 (`!n c. ~(!i. i IN 0..n ==> c(i) = Cx(&0))
  ...
  FIRST_X_ASSUM(MP_TAC o SPEC `&1`) THEN CONV_TAC "Multivariate/complexes.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let COMPLEX_POLYFUN_EQ_0 = prove
 (`!n c. (!z. vsum(0..n) (\i. c i * z pow i) = Cx(&0)) <=>
  ...
  REPEAT GEN_TAC THEN EQ_TAC THEN DISCH_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/complexes.ml:I" I [TAUT `p <=> ~ ~p`] THEN DISCH_THEN(MP_TAC o MATCH_MP
 ...
 (`!n c k. (!z. vsum(0..n) (\i. c i * z pow i) = k) <=>
  ...
  CONJ_TAC THENL
   [SIMP_TAC[VSUM_CLAUSES_LEFT; LE_0; complex_pow; COMPLEX_MUL_RID] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/complexes.ml:LAND_CONV" LAND_CONV [MESON[]
...
let CPRODUCT_REFLECT = prove
 (`!x m n. cproduct(m..n) x =
  ...
  GEN_REWRITE_TAC "Multivariate/complexes.ml:LAND_CONV" LAND_CONV
...
let CX_PRODUCT = prove
 (`!f s. FINITE s ==> Cx(product s f) = cproduct s (\i. Cx(f i))`,
  GEN_TAC THEN CONV_TAC "Multivariate/complexes.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
