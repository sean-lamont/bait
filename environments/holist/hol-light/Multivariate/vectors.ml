...
let VECTOR_ARITH_TAC =
  ...
  (MATCH_MP_TAC SUM_EQ_NUMSEG ORELSE MATCH_MP_TAC SUM_EQ_0_NUMSEG ORELSE
   GEN_REWRITE_TAC "Multivariate/vectors.ml:ONCE_DEPTH_CONV" ONCE_DEPTH_CONV [CART_EQ]) THEN
...
let TRANSLATION_SUBSET_GALOIS_RIGHT = prove
 (`!s t a:real^N.
  ...
  REWRITE_TAC[] THEN CONV_TAC "Multivariate/vectors.ml:VECTOR_ARITH" VECTOR_ARITH);;
 ...
 (`!x e. &0 < e
  ...
  REPEAT STRIP_TAC THEN ASM_CASES_TAC `x = &0` THENL
   [ASM_REWRITE_TAC[REAL_MUL_LZERO; REAL_SUB_RZERO] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_INV] THEN
...
let SQRT_WORKS_GEN = prove
 (`!x. real_sgn(sqrt x) = real_sgn x /\ sqrt(x) pow 2 = abs x`,
  GEN_TAC THEN REWRITE_TAC[sqrt] THEN CONV_TAC "Multivariate/vectors.ml:SELECT_CONV" SELECT_CONV THEN
  SUBGOAL_THEN `!x. &0 < x ==> ?y. &0 < y /\ y pow 2 = x` ASSUME_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    CONV_TAC "Multivariate/vectors.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let NORM_EQ_1 = prove
 (`!x. norm(x) = &1 <=> x dot x = &1`,
  GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM SQRT_1] THEN
 ...
 (`!x:real^N.
  ...
  let lemma = prove
   (`!x n. &n * sum(1..n) (\i. x i pow 2) - (sum(1..n) x) pow 2 =
    ...
    GEN_TAC THEN CONV_TAC "Multivariate/vectors.ml:(BINDER_CONV SYM_CONV)" (BINDER_CONV SYM_CONV) THEN INDUCT_TAC THEN
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [GSYM REAL_POW2_ABS] THEN
...
let NORM_LT_SQUARE_ALT = prove
 (`!x:real^N. norm(x) < a <=> &0 <= a /\ x dot x < a pow 2`,
  ...
  REPEAT GEN_TAC THEN ASM_CASES_TAC `a = &0` THENL
   [ASM_REWRITE_TAC[real_ge] THEN CONV_TAC "Multivariate/vectors.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
 ...
 (`!f:real->real.
  ...
  SUBGOAL_THEN `!x y. x <= y ==> (f:real->real) x <= f y` ASSUME_TAC THENL
   [REPEAT GEN_TAC THEN ONCE_REWRITE_TAC[GSYM REAL_SUB_LE] THEN
    FIRST_X_ASSUM(fun th ->
      GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM th]) THEN
    SPEC_TAC(`y - x:real`,`z:real`) THEN GEN_TAC THEN
    GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [GSYM SQRT_POW2] THEN
  ...
  SUBGOAL_THEN `!x. rational x ==> f x = x` ASSUME_TAC THENL
   [REWRITE_TAC[rational; LEFT_IMP_EXISTS_THM] THEN
    ...
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [GSYM th]) THEN
...
let DIST_RESCALE = prove
 (`!a x y:real^N. norm(x) = norm(y) ==> dist(a % x,y) = dist(x,a % y)`,
  ...
  CONV_TAC "Multivariate/vectors.ml:REAL_RING" REAL_RING);;
...
let VSUM_SUPPORT = prove
 (`!f:A->real^N s. vsum {x | x IN s /\ ~(f x = vec 0)} f = vsum s f`,
  REPEAT GEN_TAC THEN CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC VSUM_SUPERSET THEN
  SET_TAC[]);;
...
let VSUM_DELETE_CASES = prove
 (`!x f s.
  ...
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV)
...
let VSUM_NORM_ALLSUBSETS_BOUND = prove
 (`!f:A->real^N p e.
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV)
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [GSYM REAL_ABS_NEG] THEN
...
let VSUM_BIJECTION = prove
 (`!f:A->real^N p s:A->bool.
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN
...
let VSUM_REFLECT = prove
 (`!x m n. vsum(m..n) x =
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [MATCH_MP ITERATE_REFLECT MONOIDAL_VECTOR_ADD] THEN
  REWRITE_TAC[NEUTRAL_VECTOR_ADD]);;
...
let NORM_BASIS = prove
 (`!k. 1 <= k /\ k <= dimindex(:N)
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM SQRT_1] THEN AP_TERM_TAC THEN
...
let BASIS_EXPANSION = prove
 (`!x:real^N. vsum(1..dimindex(:N)) (\i. x$i % basis i) = x`,
  ...
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [EQ_SYM_EQ] THEN
...
let BASIS_EXPANSION_UNIQUE = prove
 (`!f x:real^N. (vsum(1..dimindex(:N)) (\i. f(i) % basis i) = x) <=>
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o BINDER_CONV o RAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o BINDER_CONV o RAND_CONV o LAND_CONV o ONCE_DEPTH_CONV) [EQ_SYM_EQ] THEN
...
let NORM_VSUM_PYTHAGOREAN = prove
 (`!k u:A->real^N.
  ...
  CONV_TAC "Multivariate/vectors.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[PAIRWISE_INSERT] THEN
...
let LINEAR_INJECTIVE_0 = prove
 (`!f. linear f
       ==> ((!x y. (f(x) = f(y)) ==> (x = y)) <=>
            (!x. (f(x) = vec 0) ==> (x = vec 0)))`,
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [GSYM VECTOR_SUB_EQ] THEN
...
let LINEAR_BOUNDED = prove
 (`!f:real^M->real^N. linear f ==> ?B. !x. norm(f x) <= B * norm(x)`,
  ...
  GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o funpow 2 RAND_CONV)" (LAND_CONV o funpow 2 RAND_CONV) [GSYM BASIS_EXPANSION] THEN
...
let BILINEAR_BOUNDED = prove
 (`!h:real^M->real^N->real^P.
  ...
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o RAND_CONV o BINOP_CONV)" (LAND_CONV o RAND_CONV o BINOP_CONV) [GSYM BASIS_EXPANSION] THEN
...
let ADJOINT_WORKS = prove
 (`!f:real^M->real^N. linear f ==> !x y. f(x) dot y = x dot (adjoint f)(y)`,
  GEN_TAC THEN DISCH_TAC THEN SIMP_TAC[adjoint] THEN CONV_TAC "Multivariate/vectors.ml:SELECT_CONV" SELECT_CONV THEN
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(funpow 2 LAND_CONV o RAND_CONV)" (funpow 2 LAND_CONV o RAND_CONV) [GSYM BASIS_EXPANSION] THEN
  ASM_SIMP_TAC[LINEAR_VSUM; FINITE_NUMSEG] THEN
  SIMP_TAC[dot; LAMBDA_BETA; VSUM_COMPONENT; GSYM SUM_LMUL; GSYM SUM_RMUL] THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [SUM_SWAP_NUMSEG] THEN
...
let ADJOINT_COMPOSE = prove
 (`!f g:real^N->real^N.
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC ADJOINT_UNIQUE THEN
...
let ORTHOGONAL_PROJECTION_EQ_SELF_ADJOINT_IDEMPOTENT = prove
 (`!f:real^N->real^N.
  ...
  EQ_TAC THENL
   [REPEAT STRIP_TAC THENL
     [CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC ADJOINT_UNIQUE THEN
...
let LIPSCHITZ_ON_COMPONENTWISE = prove
 (`!f:real^M->real^N s.
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [RIGHT_IMP_EXISTS_THM] THEN
...
let MATRIX_MUL_ASSOC = prove
 (`!A:real^N^M B:real^P^N C:real^Q^P. A ** B ** C = (A ** B) ** C`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [SUM_SWAP_NUMSEG] THEN REWRITE_TAC[]);;
...
let DOT_LMUL_MATRIX = prove
 (`!A:real^N^M x:real^M y:real^N. (x ** A) dot y = x dot (A ** y)`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [SUM_SWAP_NUMSEG] THEN REWRITE_TAC[REAL_MUL_AC]);;
...
let CARD_ROWS_LE = prove
 (`!A:real^M^N. CARD(rows A) <= dimindex(:N)`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM CARD_NUMSEG_1] THEN
...
let MATRIX_VECTOR_MUL_LNEG = prove
 (`!A:real^M^N x:real^M. --A ** x = --(A ** x)`,
  ...
  CONV_TAC "Multivariate/vectors.ml:VECTOR_ARITH" VECTOR_ARITH);;
...
let LINEAR_COMPONENTWISE_EXPANSION = prove
 (`!f:real^M->real^N.
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o LAND_CONV o RAND_CONV)" (LAND_CONV o LAND_CONV o RAND_CONV)
 ...
 (`!A:real^N^M c. invertible(c %% A) <=> ~(c = &0) /\ invertible(A)`,
  REPEAT GEN_TAC THEN ASM_CASES_TAC `c = &0` THEN ASM_REWRITE_TAC[] THENL
   [REWRITE_TAC[invertible; MATRIX_MUL_LZERO; MATRIX_CMUL_LZERO; MAT_EQ] THEN
    CONV_TAC "Multivariate/vectors.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
    ALL_TAC] THEN
...
let ADJOINT_MATRIX = prove
 (`!A:real^N^M. adjoint(\x. A ** x) = (\x. transp A ** x)`,
  GEN_TAC THEN CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC ADJOINT_UNIQUE THEN
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [SUM_SWAP_NUMSEG] THEN REWRITE_TAC[REAL_MUL_AC]);;

let MATRIX_ADJOINT = prove
 (`!f. linear f ==> matrix(adjoint f) = transp(matrix f)`,
  GEN_TAC THEN DISCH_THEN
   (fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o funpow 2 RAND_CONV)" (LAND_CONV o funpow 2 RAND_CONV)
...
let ONORM_ADJOINT = prove
 (`!f:real^N->real^N. linear f ==> onorm(adjoint f) = onorm f`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [DOT_SYM] THEN
 ...
 (`!f:real^N->real^N. linear f ==> onorm(adjoint f o f) = onorm f pow 2`,
  REWRITE_TAC[GSYM REAL_LE_ANTISYM] THEN REPEAT STRIP_TAC THENL
   [ASM_MESON_TAC[REAL_POW_2; ONORM_COMPOSE; ADJOINT_LINEAR; ONORM_ADJOINT];
    ...
    GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [vector_norm] THEN
...
let BILINEAR_MUL_DROP = prove
 (`bilinear(\y:real^N x. drop x % y)`,
  GEN_REWRITE_TAC "Multivariate/vectors.ml:I" I [GSYM BILINEAR_SWAP] THEN
...
let LINEAR_COMPONENTWISE = prove
 (`!f:real^M->real^N.
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [CART_EQ] THEN
...
let NORM_FSTCART = prove
 (`!x. norm(fstcart x) <= norm x`,
  GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM PASTECART_FST_SND] THEN
...
let SPAN_LINEAR_IMAGE = prove
 (`!f:real^M->real^N s. linear f ==> (span(IMAGE f s) = IMAGE f (span s))`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:I" I [EXTENSION] THEN
 ...
 (`!p. dependent (p:real^N -> bool) <=>
  ...
  EQ_TAC THEN REWRITE_TAC[LEFT_IMP_EXISTS_THM] THENL
   [MAP_EVERY X_GEN_TAC [`a:real^N`; `s:real^N->bool`; `u:real^N->real`] THEN
    ...
    CONJ_TAC THENL [ASM SET_TAC[]; CONV_TAC "Multivariate/vectors.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV] THEN
    ASM_SIMP_TAC[VSUM_CLAUSES] THEN
    COND_CASES_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
    REWRITE_TAC[VECTOR_ARITH `-- &1 % a + s = vec 0 <=> a = s`] THEN
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [SYM th]) THEN
...
let SPAN_STDBASIS = prove
 (`span {basis i :real^N | 1 <= i /\ i <= dimindex(:N)} = UNIV`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [GSYM BASIS_EXPANSION] THEN
...
let EXCHANGE_LEMMA = prove
 (`!s t:real^N->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [SUBSET] THEN
  REWRITE_TAC[NOT_FORALL_THM; NOT_IMP] THEN
...
let BASIS_EXISTS = prove
 (`!v. ?b. b SUBSET v /\ independent b /\ v SUBSET (span b) /\
           b HAS_SIZE (dim v)`,
  GEN_TAC THEN REWRITE_TAC[dim] THEN CONV_TAC "Multivariate/vectors.ml:SELECT_CONV" SELECT_CONV THEN
...
let DIM_SPAN = prove
 (`!s:real^N->bool. dim(span s) = dim s`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM SPAN_SPAN] THEN
 ...
 (`!f:real^M->real^N s.
  ...
  FIRST_ASSUM MATCH_MP_TAC THEN REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC SPAN_VSUM THEN ASM_REWRITE_TAC[] THEN
    ...
    FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN
    W(MP_TAC o PART_MATCH (lhs o rand) VSUM_IMAGE o lhand o snd) THEN
...
let LINEAR_INDEP_IMAGE_LEMMA = prove
 (`!f b. linear(f:real^M->real^N) /\
  ...
  REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM] THEN
  GEN_TAC THEN DISCH_TAC THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(BINDER_CONV o RAND_CONV)" (BINDER_CONV o RAND_CONV) [IMP_IMP] THEN
  MATCH_MP_TAC FINITE_INDUCT_STRONG THEN
...
let LINEAR_INDEPENDENT_EXTEND_LEMMA = prove
 (`!f b. FINITE b
  ...
  MP_TAC THENL
   [GEN_TAC THEN DISCH_TAC THEN
    MATCH_MP_TAC(TAUT `a /\ (a ==> b) ==> a /\ b`) THEN CONJ_TAC THENL
     [EXPAND_TAC "h" THEN CONV_TAC "Multivariate/vectors.ml:SELECT_CONV" SELECT_CONV THEN
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [FORALL_AND_THM] THEN STRIP_TAC THEN
  EXISTS_TAC `\z:real^M. h(z) % (f:real^M->real^N)(a) + g(z - h(z) % a)` THEN
  REPEAT CONJ_TAC THENL
   [MAP_EVERY X_GEN_TAC [`x:real^M`; `y:real^M`] THEN STRIP_TAC THEN
    SUBGOAL_THEN `(h:real^M->real)(x + y) = h(x) + h(y)` ASSUME_TAC THENL
     [CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN FIRST_X_ASSUM MATCH_MP_TAC THEN
...
let INVERTIBLE_TRANSP = prove
 (`!A:real^N^M. invertible(transp A) <=> invertible A`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [MESON[TRANSP_TRANSP]
    `(?A:real^M^N. P A) <=> (?A:real^N^M. P(transp A))`] THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [GSYM TRANSP_MAT] THEN
 ...
 (`!f:real^M->real^N.
  ...
  GEN_TAC THEN DISCH_TAC THEN EQ_TAC THEN STRIP_TAC THENL
   [EXISTS_TAC `\y:real^N. (B:real^N^M) ** y` THEN
    ...
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV)
                [MATCH_MP MATRIX_VECTOR_MUL th]) THEN
...
let MATRIX_RIGHT_INVERTIBLE = prove
 (`!f:real^M->real^N.
  ...
  GEN_TAC THEN DISCH_TAC THEN EQ_TAC THEN STRIP_TAC THENL
   [EXISTS_TAC `\y:real^N. (B:real^N^M) ** y` THEN
    REWRITE_TAC[MATRIX_VECTOR_MUL_LINEAR] THEN
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV)
                [MATCH_MP MATRIX_VECTOR_MUL th]) THEN
    ASM_REWRITE_TAC[FUN_EQ_THM; o_THM; I_THM; MATRIX_VECTOR_MUL_ASSOC;
                    MATRIX_VECTOR_MUL_LID];
    EXISTS_TAC `matrix(g:real^N->real^M)` THEN
...
let RANK_TRANSP = prove
 (`!A:real^M^N. rank(transp A) = rank A`,
  GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [RANK_ROW] THEN
...
let RANK_DIM_IM = prove
 (`!A:real^M^N. rank A = dim(IMAGE (\x. A ** x) (:real^M))`,
  ...
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [GSYM SPAN_SPAN] THEN
...
let DIM_EQ_FULL = prove
 (`!s:real^N->bool. dim s = dimindex(:N) <=> span s = (:real^N)`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM SPAN_UNIV] THEN MATCH_MP_TAC DIM_EQ_SPAN THEN
...
let RANK_EQ_0 = prove
 (`!A:real^M^N. rank A = 0 <=> A = mat 0`,
  REWRITE_TAC[RANK_DIM_IM; DIM_EQ_0; SUBSET; FORALL_IN_IMAGE; IN_SING;
              IN_UNIV] THEN
  GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [CART_EQ] THEN
...
let COMPONENT_LE_ONORM = prove
 (`!f:real^M->real^N i j. linear f ==> abs(matrix f$i$j) <= onorm f`,
  REPEAT STRIP_TAC THEN
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
        [MATCH_MP MATRIX_VECTOR_MUL th]) THEN
...
let MATRIX_RATIONAL_APPROXIMATION = prove
 (`!A:real^N^M e.
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(BINOP_CONV o TOP_DEPTH_CONV)" (BINOP_CONV o TOP_DEPTH_CONV)
...
let LOWDIM_EXPAND_DIMENSION = prove
 (`!s:real^N->bool n.
  ...
  GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(BINDER_CONV o LAND_CONV o LAND_CONV)" (BINDER_CONV o LAND_CONV o LAND_CONV) [LE_EXISTS] THEN
 ...
 (`!s t:real^N->bool.
  ...
  EXISTS_TAC `u DIFF ((vec 0:real^N) INSERT s)` THEN REPEAT CONJ_TAC THENL
   [SET_TAC[];
    ...
    FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN
    GEN_REWRITE_TAC "Multivariate/vectors.ml:BINOP_CONV" BINOP_CONV [GSYM SPAN_DELETE_0] THEN
  ...
  REPEAT CONJ_TAC THENL
   [X_GEN_TAC `x:real^N` THEN DISCH_TAC THEN
    ...
    SUBGOAL_THEN `x:real^N = norm(x) % inv(norm x) % x`
     (fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [th])
    THENL
...
let ORTHONORMAL_BASIS_EXPAND_DOT = prove
 (`!b x y:real^N.
  ...
  ASM_REWRITE_TAC[] THEN DISCH_THEN(fun th ->
    GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [SYM th]) THEN
 ...
 (`!s t:real^N->bool.
  ...
  EQ_TAC THEN DISCH_TAC THEN REPEAT GEN_TAC THEN STRIP_TAC THENL
   [ONCE_REWRITE_TAC[GSYM VECTOR_SUB_EQ] THEN
    ...
    FIRST_X_ASSUM(MP_TAC o SYM) THEN CONV_TAC "Multivariate/vectors.ml:VECTOR_ARITH" VECTOR_ARITH;
...
let ORTHOGONAL_SUBSPACE_DECOMP_UNIQUE = prove
 (`!s t x y x' y':real^N.
  ...
  FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
...
let SPECIAL_HYPERPLANE_SPAN = prove
 (`!k. 1 <= k /\ k <= dimindex(:N)
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SPAN_SUBSPACE THEN
...
let DIM_IMAGE_KERNEL_GEN = prove
 (`!f:real^M->real^N s.
  ...
  SUBGOAL_THEN `span(w:real^M->bool) = s`
   (fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [SYM th] THEN
              ASSUME_TAC th)
  ...
  SUBST1_TAC THENL
   [MATCH_MP_TAC SUBSET_ANTISYM THEN CONJ_TAC THENL
    ...
    (CONV_TAC "Multivariate/vectors.ml:o" o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 4)
    ...
    CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC VSUM_EQ_SUPERSET THEN
...
let DIM_SUMS_INTER = prove
 (`!s t:real^N->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
  ...
  SUBGOAL_THEN `d:real^N->bool = b UNION (d DIFF c)`
   (fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o LAND_CONV o ONCE_DEPTH_CONV) [th])
  ...
  CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC VSUM_SUPERSET THEN
...
let DIM_UNION_INTER = prove
 (`!s t:real^N->bool.
  ...
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [GSYM DIM_SPAN] THEN
 ...
 (`!k. dim {x:real^N |
  ...
  REPEAT CONJ_TAC THENL
   [REWRITE_TAC[SUBSET; FORALL_IN_IMAGE; IN_ELIM_THM] THEN
    ...
    DISCH_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [GSYM BASIS_EXPANSION] THEN
    MATCH_MP_TAC SPAN_VSUM THEN REWRITE_TAC[FINITE_NUMSEG; IN_NUMSEG] THEN
...
let RANK_SYLVESTER = prove
 (`!A:real^N^M B:real^P^N.
  ...
  CONJ_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM ETA_AX] THEN
  REWRITE_TAC[MATRIX_VECTOR_MUL_LINEAR]);;
...
let MATRIX_MUL_COVARIANCE_LCANCEL = prove
 (`!A:real^N^P B C:real^M^N.
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM COVARIANCE_MATRIX_EQ_0] THEN
  ...
  and lemma_uniqueness = prove
   (`!A:real^M^N u v y.
    ...
    (SUBGOAL_THEN
     ...
     THENL
      [SIMP_TAC[MATRIX_VECTOR_MUL_SUB_LDISTRIB] THEN CONV_TAC "Multivariate/vectors.ml:VECTOR_ARITH" VECTOR_ARITH;
       DISCH_THEN(fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [th])]) THEN
...
let MATRIX_INV_MUL_INNER = prove
 (`!A:real^M^N. A ** matrix_inv A ** A = A`,
  ...
  REPEAT GEN_TAC THEN CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN
  ONCE_REWRITE_TAC[GSYM VECTOR_SUB_EQ] THEN
  REWRITE_TAC[GSYM ORTHOGONAL_REFL] THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM MATRIX_VECTOR_MUL_SUB_LDISTRIB] THEN
...
let MATRIX_INV_UNIQUE_STRONG = prove
 (`!A:real^M^N X.
  ...
  POP_ASSUM(K ALL_TAC) THEN CONV_TAC "Multivariate/vectors.ml:SYM_CONV" SYM_CONV THEN
  SUBGOAL_THEN
...
let RANK_CMUL = prove
 (`!A:real^N^M c. rank(c %% A) = if c = &0 then 0 else rank A`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o RAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV o RAND_CONV) [GSYM MATRIX_MUL_LID] THEN
...
let RANK_NEG = prove
 (`!A:real^N^M. rank(--A) = rank A`,
  ...
  CONV_TAC "Multivariate/vectors.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let RANK_MATRIX_INV = prove
 (`!A:real^M^N. rank(matrix_inv A) = rank A`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM MATRIX_INV_MUL_INNER] THEN
...
let RANK_MATRIX_INV_RMUL = prove
 (`!A:real^M^N. rank(A ** matrix_inv A) = rank A`,
  ...
  REWRITE_TAC[RANK_MUL_LE_LEFT] THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [GSYM RANK_MATRIX_INV] THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM MATRIX_INV_MUL_OUTER] THEN
  REWRITE_TAC[RANK_MUL_LE_RIGHT]);;
...
let MATRIX_INV_MULTIPLE_TRANP_LEFT = prove
 (`!A:real^M^N.
  ...
  GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [GSYM MATRIX_INV_MUL_OUTER] THEN
...
let COMMUTING_MATRIX_INV_COVARIANCE = prove
 (`!A:real^M^N.
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM SYMMETRIC_MATRIX_INV_RMUL] THEN
...
let COMMUTING_MATRIX_INV_NORMAL = prove
 (`!A:real^N^N.
  ...
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV)
   [GSYM MATRIX_INV_COVARIANCE_RMUL] THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
 ...
 (`!A:real^P^N B:real^N^M.
  ...
  let lemma = prove
   (`!A:real^P^N B:real^N^M.
    ...
    GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV)
     [MATRIX_INV_MULTIPLE_TRANP_LEFT] THEN
    ONCE_REWRITE_TAC[MATRIX_MUL_ASSOC] THEN
    GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o LAND_CONV o TOP_DEPTH_CONV)
...
let INFNORM_MUL_LEMMA = prove
 (`!a x. infnorm(a % x) <= abs a * infnorm x`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [infnorm] THEN
  SIMP_TAC[REAL_SUP_LE_FINITE; INFNORM_SET_LEMMA] THEN
...
let INFNORM_MUL = prove
 (`!a x:real^N. infnorm(a % x) = abs a * infnorm x`,
  ...
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o funpow 2 RAND_CONV)" (LAND_CONV o funpow 2 RAND_CONV) [GSYM VECTOR_MUL_LID] THEN
...
let NORM_LE_INFNORM = prove
 (`!x:real^N. norm(x) <= sqrt(&(dimindex(:N))) * infnorm(x)`,
  GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o LAND_CONV o funpow 2 RAND_CONV)" (RAND_CONV o LAND_CONV o funpow 2 RAND_CONV)
   [GSYM CARD_NUMSEG_1] THEN
...
let NORM_CAUCHY_SCHWARZ_ABS_EQ = prove
 (`!x:real^N y. abs(x dot y) = norm(x) * norm(y) <=>
  ...
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/vectors.ml:(LAND_CONV o funpow 3 RAND_CONV)" (LAND_CONV o funpow 3 RAND_CONV) [GSYM NORM_NEG] THEN
...
let COLLINEAR_SMALL = prove
 (`!s. FINITE s /\ CARD s <= 2 ==> collinear s`,
  ...
  CONV_TAC "Multivariate/vectors.ml:(ONCE_DEPTH_CONV HAS_SIZE_CONV)" (ONCE_DEPTH_CONV HAS_SIZE_CONV) THEN
  REPEAT STRIP_TAC THEN
...
let ORTHOGONAL_TO_ORTHOGONAL_2D = prove
 (`!x y z:real^2.
  ...
  REWRITE_TAC[DOT_2] THEN CONV_TAC "Multivariate/vectors.ml:REAL_RING" REAL_RING);;
...
let COLLINEAR_3_2D = prove
 (`!x y z:real^2. collinear{x,y,z} <=>
  ...
  REWRITE_TAC[DOT_2; VECTOR_SUB_COMPONENT] THEN CONV_TAC "Multivariate/vectors.ml:REAL_RING" REAL_RING);;
 ...
 (`!a b c:real^N.
  ...
  REPEAT GEN_TAC THEN ASM_CASES_TAC `b:real^N = a` THENL
   [ASM_REWRITE_TAC[COLLINEAR_2; INSERT_AC; DOT_RZERO; VECTOR_MUL_LZERO;
    ...
    REWRITE_TAC[DOT_SYM] THEN CONV_TAC "Multivariate/vectors.ml:REAL_RING" REAL_RING]);;
...
let BETWEEN_CMUL_LIFT = prove
 (`!a b c v:real^N.
  ...
  REWRITE_TAC[GSYM NORM_EQ_0] THEN CONV_TAC "Multivariate/vectors.ml:REAL_RING" REAL_RING);;
 ...
 (`!(f:A->B) P P'.
  ...
  REPEAT STRIP_TAC THEN REWRITE_TAC[UNION_OF] THEN EQ_TAC THENL
   [DISCH_THEN(X_CHOOSE_THEN `u:(B->bool)->bool` STRIP_ASSUME_TAC) THEN
    ...
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM th]) THEN
 ...
 (`(!A:real^N^N.
  ...
  CONJ_TAC THENL
   [EQ_TAC THENL [ALL_TAC; SIMP_TAC[NORM_EQ]] THEN
    ...
    ASM_REWRITE_TAC[MATRIX_VECTOR_MUL_LINEAR] THEN
    GEN_REWRITE_TAC "Multivariate/vectors.ml:RAND_CONV" RAND_CONV [GSYM ETA_AX] THEN
    REWRITE_TAC[MATRIX_VECTOR_MUL_LINEAR];
    REWRITE_TAC[DOT_MATRIX_TRANSP_RMUL] THEN
    GEN_REWRITE_TAC "Multivariate/vectors.ml:(RAND_CONV o funpow 2 BINDER_CONV o RAND_CONV)" (RAND_CONV o funpow 2 BINDER_CONV o RAND_CONV)
...
let ORTHOGONAL_SCALING = prove
 (`!c. &0 < c ==> !x y. orthogonal (c % x) (c % y) <=> orthogonal x y`,
  REWRITE_TAC[orthogonal; DOT_LMUL; DOT_RMUL] THEN CONV_TAC "Multivariate/vectors.ml:REAL_FIELD" REAL_FIELD) in
...
let QUANTIFY_SURJECTION_THM = prove
 (`!f:A->B.
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/vectors.ml:LAND_CONV" LAND_CONV [SURJECTIVE_RIGHT_INVERSE] THEN
...
let QUANTIFY_SURJECTION_HIGHER_THM = prove
 (`!f:A->B.
  ...
  GEN_TAC THEN DISCH_TAC THEN CONV_TAC "Multivariate/vectors.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
