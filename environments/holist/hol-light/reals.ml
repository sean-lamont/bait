...
let REAL_OF_NUM_SUB_CASES = prove
 (`!m n. &m - &n = if n <= m then &(m - n) else -- &(n - m)`,
  ...
  GEN_REWRITE_TAC "reals.ml:LAND_CONV" LAND_CONV [GSYM REAL_NEG_SUB] THEN AP_TERM_TAC THEN
...
let REAL_LT_LADD_IMP = prove
 (`!x y z. y < z ==> x + y < x + z`,
  REPEAT GEN_TAC THEN CONV_TAC "reals.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let REAL_LT_MUL = prove
 (`!x y. &0 < x /\ &0 < y ==> &0 < x * y`,
  ...
  CONV_TAC "reals.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
...
let REAL_ARITH_TAC = replace_tactic_log Real_arith_tac2_log
    (CONV_TAC "reals.ml:REAL_ARITH" REAL_ARITH);;

let () = real_arith_tac2 := Some REAL_ARITH_TAC;;
...
let REAL_ABS_MUL = prove
 (`!x y. abs(x * y) = abs(x) * abs(y)`,
  ...
  DISJ_CASES_TAC (SPEC `x:real` REAL_LE_NEGTOTAL) THENL
   [ALL_TAC;
    GEN_REWRITE_TAC "reals.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [GSYM REAL_ABS_NEG]] THEN
  (DISJ_CASES_TAC (SPEC `y:real` REAL_LE_NEGTOTAL) THENL
    [ALL_TAC;
     GEN_REWRITE_TAC "reals.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_ABS_NEG]]) THEN
  ...
  REWRITE_TAC[REAL_MUL_LNEG; REAL_MUL_RNEG; REAL_NEG_NEG] THEN DISCH_TAC THENL
   [ALL_TAC;
    GEN_REWRITE_TAC "reals.ml:LAND_CONV" LAND_CONV [GSYM REAL_ABS_NEG];
    GEN_REWRITE_TAC "reals.ml:LAND_CONV" LAND_CONV [GSYM REAL_ABS_NEG];
    ALL_TAC] THEN
...
let REAL_LT_INV_EQ = prove
 (`!x. &0 < inv x <=> &0 < x`,
  ...
  GEN_REWRITE_TAC "reals.ml:(funpow 2 RAND_CONV)" (funpow 2 RAND_CONV) [GSYM REAL_INV_INV] THEN
...
let REAL_ABS_INV = prove
 (`!x. abs(inv x) = inv(abs x)`,
  GEN_TAC THEN CONV_TAC "reals.ml:SYM_CONV" SYM_CONV THEN
...
let REAL_POW_SUB = prove
 (`!x m n. ~(x = &0) /\ m <= n ==> (x pow (n - m) = x pow n / x pow m)`,
  ...
  GEN_REWRITE_TAC "reals.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_LID] THEN
  REWRITE_TAC[REAL_MUL_ASSOC] THEN AP_THM_TAC THEN AP_TERM_TAC THEN
  CONV_TAC "reals.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_MUL_LINV THEN
...
let REAL_LE_RDIV_EQ = prove
 (`!x y z. &0 < z ==> (x <= y / z <=> x * z <= y)`,
  REPEAT STRIP_TAC THEN
  FIRST_ASSUM(fun th ->
    GEN_REWRITE_TAC "reals.ml:LAND_CONV" LAND_CONV [GSYM(MATCH_MP REAL_LE_RMUL_EQ th)]) THEN
...
let REAL_LT_SQUARE = prove
 (`!x. (&0 < x * x) <=> ~(x = &0)`,
  ...
  GEN_REWRITE_TAC "reals.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [EQ_SYM_EQ] THEN
   ...
   [MATCH_MP_TAC REAL_LT_MUL THEN
    ...
    SUBGOAL_THEN `(inv x * x = &1) /\ (inv y * y = &1)` ASSUME_TAC THENL
     [CONJ_TAC THEN MATCH_MP_TAC REAL_MUL_LINV THEN
      ...
      GEN_REWRITE_TAC "reals.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
...
let REAL_POW_MONO = prove
 (`!m n x. &1 <= x /\ m <= n ==> x pow m <= x pow n`,
  ...
  GEN_REWRITE_TAC "reals.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_RID] THEN
...
let REAL_DIV_POW2 = prove
 (`!x m n. ~(x = &0)
  ...
  GEN_REWRITE_TAC "reals.ml:LAND_CONV" LAND_CONV [GSYM REAL_INV_INV] THEN
 ...
 (`!m n x. &0 <= x /\ x <= &1 /\ n <= m ==> x pow m <= x pow n`,
  REPEAT STRIP_TAC THEN ASM_CASES_TAC `x = &0` THENL
   [ASM_REWRITE_TAC[REAL_POW_ZERO] THEN
    ...
    GEN_REWRITE_TAC "reals.ml:BINOP_CONV" BINOP_CONV [GSYM REAL_INV_INV] THEN
...
let REAL_POW_EQ_1_IMP = prove
 (`!x n. ~(n = 0) /\ x pow n = &1 ==> abs(x) = &1`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "reals.ml:RAND_CONV" RAND_CONV [GSYM REAL_ABS_NUM] THEN
 ...
 (`!x. real_sgn x = x / abs x`,
  GEN_TAC THEN ASM_CASES_TAC `x = &0` THENL
   [ASM_REWRITE_TAC[real_div; REAL_MUL_LZERO; REAL_SGN_0];
    GEN_REWRITE_TAC "reals.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [GSYM REAL_SGN_ABS] THEN
