...
let ARITH_EXP = prove
 (`(!m n. (NUMERAL m) EXP (NUMERAL n) = NUMERAL(m EXP n)) /\
  ...
  TRY(GEN_REWRITE_TAC "calc_num.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [BIT0; BIT1]) THEN
  ...
  let NUM_SHIFT_CONV =
    ...
    and pth_triv = (STANDARDIZE o prove)
     (`_0 = a + p * b <=> _0 = a + BIT0 p * b`,
      CONV_TAC "calc_num.ml:(BINOP_CONV SYM_CONV)" (BINOP_CONV SYM_CONV) THEN
      SUBST1_TAC(SYM(SPEC `_0` NUMERAL)) THEN
  ...
  let NUM_SQUARE_RULE =
    ...
    and pth_odd = (STANDARDIZE o prove)
     (`m EXP 2 = n <=> (BIT1 m) EXP 2 = BIT1(BIT0(m + n))`,
      ...
      GEN_REWRITE_TAC "calc_num.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [ADD_SYM] THEN
    ...
    and pth_rec = (UNDISCH o STANDARDIZE o prove)
     (`n = l + p * h /\
      ...
      REWRITE_TAC[MULT_AC] THEN CONV_TAC "calc_num.ml:(BINOP_CONV NUM_CANCEL_CONV)" (BINOP_CONV NUM_CANCEL_CONV) THEN
    ...
    and pth_toom3 = (STANDARDIZE o prove)
     (`h EXP 2 = e /\
      ...
      CONV_TAC "calc_num.ml:(LAND_CONV NUM_CANCEL_CONV)" (LAND_CONV NUM_CANCEL_CONV) THEN DISCH_THEN SUBST_ALL_TAC THEN
      FIRST_ASSUM(MP_TAC o MATCH_MP (MESON[]
       `b = b' /\ c = c' /\ d = d'
      ...
      REWRITE_TAC(map (fun k ->
         ...
         (1--4)) THEN
      REWRITE_TAC[MULT_CLAUSES; ADD_CLAUSES] THEN
      CONV_TAC "calc_num.ml:(LAND_CONV(BINOP_CONV NUM_CANCEL_CONV))" (LAND_CONV(BINOP_CONV NUM_CANCEL_CONV)) THEN
...
let NUM_REDUCE_TAC = CONV_TAC "calc_num.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;;
