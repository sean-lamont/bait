...
from environments.int_environment.algos.model.gnns import FCResBlock
...
def list_to_sequence(data_list, max_len=1024):
    if data_list == []:
        data_list = torch.LongTensor([[0]]).to(device)
...
class GroundTruthEncoder(torch.nn.Module):
    def __init__(self,
        ...
        self.encoder = INTTransformer(ntoken=60,
                                      nlayers=6,
                                      nhead=8,
                                      in_embed=False,
                                      dropout=0.1,
    ...
    def forward(self, data, gnn_ind, batch_gnn_ind=None, attention_keys=None):
        ...
        # add one for padding
        tok_data = [d.nonzero()[:,1][::2] + 1 for d in data]
        iv_data = [d.nonzero()[:,1][1::2] + 1 for d in data]

        lens = [d.shape[0] for d in data]
        # pad and get padding mask for transformer
        iv_data = list_to_sequence(iv_data, max_len=2048)
        tok_data = list_to_sequence(tok_data, max_len=2048)

        # input = (tok_data[0], iv_data[0], tok_data[1])

        # as masks will be the same
        state_tensor = self.encoder(tok_data[0], iv_data[0], tok_data[1])


        state_tensor = torch.nn.utils.rnn.unpad_sequence(state_tensor, lens, batch_first=True)
        ...
        if len(gt_gnns) > 0:
            if self.attention_type == 1:
                gt_state_tensor, gt_out = self.gt_encoder(
                    gt_gnns, gt_gnns_ind, gt_batch_gnn_ind, obj_out)
            else:
                gt_state_tensor, gt_out = self.gt_encoder(
                    gt_gnns, gt_gnns_ind, gt_batch_gnn_ind)
        else:
