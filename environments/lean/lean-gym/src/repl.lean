...
(tactic_state_sexp : option string)
...
| ⟨sid, tsid, ts, err, steps, ts_sexp⟩ :=
    json.object [
      ...
      ⟨"tactic_state_sexp", match ts_sexp with
        | none := json.null
        | some ts_sexp := json.of_string ts_sexp
        end⟩
    ]
...
meta def handle_init_search
  ...
  : LeanREPL LeanREPLResponse := do {
   ...
   | ff := do {
     ...
     pure ⟨none, none, none, some err.to_string, [], none⟩
   }
   ...
   | tt := do {
     ...
     ts_sexp ← (state_t.lift ∘ io.run_tactic'') $ tactic_state_to_sexp ts,
     pure $ ⟨sid, tsid, ts_str, none, [], ts_sexp⟩
   }
   end
}
...
meta def handle_clear_search
  ...
  : LeanREPL LeanREPLResponse := do {
   ...
   pure $ ⟨req.sid, none, none, none, [], none⟩
}
...
meta def finalize_proof
  ...
  | (some ts₀) := do {
    ...
    | (interaction_monad.result.success r s') := do {
      ...
      ts_str ← (state_t.lift ∘ io.run_tactic'') $ postprocess_tactic_state ts',
      ts_sexp ← (state_t.lift ∘ io.run_tactic'') $ tactic_state_to_sexp ts',
      pure $ ⟨req.sid, tsid, ts_str, none, [], ts_sexp⟩
    }
...
meta def handle_conjecture
  ...
  | (some ts) := do {
    ...
    | interaction_monad.result.success _ ts' := do {
        ...
        ts_sexp ← (state_t.lift ∘ io.run_tactic'') $ tactic_state_to_sexp ts_narrowed,
        pure $ ⟨sid, tsid, ts_str, none, [], ts_sexp⟩
    }
...
meta def handle_shrink_proof
  ...
  | (some ts_final) := do {
    ...
    ts_sexp ← state_t.lift ∘ io.run_tactic'' $ tactic_state_to_sexp ts_final,
    ...
    pure ⟨req.sid, req.tsid, ts_str, none, new_steps, ts_sexp⟩
  }
  end
  }
...
meta def handle_try_finish
  ...
  | (some ts) := do {
    ...
    | some (action, ts') := do {
      ...
      ts_str ← (state_t.lift ∘ io.run_tactic'') $ postprocess_tactic_state ts',
      ts_sexp ← (state_t.lift ∘ io.run_tactic'') $ tactic_state_to_sexp ts',
      pure $ ⟨req.sid, tsid, ts_str, none, [(action, ts_str)], ts_sexp⟩
    }
    }
    end
  }
...
meta def handle_assume
  ...
  | (some ts) := do {
    ...
    | interaction_monad.result.success _ ts' := do {
        ...
        ts_sexp ← (state_t.lift ∘ io.run_tactic'') $ tactic_state_to_sexp ts_assumed,
        pure $ ⟨sid, tsid, ts_str, none, [], ts_sexp⟩
    }
    | interaction_monad.result.exception fn pos ts' := do {
      state_t.lift $ do {
