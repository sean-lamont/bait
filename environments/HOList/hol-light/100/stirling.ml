...
let LN_LIM_BOUND = prove
 (`!n. ~(n = 0) ==> abs(&n * ln(&1 + &1 / &n) - &1) <= &1 / (&2 * &n)`,
  ...
  CONV_TAC "100/stirling.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN
  MATCH_MP_TAC REAL_LE_LMUL THEN CONV_TAC "100/stirling.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  GEN_REWRITE_TAC "100/stirling.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
  ...
  GEN_REWRITE_TAC "100/stirling.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_LID] THEN
  MATCH_MP_TAC REAL_LE_MUL2 THEN REWRITE_TAC[REAL_POS] THEN
  UNDISCH_TAC `&0 < t` THEN REAL_ARITH_TAC);;

let LN_LIM_LEMMA = prove
 (`(\n. &n * ln(&1 + &1 / &n)) --> &1`,
  GEN_REWRITE_TAC "100/stirling.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV)
   [REAL_ARITH `a = (a - &1) + &1`] THEN
  GEN_REWRITE_TAC "100/stirling.ml:RAND_CONV" RAND_CONV [GSYM REAL_ADD_LID] THEN
 ...
 (`!x. &0 < x
  ...
  REWRITE_TAC[IMP_IMP] THEN ANTS_TAC THENL
   [REPEAT CONJ_TAC THEN TRY(MATCH_MP_TAC REAL_LT_DIV) THEN
    POP_ASSUM MP_TAC THEN CONV_TAC "100/stirling.ml:REAL_FIELD" REAL_FIELD;
    ALL_TAC] THEN
...
let STIRLING_DELTA_LIMIT = prove
 (`(\n. ln ((&n + &1) / &n) - &1 / (&n + &1 / &2)) --> &0`,
  GEN_REWRITE_TAC "100/stirling.ml:RAND_CONV" RAND_CONV [GSYM REAL_SUB_RZERO] THEN
 ...
 (`!n. ~(n = 0)
  ...
  REWRITE_TAC[] THEN CONJ_TAC THENL
   [ALL_TAC;
    ...
    GEN_REWRITE_TAC "100/stirling.ml:RAND_CONV" RAND_CONV [SYM(REAL_RAT_REDUCE_CONV `&0 * &0 * &0`)] THEN
  ...
  X_GEN_TAC `x:real` THEN DISCH_TAC THEN CONJ_TAC THENL
   [GEN_REWRITE_TAC "100/stirling.ml:LAND_CONV" LAND_CONV
...
let STIRLING_UPPERBOUND = prove
 (`!n. stirling(SUC n) <= &1`,
  INDUCT_TAC THENL
   [REWRITE_TAC[stirling] THEN CONV_TAC "100/stirling.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    REWRITE_TAC[LN_1] THEN CONV_TAC "100/stirling.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    ALL_TAC] THEN
 ...
 (`!n. (&n + &2) * integral(&0,pi / &2) (\x. sin(x) pow (n + 2)) =
  ...
  REWRITE_TAC[] THEN ANTS_TAC THENL
   [SIMP_TAC[REAL_LT_IMP_LE; PI_POS; REAL_LT_DIV; REAL_OF_NUM_LT; ARITH] THEN
    CONV_TAC "100/stirling.ml:(ONCE_DEPTH_CONV INTEGRABLE_CONV)" (ONCE_DEPTH_CONV INTEGRABLE_CONV) THEN REWRITE_TAC[] THEN
  ...
  CONV_TAC "100/stirling.ml:(ONCE_DEPTH_CONV INTEGRABLE_CONV)" (ONCE_DEPTH_CONV INTEGRABLE_CONV) THEN SIMP_TAC[PI2_LE]);;
...
let WALLIS_PARTS' = prove
 (`!n. integral(&0,pi / &2) (\x. sin(x) pow (n + 2)) =
  ...
  CONV_TAC "100/stirling.ml:REAL_FIELD" REAL_FIELD);;
...
let WALLIS_1 = prove
 (`integral(&0,pi / &2) (\x. sin(x) pow 1) = &1`,
  ...
  REWRITE_TAC[COS_0; COS_PI2] THEN CONV_TAC "100/stirling.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let WALLIS_EVEN = prove
 (`!n. integral(&0,pi / &2) (\x. sin(x) pow (2 * n)) =
  ...
  INDUCT_TAC THENL
   [CONV_TAC "100/stirling.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "100/stirling.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  GEN_REWRITE_TAC "100/stirling.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [REAL_POW_MUL] THEN
...
let WALLIS_ODD = prove
 (`!n. integral(&0,pi / &2) (\x. sin(x) pow (2 * n + 1)) =
  ...
  INDUCT_TAC THENL
   [CONV_TAC "100/stirling.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "100/stirling.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    REWRITE_TAC[WALLIS_1] THEN CONV_TAC "100/stirling.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    ALL_TAC] THEN
  ASM_REWRITE_TAC[ARITH_RULE `2 * SUC n + 1 = (2 * n + 1) + 2`;
                  WALLIS_PARTS'] THEN
  GEN_REWRITE_TAC "100/stirling.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
  REWRITE_TAC[FACT; real_pow; GSYM REAL_OF_NUM_MUL] THEN
  ONCE_REWRITE_TAC[REAL_ARITH `(&2 * x) * y * z = (x * z) * (&2 * y)`] THEN
  GEN_REWRITE_TAC "100/stirling.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [REAL_POW_MUL] THEN
  REWRITE_TAC[real_div; GSYM REAL_MUL_ASSOC] THEN AP_TERM_TAC THEN
  GEN_REWRITE_TAC "100/stirling.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
...
let WALLIS_QUOTIENT' = prove
 (`!n. integral(&0,pi / &2) (\x. sin(x) pow (2 * n)) /
  ...
  GEN_REWRITE_TAC "100/stirling.ml:(LAND_CONV o LAND_CONV o RAND_CONV)" (LAND_CONV o LAND_CONV o RAND_CONV) [GSYM REAL_INV_DIV] THEN
  REWRITE_TAC[GSYM real_div] THEN MATCH_MP_TAC REAL_DIV_RMUL THEN
  MP_TAC PI2_NZ THEN CONV_TAC "100/stirling.ml:REAL_FIELD" REAL_FIELD);;
...
let WALLIS_MONO = prove
 (`!m n. m <= n
  ...
  CONV_TAC "100/stirling.ml:(ONCE_DEPTH_CONV INTEGRABLE_CONV)" (ONCE_DEPTH_CONV INTEGRABLE_CONV) THEN REWRITE_TAC[PI2_LE] THEN
  REPEAT STRIP_TAC THEN REWRITE_TAC[REAL_POW_ADD] THEN
  GEN_REWRITE_TAC "100/stirling.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
 ...
 (`!n. integral(&0,pi / &2) (\x. sin(x) pow (n + 1))
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[WALLIS_PARTS'] THEN
    MATCH_MP_TAC REAL_EQ_IMP_LE THEN CONV_TAC "100/stirling.ml:REAL_FIELD" REAL_FIELD;
    ALL_TAC] THEN
...
let WALLIS = prove
 (`(\n. (&2 pow n * &(FACT n)) pow 4 / (&(FACT(2 * n)) * &(FACT(2 * n + 1))))
  ...
  CONJ_TAC THENL [ALL_TAC; MP_TAC PI2_NZ THEN CONV_TAC "100/stirling.ml:REAL_FIELD" REAL_FIELD] THEN
  ...
  GEN_REWRITE_TAC "100/stirling.ml:(LAND_CONV o ABS_CONV)" (LAND_CONV o ABS_CONV) [REAL_ARITH `x = (x - &1) + &1`] THEN
  GEN_REWRITE_TAC "100/stirling.ml:RAND_CONV" RAND_CONV [GSYM REAL_ADD_LID] THEN
  MATCH_MP_TAC SEQ_ADD THEN REWRITE_TAC[SEQ_CONST] THEN
  MATCH_MP_TAC SEQ_LE_0 THEN EXISTS_TAC `\n. &1 / &n` THEN
...
let STIRLING = prove
 (`(\n. ln(&(FACT n)) - ((&n + &1 / &2) * ln(&n) - &n + ln(&2 * pi) / &2))
  ...
  GEN_REWRITE_TAC "100/stirling.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [GSYM ETA_AX] THEN
  ...
  CONV_TAC "100/stirling.ml:(LAND_CONV(GEN_ALPHA_CONV `n:num`))" (LAND_CONV(GEN_ALPHA_CONV `n:num`)) THEN
  REWRITE_TAC[REAL_ARITH
  ...
  REWRITE_TAC[REAL_ARITH
   `(&2 * n + &3 / &2) * l + &1 - ((&2 * n + &1) + &1 / &2) * l' =
    (&2 * n + &3 / &2) * (l - l') + &1`] THEN
  GEN_REWRITE_TAC "100/stirling.ml:RAND_CONV" RAND_CONV [REAL_ARITH `&0 = -- &1 + &1`] THEN
  MATCH_MP_TAC SEQ_ADD THEN REWRITE_TAC[SEQ_CONST] THEN
  ...
  DISCH_THEN MATCH_MP_TAC THEN CONV_TAC "100/stirling.ml:(LAND_CONV(GEN_ALPHA_CONV `n:num`))" (LAND_CONV(GEN_ALPHA_CONV `n:num`)) THEN
  REWRITE_TAC[REAL_ADD_RDISTRIB] THEN
  GEN_REWRITE_TAC "100/stirling.ml:RAND_CONV" RAND_CONV [REAL_ARITH `&1 = &1 + &3 / &2 * &0`] THEN
