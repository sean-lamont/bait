...
(* Misc not compatible with Analysis *)
(*
open Misc;;
*)

(*** "Multivariate/misc.ml" Just for REAL_ARCH_INV! ***)
...
let REAL_TRIANGLE = prove
 (`&(triangle n) = (&n * (&n + &1)) / &2`,
  ...
  SUBGOAL_THEN `EVEN(n * (n + 1))` MP_TAC THENL
   [REWRITE_TAC[EVEN_MULT; EVEN_ADD; ARITH] THEN CONV_TAC "100/triangular.ml:TAUT" TAUT;
...
let TRIANGLE_FINITE_SUM = prove
 (`!n. sum(1..n) (\k. &1 / &(triangle k)) = &2 - &2 / (&n + &1)`,
  ...
  CONV_TAC "100/triangular.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  REWRITE_TAC[REAL_TRIANGLE; GSYM REAL_OF_NUM_SUC] THEN CONV_TAC "100/triangular.ml:REAL_FIELD" REAL_FIELD);;
...
let TRIANGLE_CONVERGES = prove
 (`!e. &0 < e
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "100/triangular.ml:LAND_CONV" LAND_CONV [REAL_ARCH_INV] THEN
