...
let INVOLUTION_EVEN_STEP = prove
 (`FINITE(s) /\
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/four_squares.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [th]) THEN
...
let PRIME_SQUARE = prove
 (`!n. ~prime(n * n)`,
  ...
  GEN_REWRITE_TAC "100/four_squares.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [ARITH_RULE `n = n * 1`] THEN
...
let PRIME_XYZ_NONZERO = prove
 (`prime(x EXP 2 + 4 * y * z) ==> 0 < x /\ 0 < y /\ 0 < z`,
  CONV_TAC "100/four_squares.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
  ...
  FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN CONJ_TAC THENL
   [MESON_TAC[EXP_2; LE_SQUARE_REFL; ARITH_RULE `(a <= b ==> a <= b + c)`];
    CONJ_TAC THEN MATCH_MP_TAC(ARITH_RULE `y <= z ==> y <= x + z`) THENL
     [GEN_REWRITE_TAC "100/four_squares.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [MULT_SYM]; ALL_TAC] THEN
...
let LAGRANGE_LEMMA_ODD = prove
 (`!a p. prime(p) /\ ODD(p)
  ...
  SUBST1_TAC THENL
   [CONV_TAC "100/four_squares.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MOD_EQ THEN
 ...
 (`!x n. &0 <= x /\ (abs(x - &n) = &1 / &2)
  ...
  ASM_REWRITE_TAC[ARITH] THENL
   [MP_TAC(REAL_RAT_REDUCE_CONV `&0 <= &1 / &2`) THEN REAL_ARITH_TAC;
    ...
    CONV_TAC "100/four_squares.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REAL_ARITH_TAC]);;
   ...
   [ASM_CASES_TAC
     ...
     [FIRST_X_ASSUM(CHOOSE_THEN STRIP_ASSUME_TAC) THEN
      ...
      DISJ_CASES_TAC THENL
       [REWRITE_TAC[EVEN_ADD; ARITH_EVEN] THEN CONV_TAC "100/four_squares.ml:TAUT" TAUT;
        EXISTS_TAC `r':num` THEN DISJ2_TAC THEN ASM_REWRITE_TAC[] THEN
        REWRITE_TAC[REAL_ARITH `(a + b) - a = b`] THEN
        CONV_TAC "100/four_squares.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    ...
    CONV_TAC "100/four_squares.ml:(RAND_CONV REAL_RAT_REDUCE_CONV)" (RAND_CONV REAL_RAT_REDUCE_CONV) THEN REWRITE_TAC[] THEN
  ...
  SUBGOAL_THEN `?n. abs((&N' - &N) / &2) = &n` ASSUME_TAC THENL
   [REWRITE_TAC[GSYM(ASSUME
    ...
    SUBGOAL_THEN `EVEN(n' EXP 2 + p' EXP 2 + q' EXP 2 + r' EXP 2) =
                  EVEN N`
    MP_TAC THENL
     [FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "100/four_squares.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
    ...
    CONV_TAC "100/four_squares.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let AUBREY_THM_4 = prove
 (`(?q. ~(q = 0) /\
  ...
  GEN_REWRITE_TAC "100/four_squares.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
 ...
 (`!n. ?w x y z. &n = &w pow 2 + &x pow 2 + &y pow 2 + &z pow 2`,
  ...
  ASM_CASES_TAC `n = 1` THENL
   [EXISTS_TAC `1` THEN REPEAT(EXISTS_TAC `0`) THEN
    ASM_REWRITE_TAC[] THEN CONV_TAC "100/four_squares.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    ALL_TAC] THEN
  ...
  CONV_TAC "100/four_squares.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN MATCH_MP_TAC AUBREY_THM_4 THEN
  ...
  REWRITE_TAC[REAL_INV_EQ_0; REAL_POW_EQ_0; REAL_OF_NUM_EQ;
              ASSUME `~(q = 0)`] THEN
  CONV_TAC "100/four_squares.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN MATCH_MP_TAC lemma THEN
