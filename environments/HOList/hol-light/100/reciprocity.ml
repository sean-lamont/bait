...
let CARD_SUBCROSS_DETERMINATE = prove
 (`FINITE s /\ FINITE t /\ (!x. x IN s /\ p(x) ==> f(x) IN t)
   ==> CARD {(x:A),(y:B) | x IN s /\ y IN t /\ y = f x /\ p x} =
       CARD {x | x IN s /\ p(x)}`,
  REPEAT STRIP_TAC THEN CONV_TAC "100/reciprocity.ml:SYM_CONV" SYM_CONV THEN
  ...
  SUBGOAL_THEN `s = (a:A) INSERT (b INSERT (s DELETE a DELETE b))`
   ...
   [CONJ_TAC THENL
     [UNDISCH_TAC `(s:A->bool) HAS_SIZE 2 * n` THEN
      FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/reciprocity.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV)
        [SYM th]) THEN
...
let WILSON_LEMMA = prove
 (`!p. prime(p) ==> (FACT(p - 2) == 1) (mod p)`,
  ...
  THENL [CONV_TAC "100/reciprocity.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "100/reciprocity.ml:CONG_CONV" CONG_CONV; ALL_TAC] THEN
...
let WILSON = prove
 (`!p. ~(p = 1) ==> (prime p <=> (FACT(p - 1) == p - 1) (mod p))`,
  ...
  ASM_CASES_TAC `n = 0` THEN ASM_REWRITE_TAC[] THENL
   [ASM_REWRITE_TAC[CONG_MOD_0] THEN CONV_TAC "100/reciprocity.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV; ALL_TAC] THEN
...
let EULER_CRITERION = prove
 (`!a p. prime p /\ coprime(a,p)
  ...
  REPEAT STRIP_TAC THEN FIRST_ASSUM(DISJ_CASES_THEN2 SUBST_ALL_TAC ASSUME_TAC o
    MATCH_MP PRIME_ODD) THEN CONV_TAC "100/reciprocity.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
...
let GAUSS_LEMMA_1 = prove
 (`prime p /\ coprime(a,p) /\ 2 * r + 1 = p
  ...
  GEN_REWRITE_TAC "100/reciprocity.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [SYM(CONJUNCT1(SPEC_ALL I_O_ID))] THEN
...
let GAUSS_LEMMA_SYM = prove
 (`!p q r s. prime p /\ prime q /\ coprime(p,q) /\
  ...
  CONJ_TAC THENL
   [CONV_TAC "100/reciprocity.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_SUBCROSS_DETERMINATE THEN
    ...
    SUBGOAL_THEN `p * (q * x) DIV p + r < q * r` MP_TAC THENL
     [MATCH_MP_TAC LTE_TRANS THEN EXISTS_TAC `q * x` THEN
      ...
      GEN_REWRITE_TAC "100/reciprocity.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [MULT_SYM] THEN
...
let GAUSS_LEMMA_SYM' = prove
 (`!p q r s. prime p /\ prime q /\ coprime(p,q) /\
  ...
  GEN_REWRITE_TAC "100/reciprocity.ml:LAND_CONV" LAND_CONV [CARD_SUBCROSS_SWAP] THEN
 ...
 (`!a b c d r s.
  ...
  SUBGOAL_THEN `CARD(a:num#num->bool) + CARD(b:num#num->bool) +
   ...
   (fun th -> MP_TAC(AP_TERM `EVEN` th) THEN
              ...
              CONV_TAC "100/reciprocity.ml:TAUT" TAUT) THEN
  SUBGOAL_THEN
