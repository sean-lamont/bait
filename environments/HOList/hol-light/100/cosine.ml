 ...
 (`!x y:real^N. orthogonal x y <=> vangle x y = pi / &2`,
  ...
  EQ_TAC THENL
   [SIMP_TAC[real_div; REAL_MUL_LZERO] THEN DISCH_TAC THEN
    ...
    MP_TAC(SPECL [`x:real^N`; `y:real^N`] NORM_CAUCHY_SCHWARZ_ABS) THEN
    GEN_REWRITE_TAC "100/cosine.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM REAL_MUL_LID] THEN
...
let LAW_OF_SINES = prove
 (`!A B C:real^N.
  ...
  GEN_REWRITE_TAC "100/cosine.ml:(RAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o LAND_CONV o ONCE_DEPTH_CONV) [DIST_SYM] THEN
  ...
  REWRITE_TAC[DOT_LSUB; DOT_RSUB; DOT_SYM] THEN CONV_TAC "100/cosine.ml:REAL_RING" REAL_RING);;
 ...
 (`!x. cos(x) = -- &1 /\ &0 <= x /\ x < &3 * pi ==> x = pi`,
  ...
  REWRITE_TAC[GSYM SIN_EQ_0] THENL
   [MP_TAC(SPEC `x:real` SIN_CIRCLE) THEN ASM_REWRITE_TAC[] THEN
    CONV_TAC "100/cosine.ml:REAL_RING" REAL_RING;
    ALL_TAC] THEN
