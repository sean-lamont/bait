 ...
 (`!A B C D:real^N.
  ...
  ASM_CASES_TAC `D:real^N = A` THENL
   [DISCH_TAC THEN ASM_SIMP_TAC[ANGLE_REFL] THEN
    GEN_REWRITE_TAC "100/isosceles.ml:LAND_CONV" LAND_CONV [GSYM ORTHOGONAL_LNEG] THEN
...
let ISOSCELES_TRIANGLE_3 = prove
 (`!A B C D:real^N.
  ...
  CONV_TAC "100/isosceles.ml:SYM_CONV" SYM_CONV THEN
  UNDISCH_TAC `angle(C:real^N,A,B) = angle (A,B,C)` THEN
  MATCH_MP_TAC EQ_IMP THEN BINOP_TAC THENL
   [MATCH_MP_TAC ANGLE_EQ_0_LEFT;
    GEN_REWRITE_TAC "100/isosceles.ml:RAND_CONV" RAND_CONV [ANGLE_SYM] THEN
 ...
 (`!A B C D:real^N.
  ...
  STRIP_TAC THENL
   [MP_TAC(ISPECL [`D:real^N`; `C:real^N`; `A:real^N`;
    ...
    THENL
     [CONJ_TAC THEN GEN_REWRITE_TAC "100/isosceles.ml:LAND_CONV" LAND_CONV [ANGLE_SYM] THEN
