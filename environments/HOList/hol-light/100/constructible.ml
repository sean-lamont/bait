...
let STEP_LEMMA = prove
 (`!P. (!n. P(&n)) /\
  ...
  SUBGOAL_THEN `A + B * s = &0` ASSUME_TAC THENL
   [REPEAT(FIRST_X_ASSUM(MP_TAC o SYM)) THEN CONV_TAC "100/constructible.ml:REAL_RING" REAL_RING; ALL_TAC] THEN
  ...
  CONV_TAC "100/constructible.ml:REAL_RING" REAL_RING);;
 ...
 (`!e. wellformed e /\ ~(radicals e = {})
  ...
  X_GEN_TAC `e1:expression` THEN TRY(X_GEN_TAC `e2:expression`) THENL
   [DISCH_THEN(fun th -> STRIP_TAC THEN MP_TAC th) THEN
    ...
    ASM_CASES_TAC `value b * sqrt(value r) = value a` THENL
     [ASM_REWRITE_TAC[] THEN STRIP_TAC THEN FIRST_X_ASSUM SUBST_ALL_TAC THEN
      ...
      REWRITE_TAC[RATIONAL_NUM; EMPTY_SUBSET; CONJ_ASSOC] THEN CONJ_TAC THENL
       [UNDISCH_TAC `~(value a + value a = &0)` THEN CONV_TAC "100/constructible.ml:REAL_FIELD" REAL_FIELD;
    ...
    FIRST_ASSUM(MP_TAC o MATCH_MP SQRT_POW_2) THEN CONV_TAC "100/constructible.ml:REAL_FIELD" REAL_FIELD;
    ...
    ASM_REWRITE_TAC[value; wellformed; radicals] THEN CONJ_TAC THENL
     [FIRST_ASSUM(MP_TAC o MATCH_MP SQRT_POW_2) THEN CONV_TAC "100/constructible.ml:REAL_RING" REAL_RING;
      ALL_TAC] THEN
    MP_TAC(SPEC `r:expression` NOT_IN_OWN_RADICALS) THEN
    MP_TAC(SPECL [`e1:expression`; `r:expression`] RADICALS_SUBSET) THEN
    MP_TAC(SPECL [`e2:expression`; `r:expression`] RADICALS_SUBSET) THEN
...
let RATIONAL_LOWEST_LEMMA = prove
 (`!p q. ~(q = 0) ==> ?p' q'. ~(q' = 0) /\ coprime(p',q') /\ p * q' = p' * q`,
  ...
  GEN_REWRITE_TAC "100/constructible.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [ARITH_RULE `a < b <=> 1 * a < b`] THEN
  ...
  CONV_TAC "100/constructible.ml:NUM_RING" NUM_RING);;
...
let RATIONAL_LOWEST = prove
 (`!x. rational x <=> ?p q. ~(q = 0) /\ coprime(p,q) /\ abs(x) = &p / &q`,
  ...
  REWRITE_TAC[GSYM REAL_OF_NUM_MUL] THEN CONV_TAC "100/constructible.ml:REAL_FIELD" REAL_FIELD);;
...
let RADICAL_LINEAR_EQUATION = prove
 (`!a b x. radical a /\ radical b /\ ~(a = &0 /\ b = &0) /\ a * x + b = &0
  ...
  REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "100/constructible.ml:REAL_FIELD" REAL_FIELD);;
 ...
 (`!a b c x. radical a /\ radical b /\ radical c /\
  ...
  MP_TAC THENL
   [REWRITE_TAC[real_sub; REAL_ARITH `a + (b + c) = &0 <=> c = --(a + b)`] THEN
    ...
    FIRST_X_ASSUM(MP_TAC o SYM) THEN CONV_TAC "100/constructible.ml:REAL_RING" REAL_RING;
    STRIP_TAC THENL
...
let RADICAL_SIMULTANEOUS_LINEAR_QUADRATIC = prove
 (`!a b c d e f x.
  ...
  (REPLICATE_TAC 3
   ...
   CONJ_TAC THENL
    [REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "100/constructible.ml:REAL_RING" REAL_RING; ALL_TAC] THEN
...
let RADICAL_SIMULTANEOUS_QUADRATIC_QUADRATIC = prove
 (`!a b c d e f x.
  ...
  REPLICATE_TAC 3
   (CONJ_TAC THENL [RADICAL_TAC THEN ASM_REWRITE_TAC[]; ALL_TAC]) THEN
  REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "100/constructible.ml:REAL_RING" REAL_RING);;
...
let COS_TRIPLE = prove
 (`!x. cos(&3 * x) = &4 * cos(x) pow 3 - &3 * cos(x)`,
  ...
  MP_TAC(SPEC `x:real` SIN_CIRCLE) THEN CONV_TAC "100/constructible.ml:REAL_RING" REAL_RING);;
...
let TRISECT_60_DEGREES_ALGEBRA = prove
 (`~(?x. radical x /\ x pow 3 - &3 * x - &1 = &0)`,
  ...
  CONV_TAC "100/constructible.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[GSYM REAL_OF_NUM_ADD] THEN
