...
let VARIATION_SPLIT = prove
 (`!a s n.
  ...
  CONV_TAC "100/descartes.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_UNION_EQ THEN
 ...
 (`!n a b.
  ...
  THENL
   [FIRST_X_ASSUM SUBST_ALL_TAC THEN EXISTS_TAC `1` THEN
    CONV_TAC "100/descartes.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    CONV_TAC "100/descartes.ml:(ONCE_DEPTH_CONV NUMSEG_CONV)" (ONCE_DEPTH_CONV NUMSEG_CONV) THEN
    ...
    CONV_TAC "100/descartes.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN COND_CASES_TAC THEN REWRITE_TAC[] THEN
    ASM_ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN `?p. 1 < p /\ p <= n /\ ~(a p = &0)` MP_TAC THENL
   [ASM_MESON_TAC[ARITH_RULE `2 <= n ==> 1 < n`; LE_REFL];
    GEN_REWRITE_TAC "100/descartes.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
   ...
   [X_GEN_TAC `m:num` THEN MATCH_MP_TAC EQ_TRANS THEN
    EXISTS_TAC `sum(0..m+1) a` THEN CONJ_TAC THENL
     [SIMP_TAC[SUM_CLAUSES_LEFT; LE_0; ARITH_RULE `0 + 1 <= n + 1`] THEN
      CONV_TAC "100/descartes.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[REAL_ADD_ASSOC] THEN
  ...
  ASM_CASES_TAC `a 0 + a 1 = &0` THENL
   [SUBGOAL_THEN `!i. 0 < i /\ i < p ==> b i = &0` ASSUME_TAC THENL
    ...
    SUBGOAL_THEN `(b:num->real) p = a p` ASSUME_TAC THENL
     [FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "100/descartes.ml:LAND_CONV" LAND_CONV [GSYM th]) THEN
     ...
     [MATCH_MP_TAC EQ_TRANS THEN
      EXISTS_TAC `variation(0..p) b + variation(p..n) b` THEN CONJ_TAC THENL
       [MATCH_MP_TAC VARIATION_SPLIT_NUMSEG THEN REWRITE_TAC[IN_NUMSEG] THEN
        ...
        FIRST_X_ASSUM MATCH_MP_TAC THEN ASM_ARITH_TAC;
        BINOP_TAC THENL [ALL_TAC; CONV_TAC "100/descartes.ml:SYM_CONV" SYM_CONV] THEN
     ...
     [MATCH_MP_TAC EQ_TRANS THEN
      EXISTS_TAC `variation(0..1) b + variation(1..n) b` THEN CONJ_TAC THENL
       [MATCH_MP_TAC VARIATION_SPLIT_NUMSEG THEN REWRITE_TAC[IN_NUMSEG] THEN
        ...
        CONV_TAC "100/descartes.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ASM_REWRITE_TAC[];
      ...
      SUBGOAL_THEN `(b:num->real) 1 = a 0 + a 1` ASSUME_TAC THENL
       [FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "100/descartes.ml:LAND_CONV" LAND_CONV [GSYM th]) THEN
        SIMP_TAC[num_CONV `1`; SUM_CLAUSES_NUMSEG] THEN
        CONV_TAC "100/descartes.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
        ALL_TAC]]] THEN
  REPEAT(FIRST_X_ASSUM(MP_TAC o SPEC `0`)) THEN CONV_TAC "100/descartes.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  EXPAND_TAC "a'" THEN CONV_TAC "100/descartes.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ASM_SIMP_TAC[ARITH_RULE `1 < p ==> ~(p = 1)`; REAL_LT_REFL] THEN
  REPEAT(COND_CASES_TAC THEN ASM_REWRITE_TAC[]) THEN
  CONV_TAC "100/descartes.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  CONV_TAC "100/descartes.ml:(BINDER_CONV(RAND_CONV(RAND_CONV INT_POLY_CONV)))" (BINDER_CONV(RAND_CONV(RAND_CONV INT_POLY_CONV))) THEN
...
let ODD_VARIATION_POSITIVE_ROOT = prove
 (`!a n. ODD(variation(0..n) a)
  ...
  GEN_REWRITE_TAC "100/descartes.ml:LAND_CONV" LAND_CONV [num_WOP] THEN REWRITE_TAC[num_MAX] THEN
...
let MULTIPLICITY_UNIQUE = prove
 (`!f a r b m k.
  ...
  CONV_TAC "100/descartes.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SELECT_UNIQUE THEN
...
let MULTIPLICITY_WORKS = prove
 (`!r n a. 
  ...
  REWRITE_TAC[multiplicity] THEN CONV_TAC "100/descartes.ml:(ONCE_DEPTH_CONV SELECT_CONV)" (ONCE_DEPTH_CONV SELECT_CONV) THEN
...
let MULTIPLICITY_OTHER_ROOT = prove
 (`!a n r s.
  ...
  CONV_TAC "100/descartes.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MULTIPLICITY_UNIQUE THEN
  REWRITE_TAC[] THEN
 ...
 (`!r n a.
  ...
  ASM_CASES_TAC `sum(0..n-1) (\i. c i * r pow i) = &0` THENL
   [FIRST_X_ASSUM(MP_TAC o SPEC `n - 1`) THEN
    ...
    REPEAT(CONJ_TAC THENL [ASM_ARITH_TAC; ALL_TAC]) THEN
    EXISTS_TAC `d - 1`] THEN
  UNDISCH_TAC `ODD d` THEN GEN_REWRITE_TAC "100/descartes.ml:LAND_CONV" LAND_CONV [ODD_EXISTS] THEN
...
let DESCARTES_RULE_OF_SIGNS = prove
 (`!f a n. f = (\x. sum(0..n) (\i. a i * x pow i)) /\ 
  ...
  FIRST_X_ASSUM(fun t -> GEN_REWRITE_TAC "100/descartes.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [t]) THEN
  CONV_TAC "100/descartes.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MULTIPLICITY_OTHER_ROOT THEN
