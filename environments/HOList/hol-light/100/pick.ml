...
let TRIANGLE_ADDITIVE_DECOMPOSITION = prove
 (`!f:(real^2->bool)->real a b c d.
  ...
  ASM (CONV_TAC "100/pick.ml:o" o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 5)
 ...
 (`!f:real^N->real^N.
  ...
  STRIP_ASSUME_TAC THENL
   [MATCH_MP_TAC LINEAR_BIJECTIVE_LEFT_RIGHT_INVERSE THEN ASM_SIMP_TAC[] THEN
    ...
    GEN_TAC THEN GEN_REWRITE_TAC "100/pick.ml:LAND_CONV" LAND_CONV [GSYM BASIS_EXPANSION] THEN
...
let PICK_TRIANGLE_ALT = prove
 (`!a b c:real^2.
    ...
    (REPEAT_TCL CONJUNCTS_THEN SUBST1_TAC) THENL
     [REPEAT CONJ_TAC THEN
      (FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (SET_RULE
       ...
       CONV_TAC "100/pick.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "100/pick.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV];
     ALL_TAC] THEN
  SUBGOAL_THEN
  ...
  CONV_TAC "100/pick.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  REWRITE_TAC[SET_RULE `{x | x IN s /\ P x} = s INTER P`] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[REAL_OF_NUM_ADD; REAL_OF_NUM_EQ] THEN
    ...
    SIMP_TAC[NOT_IN_EMPTY; EMPTY_GSPEC; CARD_CLAUSES; FINITE_INSERT;
             FINITE_EMPTY] THEN CONV_TAC "100/pick.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    REWRITE_TAC[SET_RULE `{x | x IN s /\ P x} = s INTER P`] THEN
 ...
 (`!a b c d p x:real^2.
  ...
  ASSUME_TAC THENL
   [ALL_TAC;
    ...
    GEN_REWRITE_TAC "100/pick.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
     ...
     [EXISTS_TAC `segment[x:real^2,d] DELETE x` THEN
      ...
      ASM_REWRITE_TAC[IN_DELETE; ENDS_IN_SEGMENT] THEN REPEAT CONJ_TAC THENL
       [FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP (SET_RULE
        ...
        REWRITE_TAC[REAL_LE_MIN] THEN CONV_TAC "100/pick.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let PATH_IMAGE_POLYGONAL_PATH_SUBSET_SEGMENTS = prove
 (`!p x:real^N.
  ...
  GEN_REWRITE_TAC "100/pick.ml:LAND_CONV" LAND_CONV [IN_UNION] THEN STRIP_TAC THENL
...
let SET_OF_LIST_POLYGONAL_PATH_ROTATE = prove
 (`!p. ~(p = []) ==> set_of_list(CONS (LAST p) (BUTLAST p)) = set_of_list p`,
  REPEAT STRIP_TAC THEN
  FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/pick.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
...
let PATH_IMAGE_POLYGONAL_PATH_ROTATE = prove
 (`!p:(real^N)list.
  ...
  GEN_REWRITE_TAC "100/pick.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [polygonal_path] THEN
...
let POLYGON_CHOP_IN_TWO = prove
 (`!p:(real^2)list.
  ...
  GEN_REWRITE_TAC "100/pick.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [EXTENSION] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [REPEAT STRIP_TAC THEN REWRITE_TAC[REAL_LT_LE] THEN
    CONV_TAC "100/pick.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
  ...
  MP_TAC THENL
   [MATCH_MP_TAC SUBSET_TRANS THEN
    ...
    ASM_SIMP_TAC[SUBSET; IN_SET_OF_LIST; real_ge; IN_ELIM_THM; REAL_LT_IMP_LE;
                 REAL_LE_REFL];
    GEN_REWRITE_TAC "100/pick.ml:LAND_CONV" LAND_CONV [SUBSET] THEN
  ...
  MP_TAC THENL
   [MATCH_MP_TAC CONNECTED_INTER_FRONTIER THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "100/pick.ml:LAND_CONV" LAND_CONV [GSYM MEMBER_NOT_EMPTY] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [FIRST_X_ASSUM(X_CHOOSE_THEN `mx:real^2` STRIP_ASSUME_TAC) THEN
    SUBGOAL_THEN `n dot (mx - b:real^2) <= n dot (c - b)` MP_TAC THENL
     [ASM SET_TAC[]; ALL_TAC] THEN
    GEN_REWRITE_TAC "100/pick.ml:LAND_CONV" LAND_CONV [REAL_LE_LT] THEN STRIP_TAC THENL
     ...
     [REWRITE_TAC[INTER; GSYM(ASSUME
      ...
      CONJ_TAC THENL
       [EXISTS_TAC `min (&1 / &2) (e / &2 / norm (z - b:real^2))` THEN
        ...
        REWRITE_TAC[REAL_LT_MIN] THEN CONV_TAC "100/pick.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
   ...
   [SUBGOAL_THEN `~(u:real^2 = a) /\ ~(u = c)` STRIP_ASSUME_TAC THENL
    ...
    ANTS_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      ...
      CONV_TAC "100/pick.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
      ALL_TAC] THEN
    GEN_REWRITE_TAC "100/pick.ml:LAND_CONV" LAND_CONV [EXTENSION] THEN
    ...
    STRIP_ASSUME_TAC THENL
     [EXISTS_TAC
      ...
      CONV_TAC "100/pick.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REPEAT CONJ_TAC THENL
       [ASM_CASES_TAC `r:real = rx` THENL
        ...
        CONV_TAC "100/pick.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
        COND_CASES_TAC THEN ASM_SIMP_TAC[REAL_LE_DIV; REAL_ABS_POS] THEN
        CONV_TAC "100/pick.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
        ...
        COND_CASES_TAC THEN ASM_SIMP_TAC[REAL_LE_DIV; REAL_ABS_POS] THEN
        CONV_TAC "100/pick.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
        COND_CASES_TAC THEN CONV_TAC "100/pick.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let PICK = prove
 (`!p:(real^2)list.
  ...
  SUBST1_TAC THENL
   [(CONV_TAC "100/pick.ml:o" o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 5)
  ...
  SUBST1_TAC THENL
   [FIRST_X_ASSUM(MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
    ...
    ALL_TAC] THEN
  SIMP_TAC[CARD_CLAUSES; FINITE_INSERT; FINITE_EMPTY] THEN
  ASM_REWRITE_TAC[IN_INSERT; NOT_IN_EMPTY] THEN CONV_TAC "100/pick.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  MATCH_MP_TAC(REAL_ARITH
