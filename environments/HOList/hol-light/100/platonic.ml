...
let REAL_RAT5_MUL_CONV =
  let pth = prove
    (`(a1 + b1 * sqrt(&5)) * (a2 + b2 * sqrt(&5)) =
     ...
     MP_TAC(ISPEC `&5` SQRT_POW_2) THEN CONV_TAC "100/platonic.ml:REAL_FIELD" REAL_FIELD) in
...
let REAL_RAT5_INV_CONV =
  let pth = prove
   (`~(a pow 2 = &5 * b pow 2)
    ...
    REPEAT GEN_TAC THEN
    GEN_REWRITE_TAC "100/platonic.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [GSYM REAL_SUB_0] THEN
    SUBGOAL_THEN
     `a pow 2 - &5 * b pow 2 = (a + b * sqrt(&5)) * (a - b * sqrt(&5))`
    SUBST1_TAC THENL
     [MP_TAC(SPEC `&5` SQRT_POW_2) THEN CONV_TAC "100/platonic.ml:REAL_FIELD" REAL_FIELD;
      REWRITE_TAC[REAL_ENTIRE; DE_MORGAN_THM] THEN CONV_TAC "100/platonic.ml:REAL_FIELD" REAL_FIELD]) in
  fun tm ->
  ...
  let lemma = prove
   (`!x y. x <= y * sqrt(&5) <=>
    ...
    REWRITE_TAC[REAL_POS] THEN DISCH_THEN(fun th ->
      GEN_REWRITE_TAC "100/platonic.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [SYM th]) THEN
...
let STD_DODECAHEDRON = prove
 (`std_dodecahedron =
  ...
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN
  ...
  CONV_TAC "100/platonic.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[]);;
   ...
   [STRIP_TAC THEN
    ...
    SUBGOAL_THEN `(u:real^3->bool) HAS_SIZE 3` MP_TAC THENL
     [ASM_SIMP_TAC[HAS_SIZE; AFFINE_INDEPENDENT_IMP_FINITE] THEN
      ...
      ALL_TAC] THEN
    CONV_TAC "100/platonic.ml:(LAND_CONV HAS_SIZE_CONV)" (LAND_CONV HAS_SIZE_CONV) THEN SIMP_TAC[LEFT_IMP_EXISTS_THM] THEN
    ...
    MP_TAC THENL
     [GEN_REWRITE_TAC "100/platonic.ml:LAND_CONV" LAND_CONV [GSYM AFF_DIM_AFFINE_HULL] THEN
      FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/platonic.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV)
       [SYM th]) THEN
    ...
    REPEAT(COND_CASES_TAC THEN CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV) THEN
     ...
     [RULE_ASSUM_TAC(REWRITE_RULE[INTER_UNIV]) THEN
      SUBGOAL_THEN `s SUBSET {x:real^3 | a dot x = b}` ASSUME_TAC THENL
       [MATCH_MP_TAC SUBSET_TRANS THEN
        ...
        FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/platonic.ml:LAND_CONV" LAND_CONV [SYM th]) THEN
     ...
     [MATCH_MP_TAC(TAUT `(~p /\ ~q ==> F) ==> p \/ q`) THEN
      ...
      MATCH_MP_TAC SUBSET_ANTISYM THEN CONJ_TAC THENL
       [MATCH_MP_TAC HULL_MONO THEN REWRITE_TAC[SUBSET_INTER] THEN
        ...
        EXISTS_TAC `convex hull t:real^3->bool` THEN
        REWRITE_TAC[HULL_SUBSET] THEN ASM SET_TAC[];
        FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/platonic.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
       ...
       [MATCH_MP_TAC INT_LE_TRANS THEN
        ...
        CONJ_TAC THENL
         [SUBGOAL_THEN `~collinear{x:real^3,y,z}` MP_TAC THENL
          ...
          CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV;
...
let COMPUTE_FACES_2_STEP_2 = prove
 (`!f u v s:real^3->bool.
  ...
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN SUBST1_TAC
...
let COMPUTE_FACES_TAC =
  ...
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV VECTOR3_SUB_CONV)" (ONCE_DEPTH_CONV VECTOR3_SUB_CONV) THEN
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV VECTOR3_CROSS_CONV)" (ONCE_DEPTH_CONV VECTOR3_CROSS_CONV) THEN
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV VECTOR3_EQ_0_CONV)" (ONCE_DEPTH_CONV VECTOR3_EQ_0_CONV) THEN
  REWRITE_TAC[real_ge] THEN
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV VECTOR3_DOT_CONV)" (ONCE_DEPTH_CONV VECTOR3_DOT_CONV) THEN
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV REAL_RAT5_LE_CONV)" (ONCE_DEPTH_CONV REAL_RAT5_LE_CONV) THEN
  REWRITE_TAC[INSERT_AC] THEN REWRITE_TAC[DISJ_ACI] THEN
  REPEAT(CHANGED_TAC
   (ONCE_REWRITE_TAC[lemma] THEN
    CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV (LAND_CONV VECTOR3_DOT_CONV THENC REAL_RAT5_EQ_CONV))" (ONCE_DEPTH_CONV (LAND_CONV VECTOR3_DOT_CONV THENC REAL_RAT5_EQ_CONV))) THEN
    REWRITE_TAC[]) THEN
     ...
     [MATCH_MP_TAC(TAUT `(~p /\ ~q ==> F) ==> p \/ q`) THEN
      ...
      MP_TAC THENL
       [ASM_REWRITE_TAC[] THEN REWRITE_TAC[IN_ELIM_THM] THEN
        ...
        MATCH_MP_TAC(REAL_ARITH `a < b ==> a = b ==> F`) THEN
        MATCH_MP_TAC REAL_CONVEX_BOUND_LT THEN ASM_REAL_ARITH_TAC];
      FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "100/platonic.ml:LAND_CONV" LAND_CONV [SYM th]) THEN
      MATCH_MP_TAC SUBSET_ANTISYM THEN CONJ_TAC THENL
       [MATCH_MP_TAC HULL_MONO THEN REWRITE_TAC[SUBSET_INTER] THEN
        ASM_REWRITE_TAC[] THEN MATCH_MP_TAC SUBSET_TRANS THEN
        EXISTS_TAC `convex hull t:real^3->bool` THEN
...
let EDGES_PER_FACE_TAC th =
  ...
  CONJ_TAC THENL
   [AP_TERM_TAC THEN GEN_REWRITE_TAC "100/platonic.ml:I" I [EXTENSION] THEN
  ...
  (MATCH_MP_TAC CARD_EQ_LEMMA THEN REPEAT CONJ_TAC THENL
    [CONV_TAC "100/platonic.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN NO_TAC;
     ...
     ALL_TAC]) THEN
  CONV_TAC "100/platonic.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[CONJUNCT1 HAS_SIZE_CLAUSES];;
...
let POLYTOPE_FULLDIM_TAC =
  ...
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV VECTOR3_SUB_CONV)" (ONCE_DEPTH_CONV VECTOR3_SUB_CONV) THEN
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV VECTOR3_CROSS_CONV)" (ONCE_DEPTH_CONV VECTOR3_CROSS_CONV) THEN
  CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN CONJ_TAC THENL
   [CONV_TAC "100/platonic.ml:(RAND_CONV VECTOR3_EQ_0_CONV)" (RAND_CONV VECTOR3_EQ_0_CONV) THEN REWRITE_TAC[];
    CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV VECTOR3_DOT_CONV)" (ONCE_DEPTH_CONV VECTOR3_DOT_CONV) THEN
    REWRITE_TAC[EXISTS_IN_INSERT; NOT_IN_EMPTY] THEN
    CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV VECTOR3_DOT_CONV)" (ONCE_DEPTH_CONV VECTOR3_DOT_CONV) THEN
    CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV REAL_RAT5_EQ_CONV)" (ONCE_DEPTH_CONV REAL_RAT5_EQ_CONV) THEN
    REWRITE_TAC[]];;
   ...
   [EQ_TAC THENL [STRIP_TAC; MESON_TAC[FACE_OF_TRANS]] THEN
    ...
    ANTS_TAC THENL
     [ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
      ...
      REWRITE_TAC[fulldim] THEN CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN
      ASM_MESON_TAC[FACE_OF_FACE]];
    REWRITE_TAC[facets] THEN
    REWRITE_TAC[TAUT `(a \/ b) /\ c <=> a /\ c \/ b /\ c`] THEN
    REWRITE_TAC[EXISTS_OR_THM; UNWIND_THM2] THEN
    CONV_TAC "100/platonic.ml:(LAND_CONV(DEPTH_BINOP_CONV `\/` (fun tm -> REWR_CONV (COMPUTE_EDGES_CONV(rand(rand(lhand tm)))) tm)))" (LAND_CONV(DEPTH_BINOP_CONV `\/` (fun tm -> REWR_CONV (COMPUTE_EDGES_CONV(rand(rand(lhand tm)))) tm))) THEN
    REWRITE_TAC[INSERT_AC] THEN REWRITE_TAC[DISJ_ACI]];;
    ...
    ANTS_TAC THENL
     [REPEAT CONJ_TAC THENL
       [MATCH_MP_TAC FACE_OF_POLYHEDRON_POLYHEDRON THEN
        ...
        CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV];
      MATCH_MP_TAC MONO_EXISTS THEN REWRITE_TAC[facet_of] THEN
      ASM_REWRITE_TAC[fulldim] THEN CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN
...
let SIZE_ZERO_DIMENSIONAL_FACES = prove
 (`!s:real^N->bool.
  ...
  SUBST1_TAC THENL
   [CONV_TAC "100/platonic.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
 ...
 (`!p:real^3->bool m n.
    ...
    X_GEN_TAC `e:real^3->bool` THEN STRIP_TAC THENL
     [MP_TAC(ISPECL [`p:real^3->bool`; `e:real^3->bool`]
      ...
      ASM_SIMP_TAC[POLYTOPE_IMP_POLYHEDRON] THEN ANTS_TAC THENL
       [CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN DISCH_THEN SUBST_ALL_TAC THEN
        RULE_ASSUM_TAC(REWRITE_RULE[AFF_DIM_EMPTY]) THEN ASM_INT_ARITH_TAC;
        CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN
        ...
        REPEAT CONJ_TAC THENL
         [DISCH_THEN SUBST_ALL_TAC THEN
          ...
          SUBGOAL_THEN `(b:real^3->bool) HAS_SIZE 2` MP_TAC THENL
           [ASM_REWRITE_TAC[HAS_SIZE]; CONV_TAC "100/platonic.ml:(LAND_CONV HAS_SIZE_CONV)" (LAND_CONV HAS_SIZE_CONV)] THEN
  ...
  ASSUME_TAC THENL
   [ASM_SIMP_TAC[SIZE_ZERO_DIMENSIONAL_FACES; POLYTOPE_IMP_POLYHEDRON] THEN
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN
  ...
  ASSUME_TAC THENL
   [MP_TAC(ISPEC `p:real^3->bool` POLYTOPE_FACET_LOWER_BOUND) THEN
    ...
    GEN_REWRITE_TAC "100/platonic.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_ELIM_THM] THEN
    CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN GEN_TAC THEN EQ_TAC THEN
  ...
  SUBGOAL_THEN `3 <= m` ASSUME_TAC THENL
   [ASM_CASES_TAC `{f:real^3->bool | f face_of p /\ aff_dim f = &2} = {}` THENL
    ...
    ASM_REWRITE_TAC[facet_of] THEN CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN
    ...
    GEN_REWRITE_TAC "100/platonic.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_ELIM_THM] THEN
    CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN X_GEN_TAC `e:real^3->bool` THEN
    EQ_TAC THEN ASM_CASES_TAC `e:real^3->bool = {}` THEN
    ASM_SIMP_TAC[AFF_DIM_EMPTY] THEN CONV_TAC "100/platonic.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THENL
     [ASM_MESON_TAC[FACE_OF_TRANS; FACE_OF_IMP_SUBSET];
      ASM_MESON_TAC[FACE_OF_FACE]];
    ALL_TAC] THEN
  ...
  ASM_REWRITE_TAC[] THEN CONV_TAC "100/platonic.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ASM_ARITH_TAC);;
...
let MATRIX_BY_CRAMER_LEMMA = prove
 (`!A:real^3^3.
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "100/platonic.ml:LAND_CONV" LAND_CONV [MATRIX_LEMMA] THEN
        ...
        ==> (A ** x1 = x2 /\
             ...
             A ** z1 = z2 <=>
               ...
               (x1$1 * y1$2 * z2$3 +
                ...
                x1$2 * y1$1 * z2$3 -
                x2$3 * y1$2 * z1$1) / d)`,
  REPEAT GEN_TAC THEN CONV_TAC "100/platonic.ml:let_CONV" let_CONV THEN DISCH_TAC THEN
  ...
  W(fun (asl,w) ->
        ...
        MATCH_MP_TAC CONGRUENT_SIMPLE THEN EXISTS_TAC matt THEN CONJ_TAC THENL
         [REWRITE_TAC[ORTHOGONAL_MATRIX; CART_EQ] THEN
          ...
          CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV REAL_RAT5_MUL_CONV)" (ONCE_DEPTH_CONV REAL_RAT5_MUL_CONV) THEN
          CONV_TAC "100/platonic.ml:(DEPTH_CONV REAL_RAT5_ADD_CONV)" (DEPTH_CONV REAL_RAT5_ADD_CONV) THEN
          CONV_TAC "100/platonic.ml:(ONCE_DEPTH_CONV REAL_RAT5_EQ_CONV)" (ONCE_DEPTH_CONV REAL_RAT5_EQ_CONV) THEN
          REWRITE_TAC[] THEN NO_TAC;
