...
let EQUIVALENCE_UNIFORM_PARTITION = prove
 (`!R s k. FINITE s /\
  ...
  CONV_TAC "100/friendship.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_UNION_EQ THEN ASM SET_TAC[]);;
...
let SHIFTABLE_LOCAL = prove
 (`!x y p r. cycle r p x /\ cycle r p y /\ ~(p = 0)
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/friendship.ml:(BINDER_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (BINDER_CONV o LAND_CONV o ONCE_DEPTH_CONV) [th]) THEN
...
let FRIENDSHIP = prove
 (`!friend:person->person->bool.
  ...
  GEN_REWRITE_TAC "100/friendship.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
  ...
  ASSUME_TAC THENL
   [REPEAT STRIP_TAC THEN ASM_CASES_TAC `x:person = y` THENL
    ...
    MATCH_MP_TAC EQ_IMP_LE THEN CONV_TAC "100/friendship.ml:SYM_CONV" SYM_CONV THEN
  ...
  GEN_REWRITE_TAC "100/friendship.ml:I" I [TAUT `p <=> ~ ~ p`] THEN
  GEN_REWRITE_TAC "100/friendship.ml:RAND_CONV" RAND_CONV [NOT_EXISTS_THM] THEN
  ...
  SUBGOAL_THEN `~(m = 2)` ASSUME_TAC THENL
   [DISCH_THEN SUBST_ALL_TAC THEN
    ...
    CONV_TAC "100/friendship.ml:(LAND_CONV HAS_SIZE_CONV)" (LAND_CONV HAS_SIZE_CONV) THEN REWRITE_TAC[NOT_EXISTS_THM] THEN
    ...
    CONV_TAC "100/friendship.ml:(LAND_CONV HAS_SIZE_CONV)" (LAND_CONV HAS_SIZE_CONV) THEN
  ...
  ASM_SIMP_TAC[ARITH_RULE `~(m = 0) ==> m - 1 + 1 = m`] THEN CONJ_TAC THENL
   [MP_TAC(ISPECL[`friend:person->person->bool`; `p:num`] HAS_SIZE_CYCLES) THEN
    ...
    SUBGOAL_THEN `p = (p - 2) + 2` (fun th ->
      GEN_REWRITE_TAC "100/friendship.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [th])
