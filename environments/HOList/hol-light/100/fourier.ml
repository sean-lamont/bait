 ...
 (`!f a. f real_integrable_on real_interval[--a,a]
  ...
  REPEAT STRIP_TAC THEN ASM_CASES_TAC `&0 <= a` THENL
   [MP_TAC(SPECL [`f:real->real`; `--a:real`; `a:real`; `&0:real`]
    ...
    GEN_REWRITE_TAC "100/fourier.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_INTEGRAL_REFLECT] THEN
...
let SQUARE_INTEGRABLE_0 = prove
 (`!s. (\x. &0) square_integrable_on s`,
  ...
  CONV_TAC "100/fourier.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_INTEGRABLE_0]);;
...
let SQUARE_INTEGRABLE_IMP_ABSOLUTELY_INTEGRABLE_PRODUCT = prove
 (`!f g s. f square_integrable_on s /\ g square_integrable_on s
  ...
  CONV_TAC "100/fourier.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let SQUARE_INTEGRABLE_IMP_ABSOLUTELY_INTEGRABLE = prove
 (`!f s. f square_integrable_on s /\ real_measurable s
  ...
  CONV_TAC "100/fourier.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let L2NORM_LNORM = prove
 (`!f s. f square_integrable_on s
  ...
  REWRITE_TAC[REAL_POW2_ABS] THEN CONV_TAC "100/fourier.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let L2NORM_SUB = prove
 (`!f g s.  f square_integrable_on s /\ g square_integrable_on s
  ...
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "100/fourier.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [GSYM REAL_NEG_SUB] THEN
...
let L2_SUMMABLE = prove
 (`!f s t.
  ...
  CONV_TAC "100/fourier.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ANTS_TAC THENL
...
let ORTHONORMAL_OPTIMAL_PARTIAL_SUM = prove
 (`!s w a f t.
  ...
  REPEAT STRIP_TAC THEN
  ASM (CONV_TAC "100/fourier.ml:o" o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 5)
...
let FOURIER_SERIES_L2_SUMMABLE = prove
 (`!s w f t.
  ...
  GEN_REWRITE_TAC "100/fourier.ml:I" I [SWAP_FORALL_THM] THEN MATCH_MP_TAC WLOG_LE THEN
...
let FOURIER_SERIES_L2_SUMMABLE_STRONG = prove
 (`!s w f t.
  ...
  GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [GSYM REAL_ADD_LID] THEN
  MATCH_MP_TAC REALLIM_ADD THEN CONJ_TAC THENL
   [MATCH_MP_TAC REALLIM_EVENTUALLY THEN
    ...
    GEN_REWRITE_TAC "100/fourier.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [EQ_SYM_EQ] THEN
...
let HAS_REAL_INTEGRAL_SIN_NX = prove
 (`!n. ((\x. sin(&n * x)) has_real_integral &0) (real_interval[--pi,pi])`,
  ...
  CONV_TAC "100/fourier.ml:REAL_FIELD" REAL_FIELD);;
...
let REAL_INTEGRAL_SIN_AND_COS = prove
 (`!m n. real_integral (real_interval[--pi,pi])
  ...
  GEN_REWRITE_TAC "100/fourier.ml:I" I [SWAP_FORALL_THM] THEN
...
let WEIERSTRASS_TRIG_POLYNOMIAL = prove
 (`!f e. f real_continuous_on real_interval[--pi,pi] /\
     ...
     [ALL_TAC;
      ...
      CONJ_TAC THENL
       [X_GEN_TAC `w:complex` THEN ASM_CASES_TAC `w = Cx(&0)` THEN
        ...
        CONJ_TAC THENL
         [GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN CONTINUOUS_TAC;
  ...
  ASSUME_TAC THENL
   [REPEAT GEN_TAC THEN
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/fourier.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [SYM
         th]) THEN
    REWRITE_TAC[] THEN
     ...
     [GEN_TAC THEN MATCH_MP_TAC REAL_INTEGRABLE_CONTINUOUS THEN
      ...
      ASM_CASES_TAC `x:real = a` THEN ASM_REWRITE_TAC[] THENL
       [ASM_REWRITE_TAC[REAL_LE_ADDR; REAL_LE_INV_EQ;
        ...
        COND_CASES_TAC THEN CONV_TAC "100/fourier.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let WEIERSTRASS_L2_TRIGONOMETRIC_SET = prove
 (`!f e. f square_integrable_on real_interval[--pi,pi] /\ &0 < e
  ...
  GEN_REWRITE_TAC "100/fourier.ml:RAND_CONV" RAND_CONV [REAL_ADD_SYM] THEN BINOP_TAC THENL
 ...
 (`!f. f absolutely_real_integrable_on real_interval[--pi,pi]
  ...
  MATCH_MP_TAC REAL_INTEGRAL_ABS_BOUND_INTEGRAL THEN REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC REAL_INTEGRABLE_SUB THEN
    ...
    GEN_REWRITE_TAC "100/fourier.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
...
let RIEMANN_LEBESGUE_SIN = prove
 (`!f. f absolutely_real_integrable_on real_interval[--pi,pi]
  ...
  GEN_REWRITE_TAC "100/fourier.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
...
let RIEMANN_LEBESGUE_SIN_HALF = prove
 (`!f. f absolutely_real_integrable_on real_interval[--pi,pi]
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC ALWAYS_EVENTUALLY THEN X_GEN_TAC `n:num` THEN
    REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN CONV_TAC "100/fourier.ml:SYM_CONV" SYM_CONV THEN
...
let FOURIER_SUM_0 = prove
 (`!f n.
  ...
  CONJ_TAC THENL
   [CONV_TAC "100/fourier.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "100/fourier.ml:SYM_CONV" SYM_CONV THEN
...
let DIRICHLET_KERNEL_CONTINUOUS_STRONG = prove
 (`!n. (dirichlet_kernel n) real_continuous_on
  ...
  GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
  ...
  REWRITE_TAC[REAL_LT_01; REAL_SUB_RZERO] THEN REPEAT STRIP_TAC THENL
   [REAL_DIFF_TAC THEN REAL_ARITH_TAC;
    ...
    GEN_REWRITE_TAC "100/fourier.ml:(RATOR_CONV o RAND_CONV)" (RATOR_CONV o RAND_CONV) [GSYM REAL_DIV_1] THEN
...
let HAS_REAL_INTEGRAL_DIRICHLET_KERNEL = prove
 (`!n. (dirichlet_kernel n has_real_integral pi) (real_interval[--pi,pi])`,
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [th]) THEN
  MATCH_MP_TAC HAS_REAL_INTEGRAL_ADD THEN CONJ_TAC THENL
   [GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [REAL_ARITH  `pi = (&1 / &2) * (pi - --pi)`] THEN
...
let FOURIER_SUM_LIMIT_DIRICHLET_KERNEL = prove
 (`!f t l.
  ...
  SUBGOAL_THEN `l = (l * pi) / pi`
   (fun th -> GEN_REWRITE_TAC "100/fourier.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [th])
  THENL [MP_TAC PI_POS THEN CONV_TAC "100/fourier.ml:REAL_FIELD" REAL_FIELD; ALL_TAC] THEN
...
let REAL_SIN_X2_ZEROS = prove
 (`{x | sin(x / &2) = &0} = IMAGE (\n. &2 * pi * n) integer`,
  CONV_TAC "100/fourier.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
 ...
 (`!f d M a t.
  ...
  STRIP_ASSUME_TAC THENL
   [MP_TAC(REAL_DIFF_CONV
    ...
    CONV_TAC "100/fourier.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[COS_0; REAL_MUL_RID] THEN
    REWRITE_TAC[HAS_REAL_DERIVATIVE_ATREAL; REALLIM_ATREAL] THEN
    DISCH_THEN(MP_TAC o SPEC `&1 / &4`) THEN CONV_TAC "100/fourier.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    ...
    SUBST1_TAC THENL
     [REWRITE_TAC[REAL_ABS_DIV; REAL_ABS_INV] THEN
      UNDISCH_TAC `~(x = &0)` THEN CONV_TAC "100/fourier.ml:REAL_FIELD" REAL_FIELD;
      ALL_TAC] THEN
    ...
    SIMP_TAC[REAL_ABS_POS; REAL_LE_DIV] THEN CONJ_TAC THENL
     [REWRITE_TAC[REAL_ABS_INV] THEN
      ...
      GEN_REWRITE_TAC "100/fourier.ml:(funpow 3 RAND_CONV)" (funpow 3 RAND_CONV) [GSYM REAL_POW_1] THEN
    ...
    REWRITE_TAC[PI_POS_LE] THEN ANTS_TAC THENL
     [CONJ_TAC THENL
       [MATCH_MP_TAC REAL_CONTINUOUS_ON_LMUL THEN
        ...
        REAL_DIFF_TAC THEN
        MAP_EVERY UNDISCH_TAC [`&0 < a`; `&0 < x`] THEN CONV_TAC "100/fourier.ml:REAL_FIELD" REAL_FIELD];
 ...
 (`!f t l.
        ...
        ==> (((\n. sum (0..n)
            ...
            ((\n. real_integral (real_interval[&0,pi])
             ...
             ---> &0) sequentially)`,
  REPEAT STRIP_TAC THEN ASM_SIMP_TAC[GSYM FOURIER_SUM_LIMIT_PAIR] THEN
  GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [REALLIM_NULL] THEN
  ...
  MP_TAC PI_POS THEN CONV_TAC "100/fourier.ml:REAL_FIELD" REAL_FIELD);;
 ...
 (`!f t l d.
  ...
  let lemma0 = prove
   (`!x. abs(sin(x) - x) <= abs(x) pow 3`,
    ...
    CONV_TAC "100/fourier.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  MP_TAC(ISPECL [`f:real->real`; `--pi`; `pi`; `t:real`]
        ABSOLUTELY_REAL_INTEGRABLE_PERIODIC_OFFSET) THEN
  ASM_REWRITE_TAC[REAL_ARITH `pi - --pi = &2 * pi`] THEN
  GEN_REWRITE_TAC "100/fourier.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [REAL_ADD_SYM] THEN
  DISCH_THEN(fun th -> ASSUME_TAC th THEN MP_TAC th) THEN
  GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [absolutely_real_integrable_on] THEN
  GEN_REWRITE_TAC "100/fourier.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
   ...
   [EXISTS_TAC `1` THEN X_GEN_TAC `n:num` THEN DISCH_TAC THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_MEASURABLE_ON_CASES THEN
      ...
      GEN_REWRITE_TAC "100/fourier.ml:(LAND_CONV)" (LAND_CONV) [GSYM ETA_AX] THEN REWRITE_TAC[] THEN
    ...
    REWRITE_TAC[GSYM real_div] THEN
    GEN_REWRITE_TAC "100/fourier.ml:(LAND_CONV o RAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV o RAND_CONV)
 ...
 (`!a b c.
  ...
  let lemma2 = prove
   (`!x. &0 <= x ==> sin(x) <= x`,
    ...
    CONV_TAC "100/fourier.ml:(TOP_DEPTH_CONV num_CONV)" (TOP_DEPTH_CONV num_CONV) THEN
    REWRITE_TAC[VSUM_CLAUSES_NUMSEG; GSYM CX_SIN] THEN
    CONV_TAC "100/fourier.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
     ...
     [MATCH_MP_TAC(TAUT `a /\ (a ==> b) ==> a /\ b`) THEN CONJ_TAC THENL
       [MATCH_MP_TAC REAL_MEASURABLE_BOUNDED_BY_INTEGRABLE_IMP_INTEGRABLE THEN
        ...
        REWRITE_TAC[REAL_INTEGRABLE_CONST] THEN CONJ_TAC THENL
         [MATCH_MP_TAC REAL_MEASURABLE_ON_DIV THEN REPEAT CONJ_TAC THENL
           [MATCH_MP_TAC INTEGRABLE_IMP_REAL_MEASURABLE THEN
            GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN REWRITE_TAC[lemma0];
            MATCH_MP_TAC CONTINUOUS_IMP_REAL_MEASURABLE_ON THEN
  ...
  let lemma5 = prove
   (`!a b. &0 <= a
    ...
    CONJ_TAC THENL
     [ASM_MESON_TAC[lemma4; REAL_LE_REFL];
      GEN_REWRITE_TAC "100/fourier.ml:RAND_CONV" RAND_CONV [REAL_ARITH `&2 = &4 / &2`] THEN
  ...
  REPEAT GEN_TAC THEN STRIP_TAC THEN ASM_CASES_TAC `a <= b` THENL
   [MP_TAC(ISPECL [`c * a:real`; `c * b:real`] lemma5) THEN
    ...
    GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [HAS_REAL_INTEGRAL_INTEGRAL] THEN
    ...
    STRIP_ASSUME_TAC THENL
     [MP_TAC(ISPECL [`h:real->real`; `&0`; `d:real`]
      ...
      SUBGOAL_THEN `l':real = l` SUBST_ALL_TAC THENL
       [CONV_TAC "100/fourier.ml:SYM_CONV" SYM_CONV THEN ONCE_REWRITE_TAC[GSYM REAL_SUB_0] THEN
        ...
        EXISTS_TAC `h:real->real` THEN ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
         [W(MP_TAC o PART_MATCH (lhs o rand) TRIVIAL_LIMIT_WITHIN_REALINTERVAL o
          ...
          GEN_REWRITE_TAC "100/fourier.ml:(RATOR_CONV o LAND_CONV)" (RATOR_CONV o LAND_CONV) [GSYM ETA_AX] THEN
...
let FOURIER_JORDAN_BOUNDED_VARIATION_SIMPLE = prove
 (`!f x. f has_bounded_real_variation_on real_interval[--pi,pi] /\
  ...
  EXISTS_TAC `&1` THEN ASM_REWRITE_TAC[REAL_LT_01] THEN CONJ_TAC THENL
   [FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I
     [HAS_BOUNDED_REAL_VARIATION_DARBOUX]) THEN
    STRIP_TAC THEN GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
...
let FEJER_KERNEL_CONTINUOUS_STRONG = prove
 (`!n. (fejer_kernel n) real_continuous_on
       real_interval(--(&2 * pi),&2 * pi)`,
  GEN_TAC THEN GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
 ...
 (`!f t l.
        ...
        ==> (((\n. sum(0..n-1) (\r. sum (0..2*r)
             ...
             ((\n. real_integral (real_interval[&0,pi])
              ...
              ---> &0) sequentially)`,
  REPEAT STRIP_TAC THEN ASM_SIMP_TAC[GSYM FOURIER_SUM_LIMIT_PAIR] THEN
  GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [REALLIM_NULL] THEN REWRITE_TAC[] THEN
  GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [GSYM(MATCH_MP REALLIM_NULL_RMUL_EQ PI_NZ)] THEN
...
let HAS_REAL_INTEGRAL_FEJER_KERNEL = prove
 (`!n. (fejer_kernel n has_real_integral (if n = 0 then &0 else pi))
       (real_interval[--pi,pi])`,
  GEN_TAC THEN GEN_REWRITE_TAC "100/fourier.ml:(RATOR_CONV o LAND_CONV)" (RATOR_CONV o LAND_CONV) [GSYM ETA_AX] THEN
  REWRITE_TAC[fejer_kernel] THEN
  ASM_CASES_TAC `n = 0` THEN ASM_REWRITE_TAC[HAS_REAL_INTEGRAL_0] THEN
  SUBGOAL_THEN `pi = sum(0..n-1) (\r. pi) / &n`
   (fun th -> GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [th])
  THENL
 ...
 (`!f x l r.
  ...
  MP_TAC THENL
   [MATCH_MP_TAC REALLIM_NULL_COMPARISON THEN
    ...
    GEN_REWRITE_TAC "100/fourier.ml:RAND_CONV" RAND_CONV [real_div] THEN
    ...
    MATCH_MP_TAC(TAUT `b /\ (b ==> a) ==> a /\ b`) THEN CONJ_TAC THENL
     [X_GEN_TAC `x:real` THEN REWRITE_TAC[IN_REAL_INTERVAL] THEN
      ...
      GEN_REWRITE_TAC "100/fourier.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
      REWRITE_TAC[real_div; GSYM REAL_MUL_ASSOC] THEN
      MATCH_MP_TAC REAL_LE_LMUL THEN REWRITE_TAC[REAL_ABS_POS] THEN
...
let FOURIER_FEJER_CESARO_SUMMABLE_SIMPLE = prove
 (`!f x l r.
        ...
        ==> ((\n. sum(0..n-1) (\m. sum (0..2*m)
            ...
            sequentially`,
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "100/fourier.ml:(RATOR_CONV o RAND_CONV)" (RATOR_CONV o RAND_CONV) [REAL_ARITH `x = (x + x) / &2`] THEN
