 ...
 (`!(A:real^N^N) B (C:real^N^N) n.
  ...
  MP_TAC THENL
   [SIMP_TAC[MATRIX_CMUL_SUB_LDISTRIB; MSUM_SUB; FINITE_NUMSEG;
    ...
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "100/cayley_hamilton.ml:ONCE_DEPTH_CONV" ONCE_DEPTH_CONV [GSYM th]) THEN
 ...
 (`!A:real^N^N x n.
  ...
  GEN_TAC THEN GEN_TAC THEN INDUCT_TAC THENL
   [REWRITE_TAC[MSUM_CLAUSES_NUMSEG; real_pow; SUB_0; mpow] THEN
    ...
    CONJ_TAC THENL
     [GEN_REWRITE_TAC "100/cayley_hamilton.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [MPOW_SUC] THEN
 ...
 (`!A:real^N^N. ?B.
  ...
  SIMP_TAC[MSUM_CLAUSES_NUMSEG; SUM_CLAUSES_NUMSEG; LE_0] THENL
   [EXISTS_TAC `(\i. mat 0):num->real^N^N` THEN
    CONV_TAC "100/cayley_hamilton.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[MSUM_CLAUSES_NUMSEG] THEN
