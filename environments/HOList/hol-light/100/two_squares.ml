...
let INVOLUTION_EVEN_STEP = prove
 (`FINITE(s) /\
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "100/two_squares.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [th]) THEN
...
let PRIME_SQUARE = prove
 (`!n. ~prime(n * n)`,
  ...
  GEN_REWRITE_TAC "100/two_squares.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [ARITH_RULE `n = n * 1`] THEN
...
let PRIME_XYZ_NONZERO = prove
 (`prime(x EXP 2 + 4 * y * z) ==> 0 < x /\ 0 < y /\ 0 < z`,
  CONV_TAC "100/two_squares.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
  ...
  FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN CONJ_TAC THENL
   [MESON_TAC[EXP_2; LE_SQUARE_REFL; ARITH_RULE `(a <= b ==> a <= b + c)`];
    CONJ_TAC THEN MATCH_MP_TAC(ARITH_RULE `y <= z ==> y <= x + z`) THENL
     [GEN_REWRITE_TAC "100/two_squares.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [MULT_SYM]; ALL_TAC] THEN
