...
let WILSON = prove
 (`!p. prime(p) ==> (FACT(p - 1) == p - 1) (mod p)`,
  ...
  ASM_CASES_TAC `p = 2` THENL
   [ASM_REWRITE_TAC[] THEN CONV_TAC "100/wilson.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  SUBGOAL_THEN `FACT(p - 1) = FACT(p - 2) * (p - 1)` SUBST1_TAC THENL
   [SUBGOAL_THEN `p - 1 = SUC(p - 2)`
    ...
    ASM_ARITH_TAC;
    ALL_TAC] THEN
  GEN_REWRITE_TAC "100/wilson.ml:LAND_CONV" LAND_CONV [ARITH_RULE `x = 1 * x`] THEN
   ...
   [ASM_CASES_TAC `y = 1` THEN
    ...
    SUBGOAL_THEN `x * p = x + x * (p - 1)` SUBST1_TAC THENL
     [REWRITE_TAC[LEFT_SUB_DISTRIB; MULT_CLAUSES] THEN
      ...
      GEN_REWRITE_TAC "100/wilson.ml:LAND_CONV" LAND_CONV [ARITH_RULE `x = x * 1`] THEN
...
let WILSON_EQ = prove
 (`!p. ~(p = 1) ==> (prime p <=> (FACT(p - 1) == p - 1) (mod p))`,
  ...
  ASM_CASES_TAC `n = 0` THEN ASM_REWRITE_TAC[] THENL
   [ASM_REWRITE_TAC[CONG_MOD_0] THEN CONV_TAC "100/wilson.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV; ALL_TAC] THEN
