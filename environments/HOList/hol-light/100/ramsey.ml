...
let INFINITE_MEMBER = prove(
  ...
  SUBGOAL_THEN `~(s:A->bool = {})` MP_TAC THENL
   [UNDISCH_TAC `INFINITE (s:A->bool)` THEN
    CONV_TAC "100/ramsey.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[] THEN
...
let INFINITE_CHOOSE = prove(
  ...
  CONV_TAC "100/ramsey.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN REWRITE_TAC[]);;
    ...
    EXISTS_TAC `((@) s :A) INSERT t` THEN CONJ_TAC THENL
     [REWRITE_TAC[INSERT_SUBSET] THEN CONJ_TAC THENL
       [MATCH_MP_TAC INFINITE_CHOOSE THEN ASM_REWRITE_TAC[];
        ...
        REWRITE_TAC[IN_DELETE] THEN CONV_TAC "100/ramsey.ml:(EQT_INTRO o TAUT)" (EQT_INTRO o TAUT)];
...
let IMAGE_WOP_LEMMA = prove(
  ...
  MAP_EVERY EXISTS_TAC [`m:num`; `u DELETE (x:A)`] THEN CONJ_TAC THENL
   [ASM_REWRITE_TAC[] THEN CONV_TAC "100/ramsey.ml:SYM_CONV" SYM_CONV THEN
 ...
 `(!C s. INFINITE(s:A->bool) /\
  ...
  MP_TAC THENL
   [MATCH_MP_TAC num_INDUCTION THEN REPEAT STRIP_TAC THENL
     [ASM_REWRITE_TAC[SUBSET_REFL];
      FIRST_ASSUM(SUBST1_TAC o SPEC `0`) THEN CONV_TAC "100/ramsey.ml:SELECT_CONV" SELECT_CONV THEN
      FIRST_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[];
      MATCH_MP_TAC SUBSET_TRANS THEN EXISTS_TAC `f(n:num):A->bool` THEN
      CONJ_TAC THEN FIRST_ASSUM MATCH_ACCEPT_TAC;
      FIRST_ASSUM(SUBST1_TAC o SPEC `SUC n`) THEN CONV_TAC "100/ramsey.ml:SELECT_CONV" SELECT_CONV THEN
     ...
     [X_GEN_TAC `n:num` THEN
      ...
      CONJ_TAC THENL
       [REWRITE_TAC[INSERT_SUBSET] THEN CONJ_TAC THENL
        ...
        DISCH_THEN(MP_TAC o SPEC `n:num`) THEN CONV_TAC "100/ramsey.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
        REWRITE_TAC[] THEN
  ...
  REPEAT CONJ_TAC THENL
   [UNDISCH_TAC `INFINITE(t:num->bool)` THEN
    ...
    SUBGOAL_THEN `c = (col:num->num) n` SUBST1_TAC THENL
     [CONV_TAC "100/ramsey.ml:SYM_CONV" SYM_CONV THEN FIRST_ASSUM MATCH_MP_TAC THEN
        ...
        SUBGOAL_THEN `v = u DELETE ((x:num->A) n)` SUBST1_TAC THENL
         [ASM_REWRITE_TAC[] THEN REWRITE_TAC[DELETE_INSERT] THEN
          REWRITE_TAC[EXTENSION; IN_DELETE;
            TAUT `(a <=> a /\ b) <=> a ==> b`] THEN
          GEN_TAC THEN CONV_TAC "100/ramsey.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[] THEN
