...
let ALLN = prove
 (`(ALLN P 0 <=> T) /\
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Arithmetic/arithprov.ml:LAND_CONV" LAND_CONV [ALLN_DEF] THEN
...
let AXIOM,AXIOM_THM =
  let th0 = prove
   (`((?x p. P (number x) (gform p) /\ ~(x IN FV(p))) <=> 
    ...
    GEN_REWRITE_TAC "Arithmetic/arithprov.ml:(RAND_CONV o BINDER_CONV)" (RAND_CONV o BINDER_CONV) [SWAP_EXISTS_THM] THEN
...
let AXIOM_FORMULA = prove
 (`!a. AXIOM a ==> ?p. a = gform p`,
  ...
  CONV_TAC "Arithmetic/arithprov.ml:(BINDER_CONV SYM_CONV)" (BINDER_CONV SYM_CONV) THEN
...
let ARITH_PROV = prove
 (`!Ax A. (!v t. holds v (A t) <=> Ax(termval v t))
  ...
  CONV_TAC "Arithmetic/arithprov.ml:(TOP_DEPTH_CONV ETA_CONV)" (TOP_DEPTH_CONV ETA_CONV) THEN DISCH_TAC THEN
