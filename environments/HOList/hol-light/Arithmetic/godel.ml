...
let SIGMAPI_PROP = prove
 (`(!n b. sigmapi b n False <=> T) /\
   ...
   GEN_REWRITE_TAC "Arithmetic/godel.ml:DEPTH_CONV" DEPTH_CONV [AND_FORALL_THM] THEN
   INDUCT_TAC THEN REWRITE_TAC[NOT_SUC; SUC_SUB1] THEN
   REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Arithmetic/godel.ml:LAND_CONV" LAND_CONV [sigmapi_CASES] THEN
...
let SIGMAPI_REV_EXISTS = prove
 (`!n b x p. sigmapi b n (??x p) ==> sigmapi b n p`,
  ...
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC "Arithmetic/godel.ml:LAND_CONV" LAND_CONV [sigmapi_CASES] THEN
  REWRITE_TAC[form_DISTINCT; form_INJ] THEN
...
let SIGMAPI_CLAUSES_CODE = prove
 (`(!n b. sigmapi b n False <=> T) /\
  ...
  GEN_REWRITE_TAC "Arithmetic/godel.ml:LAND_CONV" LAND_CONV [sigmapi_CASES] THEN
  REWRITE_TAC[form_DISTINCT; form_INJ] THEN
  REWRITE_TAC[GSYM CONJ_ASSOC; RIGHT_EXISTS_AND_THM; UNWIND_THM1] THEN
...
let SIGMAPI_CLAUSES = prove
 (`(!n b. sigmapi b n False <=> T) /\
  ...
  GEN_REWRITE_TAC "Arithmetic/godel.ml:LAND_CONV" LAND_CONV [SIGMAPI_CLAUSES_CODE] THEN
  REWRITE_TAC[] THEN
...
let SIGMAPI_FORMSUBST = prove
 (`!p v n b. sigmapi b n p ==> sigmapi b n (formsubst v p)`,
  ...
  CONV_TAC "Arithmetic/godel.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN
  ...
  FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Arithmetic/godel.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [SYM th]) THEN
...
let HSENTENCE_FIX_STRONG = prove
 (`!A Arep.
  ...
  REWRITE_TAC[IN] THEN CONV_TAC "Arithmetic/godel.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
