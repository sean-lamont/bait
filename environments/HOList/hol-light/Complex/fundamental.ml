...
let SQRT_SOS_LT_1 = prove
 (`!x y. sqrt(x pow 2 + y pow 2) < &1 <=> x pow 2 + y pow 2 < &1`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Complex/fundamental.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM SQRT_1] THEN
...
let METRIC_BOUND_LEMMA = prove
 (`!x y. norm(x - y) <= abs(Re(x) - Re(y)) + abs(Im(x) - Im(y))`,
  ...
  GEN_REWRITE_TAC "Complex/fundamental.ml:RAND_CONV" RAND_CONV [GSYM POW_2_SQRT_ABS] THEN
 ...
 (`!s r. (!n. norm(s n) <= r)
  ...
  MP_TAC THENL
   [CONJ_TAC THEN MATCH_MP_TAC SEQ_BCONV THEN ASM_REWRITE_TAC[bounded] THEN
    ...
    GEN_REWRITE_TAC "Complex/fundamental.ml:LAND_CONV" LAND_CONV [GSYM POW_2_SQRT_ABS] THEN
    MATCH_MP_TAC SQRT_MONO_LE THEN
   ...
   [MATCH_MP_TAC REAL_LTE_TRANS THEN EXISTS_TAC `inv(&(SUC(N1 + N2)))` THEN
    CONJ_TAC THENL
     [MATCH_MP_TAC(REAL_ARITH
      ...
      GEN_REWRITE_TAC "Complex/fundamental.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_DIV] THEN
...
let POLY_DECOMPOSE_LEMMA = prove
 (`!p. ~(!z. ~(z = Cx(&0)) ==> (poly p z = Cx(&0)))
  ...
  ASM_CASES_TAC `h = Cx(&0)` THENL
   [GEN_REWRITE_TAC "Complex/fundamental.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [poly] THEN
 ...
 (`!p. ~constant(poly p) ==> ?z. poly p z = Cx(&0)`,
  ...
  SUBGOAL_THEN `poly r (Cx(&0)) = Cx(&1)` ASSUME_TAC THENL
   [FIRST_X_ASSUM(MP_TAC o SPEC `Cx(&0)`) THEN
    ASM_REWRITE_TAC[] THEN
    GEN_REWRITE_TAC "Complex/fundamental.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [GSYM COMPLEX_MUL_LID] THEN
  ...
  GEN_REWRITE_TAC "Complex/fundamental.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
  ...
  EXISTS_TAC `abs t * norm(w pow (k + 1)) * m` THEN CONJ_TAC THENL
   [MATCH_MP_TAC REAL_LE_LMUL THEN REWRITE_TAC[REAL_ABS_POS] THEN
    ...
    GEN_REWRITE_TAC "Complex/fundamental.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
