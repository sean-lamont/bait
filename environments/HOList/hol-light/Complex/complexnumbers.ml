...
let CX_INV = prove
 (`!x. Cx(inv x) = inv(Cx x)`,
  ...
  GEN_REWRITE_TAC "Complex/complexnumbers.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_LID] THEN
...
let COMPLEX_ENTIRE = prove
 (`!x y. (x * y = Cx(&0)) <=> (x = Cx(&0)) \/ (y = Cx(&0))`,
  ...
  CONV_TAC "Complex/complexnumbers.ml:REAL_RING" REAL_RING);;
...
let COMPLEX_POW_II_2 = prove
 (`ii pow 2 = --Cx(&1)`,
  ...
  CONV_TAC "Complex/complexnumbers.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;

let COMPLEX_POW_EQ_0 = prove
 (`!x n. (x pow n = Cx(&0)) <=> (x = Cx(&0)) /\ ~(n = 0)`,
  GEN_TAC THEN INDUCT_TAC THEN
  ASM_REWRITE_TAC[NOT_SUC; complex_pow; COMPLEX_ENTIRE] THENL
   [SIMPLE_COMPLEX_ARITH_TAC; CONV_TAC "Complex/complexnumbers.ml:TAUT" TAUT]);;
...
let COMPLEX_NORM_ZERO = prove
 (`!z. (norm z = &0) <=> (z = Cx(&0))`,
  ...
  GEN_REWRITE_TAC "Complex/complexnumbers.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM SQRT_0] THEN
...
let COMPLEX_NORM_INV = prove
 (`!z. norm(inv z) = inv(norm z)`,
  ...
  CONV_TAC "Complex/complexnumbers.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_MUL_RINV_UNIQ THEN
...
let COMPLEX_NORM_TRIANGLE = prove
 (`!w z. norm(w + z) <= norm(w) + norm(z)`,
  ...
  GEN_REWRITE_TAC "Complex/complexnumbers.ml:RAND_CONV" RAND_CONV[REAL_ARITH
...
let COMPLEX_MUL_LINV = prove
 (`!z. ~(z = Cx(&0)) ==> (inv(z) * z = Cx(&1))`,
  ...
  REWRITE_TAC[GSYM REAL_SOS_EQ_0] THEN CONV_TAC "Complex/complexnumbers.ml:REAL_FIELD" REAL_FIELD);;
...
let COMPLEX_RING,complex_ideal_cofactors =
  ...
  and COMPLEX_RABINOWITSCH = prove
   (`!x y:complex. ~(x = y) <=> ?z. (x - y) * z = Cx(&1)`,
    REPEAT GEN_TAC THEN
    GEN_REWRITE_TAC "Complex/complexnumbers.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM COMPLEX_SUB_0] THEN
...
let COMPLEX_INV_1 = prove
 (`inv(Cx(&1)) = Cx(&1)`,
  ...
  CONV_TAC "Complex/complexnumbers.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_DIV_1]);;
...
let COMPLEX_INV_INV = prove
 (`!x:complex. inv(inv x) = x`,
  ...
  CONV_TAC "Complex/complexnumbers.ml:COMPLEX_RING" COMPLEX_RING);;
...
let COMPLEX_DIV_REFL = prove
 (`!x. ~(x = Cx(&0)) ==> (x / x = Cx(&1))`,
  CONV_TAC "Complex/complexnumbers.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
