 ...
 (`!z. csqrt(z) pow 2 = z`,
  ...
  REWRITE_TAC[COMPLEX_EQ; RE; IM] THEN CONJ_TAC THENL
   [ASM_SIMP_TAC[REAL_POW_DIV; REAL_POW2_ABS;
    ...
    REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN CONV_TAC "Complex/complex_transc.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  CONV_TAC "Complex/complex_transc.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let CEXP_ADD = prove
 (`!w z. cexp(w + z) = cexp(w) * cexp(z)`,
  ...
  REWRITE_TAC[REAL_EXP_ADD; SIN_ADD; COS_ADD] THEN CONV_TAC "Complex/complex_transc.ml:REAL_RING" REAL_RING);;
...
let CEXP_NONZERO = prove
 (`!z. ~(cexp z = Cx(&0))`,
  ...
  MP_TAC(SPEC `Im z` SIN_CIRCLE) THEN CONV_TAC "Complex/complex_transc.ml:REAL_RING" REAL_RING);;
...
let CX_CSIN,CX_CCOS = (CONJ_PAIR o prove)
 (`(!x. Cx(sin x) = csin(Cx x)) /\ (!x. Cx(cos x) = ccos(Cx x))`,
  ...
  CONJ_TAC THEN GEN_TAC THEN CONV_TAC "Complex/complex_transc.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  REWRITE_TAC[REAL_MUL_RZERO] THEN
  AP_TERM_TAC THEN AP_THM_TAC THEN AP_TERM_TAC THEN CONV_TAC "Complex/complex_transc.ml:REAL_RING" REAL_RING);;
...
let CSIN_CIRCLE = prove
 (`!z. csin(z) pow 2 + ccos(z) pow 2 = Cx(&1)`,
  ...
  CONV_TAC "Complex/complex_transc.ml:COMPLEX_FIELD" COMPLEX_FIELD);;

let CSIN_ADD = prove
 (`!w z. csin(w + z) = csin(w) * ccos(z) + ccos(w) * csin(z)`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC[csin; ccos; COMPLEX_ADD_LDISTRIB; CEXP_ADD] THEN
  MP_TAC COMPLEX_POW_II_2 THEN CONV_TAC "Complex/complex_transc.ml:COMPLEX_FIELD" COMPLEX_FIELD);;
...
let CSIN_DOUBLE = prove
 (`!z. csin(Cx(&2) * z) = Cx(&2) * csin(z) * ccos(z)`,
  ...
  CONV_TAC "Complex/complex_transc.ml:COMPLEX_RING" COMPLEX_RING);;
...
let SINCOS_PRINCIPAL_VALUE = prove
 (`!x. ?y. (--pi < y /\ y <= pi) /\ (sin(y) = sin(x) /\ cos(y) = cos(x))`,
  ...
  CONJ_TAC THENL
   ...
   CONV_TAC "Complex/complex_transc.ml:REAL_RING" REAL_RING]);;
...
let CLOG_WORKS = prove
 (`!z. ~(z = Cx(&0))
  ...
  GEN_TAC THEN DISCH_TAC THEN REWRITE_TAC[clog] THEN CONV_TAC "Complex/complex_transc.ml:SELECT_CONV" SELECT_CONV THEN
...
let COMPLEX_II_NZ = prove
 (`~(ii = Cx(&0))`,
  MP_TAC COMPLEX_POW_II_2 THEN CONV_TAC "Complex/complex_transc.ml:COMPLEX_RING" COMPLEX_RING);;
