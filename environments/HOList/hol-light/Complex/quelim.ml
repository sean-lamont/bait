...
let NULLSTELLENSATZ_LEMMA = prove
 (`!n p q. (!x. (poly p x = Cx(&0)) ==> (poly q x = Cx(&0))) /\
  ...
  GEN_REWRITE_TAC "Complex/quelim.ml:LAND_CONV" LAND_CONV [ORDER_ROOT] THEN
  ...
  GEN_REWRITE_TAC "Complex/quelim.ml:LAND_CONV" LAND_CONV [divides] THEN
  ...
  ASM_CASES_TAC `degree s = 0` THENL
   [SUBGOAL_THEN `?k. ~(k = Cx(&0)) /\ (poly s = poly [k])` MP_TAC THENL
     [EXISTS_TAC `LAST(normalize s)` THEN
      ...
      GEN_REWRITE_TAC "Complex/quelim.ml:LAND_CONV" LAND_CONV [GSYM POLY_NORMALIZE] THEN
  ...
  GEN_REWRITE_TAC "Complex/quelim.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM POLY_MUL] THEN
   ...
   [DISCH_THEN(ASSUME_TAC o GSYM o SPEC `Cx(&0)`) THEN
    SUBGOAL_THEN `degree [poly p (Cx(&0))] = 0` MP_TAC THENL
     [REWRITE_TAC[degree; normalize] THEN
      COND_CASES_TAC THEN REWRITE_TAC[LENGTH] THEN CONV_TAC "Complex/quelim.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
      ALL_TAC] THEN
...
let DIVIDES_DEGREE_LEMMA = prove
 (`!n p q. (degree(p) = n)
  ...
  GEN_REWRITE_TAC "Complex/quelim.ml:LAND_CONV" LAND_CONV [POLY_LINEAR_DIVIDES] THEN
...
let BASIC_QUELIM_CONV =
  ...
  let pth_3 = prove
   (`~(LAST(CONS a p) = Cx(&0))
    ...
    CONV_TAC "Complex/quelim.ml:TAUT" TAUT) in
...
let COMPLEX_QUELIM_CONV =
  ...
  and pth_2 = prove
   (`(p ==> (q <=> r)) ==> (p /\ q <=> p /\ r)`,
    CONV_TAC "Complex/quelim.ml:TAUT" TAUT)
  and zero_tm = `Cx(&0)`
  and true_tm = `T` in
