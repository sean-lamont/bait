...
let LIST_EQ = prove
 (`!l1 l2:A list.
  ...
  GEN_REWRITE_TAC "lists.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
...
let AND_ALL = prove
 (`!l. ALL P l /\ ALL Q l <=> ALL (\x. P x /\ Q x) l`,
  CONV_TAC "lists.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
...
let MEM_EXISTS_EL = prove
 (`!l x. MEM x l <=> ?i. i < LENGTH l /\ x = EL i l`,
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "lists.ml:RAND_CONV" RAND_CONV
...
let AND_ALL2 = prove
 (`!P Q l m. ALL2 P l m /\ ALL2 Q l m <=> ALL2 (\x y. P x y /\ Q x y) l m`,
  GEN_TAC THEN GEN_TAC THEN CONV_TAC "lists.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
