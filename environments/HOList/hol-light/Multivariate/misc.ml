 ...
 (`!x y a u v. x < a /\ y < a /\ &0 <= u /\ &0 <= v /\ (u + v = &1)
  ...
  EXISTS_TAC `u * a + v * a:real` THEN CONJ_TAC THENL
   [ASM_SIMP_TAC[REAL_CONVEX_BOUND2_LT];
    ...
    UNDISCH_TAC `u + v = &1` THEN CONV_TAC "Multivariate/misc.ml:REAL_RING" REAL_RING]);;
...
let LAMBDA_PAIR = prove
 (`(\(x,y). P x y) = (\p. P (FST p) (SND p))`,
  ...
  CONV_TAC "Multivariate/misc.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN REWRITE_TAC[]);;
...
let HULL_IMAGE_GALOIS = prove
 (`!P f g s. (!s. P(P hull s)) /\
  ...
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/misc.ml:I" I [GSYM th]) THEN
...
let HULL_IMAGE = prove
 (`!P f s. (!s. P(P hull s)) /\ (!s. P(IMAGE f s) <=> P s) /\
  ...
  FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/misc.ml:RAND_CONV" RAND_CONV [GSYM th]) THEN
...
let REAL_ARCH_POW_INV = prove
 (`!x y. &0 < y /\ x < &1 ==> ?n. x pow n < y`,
  ...
  GEN_REWRITE_TAC "Multivariate/misc.ml:BINOP_CONV" BINOP_CONV [GSYM REAL_INV_INV] THEN
...
let FINITE_INDEX_NUMSEG_SPECIAL = prove
 (`!s a:A.
  ...
  UNDISCH_THEN `s = IMAGE (f:num->A) (1..CARD(s:A->bool))`
   (fun th -> GEN_REWRITE_TAC "Multivariate/misc.ml:LAND_CONV" LAND_CONV [th]) THEN
...
let REAL_NON_MONOTONE = prove
 (`!P f:real->real.
  ...
  GEN_REWRITE_TAC "Multivariate/misc.ml:I" I [GSYM CONTRAPOS_THM] THEN
  ...
  CONV_TAC "Multivariate/misc.ml:NNFC_CONV" NNFC_CONV THEN CONV_TAC "Multivariate/misc.ml:CNF_CONV" CNF_CONV THEN
...
let COUNTABLE_LOCAL_MAXIMA = prove
 (`!f:real->real.
  ...
  STRIP_TAC THEN ASM_REWRITE_TAC[IN] THEN CONJ_TAC THENL
   [CONV_TAC "Multivariate/misc.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_SUP_UNIQUE THEN
...
let RELATIVE_TO_COMPL = prove
 (`!P u s:A->bool.
  ...
  GEN_REWRITE_TAC "Multivariate/misc.ml:RAND_CONV" RAND_CONV [GSYM EXISTS_DIFF] THEN
...
let GENERAL_REDUCTION_THEOREM = prove
 (`!P. P {} /\
  ...
  GEN_REWRITE_TAC "Multivariate/misc.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [IN_UNIV] THEN
   ...
   [X_GEN_TAC `n:num` THEN
    ...
    REWRITE_TAC[] THEN CONJ_TAC THENL
     [MATCH_MP_TAC COUNTABLE_SUBSET THEN EXISTS_TAC
      ...
      GEN_REWRITE_TAC "Multivariate/misc.ml:I" I [EXTENSION] THEN X_GEN_TAC `v:A->bool` THEN
    ...
    MP_TAC THENL
     [REWRITE_TAC[ARITH_RULE `m < n \/ n < m <=> ~(m:num = n)`] THEN
      ...
      SET_TAC[]];
     GEN_REWRITE_TAC "Multivariate/misc.ml:I" I [EXTENSION] THEN X_GEN_TAC `x:A` THEN
     ...
     CONJ_TAC THENL
      [REPEAT(AP_TERM_TAC THEN ABS_TAC) THEN
       ...
       ALL_TAC] THEN
     FIRST_ASSUM(fun t -> GEN_REWRITE_TAC "Multivariate/misc.ml:(RAND_CONV o BINDER_CONV o RAND_CONV)" (RAND_CONV o BINDER_CONV o RAND_CONV)
        [GSYM t]) THEN
     ...
     GEN_REWRITE_TAC "Multivariate/misc.ml:RAND_CONV" RAND_CONV [num_WOP] THEN
...
let GENERAL_REDUCTION_THEOREM_2 = prove
 (`!P. P {} /\
  ...
  CONV_TAC "Multivariate/misc.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN SIMP_TAC[SUBSET_EMPTY] THEN
  STRIP_TAC THEN
  ...
  CONV_TAC "Multivariate/misc.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[IN_INSERT] THEN STRIP_TAC THEN
...
let SUSLIN_SUSLIN = prove
 (`!u:(A->bool)->bool. suslin (suslin u) = suslin u`,
  ...
  GEN_REWRITE_TAC "Multivariate/misc.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
  ...
  SUBST1_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/misc.ml:BINOP_CONV" BINOP_CONV [suslin_operation] THEN
    REWRITE_TAC[SIMPLE_IMAGE] THEN
    AP_TERM_TAC THEN AP_THM_TAC THEN AP_TERM_TAC THEN
    GEN_REWRITE_TAC "Multivariate/misc.ml:I" I [FUN_EQ_THM] THEN X_GEN_TAC `s:num->num` THEN
   ...
   [SUBGOAL_THEN
    ...
    MP_TAC THENL
     [TRANS_TAC CARD_EQ_TRANS `(:num#num)` THEN CONJ_TAC THENL
      ...
      CONV_TAC "Multivariate/misc.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
      CONJ_TAC THEN REPEAT GEN_TAC THEN
      REPEAT(COND_CASES_TAC THEN ASM_REWRITE_TAC[]) THEN
      CONV_TAC "Multivariate/misc.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ASM_MESON_TAC[]];
    ALL_TAC] THEN
   ...
   [SUBGOAL_THEN
    ...
    STRIP_ASSUME_TAC THENL
     [SUBGOAL_THEN `((:num) DELETE 0) CROSS ((:num) DELETE 0) =_c (:num)`
      ...
      GEN_REWRITE_TAC "Multivariate/misc.ml:LAND_CONV" LAND_CONV [GSYM SUBSET_ANTISYM_EQ] THEN
      REPEAT CONJ_TAC THENL
       [REWRITE_TAC[SUBSET; FORALL_IN_IMAGE] THEN
        REPEAT STRIP_TAC THEN COND_CASES_TAC THEN
        REWRITE_TAC[IN_CROSS; IN_UNIV; IN_DELETE] THEN
        CONV_TAC "Multivariate/misc.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ASM SET_TAC[];
        ASM SET_TAC[];
        REPEAT STRIP_TAC THEN
        REWRITE_TAC[SUBSET; FORALL_IN_IMAGE] THEN
   ...
   [MAP_EVERY X_GEN_TAC [`s:num->num`; `s':num->num->num`] THEN STRIP_TAC THEN
    ...
    STRIP_ASSUME_TAC THENL
     [SUBGOAL_THEN
      ...
      REWRITE_TAC[LIST_OF_SEQ_EQ_NIL] THEN CONV_TAC "Multivariate/misc.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    ...
    SIMP_TAC[LE_1; EL_LIST_OF_SEQ] THEN DISCH_TAC THEN COND_CASES_TAC THENL
     [ASM_REWRITE_TAC[] THEN FIRST_X_ASSUM MATCH_MP_TAC THEN
      CONV_TAC "Multivariate/misc.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
