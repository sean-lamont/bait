...
let TRACE_MUL_SYM = prove
 (`!A B:real^N^M. trace(A ** B) = trace(B ** A)`,
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:RAND_CONV" RAND_CONV [SUM_SWAP_NUMSEG] THEN REWRITE_TAC[REAL_MUL_SYM]);;
...
let TRACE_MUL_CYCLIC = prove
 (`!A:real^P^M B C:real^M^N. trace(A ** B ** C) = trace(B ** C ** A)`,
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:RAND_CONV" RAND_CONV [TRACE_MUL_SYM] THEN
...
let DET_TRANSP = prove
 (`!A:real^N^N. det(transp A) = det A`,
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:LAND_CONV" LAND_CONV [SUM_PERMUTATIONS_INVERSE] THEN
  ...
  REWRITE_TAC[IN_ELIM_THM] THEN DISCH_TAC THEN BINOP_TAC THENL
   [ASM_MESON_TAC[SIGN_INVERSE; PERMUTATION_PERMUTES; FINITE_NUMSEG];
    ALL_TAC] THEN
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
...
let DET_PERMUTE_ROWS = prove
 (`!A:real^N^N p.
  ...
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/determinants.ml:LAND_CONV" LAND_CONV
  ...
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Multivariate/determinants.ml:LAND_CONV" LAND_CONV
...
let DET_PERMUTE_COLUMNS = prove
 (`!A:real^N^N p.
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(funpow 2 RAND_CONV)" (funpow 2 RAND_CONV) [GSYM DET_TRANSP] THEN
  FIRST_ASSUM(fun th -> ONCE_REWRITE_TAC
   [GSYM(MATCH_MP DET_PERMUTE_ROWS th)]) THEN
  GEN_REWRITE_TAC "Multivariate/determinants.ml:RAND_CONV" RAND_CONV [GSYM DET_TRANSP] THEN AP_TERM_TAC THEN
...
let BOUNDED_FUNCTIONS_BIJECTIONS_1 = prove
 (`!p. p IN {(y,g) | y IN s /\
  ...
  CONV_TAC "Multivariate/determinants.ml:(REDEPTH_CONV GEN_BETA_CONV)" (REDEPTH_CONV GEN_BETA_CONV) THEN REWRITE_TAC[IN_ELIM_THM] THEN
...
let FINITE_BOUNDED_FUNCTIONS = prove
 (`!s k. FINITE s
  ...
  PURE_ONCE_REWRITE_TAC[CONJ_SYM] THEN CONV_TAC "Multivariate/determinants.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let DET_LINEAR_ROWS_VSUM_LEMMA = prove
 (`!s k a c.
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [LE] THEN
  REWRITE_TAC[TAUT
  ...
  CONV_TAC "Multivariate/determinants.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let DET_MUL = prove
 (`!A B:real^N^N. det(A ** B) = det(A) * det(B)`,
  ...
  MATCH_MP_TAC SUM_EQ THEN SIMP_TAC[FINITE_PERMUTATIONS; FINITE_NUMSEG] THEN
  X_GEN_TAC `p:num->num` THEN REWRITE_TAC[IN_ELIM_THM] THEN DISCH_TAC THEN
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/determinants.ml:RAND_CONV" RAND_CONV
  ...
  BINOP_TAC THENL
   [SUBGOAL_THEN `sign(q o inverse p) = sign(p:num->num) * sign(q:num->num)`
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
...
let DET_LINEAR_ROWS = prove
 (`!f:real^N->real^N A:real^N^N.
  ...
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM DET_TRANSP] THEN
  REWRITE_TAC[GSYM DET_MUL] THEN
  FIRST_ASSUM(fun th -> REWRITE_TAC[GSYM(MATCH_MP MATRIX_WORKS th)]) THEN
  GEN_REWRITE_TAC "Multivariate/determinants.ml:LAND_CONV" LAND_CONV [GSYM DET_TRANSP] THEN
 ...
 (`!A:real^N^N. invertible(A) <=> ~(det A = &0)`,
  GEN_TAC THEN EQ_TAC THENL
   [REWRITE_TAC[INVERTIBLE_RIGHT_INVERSE; LEFT_IMP_EXISTS_THM] THEN
    ...
    REWRITE_TAC[DET_MUL; DET_I] THEN CONV_TAC "Multivariate/determinants.ml:REAL_RING" REAL_RING;
    ALL_TAC] THEN
...
let MATRIX_INV_MUL = prove
 (`!A:real^N^N B:real^N^N.
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(BINOP_CONV o LAND_CONV o LAND_CONV)" (BINOP_CONV o LAND_CONV o LAND_CONV)
...
let DET_SIMILAR = prove
 (`!S:real^N^N A. invertible S ==> det(matrix_inv S ** A ** S) = det A`,
  ...
  CONV_TAC "Multivariate/determinants.ml:REAL_FIELD" REAL_FIELD);;
...
let INVERTIBLE_NEARBY_ONORM = prove
 (`!A B:real^N^N.
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [GSYM ONORM_NEG] THEN
  ...
  CONV_TAC "Multivariate/determinants.ml:(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN REWRITE_TAC[MATRIX_VECTOR_MUL_LID] THEN
  ...
  CONJ_TAC THENL
    [ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
     ...
     ASM_REWRITE_TAC[] THEN
     GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [GSYM REAL_MUL_LID] THEN
...
let CRAMER_LEMMA = prove
 (`!A:real^N^N x:real^N.
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:LAND_CONV" LAND_CONV [GSYM DET_TRANSP] THEN AP_TERM_TAC THEN
 ...
 (`!A:real^N^N X:real^N^N B:real^N^N.
        ...
        ==> (A ** X = B <=>
             X = lambda k l.
                   ...
                   det A)`,
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/determinants.ml:LAND_CONV" LAND_CONV [GSYM TRANSP_EQ] THEN
...
let COFACTOR_TRANSP = prove
 (`!A:real^N^N. cofactor(transp A) = transp(cofactor A)`,
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:RAND_CONV" RAND_CONV [GSYM DET_TRANSP] THEN
...
let COFACTOR_COLUMN = prove
 (`!A:real^N^N.
  ...
  GEN_TAC THEN CONV_TAC "Multivariate/determinants.ml:SYM_CONV" SYM_CONV THEN
  ...
  CONJ_TAC THENL
   [SIMP_TAC[PRODUCT_CLAUSES; FINITE_DELETE; FINITE_PERMUTATIONS;
    ...
    SUBGOAL_THEN
     ...
     (fun th -> GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [th])
    THENL
...
let DET_2 = prove
 (`!A:real^2^2. det A = A$1$1 * A$2$2 - A$1$2 * A$2$1`,
  ...
  CONV_TAC "Multivariate/determinants.ml:(LAND_CONV(RATOR_CONV(ONCE_DEPTH_CONV NUMSEG_CONV)))" (LAND_CONV(RATOR_CONV(ONCE_DEPTH_CONV NUMSEG_CONV))) THEN
...
let DISJOINT_HYPERPLANES_IMP_COLLINEAR = prove
 (`!a b:real^N c d.
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:I" I [GSYM CONTRAPOS_THM] THEN
  MP_TAC(ISPECL
...
let EIGENVALUES_CHARACTERISTIC = prove
 (`!A:real^N^N c.
  ...
  CONV_TAC "Multivariate/determinants.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
 ...
 (`!A:real^N^N.
  ...
  MP_TAC THENL
   [GEN_TAC THEN REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/determinants.ml:LAND_CONV" LAND_CONV [SWAP_FORALL_THM] THEN
    DISCH_THEN(MP_TAC o SPEC `dimindex(:N)`) THEN REWRITE_TAC[LE_REFL] THEN
    GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [RIGHT_IMP_EXISTS_THM] THEN
...
let DIAGONAL_MATRIX_MUL = prove
 (`!A:real^N^N B:real^N^N.
        ...
        ==> diagonal_matrix(A ** B)`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/determinants.ml:RAND_CONV" RAND_CONV [diagonal_matrix] THEN
 ...
 (`!A:real^N^N.
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THENL
     [ALL_TAC;
      SUBGOAL_THEN
       ...
       (fun th -> GEN_REWRITE_TAC "Multivariate/determinants.ml:LAND_CONV" LAND_CONV [th])
  ...
  ANTS_TAC THENL
   [MATCH_MP_TAC(MATCH_MP (REWRITE_RULE[IMP_CONJ] INDEPENDENT_MONO)
    ...
    DISCH_THEN SUBST1_TAC] THEN
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [SIMPLE_IMAGE_GEN] THEN
   ...
   [X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    ...
    TRANS_TAC REAL_LE_TRANS `norm(b % x:real^N)` THEN CONJ_TAC THENL
     [MATCH_MP_TAC NORM_LE_COMPONENTWISE THEN
      ...
      CONV_TAC "Multivariate/determinants.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
...
let POSITIVE_SEMIDEFINITE_DIAGONAL_MATRIX = prove
 (`!D:real^N^N.
  ...
  CONV_TAC "Multivariate/determinants.ml:(RAND_CONV(ONCE_DEPTH_CONV SYM_CONV))" (RAND_CONV(ONCE_DEPTH_CONV SYM_CONV)) THEN
...
let TRACE_LE_MUL_SQUARES = prove
 (`!A B:real^N^N.
  ...
  REPEAT(GEN_REWRITE_TAC "Multivariate/determinants.ml:LAND_CONV" LAND_CONV [TRACE_MUL_SYM] THEN
...
let TRACE_COVARIANCE_CAUCHY_SCHWARZ = prove
 (`!A B:real^M^N.
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o funpow 3 RAND_CONV)" (LAND_CONV o funpow 3 RAND_CONV) [GSYM TRACE_TRANSP] THEN
 ...
 (`!A:real^N^N. abs(det A) <= product(1..dimindex(:N)) (\i. norm(row i A))`,
     ...
     [MATCH_MP_TAC num_INDUCTION THEN CONJ_TAC THENL
      ...
      ANTS_TAC THENL
       [MATCH_MP_TAC SPAN_NEG THEN MATCH_MP_TAC SPAN_VSUM THEN
        ...
        GEN_REWRITE_TAC "Multivariate/determinants.ml:I" I [CART_EQ] THEN X_GEN_TAC `k:num` THEN
      ...
      GEN_REWRITE_TAC "Multivariate/determinants.ml:I" I [CART_EQ] THEN EXPAND_TAC "B" THEN
  ...
  ASSUME_TAC THENL
   [ONCE_REWRITE_TAC[SWAP_FORALL_THM] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/determinants.ml:I" I [SWAP_FORALL_THM] THEN REWRITE_TAC[IMP_IMP] THEN
    REWRITE_TAC[ARITH_RULE
     `j < n /\ 1 <= n /\ n <= N /\ 1 <= j /\ j <= N /\ ~(n = j) <=>
      (1 <= n /\ n <= N) /\ (1 <= j /\ j <= N /\ j < n)`] THEN
    MATCH_MP_TAC num_WF THEN CONV_TAC "Multivariate/determinants.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    X_GEN_TAC `n:num` THEN
    GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_FORALL_THM] THEN
     ...
     [REWRITE_TAC[SUM_DELTA; IN_NUMSEG] THEN COND_CASES_TAC THEN
      ...
      COND_CASES_TAC THEN ASM_REWRITE_TAC[] THENL
       [ASM_CASES_TAC `(b:num->real^N) m = vec 0` THEN
        ...
        CONV_TAC "Multivariate/determinants.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[REAL_ENTIRE] THEN
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM DET_TRANSP] THEN
  REWRITE_TAC[GSYM DET_MUL] THEN
...
let ORTHOGONAL_MATRIX_MUL = prove
 (`!A B. orthogonal_matrix A /\ orthogonal_matrix B
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [MATRIX_MUL_ASSOC] THEN
...
let DET_ORTHOGONAL_MATRIX = prove
 (`!Q. orthogonal_matrix Q ==> det(Q) = &1 \/ det(Q) = -- &1`,
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV) [GSYM DET_TRANSP] THEN
...
let ORTHOGONAL_MATRIX_ORTHONORMAL_ROWS_INDEXED = prove
 (`!A:real^N^N.
  ...
  CONV_TAC "Multivariate/determinants.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[orthogonal] THEN
  MESON_TAC[]);;
...
let ORTHOGONAL_MATRIX_ORTHONORMAL_ROWS_PAIRWISE = prove
 (`!A:real^N^N.
  ...
  DISCH_TAC THEN GEN_REWRITE_TAC "Multivariate/determinants.ml:(RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV)
 ...
 (`!A:real^N^N.
  ...
  EQ_TAC THEN STRIP_TAC THEN ASM_REWRITE_TAC[] THENL
   [MATCH_MP_TAC(SET_RULE `UNIV SUBSET s ==> s = UNIV`) THEN
    ...
    CONV_TAC "Multivariate/determinants.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[GSYM DIM_UNIV] THEN
...
let ORTHOGONAL_MATRIX_2 = prove
 (`!A:real^2^2. orthogonal_matrix A <=>
  ...
  CONV_TAC "Multivariate/determinants.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "Multivariate/determinants.ml:REAL_RING" REAL_RING);;
...
let ORTHOGONAL_MATRIX_2_ALT = prove
 (`!A:real^2^2. orthogonal_matrix A <=>
                ...
                (A$1$1 = A$2$2 /\ A$1$2 = --(A$2$1) \/
                 A$1$1 = --(A$2$2) /\ A$1$2 = A$2$1)`,
  REWRITE_TAC[ORTHOGONAL_MATRIX_2] THEN CONV_TAC "Multivariate/determinants.ml:REAL_RING" REAL_RING);;
...
let ORTHOGONAL_TRANSFORMATION_ORTHOGONAL_EIGENVECTORS = prove
 (`!f:real^N->real^N v w a b.
  ...
  UNDISCH_TAC `~(a:real = b)` THEN CONV_TAC "Multivariate/determinants.ml:REAL_RING" REAL_RING);;
...
let ORTHOGONAL_TRANSFORMATION_1_GEN = prove
 (`!f:real^N->real^N.
  ...
  GEN_REWRITE_TAC "Multivariate/determinants.ml:(funpow 3 RAND_CONV)" (funpow 3 RAND_CONV) [GSYM ETA_AX] THEN
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/determinants.ml:VECTOR_ARITH" VECTOR_ARITH);;
 ...
 (`!f:real^M->real^N.
  ...
  MP_TAC THENL
   [MATCH_MP_TAC(MESON[]
    ...
    GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o LAND_CONV o RAND_CONV o RAND_CONV)" (LAND_CONV o LAND_CONV o RAND_CONV o RAND_CONV)
 ...
 (`!f:real^N->real^N.
  ...
  let lemma = prove
   (`!x:real^N y:real^N x':real^N y':real^N x0 y0 x0' y0'.
    ...
    REWRITE_TAC[DOT_SYM] THEN CONV_TAC "Multivariate/determinants.ml:REAL_RING" REAL_RING) in
  REPEAT STRIP_TAC THEN
   ...
   [MP_TAC(ISPECL [`b:real^N->bool`; `c:real^N->bool`]
    ...
    ANTS_TAC THENL
     [ASM_SIMP_TAC[FINITE_UNION; FINITE_DIFF] THEN
      ...
      GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [UNION_COMM] THEN
...
let ROTATION_MATRIX_2 = prove
 (`!A:real^2^2. rotation_matrix A <=>
  ...
  CONV_TAC "Multivariate/determinants.ml:REAL_RING" REAL_RING);;
 ...
 (`!v:num->real^N w k.
  ...
  let lemma2 = prove
   (`!v:num->real^N w k.
    ...
    REWRITE_TAC[IN_NUMSEG] THEN GEN_REWRITE_TAC "Multivariate/determinants.ml:I" I [IMP_CONJ] THEN
...
let REFLECT_ALONG_INVOLUTION = prove
 (`!v x:real^N. reflect_along v (reflect_along v x) = x`,
  ...
  REWRITE_TAC[DOT_LMUL; GSYM DOT_EQ_0] THEN CONV_TAC "Multivariate/determinants.ml:REAL_FIELD" REAL_FIELD);;
...
let ORTHOGONAL_TRANSFORMATION_REFLECT_ALONG = prove
 (`!v:real^N. orthogonal_transformation(reflect_along v)`,
  GEN_TAC THEN ASM_CASES_TAC `v:real^N = vec 0` THENL
   [GEN_REWRITE_TAC "Multivariate/determinants.ml:RAND_CONV" RAND_CONV [GSYM ETA_AX] THEN
...
let REFLECT_ALONG_SCALE = prove
 (`!c v x:real^N. ~(c = &0) ==> reflect_along (c % v) x = reflect_along v x`,
  ...
  FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE RAND_CONV [GSYM DOT_EQ_0]) THEN
  POP_ASSUM MP_TAC THEN CONV_TAC "Multivariate/determinants.ml:REAL_FIELD" REAL_FIELD);;
   ...
   [UNDISCH_THEN `(f:real^N->real^N) v = basis 1` (K ALL_TAC) THEN
    ...
    CONJ_TAC THENL
     [ASM_MESON_TAC[ORTHOGONAL_MATRIX; MATRIX_VECTOR_MUL_LID;
      ...
      AP_TERM_TAC THEN CONV_TAC "Multivariate/determinants.ml:SYM_CONV" SYM_CONV THEN
   ...
   [REWRITE_TAC[CONJUNCT1 LE; LENGTH_EQ_NIL; ADD_CLAUSES; UNWIND_THM2] THEN
    ...
    ANTS_TAC THENL
     [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (ARITH_RULE
      ...
      CONV_TAC "Multivariate/determinants.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM_REWRITE_TAC[VECTOR_SUB_EQ] THEN
...
let GEN_GEOM_ORIGIN_TAC x avoid (asl,w as gl) =
  ...
  (MAP_EVERY X_GEN_TAC avs THEN
   ...
   SPEC_TAC(x,x) THEN CONV_TAC "Multivariate/determinants.ml:(GEOM_ORIGIN_CONV avoid)" (GEOM_ORIGIN_CONV avoid)) gl;;
...
let GEOM_TRANSLATE_TAC avoid (asl,w) =
  ...
  (X_GEN_TAC a THEN
   CONV_TAC "Multivariate/determinants.ml:(funpow n BINDER_CONV (LAND_CONV(GEOM_TRANSLATE_CONV avoid a)))" (funpow n BINDER_CONV (LAND_CONV(GEOM_TRANSLATE_CONV avoid a))) THEN
   REWRITE_TAC[]) (asl,w);;
...
let GEOM_TRANSFORM_TAC =
  ...
  fun avoid (asl,w as gl) ->
    ...
    (MAP_EVERY X_GEN_TAC (f::vs) THEN DISCH_TAC THEN
     GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) cths THEN
     CONV_TAC "Multivariate/determinants.ml:(LAND_CONV vconv)" (LAND_CONV vconv) THEN
     GEN_REWRITE_TAC "Multivariate/determinants.ml:(LAND_CONV o REDEPTH_CONV)" (LAND_CONV o REDEPTH_CONV) [bths] THEN
     REWRITE_TAC[] THEN
