...
let NORM_CROSS = prove
 (`!x y. norm(x cross y) = norm(x) * norm(y) * sin(vector_angle x y)`,
  ...
  CONV_TAC "Multivariate/flyspeck.ml:REAL_RING" REAL_RING);;
 ...
 (`!x y:real^3.
  ...
  MATCH_MP_TAC(TAUT `a /\ (a ==> b) ==> a /\ b`) THEN CONJ_TAC THENL
   [REWRITE_TAC[AFFINE_HULL_3; IN_ELIM_THM; LEFT_IMP_EXISTS_THM] THEN
    ...
    REPEAT(FIRST_X_ASSUM(MP_TAC o SYM)) THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_RING" REAL_RING;
    ALL_TAC] THEN
...
let PAD2D3D_TAC =
  ...
  let lasttac =
    GEN_REWRITE_TAC "Multivariate/flyspeck.ml:REDEPTH_CONV" REDEPTH_CONV [LINEAR_INVARIANTS pad2d3d_tm pths] in
  fun gl -> (GEN_REWRITE_TAC "Multivariate/flyspeck.ml:ONCE_DEPTH_CONV" ONCE_DEPTH_CONV [cth] THEN
             CONV_TAC "Multivariate/flyspeck.ml:(DEPTH_CONV PAD2D3D_QUANTIFY_CONV)" (DEPTH_CONV PAD2D3D_QUANTIFY_CONV) THEN
             lasttac) gl;;
...
let ROTATION_PLANE_HORIZONTAL = prove
 (`!s. plane s
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:RAND_CONV" RAND_CONV [GSYM lemma] THEN
...
let COPLANAR = prove
 (`2 <= dimindex(:N)
  ...
  CONV_TAC "Multivariate/flyspeck.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[LEFT_AND_EXISTS_THM] THEN
  ...
  CONV_TAC "Multivariate/flyspeck.ml:(LAND_CONV HAS_SIZE_CONV)" (LAND_CONV HAS_SIZE_CONV) THEN
  REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN
...
let COPLANAR_DET_EQ_0 = prove
 (`!v0 v1 (v2: real^3) v3.
  ...
  CONV_TAC "Multivariate/flyspeck.ml:(ONCE_DEPTH_CONV NUMSEG_CONV)" (ONCE_DEPTH_CONV NUMSEG_CONV) THEN
  SIMP_TAC[IMAGE_CLAUSES; coplanar; VECTOR_3] THEN EQ_TAC THENL
   [REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [GSYM DIM_SPAN] THEN MATCH_MP_TAC LET_TRANS THEN
    EXISTS_TAC `CARD {b - a:real^3,c - a}` THEN
    CONJ_TAC THENL
    ...
    GEN_REWRITE_TAC "Multivariate/flyspeck.ml:RAND_CONV" RAND_CONV [GSYM SPAN_SPAN] THEN
    ...
    DISCH_THEN(fun th -> REPEAT CONJ_TAC THEN
      GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [th]) THEN
...
let AFFINE_HULL_3_GENERATED = prove
 (`!s u v w:real^N.
  ...
  CONV_TAC "Multivariate/flyspeck.ml:SYM_CONV" SYM_CONV THEN
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:RAND_CONV" RAND_CONV [GSYM HULL_HULL] THEN
 ...
 (`!f:real^M->real^N sgn s t v.
  ...
  let lemma1 = prove
   (`!f:real^M->real^N s.
    ...
    SUBST1_TAC THENL [ASM SET_TAC[]; CONV_TAC "Multivariate/flyspeck.ml:SYM_CONV" SYM_CONV] THEN
...
let IN_AFFSIGN_TRANSLATION = prove
 (`!sgn s t a v:real^N.
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    CONV_TAC "Multivariate/flyspeck.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC VSUM_SUPERSET THEN
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [GSYM VECTOR_MUL_LID] THEN
...
let AFF_GE_1_1_0 = prove
 (`!v. ~(v = vec 0) ==> aff_ge {vec 0} {v} = {a % v | &0 <= a}`,
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [SET_RULE `{a} = {a,a}`] THEN
...
let CONVEX_AFFSIGN = prove
 (`!sgn. (!x y u. sgn(x) /\ sgn(y) /\ &0 <= u /\ u <= &1
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [sum] THEN
...
let CONVEX_AFF_GT = prove
 (`!s t. convex(aff_gt s t)`,
  ...
  CONV_TAC "Multivariate/flyspeck.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let AFFSIGN_SPECIAL_SCALE = prove
 (`!sgn s t a v.
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:BINOP_CONV" BINOP_CONV [SWAP_EXISTS_THM] THEN
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(BINOP_CONV o BINDER_CONV)" (BINOP_CONV o BINDER_CONV) [SWAP_EXISTS_THM] THEN
  ...
  DISCH_THEN(CONV_TAC "Multivariate/flyspeck.ml:o" o RAND_CONV o EXPAND_QUANTS_CONV) THEN
...
let AFF_GE_SCALE_LEMMA = prove
 (`!a u v:real^N.
  ...
  UNDISCH_TAC `&0 < a` THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_FIELD" REAL_FIELD);;
...
let ANGLES_ADD_AFF_GE = prove
 (`!u v w x:real^N.
  ...
  UNDISCH_TAC `&0 < a + b` THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_FIELD" REAL_FIELD);;
 ...
 (`!s a b:real^N.
  ...
  STRIP_ASSUME_TAC THENL
   [MATCH_MP_TAC COMPACT_CONVEX_COLLINEAR_SEGMENT THEN
    ...
    GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV) [HALFLINE; IN_ELIM_THM] THEN
    DISCH_THEN(CONJUNCTS_THEN2
...
let ORTHONORMAL_CROSS = prove
 (`!e1 e2 e3.
  ...
  CONJ_TAC THENL [CONV_TAC "Multivariate/flyspeck.ml:REAL_RING" REAL_RING; ALL_TAC] THEN
...
let AZIM_EXISTS = prove
 (`!v w w1 w2.
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:I" I [SWAP_EXISTS_THM] THEN
  EXISTS_TAC `(w dot (w1:real^3)) / (w dot w)` THEN
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:I" I [SWAP_EXISTS_THM] THEN
  EXISTS_TAC `(w dot (w2:real^3)) / (w dot w)` THEN
  GEOM_BASIS_MULTIPLE_TAC 3 `w:real^3` THEN
  X_GEN_TAC `w:real` THEN GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV
  ...
  THENL
   [ASM_SIMP_TAC[COLLINEAR_LEMMA; VECTOR_MUL_EQ_0; REAL_LT_IMP_NZ;
    ...
    CONV_TAC "Multivariate/flyspeck.ml:(BINOP_CONV(BINOP_CONV(ONCE_DEPTH_CONV SYM_CONV)))" (BINOP_CONV(BINOP_CONV(ONCE_DEPTH_CONV SYM_CONV))) THEN
  ...
  ASM_CASES_TAC `v1:real^2 = vec 0` THEN ASM_REWRITE_TAC[NORM_POS_LT] THENL
   [MP_TAC(ISPECL [`basis 1:real^2`; `v2:real^2`]
    ...
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [SYM th]) THEN
 ...
 (`!v w w1 w2:real^3.
  ...
  COND_CASES_TAC THENL
   [ALL_TAC;
    ...
    CONV_TAC "Multivariate/flyspeck.ml:SELECT_CONV" SELECT_CONV THEN
...
let AZIM_UNIQUE = prove
 (`!v w w1 w2 h1 h2 r1 r2 e1 e2 e3 psi theta.
  ...
  SUBGOAL_THEN `(azim v w w1 w2 - theta) / (&2 * pi) = &0` MP_TAC THENL
   [ALL_TAC; MP_TAC PI_POS THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_FIELD" REAL_FIELD] THEN
 ...
 (`!f. linear f /\ (!x. norm(f x) = norm x) /\
  ...
  ANTS_TAC THENL
   [ASM_MESON_TAC[ORTHOGONAL_TRANSFORMATION;
    ...
    ALL_TAC] THEN
  DISCH_THEN(CONV_TAC "Multivariate/flyspeck.ml:o" o LAND_CONV o EXPAND_QUANTS_CONV) THEN
...
let AZIM_SPECIAL_SCALE = prove
 (`!a v w1 w2.
  ...
  FIRST_ASSUM(MP_TAC o MATCH_MP(MESON[REAL_LT_IMP_NZ; REAL_DIV_LMUL]
   `!a. &0 < a ==> (!y. ?x. a * x = y)`)) THEN
  DISCH_THEN(MP_TAC o MATCH_MP QUANTIFY_SURJECTION_THM) THEN
  DISCH_THEN(CONV_TAC "Multivariate/flyspeck.ml:o" o RAND_CONV o
...
let AZIM_SCALE_ALL = prove
 (`!a v w1 w2.
  ...
  let SCALE_QUANT_TAC side asm avoid =
    ...
    DISCH_THEN(MP_TAC o MATCH_MP QUANTIFY_SURJECTION_THM) THEN
    DISCH_THEN(CONV_TAC "Multivariate/flyspeck.ml:o" o side o PARTIAL_EXPAND_QUANTS_CONV avoid) in
  REPEAT STRIP_TAC THEN
 ...
 (`!x y:real^3. azim (vec 0) (basis 3) x y = Arg(dropout 3 y / dropout 3 x)`,
  let lemma = prove
   (`(r * cos t) % basis 1 + (r * sin t) % basis 2 = Cx r * cexp(ii * Cx t)`,
    ...
    CONV_TAC "Multivariate/flyspeck.ml:COMPLEX_RING" COMPLEX_RING) in
  REPEAT STRIP_TAC THEN
  ...
  CONV_TAC "Multivariate/flyspeck.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC ARG_UNIQUE THEN
  EXISTS_TAC `r2 / r1:real` THEN ASM_SIMP_TAC[REAL_LT_DIV] THEN
  AP_TERM_TAC THEN AP_TERM_TAC THEN REWRITE_TAC[CX_ADD] THEN
  CONV_TAC "Multivariate/flyspeck.ml:COMPLEX_RING" COMPLEX_RING);;
...
let REAL_CONTINUOUS_AT_AZIM_SHARP = prove
 (`!v w w1 w2.
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [REAL_LE_LT] THEN ASM_REWRITE_TAC[] THEN
...
let AZIM_REFL = prove
 (`!v0 v1 w. azim v0 v1 w w = &0`,
  ...
  GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [REAL_ARITH `&0 <= x <=> x = &0 \/ &0 < x`] THEN
 ...
 (`!v0 v1 w x y.
  ...
  ANTS_TAC THENL
   [ASM_REWRITE_TAC[IN_INSERT; FINITE_INSERT; FINITE_EMPTY; NOT_IN_EMPTY] THEN
    ...
    DISCH_THEN SUBST1_TAC] THEN
  REWRITE_TAC[AZIM_ARG] THEN CONV_TAC "Multivariate/flyspeck.ml:(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN
  ...
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/flyspeck.ml:I" I [FUN_EQ_THM] THEN
...
let AZIM_COMPL_EQ_0 = prove
 (`!z w w1 w2.
  ...
  GEOM_BASIS_MULTIPLE_TAC 3 `w:real^3` THEN
  X_GEN_TAC `w:real` THEN GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [REAL_LE_LT] THEN
 ...
 (`!v0 v1 w x.
  ...
  ANTS_TAC THENL
   [ASM_REWRITE_TAC[IN_INSERT; FINITE_INSERT; FINITE_EMPTY; NOT_IN_EMPTY] THEN
    ...
    DISCH_THEN SUBST1_TAC] THEN
  REWRITE_TAC[AZIM_ARG] THEN CONV_TAC "Multivariate/flyspeck.ml:(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN
  CONV_TAC "Multivariate/flyspeck.ml:(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN REWRITE_TAC[ARG_EQ_PI] THEN
  MATCH_MP_TAC EQ_TRANS THEN EXISTS_TAC
  ...
  CONJ_TAC THENL
   [REPEAT(POP_ASSUM MP_TAC) THEN REWRITE_TAC[COLLINEAR_BASIS_3] THEN
    ...
    X_GEN_TAC `v:real` THEN GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [REAL_LE_LT] THEN
    ...
    EQ_TAC THENL
     [REWRITE_TAC[GSYM real; REAL] THEN
      ...
      CONJ_TAC THENL
       [ALL_TAC; REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_FIELD" REAL_FIELD];
    ...
    GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(RAND_CONV o BINDER_CONV)" (RAND_CONV o BINDER_CONV) [SWAP_EXISTS_THM] THEN
    REWRITE_TAC[RIGHT_EXISTS_AND_THM; UNWIND_THM2] THEN
 ...
 (`!v0 v1 w1 w2.
  ...
  REPEAT GEN_TAC THEN ASM_CASES_TAC `collinear {v0:real^3,v1,w1}` THENL
   [MP_TAC(ISPECL [`v0:real^3`; `v1:real^3`; `w1:real^3`; `w2:real^3`]
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "Multivariate/flyspeck.ml:TAUT" TAUT;
    POP_ASSUM MP_TAC] THEN
  ASM_CASES_TAC `collinear {v0:real^3,v1,w2}` THENL
   [MP_TAC(ISPECL [`v0:real^3`; `v1:real^3`; `w2:real^3`; `w1:real^3`]
                NOT_COPLANAR_NOT_COLLINEAR) THEN
    ASM_REWRITE_TAC[] THEN REWRITE_TAC[INSERT_AC] THEN CONV_TAC "Multivariate/flyspeck.ml:TAUT" TAUT;
    POP_ASSUM MP_TAC] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_RING" REAL_RING;
    EXISTS_TAC `(w2:real^3)$2 / (w1:real^3)$2` THEN
    REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_FIELD" REAL_FIELD;
...
let REAL_SGN_SIN_AZIM = prove
 (`!v w x y. real_sgn(sin(azim v w x y)) =
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [real_sgn] THEN
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [real_sgn] THEN
  ASM_REWRITE_TAC[REAL_DIV_1] THEN ASM_CASES_TAC `z = Cx(&0)` THEN
  ASM_REWRITE_TAC[IM_CX; ARG_0; SIN_0] THEN
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(funpow 3 RAND_CONV)" (funpow 3 RAND_CONV) [ARG] THEN
...
let DIHV_LINEAR_IMAGE = prove
 (`!f:real^M->real^N w0 w1 w2 w3.
  ...
  CONV_TAC "Multivariate/flyspeck.ml:(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN
...
let DIHV_RANGE = prove
 (`!w0 w1 w2 w3. &0 <= dihV w0 w1 w2 w3 /\ dihV w0 w1 w2 w3 <= pi`,
  REWRITE_TAC[DIHV] THEN CONV_TAC "Multivariate/flyspeck.ml:(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN
...
let COS_AZIM_DIHV = prove
 (`!v w v1 v2:real^3.
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(funpow 3 LAND_CONV)" (funpow 3 LAND_CONV) [GSYM COS_NEG] THEN
...
let DIHV_NEG_0 = prove
 (`!v1 v2 v3. dihV (vec 0) (--v1) (--v2) (--v3) = dihV (vec 0) v1 v2 v3`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:RAND_CONV" RAND_CONV [GSYM DIHV_NEG] THEN
...
let REAL_CONTINUOUS_AT_DIHV = prove
 (`!v w w1 w2:real^N.
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
  REWRITE_TAC[dihV] THEN CONV_TAC "Multivariate/flyspeck.ml:(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [GSYM o_DEF] THEN
  MATCH_MP_TAC REAL_CONTINUOUS_CONTINUOUS_AT_COMPOSE THEN CONJ_TAC THENL
   [MATCH_MP_TAC CONTINUOUS_SUB THEN CONJ_TAC THEN
    ...
    GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
 ...
 (`!f:real^M->real^3 g h k x s.
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[GSYM closed] THEN
    ...
    TRY(GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [GSYM o_DEF]) THEN
 ...
 (`!u v w u' e1 e2 e3 r phi theta.
  ...
  MP_TAC THENL
   [ALL_TAC;
    ...
    SIMP_TAC[DOT_BASIS; DIMINDEX_3; ARITH] THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_RING" REAL_RING] THEN
  REPEAT(FIRST_X_ASSUM(MP_TAC o
  ...
  X_GEN_TAC `k:real` THEN GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [REAL_LE_LT] THEN
...
let WEDGE_TRANSLATION = prove
 (`!a v w w1 w2. wedge (a + v) (a + w) (a + w1) (a + w2) =
  ...
  REPEAT GEN_TAC THEN CONV_TAC "Multivariate/flyspeck.ml:SYM_CONV" SYM_CONV THEN
...
let WEDGE_LINEAR_IMAGE = prove
 (`!f. linear f /\ (!x. norm(f x) = norm x) /\
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/flyspeck.ml:SYM_CONV" SYM_CONV THEN
  MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN CONJ_TAC THENL
   [ASM_MESON_TAC[ORTHOGONAL_TRANSFORMATION_SURJECTIVE;
...
let WEDGE_LUNE_GT = prove
 (`!v0 v1 w1 w2.
  ...
  X_GEN_TAC `v1:real` THEN GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [REAL_LE_LT] THEN
     ...
     [REWRITE_TAC[REAL_CONTINUOUS_ON_EQ_CONTINUOUS_WITHIN] THEN
      ...
      REWRITE_TAC[] THEN CONJ_TAC THENL
       [MATCH_MP_TAC CONTINUOUS_ADD THEN CONJ_TAC THENL
         [GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [SYM(CONJUNCT2(SPEC_ALL I_O_ID))] THEN
          REWRITE_TAC[GSYM REAL_CONTINUOUS_CONTINUOUS] THEN
        ...
        EXISTS_TAC `{z | &0 <= Im z}` THEN CONJ_TAC THENL
         [MATCH_MP_TAC CONTINUOUS_WITHIN_UPPERHALF_ARG THEN
          ASM_CASES_TAC `t = &1` THENL
           [ASM_REWRITE_TAC[REAL_SUB_REFL] THEN CONV_TAC "Multivariate/flyspeck.ml:COMPLEX_RING" COMPLEX_RING;
            ALL_TAC] THEN
      ...
      ASM_CASES_TAC `t = &1` THENL
       [ASM_REWRITE_TAC[REAL_SUB_REFL; COMPLEX_MUL_LZERO] THEN
        ...
        ALL_TAC] THEN
      GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [REAL_LE_LT] THEN
      ...
      GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(BINOP_CONV o RAND_CONV)" (BINOP_CONV o RAND_CONV) [ARG] THEN
...
let WEDGE = prove
 (`wedge v1 v2 w1 w2 =
   ...
   [REPEAT STRIP_TAC THEN REWRITE_TAC[EXTENSION] THEN X_GEN_TAC `y:real^3` THEN
    ...
    CONJ_TAC THENL
     [FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE LAND_CONV [AZIM_ARG]) THEN
      ...
      ASM_REWRITE_TAC[DISJOINT_INSERT; DISJOINT_EMPTY; IN_SING] THEN
      ANTS_TAC THENL
       [CONV_TAC "Multivariate/flyspeck.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
      ...
      EQ_TAC THENL
       [DISCH_TAC THEN
        ...
        UNDISCH_TAC `~(v = &0)` THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_FIELD" REAL_FIELD;
        REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN
        ...
        CONV_TAC "Multivariate/flyspeck.ml:REAL_FIELD" REAL_FIELD];
      ALL_TAC] THEN
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(funpow 3 RAND_CONV)" (funpow 3 RAND_CONV) [SET_RULE `{a,b} = {b,a}`] THEN
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM COMPLEX_INV_DIV] THEN
...
let VOLUME_OF_TETRAHEDRON = prove
 (`!v1 v2 v3 v4:real^3.
  ...
  REPEAT GEN_TAC THEN CONV_TAC "Multivariate/flyspeck.ml:(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN
...
let AREA_UNIT_CBALL = prove
 (`measure(cball(vec 0:real^2,&1)) = pi`,
  ...
  REWRITE_TAC[ASN_1; ASN_NEG_1] THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  DISCH_THEN MATCH_MP_TAC THEN CONJ_TAC THENL
   [MATCH_MP_TAC REAL_CONTINUOUS_ON_ADD THEN
    ...
    REAL_DIFF_TAC THEN
    CONV_TAC "Multivariate/flyspeck.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
...
let AREA_CBALL = prove
 (`!z:real^2 r. &0 <= r ==> measure(cball(z,r)) = pi * r pow 2`,
  ...
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [REAL_MUL_SYM] THEN
 ...
 (`!z:real^3 r. &0 <= r ==> measure(cball(z,r)) = &4 / &3 * pi * r pow 3`,
  ...
  REWRITE_TAC[] THEN ANTS_TAC THENL
   [CONJ_TAC THENL [ASM_REAL_ARITH_TAC; ALL_TAC] THEN
    ...
    CONV_TAC "Multivariate/flyspeck.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_RING" REAL_RING;
...
let VOLUME_FRUSTT_STRONG = prove
 (`!v0 v1:real^3 h a.
  ...
  CONV_TAC "Multivariate/flyspeck.ml:(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN GEOM_ORIGIN_TAC `v0:real^3` THEN
...
let MEASURE_SCALE = prove
 (`!s. measurable s
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [HAS_MEASURE_MEASURE] THEN
 ...
 (`!w z:real^3 h. ~(w = z) ==> negligible(rcone_eq z w h)`,
  ...
  ASM_CASES_TAC `abs(h) <= &1` THENL
   [MP_TAC(ISPECL [`w:real^3`; `acs h`] NEGLIGIBLE_CIRCULAR_CONE_0) THEN
    ...
    GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [DOT_SYM] THEN
 ...
 (`!t r. &0 <= r /\ &0 <= t /\ t <= &2 * pi
  ...
  REWRITE_TAC[] THEN REPEAT STRIP_TAC THENL
   [MATCH_MP_TAC REALLIM_TRANSFORM_BOUND THEN
    ...
    AP_TERM_TAC THEN CONV_TAC "Multivariate/flyspeck.ml:SYM_CONV" SYM_CONV THEN
    MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN CONJ_TAC THENL
     [ASM_MESON_TAC[ORTHOGONAL_TRANSFORMATION_SURJECTIVE;
   ...
   [DISCH_THEN(X_CHOOSE_THEN `v:real` ASSUME_TAC) THEN
    ...
    REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ASM_MESON_TAC[];
      ...
      FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/flyspeck.ml:RAND_CONV" RAND_CONV [GSYM th]) THEN
    ...
    DISCH_THEN(DISJ_CASES_THEN MP_TAC) THENL
     [DISCH_THEN(X_CHOOSE_THEN `a:real`
      ...
      FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC "Multivariate/flyspeck.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
      ...
      GEN_REWRITE_TAC "Multivariate/flyspeck.ml:RAND_CONV" RAND_CONV [SWAP_EXISTS_THM] THEN
      MATCH_MP_TAC MONO_EXISTS THEN REPEAT STRIP_TAC THEN
   ...
   [MATCH_MP_TAC(SET_RULE
    ...
    ASM_SIMP_TAC[SET_RULE
     `~(a IN s) ==> ((w IN s UNION t ==> w = a \/ w IN t ==> P w) <=>
                     (w IN t ==> P w))`] THEN
    GEN_REWRITE_TAC "Multivariate/flyspeck.ml:I" I [EXTENSION] THEN
 ...
 (`!r s t v:real^N.
  ...
  DISCH_TAC THEN ASM_REWRITE_TAC[VECTOR_NEG_EQ_0] THEN CONJ_TAC THENL
   [FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP
    ...
    GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [GSYM VECTOR_NEG_NEG] THEN
...
let MEASURE_LUNE_DECOMPOSITION = prove
 (`!v1 v2 v3:real^3.
  ...
  CONV_TAC "Multivariate/flyspeck.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MEASURE_BALL_AFF_GT_SHUFFLE THEN
...
let SOLID_TRIANGLE_CONGRUENT_NEG = prove
 (`!r v1 v2 v3:real^N.
  ...
  CONV_TAC "Multivariate/flyspeck.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
...
let VOLUME_SOLID_TRIANGLE = prove
 (`!r v0 v1 v2 v3:real^3.
  ...
  CONV_TAC "Multivariate/flyspeck.ml:(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
  ...
  MP_TAC THENL
   [MP_TAC(SPECL [`vec 0:real^3`; `r:real`] VOLUME_BALL) THEN
    ...
    CONV_TAC "Multivariate/flyspeck.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
    ALL_TAC] THEN
  SUBGOAL_THEN
...
let VOLUME_FRUSTT_WEDGE = prove
 (`!v0 v1:real^3 w1 w2 h a.
  ...
  REWRITE_TAC[VECTOR_SUB_RZERO; REAL_MUL_LZERO; DIST_0; real_gt] THEN
  GEOM_BASIS_MULTIPLE_TAC 3 `v1:real^3` THEN
  X_GEN_TAC `b:real` THEN GEN_REWRITE_TAC "Multivariate/flyspeck.ml:LAND_CONV" LAND_CONV [REAL_LE_LT] THEN
  ...
  THENL
   [REWRITE_TAC[EXTENSION] THEN
    ...
    REAL_ARITH_TAC;
    ALL_TAC] THEN
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV) [COND_RATOR; COND_RAND] THEN
  GEN_REWRITE_TAC "Multivariate/flyspeck.ml:(RAND_CONV o RATOR_CONV o LAND_CONV o TOP_DEPTH_CONV)" (RAND_CONV o RATOR_CONV o LAND_CONV o TOP_DEPTH_CONV)
  ...
  CONJ_TAC THENL
   [X_GEN_TAC `t:real` THEN DISCH_TAC THEN REWRITE_TAC[] THEN
    ...
    CONV_TAC "Multivariate/flyspeck.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_MUL_RZERO] THEN
 ...
 (`!v0 v1:real^3 w1 w2 r a.
  ...
  THENL
   [ASM_SIMP_TAC[VOLUME_CONIC_CAP_WEDGE_WEAK] THEN
    ...
    ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN CONV_TAC "Multivariate/flyspeck.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  COND_CASES_TAC THENL
...
let POLYHEDRON_COLLINEAR_FACES_STRONG = prove
 (`!P:real^N->bool f f' p q s t.
  ...
  EXPAND_TAC "p" THEN REWRITE_TAC[IN_SEGMENT] THEN CONJ_TAC THENL
   [CONV_TAC "Multivariate/flyspeck.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
