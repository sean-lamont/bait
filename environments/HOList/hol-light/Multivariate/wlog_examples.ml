 ...
 (`!f:real^M->real^N sgn s t v.
  ...
  let lemma1 = prove
   (`!f:real^M->real^N s.
    ...
    SUBST1_TAC THENL [ASM SET_TAC[]; CONV_TAC "Multivariate/wlog_examples.ml:SYM_CONV" SYM_CONV] THEN
...
let IN_AFFSIGN_TRANSLATION = prove
 (`!sgn s t a v:real^N.
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    CONV_TAC "Multivariate/wlog_examples.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC VSUM_SUPERSET THEN
  ...
  GEN_REWRITE_TAC "Multivariate/wlog_examples.ml:LAND_CONV" LAND_CONV [GSYM VECTOR_MUL_LID] THEN
 ...
 (`!v1 v2 v3 w:real^3 p.
   ...
   [ASM_SIMP_TAC[AFFINE_HULL_EQ_SPAN] THEN
    ...
    EXISTS_TAC `span {x - v1:real^3 | x IN {v2,v3}}` THEN CONJ_TAC THENL
     [REWRITE_TAC[SUBSET] THEN MATCH_MP_TAC SPAN_INDUCT THEN
      ...
      ALL_TAC] THEN
    CONJ_TAC THENL
     [GEN_REWRITE_TAC "Multivariate/wlog_examples.ml:RAND_CONV" RAND_CONV [GSYM SPAN_SPAN] THEN
  ...
  EQ_TAC THEN REWRITE_TAC[LEFT_IMP_EXISTS_THM] THENL
   [MAP_EVERY X_GEN_TAC [`a:real`; `b:real`; `c:real`; `h:real`] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/wlog_examples.ml:LAND_CONV" LAND_CONV [GSYM BASIS_EXPANSION] THEN
    REWRITE_TAC[DIMINDEX_3] THEN CONV_TAC "Multivariate/wlog_examples.ml:(ONCE_DEPTH_CONV NUMSEG_CONV)" (ONCE_DEPTH_CONV NUMSEG_CONV) THEN
    ...
    GEN_REWRITE_TAC "Multivariate/wlog_examples.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM BASIS_EXPANSION] THEN
    REWRITE_TAC[DIMINDEX_3] THEN CONV_TAC "Multivariate/wlog_examples.ml:(ONCE_DEPTH_CONV NUMSEG_CONV)" (ONCE_DEPTH_CONV NUMSEG_CONV) THEN
    SIMP_TAC[VSUM_CLAUSES; FINITE_INSERT; CONJUNCT1 FINITE_RULES] THEN
