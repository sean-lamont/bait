...
let ROTATION_PLANE_HORIZONTAL = prove
 (`!s. plane s
  ...
  GEN_REWRITE_TAC "Multivariate/wlog.ml:RAND_CONV" RAND_CONV [GSYM lemma] THEN
...
let PAD2D3D_TAC =
  ...
  let lasttac =
    GEN_REWRITE_TAC "Multivariate/wlog.ml:REDEPTH_CONV" REDEPTH_CONV [LINEAR_INVARIANTS pad2d3d_tm pths] in
  fun gl -> (GEN_REWRITE_TAC "Multivariate/wlog.ml:ONCE_DEPTH_CONV" ONCE_DEPTH_CONV [cth] THEN
             CONV_TAC "Multivariate/wlog.ml:(DEPTH_CONV PAD2D3D_QUANTIFY_CONV)" (DEPTH_CONV PAD2D3D_QUANTIFY_CONV) THEN
             lasttac) gl;;
