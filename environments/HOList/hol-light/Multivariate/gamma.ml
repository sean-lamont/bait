...
let EULER_MASCHERONI = prove
 (`((\n. sum(1..n) (\k. inv(&k)) - log(&n)) ---> euler_mascheroni)
   sequentially`,
  REWRITE_TAC[euler_mascheroni; reallim] THEN CONV_TAC "Multivariate/gamma.ml:SELECT_CONV" SELECT_CONV THEN
  SUBGOAL_THEN
  ...
  REWRITE_TAC[VSUM_CLAUSES_NUMSEG] THEN CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  GEN_REWRITE_TAC "Multivariate/gamma.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
  MATCH_MP_TAC REAL_LE_LMUL THEN
  SIMP_TAC[REAL_LE_INV_EQ; REAL_POW_LE; REAL_POS] THEN
  GEN_REWRITE_TAC "Multivariate/gamma.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_INV] THEN
  MATCH_MP_TAC REAL_LE_INV2 THEN
  CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let LOG2_APPROX_40 = prove
 (`abs(log(&2) - &381061692393 / &549755813888) <= inv(&2 pow 40)`,
  ...
  CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  REWRITE_TAC[REAL_ARITH `a * b / c:real = a / c * b`] THEN
  CONV_TAC "Multivariate/gamma.ml:(ONCE_DEPTH_CONV EXPAND_SUM_CONV)" (ONCE_DEPTH_CONV EXPAND_SUM_CONV) THEN
  CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
 ...
 (`abs(euler_mascheroni - &2479122403 / &4294967296) <= inv(&2 pow 32)`,
  let lemma1 = prove
   (`!m n. 1 <= m /\ m <= n
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ANTS_TAC THENL
     [REWRITE_TAC[IN_REAL_INTERVAL] THEN REPEAT
      ...
      CONV_TAC "Multivariate/gamma.ml:(ONCE_DEPTH_CONV EXPAND_SUM_CONV)" (ONCE_DEPTH_CONV EXPAND_SUM_CONV) THEN
      CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
      CONV_TAC "Multivariate/gamma.ml:(ONCE_DEPTH_CONV BERNOULLI_CONV)" (ONCE_DEPTH_CONV BERNOULLI_CONV) THEN
    ...
    ASM_REWRITE_TAC[REAL_OF_NUM_LE] THEN ANTS_TAC THENL
     [REWRITE_TAC[IN_REAL_INTERVAL] THEN REPEAT STRIP_TAC THEN
      REAL_DIFF_TAC THEN CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
      SUBGOAL_THEN `~(x = &0)` MP_TAC THENL
       [RULE_ASSUM_TAC(REWRITE_RULE[GSYM REAL_OF_NUM_LE]) THEN
        ASM_REAL_ARITH_TAC;
        CONV_TAC "Multivariate/gamma.ml:REAL_FIELD" REAL_FIELD];
    ...
    CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_MUL_LID] THEN
  ...
  REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [GEN_TAC THEN
    ...
    GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM REAL_ADD_RID] THEN
    MATCH_MP_TAC REALLIM_ADD THEN REWRITE_TAC[REALLIM_CONST] THEN
    REWRITE_TAC[REALLIM_NULL_NEG] THEN
    REWRITE_TAC[REALLIM_1_OVER_N] THEN
    MATCH_MP_TAC REALLIM_1_OVER_POW THEN CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
    ALL_TAC] THEN
  ...
  SUBGOAL_THEN `log(&64) = &6 * log(&2)` SUBST1_TAC THENL
   [SIMP_TAC[GSYM LOG_POW; REAL_ARITH `&0 < &2`] THEN
    AP_TERM_TAC THEN CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    CONV_TAC "Multivariate/gamma.ml:(ONCE_DEPTH_CONV EXPAND_SUM_CONV)" (ONCE_DEPTH_CONV EXPAND_SUM_CONV) THEN
    CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let LGAMMA,COMPLEX_DIFFERENTIABLE_AT_LGAMMA = (CONJ_PAIR o prove)
 (`(!z. (!n. ~(z + Cx(&n) = Cx(&0)))
   ...
   [REPEAT STRIP_TAC THEN
    ...
    THENL
     [REWRITE_TAC[VSUM_SUB_NUMSEG] THEN
      ...
      REWRITE_TAC[GSYM REAL_OF_NUM_ADD; REAL_ARITH `(x + &1) - &1 = x`;
                  REAL_SUB_REFL] THEN
      REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING;
      ALL_TAC] THEN
    ...
    SUBST1_TAC THENL
     [ASM_SIMP_TAC[GSYM CX_LOG; REAL_OF_NUM_LE; REAL_SUB_LT; REAL_INV_LT_1;
      ...
      REWRITE_TAC[GSYM REAL_OF_NUM_LE] THEN CONV_TAC "Multivariate/gamma.ml:REAL_FIELD" REAL_FIELD;
      ALL_TAC] THEN
    MP_TAC(ISPECL [`1`; `z / Cx(&k)`] TAYLOR_CLOG) THEN
    MP_TAC(ISPECL [`1`; `--inv(Cx(&k))`] TAYLOR_CLOG) THEN
    REWRITE_TAC[VSUM_SING_NUMSEG] THEN CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    ...
    SUBGOAL_THEN `norm(z:complex) < &k` ASSUME_TAC THENL
     [FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [IN_CBALL]) THEN
      ...
      CONV_TAC "Multivariate/gamma.ml:NORM_ARITH" NORM_ARITH;
    ...
    REPEAT CONJ_TAC THENL
     [FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [IN_CBALL]) THEN
      ...
      REWRITE_TAC[real_div; REAL_MUL_LID] THEN
      GEN_REWRITE_TAC "Multivariate/gamma.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_INV] THEN
      MATCH_MP_TAC REAL_LE_INV2 THEN CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    ...
    REWRITE_TAC[GSYM REAL_OF_NUM_LE; GSYM REAL_OF_NUM_ADD] THEN
    CONV_TAC "Multivariate/gamma.ml:NORM_ARITH" NORM_ARITH;
    GEN_REWRITE_TAC "Multivariate/gamma.ml:(LAND_CONV o REDEPTH_CONV)" (LAND_CONV o REDEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
  ...
  MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
   [REPEAT STRIP_TAC THEN
    REWRITE_TAC[lgamma; lim] THEN CONV_TAC "Multivariate/gamma.ml:SELECT_CONV" SELECT_CONV THEN
  ...
  MP_TAC THENL
   [MATCH_MP_TAC(ISPEC `sequentially` HOLOMORPHIC_UNIFORM_LIMIT) THEN
    ...
    GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM o_DEF] THEN
 ...
 (`!z. (!n. ~(z + Cx(&n) = Cx(&0)))
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN EXISTS_TAC `1` THEN
    ...
    CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING;
   ...
   [MATCH_MP_TAC LIM_TRANSFORM_EVENTUALLY THEN
    ...
    CONJ_TAC THENL
     [REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN EXISTS_TAC `1` THEN
      ...
      GEN_REWRITE_TAC "Multivariate/gamma.ml:RAND_CONV" RAND_CONV [COMPLEX_RING
      ...
      GEN_REWRITE_TAC "Multivariate/gamma.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [GSYM COMPLEX_INV_INV] THEN
      REWRITE_TAC[GSYM COMPLEX_INV_MUL] THEN AP_TERM_TAC THEN
      GEN_REWRITE_TAC "Multivariate/gamma.ml:RAND_CONV" RAND_CONV [COMPLEX_MUL_SYM] THEN
      GEN_REWRITE_TAC "Multivariate/gamma.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [COMPLEX_ADD_SYM] THEN
    ...
    MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
     [REWRITE_TAC[cgamma; lim] THEN CONV_TAC "Multivariate/gamma.ml:SELECT_CONV" SELECT_CONV THEN ASM_MESON_TAC[];
      DISCH_TAC] THEN
 ...
 (`!z. cgamma(z) = cgamma(z + Cx(&1)) / z`,
  GEN_TAC THEN ASM_CASES_TAC `?n.  z + Cx(&n) = Cx(&0)` THENL
   [FIRST_ASSUM(fun th -> REWRITE_TAC[REWRITE_RULE[GSYM CGAMMA_EQ_0] th]) THEN
    CONV_TAC "Multivariate/gamma.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[COMPLEX_DIV_EQ_0] THEN
  ...
  GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM COMPLEX_MUL_LID] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN EXISTS_TAC `1` THEN
    ...
    REWRITE_TAC[CX_SUB];
    ALL_TAC] THEN
  GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM CEXP_0] THEN
  MATCH_MP_TAC(ISPEC `cexp` LIM_CONTINUOUS_FUNCTION) THEN
 ...
 (`!n. ((\z. (z + Cx(&n)) * cgamma z) --> --Cx(&1) pow n / Cx(&(FACT n)))
  ...
  INDUCT_TAC THENL
   [REWRITE_TAC[COMPLEX_ADD_RID; COMPLEX_NEG_0; FACT; complex_pow;
    ...
    CONJ_TAC THENL [CONV_TAC "Multivariate/gamma.ml:COMPLEX_FIELD" COMPLEX_FIELD; ALL_TAC] THEN
    ...
    CONJ_TAC THENL
     [FIRST_X_ASSUM(MP_TAC o
      ...
      CONJ_TAC THENL
       [LIM_TAC THEN CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING;
      ...
      GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM ETA_AX] THEN
...
let CNJ_CGAMMA = prove
 (`!z. cnj(cgamma z) = cgamma(cnj z)`,
  ...
  MATCH_MP_TAC(REWRITE_RULE[TAUT `a /\ b /\ c ==> d <=> a /\ b ==> c ==> d`]
        LIM_UNIQUE) THEN
  REWRITE_TAC[TRIVIAL_LIMIT_SEQUENTIALLY] THEN
  GEN_REWRITE_TAC "Multivariate/gamma.ml:I" I [GSYM LIM_CNJ] THEN REWRITE_TAC[CNJ_CNJ] THEN
  MP_TAC(SPEC `z:complex` CGAMMA) THEN
 ...
 (`!z. cgamma(z) * cgamma(z + Cx(&1) / Cx(&2)) =
  ...
  CHOOSE_TAC THENL
   [EXISTS_TAC
    ...
    REWRITE_TAC[ADD1] THEN CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  ASM_CASES_TAC `!n. ~(Cx(&2) * z + Cx(&n) = Cx(&0))` THENL
   [FIRST_X_ASSUM(fun th ->
    ...
    ANTS_TAC THENL [REAL_ARITH_TAC; CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV] THEN
 ...
 (`!z. cgamma(z) * cgamma(Cx(&1) - z) = Cx pi / csin(Cx pi * z)`,
   ...
   [GEN_TAC THEN EXPAND_TAC "g" THEN
    ...
    CONJ_TAC THENL
     [REWRITE_TAC[GSYM CX_SUB] THEN
      ...
      GEN_REWRITE_TAC "Multivariate/gamma.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [CGAMMA_RECURRENCE_ALT] THEN
      REWRITE_TAC[COMPLEX_RING `a - (z + a):complex = --z`] THEN
      GEN_REWRITE_TAC "Multivariate/gamma.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV)
     ...
     [X_GEN_TAC `w:complex` THEN REWRITE_TAC[COMPLEX_IN_BALL_0] THEN
      STRIP_TAC THEN COND_CASES_TAC THENL
       [UNDISCH_TAC `~(w = Cx(&0))` THEN
        ...
        REWRITE_TAC[GSYM CX_INJ] THEN CONV_TAC "Multivariate/gamma.ml:COMPLEX_FIELD" COMPLEX_FIELD];
      GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [COMPLEX_RING
   ...
   [MATCH_MP_TAC(SET_RULE
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC CONTINUOUS_ON_SUB THEN CONJ_TAC THEN
      ...
      TRY(GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM o_DEF] THEN
    ...
    CONV_TAC "Multivariate/gamma.ml:(DEPTH_BINOP_CONV `==>` (BINOP_CONV(DEPTH_BINOP_CONV `complex_mul` (RAND_CONV COMPLEX_POLY_CONV))))" (DEPTH_BINOP_CONV `==>` (BINOP_CONV(DEPTH_BINOP_CONV `complex_mul` (RAND_CONV COMPLEX_POLY_CONV)))) THEN
  ...
  ASSUME_TAC THENL
   [X_GEN_TAC `z:complex` THEN MATCH_MP_TAC
    ...
    (CONJ_TAC THENL [ASM_REWRITE_TAC[]; CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING]);
    ALL_TAC] THEN
  ...
  ASSUME_TAC THENL
   [X_GEN_TAC `z:complex` THEN
    ...
    EXISTS_TAC `z:complex` THEN CONJ_TAC THENL
     [REWRITE_TAC[]; ASM_REWRITE_TAC[]] THEN
    ASM GEN_COMPLEX_DIFF_TAC [] THEN
    (CONJ_TAC THENL [ASM_REWRITE_TAC[ETA_AX]; CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING]);
    ALL_TAC] THEN
  MP_TAC(ISPECL [`h':complex->complex`; `h'':complex->complex`; `(:complex)`]
        HOLOMORPHIC_DERIVATIVE) THEN
  ASM_REWRITE_TAC[OPEN_UNIV] THEN DISCH_TAC THEN
  SUBGOAL_THEN `!z. z IN (:complex) ==> h''(z) = Cx(&0)` MP_TAC THENL
   [MATCH_MP_TAC ANALYTIC_CONTINUATION THEN
    ...
    REWRITE_TAC[o_DEF] THEN CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ANTS_TAC THENL
     [MATCH_MP_TAC CONTINUOUS_ON_LIFT_NORM_COMPOSE THEN
      ...
      FIRST_X_ASSUM(fun th ->
        GEN_REWRITE_TAC "Multivariate/gamma.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM th]) THEN
      ...
      MP_TAC(SPEC `&1` COMPLEX_NORM_CX) THEN CONV_TAC "Multivariate/gamma.ml:NORM_ARITH" NORM_ARITH];
  ...
  REWRITE_TAC[CONVEX_UNIV; IN_UNIV] THEN ANTS_TAC THENL
   [GEN_TAC THEN ASM GEN_COMPLEX_DIFF_TAC[] THEN
    ASM_REWRITE_TAC[] THEN CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING;
   ...
   [SUBST1_TAC(SYM(SPEC `a:complex` COMPLEX)) THEN
    ...
    ASM_CASES_TAC `?n. integer n /\ z = Cx n` THENL
     [REWRITE_TAC[complex_div] THEN
      ...
      MP_TAC PI_NZ THEN REWRITE_TAC[GSYM CX_INJ] THEN CONV_TAC "Multivariate/gamma.ml:COMPLEX_FIELD" COMPLEX_FIELD];
    DISCH_THEN(fun th ->
    ...
    SUBST1_TAC(SYM(SPEC `a:complex` COMPLEX)) THEN
    ASM_REWRITE_TAC[] THEN
    GEN_REWRITE_TAC "Multivariate/gamma.ml:(funpow 3 LAND_CONV o RAND_CONV)" (funpow 3 LAND_CONV o RAND_CONV) [complex_mul] THEN
    ...
    COND_CASES_TAC THENL
     [FIRST_X_ASSUM(X_CHOOSE_THEN `m:real` STRIP_ASSUME_TAC) THEN
      ...
      CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
...
let CGAMMA_HALF = prove
 (`cgamma(Cx(&1) / Cx(&2)) = Cx(sqrt pi)`,
  ...
  SUBGOAL_THEN `Cx pi = Cx(sqrt pi) pow 2` SUBST1_TAC THENL
   [REWRITE_TAC[GSYM CX_POW] THEN AP_TERM_TAC THEN CONV_TAC "Multivariate/gamma.ml:SYM_CONV" SYM_CONV THEN
    ...
    CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN MATCH_MP_TAC(TAUT `~p ==> p ==> q`) THEN
 ...
 (`!z. ((\n. cexp(--(Cx euler_mascheroni) * z) / z *
  ...
  REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN CONJ_TAC THENL
   [EXISTS_TAC `1` THEN REPEAT STRIP_TAC THEN REWRITE_TAC[complex_div] THEN
    ...
    ASM_SIMP_TAC[CX_LOG; REAL_OF_NUM_LT; LE_1] THEN CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING;
    ALL_TAC] THEN
  GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM COMPLEX_MUL_LID] THEN
  MATCH_MP_TAC LIM_COMPLEX_MUL THEN CONJ_TAC THENL
   [SIMP_TAC[GSYM CEXP_VSUM; FINITE_NUMSEG; GSYM CEXP_ADD] THEN
    ...
    AP_TERM_TAC THEN GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV
     [COMPLEX_RING `a * b * c:complex = b * a * c`] THEN
    AP_TERM_TAC THEN SIMP_TAC[CPRODUCT_INV; FINITE_NUMSEG] THEN
    GEN_REWRITE_TAC "Multivariate/gamma.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [GSYM COMPLEX_INV_INV] THEN
    REWRITE_TAC[GSYM COMPLEX_INV_MUL] THEN AP_TERM_TAC THEN
    MATCH_MP_TAC(COMPLEX_FIELD
     `~(z = Cx(&0)) /\ z * x = y ==> inv z * y = x`) THEN
    REWRITE_TAC[CX_INJ; REAL_OF_NUM_EQ; FACT_NZ] THEN
    CONV_TAC "Multivariate/gamma.ml:SYM_CONV" SYM_CONV THEN SPEC_TAC(`n:num`,`p:num`) THEN
    INDUCT_TAC THEN REWRITE_TAC[FACT; CPRODUCT_CLAUSES_NUMSEG; ARITH] THEN
 ...
 (`!z. (inv o cgamma) complex_differentiable (at z)`,
  GEN_TAC THEN ASM_CASES_TAC `!n. ~(z + Cx(&n) = Cx(&0))` THENL
   [MATCH_MP_TAC COMPLEX_DIFFERENTIABLE_COMPOSE_AT THEN
    ...
    ONCE_REWRITE_TAC[COMPLEX_MUL_SYM] THEN
    REWRITE_TAC[CGAMMA_SIMPLE_POLES] THEN CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING]);;
 ...
 (`!n. ((\n. (z * cexp(Cx euler_mascheroni * z) *
  ...
  GEN_TAC THEN ASM_CASES_TAC `?n. z + Cx(&n) = Cx(&0)` THENL
   [FIRST_X_ASSUM(X_CHOOSE_TAC `N:num`) THEN
    ...
    GEN_REWRITE_TAC "Multivariate/gamma.ml:(RATOR_CONV o LAND_CONV o ABS_CONV)" (RATOR_CONV o LAND_CONV o ABS_CONV)
...
let GAMMA_RECURRENCE = prove
 (`!x. gamma(x + &1) = if x = &0 then &1 else x * gamma(x)`,
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "Multivariate/gamma.ml:RAND_CONV" RAND_CONV [COND_RAND] THEN
 ...
 (`!f:real->real.
   ...
   [REPEAT STRIP_TAC THEN
    SUBGOAL_THEN `(f:real->real) n = f((n - &1) + &1)` SUBST1_TAC THENL
     [AP_TERM_TAC THEN REAL_ARITH_TAC;
      ...
      UNDISCH_TAC `&2 <= n` THEN CONV_TAC "Multivariate/gamma.ml:REAL_FIELD" REAL_FIELD];
  ...
  THENL
   [INDUCT_TAC THEN REWRITE_TAC[ARITH; LT_0] THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "Multivariate/gamma.ml:(LAND_CONV o BINDER_CONV o RAND_CONV o BINOP_CONV)" (LAND_CONV o BINDER_CONV o RAND_CONV o BINOP_CONV)
  ...
  CONJ_TAC THENL
   [ASM_SIMP_TAC[REAL_LE_RDIV_EQ; REAL_MUL_LID] THEN
    ...
    REWRITE_TAC[real_div; GSYM REAL_MUL_ASSOC] THEN
    GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV
     ...
     [REWRITE_TAC[complex_pow; COMPLEX_MUL_RID; CPRODUCT_CLAUSES_NUMSEG] THEN
      ...
      DISCH_THEN MATCH_MP_TAC THEN REWRITE_TAC[REAL_POS] THEN CONJ_TAC THENL
       [MATCH_MP_TAC CONTINUOUS_ON_COMPLEX_RMUL THEN
        ...
        UNDISCH_TAC `~(z = Cx(&0))` THEN CONV_TAC "Multivariate/gamma.ml:COMPLEX_FIELD" COMPLEX_FIELD];
      ...
      ANTS_TAC THENL
       [REPEAT CONJ_TAC THENL
         [MATCH_MP_TAC CONTINUOUS_ON_COMPLEX_MUL THEN
          ...
          CONV_TAC "Multivariate/gamma.ml:COMPLEX_FIELD" COMPLEX_FIELD;
    ...
    ANTS_TAC THENL
     [CONJ_TAC THENL
       [REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN
          ...
          CONJ_TAC THENL
           [REWRITE_TAC[EVENTUALLY_WITHINREAL] THEN
            ...
            SUBGOAL_THEN `&0 = &0 rpow x`
             (fun th -> GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [th])
        ...
        REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Multivariate/gamma.ml:REAL_FIELD" REAL_FIELD];
...
let EULER_ORIGINAL_HAS_INTEGRAL_CGAMMA = prove
 (`!z. &0 < Re z
  ...
  REWRITE_TAC[EVENTUALLY_WITHIN] THEN CONJ_TAC THENL
     ...
     [REWRITE_TAC[REAL_POS; COUNTABLE_INSERT; COUNTABLE_EMPTY] THEN
      ...
      ASM_SIMP_TAC[EULER_ORIGINAL_INTEGRABLE] THEN REPEAT CONJ_TAC THENL
       [REWRITE_TAC[GSYM LIFT_NEG; DROP_NEG] THEN
        ...
        GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM REAL_EXP_0] THEN
   ...
   GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [HAS_INTEGRAL_ALT] THEN
   ...
   ANTS_TAC THENL
    [REWRITE_TAC[BALL_1; SUBSET_INTERVAL_1; DROP_ADD; DROP_SUB; DROP_VEC] THEN
     ...
     GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM REAL_INV_INV] THEN
 ...
 (`(!z n. 1 <= n /\ (Im z = &0 ==> &0 < Re z)
  ...
  let lemma1 = prove
   (`!p n z. (Im z = &0 ==> &0 < Re z)
    ...
    CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ANTS_TAC THENL
      ...
      ASM_CASES_TAC `k = 0` THEN ASM_REWRITE_TAC[] THENL
       [COMPLEX_DIFF_TAC THEN CONJ_TAC THENL
         [REWRITE_TAC[RE_ADD; IM_ADD; RE_CX; IM_CX] THEN ASM_REAL_ARITH_TAC;
          UNDISCH_TAC `~(z + Cx x = Cx(&0))` THEN CONV_TAC "Multivariate/gamma.ml:COMPLEX_FIELD" COMPLEX_FIELD];
        ALL_TAC] THEN
    ...
    CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING) in
  let lemma2 = prove
   (`!z n. 2 <= n /\ (Im z = &0 ==> &0 < Re z)
    ...
    CONJ_TAC THENL
     [REWRITE_TAC[EVENTUALLY_AT_POSINFINITY; real_ge] THEN
      ...
      REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/gamma.ml:SYM_CONV" SYM_CONV THEN
      MATCH_MP_TAC INTEGRAL_UNIQUE THEN FIRST_X_ASSUM MATCH_MP_TAC THEN
      ASM_REAL_ARITH_TAC;
      ALL_TAC] THEN
    REWRITE_TAC[LIFT_SUB; VECTOR_SUB] THEN
    GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM VECTOR_ADD_LID] THEN
  ...
  let lemma4 = prove
   (`!z p. (Im z = &0 ==> &0 < Re z) /\ 1 <= p
    ...
    DISCH_THEN(fun th -> ONCE_REWRITE_TAC[th]) THEN
    GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [COMPLEX_RING `a - b:complex = --b + a`] THEN
     ...
     [REWRITE_TAC[GSYM VSUM_NEG] THEN MATCH_MP_TAC LIM_VSUM THEN
      ...
      THENL
       [MATCH_MP_TAC(COMPLEX_FIELD
        ...
        CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN DISCH_THEN (K ALL_TAC) THEN
     ...
     [REWRITE_TAC[CLOG_1; COMPLEX_MUL_RZERO; COMPLEX_SUB_RZERO] THEN
      ...
      MATCH_MP_TAC LIM_TRANSFORM_EVENTUALLY THEN EXISTS_TAC
       `\n. z * (clog(Cx(&n)) - clog(z + Cx(&n))) +
                ...
                (Cx(&1) - z)` THEN
      CONJ_TAC THENL
       [MATCH_MP_TAC ALWAYS_EVENTUALLY THEN CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING;
        ALL_TAC] THEN
      MATCH_MP_TAC LIM_NULL_COMPLEX_ADD THEN CONJ_TAC THENL
       [MATCH_MP_TAC LIM_NULL_COMPLEX_LMUL THEN
        GEN_REWRITE_TAC "Multivariate/gamma.ml:(RATOR_CONV o LAND_CONV o BINDER_CONV o LAND_CONV o RAND_CONV)" (RATOR_CONV o LAND_CONV o BINDER_CONV o LAND_CONV o RAND_CONV) [GSYM COMPLEX_ADD_LID] THEN
  ...
  let lemma7 = prove
   (`((\y. integral {t | &0 <= drop t}
    ...
    GEN_REWRITE_TAC "Multivariate/gamma.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [GSYM REAL_INV_INV] THEN
    REWRITE_TAC[GSYM REAL_INV_MUL] THEN MATCH_MP_TAC REAL_LE_INV2 THEN
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LT_MUL THEN CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
   ...
   (`integral {t | &0 <= drop t}
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_COMPLEX_INTEGRAL THEN CONJ_TAC THENL
      ...
      GEN_REWRITE_TAC "Multivariate/gamma.ml:I" I [GSYM CX_INJ]] THEN
    ...
    GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM COMPLEX_ADD_RID] THEN
    MATCH_MP_TAC LIM_ADD THEN CONJ_TAC THENL
     [ALL_TAC;
      MATCH_MP_TAC LIM_NULL_COMPARISON_COMPLEX THEN
     ...
     [EXISTS_TAC `&1` THEN X_GEN_TAC `y:real` THEN DISCH_TAC THEN
      ...
      BINOP_TAC THENL
       [MP_TAC(SPEC `ii * Cx y` CGAMMA_LGAMMA) THEN COND_CASES_TAC THENL
        ...
        AP_THM_TAC THEN AP_TERM_TAC THEN
        AP_TERM_TAC THEN CONV_TAC "Multivariate/gamma.ml:SYM_CONV" SYM_CONV THEN
    ...
    CONJ_TAC THENL [ALL_TAC; CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING] THEN
    GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM COMPLEX_SUB_RZERO] THEN
    MATCH_MP_TAC LIM_SUB THEN REWRITE_TAC[LIM_CONST] THEN
    MP_TAC(ISPECL
     [`clog`; `at_posinfinity`;
    ...
    REWRITE_TAC[CLOG_1] THEN ANTS_TAC THENL
     [SIMP_TAC[CONTINUOUS_AT_CLOG; RE_CX; REAL_LT_01; CX_SUB] THEN
      ...
      MATCH_MP_TAC LIM_NULL_COMPLEX_POW THEN CONV_TAC "Multivariate/gamma.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
...
let LOG_GAMMA_STIRLING = prove
 (`!x p. &0 < x
         ==> log(gamma x) =
             ...
             &(2 * p + 1)`,
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/gamma.ml:I" I [GSYM REAL_EXP_INJ] THEN
  ASM_SIMP_TAC[EXP_LOG; GAMMA_POS_LT] THEN
  GEN_REWRITE_TAC "Multivariate/gamma.ml:I" I [GSYM CX_INJ] THEN REWRITE_TAC[CX_GAMMA] THEN
  ...
  GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [INTEGRABLE_COMPONENTWISE] THEN
...
let CSIN_PRODUCT = prove
 (`!z. ((\n. z * cproduct(1..n) (\m. Cx(&1) - (z / Cx(pi * &m)) pow 2))
  ...
  MP_TAC THENL
   [GEN_REWRITE_TAC "Multivariate/gamma.ml:LAND_CONV" LAND_CONV [GSYM COMPLEX_INV_1] THEN
    ONCE_REWRITE_TAC[GSYM COMPLEX_INV_DIV] THEN
    MATCH_MP_TAC LIM_COMPLEX_INV THEN
    CONJ_TAC THENL [ALL_TAC; CONV_TAC "Multivariate/gamma.ml:COMPLEX_RING" COMPLEX_RING] THEN
...
let WALLIS_ALT = prove
 (`((\n. product(1..n) (\k. (&2 * &k) / (&2 * &k - &1) *
  ...
  CONJ_TAC THENL [ALL_TAC; MP_TAC PI_POS THEN CONV_TAC "Multivariate/gamma.ml:REAL_FIELD" REAL_FIELD] THEN
...
let WALLIS = prove
 (`((\n. (&2 pow n * &(FACT n)) pow 4 / (&(FACT(2 * n)) * &(FACT(2 * n + 1))))
  ...
  REWRITE_TAC[GSYM REAL_OF_NUM_EQ] THEN CONV_TAC "Multivariate/gamma.ml:REAL_FIELD" REAL_FIELD);;
 ...
 (`((\x. exp(--(x pow 2))) has_real_integral sqrt pi) (:real)`,
  ...
  REWRITE_TAC[REAL_ARITH
   `max (&0) a = &0 /\ &0 < B /\ a <= --B /\ B <= b <=>
    a <= --B /\ &0 < B /\ B <= b`] THEN
  GEN_REWRITE_TAC "Multivariate/gamma.ml:(BINOP_CONV o ONCE_DEPTH_CONV)" (BINOP_CONV o ONCE_DEPTH_CONV) [IMP_CONJ] THEN
  ...
  MP_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    REWRITE_TAC[] THEN CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    ...
    ANTS_TAC THENL
     [REPEAT CONJ_TAC THENL
      ...
      ASM_SIMP_TAC[RPOW_RPOW] THEN CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
      REWRITE_TAC[RPOW_NEG; RPOW_POW; REAL_POW_1] THEN
      MP_TAC(SPEC `(x:real) pow 2` REAL_EXP_NZ) THEN
      UNDISCH_TAC `~(x = &0)` THEN CONV_TAC "Multivariate/gamma.ml:REAL_FIELD" REAL_FIELD];
  ...
  SIMP_TAC[] THEN DISCH_THEN(K ALL_TAC) THEN CONV_TAC "Multivariate/gamma.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `e:real`) THEN ASM_REWRITE_TAC[] THEN
  DISCH_THEN(X_CHOOSE_THEN `B:real` STRIP_ASSUME_TAC) THEN
