...
let PYTHAGORAS = prove
 (`!A B C:real^N.
  ...
  CONV_TAC "Multivariate/geom.ml:REAL_RING" REAL_RING);;
...
let VECTOR_ANGLE_LNEG = prove
 (`!x y. vector_angle (--x) y = pi - vector_angle x y`,
  ...
  CONV_TAC "Multivariate/geom.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let VECTOR_ANGLE_EQ_0 = prove
 (`!x y:real^N. vector_angle x y = &0 <=>
  ...
  GEN_REWRITE_TAC "Multivariate/geom.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_MUL_LID] THEN
   ...
   [MATCH_MP_TAC CONTINUOUS_WITHIN_COMPOSE THEN CONJ_TAC THENL
    ...
    MP_TAC THENL
     [REWRITE_TAC[EVENTUALLY_AND] THEN CONJ_TAC THENL
      ...
      REWRITE_TAC[] THEN CONV_TAC "Multivariate/geom.ml:NORM_ARITH" NORM_ARITH;
      MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ] EVENTUALLY_MONO) THEN
...
let SIN_VECTOR_ANGLE = prove
 (`!x y:real^N.
  ...
  CONV_TAC "Multivariate/geom.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[SQRT_1]);;
...
let ARG_EQ_VECTOR_ANGLE_1 = prove
 (`!z. ~(z = Cx(&0)) /\ &0 <= Im z ==> Arg z = vector_angle z (Cx(&1))`,
  ...
  GEN_REWRITE_TAC "Multivariate/geom.ml:(RAND_CONV o RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV o LAND_CONV o RAND_CONV) [ARG] THEN
  ...
  ASM_SIMP_TAC[COMPLEX_NORM_ZERO; REAL_FIELD
   `~(z = &0) ==> (z * x) / z = x`] THEN
  CONV_TAC "Multivariate/geom.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC ACS_COS THEN
 ...
 (`!w z. ~(w = Cx(&0)) /\ ~(z = Cx(&0))
  ...
  REPEAT STRIP_TAC THEN COND_CASES_TAC THENL
   [SUBGOAL_THEN `z = w * (z / w) /\ w = w * Cx(&1)` MP_TAC THENL
     [REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Multivariate/geom.ml:COMPLEX_FIELD" COMPLEX_FIELD; ALL_TAC];
    SUBGOAL_THEN `w = z * (w / z) /\ z = z * Cx(&1)` MP_TAC THENL
     [REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Multivariate/geom.ml:COMPLEX_FIELD" COMPLEX_FIELD; ALL_TAC]] THEN
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Multivariate/geom.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [th]) THEN
  ASM_SIMP_TAC[VECTOR_ANGLE_COMPLEX_LMUL] THENL
   [ONCE_REWRITE_TAC[VECTOR_ANGLE_SYM] THEN CONV_TAC "Multivariate/geom.ml:SYM_CONV" SYM_CONV THEN
    MATCH_MP_TAC ARG_EQ_VECTOR_ANGLE_1 THEN ASM_REWRITE_TAC[] THEN
    REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Multivariate/geom.ml:COMPLEX_FIELD" COMPLEX_FIELD;
    MP_TAC(ISPEC `z / w:complex` ARG_INV) THEN ANTS_TAC THENL
     [ASM_MESON_TAC[real; REAL_LE_REFL]; DISCH_THEN(SUBST1_TAC o SYM)] THEN
    REWRITE_TAC[COMPLEX_INV_DIV] THEN CONV_TAC "Multivariate/geom.ml:SYM_CONV" SYM_CONV THEN
...
let SIN_OPPOSITE_HYPOTENUSE = prove
 (`!A B C:real^N.
  ...
  DISCH_TAC THEN CONV_TAC "Multivariate/geom.ml:SYM_CONV" SYM_CONV THEN
...
let TAN_OPPOSITE_ADJACENT = prove
 (`!A B C:real^N.
  ...
  ASM_CASES_TAC `cos (angle (B:real^N,A,C)) = &0` THENL
   [ALL_TAC; POP_ASSUM MP_TAC THEN CONV_TAC "Multivariate/geom.ml:REAL_FIELD" REAL_FIELD] THEN
...
let LAW_OF_SINES = prove
 (`!A B C:real^N.
  ...
  GEN_REWRITE_TAC "Multivariate/geom.ml:(RAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o LAND_CONV o ONCE_DEPTH_CONV) [DIST_SYM] THEN
  ...
  REWRITE_TAC[DOT_LSUB; DOT_RSUB; DOT_SYM] THEN CONV_TAC "Multivariate/geom.ml:REAL_RING" REAL_RING);;
...
let TRIANGLE_ANGLE_SUM_LEMMA = prove
 (`!A B C:real^N. ~(A = B) /\ ~(A = C) /\ ~(B = C)
  ...
  REWRITE_TAC[COS_ADD; SIN_ADD; ANGLE_SYM] THEN CONV_TAC "Multivariate/geom.ml:REAL_RING" REAL_RING);;
...
let CONGRUENT_TRIANGLES_SSS = prove
 (`!A B C:real^M A' B' C':real^N.
  ...
  CONV_TAC "Multivariate/geom.ml:REAL_FIELD" REAL_FIELD);;
...
let SETDIST_POINT_LINE = prove
 (`!x y z:real^N.
  ...
  GEN_REWRITE_TAC "Multivariate/geom.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [ANGLE_SYM] THEN
...
let AREA_TRIANGLE_SIN = prove
 (`!a b c:real^2.
  ...
  REWRITE_TAC[NORM_POW_2; DOT_2] THEN CONV_TAC "Multivariate/geom.ml:REAL_RING" REAL_RING);;
