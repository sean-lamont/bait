...
let CARD_DIFF_INTER = prove
 (`!s t. FINITE s ==> CARD s = CARD(s DIFF t) + CARD(s INTER t)`,
  REPEAT STRIP_TAC THEN CONV_TAC "Multivariate/clifford.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_UNION_EQ THEN
...
let DEST_MK_MULTIVECTOR = prove
 (`!s. s SUBSET 1..dimindex(:N)
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/clifford.ml:I" I [GSYM multivector_tybij] THEN
  ASM_REWRITE_TAC[]);;
...
let MBASIS_EXPANSION = prove
 (`!x:real^(N)multivector.
  ...
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Multivariate/clifford.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [EQ_SYM_EQ] THEN
...
let SPAN_MBASIS = prove
 (`span {mbasis s :real^(N)multivector | s SUBSET 1..dimindex(:N)} = UNIV`,
  ...
  GEN_REWRITE_TAC "Multivariate/clifford.ml:LAND_CONV" LAND_CONV [GSYM MBASIS_EXPANSION] THEN
...
let MBASIS_EXTENSION = prove
 (`!P. (!s. s SUBSET 1..dimindex(:N) ==> P(mbasis s)) /\
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/clifford.ml:RAND_CONV" RAND_CONV [GSYM MBASIS_EXPANSION] THEN
...
let PRODUCT_ASSOCIATIVE = prove
 (`!op mult. (!s t. s SUBSET 1..dimindex(:N) /\ t SUBSET 1..dimindex(:N)
  ...
  let SWAP_TAC cnv n =
    GEN_REWRITE_TAC "Multivariate/clifford.ml:(cnv o funpow n BINDER_CONV)" (cnv o funpow n BINDER_CONV) [SUM_SWAP_POWERSET] THEN
    REWRITE_TAC[] in
 ...
 (`!x y z:real^(N)multivector. x * (y * z) = (x * y) * z`,
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC(TAUT `(x <=> y <=> z) ==> ((a <=> x) <=> (y <=> z <=> a))`);
    AP_TERM_TAC THEN CONV_TAC "Multivariate/clifford.ml:SYM_CONV" SYM_CONV] THEN
  ...
  CONV_TAC "Multivariate/clifford.ml:TAUT" TAUT);;
...
let PRODUCT_MBASIS = prove
 (`!s t. Product mult op (mbasis s) (mbasis t) :real^(N)multivector =
  ...
  REPEAT
   (GEN_REWRITE_TAC "Multivariate/clifford.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [COND_RAND] THEN
    GEN_REWRITE_TAC "Multivariate/clifford.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [COND_RATOR] THEN
...
let seqiterate_EXISTS = prove
 (`!op f. ?h.
  ...
  LET_TAC THEN CONV_TAC "Multivariate/clifford.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN
  ...
  GEN_REWRITE_TAC "Multivariate/clifford.ml:LAND_CONV" LAND_CONV [MINIMAL] THEN
...
let SEQITERATE_CLAUSES = prove
 (`(!op f. seqiterate op {} f = neutral op) /\
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Multivariate/clifford.ml:LAND_CONV" LAND_CONV [seqiterate] THEN
...
let SEQITERATE_ITERATE = prove
 (`!op f s. monoidal op /\ FINITE s ==> seqiterate op s f = iterate op s f`,
  ...
  GEN_REWRITE_TAC "Multivariate/clifford.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
