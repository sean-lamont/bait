...
let REAL_SUB_SUM0 = prove
 (`!x y m. sum(0,m) x - sum(0,m) y = sum(0,m) (\i. x i - y i)`,
  CONV_TAC "Library/calc_real.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
...
let REAL_NDIV_LEMMA1a = prove
 (`!a m n. &2 * abs(&2 pow m * &a - &2 pow (m + n)) <= &2 pow m
  ...
  CONV_TAC "Library/calc_real.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THEN
   ...
   [DISCH_TAC THEN MATCH_MP_TAC REAL_LT_LCANCEL_IMP THEN
    EXISTS_TAC `&2 * abs(a)` THEN CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LT_MUL THEN
      ...
      ASM_REWRITE_TAC[]] THEN
    GEN_REWRITE_TAC "Library/calc_real.ml:(LAND_CONV o funpow 3 RAND_CONV)" (LAND_CONV o funpow 3 RAND_CONV) [REAL_MUL_SYM] THEN
     ...
     [MATCH_MP_TAC REAL_LT_LMUL THEN
      ...
      MATCH_MP_TAC REAL_LT_LMUL THEN CONJ_TAC THENL
       [REWRITE_TAC[GSYM REAL_ABS_NZ; REAL_POW_EQ_0] THEN
        CONV_TAC "Library/calc_real.ml:(RAND_CONV(LAND_CONV REAL_INT_EQ_CONV))" (RAND_CONV(LAND_CONV REAL_INT_EQ_CONV)) THEN REWRITE_TAC[];
        REWRITE_TAC[REAL_SUB_RDISTRIB] THEN
        ONCE_REWRITE_TAC[REAL_ABS_SUB] THEN
        GEN_REWRITE_TAC "Library/calc_real.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
    ...
    DISJ_CASES_TAC (SPECL [`e:num`; `n + k:num`] LET_CASES) THENL
     [SUBGOAL_THEN `a * b = &2 pow (n + k)` ASSUME_TAC THENL
      ...
      GEN_REWRITE_TAC "Library/calc_real.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM REAL_ABS_MUL] THEN
      ...
      SUBGOAL_THEN `abs(b) <= &1 /\ &0 <= a * b` STRIP_ASSUME_TAC THENL
       [ASM_CASES_TAC `b = &0` THEN
        ...
        SUBGOAL_THEN `abs(a) <= abs(a * b)` ASSUME_TAC THENL
         [GEN_REWRITE_TAC "Library/calc_real.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM REAL_MUL_RID] THEN
         ...
         [MP_TAC(SPEC `(n:num) + k` REAL_LT_POW2) THEN
          ...
          SUBGOAL_THEN `&2 * &2 pow (n + k) <= abs(a)` MP_TAC THENL
           [REWRITE_TAC[GSYM(CONJUNCT2 real_pow)] THEN
            FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [SYM th]) THEN
       ...
       [ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
        ...
        SUBGOAL_THEN `&0 <= b * (&2 pow n * inv x)` MP_TAC THENL
         [MATCH_MP_TAC REAL_LE_LCANCEL_IMP THEN
          ...
          REWRITE_TAC[MATCH_MP REAL_LT_IMP_LE (SPEC_ALL REAL_LT_POW2)] THEN
          FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/calc_real.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV)
          ...
          EXISTS_TAC `abs(a)` THEN CONJ_TAC THENL
           [UNDISCH_TAC `?m. (a = &m) \/ (a = -- (&m))` THEN
            ...
            CONV_TAC "Library/calc_real.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_LT; LE];
...
let REALCALC_ADD = prove
 (`(n + 2 = n') ==>
  ...
  CONV_TAC "Library/calc_real.ml:(LAND_CONV REAL_INT_REDUCE_CONV)" (LAND_CONV REAL_INT_REDUCE_CONV) THEN
  ...
  EXISTS_TAC `a + b` THEN
  GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [SYM(REAL_INT_REDUCE_CONV `&2 + &2`)] THEN
  MATCH_MP_TAC REAL_LTE_ADD2 THEN ASM_REWRITE_TAC[] THEN ONCE_REWRITE_TAC
   [REAL_ARITH `(x + y) - a * (u + v) = (x - a * u) + (y - a * v)`] THEN
  GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [SYM(REAL_INT_REDUCE_CONV `&1 + &1`)] THEN
...
let REALCALC_MUL = prove
 (`abs(a - &2 pow k * x) < &1 ==>
  ...
  CONV_TAC "Library/calc_real.ml:(LAND_CONV (EQT_INTRO o REAL_ARITH))" (LAND_CONV (EQT_INTRO o REAL_ARITH)) THEN
  REWRITE_TAC[REAL_MUL_RID] THEN
  GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [REAL_MUL_2] THEN
  MATCH_MP_TAC REAL_LET_TRANS THEN
  EXISTS_TAC `&2 * abs(&2 pow m * c - a * b) +
              &2 * abs(a * b - &2 pow m * &2 pow n * x * y)` THEN
  CONV_TAC "Library/calc_real.ml:(LAND_CONV (EQT_INTRO o REAL_ARITH))" (LAND_CONV (EQT_INTRO o REAL_ARITH)) THEN REWRITE_TAC[] THEN
...
let REALCALC_SQRT = prove
 (`abs(a - &2 pow n * x) < &1
  ...
  UNDISCH_TAC `&1 <= x` THEN CONV_TAC "Library/calc_real.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
  REWRITE_TAC[REAL_NOT_LE] THEN DISCH_TAC THEN
  SUBGOAL_THEN `x = sqrt(x) pow 2` SUBST1_TAC THENL
   [CONV_TAC "Library/calc_real.ml:SYM_CONV" SYM_CONV THEN ASM_REWRITE_TAC[SQRT_POW2];
    GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
...
let STEP_LEMMA2 = prove
 (`!n s t u x y k l a d.
   ...
   GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [REAL_ADD_SYM] THEN
   ...
   MATCH_MP_TAC REAL_LE_ADD2 THEN CONJ_TAC THENL
    [GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
     ASM_REWRITE_TAC[GSYM REAL_MUL_ASSOC; REAL_MUL_RID] THEN
     REWRITE_TAC[GSYM REAL_SUB_LDISTRIB; GSYM REAL_ABS_LEMMA1] THEN
     REWRITE_TAC[REAL_MUL_ASSOC] THEN
     GEN_REWRITE_TAC "Library/calc_real.ml:(LAND_CONV o funpow 2 RAND_CONV o LAND_CONV)" (LAND_CONV o funpow 2 RAND_CONV o LAND_CONV)
     ...
     CONV_TAC "Library/calc_real.ml:(LAND_CONV (EQT_INTRO o REAL_ARITH))" (LAND_CONV (EQT_INTRO o REAL_ARITH)) THEN REWRITE_TAC[] THEN
     GEN_REWRITE_TAC "Library/calc_real.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
      [AC REAL_MUL_AC
        `a * b * c * d * e * f * g = d * (a * f) * (c * g) * (e * b)`] THEN
     GEN_REWRITE_TAC "Library/calc_real.ml:(RAND_CONV o LAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV o LAND_CONV)
      [AC REAL_MUL_AC
        `a * b * c * d * e * f = c * (a * e) * f * (d * b)`] THEN
     GEN_REWRITE_TAC "Library/calc_real.ml:(RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV)
      [AC REAL_MUL_AC
        `a * b * c * d * e * f * g = c * (e * g) * (f * a) * (d * b)`] THEN
     GEN_REWRITE_TAC "Library/calc_real.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
      [AC REAL_MUL_AC
        `a * b * c * d * e * f = c * (a * f) * e * (d * b)`] THEN
     GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV
 ...
 (`abs(x) <= &1 /\
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[REAL_MUL_AC];
    ...
    GEN_REWRITE_TAC "Library/calc_real.ml:(RAND_CONV o LAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
     ...
     [MATCH_MP_TAC REAL_LE_INV THEN REWRITE_TAC[REAL_OF_NUM_LE; LE_0];
      MATCH_MP_TAC REAL_LE_ADD2 THEN CONJ_TAC THENL
       [MATCH_MP_TAC REAL_LE_RMUL THEN CONJ_TAC THENL
         [GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [REAL_ARITH `&2 = &1 + &1`] THEN
 ...
 (`abs(x) <= &1 /\
  ...
  CONJ_TAC THENL
   [AP_TERM_TAC THEN AP_TERM_TAC THEN AP_TERM_TAC THEN SUBGOAL_THEN
    ...
    SUBST1_TAC THENL
     [GEN_REWRITE_TAC "Library/calc_real.ml:I" I [GSYM REAL_OF_NUM_EQ] THEN
    ...
    GEN_REWRITE_TAC "Library/calc_real.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_MUL_RID] THEN
    ...
    MATCH_MP_TAC REAL_LE_ADD2 THEN CONJ_TAC THENL
     [GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [REAL_ARITH `&2 = &1 + &1`] THEN
      ...
      GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
      MATCH_MP_TAC REAL_LE_MUL2 THEN REWRITE_TAC[REAL_ABS_POS] THEN
      CONJ_TAC THENL
       [REWRITE_TAC[REAL_ABS_POW] THEN MATCH_MP_TAC REAL_POW_1_LE THEN
        ASM_REWRITE_TAC[REAL_ABS_POS];
   ...
   [AP_TERM_TAC THEN AP_TERM_TAC THEN AP_TERM_TAC THEN SUBGOAL_THEN
    ...
    MATCH_MP_TAC REAL_LE_ADD2 THEN CONJ_TAC THENL
     [REWRITE_TAC[REAL_MUL_ASSOC] THEN
      MATCH_MP_TAC REAL_LE_RMUL THEN CONJ_TAC THENL
       [REWRITE_TAC[real_div; REAL_MUL_LID] THEN
        GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_SYM] THEN
        MATCH_MP_TAC REAL_LE_LMUL THEN CONJ_TAC THENL
         [MATCH_MP_TAC REAL_LE_INV THEN
          REWRITE_TAC[REAL_OF_NUM_MUL; REAL_POS];
          GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [REAL_ARITH `&2 = &1 + &1`] THEN
 ...
 (`2 <= n /\
  ...
  CONJ_TAC THENL
   [AP_TERM_TAC THEN AP_TERM_TAC THEN AP_TERM_TAC THEN
    ...
    GEN_REWRITE_TAC "Library/calc_real.ml:LAND_CONV" LAND_CONV [REAL_MUL_SYM] THEN
    SUBGOAL_THEN `inv(&(SUC i)) * &(SUC i) = &1` ASSUME_TAC THENL
     [MATCH_MP_TAC REAL_MUL_LINV THEN
      ...
      REWRITE_TAC[REAL_MUL_AC]];
    GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [SYM(REAL_RAT_REDUCE_CONV
      `(&1 / &2 + &1 / &4) * &3 + &1 / &4 + &1 / &2`)] THEN
    REWRITE_TAC[REAL_ADD_ASSOC; REAL_LE_RADD] THEN
    MATCH_MP_TAC REAL_LE_ADD2 THEN CONJ_TAC THENL
     [REWRITE_TAC[REAL_MUL_ASSOC] THEN MATCH_MP_TAC REAL_LE_RMUL THEN
      CONV_TAC "Library/calc_real.ml:(RAND_CONV (EQT_INTRO o REAL_ARITH))" (RAND_CONV (EQT_INTRO o REAL_ARITH)) THEN REWRITE_TAC[] THEN
      GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
      MATCH_MP_TAC REAL_LE_MUL2 THEN REPEAT CONJ_TAC THENL
       ...
       [MATCH_MP_TAC REAL_MUL_LINV THEN REWRITE_TAC[REAL_OF_NUM_EQ; NOT_SUC];
        ...
        CONJ_TAC THENL
         [MATCH_MP_TAC REAL_LE_INV2 THEN REWRITE_TAC[REAL_POS] THEN
          ...
          REPEAT CONJ_TAC THENL
           [CONV_TAC "Library/calc_real.ml:(RAND_CONV REAL_RAT_REDUCE_CONV)" (RAND_CONV REAL_RAT_REDUCE_CONV) THEN ASM_REWRITE_TAC[];
            ...
            ASM_REWRITE_TAC[] THEN CONV_TAC "Library/calc_real.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV]]]]]);;
 ...
 (`4 <= m /\
  ...
  INDUCT_TAC THENL
   [REWRITE_TAC[ADD_CLAUSES] THEN
    ...
    CONV_TAC "Library/calc_real.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC(GEN_ALL STEP_EXP) THEN ASM_REWRITE_TAC[] THEN
      ...
      CONV_TAC "Library/calc_real.ml:(RAND_CONV REAL_RAT_REDUCE_CONV)" (RAND_CONV REAL_RAT_REDUCE_CONV) THEN REWRITE_TAC[] THEN
      MATCH_MP_TAC REAL_LE_ADD2 THEN CONJ_TAC THENL
       [ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
...
let STEPS_EXP_1 = prove
 (`abs(x) <= &1 /\
   ...
   ==> abs(sum(0,1) t - &2 pow n * sum(0,1)(\i. x pow i / &(FACT i))) 
         <= &2 * &1`,
  CONV_TAC "Library/calc_real.ml:(ONCE_DEPTH_CONV EXPAND_RANGE_CONV)" (ONCE_DEPTH_CONV EXPAND_RANGE_CONV) THEN REWRITE_TAC[] THEN
  STRIP_TAC THEN CONV_TAC "Library/calc_real.ml:(ONCE_DEPTH_CONV REAL_SUM_CONV)" (ONCE_DEPTH_CONV REAL_SUM_CONV) THEN
...
let STEPS_LN = prove
 (`2 <= n /\
  ...
  REWRITE_TAC[REAL_MUL_RSUM0; REAL_SUB_SUM0] THEN
  STRIP_TAC THEN GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
  ...
  MATCH_MP_TAC REAL_LE_ADD2 THEN CONJ_TAC THENL
   [MATCH_MP_TAC REAL_LE_RCANCEL_IMP THEN
    ...
    REWRITE_TAC[REAL_MUL_ASSOC] THEN CONV_TAC "Library/calc_real.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  MATCH_MP_TAC REAL_LE_ADD2 THEN CONJ_TAC THENL
   [SUBGOAL_THEN `--(&1) pow p * s * x pow (SUC p) / &(SUC(SUC p)) =
    ...
    GEN_REWRITE_TAC "Library/calc_real.ml:(LAND_CONV o LAND_CONV o REDEPTH_CONV)" (LAND_CONV o LAND_CONV o REDEPTH_CONV)
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LE_LCANCEL_IMP THEN
      ...
      CONV_TAC "Library/calc_real.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
      REWRITE_TAC[real_div; REAL_MUL_LID] THEN
      MATCH_MP_TAC REAL_LE_INV2 THEN
  ...
  CONJ_TAC THENL [ALL_TAC; CONV_TAC "Library/calc_real.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV] THEN
  MATCH_MP_TAC REAL_LE_MUL2 THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC REAL_LE_TRANS THEN EXISTS_TAC `inv(&2) pow (SUC p)` THEN
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_POW_LE2 THEN CONV_TAC "Library/calc_real.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
      ASM_REWRITE_TAC[REAL_ABS_POS];
      REWRITE_TAC[REAL_POW_INV] THEN MATCH_MP_TAC REAL_LE_INV2 THEN
      CONJ_TAC THENL [CONV_TAC "Library/calc_real.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV; ALL_TAC] THEN
      MATCH_MP_TAC REAL_POW_MONO THEN
...
let REAL_EXP_15 = prove
 (`exp(&1) < &5`,
  ...
  CONV_TAC "Library/calc_real.ml:(RAND_CONV REAL_RAT_REDUCE_CONV)" (RAND_CONV REAL_RAT_REDUCE_CONV) THEN
  REWRITE_TAC[] THEN
  MATCH_MP_TAC REAL_LE_MUL2 THEN
...
let REAL_EXP_13 = prove
 (`exp(&1) < &3`,
  ...
  CONV_TAC "Library/calc_real.ml:(ONCE_DEPTH_CONV REAL_SUM_CONV)" (ONCE_DEPTH_CONV REAL_SUM_CONV) THEN
  REWRITE_TAC[ADD_CLAUSES] THEN
  CONV_TAC "Library/calc_real.ml:(ONCE_DEPTH_CONV NUM_FACT_CONV)" (ONCE_DEPTH_CONV NUM_FACT_CONV) THEN
  CONV_TAC "Library/calc_real.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  MATCH_MP_TAC(REAL_ARITH
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN
    ...
    SUBGOAL_THEN `abs(inv(&1 + t)) < &1` ASSUME_TAC THENL
     [REWRITE_TAC[REAL_ABS_INV] THEN
      GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [GSYM REAL_INV_1] THEN
...
let APPROX_LEMMA1 = prove
 (`abs(f(x:real) - sum(0,m) (\i. P i x)) < inv(&2 pow (n + 2)) /\
  ...
  CONV_TAC "Library/calc_real.ml:(funpow 4 RAND_CONV num_CONV)" (funpow 4 RAND_CONV num_CONV) THEN
  ...
  MATCH_MP_TAC REAL_LET_ADD2 THEN CONJ_TAC THENL
   [ONCE_REWRITE_TAC[REAL_POW_ADD] THEN
    ...
    MATCH_MP_TAC REAL_LT_IMP_LE THEN REWRITE_TAC[REAL_LT_POW2];
    REWRITE_TAC[GSYM REAL_MUL_ASSOC; GSYM REAL_SUB_LDISTRIB] THEN
    GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [REAL_POW_ADD] THEN
    REWRITE_TAC[GSYM REAL_ABS_LEMMA] THEN
   ...
   [MATCH_MP_TAC REAL_LTE_TRANS THEN
    ...
    SUBGOAL_THEN `2 <= (n + e + 2)` MP_TAC THENL
     [REWRITE_TAC[ADD_ASSOC] THEN ONCE_REWRITE_TAC[ADD_SYM] THEN
      ...
      REWRITE_TAC[real_div; REAL_MUL_LID] THEN
      GEN_REWRITE_TAC "Library/calc_real.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_POW_1] THEN
...
let REALCALC_CONV,thm_eval,raw_eval,thm_wrap =
  ...
  let REALCALC_SQRT_CONV =
    ...
    let PROVE_1_LE_SQRT =
      let pth = prove
       (`&1 <= x ==> &1 <= sqrt(x)`,
        DISCH_THEN(fun th ->
          ...
          MP_TAC th) THEN
        CONV_TAC "Library/calc_real.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[REAL_NOT_LE] THEN
        DISCH_TAC THEN
        SUBGOAL_THEN `x = sqrt(x) pow 2` SUBST1_TAC THENL
         [CONV_TAC "Library/calc_real.ml:SYM_CONV" SYM_CONV THEN ASM_REWRITE_TAC[SQRT_POW2];
          GEN_REWRITE_TAC "Library/calc_real.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
      ...
      let tac = REPEAT(MATCH_MP_TAC pth) THEN CONV_TAC "Library/calc_real.ml:REAL_RAT_LE_CONV" REAL_RAT_LE_CONV in
