...
let ORDINAL_CHAINED = prove
 (`!(l:A#A->bool) m. ordinal(l) /\ ordinal(m) ==> m inseg l \/ l inseg m`,
  ...
  CONV_TAC "Library/wo.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THEN
...
let ORDINAL_SUC = prove
 (`!l:A#A->bool. ordinal(l) /\ (?x. ~(fl(l) x)) ==>
  ...
  SUBGOAL_THEN `~fl(l:A#A->bool) a` ASSUME_TAC THENL
   [EXPAND_TAC "a" THEN CONV_TAC "Library/wo.ml:SELECT_CONV" SELECT_CONV THEN
  ...
  PBETA_TAC THEN CONJ_TAC THENL
   [REWRITE_TAC[WOSET] THEN PBETA_TAC THEN CONJ_TAC THENL
     [REPEAT GEN_TAC THEN STRIP_TAC THEN ASM_REWRITE_TAC[] THENL
      ...
      UNDISCH_TAC `~fl(l:A#A->bool) a` THEN CONV_TAC "Library/wo.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
    ...
    PBETA_TAC THEN DISCH_THEN DISJ_CASES_TAC THENL
     [UNDISCH_TAC `!x:A. fl l x ==> (x = (@y. ~less l (y,x)))` THEN
      ...
      DISCH_THEN(fun th -> GEN_REWRITE_TAC "Library/wo.ml:LAND_CONV" LAND_CONV [th]) THEN
      AP_TERM_TAC THEN GEN_REWRITE_TAC "Library/wo.ml:I" I [FUN_EQ_THM] THEN
      X_GEN_TAC `y:A` THEN
      ...
      GEN_REWRITE_TAC "Library/wo.ml:LAND_CONV" LAND_CONV [SYM(ASSUME `(@y:A. ~fl(l) y) = a`)] THEN
      AP_TERM_TAC THEN GEN_REWRITE_TAC "Library/wo.ml:I" I [FUN_EQ_THM] THEN
      X_GEN_TAC `y:A` THEN
 ...
 (`!P. (!l:A#A->bool. P l ==> ordinal(l)) ==> ordinal(UNIONS P)`,
  ...
  REPEAT CONJ_TAC THENL
   [MAP_EVERY X_GEN_TAC [`x:A`; `y:A`] THEN REWRITE_TAC[UNIONS_PRED] THEN
    ...
    REPEAT AP_TERM_TAC THEN GEN_REWRITE_TAC "Library/wo.ml:I" I [FUN_EQ_THM] THEN
...
let ORDINAL_UP = prove
 (`!l:A#A->bool. ordinal(l) ==> (!x. fl(l) x) \/
  ...
  GEN_REWRITE_TAC "Library/wo.ml:LAND_CONV" LAND_CONV [NOT_FORALL_THM] THEN DISCH_TAC THEN
  ...
  CONV_TAC "Library/wo.ml:SELECT_CONV" SELECT_CONV THEN ASM_REWRITE_TAC[]);;
...
let WO = prove
 (`!P. ?l:A#A->bool. woset l /\ (fl(l) = P)`,
  ...
  PBETA_TAC THEN
  GEN_REWRITE_TAC "Library/wo.ml:RAND_CONV" RAND_CONV [FUN_EQ_THM] THEN
...
let WF_INSEG_WOSET = prove
 (`WF(\(x:A#A->bool) y. woset x /\ woset y /\ x inseg y /\ ~(x = y))`,
  ...
  GEN_REWRITE_TAC "Library/wo.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [NOT_FORALL_THM] THEN
  ...
  FIRST_X_ASSUM(fun th ->
    GEN_REWRITE_TAC "Library/wo.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [SYM th]) THEN
...
let SUBWOSET_ISO_INSEG = prove
 (`!l s. woset l /\ fl l = (:A)
  ...
  GEN_REWRITE_TAC "Library/wo.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [RIGHT_IMP_EXISTS_THM] THEN
...
let HP = prove
 (`!l:A#A->bool. poset l ==>
  ...
  DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
  GEN_REWRITE_TAC "Library/wo.ml:LAND_CONV" LAND_CONV [FUN_EQ_THM] THEN BETA_TAC THEN
  ...
  ASM_CASES_TAC `?x:A. fl(l) x` THEN ASM_REWRITE_TAC[] THENL
   [DISCH_THEN(X_CHOOSE_THEN `b:A` STRIP_ASSUME_TAC);
    ...
    GEN_TAC THEN GEN_REWRITE_TAC "Library/wo.ml:RAND_CONV" RAND_CONV [FUN_EQ_THM] THEN
    CONV_TAC "Library/wo.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[] THEN
  ...
  SUBGOAL_THEN `!x:A. (l:A#A->bool)(b,f x) \/ l(f x,b)` ASSUME_TAC THENL
   [GEN_TAC THEN MP_TAC(SPEC `x:A` (ASSUME `!x:A. (w:A#A->bool)(b,f x)`)) THEN
    ...
    SUBGOAL_THEN `(less w)(b:A,x)` MP_TAC THENL
     [ASM_REWRITE_TAC[less] THEN CONV_TAC "Library/wo.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
  ...
  GEN_REWRITE_TAC "Library/wo.ml:I" I [FUN_EQ_THM] THEN X_GEN_TAC `z:A` THEN EQ_TAC THENL
...
let ZL = prove
 (`!l:A#A->bool. poset l /\
  ...
  DISCH_TAC THEN GEN_REWRITE_TAC "Library/wo.ml:I" I [TAUT `a <=> ~ ~a`] THEN DISCH_TAC THEN
  ...
  GEN_REWRITE_TAC "Library/wo.ml:I" I [TAUT `(a ==> b ==> c) <=> (b /\ a ==> c)`] THEN
  DISCH_THEN(fun th -> FIRST_ASSUM(MP_TAC o C MATCH_MP th)) THEN
  REWRITE_TAC[] THEN GEN_REWRITE_TAC "Library/wo.ml:RAND_CONV" RAND_CONV [FUN_EQ_THM] THEN
...
let KL = prove
 (`!l:A#A->bool. poset l ==>
  ...
  funpow 2 (fst o dest_imp) o snd) THENL
   [X_GEN_TAC `P:(A->bool)->bool` THEN GEN_REWRITE_TAC "Library/wo.ml:LAND_CONV" LAND_CONV [chain] THEN
    ...
    REPEAT STRIP_TAC THEN CONV_TAC "Library/wo.ml:SYM_CONV" SYM_CONV THEN
...
let POSET_RESTRICTED_SUBSET = prove
 (`!P. poset(\(x,y). P(x) /\ P(y) /\ x SUBSET y)`,
  ...
  CONV_TAC "Library/wo.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let FL_RESTRICTED_SUBSET = prove
 (`!P. fl(\(x,y). P(x) /\ P(y) /\ x SUBSET y) = P`,
  ...
  CONV_TAC "Library/wo.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN MESON_TAC[SUBSET_REFL]);;
...
let TUKEY = prove
 (`!s:(A->bool)->bool.
  ...
  DISCH_THEN(REPEAT_TCL CONJUNCTS_THEN ASSUME_TAC) THEN
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/wo.ml:I" I [GSYM th]) THEN
  SUBGOAL_THEN
 ...
 (`!l. toset l
  ...
  ASSUME_TAC THENL
   [GEN_TAC THEN ONCE_ASM_REWRITE_TAC[] THEN
    ...
    CONV_TAC "Library/wo.ml:SELECT_CONV" SELECT_CONV THEN ASM_REWRITE_TAC[MEMBER_NOT_EMPTY];
    ALL_TAC] THEN
  SUBGOAL_THEN
   ...
   [FIRST_ASSUM(MATCH_MP_TAC o last o CONJUNCTS o REWRITE_RULE[woset] o
    ...
    SUBGOAL_THEN `!v w:A->bool. f v:A = f w ==> v = w` MP_TAC THENL
     [FIRST_ASSUM(MP_TAC o el 3 o CONJUNCTS o REWRITE_RULE[woset] o
      ...
      REPEAT GEN_TAC THEN DISCH_TAC THEN
      GEN_REWRITE_TAC "Library/wo.ml:I" I [GSYM CONTRAPOS_THM] THEN DISCH_TAC THEN
      FIRST_X_ASSUM(fun t -> GEN_REWRITE_TAC "Library/wo.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [t]) THEN
    ...
    CONJ_TAC THENL
     [RULE_ASSUM_TAC(REWRITE_RULE[toset]) THEN ASM SET_TAC[];
      DISCH_THEN(ASSUME_TAC o MATCH_MP (MESON[]
       `~(f x = f y) ==> ~(x = y)`))] THEN
    FIRST_X_ASSUM(fun th ->
     GEN_REWRITE_TAC "Library/wo.ml:(RAND_CONV o RAND_CONV o LAND_CONV)" (RAND_CONV o RAND_CONV o LAND_CONV) [th]) THEN
  ...
  SUBGOAL_THEN
   `fl(\(x,y). x IN IMAGE (f:(A->bool)->A) {v | r(v,w) /\ ~(v = w)} /\
    ...
    IMAGE f {v | r(v,w) /\ ~(v = w)}`
  ASSUME_TAC THENL
   [GEN_REWRITE_TAC "Library/wo.ml:I" I [EXTENSION] THEN
