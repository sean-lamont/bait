...
let INT_COPRIME_RPOW = prove
 (`!m n k. coprime(m,n pow k) <=> coprime(m,n) \/ k = 0`,
  ...
  CONV_TAC "Library/integer.ml:TAUT" TAUT);;
...
let INT_PRIME_2 = prove
 (`int_prime(&2)`,
  REWRITE_TAC[int_prime] THEN CONV_TAC "Library/integer.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN
  X_GEN_TAC `x:int` THEN
  ASM_CASES_TAC `x = &0` THEN ASM_REWRITE_TAC[INT_DIVIDES_ZERO] THEN
  CONV_TAC "Library/integer.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN
...
let INT_COPRIME_POW_ABS = prove
 (`!n a b c. coprime(a,b) /\ a * b = c pow n
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "Library/integer.ml:BINDER_CONV" BINDER_CONV [SWAP_FORALL_THM] THEN
  GEN_REWRITE_TAC "Library/integer.ml:I" I [SWAP_FORALL_THM] THEN ASM_CASES_TAC `n = 0` THENL
   ...
   [REPEAT GEN_TAC THEN ASM_REWRITE_TAC[INT_POW_ZERO; INT_ENTIRE] THEN
    ...
    (ANTS_TAC THENL
      [MATCH_MP_TAC(INT_RING `!p. ~(p = &0) /\ a * p = b * p ==> a = b`) THEN
       ...
       FIRST_X_ASSUM(MP_TAC o SYM) THEN CONV_TAC "Library/integer.ml:INT_RING" INT_RING;
       STRIP_TAC THEN
...
let INT_DIVIDES_PRIME_POW_LE = prove
 (`!p q m n. int_prime p /\ int_prime q
  ...
  GEN_REWRITE_TAC "Library/integer.ml:LAND_CONV" LAND_CONV [GSYM INT_DIVIDES_LABS] THEN
  GEN_REWRITE_TAC "Library/integer.ml:LAND_CONV" LAND_CONV [GSYM INT_DIVIDES_RABS] THEN
...
let INT_EQ_PRIME_POW_ABS = prove
 (`!p q m n. int_prime p /\ int_prime q
  ...
  GEN_REWRITE_TAC "Library/integer.ml:LAND_CONV" LAND_CONV [GSYM INT_DIVIDES_ANTISYM_ABS] THEN
