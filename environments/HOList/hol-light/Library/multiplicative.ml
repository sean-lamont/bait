...
let MULTIPLICATIVE_CONVOLUTION = prove
 (`!f g. multiplicative f /\ multiplicative g
         ==> multiplicative (\n. nsum {d | d divides n}
                                      (\d. f(d) * g(n DIV d)))`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/multiplicative.ml:(LAND_CONV o BINOP_CONV)" (LAND_CONV o BINOP_CONV) [multiplicative] THEN
  ...
  GEN_REWRITE_TAC "Library/multiplicative.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [MULT_SYM] THEN
  ASM_SIMP_TAC[GSYM NSUM_LMUL; NSUM_NSUM_PRODUCT; FINITE_DIVISORS] THEN
  CONV_TAC "Library/multiplicative.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC NSUM_EQ_GENERAL THEN
  ...
  CONV_TAC "Library/multiplicative.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN REWRITE_TAC[IN_ELIM_THM] THEN
...
let REAL_MULTIPLICATIVE_CONVOLUTION = prove
 (`!f g. real_multiplicative f /\ real_multiplicative g
         ==> real_multiplicative (\n. sum {d | d divides n}
                                          (\d. f(d) * g(n DIV d)))`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/multiplicative.ml:(LAND_CONV o BINOP_CONV)" (LAND_CONV o BINOP_CONV) [real_multiplicative] THEN
  ...
  MAP_EVERY X_GEN_TAC [`m:num`; `n:num`] THEN STRIP_TAC THEN
  GEN_REWRITE_TAC "Library/multiplicative.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [REAL_MUL_SYM] THEN
  ASM_SIMP_TAC[GSYM SUM_LMUL; SUM_SUM_PRODUCT; FINITE_DIVISORS] THEN
  CONV_TAC "Library/multiplicative.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_EQ_GENERAL THEN
  EXISTS_TAC `\(a:num,b). a * b` THEN REWRITE_TAC[EXISTS_UNIQUE_DEF] THEN
  REWRITE_TAC[FORALL_PAIR_THM; EXISTS_PAIR_THM; IN_ELIM_PAIR_THM] THEN
...
let REAL_ABS_MOBIUS = prove
 (`!n. abs(mobius n) <= &1`,
  ...
  CONV_TAC "Library/multiplicative.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let MOBIUS_MULT = prove
 (`!a b. coprime(a,b) ==> mobius(a * b) = mobius a * mobius b`,
  ...
  CONV_TAC "Library/multiplicative.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_UNION_EQ THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC FINITE_SUBSET THEN EXISTS_TAC `{p | p divides a * b}` THEN
    ...
    REWRITE_TAC[CONTRAPOS_THM; EXP_2] THEN CONV_TAC "Library/multiplicative.ml:NUMBER_RULE" NUMBER_RULE]);;
 ...
 (`!f g. (!n. 1 <= n ==> g(n) = sum {d | d divides n} f)
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC EQ_TRANS THEN
    ...
    CONV_TAC "Library/multiplicative.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_SUPERSET THEN
