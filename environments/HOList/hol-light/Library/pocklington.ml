...
(* Removed for analysis compatibility *)
(*
open Binary;;
open Card;;
open Permutations;;
...
let nat_mod = prove
 (`!x y n:num. (mod n) x y <=> ?q1 q2. x + n * q1 = y + n * q2`,
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC "Library/pocklington.ml:LAND_CONV" LAND_CONV [GSYM cong] THEN
 ...
 (`!n. { m | 0 < m /\ m < n } HAS_SIZE (n - 1)`,
  INDUCT_TAC THENL
   [SUBGOAL_THEN `{m | 0 < m /\ m < 0} = EMPTY` SUBST1_TAC THENL
    ...
    CONV_TAC "Library/pocklington.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
    ASM_CASES_TAC `n = 0` THENL
     [SUBGOAL_THEN `{m | 0 < m /\ m < SUC n} = EMPTY` SUBST1_TAC THENL
       [ASM_REWRITE_TAC[EXTENSION; IN_ELIM_THM; NOT_IN_EMPTY] THEN
        ARITH_TAC; ALL_TAC] THEN
      ASM_REWRITE_TAC[] THEN CONV_TAC "Library/pocklington.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
...
let COPRIME_MOD = prove
 (`!a n. ~(n = 0) ==> (coprime(a MOD n,n) <=> coprime(a,n))`,
  ...
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/pocklington.ml:(RAND_CONV o RAND_CONV o LAND_CONV)" (RAND_CONV o RAND_CONV o LAND_CONV)
 ...
 (`!x y n. ~(n = 0) ==> ((x == y) (mod n) <=> (x MOD n = y MOD n))`,
  ...
  REPEAT STRIP_TAC THEN EQ_TAC THEN STRIP_TAC THENL
   [ASM_CASES_TAC `x <= y` THENL
     [CONV_TAC "Library/pocklington.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MOD_EQ THEN EXISTS_TAC `q1 - q2`;
...
let CONG_SUB_CASES = prove
 (`!x y n. (x == y) (mod n) <=>
  ...
  COND_CASES_TAC THENL
   [GEN_REWRITE_TAC "Library/pocklington.ml:LAND_CONV" LAND_CONV [SWAP_EXISTS_THM]; ALL_TAC] THEN
...
let CONG_MOD = prove
 (`!a n. ~(n = 0) ==> (a MOD n == a) (mod n)`,
  ...
  DISCH_THEN(CONJUNCTS_THEN2 MP_TAC ASSUME_TAC) THEN
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Library/pocklington.ml:(RATOR_CONV o RAND_CONV)" (RATOR_CONV o RAND_CONV) [th]) THEN
  REWRITE_TAC[cong; nat_mod] THEN
...
let CONG_SOLVE = prove
 (`!a b n. coprime(a,n) ==> ?x. (a * x == b) (mod n)`,
  ...
  ASM_REWRITE_TAC[] THEN GEN_REWRITE_TAC "Library/pocklington.ml:LAND_CONV" LAND_CONV [SWAP_EXISTS_THM] THEN
  ...
  GEN_REWRITE_TAC "Library/pocklington.ml:LAND_CONV" LAND_CONV [ARITH_RULE `b = 0 + b`] THEN
...
let CONG_SOLVE_UNIQUE_NONTRIVIAL = prove
 (`!a p x. prime p /\ coprime(p,a) /\ 0 < x /\ x < p
  ...
  AP_TERM_TAC THEN GEN_REWRITE_TAC "Library/pocklington.ml:I" I [FUN_EQ_THM] THEN
 ...
 (`!a b m n.
  ...
  REPEAT STRIP_TAC THEN REWRITE_TAC[EXISTS_UNIQUE_THM] THEN CONJ_TAC THENL
   [MP_TAC(SPECL [`a:num`; `b:num`; `m:num`; `n:num`] CHINESE_REMAINDER) THEN
    ...
    GEN_REWRITE_TAC "Library/pocklington.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [MULT_SYM] THEN
    CONJ_TAC THENL
...
let PHI_LIMIT = prove
 (`!n. phi(n) <= n`,
  ...
  GEN_REWRITE_TAC "Library/pocklington.ml:RAND_CONV" RAND_CONV [GSYM CARD_NUMSEG_LT] THEN
 ...
 (`!n. (phi n = n - 1) /\ ~(n = 0) /\ ~(n = 1) <=> prime n`,
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    ...
    REWRITE_TAC[COPRIME_SYM] THEN CONV_TAC "Library/pocklington.ml:TAUT" TAUT] THEN
...
let PHI_2 = prove
 (`phi 2 = 1`,
  SIMP_TAC[PHI_PRIME; PRIME_2] THEN CONV_TAC "Library/pocklington.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV);;
...
let PHI_MULTIPLICATIVE = prove
 (`!a b. coprime(a,b) ==> phi(a * b) = phi(a) * phi(b)`,
  ...
  CONV_TAC "Library/pocklington.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_IMAGE_INJ_EQ THEN
 ...
 (`!n. EVEN(phi n) <=> n = 0 \/ 3 <= n`,
  GEN_TAC THEN EQ_TAC THENL
   [ONCE_REWRITE_TAC[GSYM CONTRAPOS_THM] THEN
    ...
    STRIP_TAC THEN ASM_REWRITE_TAC[PHI_1; PHI_2] THEN CONV_TAC "Library/pocklington.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
...
let ITERATE_OVER_COPRIME = prove
 (`!op f n k.
  ...
  UNDISCH_TAC `(k * m == 1) (mod n)` THEN CONV_TAC "Library/pocklington.ml:NUMBER_RULE" NUMBER_RULE);;
...
let FERMAT_LITTLE = prove
 (`!a n. coprime(a,n) ==> (a EXP (phi n) == 1) (mod n)`,
  ...
  ASM_SIMP_TAC[COPRIME_0; PHI_0; CONG_MOD_0] THEN CONV_TAC "Library/pocklington.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
...
let LUCAS = prove
 (`!a n. 2 <= n /\
  ...
  SUBGOAL_THEN `~(r = 1)` MP_TAC THENL
   [UNDISCH_TAC `m < m * r` THEN CONV_TAC "Library/pocklington.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let ORDER_DIVIDES = prove
 (`!n a d. (a EXP d == 1) (mod n) <=> order(n) a divides d`,
  GEN_TAC THEN GEN_TAC THEN REWRITE_TAC[order] THEN CONV_TAC "Library/pocklington.ml:SELECT_CONV" SELECT_CONV THEN
  ...
  REPEAT GEN_TAC THEN EQ_TAC THEN DISCH_TAC THENL
   [ONCE_REWRITE_TAC[COPRIME_SYM] THEN DISCH_TAC THEN
    ...
    FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP (TAUT
     `~p ==> (q ==> p) ==> q ==> r`)) THEN
    REWRITE_TAC[EXP] THEN CONV_TAC "Library/pocklington.ml:NUMBER_RULE" NUMBER_RULE]);;
...
let ORDER_UNIQUE = prove
 (`!n a k. 0 < k /\
  ...
  UNDISCH_TAC `(a EXP k == 1) (mod n)` THEN CONV_TAC "Library/pocklington.ml:NUMBER_RULE" NUMBER_RULE);;
...
let PRIME_PRIME_FACTOR = prove
 (`!n. prime n <=> ~(n = 1) /\ !p. prime p /\ p divides n ==> (p = n)`,
  GEN_TAC THEN GEN_REWRITE_TAC "Library/pocklington.ml:LAND_CONV" LAND_CONV [prime] THEN
...
let PRIME_PRIME_FACTOR_SQRT = prove
 (`!n. prime n <=>
  ...
  GEN_REWRITE_TAC "Library/pocklington.ml:LAND_CONV" LAND_CONV [PRIME_DIVISOR_SQRT] THEN
...
let POCKLINGTON_ALT = prove
 (`!a n q r.
  ...
  GEN_REWRITE_TAC "Library/pocklington.ml:LAND_CONV" LAND_CONV [divides] THEN
  ...
  ASM_SIMP_TAC[DIV_MULT] THEN
  GEN_REWRITE_TAC "Library/pocklington.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [MULT_SYM] THEN
  GEN_REWRITE_TAC "Library/pocklington.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [EXP_MULT] THEN
...
let PRIMEFACT = prove
 (`!n. ~(n = 0) ==> ?ps. primefact ps n`,
  ...
  GEN_REWRITE_TAC "Library/pocklington.ml:(funpow 3 LAND_CONV)" (funpow 3 LAND_CONV) [ARITH_RULE `n = 1 * n`] THEN
...
let EXP_MOD_CONV =
  let pth = prove
   (`~(n = 0)
    ...
    CONV_TAC "Library/pocklington.ml:(ONCE_DEPTH_CONV let_CONV)" (ONCE_DEPTH_CONV let_CONV) THEN
