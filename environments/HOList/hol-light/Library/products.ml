...
let NPRODUCT_REFLECT = prove
 (`!x m n. nproduct(m..n) x =
  ...
  GEN_REWRITE_TAC "Library/products.ml:LAND_CONV" LAND_CONV [MATCH_MP ITERATE_REFLECT MONOIDAL_MUL] THEN
   ...
   [REWRITE_TAC[GSYM SUBSET_ANTISYM_EQ] THEN CONJ_TAC THENL
    ...
    MATCH_MP_TAC HAS_SIZE_IMAGE_INJ THEN CONJ_TAC THENL
     [REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM; FORALL_IN_GSPEC] THEN
      ...
      GEN_REWRITE_TAC "Library/products.ml:RAND_CONV" RAND_CONV [MULT_SYM] THEN
...
let PRODUCT_LE_1 = prove
 (`!f s. FINITE s /\ (!x. x IN s ==> &0 <= f x /\ f x <= &1)
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Library/products.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
...
let PRODUCT_REFLECT = prove
 (`!x m n. product(m..n) x =
  ...
  GEN_REWRITE_TAC "Library/products.ml:LAND_CONV" LAND_CONV [MATCH_MP ITERATE_REFLECT MONOIDAL_REAL_MUL] THEN
