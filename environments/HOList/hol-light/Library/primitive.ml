 ...
 (`!p l r.
  ...
  X_GEN_TAC `c:int` THEN STRIP_TAC THEN ASM_CASES_TAC `n = 0` THENL
   [MAP_EVERY UNDISCH_TAC
    ...
    CONV_TAC "Library/primitive.ml:TAUT" TAUT;
    ALL_TAC] THEN
...
let COUNT_ORDERS_MODULO_PRIME = prove
 (`!p d. prime p /\ d divides (p - 1)
  ...
  MP_TAC THENL
   [CONV_TAC "Library/primitive.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_SUBSET_LE THEN
    SIMP_TAC[FINITE_RESTRICT; FINITE_NUMSEG] THEN CONJ_TAC THENL
     [REWRITE_TAC[SUBSET; FORALL_IN_IMAGE; IN_ELIM_THM] THEN
      ...
      GEN_REWRITE_TAC "Library/primitive.ml:LAND_CONV" LAND_CONV [GSYM CARD_NUMSEG_LT] THEN
      MATCH_MP_TAC EQ_IMP_LE THEN CONV_TAC "Library/primitive.ml:SYM_CONV" SYM_CONV THEN
 ...
 (`!p. prime p /\ 3 <= p
  ...
  STRIP_ASSUME_TAC THENL
   [ASM_REWRITE_TAC[GSYM EXP_EXP] THEN
    ...
    GEN_REWRITE_TAC "Library/primitive.ml:(BINDER_CONV o funpow 3 RAND_CONV)" (BINDER_CONV o funpow 3 RAND_CONV) [MULT_SYM] THEN
...
let PRIMITIVE_ROOT_MODULO_2 = prove
 (`?x. x IN 1..1 /\ order 2 x = phi(2)`,
  ...
  SIMP_TAC[PHI_PRIME; PRIME_2] THEN CONV_TAC "Library/primitive.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  MATCH_MP_TAC ORDER_UNIQUE THEN
  REWRITE_TAC[ARITH_RULE `~(0 < m /\ m < 1)`] THEN
  CONV_TAC "Library/primitive.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "Library/primitive.ml:(ONCE_DEPTH_CONV CONG_CONV)" (ONCE_DEPTH_CONV CONG_CONV) THEN
  REWRITE_TAC[]);;
...
let PRIMITIVE_ROOT_MODULO_4 = prove
 (`?x. x IN 1..3 /\ order 4 x = phi(4)`,
  ...
  SIMP_TAC[PHI_PRIMEPOW; PRIME_2] THEN CONV_TAC "Library/primitive.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  MATCH_MP_TAC ORDER_UNIQUE THEN
...
let PRIMITIVE_ROOT_MODULO_DOUBLE_PRIMEPOW = prove
 (`!p k. prime p /\ 3 <= p /\ 1 <= k
  ...
  CONV_TAC "Library/primitive.ml:NUMBER_RULE" NUMBER_RULE);;
...
let CONG_TO_1_POW2 = prove
 (`!k x. ODD x /\ 1 <= k ==> (x EXP (2 EXP k) == 1) (mod (2 EXP (k + 2)))`,
  ...
  CONV_TAC "Library/primitive.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN GEN_TAC THEN ASM_CASES_TAC `k = 0` THENL
   [ASM_REWRITE_TAC[] THEN CONV_TAC "Library/primitive.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    ...
    REWRITE_TAC[EVEN_MULT; EVEN_ADD; ARITH] THEN CONV_TAC "Library/primitive.ml:TAUT" TAUT;
    ...
    CONV_TAC "Library/primitive.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
 ...
 (`!k. 3 <= k ==> ~(?x. order (2 EXP k) x = phi(2 EXP k))`,
  REPEAT STRIP_TAC THEN DISJ_CASES_TAC(SPEC `x:num` EVEN_OR_ODD) THENL
   [FIRST_X_ASSUM(MP_TAC o MATCH_MP (ARITH_RULE
    ...
    REWRITE_TAC[EXP_ADD] THEN CONV_TAC "Library/primitive.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
 ...
 (`!n. (?x. order n x = phi n) <=>
  ...
  ASM_REWRITE_TAC[] THEN ASM_CASES_TAC `n = 1` THENL
   [ASM_REWRITE_TAC[PRIMITIVE_ROOT_MODULO_1] THEN
    ...
    CONV_TAC "Library/primitive.ml:(ONCE_DEPTH_CONV PRIME_CONV)" (ONCE_DEPTH_CONV PRIME_CONV) THEN CONV_TAC "Library/primitive.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
    ALL_TAC] THEN
  EQ_TAC THENL
   [ALL_TAC;
  ...
  ASM_CASES_TAC `m = 1` THENL
   [ASM_REWRITE_TAC[MULT_CLAUSES] THEN
    ...
    REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "Library/primitive.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
    ALL_TAC] THEN
...
let PRIMITIVE_ROOT_EXISTS_NONTRIVIAL = prove
 (`!n. (?x. x IN 1..n-1 /\ order n x = phi n) <=>
  ...
  GEN_TAC THEN ASM_CASES_TAC `n = 0` THENL
   [ASM_REWRITE_TAC[IN_NUMSEG] THEN CONV_TAC "Library/primitive.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    MATCH_MP_TAC(TAUT `~a /\ ~b ==> (a <=> b)`) THEN
    CONV_TAC "Library/primitive.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
  ...
  EXISTS_TAC `?x. order n x = phi n` THEN CONJ_TAC THENL
   [EQ_TAC THENL [MESON_TAC[]; ALL_TAC] THEN
    ...
    CONV_TAC "Library/primitive.ml:(BINOP_CONV(ONCE_DEPTH_CONV SYM_CONV))" (BINOP_CONV(ONCE_DEPTH_CONV SYM_CONV)) THEN
