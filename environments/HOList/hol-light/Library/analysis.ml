...
let REAL_ADD_LID_UNIQ = prove(
  `!x y. (x + y = y) <=> (x = &0)`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM REAL_ADD_LID]
...
let REAL_NEG_ADD = prove(
  `!x y. --(x + y) = (--x) + (--y)`,
  REPEAT GEN_TAC THEN CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN
...
let REAL_NEGNEG = prove(
  `!x. --(--x) = x`,
  GEN_TAC THEN CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN
...
let REAL_LT_LE = prove(
  ...
  POP_ASSUM MP_TAC THEN CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[] THEN
...
let REAL_LE_TRANS = prove(
  `!x y z. x <= y /\ y <= z ==> x <= z`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [REAL_LE_LT] THEN
...
let REAL_LT_01 = prove(
  ...
  CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
  REWRITE_TAC[REAL_10]);;
...
let REAL_LT_ADD1 = prove(
  ...
  DISCH_THEN DISJ_CASES_TAC THENL
   [POP_ASSUM(MP_TAC o MATCH_MP REAL_LT_ADD2 o C CONJ REAL_LT_01) THEN
    ...
    GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [GSYM REAL_ADD_RID] THEN
...
let REAL_LE_DOUBLE = prove(
  ...
  GEN_TAC THEN EQ_TAC THENL
   [CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[REAL_NOT_LE] THEN
...
let REAL_LE_NEGR = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [REAL_NEGNEG] THEN
...
let REAL_LT_IMP_NE = prove(
  `!x y. x < y ==> ~(x = y)`,
  REPEAT GEN_TAC THEN CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let REAL_LINV_UNIQ = prove(
  ...
  DISCH_THEN SUBST1_TAC THEN CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN
...
let REAL_INV1 = prove(
  `inv(&1) = &1`,
  CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_LINV_UNIQ THEN
  REWRITE_TAC[REAL_MUL_LID]);;
...
let REAL_LT_NZ = prove(
  ...
  CONV_TAC "Library/analysis.ml:(RAND_CONV(ONCE_DEPTH_CONV SYM_CONV))" (RAND_CONV(ONCE_DEPTH_CONV SYM_CONV)) THEN
...
let REAL_LT_FRACTION = prove(
  ...
  FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV)
...
let REAL_LT_HALF2 = prove(
  ...
  CONV_TAC "Library/analysis.ml:(RAND_CONV num_CONV)" (RAND_CONV num_CONV) THEN
...
let REAL_LE_LDIV = prove(
  ...
  SUBGOAL_THEN `y = (y / x) * x` MP_TAC THENL
   [CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_DIV_RMUL THEN
    CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
    MATCH_MP_TAC REAL_LT_IMP_NE THEN POP_ASSUM ACCEPT_TAC;
    DISCH_THEN(fun t -> GEN_REWRITE_TAC "Library/analysis.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [t])
...
let REAL_LE_RDIV = prove(
  ...
  SUBGOAL_THEN `z = (z / x) * x` MP_TAC THENL
   [CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_DIV_RMUL THEN
    CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
    MATCH_MP_TAC REAL_LT_IMP_NE THEN POP_ASSUM ACCEPT_TAC;
    DISCH_THEN(fun t -> GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [t])
    THEN MATCH_MP_TAC REAL_LE_RMUL_EQ THEN POP_ASSUM ACCEPT_TAC]);;
...
let REAL_LT_1 = prove(
  ...
  SUBGOAL_THEN `(x / y) < &1 <=> ((x / y) * y) < (&1 * y)` SUBST1_TAC THENL
   [CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_LT_RMUL_EQ THEN
    MATCH_MP_TAC REAL_LET_TRANS THEN EXISTS_TAC `x:real` THEN
    ASM_REWRITE_TAC[];
    SUBGOAL_THEN `(x / y) * y = x` SUBST1_TAC THENL
     [MATCH_MP_TAC REAL_DIV_RMUL THEN CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let REAL_INV_LT1 = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:I" I [TAUT `a <=> ~ ~a`] THEN
  ...
  DISCH_THEN(DISJ_CASES_THEN MP_TAC) THENL
   [DISCH_TAC THEN
    ...
    SUBGOAL_THEN `inv(inv x) = x` SUBST1_TAC THENL
     [MATCH_MP_TAC REAL_INVINV THEN CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let REAL_POS_NZ = prove(
  ...
  CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN POP_ASSUM ACCEPT_TAC);;
...
let REAL_SUMSQ = prove(
  ...
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[DE_MORGAN_THM] THEN
...
let ABS_MUL = prove(
  ...
  REPEAT GEN_TAC THEN ASM_CASES_TAC `&0 <= x` THENL
   [ALL_TAC;
    ...
    GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [GSYM ABS_NEG] THEN
    GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [GSYM ABS_NEG]
  ...
  (ASM_CASES_TAC `&0 <= y` THENL
    [ALL_TAC;
     ...
     POP_ASSUM(K ALL_TAC) THEN DISCH_TAC THEN
     GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [GSYM ABS_NEG] THEN
     GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM ABS_NEG] THEN
...
let ABS_SUB = prove(
  `!x y. abs(x - y) = abs(y - x)`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_NEG_SUB] THEN
  REWRITE_TAC[ABS_NEG]);;
...
let ABS_NZ = prove(
  ...
  GEN_TAC THEN EQ_TAC THENL
   [ONCE_REWRITE_TAC[GSYM ABS_ZERO] THEN
    ...
    CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
    REWRITE_TAC[GSYM REAL_LE_LT; ABS_POS];
    CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[] THEN
...
let ABS_ABS = prove(
  `!x. abs(abs(x)) = abs(x)`,
  GEN_TAC THEN
  GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [real_abs] THEN
  REWRITE_TAC[ABS_POS]);;
...
let ABS_BETWEEN = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(funpow 2 RAND_CONV)" (funpow 2 RAND_CONV) [REAL_ADD_SYM] THEN
...
let REAL_SUB_ABS = prove(
  ...
  EXISTS_TAC `(abs(x - y) + abs(y)) - abs(y)` THEN CONJ_TAC THENL
   [ONCE_REWRITE_TAC[real_sub] THEN REWRITE_TAC[REAL_LE_RADD] THEN
    ...
    GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [REAL_SUB_ADD] THEN
...
let ABS_SUB_ABS = prove(
  ...
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(RATOR_CONV o ONCE_DEPTH_CONV)" (RATOR_CONV o ONCE_DEPTH_CONV) [real_abs] THEN
...
let ABS_BOUNDS = prove(
  `!x k. abs(x) <= k <=> --k <= x /\ x <= k`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [GSYM REAL_LE_NEG] THEN
...
let POW_PLUS1 = prove(
  ...
  EXISTS_TAC `(&1 + e) * (&1 + (&n * e))` THEN CONJ_TAC THENL
   [REWRITE_TAC[REAL_RDISTRIB; REAL; REAL_MUL_LID] THEN
    ...
    GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [GSYM REAL_ADD_LID] THEN
...
let REAL_LE1_POW2 = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_LID] THEN
...
let POW_POS_LT = prove(
  ...
  DISCH_TAC THEN CONJ_TAC THENL
   [MATCH_MP_TAC POW_POS THEN ASM_REWRITE_TAC[];
    ...
    CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN ASM_REWRITE_TAC[]]);;
...
let REAL_ARCH = prove(
  ...
  CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV NOT_EXISTS_CONV)" (ONCE_DEPTH_CONV NOT_EXISTS_CONV) THEN
  ...
  ASM_REWRITE_TAC[] THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(funpow 3 RAND_CONV)" (funpow 3 RAND_CONV) [GSYM REAL_MUL_LID] THEN
...
let sum_EXISTS = prove
 (`?sum. (!f n. sum(n,0) f = &0) /\
  ...
  CONV_TAC "Library/analysis.ml:(DEPTH_CONV GEN_BETA_CONV)" (DEPTH_CONV GEN_BETA_CONV) THEN ASM_REWRITE_TAC[]);;
 ...
 (`!f g m n. (!r. m <= r /\ r < n + m ==> f(r) <= g(r))
  ...
  FIRST_ASSUM MATCH_MP_TAC THENL
   [GEN_TAC THEN DISCH_TAC THEN FIRST_ASSUM MATCH_MP_TAC THEN
    ...
    GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [ADD_SYM]] THEN
...
let SUM_2 = prove
 (`!f n. sum(n,2) f = f(n) + f(n + 1)`,
  REPEAT GEN_TAC THEN CONV_TAC "Library/analysis.ml:(REDEPTH_CONV num_CONV)" (REDEPTH_CONV num_CONV) THEN
  REWRITE_TAC[sum; ADD_CLAUSES; REAL_ADD_LID]);;

let SUM_OFFSET = prove
 (`!f n k. sum(0,n)(\m. f(m + k)) = sum(0,n + k) f - sum(0,k) f`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [ADD_SYM] THEN
...
let SUM_SPLIT = prove
 (`!f n p. sum(m,n) f + sum(m + n,p) f = sum(m,n + p) f`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [SUM_DIFF] THEN
  GEN_REWRITE_TAC "Library/analysis.ml:RAND_CONV" RAND_CONV [SUM_DIFF] THEN
  REWRITE_TAC[ADD_ASSOC] THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [GSYM SUM_TWO] THEN
  REAL_ARITH_TAC);;
...
let SUBSETA_ANTISYM = prove(
  ...
  CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV AND_FORALL_CONV)" (ONCE_DEPTH_CONV AND_FORALL_CONV) THEN
  REWRITE_TAC[TAUT `(a ==> b) /\ (b ==> a) <=> (a <=> b)`] THEN
  CONV_TAC "Library/analysis.ml:(RAND_CONV FUN_EQ_CONV)" (RAND_CONV FUN_EQ_CONV) THEN REFL_TAC);;
...
let CLOSED_LIMPT = prove(
  ...
  REPEAT GEN_TAC THEN CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV CONTRAPOS_CONV)" (ONCE_DEPTH_CONV CONTRAPOS_CONV) THEN
  REWRITE_TAC[closed; limpt] THEN
  CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV NOT_FORALL_CONV)" (ONCE_DEPTH_CONV NOT_FORALL_CONV) THEN
...
let metric_tybij = new_type_definition "metric" ("metric","mdist")
      (prove(`?m:(A#A->real). ismet m`,
        ...
        CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
        CONJ_TAC THEN REPEAT GEN_TAC THENL
         [BOOL_CASES_TAC `x:A = y` THEN REWRITE_TAC[REAL_10];
          ...
          THEN GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [GSYM REAL_ADD_LID] THEN
...
let METRIC_POS = prove(
  ...
  THEN CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[REAL_NOT_LE] THEN
...
let METRIC_TRIANGLE = prove(
  ...
  REPEAT GEN_TAC THEN ASSUME_TAC(SPEC `m:(A)metric` METRIC_ISMET) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[ismet]) THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [METRIC_SYM] THEN
  ASM_REWRITE_TAC[]);;
...
let mtop_istopology = prove(
  ...
  CONV_TAC "Library/analysis.ml:(REDEPTH_CONV BETA_CONV)" (REDEPTH_CONV BETA_CONV) THEN
...
let ISMET_R1 = prove(
  `ismet (\(x,y). abs(y - x))`,
  REWRITE_TAC[ismet] THEN CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
  CONJ_TAC THEN REPEAT GEN_TAC THENL
   [REWRITE_TAC[ABS_ZERO; REAL_SUB_0] THEN
    CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN REFL_TAC;
...
let MR1_DEF = prove(
  ...
  THEN CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN REFL_TAC);;
...
let MTOP_TENDS = prove(
  ...
  REPEAT GEN_TAC THEN REWRITE_TAC[tends] THEN EQ_TAC THEN DISCH_TAC THENL
   [GEN_TAC THEN DISCH_TAC THEN
    ...
    BETA_TAC THEN GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV)
...
let MTOP_TENDS_UNIQ = prove(
  ...
  CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_TAC THEN
  CONV_TAC "Library/analysis.ml:NOT_FORALL_CONV" NOT_FORALL_CONV THEN
...
let MR1_BOUNDED = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o RAND_CONV o ABS_CONV)" (LAND_CONV o RAND_CONV o ABS_CONV)
   [SWAP_EXISTS_THM] THEN
  ONCE_REWRITE_TAC[SWAP_EXISTS_THM] THEN
  AP_TERM_TAC THEN ABS_TAC THEN
  CONV_TAC "Library/analysis.ml:(REDEPTH_CONV EXISTS_AND_CONV)" (REDEPTH_CONV EXISTS_AND_CONV) THEN
  AP_TERM_TAC THEN EQ_TAC THEN DISCH_THEN(X_CHOOSE_THEN `k:real` MP_TAC) THENL
   [DISCH_THEN(X_CHOOSE_TAC `x:real`) THEN
    ...
    REWRITE_TAC[ABS_TRIANGLE] THEN
    GEN_REWRITE_TAC "Library/analysis.ml:RAND_CONV" RAND_CONV [REAL_ADD_SYM] THEN
...
let NET_NULL_ADD = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:RAND_CONV" RAND_CONV [GSYM REAL_HALF_DOUBLE] THEN
...
let NET_NULL_MUL = prove(
  ...
  CONV_TAC "Library/analysis.ml:(LAND_CONV LEFT_AND_EXISTS_CONV)" (LAND_CONV LEFT_AND_EXISTS_CONV) THEN
  ...
  SUBGOAL_THEN `e = k * (e / k)` SUBST1_TAC THENL
   [CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_DIV_LMUL THEN
    DISCH_THEN SUBST_ALL_TAC THEN UNDISCH_TAC `&0 < &0` THEN
...
let NET_NEG = prove(
  ...
  REWRITE_TAC[REAL_SUB_NEG2] THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [ABS_SUB] THEN
  REFL_TAC);;
...
let NET_SUB = prove(
  ...
  REWRITE_TAC[real_sub] THEN
  CONV_TAC "Library/analysis.ml:(EXACT_CONV[X_BETA_CONV `n:A` `--(y(n:A))`])" (EXACT_CONV[X_BETA_CONV `n:A` `--(y(n:A))`]) THEN
...
let NET_MUL = prove(
  ...
  CONV_TAC "Library/analysis.ml:(EXACT_CONV[X_BETA_CONV `n:A` `x(n:A) * (y(n) - y0)`])" (EXACT_CONV[X_BETA_CONV `n:A` `x(n:A) * (y(n) - y0)`]) THEN
  CONV_TAC "Library/analysis.ml:(EXACT_CONV[X_BETA_CONV `n:A` `(x(n:A) - x0) * y0`])" (EXACT_CONV[X_BETA_CONV `n:A` `(x(n:A) - x0) * y0`]) THEN
  FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP NET_NULL_ADD) THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [REAL_MUL_SYM] THEN
  (CONV_TAC "Library/analysis.ml:o" o EXACT_CONV o map (X_BETA_CONV `n:A`))
...
let NET_INV = prove(
  ...
  CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV LEFT_AND_EXISTS_CONV)" (ONCE_DEPTH_CONV LEFT_AND_EXISTS_CONV) THEN
  DISCH_THEN(X_CHOOSE_THEN `k:real` MP_TAC) THEN
...
let NET_DIV = prove(
  ...
  CONV_TAC "Library/analysis.ml:(EXACT_CONV[X_BETA_CONV `n:A` `inv(y(n:A))`])" (EXACT_CONV[X_BETA_CONV `n:A` `inv(y(n:A))`]) THEN
 ...
 (`!g. dorder g /\
  ...
  FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP NET_ADD) THEN CONJ_TAC THENL
   [FIRST_ASSUM MATCH_MP_TAC THEN
    ...
    CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN FIRST_ASSUM MATCH_MP_TAC THEN
    ARITH_TAC]);;
...
let SEQ_BOUNDED_2 = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [real_abs] THEN COND_CASES_TAC THENL
...
let SEQ_ICONV = prove(
  ...
  ASSUME_TAC THENL
   [FIRST_ASSUM(MP_TAC o SPEC `sup(\x. ?n:num. x = f(n))`) THEN
    ...
    REWRITE_TAC[REAL_NOT_LT] THEN
    CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV LEFT_IMP_EXISTS_CONV)" (ONCE_DEPTH_CONV LEFT_IMP_EXISTS_CONV) THEN
    ...
    BETA_TAC THEN DISCH_THEN ASSUME_TAC THEN SUBGOAL_THEN
     ...
     [INDUCT_TAC THENL
       [SUBGOAL_THEN `f(0) >= (SUC N)` MP_TAC THENL
        ...
        CONV_TAC "Library/analysis.ml:SELECT_CONV" SELECT_CONV THEN FIRST_ASSUM MATCH_MP_TAC THEN
        FIRST_ASSUM ACCEPT_TAC;
        FIRST_ASSUM(UNDISCH_TAC o
...
let SEQ_INV0 = prove(
  ...
  SUBGOAL_THEN `~(f(n:num) = &0)` ASSUME_TAC THENL
   [CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN MATCH_MP_TAC REAL_LT_IMP_NE THEN
  ...
  SUBGOAL_THEN `e = inv(inv e)` SUBST1_TAC THENL
   [CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_INVINV THEN
    CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let SEQ_POWER_ABS = prove(
  ...
  REWRITE_TAC[REAL_LE_LT] THEN DISCH_THEN DISJ_CASES_TAC THENL
   [SUBGOAL_THEN `!n. abs(c) pow n = inv(inv(abs(c) pow n))`
      (fun th -> ONCE_REWRITE_TAC[th]) THENL
     [GEN_TAC THEN CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_INVINV THEN
      MATCH_MP_TAC POW_NZ THEN
      ASM_REWRITE_TAC[ABS_NZ; ABS_ABS]; ALL_TAC] THEN
    CONV_TAC "Library/analysis.ml:(EXACT_CONV[X_BETA_CONV `n:num` `inv(abs(c) pow n)`])" (EXACT_CONV[X_BETA_CONV `n:num` `inv(abs(c) pow n)`]) THEN
    ...
    ASSUME_TAC THENL
     [MATCH_MP_TAC REAL_LTE_TRANS THEN
      ...
      FIRST_ASSUM(fun th ->
        GEN_REWRITE_TAC "Library/analysis.ml:I" I [MATCH_MP REAL_LE_RMUL_EQ th]) THEN
...
let SEQ_HARMONIC = prove
 (`!a. (\n. a / &n) --> &0`,
  ...
  ASM_REWRITE_TAC[] THEN GEN_REWRITE_TAC "Library/analysis.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
...
let NEST_LEMMA = prove(
  ...
  REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC SEQ_LE THEN
    ...
    X_GEN_TAC `m:num` THEN GEN_REWRITE_TAC "Library/analysis.ml:I" I [TAUT `a <=> ~ ~a`] THEN
...
let NEST_LEMMA_UNIQ = prove(
  ...
  CONV_TAC "Library/analysis.ml:(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN
...
let BOLZANO_LEMMA = prove(
  `!P. (!a b c. a <= b /\ b <= c /\ P(a,b) /\ P(b,c) ==> P(a,c)) /\
      ...
      ==> !a b. a <= b ==> P(a,b)`,
  REPEAT STRIP_TAC THEN
  GEN_REWRITE_TAC "Library/analysis.ml:I" I [TAUT `a <=> ~ ~a`] THEN DISCH_TAC THEN
  ...
  ASSUME_TAC THENL
   [REWRITE_TAC[real_ge] THEN INDUCT_TAC THEN
    FIRST_ASSUM
     (fun th -> GEN_REWRITE_TAC "Library/analysis.ml:(funpow 2 RAND_CONV)" (funpow 2 RAND_CONV) [th]) THEN
    COND_CASES_TAC THEN REWRITE_TAC[REAL_LE_REFL] THEN
  ...
  SUBGOAL_THEN `!n. SND(f(SUC n) :real#real) <= SND(f n)` ASSUME_TAC THENL
   [BETA_TAC THEN INDUCT_TAC THEN
    FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [th]) THEN
    COND_CASES_TAC THEN REWRITE_TAC[REAL_LE_REFL] THEN
   ...
   [INDUCT_TAC THENL
    ...
    (SUBGOAL_THEN `~(&2 = &0)` (fun th -> REWRITE_TAC[th] THEN
     REWRITE_TAC[MATCH_MP REAL_DIV_LMUL th]) THENL
      [REWRITE_TAC[REAL_INJ; num_CONV `2`; NOT_SUC]; ALL_TAC]) THEN
    REWRITE_TAC[GSYM REAL_DOUBLE] THEN
    GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [REAL_ADD_SYM] THEN
    (SUBGOAL_THEN `!x y z. (x + y) - (x + z) = y - z`
     (fun th -> REWRITE_TAC[th])
     THENL
      [REPEAT GEN_TAC THEN REWRITE_TAC[real_sub; REAL_NEG_ADD] THEN
       GEN_REWRITE_TAC "Library/analysis.ml:RAND_CONV" RAND_CONV [GSYM REAL_ADD_RID] THEN
    ...
    (SUBGOAL_THEN `~(&2 = &0) /\ ~(&2 pow n = &0)`
      ...
      [CONJ_TAC THENL [ALL_TAC; MATCH_MP_TAC POW_NZ] THEN
       ...
       REWRITE_TAC[num_CONV `2`; NOT_SUC];
       ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN REWRITE_TAC[REAL_MUL_ASSOC] THEN
       GEN_REWRITE_TAC "Library/analysis.ml:(RATOR_CONV o RAND_CONV)" (RATOR_CONV o RAND_CONV)
         [GSYM REAL_MUL_LID] THEN
       AP_THM_TAC THEN AP_TERM_TAC THEN CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN
  ...
  W(C SUBGOAL_THEN
   ...
   [ONCE_REWRITE_TAC[SEQ_NEG] THEN BETA_TAC THEN
    ...
    GEN_REWRITE_TAC "Library/analysis.ml:(funpow 2 RAND_CONV)" (funpow 2 RAND_CONV) [GSYM REAL_HALF_DOUBLE] THEN
    CONJ_TAC THENL
     [GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV) [ABS_SUB] THEN
      ASM_REWRITE_TAC[real_abs; REAL_SUB_LE] THEN
...
let BOLZANO_LEMMA_ALT = prove
 (`!P. (!a b c. a <= b /\ b <= c /\ P a b /\ P b c ==> P a c) /\
  ...
  REWRITE_TAC[] THEN
  CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN REWRITE_TAC[]);;
...
let SUMMABLE_SUM = prove(
  ...
  CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
...
let SER_POS_LE = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:I" I [IMP_IMP] THEN
...
let SER_POS_LT_PAIR = prove(
  ...
  DISCH_THEN(MP_TAC o MATCH_MP SUMMABLE_SUM) THEN
  REWRITE_TAC[sums; SEQ] THEN BETA_TAC THEN
  CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[REAL_NOT_LT] THEN DISCH_TAC THEN
  ...
  ASSUME_TAC THENL
   [SPEC_TAC(`N:num`,`N:num`) THEN INDUCT_TAC THENL
     [REWRITE_TAC[MULT_CLAUSES; ADD_CLAUSES] THEN
      ...
      GEN_REWRITE_TAC "Library/analysis.ml:(RATOR_CONV o ONCE_DEPTH_CONV)" (RATOR_CONV o ONCE_DEPTH_CONV) [ADD1] THEN
    ...
    ASSUME_TAC THENL
     [MATCH_MP_TAC REAL_LE_TRANS THEN
      ...
      CONV_TAC "Library/analysis.ml:(REDEPTH_CONV num_CONV)" (REDEPTH_CONV num_CONV) THEN
...
let SER_ADD = prove(
  ...
  CONV_TAC "Library/analysis.ml:((RAND_CONV o EXACT_CONV)[X_BETA_CONV `n:num` `sum(0,n) x`])" ((RAND_CONV o EXACT_CONV)[X_BETA_CONV `n:num` `sum(0,n) x`]) THEN
  CONV_TAC "Library/analysis.ml:((RAND_CONV o EXACT_CONV)[X_BETA_CONV `n:num` `sum(0,n) y`])" ((RAND_CONV o EXACT_CONV)[X_BETA_CONV `n:num` `sum(0,n) y`]) THEN
...
let SER_COMPARA = prove(
  ...
  (fun th -> GEN_REWRITE_TAC "Library/analysis.ml:(RATOR_CONV o ONCE_DEPTH_CONV)" (RATOR_CONV o ONCE_DEPTH_CONV) [th])
  THENL
...
let GP = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o ABS_CONV)" (LAND_CONV o ABS_CONV) [GSYM REAL_NEG_MUL2] THEN
  SUBGOAL_THEN `~(x - &1 = &0)`
  ...
  REWRITE_TAC[REAL_SUB_0] THEN CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
  ASM_REWRITE_TAC[] THEN
  GEN_REWRITE_TAC "Library/analysis.ml:RAND_CONV" RAND_CONV [GSYM REAL_SUB_RZERO] THEN
  SUBGOAL_THEN `(\n. (\n. &1) n - (\n. x pow n) n) --> &1 - &0`
  MP_TAC THENL [ALL_TAC; REWRITE_TAC[BETA_THM]] THEN
...
let ABS_NEG_LEMMA = prove(
  ...
  DISCH_THEN(MP_TAC o MATCH_MP REAL_LE_TRANS) THEN CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
    ...
    EXISTS_TAC `\n. (abs(f N) / c pow N) * (c pow n)` THEN CONJ_TAC THENL
     [EXISTS_TAC `N:num` THEN X_GEN_TAC `n:num` THEN
      ...
      SUBGOAL_THEN `~(c pow N = &0)`
       ...
       [MATCH_MP_TAC POW_NZ THEN CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
        MATCH_MP_TAC REAL_LT_IMP_NE THEN ASM_REWRITE_TAC[]; ALL_TAC] THEN
...
let LIM_CONST = prove(
  ...
  REWRITE_TAC[REAL_SUB_0] THEN CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
  ASM_REWRITE_TAC[]);;
...
let LIM_TRANSFORM = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [REAL_HALF_DOUBLE] THEN
...
let DIFF_CONT = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(RATOR_CONV o LAND_CONV o ABS_CONV o RAND_CONV)" (RATOR_CONV o LAND_CONV o ABS_CONV o RAND_CONV)
    [SYM(BETA_CONV `(\h:real. h) h`)] THEN
  CONV_TAC "Library/analysis.ml:(EXACT_CONV[X_BETA_CONV `h:real` `(f(x + h) - f(x)) / h`])" (EXACT_CONV[X_BETA_CONV `h:real` `(f(x + h) - f(x)) / h`]) THEN
...
let IVT = prove(
  ...
  DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC
   (CONJUNCTS_THEN2 MP_TAC STRIP_ASSUME_TAC)) THEN
  CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN
  DISCH_THEN(ASSUME_TAC o CONV_RULE NOT_EXISTS_CONV) THEN
  ...
  CONJ_TAC THENL
   [MAP_EVERY X_GEN_TAC [`u:real`; `v:real`; `w:real`] THEN
    CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[DE_MORGAN_THM; NOT_IMP] THEN
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(funpow 2 LAND_CONV)" (funpow 2 LAND_CONV) [GSYM ABS_NZ] THEN
...
let DIFF_ADD = prove(
  ...
  CONV_TAC "Library/analysis.ml:(EXACT_CONV[X_BETA_CONV `h:real` `(f(x + h) - f(x)) / h`])" (EXACT_CONV[X_BETA_CONV `h:real` `(f(x + h) - f(x)) / h`]) THEN
  CONV_TAC "Library/analysis.ml:(EXACT_CONV[X_BETA_CONV `h:real` `(g(x + h) - g(x)) / h`])" (EXACT_CONV[X_BETA_CONV `h:real` `(g(x + h) - g(x)) / h`]) THEN
...
let DIFF_MUL = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [REAL_ADD_SYM] THEN
  CONV_TAC "Library/analysis.ml:(EXACT_CONV(map (X_BETA_CONV `h:real`) [`((g(x + h) - g(x)) / h) * f(x + h)`; `((f(x + h) - f(x)) / h) * g(x)`]))" (EXACT_CONV(map (X_BETA_CONV `h:real`) [`((g(x + h) - g(x)) / h) * f(x + h)`; `((f(x + h) - f(x)) / h) * g(x)`])) THEN
  MATCH_MP_TAC LIM_ADD THEN
  CONV_TAC "Library/analysis.ml:(EXACT_CONV(map (X_BETA_CONV `h:real`) [`(g(x + h) - g(x)) / h`; `f(x + h):real`; `(f(x + h) - f(x)) / h`; `g(x:real):real`]))" (EXACT_CONV(map (X_BETA_CONV `h:real`) [`(g(x + h) - g(x)) / h`; `f(x + h):real`; `(f(x + h) - f(x)) / h`; `g(x:real):real`])) THEN
...
let DIFF_CMUL = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [REAL_MUL_SYM] THEN
  REWRITE_TAC[]);;
...
let DIFF_POW = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:RAND_CONV" RAND_CONV [REAL_ADD_SYM] THEN BINOP_TAC THENL
...
let DIFF_XM1 = prove(
  ...
  BETA_TAC THEN CONJ_TAC THENL
   [REWRITE_TAC[LIM] THEN X_GEN_TAC `e:real` THEN DISCH_TAC THEN
    ...
    CONV_TAC "Library/analysis.ml:(EXACT_CONV[X_BETA_CONV `h:real` `inv(x + h) * inv(x)`])" (EXACT_CONV[X_BETA_CONV `h:real` `inv(x + h) * inv(x)`]) THEN
    REWRITE_TAC[GSYM LIM_NEG] THEN
    CONV_TAC "Library/analysis.ml:(EXACT_CONV(map (X_BETA_CONV `h:real`) [`inv(x + h)`; `inv(x)`]))" (EXACT_CONV(map (X_BETA_CONV `h:real`) [`inv(x + h)`; `inv(x)`]))
    THEN MATCH_MP_TAC LIM_MUL THEN BETA_TAC THEN
    REWRITE_TAC[LIM_CONST] THEN
    CONV_TAC "Library/analysis.ml:(EXACT_CONV[X_BETA_CONV `h:real` `x + h`])" (EXACT_CONV[X_BETA_CONV `h:real` `x + h`]) THEN
    MATCH_MP_TAC LIM_INV THEN ASM_REWRITE_TAC[] THEN
    GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [GSYM REAL_ADD_RID] THEN
    CONV_TAC "Library/analysis.ml:(EXACT_CONV(map (X_BETA_CONV `h:real`) [`x:real`; `h:real`]))" (EXACT_CONV(map (X_BETA_CONV `h:real`) [`x:real`; `h:real`])) THEN
...
let DIFF_SUM = prove(
  ...
  CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN MATCH_MP_TAC DIFF_ADD THEN
...
let CONT_BOUNDED = prove(
  ...
  X_GEN_TAC `x:real` THEN ASM_CASES_TAC `a <= x /\ x <= b` THENL
   [ALL_TAC;
    ...
    MAP_EVERY X_GEN_TAC [`u:real`; `v:real`] THEN
    REPEAT STRIP_TAC THEN UNDISCH_TAC `~(a <= x /\ x <= b)` THEN
    CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THEN
    REWRITE_TAC[] THEN CONJ_TAC THEN MATCH_MP_TAC REAL_LE_TRANS THENL
     [EXISTS_TAC `u:real`; EXISTS_TAC `v:real`] THEN
    ASM_REWRITE_TAC[]] THEN
  UNDISCH_TAC `!x. a <= x /\ x <= b ==> f contl x` THEN
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [REAL_ADD_SYM] THEN
...
let CONT_ATTAINS = prove(
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:I" I [TAUT `a <=> ~ ~a`] THEN
  CONV_TAC "Library/analysis.ml:(RAND_CONV NOT_EXISTS_CONV)" (RAND_CONV NOT_EXISTS_CONV) THEN
  ...
  ASSUME_TAC THENL
   [GEN_TAC THEN DISCH_TAC THEN
    CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN
...
let CONT_ATTAINS2 = prove(
  ...
  BETA_TAC THEN DISCH_TAC THEN EXISTS_TAC `--M` THEN CONJ_TAC THENL
   [GEN_TAC THEN GEN_REWRITE_TAC "Library/analysis.ml:RAND_CONV" RAND_CONV [GSYM REAL_LE_NEG] THEN
...
let DIFF_LDEC = prove(
  ...
  REWRITE_TAC[GSYM real_div] THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o RAND_CONV o funpow 3 LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV o funpow 3 LAND_CONV o RAND_CONV)
    [real_sub] THEN
  FIRST_ASSUM MATCH_MP_TAC THEN
  FIRST_ASSUM(ASSUME_TAC o MATCH_MP REAL_LT_IMP_LE o CONJUNCT1) THEN
  ...
  ASM_CASES_TAC `a < x1 /\ x1 < b` THENL
   [FIRST_ASSUM(X_CHOOSE_THEN `d:real` MP_TAC o MATCH_MP INTERVAL_LEMMA) THEN
    ...
    ASM_REWRITE_TAC[] THEN SUBGOAL_THEN
     ...
     [CONV_TAC "Library/analysis.ml:EXISTS_AND_CONV" EXISTS_AND_CONV THEN CONJ_TAC THENL
  ...
  SUBGOAL_THEN `!x. a <= x /\ x <= b ==> (f(x):real = f(b))` MP_TAC THENL
   [REPEAT(FIRST_ASSUM(UNDISCH_TAC o check is_neg o concl)) THEN
    ...
    SUBGOAL_THEN `?l. (f diffl l)(x) /\
     ...
     [CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV EXISTS_AND_CONV)" (ONCE_DEPTH_CONV EXISTS_AND_CONV) THEN CONJ_TAC THENL
       [REWRITE_TAC[GSYM differentiable] THEN FIRST_ASSUM MATCH_MP_TAC THEN
        ASM_REWRITE_TAC[];
        EXISTS_TAC `d:real` THEN ASM_REWRITE_TAC[] THEN GEN_TAC THEN
        DISCH_THEN(fun th -> FIRST_ASSUM(MP_TAC o C MATCH_MP th)) THEN
...
let MVT_LEMMA = prove(
  ...
  ASM_REWRITE_TAC[] THEN
  DISCH_THEN(fun th -> GEN_REWRITE_TAC "Library/analysis.ml:I" I [GSYM th]) THEN
  REWRITE_TAC[REAL_SUB_RDISTRIB; GSYM REAL_MUL_ASSOC] THEN
  FIRST_ASSUM(fun th -> REWRITE_TAC[MATCH_MP REAL_DIV_RMUL th]) THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [REAL_MUL_SYM] THEN
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [REAL_MUL_SYM] THEN
...
let MVT = prove(
  ...
  funpow 2 (fst o dest_imp) o snd) THENL
   [ASM_REWRITE_TAC[MVT_LEMMA] THEN BETA_TAC THEN
    CONJ_TAC THEN X_GEN_TAC `x:real` THENL
     [DISCH_TAC THEN CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN
      MATCH_MP_TAC CONT_SUB THEN CONJ_TAC THENL
       [CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
        FIRST_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[];
        CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN MATCH_MP_TAC CONT_MUL THEN
      ...
      CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN MATCH_MP_TAC DIFF_SUB THEN
      CONJ_TAC THENL
       [CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN FIRST_ASSUM ACCEPT_TAC;
        CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN REWRITE_TAC[] THEN
        GEN_REWRITE_TAC "Library/analysis.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_RID] THEN
  ...
  ASM_REWRITE_TAC[] THEN DISCH_THEN((then_) CONJ_TAC o MP_TAC) THENL
   [ALL_TAC; DISCH_THEN(K ALL_TAC) THEN CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN
  ...
  REWRITE_TAC[REAL_SUB_ADD] THEN CONV_TAC "Library/analysis.ml:(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
  REWRITE_TAC[REAL_ADD_LID]);;
...
let DIFF_ISCONST_ALL = prove(
  ...
  REPEAT GEN_TAC THEN REPEAT_TCL DISJ_CASES_THEN MP_TAC
   ...
   [DISCH_THEN SUBST1_TAC THEN REFL_TAC;
    CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV);
    ALL_TAC] THEN
     ...
     [X_GEN_TAC `z:real` THEN STRIP_TAC THEN FIRST_ASSUM MATCH_MP_TAC THEN
      ...
      SUBGOAL_THEN `g(f(x - d):real) = x - d` SUBST1_TAC THENL
       [FIRST_ASSUM MATCH_MP_TAC THEN
        ...
        ASM_REWRITE_TAC[real_abs; REAL_LE_REFL]; ALL_TAC] THEN
      REWRITE_TAC[] THEN CONV_TAC "Library/analysis.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
      MATCH_MP_TAC REAL_LT_IMP_NE THEN
      MATCH_MP_TAC REAL_LTE_TRANS THEN EXISTS_TAC `x:real` THEN
...
let DIFF_INVERSE = prove(
  ...
  STRIP_ASSUME_TAC THENL
   [ALL_TAC;
    ...
    SUBGOAL_THEN `y - f(x) = h(g(y)) * (g(y) - x)` SUBST1_TAC THENL
     [FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Library/analysis.ml:RAND_CONV" RAND_CONV [GSYM th]) THEN
      REWRITE_TAC[ASSUME `f((g:real->real)(y)) = y`];
      UNDISCH_TAC `&0 < k` THEN
      MATCH_MP_TAC EQ_IMP THEN
      AP_THM_TAC THEN AP_TERM_TAC THEN
      CONV_TAC "Library/analysis.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[ABS_ZERO; REAL_SUB_0]] THEN
    ...
    SUBGOAL_THEN `inv((h(g(y))) * (g(y:real) - x)) =
     ...
     [MATCH_MP_TAC REAL_INV_MUL_WEAK THEN ASM_REWRITE_TAC[];
      ...
      REWRITE_TAC[REAL_MUL_ASSOC] THEN
      GEN_REWRITE_TAC "Library/analysis.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
      AP_THM_TAC THEN AP_TERM_TAC THEN
  ...
  DISCH_THEN(fun th -> FIRST_ASSUM(MP_TAC o C MATCH_MP th)) THEN
  REWRITE_TAC[GSYM ABS_NZ] THEN
  CONV_TAC "Library/analysis.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN ASM_REWRITE_TAC[] THEN
  DISCH_THEN SUBST1_TAC THEN
  REWRITE_TAC[REAL_SUB_LZERO; ABS_NEG; REAL_LT_REFL]);;
...
let IVT_DERIVATIVE_0 = prove
 (`!f f' a b.
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [REAL_LE_LT] THEN
...
let IVT_DERIVATIVE_POS = prove
 (`!f f' a b y.
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(funpow 2 LAND_CONV o BINDER_CONV o RAND_CONV o LAND_CONV o RAND_CONV)" (funpow 2 LAND_CONV o BINDER_CONV o RAND_CONV o LAND_CONV o RAND_CONV) [GSYM REAL_MUL_RID] THEN
...
let SER_COMPARA_UNIFORM = prove
 (`!s x0 g.
  ...
  GEN_REWRITE_TAC "Library/analysis.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
