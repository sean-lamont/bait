 ...
 (`!a u v x y.
  ...
  CONJ_TAC THENL
   [DISCH_THEN(X_CHOOSE_THEN `b:num` MP_TAC) THEN
    ...
    CONV_TAC "Examples/pell.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let X_CLAUSES = prove
 (`(!a. X a 0 = 1) /\
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [X_DEF] THEN
...
let Y_CLAUSES = prove
 (`(!a. Y a 0 = 0) /\
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [Y_DEF] THEN
  REWRITE_TAC[ARITH_EQ; ADD_EQ_0; ARITH_RULE `~(n + 2 = 1)`] THEN
  REWRITE_TAC[ARITH_RULE `((n + 2) - 2 = n) /\ ((n + 2) - 1 = n + 1)`]);;
...
let X_INCREASES = prove
 (`!a n. ~(a = 0) ==> X a n <= X a (n + 1)`,
  ...
  GEN_REWRITE_TAC "Examples/pell.ml:RAND_CONV" RAND_CONV [X_DEF] THEN
...
let Y_INCREASES = prove
 (`!a n. ~(a = 0) ==> Y a n <= Y a (n + 1)`,
  ...
  GEN_REWRITE_TAC "Examples/pell.ml:RAND_CONV" RAND_CONV [Y_DEF] THEN
  ASM_REWRITE_TAC[ADD_EQ_0; ARITH_EQ;
    ARITH_RULE `(n + 1 = 1) <=> (n = 0)`] THEN
  REWRITE_TAC[ADD_SUB] THEN
  MATCH_MP_TAC(ARITH_RULE `a + b <= c ==> a <= c - b:num`) THEN
 ...
 (`!a n. ~(a = 0) ==> (&(X a n) + &(Y a n) * sqrt(&a pow 2 - &1) =
  ...
  THENL
   [CONJ_TAC THEN MATCH_MP_TAC(GSYM REAL_OF_NUM_SUB) THEN
    ...
    GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [ARITH_RULE `n = 1 * n`] THEN
  ...
  SUBGOAL_THEN `n = 2 + (n - 2)`
    (fun th -> GEN_REWRITE_TAC "Examples/pell.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [th])
  THENL
 ...
 (`!a x y.
  ...
  SUBGOAL_THEN `(a EXP 2 - 1) * y <= a * x:num` ASSUME_TAC THENL
   [SUBGOAL_THEN `(a EXP 2 - 1) * y EXP 2 < a * x * y` MP_TAC THENL
    ...
    FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [SYM th]) THEN
    GEN_REWRITE_TAC "Examples/pell.ml:RAND_CONV" RAND_CONV [MULT_SYM] THEN
  ...
  CONJ_TAC THENL
   [GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [GSYM(SPEC `1` EXP_MONO_LT_SUC)] THEN
    ASM_REWRITE_TAC[ARITH_SUC] THEN
  ...
  MATCH_MP_TAC REAL_LT_LMUL THEN CONJ_TAC THENL
   [MATCH_MP_TAC REAL_LTE_ADD THEN CONJ_TAC THENL
     [REWRITE_TAC[REAL_OF_NUM_LT; LT_NZ] THEN DISCH_THEN SUBST_ALL_TAC THEN
      ...
      DISCH_THEN(MP_TAC o SYM) THEN CONV_TAC "Examples/pell.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
...
let ADDITION_FORMULA_NEG = prove
 (`!a m n.
  ...
  GEN_REWRITE_TAC "Examples/pell.ml:(LAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o LAND_CONV o ONCE_DEPTH_CONV)
...
let Y_INCREASES_LT = prove
 (`!a m n. ~(a = 0) /\ m < n ==> Y a m < Y a n`,
  ...
  GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [LT_EXISTS] THEN
...
let X_INCREASES_LE = prove
 (`!a m n. ~(a = 0) /\ m <= n ==> X a m <= X a n`,
  REPEAT GEN_TAC THEN DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
  GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [LE_EXISTS] THEN
...
let X_INCREASES_LT = prove
 (`!a m n. ~(a = 0) /\ ~(a = 1) /\ m < n ==> X a m < X a n`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "Examples/pell.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [GSYM LE_SUC_LT] THEN
  STRIP_TAC THEN
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    ...
    ASM_REWRITE_TAC[]] THEN
  GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [ARITH_RULE `a = 1 * a`] THEN
...
let XY_COPRIME = prove
 (`!a n. ~(a = 0) ==> coprime(X a n,Y a n)`,
  ...
  GEN_REWRITE_TAC "Examples/pell.ml:(funpow 2 RAND_CONV o LAND_CONV)" (funpow 2 RAND_CONV o LAND_CONV)
...
let Y_DIVIDES = prove
 (`!a m n. ~(a = 0) ==> ((Y a m) divides (Y a n) <=> m divides n)`,
  REPEAT STRIP_TAC THEN EQ_TAC THENL
   [ALL_TAC;
    GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [divides] THEN
  ...
  SUBGOAL_THEN `~((Y a m) divides (X a (q * m) * Y a r))` MP_TAC THENL
   [ALL_TAC;
    ...
    ASM_REWRITE_TAC[] THEN
    GEN_REWRITE_TAC "Examples/pell.ml:(funpow 3 RAND_CONV)" (funpow 3 RAND_CONV) [MULT_SYM] THEN
...
let BINOMIAL_TRIVIALITY = prove
 (`!x y d n. ?p q.
  ...
  GEN_TAC THEN GEN_TAC THEN GEN_TAC THEN INDUCT_TAC THENL
   [REPEAT(EXISTS_TAC `0`) THEN CONV_TAC "Examples/pell.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  GEN_REWRITE_TAC "Examples/pell.ml:(funpow 2 BINDER_CONV o LAND_CONV o TOP_DEPTH_CONV)" (funpow 2 BINDER_CONV o LAND_CONV o TOP_DEPTH_CONV)
  ...
  REWRITE_TAC[ARITH_RULE `SUC n + m = n + SUC m`] THEN
  CONV_TAC "Examples/pell.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[REAL_OF_NUM_ADD; REAL_OF_NUM_EQ] THEN
    CONV_TAC "Examples/pell.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC DIV_UNIQ THEN
    ...
    SUBGOAL_THEN `EVEN((n + 1) * (n + 2))` MP_TAC THENL
     [REWRITE_TAC[EVEN_MULT; EVEN_ADD; ARITH_EVEN] THEN
      CONV_TAC "Examples/pell.ml:(EQT_INTRO o TAUT)" (EQT_INTRO o TAUT); ALL_TAC] THEN
 ...
 (`!a m n. ~(a = 0)
  ...
  REPEAT STRIP_TAC THEN ASM_CASES_TAC `m = 0` THENL
   [ASM_REWRITE_TAC[Y_CLAUSES; MULT_CLAUSES; DIVIDES_ZERO; EXP_2] THEN
    GEN_REWRITE_TAC "Examples/pell.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM(CONJUNCT1 Y_CLAUSES)] THEN
    ASM_SIMP_TAC[Y_INJ]; ALL_TAC] THEN
  SUBGOAL_THEN `~(Y a m = 0)` ASSUME_TAC THENL
   [GEN_REWRITE_TAC "Examples/pell.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM(CONJUNCT1 Y_CLAUSES)] THEN
    ASM_SIMP_TAC[Y_INJ]; ALL_TAC] THEN
  MATCH_MP_TAC(TAUT
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[num_CONV `3`; EXP] THEN
    ...
    ALL_TAC] THEN
  REWRITE_TAC[MULT_ASSOC] THEN
  GEN_REWRITE_TAC "Examples/pell.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [MULT_SYM] THEN
 ...
 (`!a n. ~(a = 0) ==> ?q. Y a n = 2 * q + n`,
  ...
  REPEAT CONJ_TAC THENL
   [REWRITE_TAC[LEFT_ADD_DISTRIB] THEN
    ...
    GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [ARITH_RULE `n = 1 * n`] THENL
...
let X_CONGRUENT = prove
 (`!a b c n. ~(a = 0) ==> ?q. X (a + b * c) n = X a n + q * c`,
  ...
  ASM_REWRITE_TAC[] THEN MATCH_MP_TAC LE_ADD2 THEN CONJ_TAC THENL
   [GEN_REWRITE_TAC "Examples/pell.ml:RAND_CONV" RAND_CONV [MULT_SYM] THEN
    GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [ARITH_RULE `a = a * 1`] THEN
 ...
 (`!a m n. ~(a = 0) /\ m < n
  ...
  MATCH_MP_TAC EQ_IMP THEN BINOP_TAC THENL
   [EXPAND_TAC "n" THEN ASM_SIMP_TAC[ADDITION_FORMULA_POS] THEN
    GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [GCD_SYM] THEN MATCH_MP_TAC GCD_EQ THEN
    X_GEN_TAC `d:num` THEN CONV_TAC "Examples/pell.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC(TAUT
     `(a ==> (b <=> c)) ==> (a /\ b <=> a /\ c)`) THEN
    ...
    GEN_REWRITE_TAC "Examples/pell.ml:I" I [GSYM DIVIDES_ANTISYM] THEN
  ...
  ASM_CASES_TAC `i <= n:num` THENL
   [MP_TAC(SPECL [`a:num`; `n:num`; `j:num`] X_CONGRUENT_2NJ_NEG) THEN
    ...
    SUBGOAL_THEN `n = (n - 2) + 2`
     (fun th -> GEN_REWRITE_TAC "Examples/pell.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [th])
    THENL
     [UNDISCH_TAC `~(n = 0)` THEN UNDISCH_TAC `~(n = 1)` THEN ARITH_TAC;
      ALL_TAC] THEN
   ...
   [ALL_TAC;
    ...
    SUBGOAL_THEN `y <= v:num` ASSUME_TAC THENL
     [SUBST1_TAC(SYM(ASSUME `r * y EXP 2 = v`)) THEN REWRITE_TAC[EXP_2] THEN
      GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [ARITH_RULE `y = 1 * y`] THEN
      REWRITE_TAC[MULT_ASSOC; LE_MULT_RCANCEL] THEN DISJ1_TAC THEN
 ...
 (`!m n. m EXP n * (m - n) <= m * (m - 1) EXP n`,
  ...
  EXISTS_TAC `(m - 1) * m EXP n * (m - n)` THEN CONJ_TAC THENL
   [REWRITE_TAC[GSYM MULT_ASSOC] THEN
    ...
    GEN_REWRITE_TAC "Examples/pell.ml:RAND_CONV" RAND_CONV
...
let XY_EXP_LEMMA = prove
 (`!a k n.
  ...
  GEN_REWRITE_TAC "Examples/pell.ml:(RAND_CONV o LAND_CONV o RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV o LAND_CONV o RAND_CONV)
  ...
  EXISTS_TAC `&4 * (&x / &y - &z) pow 2 < &1` THEN CONJ_TAC THENL
   [SIMP_TAC[REAL_LT_RDIV_EQ; REAL_OF_NUM_LT; ARITH] THEN
    ...
    REWRITE_TAC[REAL_ABS_MUL; REAL_ABS_NUM] THEN EQ_TAC THENL
     [GEN_REWRITE_TAC "Examples/pell.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV)
       [SYM(REAL_RAT_REDUCE_CONV `&1 pow 2`)] THEN
      GEN_REWRITE_TAC "Examples/pell.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
      SIMP_TAC[ARITH_EQ; REAL_POW_LT2; REAL_LE_MUL; REAL_POS; REAL_ABS_POS];
      ONCE_REWRITE_TAC[TAUT `a ==> b <=> ~b ==> ~a`] THEN
      REWRITE_TAC[REAL_NOT_LT] THEN
      GEN_REWRITE_TAC "Examples/pell.ml:(RAND_CONV o LAND_CONV)" (RAND_CONV o LAND_CONV)
       [SYM(REAL_RAT_REDUCE_CONV `&1 pow 2`)] THEN
      GEN_REWRITE_TAC "Examples/pell.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [REAL_MUL_SYM] THEN
      SIMP_TAC[ARITH_EQ; REAL_POW_LE2; REAL_LE_MUL; REAL_POS; REAL_ABS_POS]];
    ALL_TAC] THEN
  MATCH_MP_TAC EQ_TRANS THEN
  EXISTS_TAC `&4 * (&x - &y * &z) pow 2 < &y pow 2` THEN CONJ_TAC THENL
   [GEN_REWRITE_TAC "Examples/pell.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_POW2_ABS] THEN
    REWRITE_TAC[GSYM REAL_ABS_POW] THEN
    GEN_REWRITE_TAC "Examples/pell.ml:(RAND_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV) [GSYM REAL_MUL_LID] THEN
...
let XY_EXP = prove
 (`~(a = 0)
  ...
  GEN_REWRITE_TAC "Examples/pell.ml:(RAND_CONV o BINDER_CONV o LAND_CONV)" (RAND_CONV o BINDER_CONV o LAND_CONV)
 ...
 (`(Y a k = y) <=>
  ...
  ASM_CASES_TAC `a = 0` THENL
   [ASM_CASES_TAC `y = 0` THENL
    ...
    ASM_REWRITE_TAC[ARITH_EQ] THEN CONV_TAC "Examples/pell.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    REWRITE_TAC[MULT_CLAUSES; ADD_CLAUSES] THEN
  ...
  ASM_CASES_TAC `k = 0` THEN ASM_REWRITE_TAC[Y_CLAUSES] THENL
   [REWRITE_TAC[EQ_SYM_EQ] THEN CONV_TAC "Examples/pell.ml:(EQT_INTRO o TAUT)" (EQT_INTRO o TAUT); ALL_TAC] THEN
  ASM_CASES_TAC `a = 1` THEN ASM_REWRITE_TAC[Y_DEGENERATE] THENL
   [REWRITE_TAC[EQ_SYM_EQ]; ALL_TAC] THEN
  ASM_CASES_TAC `y = 0` THEN ASM_SIMP_TAC[Y_EQ_0] THEN
  GEN_REWRITE_TAC "Examples/pell.ml:LAND_CONV" LAND_CONV [EQ_SYM_EQ] THEN ASM_SIMP_TAC[Y_DIOPH]);;
