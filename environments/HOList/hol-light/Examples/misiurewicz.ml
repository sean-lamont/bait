...
let HAS_COMPLEX_DERIVATIVE_CEXP_COMPOSE = prove
 (`!f z. (f has_complex_derivative f') (at z)
  ...
  GEN_REWRITE_TAC "Examples/misiurewicz.ml:(RATOR_CONV o LAND_CONV)" (RATOR_CONV o LAND_CONV) [GSYM ETA_AX] THEN
...
let COMPLEX_DIFFERENTIABLE_ITER_CEXP = prove
 (`!n z. (ITER n cexp) complex_differentiable at z`,
  GEN_REWRITE_TAC "Examples/misiurewicz.ml:(funpow 2 BINDER_CONV o LAND_CONV)" (funpow 2 BINDER_CONV o LAND_CONV) [GSYM ETA_AX] THEN
...
let LEMMA_1 = prove
 (`!n z. 1 <= n
  ...
  GEN_REWRITE_TAC "Examples/misiurewicz.ml:(RAND_CONV o RAND_CONV o LAND_CONV)" (RAND_CONV o RAND_CONV o LAND_CONV) [GSYM ETA_AX] THEN
 ...
 (`!z. abs(Im z) <= pi / &3 ==> Re(cexp z) >= Re z + (&1 - log(&2))`,
  ...
  TRANS_TAC REAL_LE_TRANS `exp(Re z) / &2` THEN CONJ_TAC THENL
   [ALL_TAC;
    ...
    REWRITE_TAC[COS_PI3] THEN CONV_TAC "Examples/misiurewicz.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV] THEN
 ...
 (`!z. ~(real z) ==> ?n. abs(Im(ITER n cexp z)) > pi / &3`,
  ...
  (LABEL_TAC "*") THENL
   [REPEAT GEN_TAC THEN
    ...
    REWRITE_TAC[VSUM_SING_NUMSEG] THEN CONV_TAC "Examples/misiurewicz.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    ...
    CONJ_TAC THENL [ALL_TAC; CONV_TAC "Examples/misiurewicz.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV] THEN
  ...
  SUBGOAL_THEN `!n. &0 < abs(Im(ITER n cexp z))` ASSUME_TAC THENL
   [REWRITE_TAC[GSYM REAL_ABS_NZ] THEN
    ...
    CONV_TAC "Examples/misiurewicz.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
    ALL_TAC] THEN
  SUBGOAL_THEN
   `?N. !n. N <= n
  ...
  MP_TAC(SPECL [`&2`; `pi / &3 / abs(Im (ITER N cexp z))`]
        REAL_ARCH_POW) THEN
  ASM_SIMP_TAC[REAL_LT_LDIV_EQ] THEN CONV_TAC "Examples/misiurewicz.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
 ...
 (`!n b r s.
  ...
  ASM_REWRITE_TAC[GSYM OPEN_CLOSED; IN_DIFF; IN_UNIV] THEN ANTS_TAC THENL
   [REWRITE_TAC[SET_RULE `UNIV DIFF s = {} <=> s = UNIV`] THEN
    ...
    ASM_SIMP_TAC[HOLOMORPHIC_ON_DIFFERENTIABLE;
                 ...
                 COMPLEX_DIFFERENTIABLE_AT_WITHIN];
    GEN_REWRITE_TAC "Examples/misiurewicz.ml:(LAND_CONV o RAND_CONV o BINDER_CONV)" (LAND_CONV o RAND_CONV o BINDER_CONV)
   ...
   [ALL_TAC;
    ...
    REWRITE_TAC[] THEN EXISTS_TAC `w:complex` THEN CONJ_TAC THENL
     [MATCH_MP_TAC(ISPEC `sequentially` LIM_UNIQUE) THEN
      ...
      DISJ1_TAC THEN GEN_REWRITE_TAC "Examples/misiurewicz.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
  ...
  REWRITE_TAC[CONVEX_BALL] THEN ANTS_TAC THENL
   [ALL_TAC;
    ...
    GEN_REWRITE_TAC "Examples/misiurewicz.ml:RAND_CONV" RAND_CONV [REAL_ARITH `t / s * r:real = r / s * t`] THEN
 ...
 (`!v. ~(v = {}) /\ open v /\ connected v /\
   ...
   [MATCH_MP_TAC FINITE_SUBSET THEN
    ...
    CONJ_TAC THENL
     [ASM_SIMP_TAC[FINITE_UNION; LEMMA_4] THEN
      GEN_REWRITE_TAC "Examples/misiurewicz.ml:(RAND_CONV o RAND_CONV o ABS_CONV o BINDER_CONV o RAND_CONV)" (RAND_CONV o RAND_CONV o ABS_CONV o BINDER_CONV o RAND_CONV)
  ...
  ASSUME_TAC THENL
   [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
      FINITE_SUBSET)) THEN
    GEN_REWRITE_TAC "Examples/misiurewicz.ml:I" I [SUBSET] THEN X_GEN_TAC `n:num` THEN
  ...
  MP_TAC THENL
   [SUBGOAL_THEN `?z. z IN IMAGE (ITER n cexp) v` STRIP_ASSUME_TAC THENL
    ...
    GEN_REWRITE_TAC "Examples/misiurewicz.ml:(RAND_CONV o RAND_CONV o LAND_CONV o LAND_CONV)" (RAND_CONV o RAND_CONV o LAND_CONV o LAND_CONV)
      [GSYM ETA_AX] THEN
 ...
 (`!v. ~(v = {}) /\ open v /\ connected v
  ...
  let lemma = prove
   (`!v. ~(v = {}) /\ open v /\ connected v /\
    ...
    GEN_REWRITE_TAC "Examples/misiurewicz.ml:I" I [SUBSET] THEN REWRITE_TAC[IN_INSERT; IN_ELIM_THM] THEN
 ...
 (`!v. open v /\ connected v /\ ~(v = {})
  ...
  MP_TAC THENL
   [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
    ...
    GEN_REWRITE_TAC "Examples/misiurewicz.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [EXISTS_IN_IMAGE] THEN
