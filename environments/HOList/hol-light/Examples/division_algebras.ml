 ...
 (`!m:real^N->real^N->real^N.
  ...
  REWRITE_TAC[NOT_IMP; NOT_EXISTS_THM] THEN CONJ_TAC THENL
   [MATCH_MP_TAC CONTINUOUS_ON_COMPOSE THEN CONJ_TAC THEN
    ...
    CONV_TAC "Examples/division_algebras.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN DISCH_THEN(MP_TAC o CONJUNCT2) THEN
 ...
 (`!m:real^N->real^N->real^N s.
  ...
  REPEAT CONJ_TAC THENL
   [X_GEN_TAC `k:real^N->bool` THEN STRIP_TAC THEN
    ...
    CONV_TAC "Examples/division_algebras.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ANTS_TAC THENL
    ...
    ASM_REWRITE_TAC[NORM_0] THEN CONV_TAC "Examples/division_algebras.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    STRIP_TAC THEN
    ...
    CONJ_TAC THENL
     [FIRST_X_ASSUM MATCH_MP_TAC THEN
      ...
      GEN_REWRITE_TAC "Examples/division_algebras.ml:(LAND_CONV o RAND_CONV o LAND_CONV)" (LAND_CONV o RAND_CONV o LAND_CONV)
     ...
     [EXPAND_TAC "f" THEN REWRITE_TAC[SUBSET; FORALL_IN_IMAGE] THEN
      ...
      X_GEN_TAC `x:real^N` THEN STRIP_TAC THEN CONJ_TAC THENL
       [MP_TAC(ISPECL [`m:real^N->real^N->real^N`; `\x:real^N. x`;
        ...
        MP_TAC THENL
         [ALL_TAC; ASM_REWRITE_TAC[] THEN CONV_TAC "Examples/division_algebras.ml:VECTOR_ARITH" VECTOR_ARITH] THEN
        FIRST_X_ASSUM MATCH_MP_TAC THEN
    ...
    CONV_TAC "Examples/division_algebras.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
 ...
 (`!m:real^N->real^N->real^N.
  ...
  SUBGOAL_THEN `dim(C:real^N->bool) = 2` ASSUME_TAC THENL
   [EXPAND_TAC "C" THEN REWRITE_TAC[DIM_INSERT; DIM_SPAN] THEN
    ...
    CONV_TAC "Examples/division_algebras.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV;
    ALL_TAC] THEN
  SUBGOAL_THEN `!x y:real^N. x IN C /\ y IN C ==> m x y IN C` ASSUME_TAC THENL
   [REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM] THEN EXPAND_TAC "C" THEN
    ...
    ASM_REWRITE_TAC[DIM_INSERT; DIM_SING] THEN CONV_TAC "Examples/division_algebras.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [SUBGOAL_THEN `(m:real^N->real^N->real^N) j j IN C` MP_TAC THENL
     [FIRST_X_ASSUM MATCH_MP_TAC THEN EXPAND_TAC "C" THEN
      ...
      FIRST_ASSUM(fun th -> GEN_REWRITE_TAC "Examples/division_algebras.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV)
        [SYM th])] THEN
    ...
    ASSUME_TAC THENL
     [EXPAND_TAC "k" THEN ASM_REWRITE_TAC[] THEN EXPAND_TAC "k" THEN
      ...
      CONV_TAC "Examples/division_algebras.ml:VECTOR_ARITH" VECTOR_ARITH;
      ALL_TAC] THEN
  ...
  ASSUME_TAC THENL
   [MATCH_MP_TAC(SET_RULE
    ...
    CONV_TAC "Examples/division_algebras.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM] THEN
    REWRITE_TAC[FORALL_IN_INSERT; NOT_IN_EMPTY] THEN
    ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [MAP_EVERY EXPAND_TAC ["C"; "D"] THEN
    ...
    SIMP_TAC[] THEN CONV_TAC "Examples/division_algebras.ml:VECTOR_ARITH" VECTOR_ARITH;
  ...
  SUBST1_TAC THENL
   [ONCE_REWRITE_TAC[GSYM DIM_SPAN; GSYM DIM_UNIV] THEN AP_TERM_TAC THEN
    ...
    CONJ_TAC THENL [ALL_TAC; CONV_TAC "Examples/division_algebras.ml:VECTOR_ARITH" VECTOR_ARITH] THEN
    ...
    ASM_REWRITE_TAC[VECTOR_NEG_MINUS1] THEN CONV_TAC "Examples/division_algebras.ml:VECTOR_ARITH" VECTOR_ARITH;
    ALL_TAC] THEN
  ...
  ASSUME_TAC THENL
   [REWRITE_TAC[GSYM INJECTIVE_ALT] THEN
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "Examples/division_algebras.ml:VECTOR_ARITH" VECTOR_ARITH;
    ALL_TAC] THEN
