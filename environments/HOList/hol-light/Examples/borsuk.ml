...
let THEOREM_1 = prove
 (`!f:real^3->real^2.
        ...
        ==> ?x. norm(x) = &1 /\ f(--x) = f(x)`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "Examples/borsuk.ml:I" I [TAUT `p <=> ~ ~ p`] THEN
  ...
  MP_TAC THENL
   [EXPAND_TAC "m" THEN
    ...
    THENL
     [CONJ_TAC THEN CONV_TAC "Examples/borsuk.ml:SYM_CONV" SYM_CONV THEN FIRST_X_ASSUM MATCH_MP_TAC THEN
    ...
    CONJ_TAC THENL
     [FIRST_X_ASSUM MATCH_MP_TAC THEN
      ...
      ASM_REWRITE_TAC[] THEN CONV_TAC "Examples/borsuk.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  UNDISCH_TAC `integer n` THEN GEN_REWRITE_TAC "Examples/borsuk.ml:I" I [GSYM CONTRAPOS_THM] THEN
