...
let WF_TC_EXPLICIT = prove
 (`!R:A->A->bool. WF(R) ==> WF(TC(R))`,
  ...
  GEN_TAC THEN GEN_REWRITE_TAC "Examples/reduct.ml:(LAND_CONV o REDEPTH_CONV)" (LAND_CONV o REDEPTH_CONV)
 ...
 (`!R. LF(R) /\ SN(R) ==> !x:A. FINITE {y | TC(R) x y}`,
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[EXTENSION; IN_UNION; IN_UNIONS] THEN
    ...
    GEN_TAC THEN GEN_REWRITE_TAC "Examples/reduct.ml:LAND_CONV" LAND_CONV [TC_CASES_R] THEN
  ...
  ASM_REWRITE_TAC[FINITE_UNION; FINITE_UNIONS] THEN CONJ_TAC THENL
   [MP_TAC(ISPECL [`\z:A. {y | TC R z y}`; `{z | (R:A->A->bool) x z}`]
    ...
    GEN_TAC THEN GEN_REWRITE_TAC "Examples/reduct.ml:LAND_CONV" LAND_CONV [IN_ELIM_THM] THEN
...
let FC_FINITE_BOUND_LEMMA = prove
 (`!R. (!z. ~(TC R z z))
  ...
  CONV_TAC "Examples/reduct.ml:CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_TAC THEN
