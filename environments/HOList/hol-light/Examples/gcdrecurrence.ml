...
let INDUCT_SPECIAL = prove
 (`!P. (!n. P 0 n) /\
  ...
  DISCH_THEN(X_CHOOSE_THEN `p:num` SUBST_ALL_TAC) THENL
   [ALL_TAC; ASM (GEN_REWRITE_TAC "Examples/gcdrecurrence.ml:I" I) []] THEN
...
let GCD_MERSENNE = prove
 (`!m n. gcd(2 EXP m - 1,2 EXP n - 1) = 2 EXP (gcd(m,n)) - 1`,
  ...
  CONV_TAC "Examples/gcdrecurrence.ml:INT_REDUCE_CONV" INT_REDUCE_CONV THEN REPEAT CONJ_TAC THENL
...
let FIB_EQ_0 = prove
 (`!n. fib n = 0 <=> n = 0`,
  ...
  CONV_TAC "Examples/gcdrecurrence.ml:NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[fib; ARITH_EQ]);;
 ...
 (`!m n. (fib m) divides (fib n) <=> m divides n \/ n = 0 \/ m = 2`,
  ...
  ASM_CASES_TAC `2 <= gcd(m,n)` THENL
   [MP_TAC(SPECL [`gcd(m:num,n)`; `m:num`] FIB_INCREASES_LT) THEN
    ...
    DISCH_TAC THEN CONV_TAC "Examples/gcdrecurrence.ml:(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN
