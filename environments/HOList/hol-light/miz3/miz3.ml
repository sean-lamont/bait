...
let horizon = ref 1;;
let timeout = ref 1;;
let default_prover = ref ("HOL_BY", CONV_TAC "miz3/miz3.ml:o" o HOL_BY);;
...
let rec by_item_of_toks toks =
  ...
  let i =
    ...
    try Tactic (s, use_thms (CONV_TAC "miz3/miz3.ml:(exec_conv s)" (exec_conv s))) with _ ->
    match toks with
    | [_,Ident s,_] -> Label s
...
let PURE_EXACTLY_ONCE_REWRITE_TAC =
  ...
  fun thl ->
    CONV_TAC "miz3/miz3.ml:(PURE_EXACTLY_ONCE_REWRITE_CONV thl)" (PURE_EXACTLY_ONCE_REWRITE_CONV thl);;
...
let rec tactic_of_step fake step (asl,w as g) =
  ...
  let SUBGOAL_THEN' tm tac =
    ...
    let lhs,_ = dest_eq (concl th) in
    SUBGOAL_THEN lhs tac THENL [MIZAR_NEXT' (CONV_TAC "miz3/miz3.ml:(K th)" (K th)); ALL_TAC] in
...
let reset_miz3 h =
  horizon := h;
  timeout := 1;
  default_prover := ("HOL_BY", CONV_TAC "miz3/miz3.ml:o" o HOL_BY);
  sketch_mode := false;
