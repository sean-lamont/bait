...
let BALL_SCALING = prove
 (`!c. &0 < c ==> !x r. ball(c % x,c * r) = IMAGE (\x. c % x) (ball(x,r))`,
  REPEAT STRIP_TAC THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SYM_CONV" SYM_CONV THEN
...
let SPHERE_EQ_EMPTY = prove
 (`!a:real^N r. sphere(a,r) = {} <=> r < &0`,
  ...
  REPEAT GEN_TAC THEN EQ_TAC THENL [ALL_TAC; CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH] THEN
...
let LIMPT_OF_UNIV = prove
 (`!x. x limit_point_of (:real^N)`,
  ...
  POP_ASSUM MP_TAC THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH);;
 ...
 (`!s x:real^N. x IN interior s ==> x limit_point_of s`,
  ...
  REPEAT CONJ_TAC THENL
   [FIRST_X_ASSUM MATCH_MP_TAC;
    CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN REWRITE_TAC[GSYM DIST_EQ_0];
...
let LIMPT_SEQUENTIAL_INJ = prove
 (`!x:real^N s.
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
  ...
  ASSUME_TAC THENL
   [INDUCT_TAC THEN ASM_REWRITE_TAC[] THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let UNIFORM_LIM_ADD = prove
 (`!net:(A)net P f g l m.
  ...
  CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH);;
...
let LIMPT_BALL = prove
 (`!x:real^N y e. y limit_point_of ball(x,e) <=> &0 < e /\ y IN cball(x,e)`,
  ...
  ASM_REWRITE_TAC[] THEN GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_LID] THEN
 ...
 (`!x:real^N e. interior(cball(x,e)) = ball(x,e)`,
  ...
  ASM_CASES_TAC `z:real^N = x` THENL
   [FIRST_X_ASSUM SUBST_ALL_TAC THEN
    ...
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [GSYM REAL_MUL_LID] THEN
...
let SPHERE_EQ_SING = prove
 (`!a:real^N r x. sphere(a,r) = {x} <=> x = a /\ r = &0`,
  ...
  REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH);;
...
let INTER_BALLS_EQ_EMPTY = prove
 (`(!a b:real^N r s. ball(a,r) INTER ball(b,s) = {} <=>
  ...
  (EQ_TAC THENL
    [ALL_TAC;
     SPEC_TAC(`b % basis 1:real^N`,`v:real^N`) THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH]) THEN
 ...
 (`!s:real^N->bool.
  ...
  MP_TAC THENL
   [SUBGOAL_THEN
    ...
    CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SELECT_CONV" SELECT_CONV THEN
...
let BOLZANO_WEIERSTRASS_IMP_BOUNDED = prove
 (`!s:real^N->bool.
  ...
  CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let LIMPT_OF_SEQUENCE_SUBSEQUENCE = prove
 (`!f:num->real^N l.
  ...
  REWRITE_TAC[LIM_SEQUENTIALLY] THEN
  X_GEN_TAC `e:real` THEN GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [REAL_ARCH_INV] THEN
...
let BOUNDED_CLOSED_NEST = prove
 (`!s. (!n. closed(s n)) /\ (!n. ~(s n = {})) /\
  ...
  REWRITE_TAC[LIM_SEQUENTIALLY; o_THM] THEN
  DISCH_THEN(X_CHOOSE_THEN `r:num->num` STRIP_ASSUME_TAC) THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:I" I [TAUT `p <=> ~(~p)`] THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [NOT_FORALL_THM] THEN
 ...
 (`!f:real^M->real^N x u.
  ...
  REPEAT GEN_TAC THEN REWRITE_TAC[continuous_within] THEN EQ_TAC THENL
   [DISCH_TAC THEN X_GEN_TAC `t:real^N->bool` THEN
    DISCH_THEN(CONJUNCTS_THEN2 MP_TAC ASSUME_TAC) THEN
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [open_def] THEN
...
let CONTINUOUS_OPEN_IN_PREIMAGE_EQ = prove
 (`!f:real^M->real^N s.
  ...
  X_GEN_TAC `t:real^N->bool` THEN GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [OPEN_IN_OPEN] THEN
...
let CONTINUOUS_CLOSED_IN_PREIMAGE_EQ = prove
 (`!f:real^M->real^N s.
  ...
  X_GEN_TAC `t:real^N->bool` THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [CLOSED_IN_CLOSED] THEN
  DISCH_THEN(X_CHOOSE_THEN `u:real^N->bool` STRIP_ASSUME_TAC) THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `u:real^N->bool`) THEN
  ASM_REWRITE_TAC[] THEN MATCH_MP_TAC EQ_IMP THEN AP_TERM_TAC THEN SET_TAC[]);;
...
let LINEAR_CONTINUOUS_AT = prove
 (`!f:real^M->real^N a. linear f ==> f continuous (at a)`,
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [LIM_AT_ZERO] THEN SIMP_TAC[]);;
...
let CLOSED_PCROSS_EQ = prove
 (`!s:real^M->bool t:real^N->bool.
  ...
  RULE_ASSUM_TAC(REWRITE_RULE[GSYM MEMBER_NOT_EMPTY]) THEN EQ_TAC THENL
   [GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV)
     [TAUT `p ==> q /\ r <=> (p ==> q) /\ (p ==> r)`; FORALL_AND_THM] THEN
    MATCH_MP_TAC MONO_AND THEN CONJ_TAC THENL
     [ALL_TAC; GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [SWAP_FORALL_THM]] THEN
...
let COMPLEMENT_CONNECTED_COMPONENT_UNIONS = prove
 (`!s x:real^N.
     ...
     UNIONS({connected_component s y | y | y IN s} DELETE
            (connected_component s x))`,
  REPEAT GEN_TAC THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV)
...
let CONTINUOUS_ON_COMPONENTS_GEN = prove
 (`!f:real^M->real^N s.
  ...
  SUBST1_TAC THENL
   [CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV(SUBS_CONV [ISPEC `s:real^M->bool` UNIONS_COMPONENTS]))" (LAND_CONV(SUBS_CONV [ISPEC `s:real^M->bool` UNIONS_COMPONENTS])) THEN
...
let COMPONENTS_UNIQUE = prove
 (`!s:real^N->bool k.
        ...
        ==> components s = k`,
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:I" I [EXTENSION] THEN
  ...
  EQ_TAC THENL
   [DISCH_THEN(X_CHOOSE_THEN `x:real^N`
    ...
    CONJ_TAC THENL [ASM SET_TAC[]; CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SYM_CONV" SYM_CONV] THEN
    FIRST_X_ASSUM MATCH_MP_TAC THEN
...
let COMPACT_UNIFORMLY_EQUICONTINUOUS = prove
 (`!(fs:(real^M->real^N)->bool) s.
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV) [RIGHT_IMP_EXISTS_THM] THEN
  REWRITE_TAC[SKOLEM_THM; LEFT_IMP_EXISTS_THM] THEN
  ...
  ASM_REWRITE_TAC[] THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH);;
...
let CONTINUOUS_UNIFORM_LIMIT = prove
 (`!net f:A->real^M->real^N g s.
  ...
  FIRST_X_ASSUM(fun th -> MP_TAC th THEN REWRITE_TAC[IMP_IMP] THEN
        GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [GSYM EVENTUALLY_AND]) THEN
...
let CLOSURE_SUMS = prove
 (`!s t:real^N->bool.
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [SUMS_SYM] THEN
...
let TRANSLATION_UNIV = prove
 (`!a. IMAGE (\x. a + x) (:real^N) = (:real^N)`,
  CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN GEOM_TRANSLATE_TAC[]);;
 ...
 (`!a b:real^N.
  ...
  EXISTS_TAC `\n. (x:real^N) + inv(&n + &1) % (c - x)` THEN CONJ_TAC THENL
   [X_GEN_TAC `n:num` THEN REWRITE_TAC[IN_DELETE] THEN
    ...
    ALL_TAC] THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [VECTOR_ARITH `x:real^N = x + &0 % (c - x)`] THEN
...
let FINITE_INTERVAL_1 = prove
 (`(!a b. FINITE(interval[a,b]) <=> drop b <= drop a) /\
  ...
  SUBST1_TAC THENL
   [CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
 ...
 (`!s:real^N->bool c. is_interval s ==> is_interval(IMAGE (\x. c % x) s)`,
  REPEAT GEN_TAC THEN ASM_CASES_TAC `c = &0` THENL
   [ASM_REWRITE_TAC[VECTOR_MUL_LZERO] THEN
    ...
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(BINOP_CONV o REDEPTH_CONV)" (BINOP_CONV o REDEPTH_CONV) [RIGHT_IMP_FORALL_THM] THEN
    ...
    ASM_REWRITE_TAC[VECTOR_MUL_COMPONENT; IN_IMAGE] THEN ANTS_TAC THENL
     [X_GEN_TAC `i:num` THEN STRIP_TAC THEN
      ...
      GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [GSYM REAL_LE_NEG2] THEN
...
let MIDPOINT_IN_SEGMENT = prove
 (`(!a b:real^N. midpoint(a,b) IN segment[a,b]) /\
  ...
  CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN VECTOR_ARITH_TAC);;
 ...
 (`!x a b:real^N. between x (a,b) <=> x IN segment[a,b]`,
  ...
  REWRITE_TAC[segment; IN_ELIM_THM] THEN EQ_TAC THENL
   [DISCH_THEN(ASSUME_TAC o SYM) THEN
    ...
    REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_FIELD" REAL_FIELD]);;
 ...
 (`!f g:real^M->real^N h s a.
  ...
  CONJ_TAC THENL
   [SUBGOAL_THEN
     ...
     (fun th -> GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [th])
    THENL
 ...
 (`!net f:A->real^N.
  ...
  REPEAT GEN_TAC THEN REWRITE_TAC[tendsto] THEN EQ_TAC THENL
   [DISCH_TAC THEN X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    ...
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [RIGHT_IMP_FORALL_THM] THEN
...
let CONTINUOUS_ON_COMPONENTWISE_LIFT = prove
 (`!f:real^M->real^N s.
  ...
  REPEAT GEN_TAC THEN
  REWRITE_TAC[CONTINUOUS_ON_EQ_CONTINUOUS_WITHIN] THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV)
 ...
 (`!s:real^M->bool a b:real^N.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC CONTINUOUS_ON_ADD THEN REWRITE_TAC[CONTINUOUS_ON_CONST] THEN
    ...
    CONJ_TAC THENL [CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH; REWRITE_TAC[IN_IMAGE]] THEN
...
let BOUNDED_DECREASING_CONVERGENT = prove
 (`!s:num->real^1.
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o BINDER_CONV)" (LAND_CONV o BINDER_CONV) [GSYM LIM_NEG_EQ] THEN
...
let HOMEOMORPHIC_SYM = prove
 (`!s t. s homeomorphic t <=> t homeomorphic s`,
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [SWAP_EXISTS_THM] THEN
  REPEAT(AP_TERM_TAC THEN ABS_TAC) THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:TAUT" TAUT);;
...
let HOMEOMORPHIC_PCROSS_ASSOC = prove
 (`!s:real^M->bool t:real^N->bool u:real^P->bool.
  ...
  TRY(GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [GSYM o_DEF] THEN
...
let INJECTIVE_MAP_OPEN_IFF_CLOSED = prove
 (`!f:real^M->real^N s t.
  ...
  CONJ_TAC THENL
   [CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC HOMEOMORPHISM_INJECTIVE_OPEN_MAP_EQ;
...
let [HOMEOMORPHIC_BALLS; HOMEOMORPHIC_CBALLS; HOMEOMORPHIC_SPHERES] =
 ...
 (`(!a:real^N b:real^N d e.
  ...
  ASM_SIMP_TAC[REAL_ABS_MUL; REAL_ABS_INV; REAL_ARITH
   `&0 < x ==> (abs x = x)`] THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(BINOP_CONV o BINDER_CONV o funpow 2 RAND_CONV)" (BINOP_CONV o BINDER_CONV o funpow 2 RAND_CONV)
 ...
 (`!a b c d.
  ...
  ASSUME_TAC THENL
   [REPEAT STRIP_TAC THEN REWRITE_TAC[HOMEOMORPHIC_MINIMAL] THEN
    ...
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [HOMEOMORPHIC_SYM] THEN
...
let HOMEOMORPHIC_OPEN_INTERVAL_UNIV_1 = prove
 (`!a b. drop a < drop b ==> interval(a,b) homeomorphic (:real^1)`,
  ...
  ASM_REWRITE_TAC[DROP_VEC; DROP_NEG] THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [X_GEN_TAC `x:real^1` THEN REWRITE_TAC[IN_INTERVAL_1] THEN
    ...
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [GSYM VECTOR_MUL_LID] THEN
 ...
 (`!a:real^N r. &0 < r ==> ball(a,r) homeomorphic (:real^N)`,
  ...
  REPEAT CONJ_TAC THENL
   [X_GEN_TAC `x:real^N` THEN DISCH_TAC THEN DISJ1_TAC THEN
    ...
    POP_ASSUM MP_TAC THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_FIELD" REAL_FIELD;
    ...
    REWRITE_TAC[GSYM REAL_INV_MUL; REAL_SUB_0; REAL_INV_EQ_1] THEN
    MP_TAC(ISPEC `y:real^N` NORM_POS_LE) THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_FIELD" REAL_FIELD;
    MATCH_MP_TAC CONTINUOUS_ON_MUL THEN REWRITE_TAC[CONTINUOUS_ON_ID] THEN
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM o_DEF] THEN
   ...
   [REPEAT STRIP_TAC THEN ASM_CASES_TAC `s:real^M->bool = {}` THEN
    ...
    REWRITE_TAC[GSYM MEMBER_NOT_EMPTY] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[SUBSET; IN_CBALL; IN_UNION; IN_DIFF; IN_BALL; IN_UNIV] THEN
      ...
      UNDISCH_TAC `&0 < e` THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH;
      EXISTS_TAC `(f:real^M->real^N) x` THEN
    ...
    REWRITE_TAC[NOT_IMP] THEN REPEAT CONJ_TAC THENL
     [EXPAND_TAC "s" THEN MATCH_MP_TAC CONTINUOUS_ON_CASES_LOCAL THEN
      ...
      CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let COMPLETE_ISOMETRIC_IMAGE = prove
 (`!f:real^M->real^N s e.
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o LAND_CONV)" (LAND_CONV o LAND_CONV) [GSYM FUN_EQ_THM] THEN
...
let OPEN_SURJECTIVE_LINEAR_IMAGE = prove
 (`!f:real^M->real^N.
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(RAND_CONV o LAND_CONV o RAND_CONV)" (RAND_CONV o LAND_CONV o RAND_CONV) [GSYM CARD_NUMSEG_1] THEN
  ...
  SUBST1_TAC THENL
   [ALL_TAC;
    SIMP_TAC[CLOSED_INTERS; CLOSED_HYPERPLANE; IN_ELIM_THM;
             LEFT_IMP_EXISTS_THM]] THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:I" I [EXTENSION] THEN REWRITE_TAC[IN_INTERS; IN_ELIM_THM] THEN
...
let DIM_CLOSURE = prove
 (`!s:real^N->bool. dim(closure s) = dim s`,
  GEN_TAC THEN REWRITE_TAC[GSYM LE_ANTISYM] THEN CONJ_TAC THENL
   [GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [GSYM DIM_SPAN]; ALL_TAC] THEN
 ...
 (`!b:real^N->bool.
  ...
  CONJ_TAC THENL
   [AP_TERM_TAC THEN MATCH_MP_TAC VSUM_EQ_NUMSEG THEN
    ...
    REWRITE_TAC[o_THM];
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [REAL_MUL_SYM] THEN
...
let REAL_AFFINITY_EQ = prove
 (`!m c x y. ~(m = &0) ==> (m * x + c = y <=> x = inv(m) * y + --(c / m))`,
  CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_FIELD" REAL_FIELD);;
 ...
 (`!f:real^N->real^N s.
  ...
  MP_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    REWRITE_TAC[DOT_SYM] THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_RING" REAL_RING;
...
let MATRIX_WLOG_INVERTIBLE = prove
 (`!P. (!A:real^N^N. invertible A ==> P A) /\
  ...
  ANTS_TAC THENL [ALL_TAC; CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:TAUT" TAUT] THEN
 ...
 (`!A:real^N^M B:real^M^N. det(mat 1 + A ** B) = det(mat 1 + B ** A)`,
     ...
     [REPEAT STRIP_TAC THEN
      ...
      MP_TAC THENL
       [REWRITE_TAC[MATRIX_ADD_RDISTRIB; MATRIX_ADD_LDISTRIB] THEN
        ...
        CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_RING" REAL_RING];
  ...
  let lemma2 = prove
   (`!A:real^N^M B:real^M^N.
    ...
    CONJ_TAC THENL
     [ALL_TAC;
      CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_SUPERSET THEN
    ...
    CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_EQ_GENERAL THEN
  ...
  THENL [ALL_TAC; CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SYM_CONV" SYM_CONV] THEN
 ...
 (`!A:real^N^N. det(cofactor A) = det(A) pow (dimindex(:N) - 1)`,
  ...
  X_GEN_TAC `A:real^N^N` THENL
   [REWRITE_TAC[INVERTIBLE_DET_NZ] THEN STRIP_TAC THEN
    ...
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM DET_TRANSP] THEN
...
let RANK_COFACTOR_EQ_FULL = prove
 (`!A:real^N^N. rank(cofactor A) = dimindex(:N) <=>
  ...
  CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:TAUT" TAUT);;
 ...
 (`!f:num->real^N k.
  ...
  EXISTS_TAC `\n. (f:num->real^N) k - f(n + 1)` THEN CONJ_TAC THENL
   [REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN EXISTS_TAC `k:num` THEN
    SIMP_TAC[];
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [GSYM VECTOR_SUB_RZERO] THEN
...
let SERIES_BOUND = prove
 (`!f:num->real^N g s a b.
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [GSYM SERIES_RESTRICT] THEN
...
let SUMMABLE_IFF_EVENTUALLY = prove
 (`!f g k. (?N. !n. N <= n /\ n IN k ==> f n = g n)
  ...
  REPEAT STRIP_TAC THENL [ALL_TAC; CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SYM_CONV" SYM_CONV] THEN
...
let SUMMABLE_IMP_TOZERO = prove
 (`!f:num->real^N k.
  ...
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [GSYM SUMMABLE_RESTRICT] THEN
  ...
  COND_CASES_TAC THEN ASM_REWRITE_TAC[NORM_0] THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH);;
...
let SERIES_RATIO = prove
 (`!c a s N.
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:I" I [SERIES_CAUCHY] THEN X_GEN_TAC `e:real` THEN
...
let SERIES_INJECTIVE_IMAGE_STRONG = prove
 (`!x:num->real^N s f.
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [o_DEF] THEN
...
let SERIES_INJECTIVE_IMAGE = prove
 (`!x:num->real^N s f l.
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SYM_CONV" SYM_CONV THEN REWRITE_TAC[sums] THEN
...
let CLOSEST_POINT_EXISTS = prove
 (`!s a. closed s /\ ~(s = {})
  ...
  REWRITE_TAC[closest_point] THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(ONCE_DEPTH_CONV SELECT_CONV)" (ONCE_DEPTH_CONV SELECT_CONV) THEN
...
let SETDIST_TRIANGLE = prove
 (`!s a t:real^N->bool.
  ...
  ASM_SIMP_TAC[SETDIST_LE_DIST] THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH);;
...
let SETDIST_CLOSURE = prove
 (`(!s t:real^N->bool. setdist(closure s,t) = setdist(s,t)) /\
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [SWAP_FORALL_THM] THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [SETDIST_SYM] THEN
  REWRITE_TAC[] THEN
 ...
 (`(!a b:real^N r s.
  ...
  REWRITE_TAC[GSYM REAL_LE_ANTISYM] THEN CONJ_TAC THENL
   [ALL_TAC;
    ...
    CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH] THEN
  MATCH_MP_TAC REAL_LE_TRANS THEN
...
let HAUSDIST_SYM = prove
 (`!s t:real^N->bool. hausdist(s,t) = hausdist(t,s)`,
  REPEAT GEN_TAC THEN REWRITE_TAC[hausdist; LET_DEF; LET_END_DEF] THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(RAND_CONV o ONCE_DEPTH_CONV)" (RAND_CONV o ONCE_DEPTH_CONV) [UNION_COMM] THEN
  REWRITE_TAC[]);;
...
let HAUSDIST_ALT = prove
 (`!s t:real^N->bool.
  ...
  UNDISCH_TAC `dist(y:real^N,x) <= b` THEN CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:NORM_ARITH" NORM_ARITH);;
...
let URYSOHN_LOCAL_STRONG = prove
 (`!s t u a b.
  ...
  REPEAT CONJ_TAC THENL
   [REPEAT GEN_TAC THEN
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(RAND_CONV o BINDER_CONV)" (RAND_CONV o BINDER_CONV) [SWAP_FORALL_THM] THEN
...
let CLOSURE_DYADIC_RATIONALS = prove
 (`closure { inv(&2 pow n) % x |n,x|
  ...
  CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN MATCH_MP_TAC MONO_EXISTS THEN
  ...
  MATCH_MP_TAC SUM_BOUND THEN REWRITE_TAC[FINITE_NUMSEG; IN_NUMSEG] THEN
  X_GEN_TAC `k:num` THEN STRIP_TAC THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:RAND_CONV" RAND_CONV [GSYM REAL_MUL_RID] THEN
   ...
   [ASM_REWRITE_TAC[INFINITE] THEN DISCH_TAC THEN
    ...
    MP_TAC THENL
     [REWRITE_TAC[SIMPLE_IMAGE] THEN MATCH_MP_TAC(REWRITE_RULE
      ...
      GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:I" I [SUBSET] THEN SIMP_TAC[FORALL_IN_IMAGE; IN_UNIV] THEN
...
let SUBSEQUENCE_DIAGONALIZATION_LEMMA = prove
 (`!P:num->(num->A)->bool.
  ...
  REPEAT GEN_TAC THEN
  DISCH_THEN(CONJUNCTS_THEN2 MP_TAC STRIP_ASSUME_TAC) THEN
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o TOP_DEPTH_CONV)" (LAND_CONV o TOP_DEPTH_CONV) [SKOLEM_THM] THEN
  ...
  ASSUME_TAC THENL
   [REPEAT GEN_TAC THEN GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [LE_EXISTS] THEN
...
let ARZELA_ASCOLI = prove
 (`!f:num->real^M->real^N s M.
  ...
  GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [SWAP_FORALL_THM] THEN
  DISCH_THEN(MP_TAC o SPEC `e / &3`) THEN
   ...
   [SUBGOAL_THEN
    ...
    ASSUME_TAC THENL
     [MAP_EVERY X_GEN_TAC [`u:real^N->bool`; `n:num`] THEN STRIP_TAC THEN
      ...
      GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [OPEN_IN_CONTAINS_BALL] THEN
      DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC (MP_TAC o SPEC `y:real^N`)) THEN
      ASM_REWRITE_TAC[IN_INTER] THEN
      DISCH_THEN(X_CHOOSE_THEN `d:real` STRIP_ASSUME_TAC) THEN
 ...
 (`!g s:real^N->bool.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC(SET_RULE
    ...
    FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:(LAND_CONV o RAND_CONV)" (LAND_CONV o RAND_CONV) [GSYM IMAGE_ID] THEN
 ...
 (`!s c. (!t:real^N->bool. t IN c ==> open t) /\ s SUBSET UNIONS c
  ...
  MP_TAC THENL
   [X_GEN_TAC `x:real^N` THEN DISCH_TAC THEN
    ...
    ASM_REWRITE_TAC[] THEN
    GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [OPEN_CONTAINS_CBALL] THEN
    DISCH_THEN(MP_TAC o SPEC `x:real^N`) THEN ASM_REWRITE_TAC[] THEN
    DISCH_THEN(X_CHOOSE_THEN `e:real` STRIP_ASSUME_TAC) THEN
   ...
   [X_GEN_TAC `n:num` THEN CONJ_TAC THENL
    ...
    SUBGOAL_THEN `?n. x IN (f((a:num->real^N) n):real^N->bool)` MP_TAC THENL
     [RULE_ASSUM_TAC(REWRITE_RULE[UNIONS_IMAGE; EXISTS_IN_IMAGE]) THEN
      ...
      GEN_REWRITE_TAC "RichterHilbertAxiomGeometry/from_topology.ml:LAND_CONV" LAND_CONV [num_WOP] THEN
 ...
 (`!c s. s SUBSET UNIONS c /\ (!u. u IN c ==> open u) /\
  ...
  ASM_SIMP_TAC[IN_INTER; OPEN_IN_OPEN_INTER] THEN CONJ_TAC THENL
   [X_GEN_TAC `y:real^N` THEN DISCH_TAC THEN AP_TERM_TAC THEN
    CONV_TAC "RichterHilbertAxiomGeometry/from_topology.ml:SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_EQ_SUPERSET THEN
