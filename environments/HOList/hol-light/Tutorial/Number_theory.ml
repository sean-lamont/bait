...
let FLT_LEMMA = prove
 (`!p a b. prime p ==> ((a + b) EXP p == a EXP p + b EXP p) (mod p)`,
  ...
  GEN_REWRITE_TAC "Tutorial/Number_theory.ml:LAND_CONV" LAND_CONV [ARITH_RULE `a + b = (b + 0) + a`] THEN
...
let FERMAT_LITTLE_VARIANT = prove
 (`!p a. prime p ==> (a EXP (1 + m * (p - 1)) == a) (mod p)`,
  ...
  THENL [ASM_MESON_TAC[CONG_TRIVIAL; ADD_AC; ADD1; DIVIDES_REXP_SUC]; 
         ALL_TAC] THEN
  GEN_REWRITE_TAC "Tutorial/Number_theory.ml:LAND_CONV" LAND_CONV [ARITH_RULE `a = a * 1`] THEN
...
let RSA = prove
 (`prime p /\ prime q /\ ~(p = q) /\
  ...
  GEN_REWRITE_TAC "Tutorial/Number_theory.ml:(LAND_CONV o ONCE_DEPTH_CONV)" (LAND_CONV o ONCE_DEPTH_CONV) [MULT_SYM] THEN
