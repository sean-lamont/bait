...
let SIN_N_CLAUSES = prove
 (`(sin(&(NUMERAL(BIT0 n)) * x) =
  ...
  CONV_TAC "Tutorial/Linking_external_tools.ml:REAL_RING" REAL_RING);;
...
let TRIG_IDENT_TAC x =
  ...
  MP_TAC(SPEC x SIN_CIRCLE) THEN CONV_TAC "Tutorial/Linking_external_tools.ml:REAL_RING" REAL_RING;;
