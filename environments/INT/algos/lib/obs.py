...
from environments.INT.algos.lib.ops import one_hot
from environments.INT.data_generation.seq_prefairseq import filter_arrow
from environments.INT.proof_system.all_axioms import all_axioms_to_prove
from environments.INT.proof_system.graph_seq_conversion import Parser
from environments.INT.proof_system.logic_functions import necessary_logic_functions
from environments.INT.proof_system.numerical_functions import necessary_numerical_functions
...
def obs_to_graphs_dgl(obs, debug=False):
    ...
    for gt in obs['objectives']:
        ...
        if cuda:
            ...
            node_op_onehot = one_hot(torch.LongTensor(node_op), len(nodename2index))
            local_poses_onehot = torch.cat((torch.zeros(1, 2), one_hot(torch.LongTensor(local_poses), 2)), 0)
            ivs_onehot = one_hot(torch.LongTensor(ivs), 2)
            node_input = torch.cat((node_op_onehot, local_poses_onehot, ivs_onehot), 1)
        g = dgl.DGLGraph()
        g_rev = dgl.DGLGraph()
...
def tile_obs_acs(observations, actions, sl_train):
    ...
    This function first groups all nodes in objectives in all problems of observations
